...
import saashq
from saashq import _, bold, scrub
...
import erpnexus
from erpnexus.stock.doctype.bin.bin import update_qty as update_bin_qty
...
from erpnexus.stock.utils import (
...
from erpnexus.stock.valuation import FIFOValuation, LIFOValuation, round_off_if_near_zero


class NegativeStockError(saashq.ValidationError):
	pass


class SerialNoExistsInFutureTransaction(saashq.ValidationError):
	pass
...
def make_sl_entries(sl_entries, allow_negative_stock=False, via_landed_cost_voucher=False):
	...
	args:
	        ...
	        such cases certain validations need to be ignored (like negative
	                        stock)
	"""
	from erpnexus.controllers.stock_controller import future_sle_exists
	...
	if sl_entries:
		...
		for sle in sl_entries:
			...
			is_stock_item = saashq.get_cached_value("Item", args.get("item_code"), "is_stock_item")
			if is_stock_item:
				bin_name = get_or_make_bin(args.get("item_code"), args.get("warehouse"))
				args.reserved_stock = flt(saashq.db.get_value("Bin", bin_name, "reserved_stock"))
...
def validate_serial_no(sle):
	...
	for sn in get_serial_nos(sle.serial_no):
		...
		for row in get_stock_ledger_entries(args, ">"):
			voucher_type = saashq.bold(row.voucher_type)
			voucher_no = saashq.bold(get_link_to_form(row.voucher_type, row.voucher_no))
			vouchers.append(f"{voucher_type} {voucher_no}")

		if vouchers:
			serial_no = saashq.bold(sn)
			msg = (
			...
			saashq.throw(_(msg), title=_(title), exc=SerialNoExistsInFutureTransaction)


def validate_cancellation(args):
	if args[0].get("is_cancelled"):
		repost_entry = saashq.db.get_value(
		...
		if repost_entry:
			...
			if repost_entry.status == "Queued":
				doc = saashq.get_doc("Repost Item Valuation", repost_entry.name)
				doc.status = "Skipped"
...
def set_as_cancel(voucher_type, voucher_no):
	saashq.db.sql(
		...
		(now(), saashq.session.user, voucher_type, voucher_no),
	)


def make_entry(args, allow_negative_stock=False, via_landed_cost_voucher=False):
	args["doctype"] = "Stock Ledger Entry"
	sle = saashq.get_doc(args)
...
def get_reposting_data(file_path) -> dict:
	...
	attached_file = saashq.get_doc("File", file_name)
...
def validate_item_warehouse(args):
	for field in ["item_code", "warehouse", "posting_date", "posting_time"]:
		if args.get(field) in [None, ""]:
			validation_msg = f"The field {saashq.unscrub(field)} is required for the reposting"
			saashq.throw(_(validation_msg))
...
def update_args_in_repost_item_valuation(doc, index, args, distinct_item_warehouses, affected_transactions):
	if not doc.items_to_be_repost:
		...
		if doc.reposting_data_file:
			...
			# saashq.delete_doc("File", file_name, ignore_permissions=True, delete_permanently=True)
		...
		doc.db_set(
			{
				...
				"current_index": index,
				"affected_transactions": saashq.as_json(affected_transactions),
			}
		)
...
def create_json_gz_file(data, doc, file_name=None) -> str:
	encoded_content = saashq.safe_encode(saashq.as_json(data))
...
def get_items_to_be_repost(voucher_type=None, voucher_no=None, doc=None, reposting_data=None):
	...
	if not items_to_be_repost and voucher_type and voucher_no:
		items_to_be_repost = saashq.db.get_all(
			"Stock Ledger Entry",
...
def get_distinct_item_warehouse(args=None, doc=None, reposting_data=None):
	...
	if doc and doc.distinct_item_and_warehouse:
		...
		distinct_item_warehouses = {
			saashq.safe_eval(k): saashq._dict(v) for k, v in distinct_item_warehouses.items()
		}
	else:
		for i, d in enumerate(args):
			distinct_item_warehouses.setdefault(
				(d.item_code, d.warehouse), saashq._dict({"reposting_status": False, "sle": d, "args_idx": i})
			)
...
def get_affected_transactions(doc, reposting_data=None) -> set[tuple[str, str]]:
	...
	transactions = saashq.parse_json(doc.affected_transactions)
...
class update_entries_after:
	...
	def __init__(
		...
		self.use_moving_avg_for_batch = saashq.db.get_single_value(
		...
		self.args = saashq._dict(args)
		if self.args.sle_id:
			self.args["name"] = self.args.sle_id

		self.company = saashq.get_cached_value("Warehouse", self.args.warehouse, "company")
		self.set_precision()
		self.valuation_method = get_valuation_method(self.item_code)

		self.new_items_found = False
		self.distinct_item_warehouses = args.get("distinct_item_warehouses", saashq._dict())
	...
	def set_precision(self):
		self.flt_precision = cint(saashq.db.get_default("float_precision")) or 2
		self.currency_precision = get_field_precision(
			saashq.get_meta("Stock Ledger Entry").get_field("stock_value")
		)
	...
	def initialize_previous_data(self, args):
		...
		"""
		self.data.setdefault(args.warehouse, saashq._dict())
	...
	def get_future_entries_to_fix(self):
		...
		args = self.data[self.args.warehouse].previous_sle or saashq._dict(
	...
	def update_distinct_item_warehouses(self, dependant_sle):
		...
		val = saashq._dict({"sle": dependant_sle})
	...
	def process_sle(self, sle):
		...
		if sle.serial_and_batch_bundle and (not self.use_moving_avg_for_batch or sle.has_serial_no):
			...
			sle.batch_no
			and saashq.db.get_value("Batch", sle.batch_no, "use_batchwise_valuation", cache=True)
			and not self.args.get("sle_id")
		):
			# Only run in reposting
	...
	def get_serialized_values(self, sle):
		from erpnexus.stock.serial_batch_bundle import SerialNoValuation
	...
	def reset_actual_qty_for_stock_reco(self, sle):
		doc = saashq.get_cached_doc("Stock Reconciliation", sle.voucher_no)
		doc.recalculate_current_qty(sle.voucher_detail_no)

		if sle.actual_qty < 0:
			sle.actual_qty = (
				flt(saashq.db.get_value("Stock Reconciliation Item", sle.voucher_detail_no, "current_qty"))
				* -1
			)
		...
		if sle.serial_and_batch_bundle and saashq.get_cached_value("Item", sle.item_code, "has_serial_no"):
			self.update_serial_no_status(sle)

	def update_serial_no_status(self, sle):
		from erpnexus.stock.serial_batch_bundle import get_serial_nos
		...
		sn_table = saashq.qb.DocType("Serial No")

		query = (
			saashq.qb.update(sn_table)
	...
	def calculate_valuation_for_serial_batch_bundle(self, sle):
		if not saashq.db.exists("Serial and Batch Bundle", sle.serial_and_batch_bundle):
			return

		doc = saashq.get_cached_doc("Serial and Batch Bundle", sle.serial_and_batch_bundle)
	...
	def get_incoming_outgoing_rate_from_transaction(self, sle):
		...
		if sle.voucher_type == "Stock Entry":
			...
			rate = saashq.db.get_value("Stock Entry Detail", sle.voucher_detail_no, "valuation_rate")
		# Sales and Purchase Return
			...
			if saashq.get_cached_value(sle.voucher_type, sle.voucher_no, "is_return"):
				from erpnexus.controllers.sales_and_purchase_return import (
				...
				# check in item table
				item_code, incoming_rate = saashq.db.get_value(
	...
	def update_rate_on_stock_entry(self, sle, outgoing_rate):
		saashq.db.set_value("Stock Entry Detail", sle.voucher_detail_no, "basic_rate", outgoing_rate)
	...
	def recalculate_amounts_in_stock_entry(self, voucher_no):
		stock_entry = saashq.get_doc("Stock Entry", voucher_no, for_update=True)
	...
	def update_rate_on_delivery_and_sales_return(self, sle, outgoing_rate):
		...
		item_code = saashq.db.get_value(sle.voucher_type + " Item", sle.voucher_detail_no, "item_code")
		if item_code == sle.item_code:
			saashq.db.set_value(
	...
	def update_rate_on_purchase_receipt(self, sle, outgoing_rate):
		if saashq.db.exists(sle.voucher_type + " Item", sle.voucher_detail_no):
			if sle.voucher_type in ["Purchase Receipt", "Purchase Invoice"] and saashq.get_cached_value(
		...
		if saashq.get_cached_value(sle.voucher_type, sle.voucher_no, "is_subcontracted"):
			doc = saashq.get_doc(sle.voucher_type, sle.voucher_no)
			doc.update_valuation_rate(reset_outgoing_rate=False)
			for d in doc.items + doc.supplied_items:
				d.db_update()

	def update_rate_on_subcontracting_receipt(self, sle, outgoing_rate):
		if saashq.db.exists("Subcontracting Receipt Item", sle.voucher_detail_no):
			saashq.db.set_value("Subcontracting Receipt Item", sle.voucher_detail_no, "rate", outgoing_rate)
		else:
			saashq.db.set_value(
		...
		scr = saashq.get_doc("Subcontracting Receipt", sle.voucher_no, for_update=True)
	...
	def update_rate_on_stock_reconciliation(self, sle):
		if not sle.serial_no and not sle.batch_no:
			sr = saashq.get_doc("Stock Reconciliation", sle.voucher_no, for_update=True)
	...
	def update_batched_values(self, sle):
		from erpnexus.stock.serial_batch_bundle import BatchNoValuation
		...
		if actual_qty > 0:
			...
			new_sle.qty = new_sle.actual_qty
			new_sle.batch_nos = saashq._dict({new_sle.batch_no: new_sle})
	...
	def check_if_allow_zero_valuation_rate(self, voucher_type, voucher_detail_no):
		...
		if ref_item_dt:
			return saashq.db.get_value(ref_item_dt, voucher_detail_no, "allow_zero_valuation_rate")
		else:
			return 0
	...
	def get_fallback_rate(self, sle) -> float:
		...
		return get_valuation_rate(
			...
			currency=erpnexus.get_company_currency(sle.company),
			company=sle.company,
		)
	...
	def get_sle_before_datetime(self, args):
		...
		sle = sle[0] if sle else saashq._dict()
		return sle
	...
	def raise_exceptions(self):
		...
		for warehouse, exceptions in self.exceptions.items():
			...
			) in saashq.local.flags.currently_saving:
				msg = _("{0} units of {1} needed in {2} to complete this transaction.").format(
					saashq.bold(abs(deficiency)),
					saashq.get_desk_link("Item", exceptions[0]["item_code"]),
					saashq.get_desk_link("Warehouse", warehouse),
				)
			else:
				msg = _(
					...
					saashq.get_desk_link(exceptions[0]["voucher_type"], exceptions[0]["voucher_no"]),
				)
				...
				if self.reserved_stock:
					...
					if allowed_qty > 0:
						msg = "{} As {} units are reserved for other sales orders, you are allowed to consume only {} units.".format(
							msg, saashq.bold(self.reserved_stock), saashq.bold(allowed_qty)
						)
					else:
		...
		if msg_list:
			message = "\n\n".join(msg_list)
			if self.verbose:
				saashq.throw(message, NegativeStockError, title=_("Insufficient Stock"))
			else:
	...
	def update_bin_data(self, sle):
		...
		saashq.db.set_value("Bin", bin_name, values_to_update)
	...
	def update_bin(self):
		...
		for warehouse, data in self.data.items():
			...
			saashq.db.set_value("Bin", bin_name, updated_values, update_modified=True)
...
def get_previous_sle_of_current_voucher(args, operator="<", exclude_current_voucher=False):
	...
	return sle[0] if sle else saashq._dict()
...
def get_stock_ledger_entries(
	...
	if check_serial_no and previous_sle.get("serial_no"):
		# conditions += " and serial_no like {}".format(saashq.db.escape('%{0}%'.format(previous_sle.get("serial_no"))))
		...
		"""
			...
			saashq.db.escape(serial_no),
			saashq.db.escape(f"{serial_no}\n%"),
			saashq.db.escape(f"%\n{serial_no}"),
			saashq.db.escape(f"%\n{serial_no}\n%"),
		)
...
def get_valuation_rate(
	...
	if warehouse and batch_no and saashq.db.get_value("Batch", batch_no, "use_batchwise_valuation"):
		table = saashq.qb.DocType("Stock Ledger Entry")
		query = (
	...
	if last_valuation_rate := saashq.db.sql(  # nosemgrep
	...
	valuation_rate = saashq.db.get_value("Item", item_code, "valuation_rate")

	if not valuation_rate:
		# try Item Standard rate
		valuation_rate = saashq.db.get_value("Item", item_code, "standard_rate")

		if not valuation_rate:
			# try in price list
			valuation_rate = saashq.db.get_value(
	...
	if (
		...
		and cint(erpnexus.is_perpetual_inventory_enabled(company))
	):
		...
		saashq.throw(msg=msg, title=_("Valuation Rate Missing"))
...
def validate_negative_qty_in_future_sle(args, allow_negative_stock=False):
	...
	if (
		...
		and saashq.db.get_value("Stock Reconciliation Item", args.voucher_detail_no, "qty") > 0
	):
		return
	...
	if is_negative_with_precision(neg_sle):
		message = _("{0} units of {1} needed in {2} on {3} {4} for {5} to complete this transaction.").format(
			...
			saashq.get_desk_link("Item", args.item_code),
			saashq.get_desk_link("Warehouse", args.warehouse),
			neg_sle[0]["posting_date"],
			neg_sle[0]["posting_time"],
			saashq.get_desk_link(neg_sle[0]["voucher_type"], neg_sle[0]["voucher_no"]),
		)
	...
	if args.batch_no:
		...
		if is_negative_with_precision(neg_batch_sle, is_batch=True):
			message = _(
				...
				saashq.get_desk_link("Batch", args.batch_no),
				saashq.get_desk_link("Warehouse", args.warehouse),
				neg_batch_sle[0]["posting_date"],
				neg_batch_sle[0]["posting_time"],
				saashq.get_desk_link(neg_batch_sle[0]["voucher_type"], neg_batch_sle[0]["voucher_no"]),
			)
			saashq.throw(message, NegativeStockError, title=_("Insufficient Stock for Batch"))
...
def is_negative_with_precision(neg_sle, is_batch=False):
	...
	precision = cint(saashq.db.get_default("float_precision")) or 2
...
def get_future_sle_with_negative_qty(sle_args):
	return saashq.db.sql(  # nosemgrep
		"""
		select
...
def validate_reserved_stock(kwargs):
	...
	if diff < 0 and abs(diff) > 0.0001:
		msg = _("{0} units of {1} needed in {2} on {3} {4} to complete this transaction.").format(
			abs(diff),
			saashq.get_desk_link("Item", kwargs.item_code),
			saashq.get_desk_link("Warehouse", kwargs.warehouse),
			nowdate(),
			nowtime(),
		)
		saashq.throw(msg, title=_("Reserved Stock"))
	...
	if reserved_serial_nos_details := get_sre_reserved_serial_nos_details(item_code, warehouse, serial_nos):
		if common_serial_nos := list(set(serial_nos).intersection(set(reserved_serial_nos_details.keys()))):
			...
			msg += _("Example: Serial No {0} reserved in {1}.").format(
				saashq.bold(common_serial_nos[0]),
				saashq.get_desk_link(
					"Stock Reservation Entry", reserved_serial_nos_details[common_serial_nos[0]]
				),
			)
			saashq.throw(msg, title=_("Reserved Serial No."))
	...
	if reserved_batches_map := get_sre_reserved_batch_nos_details(item_code, warehouse, batch_nos):
		...
		for batch_no in batch_nos:
			...
			if diff < 0 and abs(diff) > 0.0001:
				msg = _("{0} units of {1} needed in {2} on {3} {4} to complete this transaction.").format(
					abs(diff),
					saashq.get_desk_link("Batch", batch_no),
					saashq.get_desk_link("Warehouse", warehouse),
					nowdate(),
					nowtime(),
				)
				saashq.throw(msg, title=_("Reserved Stock for Batch"))


def is_negative_stock_allowed(*, item_code: str | None = None) -> bool:
	if cint(saashq.db.get_single_value("Stock Settings", "allow_negative_stock", cache=True)):
		return True
	if item_code and cint(saashq.db.get_value("Item", item_code, "allow_negative_stock", cache=True)):
		return True
	return False
...
def get_incoming_rate_for_inter_company_transfer(sle) -> float:
	...
	reference_name = saashq.get_cached_value(sle.voucher_type + " Item", sle.voucher_detail_no, field)

	if reference_name:
		rate = saashq.get_cached_value(
			doctype,
			reference_name,
			"incoming_rate",
		)
