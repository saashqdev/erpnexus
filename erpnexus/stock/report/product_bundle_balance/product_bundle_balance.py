...
def execute(filters=None):
	...
	for parent_item in parent_items:
		...
		for child_item in required_items:
			child_item_balance = stock_balance.get(child_item.item_code, saashq._dict())
		...
		for warehouse, company in warehouse_company_map.items():
			...
			for child_item_detail in required_items:
				child_item_balance = stock_balance.get(child_item_detail.item_code, saashq._dict()).get(
					warehouse, saashq._dict()
				)
				child_row = {
					"indent": 1,
...
def get_items(filters):
	pb_details = saashq._dict()
	item_details = saashq._dict()

	item = saashq.qb.DocType("Item")
	pb = saashq.qb.DocType("Product Bundle")
	...
	if parent_items:
		item = saashq.qb.DocType("Item")
		pb = saashq.qb.DocType("Product Bundle")
		pbi = saashq.qb.DocType("Product Bundle Item")
...
def get_stock_balance(filters, items):
	...
	stock_balance = saashq._dict()
	for d in sle:
		stock_balance.setdefault(d.item_code, saashq._dict())[d.warehouse] = d
...
def get_stock_ledger_entries(filters, items):
	...
	sle2 = saashq.qb.DocType("Stock Ledger Entry")
	...
	if date := filters.get("date"):
		...
		saashq.throw(_("'Date' is required"))
