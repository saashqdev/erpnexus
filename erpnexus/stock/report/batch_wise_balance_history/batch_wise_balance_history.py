...
from saashq.utils import add_to_date, cint, flt, get_datetime, get_table_name, getdate
...
def get_stock_ledger_entries_for_batch_no(filters):
	if not filters.get("from_date"):
		saashq.throw(_("'From Date' is required"))
	if not filters.get("to_date"):
		saashq.throw(_("'To Date' is required"))
...
def get_stock_ledger_entries_for_batch_bundle(filters):
	sle = saashq.qb.DocType("Stock Ledger Entry")
	batch_package = saashq.qb.DocType("Serial and Batch Entry")
...
def get_item_warehouse_batch_map(filters, float_precision):
	...
	for d in sle:
		iwb_map.setdefault(d.item_code, {}).setdefault(d.warehouse, {}).setdefault(
			d.batch_no, saashq._dict({"opening_qty": 0.0, "in_qty": 0.0, "out_qty": 0.0, "bal_qty": 0.0})
		)
...
def get_item_details(filters):
	item_map = {}
	for d in (saashq.qb.from_("Item").select("name", "item_name", "description", "stock_uom")).run(as_dict=1):
