...
saashq.query_reports["Batch-Wise Balance History"] = {
	filters: [
		{
			fieldname: "company",
			label: __("Company"),
		...
		},
		{
			...
			options: "Warehouse",
			get_query: function () {
				let warehouse_type = saashq.query_report.get_filter_value("warehouse_type");
	...
	formatter: function (value, row, column, data, default_formatter) {
		if (column.fieldname == "Batch" && data && !!data["Batch"]) {
			value = data["Batch"];
			column.link_onclick =
				"saashq.query_reports['Batch-Wise Balance History'].set_batch_route_to_stock_ledger(" +
				JSON.stringify(data) +
				")";
		}
