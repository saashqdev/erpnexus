...
from erpnexus.stock.doctype.stock_reconciliation.stock_reconciliation import get_stock_balance_for
...
def execute(filters=None):
	...
	precision = cint(saashq.db.get_single_value("System Settings", "float_precision"))
	bundle_details = {}
	...
	batch_balance_dict = saashq._dict({})
...
def get_serial_batch_bundle_details(sl_entries, filters=None):
	...
	_bundle_details = saashq._dict({})
	batch_entries = saashq.get_all(
...
def get_serial_and_batch_bundles(filters):
	SBB = saashq.qb.DocType("Serial and Batch Bundle")
	SBE = saashq.qb.DocType("Serial and Batch Entry")
...
def get_items(filters):
	...
	query = saashq.qb.from_(item).select(item.name)
	conditions = []
...
def get_item_details(items, sl_entries, include_uom):
	...
	if include_uom:
		ucd = saashq.qb.DocType("UOM Conversion Detail")
		query = (
			query.left_join(ucd)
...
def get_opening_balance_from_batch(filters, columns, sl_entries):
	...
	sabb_table = saashq.qb.DocType("Serial and Batch Entry")
	query = (
...
def get_opening_balance(filters, columns, sl_entries):
	...
	for sle in list(sl_entries):
		if (
			...
			and saashq.db.get_value("Stock Reconciliation", sle.voucher_no, "purpose") == "Opening Stock"
		):
			last_entry = sle
...
def get_warehouse_condition(warehouse):
	warehouse_details = saashq.db.get_value("Warehouse", warehouse, ["lft", "rgt"], as_dict=1)
...
def get_item_group_condition(item_group, item_table=None):
	item_group_details = saashq.db.get_value("Item Group", item_group, ["lft", "rgt"], as_dict=1)
	if item_group_details:
		if item_table:
			ig = saashq.qb.DocType("Item Group")
