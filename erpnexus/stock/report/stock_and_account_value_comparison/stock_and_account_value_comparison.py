...
import erpnexus
from erpnexus.accounts.utils import get_currency_precision, get_stock_accounts
from erpnexus.stock.doctype.warehouse.warehouse import get_warehouses_based_on_account


def execute(filters=None):
	if not erpnexus.is_perpetual_inventory_enabled(filters.company):
		saashq.throw(
...
def create_reposting_entries(rows, company):
	...
	if entries:
		...
		saashq.msgprint(_("Reposting entries created: {0}").format(entries))
