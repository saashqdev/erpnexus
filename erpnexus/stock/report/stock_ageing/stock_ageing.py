...
from saashq.utils import cint, date_diff, flt
...
def format_report_data(filters: Filters, item_details: dict, to_date: str) -> list[dict]:
	...
	precision = cint(saashq.db.get_single_value("System Settings", "float_precision", cache=True))
...
class FIFOSlots:
	...
	def generate(self) -> dict:
		...
		bundle_wise_serial_nos = saashq._dict({})
	...
	def __get_bundle_wise_serial_nos(self) -> dict:
		...
		entry = saashq.qb.DocType("Serial and Batch Entry")
	...
	def __get_item_query(self) -> str:
		...
		item = saashq.qb.from_("Item").select(
	...
	def __get_warehouse_conditions(self, sle, sle_query) -> str:
		...
		lft, rgt = saashq.db.get_value("Warehouse", self.filters.get("warehouse"), ["lft", "rgt"])
