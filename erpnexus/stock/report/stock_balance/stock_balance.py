...
from saashq.utils import add_days, cint, date_diff, flt, getdate
...
from erpnexus.stock.doctype.warehouse.warehouse import apply_warehouse_filter
from erpnexus.stock.report.stock_ageing.stock_ageing import FIFOSlots, get_average_age
from erpnexus.stock.utils import add_additional_uom_columns
...
class StockBalanceReport:
	...
	def set_company_currency(self) -> None:
		if self.filters.get("company"):
			self.company_currency = erpnexus.get_company_currency(self.filters.get("company"))
		else:
			self.company_currency = saashq.db.get_single_value("Global Defaults", "default_currency")

	def run(self):
		self.float_precision = cint(saashq.db.get_default("float_precision")) or 3
	...
	def prepare_opening_data_from_closing_balance(self) -> None:
		self.opening_data = saashq._dict({})
		...
		res = saashq.get_doc("Closing Stock Balance", closing_balance[0].name).get_prepared_data()

		for entry in res.data:
			entry = saashq._dict(entry)
	...
	def get_item_warehouse_map(self):
		...
		_system_settings = saashq.get_cached_doc("System Settings")
	...
	def initialize_data(self, item_warehouse_map, group_by_key, entry):
		...
		item_warehouse_map[group_by_key] = saashq._dict(
			{
	...
	def get_closing_balance(self) -> list[dict[str, Any]]:
		...
		table = saashq.qb.DocType("Closing Stock Balance")
	...
	def get_itemwise_conversion_factor(self):
		...
		table = saashq.qb.DocType("UOM Conversion Detail")
		query = (
			saashq.qb.from_(table)
			.select(
	...
	def get_opening_vouchers(self):
		...
		sr = saashq.qb.DocType("Stock Reconciliation")
...
def get_variants_attributes() -> list[str]:
	...
	return saashq.get_all("Item Attribute", pluck="name")
