...
from saashq.utils import add_days, cint, flt, nowdate
...
def reorder_item():
	...
	if not (saashq.db.a_row_exists("Company") and saashq.db.a_row_exists("Fiscal Year")):
		return

	if cint(saashq.db.get_single_value("Stock Settings", "auto_indent")):
		return _reorder_item()


def _reorder_item():
	material_requests = {"Purchase": {}, "Transfer": {}, "Material Issue": {}, "Manufacture": {}}
	warehouse_company = saashq._dict(
		saashq.db.sql(
	...
	default_company = (
		erpnexus.get_default_company() or saashq.db.sql("""select name from tabCompany limit 1""")[0][0]
	)
...
def get_items_for_reorder() -> dict[str, list]:
	reorder_table = saashq.qb.DocType("Item Reorder")
	item_table = saashq.qb.DocType("Item")

	query = (
		saashq.qb.from_(reorder_table)
...
def get_item_warehouse_projected_qty(items_to_consider):
	...
	for item_code, warehouse, projected_qty in saashq.db.sql(
		...
		while warehouse_doc.parent_warehouse:
			...
			warehouse_doc = saashq.get_doc("Warehouse", warehouse_doc.parent_warehouse)
...
def create_material_request(material_requests):
	...
	def _log_exception(mr):
		if saashq.local.message_log:
			exceptions_list.extend(saashq.local.message_log)
			saashq.local.message_log = []
		else:
			exceptions_list.append(saashq.get_traceback(with_context=True))

		mr.log_error("Unable to create material request")

	company_wise_mr = saashq._dict({})
		...
		for company in material_requests[request_type]:
			try:
				...
				for d in items:
					...
					must_be_whole_number = saashq.db.get_value("UOM", uom, "must_be_whole_number", cache=True)
	...
	if company_wise_mr:
		if getattr(saashq.local, "reorder_email_notify", None) is None:
			saashq.local.reorder_email_notify = cint(
				saashq.db.get_single_value("Stock Settings", "reorder_email_notify")
			)

		if saashq.local.reorder_email_notify:
...
def send_email_notification(company_wise_mr):
	...
	for company, mr_list in company_wise_mr.items():
		...
		msg = saashq.render_template("templates/emails/reorder_item.html", {"mr_list": mr_list})

		saashq.sendmail(recipients=email_list, subject=_("Auto Material Requests Generated"), message=msg)


def get_email_list(company):
	users = get_comapny_wise_users(company)
	user_table = saashq.qb.DocType("User")
	role_table = saashq.qb.DocType("Has Role")

	query = (
		saashq.qb.from_(user_table)
...
def get_comapny_wise_users(company):
	...
	if parent_company := saashq.db.get_value("Company", company, "parent_company"):
...
def notify_errors(exceptions_list):
	subject = _("[Important] [ERPNexus] Auto Reorder Errors")
	content = (
	...
	from saashq.email import sendmail_to_system_managers
