...
from saashq.query_builder.functions import CombineDatetime, IfNull, Sum
from saashq.utils import cstr, flt, get_link_to_form, get_time, getdate, nowdate, nowtime
...
from erpnexus.stock.serial_batch_bundle import BatchNoValuation, SerialNoValuation
from erpnexus.stock.valuation import FIFOValuation, LIFOValuation

BarcodeScanResult = dict[str, str | None]


class InvalidWarehouseCompany(saashq.ValidationError):
	pass


class PendingRepostingError(saashq.ValidationError):
	pass
...
def get_stock_value_from_bin(warehouse=None, item_code=None):
	...
	stock_value = saashq.db.sql(query, values)
...
def get_stock_value_on(
	...
	if warehouses:
		...
		for wh in list(warehouses):
			if saashq.db.get_value("Warehouse", wh, "is_group"):
...
def get_latest_stock_qty(item_code, warehouse=None):
	...
	if warehouse:
		lft, rgt, is_group = saashq.db.get_value("Warehouse", warehouse, ["lft", "rgt", "is_group"])
...
def get_bin(item_code, warehouse):
	bin = saashq.db.get_value("Bin", {"item_code": item_code, "warehouse": warehouse})
	if not bin:
		bin_obj = _create_bin(item_code, warehouse)
	else:
		bin_obj = saashq.get_doc("Bin", bin)
	bin_obj.flags.ignore_permissions = True
	return bin_obj


def get_or_make_bin(item_code: str, warehouse: str) -> str:
	bin_record = saashq.get_cached_value("Bin", {"item_code": item_code, "warehouse": warehouse})
...
def _create_bin(item_code, warehouse):
	...
	try:
		saashq.db.savepoint(bin_creation_savepoint)
		bin_obj = saashq.get_doc(doctype="Bin", item_code=item_code, warehouse=warehouse)
		bin_obj.flags.ignore_permissions = 1
		bin_obj.insert()
	except saashq.UniqueValidationError:
		saashq.db.rollback(save_point=bin_creation_savepoint)  # preserve transaction in postgres
		bin_obj = saashq.get_last_doc("Bin", {"item_code": item_code, "warehouse": warehouse})
...
def get_incoming_rate(args, raise_error_if_no_rate=True):
	...
	from erpnexus.stock.stock_ledger import get_previous_sle, get_valuation_rate
	...
	use_moving_avg_for_batch = saashq.db.get_single_value("Stock Settings", "do_not_use_batchwise_valuation")
	...
	if item_details and item_details.has_serial_no and args.get("serial_and_batch_bundle"):
		...
		args.actual_qty = args.qty
		args.batch_nos = saashq._dict({args.batch_no: args})
	...
	if in_rate is None:
		...
		in_rate = get_valuation_rate(
			...
			currency=erpnexus.get_company_currency(args.get("company")),
...
def get_valuation_method(item_code):
	...
	val_method = saashq.db.get_value("Item", item_code, "valuation_method", cache=True)
	if not val_method:
		val_method = saashq.db.get_single_value("Stock Settings", "valuation_method", cache=True) or "FIFO"
	return val_method
	...
	for val in serial_nos:
		if val:
			...
			if val in valid_serial_nos:
				saashq.throw(_("Serial number {0} entered more than once").format(val))
			else:
				valid_serial_nos.append(val)

	if qty and len(valid_serial_nos) != abs(qty):
		saashq.throw(_("{0} valid serial nos for Item {1}").format(abs(qty), item_code))

	return valid_serial_nos


def validate_warehouse_company(warehouse, company):
	warehouse_company = saashq.db.get_value("Warehouse", warehouse, "company", cache=True)
...
def is_group_warehouse(warehouse):
	if saashq.db.get_value("Warehouse", warehouse, "is_group", cache=True):
		saashq.throw(_("Group node warehouse is not allowed to select for transactions"))


def validate_disabled_warehouse(warehouse):
	if saashq.db.get_value("Warehouse", warehouse, "disabled", cache=True):
		saashq.throw(
		...
		if d.get("convertible"):
			...
			columns.insert(
				idx + 1,
				{
					...
					"fieldname": "{}_{}".format(d.get("fieldname"), saashq.scrub(include_uom)),
	...
	for row_idx, row in enumerate(result):
		...
		for key, value in data:
			...
			if not is_dict_obj:
				...
				new_key = f"{key}_{saashq.scrub(include_uom)}"
...
def add_additional_uom_columns(columns, result, include_uom, conversion_factors):
	...
	for col_idx in list(reversed(range(0, len(columns)))):
		...
		if isinstance(col, dict) and col.get("convertible") in ["rate", "qty"]:
			...
			convertible_column_map[col.get("fieldname")] = saashq._dict(
...
def is_reposting_item_valuation_in_progress():
	reposting_in_progress = saashq.db.exists(
...
def check_pending_reposting(posting_date: str, throw_error: bool = True) -> bool:
	...
	reposting_pending = saashq.db.exists("Repost Item Valuation", filters)
	if reposting_pending and throw_error:
		...
		saashq.msgprint(
			...
			primary_action={
				...
				"client_action": "erpnexus.route_to_pending_reposts",
				"args": filters,
			},
		)
...
def scan_barcode(search_value: str) -> BarcodeScanResult:
	def set_cache(data: BarcodeScanResult):
		saashq.cache().set_value(f"erpnexus:barcode_scan:{search_value}", data, expires_in_sec=120)

	def get_cache() -> BarcodeScanResult | None:
		if data := saashq.cache().get_value(f"erpnexus:barcode_scan:{search_value}"):
			return data
	...
	# search barcode no
	barcode_data = saashq.db.get_value(
		"Item Barcode",
	...
	# search serial no
	serial_no_data = saashq.db.get_value(
		"Serial No",
		search_value,
	...
	# search batch no
	batch_no_data = saashq.db.get_value(
		"Batch",
		search_value,
	...
	if batch_no_data:
		if saashq.get_cached_value("Item", batch_no_data.item_code, "has_serial_no"):
			saashq.throw(
				_(
...
def _update_item_info(scan_result: dict[str, str | None]) -> dict[str, str | None]:
	if item_code := scan_result.get("item_code"):
		if item_info := saashq.get_cached_value(
			"Item",
			item_code,
