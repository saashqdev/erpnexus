...
from saashq.core.doctype.prepared_report.prepared_report import create_json_gz_file
from saashq.desk.form.load import get_attachments
from saashq.model.document import Document
from saashq.utils import get_link_to_form, parse_json
...
class ClosingStockBalance(Document):
	...
	def validate_duplicate(self):
		...
		if query and query[0].name:
			...
			saashq.throw(_(msg), title=_("Duplicate Closing Stock Balance"))
	...
	def clear_attachment(self):
		if attachments := get_attachments(self.doctype, self.name):
			...
			saashq.delete_doc("File", attachment.name)
...
def prepare_closing_stock_balance(name):
	doc = saashq.get_doc("Closing Stock Balance", name)
