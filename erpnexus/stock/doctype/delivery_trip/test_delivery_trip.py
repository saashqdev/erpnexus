...
from saashq.utils import add_days, flt, now_datetime, nowdate

import erpnexus
from erpnexus.stock.doctype.delivery_trip.delivery_trip import (
...
class TestDeliveryTrip(IntegrationTestCase):
	...
	def tearDown(self):
		saashq.db.sql("delete from `tabDriver`")
		saashq.db.sql("delete from `tabVehicle`")
		saashq.db.sql("delete from `tabEmail Template`")
		saashq.db.sql("delete from `tabDelivery Trip`")
...
def create_address(driver):
	if not saashq.db.exists("Address", {"address_title": "_Test Address for Driver"}):
		...
		saashq.db.set_value("Driver", driver.name, "address", address.name)

		return address

	return saashq.get_doc("Address", {"address_title": "_Test Address for Driver"})


def create_driver():
	if not saashq.db.exists("Driver", {"full_name": "Newton Scmander"}):
		driver = saashq.get_doc(
			{
				"doctype": "Driver",
	...
	return saashq.get_doc("Driver", {"full_name": "Newton Scmander"})


def create_delivery_notification():
	if not saashq.db.exists("Email Template", "Delivery Notification"):
		dispatch_template = saashq.get_doc(
			{
	...
	delivery_settings = saashq.get_single("Delivery Settings")
	delivery_settings.dispatch_template = "Delivery Notification"
	delivery_settings.save()


def create_vehicle():
	if not saashq.db.exists("Vehicle", "JB 007"):
...
def create_delivery_trip(driver, address, contact=None):
	...
	delivery_trip = saashq.get_doc(
		{
			"doctype": "Delivery Trip",
			"company": erpnexus.get_default_company(),
