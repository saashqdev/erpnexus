...
import saashq
from saashq import ValidationError, _
from saashq.model.naming import make_autoname
from saashq.query_builder.functions import Coalesce
from saashq.utils import cint, cstr, getdate, nowdate, safe_json_loads
...
class SerialNo(StockController):
	...
	def validate_warehouse(self):
		if not self.get("__islocal"):
			item_code, warehouse = saashq.db.get_value("Serial No", self.name, ["item_code", "warehouse"])
			if not self.via_stock_ledger and item_code != self.item_code:
				saashq.throw(_("Item Code cannot be changed for Serial No."), SerialNoCannotCannotChangeError)
			if not self.via_stock_ledger and warehouse != self.warehouse:
				saashq.throw(_("Warehouse cannot be changed for Serial No."), SerialNoCannotCannotChangeError)
...
def get_new_serial_number(series):
	...
	if saashq.db.exists("Serial No", sr_no):
...
def update_maintenance_status():
	...
	for serial_no in serial_nos:
		doc = saashq.get_doc("Serial No", serial_no[0])
		doc.set_maintenance_status()
		saashq.db.set_value("Serial No", doc.name, "maintenance_status", doc.maintenance_status)


def get_delivery_note_serial_no(item_code, qty, delivery_note):
	serial_nos = ""
	dn_serial_nos = saashq.db.sql_list(
...
def auto_fetch_serial_number(
	...
	filters = saashq._dict({"item_code": item_code, "warehouse": warehouse})
...
def get_pos_reserved_serial_nos(filters):
	...
	POSInvoiceItem = saashq.qb.DocType("POS Invoice Item")
	query = (
		saashq.qb.from_(POSInvoice)
...
def fetch_serial_numbers(filters, qty, do_not_include=None):
	...
	serial_no = saashq.qb.DocType("Serial No")

	query = (
		saashq.qb.from_(serial_no)
