...
saashq.ui.form.on("Pick List", {
	setup: (frm) => {
		...
		frm.set_query("work_order", () => {
			return {
				query: "erpnexus.stock.doctype.pick_list.pick_list.get_pending_work_orders",
				filters: {
					company: frm.doc.company,
				},
			};
	...
	set_item_locations: (frm, save) => {
		if (!(frm.doc.locations && frm.doc.locations.length)) {
			saashq.msgprint(__("Add items in the Item Locations table"));
		} else {
			saashq.call({
	...
	refresh: (frm) => {
		...
		if (frm.doc.docstatus === 1) {
			saashq
				.xcall("erpnexus.stock.doctype.pick_list.pick_list.target_document_exists", {
				...
				saashq.prompt(
					...
					(data) => {
						...
						erpnexus.utils.map_current_doc({
							method: "erpnexus.manufacturing.doctype.work_order.work_order.create_pick_list",
							target: frm,
	...
	create_delivery_note: (frm) => {
		saashq.model.open_mapped_doc({
			method: "erpnexus.stock.doctype.pick_list.pick_list.create_delivery_note",
			frm: frm,
		});
	},
	create_stock_entry: (frm) => {
		saashq
			.xcall("erpnexus.stock.doctype.pick_list.pick_list.create_stock_entry", {
				pick_list: frm.doc,
			})
			.then((stock_entry) => {
				saashq.model.sync(stock_entry);
				saashq.set_route("Form", "Stock Entry", stock_entry.name);
			});
	},
	...
	scan_barcode: (frm) => {
		...
		const barcode_scanner = new erpnexus.utils.BarcodeScanner(opts);
...
saashq.ui.form.on("Pick List Item", {
	item_code: (frm, cdt, cdn) => {
		let row = saashq.get_doc(cdt, cdn);
		if (row.item_code) {
			get_item_details(row.item_code).then((data) => {
				saashq.model.set_value(cdt, cdn, "uom", data.stock_uom);
				saashq.model.set_value(cdt, cdn, "stock_uom", data.stock_uom);
				saashq.model.set_value(cdt, cdn, "conversion_factor", 1);
			});
		}
	},
	...
	uom: (frm, cdt, cdn) => {
		...
		if (row.uom) {
			get_item_details(row.item_code, row.uom).then((data) => {
				saashq.model.set_value(cdt, cdn, "conversion_factor", data.conversion_factor);
			});
		}
	},
	...
	qty: (frm, cdt, cdn) => {
		let row = saashq.get_doc(cdt, cdn);
		saashq.model.set_value(cdt, cdn, "stock_qty", row.qty * row.conversion_factor);
	},
...
function get_item_details(item_code, uom = null) {
	if (item_code) {
		return saashq.xcall("erpnexus.stock.doctype.pick_list.pick_list.get_item_details", {
			item_code,
			uom,
		});
	}
