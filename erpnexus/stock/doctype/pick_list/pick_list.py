...
from saashq.model.mapper import map_child_doc
from saashq.query_builder import Case
from saashq.query_builder.custom import GROUP_CONCAT
from saashq.query_builder.functions import Coalesce, Locate, Replace, Sum
from saashq.utils import ceil, cint, floor, flt, get_link_to_form
...
class PickList(Document):
	...
	if TYPE_CHECKING:
		...
		from erpnexus.stock.doctype.pick_list_item.pick_list_item import PickListItem
	...
	def onload(self) -> None:
		if saashq.get_cached_value("Stock Settings", None, "enable_stock_reservation"):
	...
	def check_serial_no_status(self):
		...
		for row in self.get("locations"):
			...
			validated_serial_nos = saashq.get_all(
				"Serial No",
	...
	def validate_sales_order_percentage(self):
		...
		for location in self.get("locations"):
			if (
				...
				and saashq.db.get_value("Sales Order", location.sales_order, "per_picked", cache=True) == 100
			):
				saashq.throw(
	...
	def validate_sales_order(self):
		...
		if so_list:
			for so in so_list:
				so_doc = saashq.get_doc("Sales Order", so)
	...
	def make_bundle_using_old_serial_batch_fields(self):
		...
		for row in self.locations:
			...
			if not row.use_serial_batch_fields and (row.serial_no or row.batch_no):
				saashq.throw(_("Please enable Use Old Serial / Batch Fields to make_bundle"))
				...
				sn_doc = SerialBatchCreation(
					{
						...
						"batches": saashq._dict({row.batch_no: row.stock_qty}) if row.batch_no else None,
	...
	def delink_serial_and_batch_bundle(self):
		for row in self.locations:
			if (
				...
				and saashq.db.get_value("Serial and Batch Bundle", row.serial_and_batch_bundle, "docstatus")
				== 1
			):
				saashq.db.set_value(
					"Serial and Batch Bundle",
	...
	def remove_serial_and_batch_bundle(self):
		for row in self.locations:
			if row.serial_and_batch_bundle:
				saashq.delete_doc("Serial and Batch Bundle", row.serial_and_batch_bundle)
	...
	def update_sales_order_picking_status(self) -> None:
		...
		for sales_order in sales_orders:
			saashq.get_doc("Sales Order", sales_order, for_update=True).update_picking_status()
	...
	def validate_picked_qty(self, data):
		over_delivery_receipt_allowance = 100 + flt(
			saashq.db.get_single_value("Stock Settings", "over_delivery_receipt_allowance")
		)
	...
	def set_item_locations(self, save=False):
		...
		self.item_location_map = saashq._dict()
		...
		updated_locations = saashq._dict()
	...
	def aggregate_item_qty(self):
		...
		for item in locations:
			if not item.item_code:
				saashq.throw(f"Row #{item.idx}: Item Code is Mandatory")
			if not cint(
				saashq.get_cached_value("Item", item.item_code, "is_stock_item")
			) and not saashq.db.exists("Product Bundle", {"new_item_code": item.item_code, "disabled": 0}):
				continue
	...
	def validate_for_qty(self):
		if self.purpose == "Material Transfer for Manufacture" and (
			...
			saashq.throw(_("Qty of Finished Goods Item should be greater than 0."))
	...
	def update_bundle_picked_qty(self):
		...
		for so_row, item_code in product_bundles.items():
			...
			already_picked = saashq.db.get_value(item_table, so_row, "picked_qty", for_update=True)
			saashq.db.set_value(
				item_table,
				so_row,
				"picked_qty",
	...
	def get_picked_items_details(self, items):
		...
		for item_data in items_data:
			...
			if key not in picked_items[item_data.item_code]:
				picked_items[item_data.item_code][key] = saashq._dict(
					{
						"picked_qty": 0,
						"serial_no": [],
	...
	def _get_pick_list_items(self, items):
		pi = saashq.qb.DocType("Pick List")
		pi_item = saashq.qb.DocType("Pick List Item")
		query = (
			saashq.qb.from_(pi)
			.inner_join(pi_item)
	...
	def _get_product_bundles(self) -> dict[str, str]:
		...
		for item in self.locations:
			if not item.product_bundle_item:
				continue
			product_bundles[item.product_bundle_item] = saashq.db.get_value(
				"Sales Order Item",
	...
	def _get_product_bundle_qty_map(self, bundles: list[str]) -> dict[str, dict[str, float]]:
		...
		for bundle_item_code in bundles:
			bundle = saashq.get_last_doc("Product Bundle", {"new_item_code": bundle_item_code, "disabled": 0})
	...
	def _compute_picked_qty_for_bundle(self, bundle_row, bundle_items) -> int:
		...
		precision = saashq.get_precision("Stock Ledger Entry", "qty_after_transaction")
...
def update_pick_list_status(pick_list):
	if pick_list:
		doc = saashq.get_doc("Pick List", pick_list)
...
def validate_item_locations(pick_list):
	if not pick_list.locations:
		saashq.throw(_("Add items in the Item Locations table"))
...
def get_items_with_location_and_quantity(item_doc, item_location_map, docstatus):
	...
	precision = saashq.get_precision("Pick List Item", "qty")
	while flt(remaining_stock_qty, precision) > 0 and available_locations:
		item_location = available_locations.pop(0)
		item_location = saashq._dict(item_location)

		stock_qty = remaining_stock_qty if item_location.qty >= remaining_stock_qty else item_location.qty
		qty = stock_qty / (item_doc.conversion_factor or 1)

		uom_must_be_whole_number = saashq.get_cached_value("UOM", item_doc.uom, "must_be_whole_number")
...
def get_available_item_locations(
	...
	has_serial_no = saashq.get_cached_value("Item", item_code, "has_serial_no")
	has_batch_no = saashq.get_cached_value("Item", item_code, "has_batch_no")
...
def get_available_item_locations_for_serial_and_batched_item(
	...
	if locations:
		sn = saashq.qb.DocType("Serial No")
...
def get_available_item_locations_for_serialized_item(
	...
	warehouse_serial_nos_map = saashq._dict()
		...
		saashq._dict(
			{
				...
				"based_on": saashq.db.get_single_value("Stock Settings", "pick_serial_and_batch_based_on"),
			}
		)
	)

	warehouse_wise_batches = saashq._dict()
...
def create_delivery_note(source_name, target_doc=None):
	pick_list = saashq.get_doc("Pick List", source_name)
	...
	saashq.msgprint(_("Delivery Note(s) created for the Pick List"))
	return delivery_note


def create_dn_wo_so(pick_list):
	delivery_note = saashq.new_doc("Delivery Note")
...
def map_pl_locations(pick_list, item_mapper, delivery_note, sales_order=None):
	for location in pick_list.locations:
		...
		if location.sales_order_item:
			sales_order_item = saashq.get_doc("Sales Order Item", location.sales_order_item)
		else:
	...
	delivery_note.customer = saashq.get_value("Sales Order", sales_order, "customer")
...
def add_product_bundles_to_delivery_note(pick_list: "PickList", delivery_note, item_mapper) -> None:
	...
	for so_row, item_code in product_bundles.items():
		sales_order_item = saashq.get_doc("Sales Order Item", so_row)
...
def create_stock_entry(pick_list):
	pick_list = saashq.get_doc(json.loads(pick_list))
	validate_item_locations(pick_list)

	if stock_entry_exists(pick_list.get("name")):
		return saashq.msgprint(_("Stock Entry has been already created against this Pick List"))
...
def target_document_exists(pick_list_name, purpose):
	if purpose == "Delivery":
		return saashq.db.exists("Delivery Note", {"pick_list": pick_list_name, "docstatus": 1})
...
def get_item_details(item_code, uom=None):
	details = saashq.db.get_value("Item", item_code, ["stock_uom", "name"], as_dict=1)
...
def update_delivery_note_item(source, target, delivery_note):
	cost_center = saashq.db.get_value("Project", delivery_note.project, "cost_center")
	if not cost_center:
...
def stock_entry_exists(pick_list_name):
	return saashq.db.exists("Stock Entry", {"pick_list": pick_list_name})


def update_stock_entry_based_on_work_order(pick_list, stock_entry):
	work_order = saashq.get_doc("Work Order", pick_list.get("work_order"))
	...
	is_wip_warehouse_group = saashq.db.get_value("Warehouse", work_order.wip_warehouse, "is_group")
...
def update_stock_entry_based_on_material_request(pick_list, stock_entry):
	for location in pick_list.locations:
		...
		if location.material_request_item:
			target_warehouse = saashq.get_value(
...
def get_rejected_warehouses():
	if not hasattr(saashq.local, "rejected_warehouses"):
		saashq.local.rejected_warehouses = []

	if not saashq.local.rejected_warehouses:
		saashq.local.rejected_warehouses = saashq.get_all(
			"Warehouse", filters={"is_rejected_warehouse": 1}, pluck="name"
		)

	return saashq.local.rejected_warehouses
