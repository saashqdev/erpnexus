...
from erpnexus.selling.doctype.sales_order.sales_order import create_pick_list
...
from erpnexus.stock.doctype.packed_item.test_packed_item import create_product_bundle
...
class TestPickList(IntegrationTestCase):
	...
	def test_pick_list_shows_serial_no_for_serialized_item(self):
		...
		stock_reconciliation = saashq.get_doc(
			{
				"doctype": "Stock Reconciliation",
	...
	def test_pick_list_shows_batch_no_for_batched_item(self):
		...
		item = saashq.db.exists("Item", {"item_name": "Batched Item"})
		if not item:
			...
			item = saashq.get_doc("Item", {"item_name": "Batched Item"})
				...
				"items": [
					{
						"item_code": item_code,
						"qty": 10,
						"delivery_date": saashq.utils.today(),
						"warehouse": "_Test Warehouse - _TC",
					}
				],
			}
	...
	def test_multiple_dn_creation(self):
		...
		for dn in saashq.get_all(
			"Delivery Note",
			filters={"pick_list": pick_list.name, "customer": "_Test Customer"},
			fields={"name"},
		):
			for dn_item in saashq.get_doc("Delivery Note", dn.name).get("items"):
		...
		for dn in saashq.get_all("Delivery Note", filters={"pick_list": pick_list_1.name}, fields={"name"}):
			for dn_item in saashq.get_doc("Delivery Note", dn.name).get("items"):
	...
	def test_picklist_for_batch_item(self):
		...
		make_stock_entry(
			...
			qty=50,
			basic_rate=100,
			batches=saashq._dict({"PICKLT-000001": 30, "PICKLT-000002": 20}),
		)
	...
	def test_validate_picked_qty_with_manual_option(self):
		...
		self.assertRaises(saashq.ValidationError, pl.save)
	...
	def test_over_allowance_picking(self):
		...
		self.assertRaises(saashq.ValidationError, pl_doc.submit)

		saashq.db.set_single_value("Stock Settings", "over_picking_allowance", 50)

		pl_doc.reload()
		pl_doc.submit()

		saashq.db.set_single_value("Stock Settings", "over_picking_allowance", 0)

	def test_ignore_pricing_rule_in_pick_list(self):
		saashq.flags.print_stmt = False
		warehouse = "_Test Warehouse - _TC"
		item = make_item(
			properties={
				"is_stock_item": 1,
