...
class OpeningEntryAccountError(saashq.ValidationError):
	pass


class EmptyStockReconciliationItemsError(saashq.ValidationError):
	pass
...
class StockReconciliation(StockController):
	...
	if TYPE_CHECKING:
		...
		from erpnexus.stock.doctype.stock_reconciliation_item.stock_reconciliation_item import (
	...
	def validate(self):
		...
		if not self.expense_account:
			self.expense_account = saashq.get_cached_value(
				"Company", self.company, "stock_adjustment_account"
			)
		if not self.cost_center:
			self.cost_center = saashq.get_cached_value("Company", self.company, "cost_center")
				...
				sn_doc = SerialBatchCreation(
					{
						...
						"batches": saashq._dict({row.batch_no: row.current_qty}) if row.batch_no else None,
	...
	def set_current_serial_and_batch_bundle(self, voucher_detail_no=None, save=False) -> None:
		...
		for item in self.items:
			if not saashq.db.exists("Item", item.item_code):
				saashq.throw(_("Item {0} does not exist").format(item.item_code))
			...
			if not item.current_serial_and_batch_bundle:
				serial_and_batch_bundle = saashq.get_doc(
					{
			...
			if save:
				sle_creation = saashq.db.get_value(
	...
	def has_change_in_serial_batch(self, row) -> bool:
		...
		if diff:
			...
			saashq.delete_doc("Serial and Batch Bundle", bundle)
	...
	def set_new_serial_and_batch_bundle(self):
		for item in self.items:
			...
			if item.current_serial_and_batch_bundle and not item.serial_and_batch_bundle:
				current_doc = saashq.get_doc("Serial and Batch Bundle", item.current_serial_and_batch_bundle)

				item.qty = abs(current_doc.total_qty)
				item.valuation_rate = abs(current_doc.avg_rate)

				bundle_doc = saashq.copy_doc(current_doc)
				...
				bundle_doc = saashq.get_doc("Serial and Batch Bundle", item.serial_and_batch_bundle)
	...
	def remove_items_with_no_change(self):
		...
		if not items:
			...
			for i, item in enumerate(self.items):
				item.idx = i + 1
			saashq.msgprint(_("Removed items with no change in quantity or value."))
	...
	def validate_data(self):
		...
		for row_num, row in enumerate(self.items):
			...
			# validate warehouse
			if not saashq.db.get_value("Warehouse", row.warehouse):
			...
			if row.qty and row.valuation_rate in ["", None]:
				...
				if not row.valuation_rate:
					...
					buying_rate = saashq.db.get_value(
						"Item Price",
					...
					if buying_rate:
						...
						row.valuation_rate = saashq.get_value("Item", row.item_code, "valuation_rate")
		...
		if self.validation_messages:
			...
			raise saashq.ValidationError(self.validation_messages)
	...
	def update_stock_ledger(self):
			...
			if item.has_serial_no or item.has_batch_no:
				...
				if row.serial_and_batch_bundle:
					saashq.throw(
						...
						).format(row.idx, saashq.bold(row.item_code))
					)
		...
		if sl_entries:
			allow_negative_stock = cint(saashq.db.get_single_value("Stock Settings", "allow_negative_stock"))
			self.make_sl_entries(sl_entries, allow_negative_stock=allow_negative_stock)

	def make_adjustment_entry(self, row, sl_entries):
		from erpnexus.stock.stock_ledger import get_stock_value_difference
	...
	def update_valuation_rate_for_serial_nos(self, row, serial_nos):
		...
		for d in serial_nos:
			saashq.db.set_value("Serial No", d, "purchase_rate", valuation_rate)
		...
		data = saashq._dict(
			{
				...
				"actual_qty": 0,
				"company": self.company,
				"stock_uom": saashq.db.get_value("Item", row.item_code, "stock_uom"),
	...
	def validate_expense_account(self):
		...
		if not self.expense_account:
			saashq.throw(_("Please enter Expense Account"))
		elif self.purpose == "Opening Stock" or not saashq.db.sql(
			"""select name from `tabStock Ledger Entry` limit 1"""
		):
			if saashq.db.get_value("Account", self.expense_account, "report_type") == "Profit and Loss":
				saashq.throw(
					_(
	...
	def set_zero_value_for_customer_provided_items(self):
		...
		for d in self.get("items"):
			is_customer_item = saashq.db.get_value("Item", d.item_code, "is_customer_provided_item")
	...
	def recalculate_current_qty(self, voucher_detail_no):
		from erpnexus.stock.stock_ledger import get_valuation_rate
	...
	def get_current_qty_for_serial_or_batch(self, row):
		doc = saashq.get_doc("Serial and Batch Bundle", row.current_serial_and_batch_bundle)
		current_qty = 0.0
...
def get_batch_qty_for_stock_reco(item_code, warehouse, batch_no, posting_date, posting_time, voucher_no):
	ledger = saashq.qb.DocType("Stock Ledger Entry")

	query = (
		saashq.qb.from_(ledger)
		.select(
...
def get_item_and_warehouses(item_code, warehouse):
	...
	if saashq.get_cached_value("Warehouse", warehouse, "is_group"):
		...
		for ch_warehouse in childrens:
			items.append(saashq._dict({"item_code": item_code, "warehouse": ch_warehouse}))
	else:
		items = [saashq._dict({"item_code": item_code, "warehouse": warehouse})]
...
def get_itemwise_batch(warehouse, posting_date, company, item_code=None):
	from erpnexus.stock.report.batch_wise_balance_history.batch_wise_balance_history import execute
...
def get_stock_balance_for(
	...
	saashq.has_permission("Stock Reconciliation", "write", throw=True)

	item_dict = saashq.get_cached_value("Item", item_code, ["has_serial_no", "has_batch_no"], as_dict=1)
	...
	if not item_dict:
		...
		saashq.throw(msg, title=_("Missing"))
