...
from erpnexus.stock.doctype.delivery_note.delivery_note import make_packing_slip
...
class TestPackingSlip(IntegrationTestCase):
	def test_packing_slip(self):
		...
		for item in dn.items:
			if not saashq.db.exists("Product Bundle", {"new_item_code": item.item_code}):
		...
		self.assertRaises(saashq.exceptions.ValidationError, ps3.save)
		...
		dn.load_from_db()
		self.assertRaises(saashq.exceptions.ValidationError, dn.submit)
