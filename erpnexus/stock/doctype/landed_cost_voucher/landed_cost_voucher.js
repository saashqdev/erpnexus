...
erpnexus.landed_cost_taxes_and_charges.setup_triggers("Landed Cost Voucher");
erpnexus.stock.LandedCostVoucher = class LandedCostVoucher extends erpnexus.stock.StockController {
	setup() {
		var me = this;
		this.frm.fields_dict.purchase_receipts.grid.get_field("receipt_document").get_query = function (
			...
			if (!me.frm.doc.company) saashq.msgprint(__("Please enter company first"));
			return {
				filters: filters,
			};
		};
	...
	refresh() {
		...
		if (this.frm.doc.company) {
			let company_currency = saashq.get_doc(":Company", this.frm.doc.company).default_currency;
	...
	get_items_from_purchase_receipts() {
		...
		if (!this.frm.doc.purchase_receipts.length) {
			saashq.msgprint(__("Please enter Purchase Receipt first"));
		} else {
...
cur_frm.script_manager.make(erpnexus.stock.LandedCostVoucher);
