...
from saashq.utils import add_days, add_to_date, flt, now, nowtime, today
...
from erpnexus.accounts.utils import update_gl_entries_after
...
class TestLandedCostVoucher(IntegrationTestCase):
	...
	def test_landed_cost_voucher_stock_impact(self):
		...
		pr1 = make_purchase_receipt(
			...
			qty=500,
			rate=80,
			posting_date=add_days(saashq.utils.nowdate(), -2),
		)
		...
		last_sle = saashq.db.get_value(  # SLE of second PR
			"Stock Ledger Entry",
			{
		...
		last_sle_after_landed_cost = saashq.db.get_value(  # SLE of second PR after LCV's effect
			"Stock Ledger Entry",
			{
				"voucher_type": pr2.doctype,
				"voucher_no": pr2.name,
	...
	def test_landed_cost_voucher_against_purchase_invoice(self):
		...
		pi_lc_value = saashq.db.get_value(
	...
	def test_landed_cost_voucher_for_serialized_item(self):
		saashq.db.set_value("Item", "_Test Serialized Item", "serial_no_series", "SNJJ.###")
	...
	def test_asset_lcv(self):
		...
		self.assertEqual(saashq.db.get_value("Asset", assets[0].name, "gross_purchase_amount"), 50080)
	...
	def test_landed_cost_voucher_with_serial_batch_for_legacy_pr(self):
		...
		if not saashq.db.exists("Batch", "BATCH-TLCVSNO-0001"):
		...
		for row in pr.items:
			...
			self.assertEqual(
				row.valuation_rate,
				saashq.db.get_value("Serial and Batch Bundle", row.serial_and_batch_bundle, "avg_rate"),
			)
	...
	def test_do_not_validate_landed_cost_voucher_with_serial_batch_for_legacy_pr(self):
		from erpnexus.stock.doctype.item.test_item import make_item
		from erpnexus.stock.doctype.serial_and_batch_bundle.serial_and_batch_bundle import get_auto_batch_nos
		...
		if not saashq.db.exists("Batch", "BATCH-TDVLCVSNO-0001"):
			batch_doc = saashq.get_doc(
				{
					"doctype": "Batch",
					"item": batch_item,
		...
		stock_ledger_entries = saashq.get_all("Stock Ledger Entry", filters={"voucher_no": pr.name})
		for sle in stock_ledger_entries:
			doc = saashq.get_doc("Stock Ledger Entry", sle.name)
		...
		stock_ledger_entries = saashq.get_all("Stock Ledger Entry", filters={"voucher_no": dn.name})
		for sle in stock_ledger_entries:
			doc = saashq.get_doc("Stock Ledger Entry", sle.name)
			if doc.item_code == sn_item:
				doc.db_set("serial_no", ", ".join(serial_nos))
...
def make_landed_cost_voucher(**args):
	...
	ref_doc = saashq.get_doc(args.receipt_document_type, args.receipt_document)
