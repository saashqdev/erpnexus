...
import saashq
from saashq import _, _dict, bold
from saashq.model.document import Document
from saashq.query_builder.functions import CombineDatetime, Sum
...
from erpnexus.stock.serial_batch_bundle import get_serial_nos as get_serial_nos_from_bundle


class SerialNoExistsInFutureTransactionError(saashq.ValidationError):
	pass


class BatchNegativeStockError(saashq.ValidationError):
	pass


class SerialNoDuplicateError(saashq.ValidationError):
	pass


class SerialNoWarehouseError(saashq.ValidationError):
	pass
...
class SerialandBatchBundle(Document):
	...
	if TYPE_CHECKING:
		...
		from erpnexus.stock.doctype.serial_and_batch_entry.serial_and_batch_entry import (
			SerialandBatchEntry,
		)
	...
	def validate(self):
		...
		if not self.flags.ignore_validate_serial_batch or saashq.flags.in_test:
	...
	def validate_serial_nos_inventory(self):
		...
		available_serial_nos = get_available_serial_nos(saashq._dict(kwargs))
	...
	def throw_error_message(self, message, exception=saashq.ValidationError):
		saashq.throw(_(message), exception, title=_("Error"))
	...
	def get_valuation_rate_for_return_entry(self, return_aginst):
		valuation_details = saashq._dict(
			{
	...
	def validate_negative_batch(self, batch_no, available_qty):
		if available_qty < 0 and not self.is_stock_reco_for_valuation_adjustment(available_qty):
			...
			saashq.throw(_(msg), BatchNegativeStockError)
	...
	def is_stock_reco_for_valuation_adjustment(self, available_qty):
		if (
			...
			and abs(saashq.db.get_value("Stock Reconciliation Item", self.voucher_detail_no, "qty"))
			== abs(available_qty)
		):
			return True
	...
	def get_return_aginst(self, parent=None):
		...
		if (
			...
			voucher_details = saashq.db.get_value(
	...
	def set_incoming_rate_for_inward_transaction(self, row=None, save=False):
		...
		if self.voucher_type == "Subcontracting Receipt":
			if not self.voucher_detail_no:
				return
			elif saashq.db.exists("Subcontracting Receipt Supplied Item", self.voucher_detail_no):
				valuation_field = "rate"
		...
		precision = saashq.get_precision(child_table, valuation_field) or 2

		if not rate and self.voucher_detail_no and self.voucher_no:
			rate = saashq.db.get_value(child_table, self.voucher_detail_no, valuation_field)
	...
	def set_serial_and_batch_values(self, parent, row, qty_field=None):
		...
		if parent.doctype in [
			...
			return_ref_field = saashq.scrub(parent.doctype) + "_item"
	...
	def validate_voucher_no(self):
		...
		if (
			self.docstatus == 1
			and self.voucher_no
			and not saashq.db.exists(self.voucher_type, self.voucher_no)
		):
		...
		if (
			self.docstatus == 1
			and saashq.get_cached_value(self.voucher_type, self.voucher_no, "docstatus") != 1
		):
	...
	def check_future_entries_exists(self):
		...
		parent = saashq.qb.DocType("Serial and Batch Bundle")
		child = saashq.qb.DocType("Serial and Batch Entry")
		...
		if future_entries:
			...
			saashq.throw(_(msg), title=_(title), exc=SerialNoExistsInFutureTransactionError)
	...
	def calculate_outgoing_rate(self):
		...
		if self.voucher_type in ["Purchase Receipt", "Purchase Invoice"]:
			return saashq.get_cached_value(self.voucher_type, self.voucher_no, "is_return")
		elif self.voucher_type in ["Sales Invoice", "Delivery Note"]:
			return not saashq.get_cached_value(self.voucher_type, self.voucher_no, "is_return")
		elif self.voucher_type == "Stock Entry":
			return saashq.get_cached_value(self.voucher_type, self.voucher_no, "purpose") in [
				"Material Receipt"
			]
	...
	def validate_incorrect_serial_nos(self, serial_nos):
		if self.voucher_type == "Stock Entry" and self.voucher_no:
			if saashq.get_cached_value("Stock Entry", self.voucher_no, "purpose") == "Repack":
				return

		incorrect_serial_nos = saashq.get_all(
			"Serial No",
	...
	def validate_incorrect_batch_nos(self, batch_nos):
		incorrect_batch_nos = saashq.get_all(
		...
		for d in data:
			if self.has_serial_no:
				...
				if d.serial_and_batch_bundle:
					...
					batches = saashq._dict({d.batch_no: d.stock_qty})
	...
	def get_orignal_document_data(self):
		...
		return saashq.get_all(child_doc, fields=fields, filters={"name": self.returned_against})
	...
	def delink_serial_and_batch_bundle(self):
		sles = saashq.get_all("Stock Ledger Entry", filters={"serial_and_batch_bundle": self.name})

		for sle in sles:
			saashq.db.set_value("Stock Ledger Entry", sle.name, "serial_and_batch_bundle", None)
	...
	def delink_refernce_from_voucher(self):
		...
		if (
			...
			and self.voucher_detail_no
			and not saashq.db.exists("Subcontracting Receipt Item", self.voucher_detail_no)
		):
			self.voucher_type = "Subcontracting Receipt Supplied"

		vouchers = saashq.get_all(
			self.child_table,
			fields=fields,
			filters={"docstatus": 0},
		...
		if not vouchers and self.voucher_type == "Delivery Note":
			saashq.db.set_value("Packed Item", self.voucher_detail_no, "serial_and_batch_bundle", None)
			return

		for voucher in vouchers:
			if voucher.get("current_serial_and_batch_bundle"):
				saashq.db.set_value(self.child_table, voucher.name, "current_serial_and_batch_bundle", None)
			elif voucher.get("rejected_serial_and_batch_bundle"):
				saashq.db.set_value(self.child_table, voucher.name, "rejected_serial_and_batch_bundle", None)

			saashq.db.set_value(self.child_table, voucher.name, "serial_and_batch_bundle", None)
	...
	def delink_reference_from_batch(self):
		...
		for batch in batches:
			saashq.db.set_value("Batch", batch.name, {"reference_name": None, "reference_doctype": None})

	def validate_serial_and_batch_data(self):
		if not self.voucher_no:
			saashq.throw(_("Voucher No is mandatory"))
	...
	def validate_batch_inventory(self):
		if (
			...
			and saashq.db.get_value(self.voucher_type, self.voucher_no, "docstatus") == 1
		):
			return
		...
		if (
			self.voucher_type == "Stock Reconciliation"
			and self.type_of_transaction == "Outward"
			and saashq.db.get_value("Stock Reconciliation Item", self.voucher_detail_no, "qty") > 0
		):
			return
	...
	def validate_voucher_no_docstatus(self):
		...
		if saashq.db.get_value(self.voucher_type, self.voucher_no, "docstatus") == 1:
	...
	def delete_serial_batch_entries(self):
		SBBE = saashq.qb.DocType("Serial and Batch Entry")

		saashq.qb.from_(SBBE).delete().where(SBBE.parent == self.name).run()
...
def get_serial_batch_from_csv(item_code, file_path):
	if "private" in file_path:
		file_path = saashq.get_site_path() + file_path
	else:
		file_path = saashq.get_site_path() + "/public" + file_path
...
def make_serial_nos(item_code, serial_nos):
	...
	existing_serial_nos = saashq.get_all("Serial No", filters={"name": ("in", serial_nos)})
	...
	saashq.msgprint(_("Serial Nos are created successfully"), alert=True)


def make_batch_nos(item_code, batch_nos):
	item = saashq.get_cached_value("Item", item_code, ["description", "item_code"], as_dict=1)
	batch_nos = [d.get("batch_no") for d in batch_nos if d.get("batch_no")]

	existing_batches = saashq.get_all("Batch", filters={"name": ("in", batch_nos)})
	...
	for batch_no in batch_nos:
		if saashq.db.exists("Batch", batch_no):
			continue
	...
	saashq.db.bulk_insert("Batch", fields=fields, values=set(batch_nos_details))

	saashq.msgprint(_("Batch Nos are created successfully"), alert=True)
...
def get_reference_serial_and_batch_bundle(child_row):
	...
	if field:
		return saashq.get_cached_value(child_row.doctype, child_row.get(field), "serial_and_batch_bundle")


@saashq.whitelist()
def add_serial_batch_ledgers(entries, child_row, doc, warehouse, do_not_save=False) -> object:
	if isinstance(child_row, str):
		child_row = saashq._dict(parse_json(child_row))
	...
	if saashq.db.exists("Serial and Batch Bundle", bundle):
...
def create_serial_batch_no_ledgers(
	...
	if (
		...
		and saashq.get_cached_value("Item", child_row.item_code, "has_batch_no")
	):
	...
	if do_not_save:
		saashq.db.set_value(child_row.doctype, child_row.name, "serial_and_batch_bundle", doc.name)

	saashq.msgprint(_("Serial and Batch Bundle created"), alert=True)
...
def update_serial_batch_no_ledgers(bundle, entries, child_row, parent_doc, warehouse=None) -> object:
	doc = saashq.get_doc("Serial and Batch Bundle", bundle)
	...
	saashq.msgprint(_("Serial and Batch Bundle updated"), alert=True)
...
def update_serial_or_batch(bundle_id, serial_no=None, batch_no=None):
	...
	doc = saashq.get_cached_doc("Serial and Batch Bundle", bundle_id)
...
def get_serial_and_batch_ledger(**kwargs):
	...
	sle_table = saashq.qb.DocType("Stock Ledger Entry")
	serial_batch_table = saashq.qb.DocType("Serial and Batch Entry")

	query = (
		saashq.qb.from_(sle_table)
...
def get_reserved_serial_nos_for_pos(kwargs):
	from erpnexus.controllers.sales_and_purchase_return import get_returned_serial_nos
	from erpnexus.stock.doctype.serial_no.serial_no import get_serial_nos

	ignore_serial_nos = []
	pos_invoices = saashq.get_all(
		"POS Invoice",
		fields=[
...
def get_reserved_serial_nos_for_sre(kwargs) -> list:
	...
	sb_entry = saashq.qb.DocType("Serial and Batch Entry")
	query = (
		saashq.qb.from_(sre)
		.inner_join(sb_entry)
...
def get_reserved_batches_for_sre(kwargs) -> dict:
	...
	reserved_batches_details = saashq._dict()
	if data:
		reserved_batches_details = saashq._dict(
			{(d.batch_no, d.warehouse): saashq._dict({"warehouse": d.warehouse, "qty": d.qty}) for d in data}
		)
...
def get_available_batches(kwargs):
	stock_ledger_entry = saashq.qb.DocType("Stock Ledger Entry")
	batch_ledger = saashq.qb.DocType("Serial and Batch Entry")
	batch_table = saashq.qb.DocType("Batch")

	query = (
		saashq.qb.from_(stock_ledger_entry)
	...
	if kwargs.based_on == "LIFO":
		query = query.orderby(batch_table.creation, order=saashq.qb.desc)
	elif kwargs.based_on == "Expiry":
...
def get_voucher_wise_serial_batch_from_bundle(**kwargs) -> dict[str, dict]:
	...
	for row in data:
		...
		if kwargs.get("get_subcontracted_item"):
			...
			subcontracted_item_code = saashq.get_cached_value(doctype, row.voucher_detail_no, field_name)
		...
		if key not in group_by_voucher:
			group_by_voucher.setdefault(
				key,
				saashq._dict({"serial_nos": [], "batch_nos": defaultdict(float), "item_row": row}),
			)
...
def get_picked_batches(kwargs) -> dict[str, dict]:
	...
	table = saashq.qb.DocType("Serial and Batch Bundle")
	child_table = saashq.qb.DocType("Serial and Batch Entry")
	pick_list_table = saashq.qb.DocType("Pick List")
	...
	for row in data:
		...
		if key not in picked_batches:
			picked_batches[key] = saashq._dict(
				{
					"qty": row.qty,
					"warehouse": row.warehouse,
				}
...
def get_ledgers_from_serial_batch_bundle(**kwargs) -> list[saashq._dict]:
	bundle_table = saashq.qb.DocType("Serial and Batch Bundle")
	serial_batch_table = saashq.qb.DocType("Serial and Batch Entry")

	query = (
		saashq.qb.from_(bundle_table)
		.inner_join(serial_batch_table)
...
def get_batch_no_from_serial_no(serial_no):
	return saashq.get_cached_value("Serial No", serial_no, "batch_no")


@saashq.whitelist()
def is_serial_batch_no_exists(item_code, type_of_transaction, serial_no=None, batch_no=None):
	if serial_no and not saashq.db.exists("Serial No", serial_no):
		if type_of_transaction != "Inward":
			saashq.throw(_("Serial No {0} does not exists").format(serial_no))

		make_serial_no(serial_no, item_code)

	if batch_no and not saashq.db.exists("Batch", batch_no):
		if type_of_transaction != "Inward":
			saashq.throw(_("Batch No {0} does not exists").format(batch_no))

		make_batch_no(batch_no, item_code)


def make_serial_no(serial_no, item_code):
	serial_no_doc = saashq.new_doc("Serial No")
...
def make_batch_no(batch_no, item_code):
	batch_doc = saashq.new_doc("Batch")
...
def is_duplicate_serial_no(bundle_id, serial_no):
	return saashq.db.exists("Serial and Batch Entry", {"parent": bundle_id, "serial_no": serial_no})
