...
from saashq.query_builder import Case, Order
from saashq.query_builder.functions import Coalesce, CombineDatetime, Sum
...
class Bin(Document):
	...
	def before_save(self):
		if self.get("__islocal") or not self.stock_uom:
			self.stock_uom = saashq.get_cached_value("Item", self.item_code, "stock_uom")
	...
	def update_reserved_qty_for_sub_contracting(self, subcontract_doctype="Subcontracting Order"):
		...
		subcontract_order = saashq.qb.DocType(subcontract_doctype)
		supplied_item = saashq.qb.DocType(
		...
		reserved_qty_for_sub_contract = (
			saashq.qb.from_(subcontract_order)
			.from_(supplied_item)
		...
		if saashq.db.field_exists("Stock Entry", "is_return"):
...
def on_doctype_update():
	saashq.db.add_unique("Bin", ["item_code", "warehouse"], constraint_name="unique_item_warehouse")
