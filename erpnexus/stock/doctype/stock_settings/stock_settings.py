...
from erpnexus.stock.utils import check_pending_reposting
...
class StockSettings(Document):
	...
	def validate(self):
		for key in [
			...
			saashq.db.set_default(key, self.get(key, ""))
	...
	def validate_use_batch_wise_valuation(self):
		...
		if self.valuation_method == "FIFO":
			saashq.throw(_("Cannot disable batch wise valuation for FIFO valuation method."))

		if saashq.get_all(
			"Item", filters={"valuation_method": "FIFO", "is_stock_item": 1, "has_batch_no": 1}, limit=1
		):
			saashq.throw(_("Can't disable batch wise valuation for items with FIFO valuation method."))

		if saashq.get_all("Batch", filters={"use_batchwise_valuation": 1}, limit=1):
			saashq.throw(_("Can't disable batch wise valuation for active batches."))

	def validate_warehouses(self):
		warehouse_fields = ["default_warehouse", "sample_retention_warehouse"]
		for field in warehouse_fields:
			if saashq.db.get_value("Warehouse", self.get(field), "is_group"):
				saashq.throw(
					_(
						"Group Warehouses cannot be used in transactions. Please change the value of {0}"
					).format(saashq.bold(self.meta.get_field(field).label)),
	...
	def validate_clean_description_html(self):
		if int(self.clean_description_html or 0) and not int(self.db_get("clean_description_html") or 0):
			# changed to text
			saashq.enqueue(
				"erpnexus.stock.doctype.stock_settings.stock_settings.clean_all_descriptions",
			...
			if self.allow_negative_stock and self.enable_stock_reservation:
				saashq.throw(
					_("As {0} is enabled, you can not enable {1}.").format(
						saashq.bold(_("Stock Reservation")), saashq.bold(_("Allow Negative Stock"))
					)
				)
		...
		if self.has_value_changed("enable_stock_reservation"):
			...
			if self.enable_stock_reservation:
				...
				if self.allow_negative_stock:
					saashq.throw(
						_("As {0} is enabled, you can not enable {1}.").format(
							saashq.bold(_("Allow Negative Stock")), saashq.bold(_("Stock Reservation"))
						)
					)

				else:
					# Don't allow if there are negative stock
					from saashq.query_builder.functions import Round

					precision = saashq.db.get_single_value("System Settings", "float_precision") or 3
			...
			# Enable -> Disable
				...
				has_reserved_stock = saashq.db.exists(
	...
	def make_property_setter_for_precision(doctypes):
		for doctype in doctypes:
			if property_name := saashq.db.exists(
				"Property Setter",
				{"doc_type": doctype, "field_name": "conversion_factor", "property": "precision"},
			):
				saashq.db.set_value("Property Setter", property_name, "value", 9)
				continue
...
def clean_all_descriptions():
	for item in saashq.get_all("Item", ["name", "description"]):
		if item.description:
			clean_description = clean_html(item.description)
		if item.description != clean_description:
			saashq.db.set_value("Item", item.name, "description", clean_description)
