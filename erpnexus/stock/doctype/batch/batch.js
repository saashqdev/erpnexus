...
saashq.ui.form.on("Batch", {
	...
	item: (frm) => {
		// saashq.db.get_value('Item', {name: frm.doc.item}, 'has_expiry_date', (r) => {
		...
		saashq.db.get_value(
			...
			(r) => {
				if (r.has_expiry_date && r.shelf_life_in_days) {
					...
					frm.set_value(
						"expiry_date",
						saashq.datetime.add_days(frm.doc.manufacturing_date, r.shelf_life_in_days)
					);
				...
				callback: (r) => {
					...
					rows.find(".btn-move").on("click", function () {
						...
						saashq.prompt(
							...
							(data) => {
								saashq.call({
									method: "erpnexus.stock.doctype.stock_entry.stock_entry_utils.make_stock_entry",
									args: {
					...
					rows.find(".btn-split").on("click", function () {
						...
						saashq.prompt(
							...
							(data) => {
								saashq
									.xcall("erpnexus.stock.doctype.batch.batch.split_batch", {
										item_code: frm.doc.item,
										batch_no: frm.doc.name,
										qty: data.qty,
...
saashq.ui.form.on("Batch", "manufacturing_date", function (frm) {
	saashq.db.get_value("Item", { name: frm.doc.item }, ["shelf_life_in_days", "has_expiry_date"], (r) => {
		if (r.has_expiry_date && r.shelf_life_in_days) {
			// Calculate expiry date based on shelf_life_in_days
			frm.set_value(
				"expiry_date",
