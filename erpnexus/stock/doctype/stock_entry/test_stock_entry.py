...
from saashq.permissions import add_user_permission, remove_user_permission
from saashq.tests import IntegrationTestCase, UnitTestCase
from saashq.utils import add_days, cstr, flt, get_time, getdate, nowtime, today
...
from erpnexus.stock.doctype.item.test_item import (
	create_item,
	make_item,
	make_item_variant,
...
from erpnexus.stock.doctype.serial_no.serial_no import *
from erpnexus.stock.doctype.stock_entry.stock_entry import FinishedGoodError, make_stock_in_entry
from erpnexus.stock.doctype.stock_entry.stock_entry_utils import make_stock_entry
from erpnexus.stock.doctype.stock_ledger_entry.stock_ledger_entry import StockFreezeError
...
from erpnexus.stock.stock_ledger import NegativeStockError, get_previous_sle
...
class TestStockEntry(IntegrationTestCase):
	...
	def test_fifo(self):
		...
		self.assertEqual([[1, 10]], saashq.safe_eval(sle.stock_queue))
		...
		self.assertEqual([[-2, 10]], saashq.safe_eval(sle.stock_queue))

		# move stock to positive
		make_stock_entry(item_code=item_code, target=warehouse, qty=3, basic_rate=20)
		sle = get_sle(item_code=item_code, warehouse=warehouse)[0]
		self.assertEqual([[1, 20]], saashq.safe_eval(sle.stock_queue))

		# incoming entry with diff rate
		make_stock_entry(item_code=item_code, target=warehouse, qty=1, basic_rate=30)
		sle = get_sle(item_code=item_code, warehouse=warehouse)[0]

		self.assertEqual([[1, 20], [1, 30]], saashq.safe_eval(sle.stock_queue))

		saashq.db.set_default("allow_negative_stock", 0)
	...
	def test_auto_material_request_for_variant(self):
		...
		make_item_variant()
		template = saashq.get_doc("Item", "_Test Variant Item")
	...
	def _test_auto_material_request(
		...
		variant = saashq.get_doc("Item", item_code)

		projected_qty, actual_qty = saashq.db.get_value(
		...
		projected_qty = (
			saashq.db.get_value("Bin", {"item_code": item_code, "warehouse": warehouse}, "projected_qty") or 0
		)

		saashq.db.set_single_value("Stock Settings", "auto_indent", 1)
		...
		from erpnexus.stock.reorder_item import reorder_item

		mr_list = reorder_item()

		saashq.db.set_single_value("Stock Settings", "auto_indent", 0)
	...
	def test_material_receipt_gl_entry(self):
		company = saashq.db.get_value("Warehouse", "Stores - TCP1", "company")
	...
	def test_repack_multiple_fg(self):
		...
		repack = saashq.copy_doc(self.globalTestRecords["Stock Entry"][3])
	...
	def test_repack_with_additional_costs(self):
		...
		default_expense_account = saashq.get_value("Company", company, "default_expense_account")
	...
	def test_serial_no_not_reqd(self):
		se = saashq.copy_doc(self.globalTestRecords["Stock Entry"][0])
		...
		self.assertRaises(saashq.ValidationError, bundle_id.make_serial_and_batch_bundle)
	...
	def test_serial_no_transfer_in(self):
		...
		self.assertTrue(saashq.db.get_value("Serial No", "ABCD1", "warehouse"))
		self.assertTrue(saashq.db.get_value("Serial No", "EFGH1", "warehouse"))

		se.cancel()
		self.assertFalse(saashq.db.get_value("Serial No", "ABCD1", "warehouse"))
	...
	def test_serial_by_series(self):
		...
		self.assertTrue(saashq.db.exists("Serial No", serial_nos[0]))
		self.assertTrue(saashq.db.exists("Serial No", serial_nos[1]))
	...
	def test_serial_move(self):
		...
		saashq.flags.use_serial_and_batch_fields = True
		...
		se.insert()
		se.submit()
		self.assertTrue(saashq.db.get_value("Serial No", serial_no, "warehouse"), "_Test Warehouse 1 - _TC")

		se.cancel()
		self.assertTrue(saashq.db.get_value("Serial No", serial_no, "warehouse"), "_Test Warehouse - _TC")
		saashq.flags.use_serial_and_batch_fields = False
	...
	def test_serial_cancel(self):
		...
		se.cancel()
		self.assertFalse(saashq.db.get_value("Serial No", serial_no, "warehouse"))
	...
	def test_serial_batch_item_stock_entry(self):
		...
		item = saashq.db.exists("Item", {"item_name": "Batched and Serialised Item"})
		if not item:
			...
			item = saashq.get_doc("Item", {"item_name": "Batched and Serialised Item"})
		...
		batch_in_serial_no = saashq.db.get_value("Serial No", serial_no, "batch_no")
		...
		batch_in_serial_no = saashq.db.get_value("Serial No", serial_no, "batch_no")
		self.assertEqual(saashq.db.get_value("Serial No", serial_no, "warehouse"), None)

	def test_warehouse_company_validation(self):
		saashq.db.get_value("Warehouse", "_Test Warehouse 2 - _TC1", "company")
		saashq.get_doc("User", "test2@example.com").add_roles(
			"Sales User", "Sales Manager", "Stock User", "Stock Manager"
		)
		saashq.set_user("test2@example.com")

		from erpnexus.stock.utils import InvalidWarehouseCompany

		st1 = saashq.copy_doc(self.globalTestRecords["Stock Entry"][0])
	...
	def test_warehouse_user(self):
		...
		st1.get("items")[0].t_warehouse = "_Test Warehouse 2 - _TC1"
		self.assertRaises(saashq.PermissionError, st1.insert)
	...
	def test_freeze_stocks(self):
		saashq.db.set_single_value("Stock Settings", "stock_auth_role", "")

		# test freeze_stocks_upto
		saashq.db.set_single_value("Stock Settings", "stock_frozen_upto", add_days(nowdate(), 5))
		se = saashq.copy_doc(self.globalTestRecords["Stock Entry"][0]).insert()
		self.assertRaises(StockFreezeError, se.submit)

		saashq.db.set_single_value("Stock Settings", "stock_frozen_upto", "")

		# test freeze_stocks_upto_days
		saashq.db.set_single_value("Stock Settings", "stock_frozen_upto_days", -1)
		se = saashq.copy_doc(self.globalTestRecords["Stock Entry"][0])
		...
		se.insert()
		self.assertRaises(StockFreezeError, se.submit)
		saashq.db.set_single_value("Stock Settings", "stock_frozen_upto_days", 0)
	...
	def test_work_order(self):
		...
		bom_no, bom_operation_cost = saashq.db.get_value(
			"BOM", {"item": "_Test FG Item 2", "is_default": 1, "docstatus": 1}, ["name", "operating_cost"]
		)

		work_order = saashq.new_doc("Work Order")
		work_order.update(
			{
				"company": "_Test Company",
	...
	def test_work_order_manufacture_with_material_consumption(self):
		...
		bom_no = saashq.db.get_value("BOM", {"item": "_Test FG Item", "is_default": 1, "docstatus": 1})
		...
		s = saashq.get_doc(_make_stock_entry(work_order.name, "Material Transfer for Manufacture", 1))
		...
		s = saashq.get_doc(_make_stock_entry(work_order.name, "Manufacture", 1))
		s.save()
		rm_cost = 0
		for d in s.get("items"):
			if d.s_warehouse:
	...
	def test_variant_work_order(self):
		bom_no = saashq.db.get_value("BOM", {"item": "_Test Variant Item", "is_default": 1, "docstatus": 1})
		...
		from erpnexus.manufacturing.doctype.work_order.work_order import make_stock_entry

		stock_entry = saashq.get_doc(make_stock_entry(work_order.name, "Manufacture", 1))
		stock_entry.insert()
	...
	def test_nagative_stock_for_batch(self):
		...
		ste = saashq.new_doc("Stock Entry")
		...
		self.assertRaises(saashq.ValidationError, ste.submit)

	def test_same_serial_nos_in_repack_or_manufacture_entries(self):
		s1 = make_serialized_item(self, target_warehouse="_Test Warehouse - _TC")
		...
		doc = saashq.get_doc("Serial and Batch Bundle", bundle_id)
		doc.db_set(
			{
				"item_code": "_Test Serialized Item",
				"warehouse": "_Test Warehouse - _TC",
	...
	def test_quality_check(self):
		...
		repack.insert()
		self.assertRaises(saashq.ValidationError, repack.submit)
	...
	def test_zero_incoming_rate(self):
		...
		value_diff = saashq.db.get_value(
			"Stock Ledger Entry",
	...
	def test_gle_for_opening_stock_entry(self):
		...
		is_opening = saashq.db.get_value(
			"GL Entry",
	...
	def test_conversion_factor_change(self):
		saashq.db.set_single_value("Stock Settings", "allow_negative_stock", 1)
		repack_entry = saashq.copy_doc(self.globalTestRecords["Stock Entry"][3])
	...
	def test_additional_cost_distribution_manufacture(self):
		se = saashq.get_doc(
			doctype="Stock Entry",
			purpose="Manufacture",
			additional_costs=[saashq._dict(base_amount=100)],
			items=[
				saashq._dict(item_code="RM", basic_amount=10),
				saashq._dict(item_code="FG", basic_amount=20, t_warehouse="X", is_finished_item=1),
				saashq._dict(item_code="scrap", basic_amount=30, t_warehouse="X"),
			],
		)
	...
	def test_additional_cost_distribution_non_manufacture(self):
		se = saashq.get_doc(
			...
			additional_costs=[saashq._dict(base_amount=100)],
			items=[
				saashq._dict(item_code="RECEIVED_1", basic_amount=20, t_warehouse="X"),
				saashq._dict(item_code="RECEIVED_2", basic_amount=30, t_warehouse="X"),
			],
		)
	...
	def test_independent_manufacture_entry(self):
		...
		se = saashq.get_doc(
			...
			items=[
				...
				saashq._dict(item_code="_Test FG Item", qty=4, t_warehouse="_Test Warehouse 1 - _TC"),
			],
		)
	...
	def test_future_negative_sle_batch(self):
		from erpnexus.stock.doctype.batch.test_batch import TestBatch
	...
	def test_multi_batch_value_diff(self):
		...
		receipt.append(
			"items", saashq.copy_doc(receipt.items[0], ignore_no_copy=False).update({"basic_rate": 20})
		)
		receipt.save()
		receipt.submit()
		receipt.load_from_db()

		batches = saashq._dict(
	...
	def test_transfer_qty_validation(self):
		...
		self.assertRaises(saashq.ValidationError, se.save)

	def test_mapped_stock_entry(self):
		"Check if rate and stock details are populated in mapped SE given warehouse."
		from erpnexus.stock.doctype.purchase_receipt.purchase_receipt import make_stock_entry
	...
	def test_reposting_for_depedent_warehouse(self):
		from erpnexus.stock.doctype.repost_item_valuation.repost_item_valuation import repost_sl_entries
		from erpnexus.stock.doctype.warehouse.test_warehouse import create_warehouse
		...
		repost_name = saashq.db.get_value(
			"Repost Item Valuation", {"voucher_no": receipt2.name, "docstatus": 1}, "name"
		)

		doc = saashq.get_doc("Repost Item Valuation", repost_name)
	...
	def test_batch_expiry(self):
		from erpnexus.controllers.stock_controller import BatchExpiredError
		from erpnexus.stock.doctype.batch.test_batch import make_new_batch
		...
		batch = make_new_batch(
			batch_id=saashq.generate_hash("", 5), item_code=item_doc.name, expiry_date=add_days(today(), -1)
		)
	...
	def test_negative_stock_reco(self):
		saashq.db.set_single_value("Stock Settings", "allow_negative_stock", 0)
		...
		self.assertRaises(saashq.ValidationError, sr_doc.submit)
	...
	def test_negative_batch(self):
		...
		self.assertRaises(saashq.ValidationError, se1.cancel)
	...
	def test_auto_reorder_level(self):
		...
		if not saashq.db.exists("Item Reorder", {"parent": item_doc.name}):
			item_doc.append(
				"reorder_levels",
				{
	...
	def test_use_serial_and_batch_fields(self):
		...
		for serial_no in serial_nos:
			self.assertTrue(saashq.db.exists("Serial No", serial_no))
			self.assertEqual(saashq.db.get_value("Serial No", serial_no, "status"), "Active")
		...
		for serial_no in serial_nos:
			self.assertTrue(saashq.db.exists("Serial No", serial_no))
			self.assertEqual(saashq.db.get_value("Serial No", serial_no, "status"), "Delivered")
	...
	def test_serial_batch_bundle_type_of_transaction(self):
		...
		saashq.db.set_value("Serial and Batch Bundle", sbb, "type_of_transaction", "Inward")
		self.assertRaises(saashq.ValidationError, se.submit)
...
def initialize_records_for_future_negative_sle_test(
	...
	from erpnexus.stock.doctype.batch.test_batch import TestBatch, make_new_batch
	from erpnexus.stock.doctype.stock_reconciliation.test_stock_reconciliation import (
		create_stock_reconciliation,
	)
	from erpnexus.stock.doctype.warehouse.test_warehouse import create_warehouse
