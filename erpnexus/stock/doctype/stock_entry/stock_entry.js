// Copyright (c) 2015, Saashq Technologies Pvt. Ltd. and Contributors // License: GNU General Public License v3. See license.txt

saashq.provide("erpnexus.stock");
saashq.provide("erpnexus.accounts.dimensions");

erpnexus.landed_cost_taxes_and_charges.setup_triggers("Stock Entry");

saashq.ui.form.on("Stock Entry", {
	setup: function (frm) {
		...
		frm.set_query("batch_no", "items", function (doc, cdt, cdn) {
			...
			if (!item.item_code) {
				saashq.throw(__("Please enter Item Code to get Batch Number"));
			} else {
				if (
					in_list(
						[
					...
					filters = {
						...
						posting_date: frm.doc.posting_date || saashq.datetime.nowdate(),
					};
				} else {
					filters = {
						item_code: item.item_code,
		...
		erpnexus.accounts.dimensions.setup_dimension_filters(frm, frm.doctype);

		saashq.db.get_single_value("Stock Settings", "disable_serial_no_and_batch_selector").then((value) => {
			if (value) {
		...
		if (!check_should_not_attach_bom_items(frm.doc.bom_no)) {
			erpnexus.accounts.dimensions.update_dimension(frm, frm.doctype);
		}
	},
	...
	setup_quality_inspection: function (frm) {
		...
		if (!frm.is_new() && frm.doc.docstatus === 0) {
			frm.add_custom_button(
				...
				() => {
					let transaction_controller = new erpnexus.TransactionController({ frm: frm });
		...
		frm.set_query("quality_inspection", "items", function (doc, cdt, cdn) {
			...
			return {
				query: "erpnexus.stock.doctype.quality_inspection.quality_inspection.quality_inspection_query",
				filters: {
	...
	refresh: function (frm) {
		...
		if (!frm.doc.docstatus) {
			...
			frm.add_custom_button(
				__("Material Request"),
				function () {
					saashq.model.with_doctype("Material Request", function () {
						var mr = saashq.model.get_new_doc("Material Request");
						...
						items.forEach(function (item) {
							var mr_item = saashq.model.add_child(mr, "items");
							...
							mr_item.required_date = saashq.datetime.nowdate();
						});
						saashq.set_route("Form", "Material Request", mr.name);
					});
				},
				__("Create")
			);
		...
		if (frm.doc.items) {
			...
			if (frm.doc.docstatus == 0 && has_alternative) {
				frm.add_custom_button(__("Alternate Item"), () => {
					erpnexus.utils.select_alternate_items({
						frm: frm,
						child_docname: "items",
			...
			if (
				...
				frm.add_custom_button(__("End Transit"), function () {
					saashq.model.open_mapped_doc({
						method: "erpnexus.stock.doctype.stock_entry.stock_entry.make_stock_in_entry",
						frm: frm,
					});
				});
			}
			...
			if (frm.doc.per_transferred > 0) {
				frm.add_custom_button(
					...
					function () {
						...
						saashq.set_route("List", "Stock Entry");
					},
					__("View")
				);
			}
		...
		if (frm.doc.docstatus === 0) {
			frm.add_custom_button(
				...
				function () {
					erpnexus.utils.map_current_doc({
						method: "erpnexus.accounts.doctype.purchase_invoice.purchase_invoice.make_stock_entry",
			...
			frm.add_custom_button(
				...
				function () {
					...
					const d = erpnexus.utils.map_current_doc({
						method: "erpnexus.stock.doctype.material_request.material_request.make_stock_entry",
		...
		if (frm.doc.docstatus === 0 && frm.doc.purpose == "Material Issue") {
			frm.add_custom_button(
				...
				function () {
					saashq.call({
						method: "erpnexus.stock.doctype.stock_entry.stock_entry.get_expired_batch_items",
						callback: function (r) {
							if (!r.exc && r.message) {
								frm.set_value("items", []);
	...
	validate_purpose_consumption: function (frm) {
		saashq
			.call({
				method: "erpnexus.manufacturing.doctype.manufacturing_settings.manufacturing_settings.is_material_consumption_enabled",
			})
			.then((r) => {
				if (cint(r.message) == 0 && frm.doc.purpose == "Material Consumption for Manufacture") {
					frm.set_value("purpose", "Manufacture");
					saashq.throw(__("Material Consumption is not set in Manufacturing Settings."));
				}
			});
	},
	...
	company: function (frm) {
		if (frm.doc.company) {
			var company_doc = saashq.get_doc(":Company", frm.doc.company);
	...
	make_retention_stock_entry: function (frm) {
		saashq.call({
			method: "erpnexus.stock.doctype.stock_entry.stock_entry.move_sample_to_retention_warehouse",
			args: {
	...
	toggle_display_account_head: function (frm) {
		var enabled = erpnexus.is_perpetual_inventory_enabled(frm.doc.company);
	...
	set_basic_rate: function (frm, cdt, cdn) {
		...
		if (item.item_code || item.serial_no) {
			saashq.call({
				...
				callback: function (r) {
					saashq.model.set_value(cdt, cdn, "basic_rate", r.message || 0.0);
	...
	get_warehouse_details: function (frm, cdt, cdn) {
		...
		if (!child.bom_no) {
			saashq.call({
				method: "erpnexus.stock.doctype.stock_entry.stock_entry.get_warehouse_details",
				args: {
					args: {
				...
				callback: function (r) {
					if (!r.exc) {
						...
						fields.forEach((field) => {
							saashq.model.set_value(cdt, cdn, field, r.message[field] || 0.0);
						});
	...
	get_items_from_bom: function (frm) {
		...
		d.get_input("fetch").on("click", function () {
			...
			if (!frm.doc.company) saashq.throw(__("Company field is required"));
			saashq.call({
				method: "erpnexus.manufacturing.doctype.bom.bom.get_bom_items",
				args: values,
				callback: function (r) {
					if (!r.message) {
						saashq.throw(__("BOM does not contain any stock item"));
					} else {
						erpnexus.utils.remove_empty_first_row(frm, "items");
						$.each(r.message, function (i, item) {
							let d = saashq.model.add_child(cur_frm.doc, "Stock Entry Detail", "items");
	...
	calculate_total_additional_costs: function (frm) {
		const total_additional_costs = saashq.utils.sum(
	...
	source_warehouse_address: function (frm) {
		erpnexus.utils.get_address_display(frm, "source_warehouse_address", "source_address_display", false);
	},

	target_warehouse_address: function (frm) {
		erpnexus.utils.get_address_display(frm, "target_warehouse_address", "target_address_display", false);
	},
	...
	set_transit_warehouse: function (frm) {
		if (
			...
			saashq.db.get_value(dt, dn, "default_in_transit_warehouse", (r) => {
	...
	apply_putaway_rule: function (frm) {
		if (frm.doc.apply_putaway_rule) erpnexus.apply_putaway_rule(frm, frm.doc.purpose);
	},
	...
	purchase_order: (frm) => {
		if (frm.doc.purchase_order) {
			...
			erpnexus.utils.map_current_doc({
				method: "erpnexus.stock.doctype.stock_entry.stock_entry.get_items_from_subcontract_order",
...
saashq.ui.form.on("Stock Entry Detail", {
	...
	s_warehouse(frm, cdt, cdn) {
		...
		if (item.s_warehouse) {
			saashq.model.set_value(cdt, cdn, "allow_zero_valuation_rate", 0);
		}
	},
	...
	uom(doc, cdt, cdn) {
		...
		if (d.uom && d.item_code) {
			return saashq.call({
				method: "erpnexus.stock.doctype.stock_entry.stock_entry.get_uom_details",
				args: {
					item_code: d.item_code,
					uom: d.uom,
					qty: d.qty,
				...
				callback: function (r) {
					if (r.message) {
						saashq.model.set_value(cdt, cdn, r.message);
					}
				},
			});
		}
				...
				callback: function (r) {
					if (r.message) {
						...
						$.each(r.message, function (k, v) {
							if (v) {
								saashq.model.set_value(cdt, cdn, k, v); // qty and it's subsequent fields weren't triggered
							}
						});
						refresh_field("items");
						...
						if (
							...
							!saashq.flags.hide_serial_batch_dialog &&
							!saashq.flags.dialog_set
						) {
							saashq.flags.dialog_set = true;
							erpnexus.stock.select_batch_and_serial_no(frm, d);
						} else {
	...
	expense_account(frm, cdt, cdn) {
		erpnexus.utils.copy_value_in_all_rows(frm.doc, cdt, cdn, "items", "expense_account");
	},

	cost_center(frm, cdt, cdn) {
		erpnexus.utils.copy_value_in_all_rows(frm.doc, cdt, cdn, "items", "cost_center");
	},
	...
	add_serial_batch_bundle(frm, cdt, cdn) {
		var child = locals[cdt][cdn];
		erpnexus.stock.select_batch_and_serial_no(frm, child);
	},
});
...
var validate_sample_quantity = function (frm, cdt, cdn) {
	...
	if (d.sample_quantity && frm.doc.purpose == "Material Receipt") {
		saashq.call({
			method: "erpnexus.stock.doctype.stock_entry.stock_entry.validate_sample_quantity",
			args: {
				batch_no: d.batch_no,
				item_code: d.item_code,
			...
			callback: (r) => {
				saashq.model.set_value(cdt, cdn, "sample_quantity", r.message);
			},
		});
	}
};

saashq.ui.form.on("Landed Cost Taxes and Charges", {
...
erpnexus.stock.StockEntry = class StockEntry extends erpnexus.stock.StockController {
	setup() {
		var me = this;
		...
		this.frm.fields_dict.items.grid.get_field("item_code").get_query = function () {
			return erpnexus.queries.item({ is_stock_item: 1 });
		};
		...
		if (me.frm.doc.company && erpnexus.is_perpetual_inventory_enabled(me.frm.doc.company)) {
			this.frm.add_fetch("company", "stock_adjustment_account", "expense_account");
		}

		this.frm.fields_dict.items.grid.get_field("expense_account").get_query = function () {
			if (erpnexus.is_perpetual_inventory_enabled(me.frm.doc.company)) {
				return {
					filters: {
						company: me.frm.doc.company,
						is_group: 0,
		...
		saashq.dynamic_link = { doc: this.frm.doc, fieldname: "supplier", doctype: "Supplier" };
		this.frm.set_query("supplier_address", erpnexus.queries.address_query);
	}
	...
	refresh() {
		...
		if (this.frm.doc.docstatus === 1 && erpnexus.is_perpetual_inventory_enabled(this.frm.doc.company)) {
			this.show_general_ledger();
		}
		erpnexus.hide_company(this.frm);
		erpnexus.utils.add_item(this.frm);
	}
	...
	set_default_account(callback) {
		...
		if (this.frm.doc.company && erpnexus.is_perpetual_inventory_enabled(this.frm.doc.company)) {
			return this.frm.call({
				method: "erpnexus.accounts.utils.get_company_default",
				args: {
	...
	clean_up() {
		...
		if (
			...
			saashq.model.remove_from_locals("Work Order", this.frm.doc.work_order);
		}
	}
	...
	get_items() {
		...
		if (!this.frm.doc.fg_completed_qty || !this.frm.doc.bom_no)
			saashq.throw(__("BOM and Manufacturing Quantity are required"));
			...
			return this.frm.call({
				...
				callback: function (r) {
					...
					if (me.frm.doc.bom_no) {
						...
						erpnexus.accounts.dimensions.update_dimension(me.frm, me.frm.doctype);
					}
				},
			});
		}
	...
	work_order() {
		...
		return saashq.call({
			method: "erpnexus.stock.doctype.stock_entry.stock_entry.get_work_order_details",
			args: {
	...
	add_excise_button() {
		if (saashq.boot.sysdefaults.country === "India")
			this.frm.add_custom_button(
				__("Excise Invoice"),
				function () {
					var excise = saashq.model.make_new_doc_and_get_name("Journal Entry");
					excise = locals["Journal Entry"][excise];
					excise.voucher_type = "Excise Entry";
					saashq.set_route("Form", "Journal Entry", excise.name);
				},
				__("Create")
			);
	}
	...
	items_add(doc, cdt, cdn) {
		...
		if (cint(saashq.user_defaults?.use_serial_batch_fields)) {
			saashq.model.set_value(row.doctype, row.name, "use_serial_batch_fields", 1);
		}
	}
	...
	set_warehouse_in_children(child_table, warehouse_field, warehouse) {
		let transaction_controller = new erpnexus.TransactionController();
	...
	toggle_related_fields(doc) {
		...
		if (erpnexus.stock.is_subcontracting_or_return_transfer(doc)) {
			doc.customer =
				doc.customer_name =
	...
	supplier(doc) {
		erpnexus.utils.get_party_details(this.frm, null, null, null);
	}
};

erpnexus.stock.select_batch_and_serial_no = (frm, item) => {
	let path = "assets/erpnexus/js/utils/serial_no_batch_selector.js";
	...
	saashq.db.get_value("Item", item.item_code, ["has_batch_no", "has_serial_no"]).then((r) => {
		if (r.message && (r.message.has_batch_no || r.message.has_serial_no)) {
			...
			new erpnexus.SerialBatchPackageSelector(frm, item, (r) => {
				if (r) {
...
erpnexus.stock.is_subcontracting_or_return_transfer = (doc) => {
...
function attach_bom_items(bom_no) {
	...
	saashq.db.get_doc("BOM", bom_no).then((bom) => {
		const { name, items } = bom;
		erpnexus.stock.bom = { name, items: {} };
		items.forEach((item) => {
			erpnexus.stock.bom.items[item.item_code] = item;
		});
	});
}

function check_should_not_attach_bom_items(bom_no) {
	return bom_no === undefined || (erpnexus.stock.bom && erpnexus.stock.bom.name === bom_no);
}

extend_cscript(cur_frm.cscript, new erpnexus.stock.StockEntry({ frm: cur_frm }));
