...
import erpnexus
from erpnexus.accounts.general_ledger import process_gl_map
from erpnexus.buying.utils import check_on_hold_or_closed_status
from erpnexus.controllers.taxes_and_totals import init_landed_taxes_and_totals
from erpnexus.manufacturing.doctype.bom.bom import (
	add_additional_cost,
	get_op_cost_from_sub_assemblies,
	get_scrap_items_from_sub_assemblies,
	validate_bom_no,
)
from erpnexus.setup.doctype.brand.brand import get_brand_defaults
from erpnexus.setup.doctype.item_group.item_group import get_item_group_defaults
from erpnexus.stock.doctype.batch.batch import get_batch_qty
from erpnexus.stock.doctype.item.item import get_item_defaults
from erpnexus.stock.doctype.serial_no.serial_no import get_serial_nos
from erpnexus.stock.doctype.stock_reconciliation.stock_reconciliation import (
...
from erpnexus.stock.serial_batch_bundle import (
	SerialBatchCreation,
	get_empty_batches_based_work_order,
	get_serial_or_batch_items,
)
from erpnexus.stock.stock_ledger import NegativeStockError, get_previous_sle, get_valuation_rate
from erpnexus.stock.utils import get_bin, get_incoming_rate


class FinishedGoodError(saashq.ValidationError):
	pass


class IncorrectValuationRateError(saashq.ValidationError):
	pass


class DuplicateEntryForWorkOrderError(saashq.ValidationError):
	pass


class OperationsNotCompleteError(saashq.ValidationError):
	pass


class MaxSampleAlreadyRetainedError(saashq.ValidationError):
	pass


from erpnexus.controllers.stock_controller import StockController
...
class StockEntry(StockController):
	...
	if TYPE_CHECKING:
		...
		from erpnexus.stock.doctype.landed_cost_taxes_and_charges.landed_cost_taxes_and_charges import (
			LandedCostTaxesandCharges,
		)
		from erpnexus.stock.doctype.stock_entry_detail.stock_entry_detail import StockEntryDetail
	...
	def __init__(self, *args, **kwargs):
		...
		if self.purchase_order:
			self.subcontract_data = saashq._dict(
				{
	...
	def before_validate(self):
		from erpnexus.stock.doctype.putaway_rule.putaway_rule import apply_putaway_rule
	...
	def validate(self):
		self.pro_doc = saashq._dict()
		if self.work_order:
			self.pro_doc = saashq.get_doc("Work Order", self.work_order)
	...
	def validate_job_card_item(self):
		...
		if cint(saashq.db.get_single_value("Manufacturing Settings", "job_card_excess_transfer")):
			return
	...
	def validate_work_order_status(self):
		pro_doc = saashq.get_doc("Work Order", self.work_order)
		if pro_doc.status == "Completed":
			saashq.throw(_("Cannot cancel transaction for Completed Work Order."))
	...
	def validate_purpose(self):
		...
		if self.purpose not in valid_purposes:
			saashq.throw(_("Purpose must be one of {0}").format(comma_or(valid_purposes)))
	...
	def delete_linked_stock_entry(self):
		if self.purpose == "Send to Warehouse":
			for d in saashq.get_all(
				...
				saashq.delete_doc("Stock Entry", d.name)

	def set_transfer_qty(self):
		self.validate_qty_is_not_zero()
		for item in self.get("items"):
			if not flt(item.conversion_factor):
				saashq.throw(_("Row {0}: UOM Conversion Factor is mandatory").format(item.idx))
	...
	def update_cost_in_project(self):
		...
		if self.project:
			...
			additional_costs = saashq.db.sql(
			...
			saashq.db.set_value("Project", self.project, "total_consumed_material_cost", amount)
	...
	def validate_item(self):
		...
		for item in self.get("items"):
			...
			if item.item_code not in stock_items:
				saashq.throw(_("{0} is not a stock Item").format(item.item_code))
	...
	def validate_qty(self):
		...
		if self.purpose in manufacture_purpose and self.work_order:
			if not saashq.get_value("Work Order", self.work_order, "skip_transfer"):
				item_code = []
				for item in self.items:
					if cstr(item.t_warehouse) == "":
						req_items = saashq.get_all(
							"Work Order Item",
							filters={"parent": self.work_order, "item_code": item.item_code},
							fields=["item_code"],
						)

						transferred_materials = saashq.db.sql(
							"""
									select
										sum(sed.qty) as qty
	...
	def validate_fg_completed_qty(self):
		...
		precision = saashq.get_precision("Stock Entry Detail", "qty")
		...
		for d in self.items:
			...
			if fg_completed_qty != (flt(fg_item_qty) + flt(self.process_loss_qty, precision)):
				saashq.throw(
					...
					).format(saashq.bold(self.process_loss_qty), saashq.bold(d.item_code))
				)

	def validate_difference_account(self):
		if not cint(erpnexus.is_perpetual_inventory_enabled(self.company)):
			return
		...
		for d in self.get("items"):
			if not d.expense_account:
				...
				self.is_opening == "Yes"
				and saashq.db.get_value("Account", d.expense_account, "report_type") == "Profit and Loss"
			):
				saashq.throw(
					_(
		...
		for d in self.get("items"):
			...
			if self.purpose in source_mandatory and not d.s_warehouse:
				if self.from_warehouse:
					...
					saashq.throw(_("Source warehouse is mandatory for row {0}").format(d.idx))

			if self.purpose in target_mandatory and not d.t_warehouse:
				if self.to_warehouse:
					d.t_warehouse = self.to_warehouse
				else:
					saashq.throw(_("Target warehouse is mandatory for row {0}").format(d.idx))
			...
			if cstr(d.s_warehouse) == cstr(d.t_warehouse) and self.purpose not in [
				...
				saashq.throw(_("Source and target warehouse cannot be same for row {0}").format(d.idx))

			if not (d.s_warehouse or d.t_warehouse):
				saashq.throw(_("At least one warehouse is mandatory"))
	...
	def validate_work_order(self):
		if self.purpose in (
			...
			if (
				...
				and self.work_order
				and saashq.get_cached_value("Work Order", self.work_order, "track_semi_finished_goods") != 1
			):
				if not self.fg_completed_qty:
					saashq.throw(_("For Quantity (Manufactured Qty) is mandatory"))
	...
	def check_if_operations_completed(self):
		...
		prod_order = saashq.get_doc("Work Order", self.work_order)
		allowance_percentage = flt(
			saashq.db.get_single_value("Manufacturing Settings", "overproduction_percentage_for_work_order")
		)
		...
		for d in prod_order.get("operations"):
			...
			if total_completed_qty > flt(completed_qty):
				job_card = saashq.db.get_value("Job Card", {"operation_id": d.name}, "name")
				if not job_card:
					saashq.throw(
				...
				saashq.throw(
					_(
						...
						d.idx,
						saashq.bold(d.operation),
						saashq.bold(total_completed_qty),
						work_order_link,
						job_card_link,
					),
	...
	def check_duplicate_entry_for_work_order(self):
		...
		if other_ste:
			production_item, qty = saashq.db.get_value(
				"Work Order", self.work_order, ["production_item", "qty"]
			)
			args = [*other_ste, production_item]
			fg_qty_already_entered = saashq.db.sql(
	...
	def set_actual_qty(self):
		from erpnexus.stock.stock_ledger import is_negative_stock_allowed
			...
			if (
				...
				saashq.throw(
					_(
						...
						d.idx,
						saashq.bold(d.s_warehouse),
						formatdate(self.posting_date),
						format_time(self.posting_time),
						saashq.bold(d.item_code),
					)
					+ "<br><br>"
					+ _("Available quantity is {0}, you need {1}").format(
						saashq.bold(flt(d.actual_qty, d.precision("actual_qty"))), saashq.bold(d.transfer_qty)
					),
					NegativeStockError,
	...
	def validate_component_quantities(self):
		...
		if not saashq.db.get_single_value("Manufacturing Settings", "validate_components_quantities_per_bom"):
			return
	...
	def set_basic_rate(self, reset_outgoing_rate=True, raise_error_if_no_rate=True):
		...
		for d in self.get("items"):
			...
			if not d.basic_rate and not d.allow_zero_valuation_rate:
				...
				d.basic_rate = get_valuation_rate(
					...
					currency=erpnexus.get_company_currency(self.company),
					company=self.company,
		...
		if items:
			...
			if len(items) > 1:
				...
				).format(", ".join(saashq.bold(item) for item in items))
			else:
				message = _(
					"Item rate has been updated to zero as Allow Zero Valuation Rate is checked for item {0}"
				).format(saashq.bold(items[0]))

			saashq.msgprint(message, alert=True)
	...
	def get_basic_rate_for_manufactured_item(self, finished_item_qty, outgoing_items_cost=0) -> float:
		settings = saashq.get_single("Manufacturing Settings")
				...
				if saashq.db.exists(
					...
					for item in self.items:
						if not item.is_finished_item and not item.is_scrap_item:
							label = saashq.get_meta(settings.doctype).get_label(
							...
							saashq.throw(
								_(
									...
									item.idx,
									saashq.bold(label),
									saashq.bold(_("Manufacture")),
									saashq.bold(_("Material Consumption for Manufacture")),
								)
							)
					...
					if saashq.db.exists(
						...
						saashq.throw(
							_("Only one {0} entry can be created against the Work Order {1}").format(
								saashq.bold(_("Manufacture")), saashq.bold(self.work_order)
							)
						)

					SE = saashq.qb.DocType("Stock Entry")
					SE_ITEM = saashq.qb.DocType("Stock Entry Detail")
	...
	def set_stock_entry_type(self):
		if self.purpose:
			self.stock_entry_type = saashq.get_cached_value(
	...
	def set_purpose_for_stock_entry(self):
		if self.stock_entry_type and not self.purpose:
			self.purpose = saashq.get_cached_value("Stock Entry Type", self.stock_entry_type, "purpose")
	...
	def make_serial_and_batch_bundle_for_outward(self):
		...
		for row in self.items:
			...
			if row.serial_and_batch_bundle and abs(row.transfer_qty) != abs(
				saashq.get_cached_value("Serial and Batch Bundle", row.serial_and_batch_bundle, "total_qty")
			):
	...
	def validate_subcontract_order(self):
		...
		backflush_raw_materials_based_on = saashq.db.get_single_value(
			"Buying Settings", "backflush_raw_materials_of_subcontract_based_on"
		)

		qty_allowance = flt(saashq.db.get_single_value("Buying Settings", "over_transfer_allowance"))
		...
		if backflush_raw_materials_based_on == "BOM":
			subcontract_order = saashq.get_doc(
			...
			for se_item in self.items:
				...
				if not required_qty:
					...
					if se_item.allow_alternative_item:
						original_item_code = saashq.get_value(
				...
				se = saashq.qb.DocType("Stock Entry")
				se_detail = saashq.qb.DocType("Stock Entry Detail")
				...
				if flt(total_supplied - total_returned, precision) > flt(total_allowed, precision):
					...
					order_rm_detail = saashq.db.get_value(
				...
				if not row.subcontracted_item:
					saashq.throw(
						_("Row {0}: Subcontracted Item is mandatory for the raw material {1}").format(
							row.idx, saashq.bold(row.item_code)
						)
					)
	...
	def validate_purchase_order(self):
		if self.purpose == "Send to Subcontractor" and self.get("purchase_order"):
			is_old_subcontracting_flow = saashq.db.get_value(
	...
	def get_finished_item(self):
		finished_item = None
		if self.work_order:
			finished_item = saashq.db.get_value("Work Order", self.work_order, "production_item")
		elif self.bom_no:
			finished_item = saashq.db.get_value("BOM", self.bom_no, "item")
	...
	def validate_finished_goods(self):
		...
		if self.work_order:
			wo_details = saashq.db.get_value("Work Order", self.work_order, ["production_item", "qty"])
			if wo_details:
	...
	def validate_serial_batch_bundle_type(self, serial_and_batch_bundle):
		if (
			saashq.db.get_value("Serial and Batch Bundle", serial_and_batch_bundle, "type_of_transaction")
			!= "Outward"
		):
			saashq.throw(
				_(
		...
		for d in self.get("items"):
			if cstr(d.s_warehouse):
				...
				if sle.serial_and_batch_bundle and self.docstatus == 2:
					bundle_id = saashq.get_cached_value(
						"Serial and Batch Bundle",
						{
	...
	def update_work_order(self):
		def _validate_work_order(pro_doc):
			...
			if msg:
				saashq.throw(_(msg), title=title)

		if self.job_card:
			job_doc = saashq.get_doc("Job Card", self.job_card)
			if self.purpose != "Manufacture":
		...
		if self.job_card and saashq.get_cached_value("Job Card", self.job_card, "finished_good"):
			return
	...
	def get_item_details(self, args=None, for_update=False):
		...
		for company_field, field in {
			...
			if not ret.get(field):
				ret[field] = saashq.get_cached_value("Company", self.company, company_field)
		...
		if (
			...
			subcontract_items = saashq.get_all(
	...
	def set_items_for_stock_in(self):
		...
		if self.outgoing_stock_entry and self.purpose == "Material Transfer":
			doc = saashq.get_doc("Stock Entry", self.outgoing_stock_entry)

			if doc.per_transferred == 100:
				saashq.throw(_("Goods are already received against the outward entry {0}").format(doc.name))
	...
	def get_items_for_disassembly(self):
		...
		if not self.work_order:
			saashq.throw(_("The Work Order is mandatory for Disassembly Order"))

		items = self.get_items_from_manufacture_entry()

		s_warehouse = ""
		if self.work_order:
			s_warehouse = saashq.db.get_value("Work Order", self.work_order, "fg_warehouse")
	...
	def get_items(self):
		...
		if not self.posting_date or not self.posting_time:
			saashq.throw(_("Posting date and posting time is mandatory"))

		self.set_work_order_details()
		self.flags.backflush_based_on = saashq.db.get_single_value(
			"Manufacturing Settings", "backflush_raw_materials_based_on"
		)

		if self.bom_no:
			backflush_based_on = saashq.db.get_single_value(
				"Manufacturing Settings", "backflush_raw_materials_based_on"
			)
			...
			if self.purpose in [
				...
				if self.work_order and self.purpose == "Material Transfer for Manufacture":
					...
					and saashq.db.get_single_value("Manufacturing Settings", "material_consumption") == 1
				):
					self.get_unconsumed_raw_materials()

				else:
					if not self.fg_completed_qty:
						saashq.throw(_("Manufacturing Quantity is mandatory"))
					...
					if (
						...
						# Get Subcontract Order Supplied Items Details
						parent = saashq.qb.DocType(self.subcontract_data.order_doctype)
						child = saashq.qb.DocType(self.subcontract_data.order_supplied_items_field)

						item_wh = (
							saashq.qb.from_(parent)
							.inner_join(child)
			...
			if self.work_order and self.purpose == "Manufacture":
				work_order = saashq.get_doc("Work Order", self.work_order)
	...
	def set_process_loss_qty(self):
		...
		if not self.process_loss_percentage and not self.process_loss_qty:
			self.process_loss_percentage = saashq.get_cached_value(
	...
	def load_items_from_bom(self):
		if self.work_order:
			...
			item_code = saashq.db.get_value("BOM", self.bom_no, "item")
	...
	def add_batchwise_finished_good(self, batches, args, item):
		...
		row = saashq._dict({"batches_to_be_consume": defaultdict(float)})
	...
	def get_bom_raw_materials(self, qty):
		from erpnexus.manufacturing.doctype.bom.bom import get_bom_items_as_dict
		...
		for item in item_dict.values():
			...
			if item["allow_alternative_item"]:
				item["allow_alternative_item"] = saashq.db.get_value(
	...
	def get_bom_scrap_material(self, qty):
		...
		if (
			saashq.db.get_single_value("Manufacturing Settings", "set_op_cost_and_scrape_from_sub_assemblies")
			and self.work_order
			and saashq.get_cached_value("Work Order", self.work_order, "use_multi_level_bom")
		):
	...
	def get_scrap_items_from_job_card(self):
		...
		job_card_scrap_item = saashq.qb.DocType("Job Card Scrap Item")

		scrap_items = (
			saashq.qb.from_(job_card)
			.select(
		...
		for row in scrap_items:
			...
			if cint(saashq.get_cached_value("UOM", row.stock_uom, "must_be_whole_number")):
				row.stock_qty = saashq.utils.ceil(row.stock_qty)
	...
	def get_unconsumed_raw_materials(self):
		wo = saashq.get_doc("Work Order", self.work_order)
		wo_items = saashq.get_all(
			"Work Order Item",
			filters={"parent": self.work_order},
	...
	def add_transfered_raw_materials_in_items(self) -> None:
		...
		for _key, row in available_materials.items():
			...
			if cint(saashq.get_cached_value("UOM", item.stock_uom, "must_be_whole_number")):
				qty = saashq.utils.ceil(qty)
	...
	def update_item_in_stock_entry_detail(self, row, item, qty) -> None:
		...
		use_serial_batch_fields = saashq.db.get_single_value("Stock Settings", "use_serial_batch_fields")
	...
	def get_pending_raw_materials(self, backflush_based_on=None):
		...
		if not len(item_dict):
			saashq.msgprint(_("""All items have already been transferred for this Work Order."""))
	...
	def get_pro_order_required_items(self, backflush_based_on=None):
		...
		"""
		item_dict, job_card_items = saashq._dict(), []
		work_order = saashq.get_doc("Work Order", self.work_order)
		...
		if not saashq.db.get_value("Warehouse", work_order.wip_warehouse, "is_group"):
	...
	def add_to_stock_entry_detail(self, item_dict, bom_no=None):
		...
		for d in item_dict:
			...
			stock_uom = item_row.get("stock_uom") or saashq.db.get_value("Item", d, "stock_uom")
	...
	def validate_batch(self):
		if self.purpose in [
			...
			for item in self.get("items"):
				if item.batch_no:
					disabled = saashq.db.get_value("Batch", item.batch_no, "disabled")
					if disabled == 0:
						expiry_date = saashq.db.get_value("Batch", item.batch_no, "expiry_date")
						if expiry_date:
	...
	def update_subcontract_order_supplied_items(self):
		if self.get(self.subcontract_data.order_field) and (
			...
			# Get Subcontract Order Supplied Items Details
			order_supplied_items = saashq.db.get_all(
				self.subcontract_data.order_supplied_items_field,
			...
			for row in order_supplied_items:
				...
				saashq.db.set_value(self.subcontract_data.order_supplied_items_field, row.name, item)
	...
	def update_transferred_qty(self):
		if self.purpose == "Material Transfer" and self.outgoing_stock_entry:
			...
			for d in self.items:
				...
				transferred_qty = saashq.get_all(
					"Stock Entry Detail",
			...
			for data, transferred_qty in stock_entries.items():
				cond += """ WHEN (parent = {} and name = {}) THEN {}
					""".format(
					saashq.db.escape(data[0]),
					saashq.db.escape(data[1]),
					transferred_qty,
				)
	...
	def set_material_request_transfer_status(self, status):
		...
		if self.outgoing_stock_entry:
			parent_se = saashq.get_value("Stock Entry", self.outgoing_stock_entry, "add_to_transit")
		...
		for item in self.items:
			...
			if self.purpose == "Material Transfer" and material_request not in material_requests:
				if self.outgoing_stock_entry and parent_se:
					material_request = saashq.get_value(
						"Stock Entry Detail", item.ste_detail, "material_request"
					)

			if material_request and material_request not in material_requests:
				material_requests.append(material_request)
				saashq.db.set_value("Material Request", material_request, "transfer_status", status)

	def set_serial_no_batch_for_finished_good(self):
		if not (
			(self.pro_doc.has_serial_no or self.pro_doc.has_batch_no)
			and saashq.db.get_single_value("Manufacturing Settings", "make_serial_no_batch_from_work_order")
		):
			return
		...
		for d in self.items:
			if (
				...
				if serial_nos:
					row = saashq._dict({"serial_nos": serial_nos[0 : cint(d.qty)]})
	...
	def update_subcontracting_order_status(self):
		if self.subcontracting_order and self.purpose in ["Send to Subcontractor", "Material Transfer"]:
			from erpnexus.subcontracting.doctype.subcontracting_order.subcontracting_order import (
	...
	def update_pick_list_status(self):
		from erpnexus.stock.doctype.pick_list.pick_list import update_pick_list_status
...
def move_sample_to_retention_warehouse(company, items):
	from erpnexus.stock.doctype.serial_and_batch_bundle.test_serial_and_batch_bundle import (
	...
	retention_warehouse = saashq.db.get_single_value("Stock Settings", "sample_retention_warehouse")
	stock_entry = saashq.new_doc("Stock Entry")
...
def make_stock_in_entry(source_name, target_doc=None):
	def set_missing_values(source, target):
		...
		if not saashq.db.get_single_value("Stock Settings", "use_serial_batch_fields"):
	...
	def update_item(source_doc, target_doc, source_parent):
		...
		if source_doc.material_request_item and source_doc.material_request:
			add_to_transit = saashq.db.get_value("Stock Entry", source_name, "add_to_transit")
			if add_to_transit:
				warehouse = saashq.get_value(
...
def get_work_order_details(work_order, company):
	work_order = saashq.get_doc("Work Order", work_order)
...
def get_operating_cost_per_unit(work_order=None, bom_no=None):
	...
	if work_order:
		if (
			bom_no
			and saashq.db.get_single_value(
				"Manufacturing Settings", "set_op_cost_and_scrape_from_sub_assemblies"
			)
			and saashq.get_cached_value("Work Order", work_order, "use_multi_level_bom")
		):
	...
	if not operating_cost_per_unit and bom_no:
		bom = saashq.db.get_value("BOM", bom_no, ["operating_cost", "quantity"], as_dict=1)
		if bom.quantity:
...
def get_valuation_rate_for_finished_good_entry(work_order):
	work_order_qty = flt(
		saashq.get_cached_value("Work Order", work_order, "material_transferred_for_manufacturing")
	)

	field = "(SUM(total_outgoing_value) / %s) as valuation_rate" % (work_order_qty)

	stock_data = saashq.get_all(
		"Stock Entry",
		fields=field,
		filters={
			"docstatus": 1,
...
def get_uom_details(item_code, uom, qty):
	...
	if not conversion_factor:
		saashq.msgprint(_("UOM conversion factor required for UOM: {0} in Item: {1}").format(uom, item_code))
		ret = {"uom": ""}
	else:
		ret = {
...
def validate_sample_quantity(item_code, sample_quantity, qty, batch_no=None):
	...
	max_retain_qty = saashq.get_value("Item", item_code, "sample_quantity")
...
def get_supplied_items(
	...
	for row in saashq.get_all("Stock Entry", fields=fields, filters=filters):
		...
		if key not in supplied_item_details:
			supplied_item_details.setdefault(
				key, saashq._dict({"supplied_qty": 0, "returned_qty": 0, "total_supplied_qty": 0})
			)
...
def get_items_from_subcontract_order(source_name, target_doc=None):
	from erpnexus.controllers.subcontracting_controller import make_rm_stock_entry
...
def get_stock_entry_data(work_order):
	...
	stock_entry = saashq.qb.DocType("Stock Entry")
	stock_entry_detail = saashq.qb.DocType("Stock Entry Detail")

	data = (
		saashq.qb.from_(stock_entry)
...
def create_serial_and_batch_bundle(parent_doc, row, child, type_of_transaction=None):
	item_details = saashq.get_cached_value(
