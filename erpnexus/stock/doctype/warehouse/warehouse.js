...
saashq.ui.form.on("Warehouse", {
	...
	refresh: function (frm) {
		...
		if (!frm.is_new()) {
			...
			if ("Stock Balance" in saashq.boot.user.all_reports) {
		...
		if (
			...
			"General Ledger" in saashq.boot.user.all_reports
		) {
...
function convert_to_group_or_ledger(frm) {
	saashq.call({
		method: "erpnexus.stock.doctype.warehouse.warehouse.convert_to_group_or_ledger",
		args: {
