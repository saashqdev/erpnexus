...
from erpnexus.stock.doctype.warehouse.warehouse import convert_to_group_or_ledger, get_children
...
class TestWarehouse(IntegrationTestCase):
	def test_parent_warehouse(self):
		parent_warehouse = saashq.get_doc("Warehouse", "_Test Warehouse Group - _TC")
		self.assertEqual(parent_warehouse.is_group, 1)

	def test_warehouse_hierarchy(self):
		p_warehouse = saashq.get_doc("Warehouse", "_Test Warehouse Group - _TC")

		child_warehouses = saashq.db.sql(
	...
	def test_naming(self):
		...
		wh = saashq.get_doc(doctype="Warehouse", warehouse_name=warehouse_name, company=company).insert()
	...
	def test_unlinking_warehouse_from_item_defaults(self):
		...
		for warehouse in warehouse_ids:
			saashq.delete_doc("Warehouse", warehouse)

		# Check Item existance
		for item in item_names:
			self.assertTrue(bool(saashq.db.exists("Item", item)), f"{item} doesn't exist")
	...
	def test_group_non_group_conversion(self):
		warehouse = saashq.get_doc("Warehouse", create_warehouse("TestGroupConversion"))
		...
		# chid exists
		self.assertRaises(saashq.ValidationError, convert_to_group_or_ledger, warehouse.name)
		saashq.delete_doc("Warehouse", child)
...
def create_warehouse(warehouse_name, properties=None, company=None):
	...
	warehouse_id = erpnexus.encode_company_abbr(warehouse_name, company)
	if not saashq.db.exists("Warehouse", warehouse_id):
		w = saashq.new_doc("Warehouse")
...
def get_warehouse(**args):
	...
	if saashq.db.exists("Warehouse", args.warehouse_name + " - " + args.abbr):
		return saashq.get_doc("Warehouse", args.warehouse_name + " - " + args.abbr)
	else:
		w = saashq.get_doc(
			{
...
def get_warehouse_account(warehouse_name, company, company_abbr=None):
	...
	if not saashq.db.exists("Account", warehouse_name + " - " + company_abbr):
...
def get_group_stock_account(company, company_abbr=None):
	group_stock_account = saashq.db.get_value(
