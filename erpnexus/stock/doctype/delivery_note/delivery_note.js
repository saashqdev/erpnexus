...
saashq.ui.form.on("Delivery Note", {
	setup: function (frm) {
		...
		erpnexus.queries.setup_warehouse_query(frm);
		...
		frm.set_query("expense_account", "items", function (doc, cdt, cdn) {
			if (erpnexus.is_perpetual_inventory_enabled(doc.company)) {
				return {
					filters: {
	...
	print_without_amount: function (frm) {
		erpnexus.stock.delivery_note.set_print_hide(frm.doc);
	},
	...
	refresh: function (frm) {
		if (
			...
			saashq.model.can_create("Sales Invoice")
		) {
		...
		if (
			...
			saashq.model.can_create("Purchase Receipt")
		) {
			...
			if (internal) {
				...
				frm.add_custom_button(
					...
					function () {
						saashq.model.open_mapped_doc({
							method: "erpnexus.stock.doctype.delivery_note.delivery_note.make_inter_company_purchase_receipt",
							frm: frm,
						});
					},
					__("Create")
...
saashq.ui.form.on("Delivery Note Item", {
...
erpnexus.stock.DeliveryNoteController = class DeliveryNoteController extends (
	...
	refresh(doc, dt, dn) {
		...
		if (
			...
			saashq.model.can_read("Sales Order") &&
		...
		if (!doc.is_return && doc.status != "Closed") {
			if (doc.docstatus == 1 && saashq.model.can_create("Shipment")) {
				this.frm.add_custom_button(
					__("Shipment"),
					function () {
						me.make_shipment();
			...
			if (
				...
				doc.docstatus == 1 &&
				saashq.model.can_create("Installation Note")
			) {
				this.frm.add_custom_button(
			...
			if (doc.docstatus == 1 && doc.status != "Completed" && saashq.model.can_create("Delivery Trip")) {
				this.frm.add_custom_button(
					__("Delivery Trip"),
					function () {
			...
			if (
				...
				saashq.model.can_create("Packing Slip")
			) {
				this.frm.add_custom_button(
					__("Packing Slip"),
					function () {
						saashq.model.open_mapped_doc({
							method: "erpnexus.stock.doctype.delivery_note.delivery_note.make_packing_slip",
							frm: me.frm,
						});
					},
					__("Create")
		...
		erpnexus.stock.delivery_note.set_print_hide(doc, dt, dn);
	}

	make_shipment() {
		saashq.model.open_mapped_doc({
			method: "erpnexus.stock.doctype.delivery_note.delivery_note.make_shipment",
			frm: this.frm,
		});
	}
	...
	make_sales_return() {
		saashq.model.open_mapped_doc({
			method: "erpnexus.stock.doctype.delivery_note.delivery_note.make_sales_return",
			frm: this.frm,
		});
	}
	...
	make_delivery_trip() {
		saashq.model.open_mapped_doc({
			method: "erpnexus.stock.doctype.delivery_note.delivery_note.make_delivery_trip",
			frm: cur_frm,
		});
	}
	...
	update_status(status) {
		...
		saashq.call({
			method: "erpnexus.stock.doctype.delivery_note.delivery_note.update_delivery_note_status",
...
extend_cscript(cur_frm.cscript, new erpnexus.stock.DeliveryNoteController({ frm: cur_frm }));
...
saashq.ui.form.on("Delivery Note", {
	...
	unhide_account_head: function (frm) {
		...
		var aii_enabled = erpnexus.is_perpetual_inventory_enabled(frm.doc.company);
		frm.fields_dict["items"].grid.set_column_disp(["expense_account", "cost_center"], aii_enabled);
	},
});

erpnexus.stock.delivery_note.set_print_hide = function (doc, cdt, cdn) {
	var dn_fields = saashq.meta.docfield_map["Delivery Note"];
	var dn_item_fields = saashq.meta.docfield_map["Delivery Note Item"];
