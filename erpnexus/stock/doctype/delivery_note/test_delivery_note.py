...
from saashq.utils import add_days, cstr, flt, getdate, nowdate, nowtime, today
...
from erpnexus.stock.doctype.delivery_note.delivery_note import (
	make_delivery_trip,
	make_sales_invoice,
)
from erpnexus.stock.doctype.delivery_trip.test_delivery_trip import create_driver
from erpnexus.stock.doctype.item.test_item import make_item
from erpnexus.stock.doctype.purchase_receipt.test_purchase_receipt import get_gl_entries
...
class TestDeliveryNote(IntegrationTestCase):
	...
	def test_over_billing_against_dn(self):
		...
		self.assertRaises(saashq.ValidationError, make_sales_invoice, dn.name)
		...
		self.assertRaises(saashq.ValidationError, saashq.get_doc(si).insert)

	def test_delivery_note_no_gl_entry(self):
		saashq.db.get_value("Warehouse", "_Test Warehouse - _TC", "company")
		...
		sle = saashq.get_doc("Stock Ledger Entry", {"voucher_type": "Delivery Note", "voucher_no": dn.name})
	...
	def test_delivery_note_gl_entry_packing_item(self):
		saashq.db.get_value("Warehouse", "Stores - TCP1", "company")
	...
	def test_delivery_note_return_against_denormalized_serial_no(self):
		from erpnexus.stock.doctype.delivery_note.delivery_note import make_sales_return
		from erpnexus.stock.doctype.serial_no.serial_no import get_serial_nos

		saashq.flags.ignore_serial_batch_bundle_validation = True
		...
		for sn in serial_nos:
			if not saashq.db.exists("Serial No", sn):
				sn_doc = saashq.get_doc(
					{
						"doctype": "Serial No",
						"item_code": sn_item,
						"serial_no": sn,
		...
		saashq.flags.ignore_serial_batch_bundle_validation = False
	...
	def test_sales_return_for_non_bundled_items_partial(self):
		...
		returned = saashq.get_doc("Delivery Note", dn1.name)
	...
	def test_sales_return_against_serial_batch_bundle(self):
		...
		for row in return_dn.items:
			total_qty = saashq.db.get_value(
				"Serial and Batch Bundle", row.serial_and_batch_bundle, "total_qty"
			)

			self.assertEqual(total_qty, 2)

			doc = saashq.get_doc("Serial and Batch Bundle", row.serial_and_batch_bundle)
	...
	def test_delivery_of_bundled_items_to_target_warehouse(self):
		...
		stock_value_difference1 = saashq.db.get_value(
			"Stock Ledger Entry",
			{
				"voucher_type": "Delivery Note",
				"voucher_no": dn.name,
	...
	def test_closed_delivery_note(self):
		from erpnexus.stock.doctype.delivery_note.delivery_note import update_delivery_note_status
		...
		self.assertEqual(saashq.db.get_value("Delivery Note", dn.name, "Status"), "Closed")
	...
	def test_sales_order_reference_validation(self):
		...
		self.assertRaises(saashq.ValidationError, dn.save)
		dn.reload()
	...
	def test_dn_billing_status_case3(self):
		...
		from erpnexus.selling.doctype.sales_order.sales_order import make_delivery_note
		from erpnexus.selling.doctype.sales_order.sales_order import (
	...
	def test_dn_billing_status_case4(self):
		# SO -> SI -> DN
		from erpnexus.accounts.doctype.sales_invoice.sales_invoice import make_delivery_note
	...
	def test_delivery_note_bundle_with_batched_item(self):
		...
		reposting_entries = saashq.get_all("Repost Item Valuation", filters={"docstatus": 1})
		for entry in reposting_entries:
			doc = saashq.get_doc("Repost Item Valuation", entry.name)
			doc.cancel()
			doc.delete()

		saashq.db.set_single_value("Accounts Settings", "delete_linked_ledger_entries", 1)

		dn.reload()
		dn.delete()

		bundle = saashq.db.get_value("Serial and Batch Bundle", {"voucher_detail_no": packed_name}, "name")
		self.assertFalse(bundle)

		saashq.db.set_single_value("Stock Settings", "use_serial_batch_fields", 1)
		saashq.db.set_single_value("Accounts Settings", "delete_linked_ledger_entries", 0)
	...
	def test_internal_transfer_precision_gle(self):
		...
		self.assertFalse(saashq.db.exists("GL Entry", {"voucher_no": dn.name, "voucher_type": dn.doctype}))
	...
	def test_reserve_qty_on_sales_return(self):
		saashq.db.set_single_value("Selling Settings", "dont_reserve_sales_order_qty_on_sales_return", 0)
		self.reserved_qty_check()

	def test_dont_reserve_qty_on_sales_return(self):
		saashq.db.set_single_value("Selling Settings", "dont_reserve_sales_order_qty_on_sales_return", 1)
		self.reserved_qty_check()

	def reserved_qty_check(self):
		from erpnexus.controllers.sales_and_purchase_return import make_return_doc
		from erpnexus.selling.doctype.sales_order.sales_order import make_delivery_note
		from erpnexus.stock.stock_balance import get_reserved_qty

		dont_reserve_qty = saashq.db.get_single_value(
		...
		returned = saashq.get_doc("Delivery Note", dn_return.name)
		returned.update_prevdoc_status()
	...
	def test_sales_return_valuation_for_moving_average_case2(self):
		...
		saashq.flags.print_debug_messages = False
		item_code = make_item(
	...
	def test_batch_with_non_stock_uom(self):
		...
		if not saashq.db.exists("UOM Conversion Detail", {"parent": item.name, "uom": "Kg"}):
		...
		delivered_batch_qty = saashq.db.get_value(
	...
	def test_delivery_note_legacy_serial_no_valuation(self):
		...
		sle_data = saashq.get_all(
			"Stock Ledger Entry",
			filters={"voucher_no": se_doc.name, "voucher_type": "Stock Entry"},
		)[0]

		sle_doc = saashq.get_doc("Stock Ledger Entry", sle_data.name)
		...
		for sn in serial_nos:
			sn_doc = saashq.get_doc("Serial No", sn)
			sn_doc.db_set(
				{
					"status": "Active",
					"warehouse": warehouse,
	...
	def test_same_posting_date_and_posting_time(self):
		...
		self.assertRaises(saashq.ValidationError, dn5.submit)
	...
	def test_warranty_expiry_date_for_serial_item(self):
		...
		for row in serial_nos:
			sn = saashq.get_doc("Serial No", row)
...
def create_delivery_note(**args):
	dn = saashq.new_doc("Delivery Note")
	...
	if not args.use_serial_batch_fields and (args.get("batch_no") or args.get("serial_no")):
		...
		if args.get("batches"):
			batches = saashq._dict(args.batches)
