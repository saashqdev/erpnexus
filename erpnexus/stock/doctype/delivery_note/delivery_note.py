...
from erpnexus.controllers.accounts_controller import get_taxes_and_charges, merge_taxes
from erpnexus.controllers.selling_controller import SellingController
from erpnexus.stock.doctype.serial_no.serial_no import get_delivery_note_serial_no
...
class DeliveryNote(SellingController):
	...
	if TYPE_CHECKING:
		...
		from erpnexus.stock.doctype.delivery_note_item.delivery_note_item import DeliveryNoteItem
	...
	def before_print(self, settings=None):
		...
		item_meta = saashq.get_meta("Delivery Note Item")
		print_hide_fields = {
	...
	def so_required(self):
		...
		if saashq.db.get_single_value("Selling Settings", "so_required") == "Yes":
			for d in self.get("items"):
				if not d.against_sales_order:
					saashq.throw(_("Sales Order required for Item {0}").format(d.item_code))
	...
	def set_serial_and_batch_bundle_from_pick_list(self):
		...
		for item in self.items:
			...
			if item.pick_list_item and not item.serial_and_batch_bundle:
				...
				bundle_id = saashq.db.get_value("Serial and Batch Bundle", filters, "name")
			...
			if missing_label and missing_label != "No Label":
				errors.append(
					_("The field {0} in row {1} is not set").format(
						saashq.bold(_(missing_label)), saashq.bold(item.idx)
					)
				)

		if errors:
			saashq.throw("<br>".join(errors), title=error_title)
	...
	def validate_warehouse(self):
		...
		for d in self.get_item_list():
			if not d["warehouse"] and saashq.get_cached_value("Item", d["item_code"], "is_stock_item") == 1:
				saashq.throw(_("Warehouse required for stock Item {0}").format(d["item_code"]))

	def update_current_stock(self):
		if self.get("_action") and self._action != "update_after_submit":
			for d in self.get("items"):
				d.actual_qty = saashq.db.get_value(
		...
		if self._action == "submit":
			for item in self.get("items"):
				...
				for sre in sre_list:
					...
					sre_doc = saashq.get_doc("Stock Reservation Entry", sre)

					qty_can_be_deliver = 0
					if sre_doc.reservation_based_on == "Serial and Batch":
						sbb = saashq.get_doc("Serial and Batch Bundle", item.serial_and_batch_bundle)
			...
			if not item.warehouse:
				...
				if item.warehouse not in reserved_warehouses:
					msg = _("Row #{0}: Stock is reserved for item {1} in warehouse {2}.").format(
						item.idx,
						saashq.bold(item.item_code),
						saashq.bold(reserved_warehouses[0])
						if len(reserved_warehouses) == 1
						else _("{0} and {1}").format(
							saashq.bold(", ".join(reserved_warehouses[:-1])),
							saashq.bold(reserved_warehouses[-1]),
						),
					)
					saashq.throw(msg, title=_("Stock Reservation Warehouse Mismatch"))
	...
	def validate_packed_qty(self):
		...
		if saashq.db.exists("Packing Slip", {"docstatus": 1, "delivery_note": self.name}):
				...
				if (
					...
					saashq.throw(
						_("Row {0}: Packed Qty must be equal to {1} Qty.").format(
							item.idx, saashq.bold(item.doctype)
						)
					)
	...
	def check_next_docstatus(self):
		...
		if submit_rv:
			saashq.throw(_("Sales Invoice {0} has already been submitted").format(submit_rv[0][0]))

		submit_in = saashq.db.sql(
			"""select t1.name
		...
		if submit_in:
			saashq.throw(_("Installation Note {0} has already been submitted").format(submit_in[0][0]))
	...
	def cancel_packing_slips(self):
		...
		if res:
			for r in res:
				ps = saashq.get_doc("Packing Slip", r[0])
				ps.cancel()
			saashq.msgprint(_("Packing Slip(s) cancelled"))
	...
	def update_billing_status(self, update_modified=True):
		...
		for dn in set(updated_delivery_notes):
			dn_doc = self if (dn == self.name) else saashq.get_doc("Delivery Note", dn)
	...
	def make_return_invoice(self):
		try:
			...
			credit_note_link = saashq.utils.get_link_to_form("Sales Invoice", return_invoice.name)

			saashq.msgprint(_("Credit Note {0} has been created automatically").format(credit_note_link))
		except Exception:
			saashq.throw(
				_(
...
def update_billed_amount_based_on_so(so_detail, update_modified=True):
	...
	si_item = saashq.qb.DocType("Sales Invoice Item").as_("si_item")
	sum_amount = Sum(si_item.amount).as_("amount")

	billed_against_so = (
		saashq.qb.from_(si_item)
		.select(sum_amount)
		.where(
	...
	dn = saashq.qb.DocType("Delivery Note").as_("dn")
	dn_item = saashq.qb.DocType("Delivery Note Item").as_("dn_item")
	...
	for dnd in dn_details:
		...
		if dnd.si_detail:
			...
			billed_amt_against_dn = saashq.db.sql(
...
def get_invoiced_qty_map(delivery_note):
	...
	for dn_detail, qty in saashq.db.sql(
...
def make_sales_invoice(source_name, target_doc=None, args=None):
	doc = saashq.get_doc("Delivery Note", source_name)
	...
	def set_missing_values(source, target):
		...
		if len(target.get("items")) == 0:
			saashq.throw(_("All these items have already been Invoiced/Returned"))
...
def make_delivery_trip(source_name, target_doc=None, kwargs=None):
	if not target_doc:
		target_doc = saashq.new_doc("Delivery Trip")
		...
		source_name,
		{
			...
			"Delivery Note Item": {
				...
				"condition": lambda item: (
					not saashq.db.exists("Product Bundle", {"new_item_code": item.item_code, "disabled": 0})
					and flt(item.packed_qty) < flt(item.qty)
				),
			},
			"Packed Item": {
...
def make_shipment(source_name, target_doc=None):
	def postprocess(source, target):
		user = saashq.db.get_value(
			"User", saashq.session.user, ["email", "full_name", "phone", "mobile_no"], as_dict=1
		)
		...
		target.pickup_contact_person = saashq.session.user

		if source.contact_person:
			contact = saashq.db.get_value(
...
def update_delivery_note_status(docname, status):
	dn = saashq.get_doc("Delivery Note", docname)
...
def make_inter_company_transaction(doctype, source_name, target_doc=None):
	...
	def set_missing_values(source, target):
		...
		if not target.get("items"):
			saashq.throw(_("All items have already been received"))
