saashq.listview_settings["Delivery Note"] = {
	add_fields: [
		"customer",
		"customer_name",
		"base_grand_total",
	...
	onload: function (doclist) {
		const action = () => {
			...
			if (selected_docs.length > 0) {
				saashq.new_doc("Delivery Trip").then(() => {
		...
		doclist.page.add_action_item(__("Sales Invoice"), () => {
			erpnexus.bulk_transaction_processing.create(doclist, "Delivery Note", "Sales Invoice");
		});

		doclist.page.add_action_item(__("Packaging Slip From Delivery Note"), () => {
			erpnexus.bulk_transaction_processing.create(doclist, "Delivery Note", "Packing Slip");
		});
	},
};
