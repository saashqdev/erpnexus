...
import saashq
from saashq.core.page.permission_manager.permission_manager import reset
...
from saashq.utils import add_days, add_to_date, flt, today

from erpnexus.accounts.doctype.gl_entry.gl_entry import rename_gle_sle_docs
...
from erpnexus.stock.doctype.stock_ledger_entry.stock_ledger_entry import BackDatedStockTransaction
...
class TestStockLedgerEntry(IntegrationTestCase, StockTestMixin):
	...
	def test_item_cost_reposting(self):
		...
		target_wh_sle = saashq.db.get_value(
			"Stock Ledger Entry",
			{
				"item_code": "_Test Item for Reposting",
				"warehouse": "Finished Goods - _TC",
		...
		finished_item_sle = saashq.db.get_value(
			"Stock Ledger Entry",
			{
	...
	def test_purchase_return_valuation_reposting(self):
		...
		# check sle
		outgoing_rate, stock_value_difference = saashq.db.get_value(
			"Stock Ledger Entry",
	...
	def test_fifo_dependent_consumption(self):
		...
		for rate in rates[1:]:
			row = saashq.copy_doc(receipt.items[0], ignore_no_copy=False)
			row.basic_rate = rate
		...
		for _ in rates[1:]:
			row = saashq.copy_doc(transfer.items[0], ignore_no_copy=False)
	...
	def test_fifo_multi_item_repack_consumption(self):
		...
		for _ in rates[1:]:
			row = saashq.copy_doc(repack.items[0], ignore_no_copy=False)
	...
	def test_tie_breaking(self):
		from erpnexus.stock.doctype.repost_item_valuation.repost_item_valuation import repost_entries

		saashq.flags.dont_execute_stock_reposts = True
		self.addCleanup(saashq.flags.pop, "dont_execute_stock_reposts")
...
def create_repack_entry(**args):
	...
	repack = saashq.new_doc("Stock Entry")
...
def create_product_bundle_item(new_item_code, packed_items):
	if not saashq.db.exists("Product Bundle", new_item_code):
		item = saashq.new_doc("Product Bundle")
...
class TestDeferredNaming(IntegrationTestCase):
	...
	def setUpClass(cls) -> None:
		super().setUpClass()
		cls.gle_autoname = saashq.get_meta("GL Entry").autoname
		cls.sle_autoname = saashq.get_meta("Stock Ledger Entry").autoname
	...
	def get_gle_sles(se):
		...
		gle = set(saashq.get_list("GL Entry", filters, pluck="name"))
		sle = set(saashq.get_list("Stock Ledger Entry", filters, pluck="name"))
		return gle, sle
