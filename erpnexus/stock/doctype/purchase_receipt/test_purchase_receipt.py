...
from saashq.utils import add_days, cint, cstr, flt, get_datetime, getdate, nowtime, today
from pypika import functions as fn

import erpnexus
from erpnexus.accounts.doctype.account.test_account import get_inventory_account
from erpnexus.buying.doctype.supplier.test_supplier import create_supplier
from erpnexus.controllers.accounts_controller import InvalidQtyError
from erpnexus.controllers.buying_controller import QtyMismatchError
from erpnexus.stock import get_warehouse_account_map
from erpnexus.stock.doctype.item.test_item import create_item, make_item
from erpnexus.stock.doctype.purchase_receipt.purchase_receipt import make_purchase_invoice
...
class TestPurchaseReceipt(IntegrationTestCase):
	def setUp(self):
		saashq.db.set_single_value("Buying Settings", "allow_multiple_items", 1)
	...
	def test_reverse_purchase_receipt_sle(self):
		...
		sl_entry_cancelled = saashq.db.get_all(
			"Stock Ledger Entry",
			{"voucher_type": "Purchase Receipt", "voucher_no": pr.name},
			["actual_qty"],
			order_by="creation",
	...
	def test_make_purchase_invoice(self):
		from erpnexus.accounts.doctype.payment_entry.test_payment_entry import create_payment_term
		...
		template = saashq.db.get_value(
			"Payment Terms Template", "_Test Payment Terms Template For Purchase Invoice"
		)
		old_template_in_supplier = saashq.db.get_value("Supplier", "_Test Supplier", "payment_terms")
		saashq.db.set_value("Supplier", "_Test Supplier", "payment_terms", template)

		pr = make_purchase_receipt(do_not_save=True)
		self.assertRaises(saashq.ValidationError, make_purchase_invoice, pr.name)
		pr.submit()
		...
		self.assertRaises(saashq.ValidationError, saashq.get_doc(pi).submit)
		...
		pi.delete()  # draft PI
		pr.cancel()
		saashq.db.set_value("Supplier", "_Test Supplier", "payment_terms", old_template_in_supplier)
		saashq.get_doc("Payment Terms Template", "_Test Payment Terms Template For Purchase Invoice").delete()

	def test_purchase_receipt_no_gl_entry(self):
		from erpnexus.stock.doctype.stock_entry.test_stock_entry import make_stock_entry

		existing_bin_qty, existing_bin_stock_value = saashq.db.get_value(
			"Bin",
		...
		stock_value_difference = saashq.db.get_value(
			"Stock Ledger Entry",
			{
		...
		current_bin_stock_value = saashq.db.get_value(
	...
	def test_batched_serial_no_purchase(self):
		item = saashq.db.exists("Item", {"item_name": "Batched Serialized Item"})
		if not item:
			...
			item = saashq.get_doc("Item", {"item_name": "Batched Serialized Item"})

		pr = make_purchase_receipt(item_code=item.name, qty=5, rate=500)

		self.assertTrue(saashq.db.get_value("Batch", {"item": item.name, "reference_name": pr.name}))
	...
	def test_duplicate_serial_nos(self):
		...
		item = saashq.db.exists("Item", {"item_name": "Test Serialized Item 123"})
		if not item:
			...
			item = saashq.get_doc("Item", {"item_name": "Test Serialized Item 123"})

		# First make purchase receipt
		pr = make_purchase_receipt(item_code=item.name, qty=2, rate=500)
		pr.load_from_db()

		bundle_id = saashq.db.get_value(
			"Stock Ledger Entry",
	...
	def test_purchase_receipt_gl_entry(self):
		...
		self.assertEqual(cint(erpnexus.is_perpetual_inventory_enabled(pr.company)), 1)
	...
	def test_serial_no_warehouse(self):
		...
		self.assertEqual(
			saashq.db.get_value("Serial No", pr_row_1_serial_no, "warehouse"), pr.get("items")[0].warehouse
		)

		pr.cancel()
		self.assertFalse(saashq.db.get_value("Serial No", pr_row_1_serial_no, "warehouse"))
	...
	def test_rejected_warehouse_filter(self):
		...
		self.assertRaises(saashq.ValidationError, pr.save)
	...
	def test_rejected_serial_no(self):
		...
		for serial_no in accepted_serial_nos:
			self.assertEqual(
				saashq.db.get_value("Serial No", serial_no, "warehouse"), pr.get("items")[0].warehouse
			)
		...
		for serial_no in rejected_serial_nos:
			self.assertEqual(
				saashq.db.get_value("Serial No", serial_no, "warehouse"),
	...
	def test_purchase_return_partial(self):
		...
		# check sle
		outgoing_rate = saashq.db.get_value(
			"Stock Ledger Entry",
		...
		returned = saashq.get_doc("Purchase Receipt", return_pr.name)
	...
	def test_purchase_return_for_rejected_qty(self):
		from erpnexus.stock.doctype.warehouse.test_warehouse import get_warehouse

		rejected_warehouse = "_Test Rejected Warehouse - TCP1"
		if not saashq.db.exists("Warehouse", rejected_warehouse):
			get_warehouse(
				company="_Test Company with perpetual inventory",
				abbr=" - TCP1",
		...
		actual_qty = saashq.db.get_value(
			"Stock Ledger Entry",
			{
				"voucher_type": "Purchase Receipt",
	...
	def test_purchase_return_for_serialized_items(self):
		def _check_serial_no_values(serial_no, field_values):
			serial_no = saashq.get_doc("Serial No", serial_no)
	...
	def test_purchase_return_for_multi_uom(self):
		...
		if not saashq.db.exists("Item", item_code):
	...
	def test_closed_purchase_receipt(self):
		from erpnexus.stock.doctype.purchase_receipt.purchase_receipt import (
		...
		self.assertEqual(saashq.db.get_value("Purchase Receipt", pr.name, "status"), "Closed")
	...
	def test_pr_billing_status(self):
		...
		3. PO -> PR2.
		"""
		from erpnexus.buying.doctype.purchase_order.purchase_order import (
			make_purchase_invoice as make_purchase_invoice_from_po,
		)
		from erpnexus.buying.doctype.purchase_order.purchase_order import make_purchase_receipt
	...
	def test_serial_no_against_purchase_receipt(self):
		...
		if not saashq.db.exists("Serial No", serial_no[0]):
			saashq.get_doc(
		...
		voucher_no = saashq.db.get_value("Serial and Batch Bundle", bundle_id, "voucher_no")
	...
	def test_auto_asset_creation(self):
		...
		if not saashq.db.exists("Item", asset_item):
			asset_category = saashq.get_all("Asset Category")
		...
		assets = saashq.db.get_all("Asset", filters={"purchase_receipt": pr.name})

		self.assertEqual(len(assets), 3)

		location = saashq.db.get_value("Asset", assets[0].name, "location")
		self.assertEqual(location, "Test Location")

		pr.cancel()

	def test_purchase_return_with_submitted_asset(self):
		from erpnexus.stock.doctype.purchase_receipt.purchase_receipt import make_purchase_return

		pr = make_purchase_receipt(item_code="Test Asset Item", qty=1)

		asset = saashq.get_doc("Asset", {"purchase_receipt": pr.name})
		asset.available_for_use_date = saashq.utils.nowdate()
		...
		self.assertRaises(saashq.exceptions.ValidationError, pr_return.submit)
	...
	def test_purchase_receipt_cost_center(self):
		...
		if not saashq.db.exists("Location", "Test Location"):
			saashq.get_doc({"doctype": "Location", "location_name": "Test Location"}).insert()
	...
	def test_make_purchase_invoice_from_pr_for_returned_qty(self):
		from erpnexus.buying.doctype.purchase_order.test_purchase_order import (
			create_pr_against_po,
			create_purchase_order,
		)

		saashq.db.set_single_value("Buying Settings", "bill_for_rejected_quantity_in_purchase_invoice", 0)
	...
	def test_stock_transfer_from_purchase_receipt(self):
		from erpnexus.stock.doctype.delivery_note.delivery_note import make_inter_company_purchase_receipt
		from erpnexus.stock.doctype.delivery_note.test_delivery_note import create_delivery_note
	...
	def test_stock_transfer_from_purchase_receipt_with_valuation(self):
		...
		from erpnexus.stock.get_item_details import get_valuation_rate
		from erpnexus.stock.utils import get_stock_balance
	...
	def test_po_to_pi_and_po_to_pr_worflow_full(self):
		...
		"""
		from erpnexus.buying.doctype.purchase_order import purchase_order, test_purchase_order
	...
	def test_purchase_receipt_with_exchange_rate_difference(self):
		from erpnexus.accounts.doctype.purchase_invoice.purchase_invoice import (
		...
		from erpnexus.accounts.party import add_party_account
		...
		exchange_gain_loss_account = saashq.db.get_value("Company", pr.company, "exchange_gain_loss_account")

		# fetching the latest GL Entry with exchange gain and loss account account
		amount = saashq.db.get_value(
	...
	def test_payment_terms_are_fetched_when_creating_purchase_invoice(self):
		from erpnexus.accounts.doctype.payment_entry.test_payment_entry import (
			create_payment_terms_template,
		)
		from erpnexus.accounts.doctype.purchase_invoice.test_purchase_invoice import make_purchase_invoice
		from erpnexus.buying.doctype.purchase_order.test_purchase_order import (
			create_purchase_order,
			make_pr_against_po,
		)
		from erpnexus.selling.doctype.sales_order.test_sales_order import (
	...
	def test_backdated_transaction_for_internal_transfer_in_trasit_warehouse_for_purchase_receipt(
		...
		stk_ledger = saashq.db.get_value(
			"Stock Ledger Entry",
	...
	def test_backdated_transaction_for_internal_transfer_in_trasit_warehouse_for_purchase_invoice(
		...
		from erpnexus.accounts.doctype.sales_invoice.sales_invoice import (
	...
	def test_disable_last_purchase_rate(self):
		from erpnexus.stock.get_item_details import get_item_details
		...
		saashq.db.set_single_value("Buying Settings", "disable_last_purchase_rate", 1)
		...
		saashq.db.set_single_value("Buying Settings", "disable_last_purchase_rate", 0)
	...
	def test_validate_received_qty_for_internal_pr(self):
		...
		self.assertRaises(saashq.ValidationError, pr.submit)

		# Step 5: Test Over Receipt Allowance
		saashq.db.set_single_value("Stock Settings", "over_delivery_receipt_allowance", 50)
		...
		saashq.db.set_single_value("Stock Settings", "over_delivery_receipt_allowance", 0)
	...
	def test_internal_pr_gl_entries(self):
		...
		stock_account_value = saashq.db.get_value(
			"GL Entry",
			{
		...
		stock_diff = saashq.db.get_value(
			"Stock Ledger Entry",
			{
				"voucher_type": "Purchase Receipt",
				"voucher_no": pr.name,
	...
	def test_purchase_receipt_with_backdated_landed_cost_voucher(self):
		from erpnexus.controllers.sales_and_purchase_return import make_return_doc
		from erpnexus.stock.doctype.landed_cost_voucher.test_landed_cost_voucher import (
	...
	def test_purchase_receipt_provisional_accounting(self):
		...
		company = saashq.get_doc("Company", "_Test Company")
		...
		amount = saashq.db.get_value("GL Entry", {"docstatus": 1, "voucher_no": pr.name}, ["sum(debit)"])
	...
	def test_use_serial_batch_fields_for_serial_nos(self):
		...
		sbb_doc = saashq.get_doc("Serial and Batch Bundle", pr.items[0].serial_and_batch_bundle)
		...
		serial_no_status = saashq.db.get_value("Serial No", "SNU-TSFISI-000015", "status")
		...
		doc = saashq.get_doc("Serial and Batch Bundle", dn.items[0].serial_and_batch_bundle)
		for row in doc.entries:
			self.assertTrue(row.serial_no in new_serial_nos)

		for sn in new_serial_nos:
			serial_no_status = saashq.db.get_value("Serial No", sn, "status")
			self.assertTrue(serial_no_status != "Active")
	...
	def test_pr_billed_amount_against_return_entry(self):
		from erpnexus.accounts.doctype.purchase_invoice.purchase_invoice import make_debit_note
		from erpnexus.stock.doctype.purchase_receipt.purchase_receipt import (
	...
	def test_valuation_taxes_lcv_repost_after_billing(self):
		...
		old_perpetual_inventory = erpnexus.is_perpetual_inventory_enabled("_Test Company")
		saashq.local.enable_perpetual_inventory["_Test Company"] = 1
		saashq.db.set_value(
			"Company",
			"_Test Company",
		...
		saashq.local.enable_perpetual_inventory["_Test Company"] = old_perpetual_inventory
	...
	def test_purchase_receipt_with_use_serial_batch_field_for_rejected_qty(self):
		...
		if not saashq.db.exists("Batch", batch_no):
			saashq.get_doc(
				{
					"doctype": "Batch",
					"batch_id": batch_no,
		...
		for serial_no in serial_nos:
			if not saashq.db.exists("Serial No", serial_no):
				saashq.get_doc(
					{
						"doctype": "Serial No",
		...
		for row in inter_transfer_pr_return.items:
			...
			if row.item_code == serial_item_doc.name:
				...
				for sn in serial_nos:
					serial_no_details = saashq.db.get_value(
	...
	def test_internal_transfer_with_serial_batch_items_without_use_serial_batch_fields(self):
		...
		saashq.db.set_single_value("Stock Settings", "use_serial_batch_fields", 0)
	...
	def test_purchase_receipt_bill_for_rejected_quantity_in_purchase_invoice(self):
		...
		return_pr.items[0].purchase_receipt_item = pr.items[0].name
		return_pr.submit()
		old_value = saashq.db.get_single_value(
		...
		saashq.db.set_single_value("Buying Settings", "bill_for_rejected_quantity_in_purchase_invoice", 1)
		pi = make_purchase_invoice(pr.name)
		pi.submit()
	...
	def test_zero_valuation_rate_for_batched_item(self):
		...
		sabb_doc = saashq.get_doc("Serial and Batch Bundle", se.items[0].serial_and_batch_bundle)
	...
	def test_manufacturing_and_expiry_date_for_batch(self):
		...
		batch_no = get_batch_from_bundle(pr.items[0].serial_and_batch_bundle)
		batch = saashq.get_doc("Batch", batch_no)
	...
	def test_tax_account_heads_on_lcv_and_item_repost(self):
		...
		company_doc = saashq.get_doc("Company", test_company)
	...
	def create_lcv(self, receipt_document_type, receipt_document, company, expense_account, charges=50):
		ref_doc = saashq.get_doc(receipt_document_type, receipt_document)

		lcv = saashq.new_doc("Landed Cost Voucher")
		lcv.company = company
	...
	def test_do_not_use_batchwise_valuation_rate(self):
		...
		valuation_rate = saashq.db.get_value("Serial and Batch Bundle", bundle, "avg_rate")
		self.assertEqual(valuation_rate, 100)

		doc = saashq.get_doc("Stock Settings")
	...
	def test_internal_transfer_for_batch_items_with_cancel_use_serial_batch_fields(self):
		...
		saashq.db.set_single_value("Stock Settings", "use_serial_batch_fields", 1)
		saashq.db.set_single_value("Stock Settings", "auto_create_serial_and_batch_bundle_for_outward", 0)
		...
		inter_transfer_pr.cancel()
		inter_transfer_dn.cancel()
		saashq.db.set_single_value("Stock Settings", "auto_create_serial_and_batch_bundle_for_outward", 1)

	def test_sles_with_same_posting_datetime_and_creation(self):
		from erpnexus.stock.doctype.stock_entry.test_stock_entry import make_stock_entry
		from erpnexus.stock.report.stock_balance.stock_balance import execute
...
def prepare_data_for_internal_transfer():
	from erpnexus.accounts.doctype.sales_invoice.test_sales_invoice import create_internal_supplier
	...
	if not saashq.db.get_value("Company", company, "unrealized_profit_loss_account"):
		account = "Unrealized Profit and Loss - TCP1"
		if not saashq.db.exists("Account", account):
			saashq.get_doc(
				{
					"doctype": "Account",
		...
		saashq.db.set_value("Company", company, "unrealized_profit_loss_account", account)
...
def get_gl_entries(voucher_type, voucher_no, skip_cancelled=False, as_dict=True):
	...
	gl_query = (
		...
		.orderby(gl.account, order=saashq.qb.desc)
	)
	if skip_cancelled:
...
def make_purchase_receipt(**args):
	...
	saashq.db.set_single_value("Buying Settings", "allow_multiple_items", 1)
	pr = saashq.new_doc("Purchase Receipt")
	args = saashq._dict(args)
	...
	uom = args.uom or saashq.db.get_value("Item", item_code, "stock_uom") or "_Test UOM"
	...
	pr.append(
		"items",
		{
			...
			"uom": uom,
			"cost_center": args.cost_center or saashq.get_cached_value("Company", pr.company, "cost_center"),
	...
	if args.get_multiple_items:
		...
		company_cost_center = saashq.get_cached_value("Company", pr.company, "cost_center")
