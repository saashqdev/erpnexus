...
saashq.provide("erpnexus.item");

const SALES_DOCTYPES = ["Quotation", "Sales Order", "Delivery Note", "Sales Invoice"];
const PURCHASE_DOCTYPES = ["Purchase Order", "Purchase Receipt", "Purchase Invoice"];

saashq.ui.form.on("Item", {
	setup: function (frm) {
	...
	onload: function (frm) {
		erpnexus.item.setup_queries(frm);
	...
	refresh: function (frm) {
		if (frm.doc.is_stock_item) {
			...
			frm.add_custom_button(
				...
				function () {
					...
					saashq.set_route("query-report", "Stock Ledger");
				},
				__("View")
			);
			frm.add_custom_button(
				...
				function () {
					...
					saashq.set_route("query-report", "Stock Projected Qty");
				},
				__("View")
			);
		}
		...
		if (frm.doc.has_variants) {
			...
			frm.add_custom_button(
				__("Show Variants"),
				function () {
					saashq.set_route("List", "Item", { variant_of: frm.doc.name });
				},
				__("View")
			);
			...
			frm.add_custom_button(
				...
				function () {
					saashq.set_route("Form", "Item Variant Settings");
				},
				__("View")
			);
			...
			frm.add_custom_button(
				...
				function () {
					saashq.set_route("query-report", "Item Variant Details", { item: frm.doc.name });
				},
				__("View")
			);
			...
			if (frm.doc.variant_based_on === "Item Attribute") {
				frm.add_custom_button(
					__("Single Variant"),
					function () {
						erpnexus.item.show_single_variant_dialog(frm);
					},
					__("Create")
				);
				frm.add_custom_button(
					...
					function () {
						erpnexus.item.show_multiple_variants_dialog(frm);
					},
					__("Create")
				);
			} else {
				frm.add_custom_button(
					__("Variant"),
					function () {
						erpnexus.item.show_modal_for_manufacturers(frm);
					},
					__("Create")
				);
			}
		...
		if (saashq.defaults.get_default("item_naming_by") != "Naming Series" || frm.doc.variant_of) {
			frm.toggle_display("naming_series", false);
		} else {
			erpnexus.toggle_naming_series();
		}

		erpnexus.item.edit_prices_button(frm);
		erpnexus.item.toggle_attributes(frm);

		if (!frm.doc.is_fixed_asset) {
			erpnexus.item.make_dashboard(frm);
		}

		frm.add_custom_button(__("Duplicate"), function () {
			var new_item = saashq.model.copy_doc(frm.doc);
			...
			saashq.set_route("Form", "Item", new_item.name);
		});
	...
	validate: function (frm) {
		erpnexus.item.weight_to_validate(frm);
	},
	...
	is_fixed_asset: function (frm) {
		...
		saashq.call({
			method: "erpnexus.stock.doctype.item.item.get_asset_naming_series",
			callback: function (r) {
	...
	page_name: saashq.utils.warn_page_name_change,
...
saashq.ui.form.on("Item Reorder", {
...
saashq.ui.form.on("Item Customer Detail", {
	customer_items_add: function (frm, cdt, cdn) {
		saashq.model.set_value(cdt, cdn, "customer_group", "");
	},
	...
	customer_group: function (frm, cdt, cdn) {
		if (set_customer_group(frm, cdt, cdn)) {
			saashq.msgprint(__("Changing Customer Group for the selected Customer is not allowed."));
		}
	},
});
...
var set_customer_group = function (frm, cdt, cdn) {
	...
	saashq.model.with_doc("Customer", row.customer_name, function () {
		var customer = saashq.model.get_doc("Customer", row.customer_name);
...
$.extend(erpnexus.item, {
	setup_queries: function (frm) {
		frm.fields_dict["item_defaults"].grid.get_field("expense_account").get_query = function (
			...
			return {
				query: "erpnexus.controllers.queries.get_expense_account",
	...
	make_dashboard: function (frm) {
		...
		if (frm.doc.is_stock_item) {
			saashq.require("item-dashboard.bundle.js", function () {
				const section = frm.dashboard.add_section("", __("Stock Levels"));
				erpnexus.item.item_dashboard = new erpnexus.stock.ItemDashboard({
					parent: section,
					item_code: frm.doc.name,
					page_length: 20,
					method: "erpnexus.stock.dashboard.item_dashboard.get_data",
					template: "item_dashboard_list",
				});
				erpnexus.item.item_dashboard.refresh();
			});
		}
	},
	...
	edit_prices_button: function (frm) {
		frm.add_custom_button(
			__("Add / Edit Prices"),
			function () {
				saashq.set_route("List", "Item Price", { item_code: frm.doc.name });
			},
			__("Actions")
		);
	},
	...
	show_modal_for_manufacturers: function (frm) {
		...
		dialog.set_primary_action(__("Create"), function () {
			...
			saashq.call({
				method: "erpnexus.controllers.item_variant.get_variant",
				args: data,
				callback: function (r) {
	...
	show_multiple_variants_dialog: function (frm) {
		...
		function make_fields_from_attribute_values(attr_dict) {
			...
			Object.keys(attr_dict).forEach((name, i) => {
				...
				fields.push({
					fieldtype: "Data",
					placeholder: "Search",
					fieldname: `search_${saashq.scrub(name)}`,
					onchange: function (e) {
		...
		function make_and_show_dialog(fields) {
			me.multiple_variant_dialog = new saashq.ui.Dialog({
			...
			me.multiple_variant_dialog.set_primary_action(__("Create Variants"), () => {
				...
				saashq.call({
					method: "erpnexus.controllers.item_variant.enqueue_multiple_variant_creation",
					args: {
						item: frm.doc.name,
			...
			$($(me.multiple_variant_dialog.$wrapper.find(".form-column")).find(".saashq-control")).css(
				"margin-bottom",
				"0px"
			);
			...
			saashq.call({
				...
				callback: function (r) {
					...
					if (r.message) {
						...
						saashq.msgprint_dialog = saashq.msgprint(
						...
						saashq.msgprint_dialog.hide_on_page_refresh = true;
						saashq.msgprint_dialog.$wrapper.find(".variant-click").on("click", function () {
							d.hide();
						});
					} else {
						d.hide();
						saashq.call({
							method: "erpnexus.controllers.item_variant.create_variant",
							args: {
								item: frm.doc.name,
								args: d.get_values(),
		...
		$.each(d.fields_dict, function (i, field) {
			...
			field.$input
				.on("input", function (e) {
					...
					saashq.call({
						method: "erpnexus.stock.doctype.item.item.get_item_attribute",
						args: {
							parent: i,
...
saashq.ui.form.on("UOM Conversion Detail", {
	uom: function (frm, cdt, cdn) {
		var row = locals[cdt][cdn];
		if (row.uom) {
			saashq.call({
				method: "erpnexus.stock.doctype.item.item.get_uom_conv_factor",
				args: {
					uom: row.uom,
				...
				callback: function (r) {
					if (!r.exc && r.message) {
						saashq.model.set_value(cdt, cdn, "conversion_factor", r.message);
					}
				},
			});
		}
...
saashq.tour["Item"] = [
	...
	},
	{
		...
		description: __(
			"If you are maintaining stock of this Item in your Inventory, ERPNexus will make a stock ledger entry for each transaction of this item."
		),
	},
	{
...
function open_form(frm, doctype, child_doctype, parentfield) {
	saashq.model.with_doctype(doctype, () => {
		...
		let new_child_doc = saashq.model.add_child(new_doc, child_doctype, parentfield);
		...
		saashq.run_serially([
			...
			() => {
				saashq.flags.ignore_company_party_validation = true;
				saashq.model.trigger("item_code", frm.doc.name, new_child_doc);
			},
		]);
	});
}
