...
from saashq.test_runner import make_test_objects
...
from erpnexus.stock.doctype.item.item import (
	DataValidationError,
	InvalidBarcode,
...
def make_item(item_code=None, properties=None, uoms=None, barcode=None):
	if not item_code:
		item_code = saashq.generate_hash(length=16)

	if saashq.db.exists("Item", item_code):
		return saashq.get_doc("Item", item_code)
...
class TestItem(IntegrationTestCase):
	...
	def get_item(self, idx):
		...
		if not saashq.db.exists("Item", item_code):
			item = saashq.copy_doc(self.globalTestRecords["Item"][idx])
			item.insert()
		else:
	...
	def test_get_item_details(self):
		...
		saashq.db.sql("""delete from `tabBin`""")
		...
		currency = saashq.get_cached_value("Company", company, "default_currency")
	...
	def test_get_asset_item_details(self):
		from erpnexus.assets.doctype.asset.test_asset import create_asset_category, create_fixed_asset_item
		...
		saashq.db.set_value("Asset Category", "Computers", "enable_cwip_accounting", "1")
		details = get_item_details(
			{
				"item_code": "Macbook Pro",
				"company": "_Test Company",
	...
	def test_item_defaults(self):
		saashq.delete_doc_if_exists("Item", "Test Item With Defaults", force=1)
		make_item(
	...
	def test_item_default_validations(self):
		with self.assertRaises(saashq.ValidationError) as ve:
			make_item(
				"Bad Item defaults",
				{
					"item_group": "_Test Item Group",
	...
	def test_item_attribute_change_after_variant(self):
		saashq.delete_doc_if_exists("Item", "_Test Variant Item-L", force=1)

		variant = create_variant("_Test Variant Item", {"Test Size": "Large"})
		variant.save()

		attribute = saashq.get_doc("Item Attribute", "Test Size")
	...
	def test_copy_fields_from_template_to_variants(self):
		saashq.delete_doc_if_exists("Item", "_Test Variant Item-XL", force=1)
		...
		if not saashq.db.get_value(
			...
			item_attribute = saashq.get_doc("Item Attribute", "Test Size")
		...
		variant = saashq.get_doc("Item", "_Test Variant Item-XL")
	...
	def test_make_item_variant_with_numeric_values(self):
		# cleanup
		for d in saashq.db.get_all("Item", filters={"variant_of": "_Test Numeric Template Item"}):
			saashq.delete_doc_if_exists("Item", d.name)

		saashq.delete_doc_if_exists("Item", "_Test Numeric Template Item")
		saashq.delete_doc_if_exists("Item Attribute", "Test Item Length")
	...
	def test_item_merging(self):
		old = create_item(saashq.generate_hash(length=20)).name
		new = create_item(saashq.generate_hash(length=20)).name
		...
		saashq.rename_doc("Item", old, new, merge=True)

		self.assertFalse(saashq.db.exists("Item", old))

		self.assertTrue(saashq.db.get_value("Bin", {"item_code": new, "warehouse": "_Test Warehouse - _TC"}))
		self.assertTrue(
			saashq.db.get_value("Bin", {"item_code": new, "warehouse": "_Test Warehouse 1 - _TC"})
		)
	...
	def test_item_merging_with_product_bundle(self):
		...
		with self.assertRaises(DataValidationError):
			saashq.rename_doc("Item", "Test Item Bundle Item 1", "Test Item Bundle Item 2", merge=True)

		bundle1.delete()
		saashq.rename_doc("Item", "Test Item Bundle Item 1", "Test Item Bundle Item 2", merge=True)

		self.assertFalse(saashq.db.exists("Item", "Test Item Bundle Item 1"))

	def test_uom_conversion_factor(self):
		if saashq.db.exists("Item", "Test Item UOM"):
			saashq.delete_doc("Item", "Test Item UOM")
	...
	def test_item_variant_by_manufacturer(self):
		...
		for manufacturer in ["DFSS", "DASA", "ASAAS"]:
			if not saashq.db.exists("Manufacturer", manufacturer):
				m_doc = saashq.new_doc("Manufacturer")
				m_doc.short_name = manufacturer
				m_doc.insert()

		self.assertFalse(saashq.db.exists("Item Manufacturer", {"manufacturer": "DFSS"}))
		variant = get_variant(template, manufacturer="DFSS", manufacturer_part_no="DFSS-123")

		item_manufacturer = saashq.db.exists(
			"Item Manufacturer", {"manufacturer": "DFSS", "item_code": variant.name}
		)
		self.assertTrue(item_manufacturer)

		saashq.delete_doc("Item Manufacturer", item_manufacturer)

	def test_stock_exists_against_template_item(self):
		stock_item = saashq.get_all("Stock Ledger Entry", fields=["item_code"], limit=1)
		if stock_item:
			item_code = stock_item[0].item_code

			item_doc = saashq.get_doc("Item", item_code)
			item_doc.has_variants = 1
			self.assertRaises(StockExistsForTemplate, item_doc.save)

	def test_add_item_barcode(self):
		# Clean up
		saashq.db.sql("""delete from `tabItem Barcode`""")
		item_code = "Test Item Barcode"
		if saashq.db.exists("Item", item_code):
			saashq.delete_doc("Item", item_code)
		...
		self.assertRaises(saashq.UniqueValidationError, item_doc.save)
	...
	def test_index_creation(self):
		...
		indices = saashq.db.sql("show index from tabItem", as_dict=1)
	...
	def test_check_stock_uom_with_bin(self):
		...
		item = saashq.get_doc("Item", "_Test Item")
		item.stock_uom = "Gram"
		self.assertRaises(saashq.ValidationError, item.save)

	def test_check_stock_uom_with_bin_no_sle(self):
		from erpnexus.stock.stock_balance import update_bin_qty
	...
	def test_erasure_of_old_conversions(self):
		...
		item.stock_uom = "Gram"
		item.append("uoms", saashq._dict(uom="Box", conversion_factor=2))
		item.save()
		item.reload()
		item.stock_uom = "Nos"
		item.save()
	...
	def test_validate_stock_item(self):
		self.assertRaises(saashq.ValidationError, validate_is_stock_item, "_Test Non Stock Item")
	...
	def test_item_dashboard(self):
		from erpnexus.stock.dashboard.item_dashboard import get_data
	...
	def test_customer_codes_length(self):
		...
		for _row in range(3):
			item.append("customer_items", {"ref_code": saashq.generate_hash("", 120)})
		item.save()
	...
	def test_update_is_stock_item(self):
		...
		pb = saashq.new_doc("Product Bundle")
	...
	def test_serach_fields_for_item(self):
		from erpnexus.controllers.queries import item_query
	...
	def test_group_warehouse_for_reorder_item(self):
		...
		warehouse_doc = saashq.get_doc("Warehouse", warehouse)
		...
		self.assertRaises(saashq.ValidationError, item_doc.save)


def set_item_variant_settings(fields):
	doc = saashq.get_doc("Item Variant Settings")
	doc.set("fields", fields)
	doc.save()


def make_item_variant():
	if not saashq.db.exists("Item", "_Test Variant Item-S"):
