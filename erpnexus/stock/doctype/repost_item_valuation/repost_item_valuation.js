...
saashq.ui.form.on("Repost Item Valuation", {
	setup: function (frm) {
	...
	setup_realtime_progress: function (frm) {
		saashq.realtime.on("item_reposting_progress", (data) => {
	...
	execute_reposting(frm) {
		frm.add_custom_button(__("Start Reposting"), () => {
			saashq.call({
				method: "erpnexus.stock.doctype.repost_item_valuation.repost_item_valuation.execute_repost_item_valuation",
				callback: function () {
					saashq.msgprint(__("Reposting has been started in the background."));
				},
			});
		});
	},
