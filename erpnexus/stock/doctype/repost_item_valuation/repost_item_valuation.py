...
from saashq.exceptions import QueryDeadlockError, QueryTimeoutError
from saashq.model.document import Document
from saashq.query_builder import DocType, Interval
from saashq.query_builder.functions import Max, Now
from saashq.utils import cint, get_link_to_form, get_weekday, getdate, now, nowtime
from saashq.utils.user import get_users_with_role
from rq.timeouts import JobTimeoutException

import erpnexus
from erpnexus.accounts.general_ledger import validate_accounting_period
from erpnexus.accounts.utils import get_future_stock_vouchers, repost_gle_for_stock_vouchers
from erpnexus.stock.stock_ledger import (
...
class RepostItemValuation(Document):
	...
	def validate_period_closing_voucher(self):
		...
		if year_end_date and getdate(self.posting_date) <= getdate(year_end_date):
			date = saashq.format(year_end_date, "Date")
		...
		if closing_stock and closing_stock[0].name:
			...
			to_date = saashq.format(closing_stock[0].to_date, "Date")
	...
	def get_closing_stock_balance(self):
		...
		return saashq.get_all("Closing Stock Balance", fields=["name", "to_date"], filters=filters)

	@staticmethod
	def get_max_period_closing_date(company):
		table = saashq.qb.DocType("Period Closing Voucher")
	...
	def validate_accounts_freeze(self):
		acc_settings = saashq.get_cached_doc("Accounts Settings")
		if not acc_settings.acc_frozen_upto:
			return
		if getdate(self.posting_date) <= getdate(acc_settings.acc_frozen_upto):
			if acc_settings.frozen_accounts_modifier and saashq.session.user in get_users_with_role(
				acc_settings.frozen_accounts_modifier
			):
				saashq.msgprint(_("Caution: This might alter frozen accounts."))
				return
			saashq.throw(_("You cannot repost item valuation before {}").format(acc_settings.acc_frozen_upto))
	...
	def set_company(self):
		if self.based_on == "Transaction":
			self.company = saashq.get_cached_value(self.voucher_type, self.voucher_no, "company")
		elif self.warehouse:
			self.company = saashq.get_cached_value("Warehouse", self.warehouse, "company")
	...
	def clear_attachment(self):
		if attachments := get_attachments(self.doctype, self.name):
			...
			saashq.delete_doc("File", attachment.name, ignore_permissions=True)
	...
	def on_submit(self):
		...
		Exceptions:
		        ...
		        2. global flag saashq.flags.dont_execute_stock_reposts is set
		...
		if not saashq.flags.in_test:
			return
		if self.flags.dont_run_in_test or saashq.flags.dont_execute_stock_reposts:
			return
	...
	def check_pending_repost_against_cancelled_transaction(self):
		...
		saashq.throw(msg, title=_("Pending processing"))
...
def on_doctype_update():
	saashq.db.add_index("Repost Item Valuation", ["warehouse", "item_code"], "item_warehouse")


def repost(doc):
	try:
		saashq.flags.through_repost_item_valuation = True
		if not saashq.db.exists("Repost Item Valuation", doc.name):
			return

		# This is to avoid TooManyWritesError in case of large reposts
		saashq.db.MAX_WRITES_PER_TRANSACTION *= 4
		...
		message = saashq.message_log.pop() if saashq.message_log else ""
		...
		outgoing_email_account = saashq.get_cached_value(
...
def remove_attached_file(docname):
	if file_name := saashq.db.get_value(
...
def repost_gl_entries(doc):
	if not cint(erpnexus.is_perpetual_inventory_enabled(doc.company)):
		return
...
def _get_directly_dependent_vouchers(doc):
	...
	if doc.based_on == "Transaction":
		ref_doc = saashq.get_doc(doc.voucher_type, doc.voucher_no)
...
def get_recipients():
	role = (
		saashq.db.get_single_value("Stock Reposting Settings", "notify_reposting_error_to_role")
		or "Stock Manager"
	)
...
def repost_entries():
	...
	for row in riv_entries:
		doc = saashq.get_doc("Repost Item Valuation", row.name)
...
def in_configured_timeslot(repost_settings=None, current_time=None):
	...
	if repost_settings is None:
		repost_settings = saashq.get_cached_doc("Stock Reposting Settings")
...
def execute_repost_item_valuation():
	...
	saashq.get_doc("Scheduled Job Type", "repost_item_valuation.repost_entries").enqueue(force=True)
