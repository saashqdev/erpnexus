...
from saashq.utils import add_days, add_to_date, now, nowdate, today

from erpnexus.accounts.doctype.sales_invoice.test_sales_invoice import create_sales_invoice
from erpnexus.accounts.utils import repost_gle_for_stock_vouchers
from erpnexus.controllers.stock_controller import create_item_wise_repost_entries
...
from erpnexus.stock.doctype.repost_item_valuation.repost_item_valuation import (
...
from erpnexus.stock.utils import PendingRepostingError


class TestRepostItemValuation(IntegrationTestCase, StockTestMixin):
	def tearDown(self):
		saashq.flags.dont_execute_stock_reposts = False

	def test_repost_time_slot(self):
		repost_settings = saashq.get_doc("Stock Reposting Settings")
	...
	def test_clear_old_logs(self):
		...
		logs = saashq.get_all("Repost Item Valuation", filters={"status": "Skipped"})
	...
	def test_deduplication(self):
		...
		riv3 = saashq.get_doc(riv_args.update({"posting_date": "2021-01-01"}))
		...
		riv4 = saashq.get_doc(riv_args.update({"warehouse": "Stores - _TC"}))
	...
	def test_prevention_of_cancelled_transaction_riv(self):
		...
		riv = saashq.get_last_doc(
			"Repost Item Valuation", {"voucher_type": old.doctype, "voucher_no": old.name}
		)
		self.assertRaises(saashq.ValidationError, riv.cancel)
	...
	def test_queue_progress_serialization(self):
		...
		self.assertEqual(
			[["a", "b"], ["c", "d"]],
			sorted(saashq.parse_json(saashq.as_json(set([("a", "b"), ("c", "d")])))),
		)

	def test_gl_repost_progress(self):
		from erpnexus.accounts import utils
	...
	def test_duplicate_ple_on_repost(self):
		...
		ple_entries = saashq.db.get_list(
	...
	def test_account_freeze_validation(self):
		...
		self.assertRaises(saashq.ValidationError, riv.save)
	...
	def test_create_repost_entry_for_cancelled_document(self):
		...
		self.assertFalse(saashq.db.exists("Repost Item Valuation", {"voucher_no": pr.name}))

		pr.load_from_db()

		pr.cancel()
		self.assertTrue(pr.docstatus == 2)
		self.assertTrue(saashq.db.exists("Repost Item Valuation", {"voucher_no": pr.name}))

	def test_repost_item_valuation_for_closing_stock_balance(self):
		from erpnexus.stock.doctype.closing_stock_balance.closing_stock_balance import (
			prepare_closing_stock_balance,
		)

		doc = saashq.new_doc("Closing Stock Balance")
		...
		riv = saashq.new_doc("Repost Item Valuation")
		riv.update(
			{
				"item_code": "_Test Item",
				"warehouse": "_Test Warehouse - _TC",
	...
	def test_remove_attached_file(self):
		...
		if docname := saashq.db.exists("Repost Item Valuation", {"voucher_no": pr1.name}):
			self.assertFalse(
				saashq.db.get_value(
					"File",
