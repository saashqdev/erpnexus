...
class DoNotChangeError(saashq.ValidationError):
	pass


class CanNotBeChildDoc(saashq.ValidationError):
	pass


class CanNotBeDefaultDimension(saashq.ValidationError):
	pass
...
class InventoryDimension(Document):
	...
	def onload(self):
		if not self.is_new() and saashq.db.has_column("Stock Ledger Entry", self.target_fieldname):
	...
	def do_not_update_document(self):
		...
		for field in saashq.get_meta("Inventory Dimension").fields:
			if field.fieldname not in allow_to_edit_fields and old_doc.get(field.fieldname) != self.get(
				...
				saashq.throw(_(msg), DoNotChangeError)
	...
	def delete_custom_fields(self):
		...
		for field in saashq.get_all("Custom Field", filters=filters):
			saashq.delete_doc("Custom Field", field.name)

		msg = f"Deleted custom fields related to the dimension {self.name}"
		saashq.msgprint(_(msg))
	...
	def validate_reference_document(self):
		if saashq.get_cached_value("DocType", self.reference_document, "istable") == 1:
			msg = f"The reference document {self.reference_document} can not be child table."
			saashq.throw(_(msg), CanNotBeChildDoc)
		...
		if self.reference_document in ["Batch", "Serial No", "Warehouse", "Item"]:
			...
			saashq.throw(_(msg), CanNotBeDefaultDimension)
...
def field_exists(doctype, fieldname) -> str or None:
	return saashq.db.get_value("DocField", {"parent": doctype, "fieldname": fieldname}, "name")
...
def get_evaluated_inventory_dimension(doc, sl_dict, parent_doc=None):
	...
	for row in dimensions:
		...
		if row.condition and saashq.safe_eval(row.condition, evals):
...
def get_document_wise_inventory_dimensions(doctype) -> dict:
	if not hasattr(saashq.local, "document_wise_inventory_dimensions"):
		saashq.local.document_wise_inventory_dimensions = {}

	if not saashq.local.document_wise_inventory_dimensions.get(doctype):
		dimensions = saashq.get_all(
			"Inventory Dimension",
			fields=[
				"name",
				"source_fieldname",
		...
		saashq.local.document_wise_inventory_dimensions[doctype] = dimensions

	return saashq.local.document_wise_inventory_dimensions[doctype]


@saashq.whitelist()
def get_inventory_dimensions():
	if not hasattr(saashq.local, "inventory_dimensions"):
		saashq.local.inventory_dimensions = {}

	if not saashq.local.inventory_dimensions:
		dimensions = saashq.get_all(
			"Inventory Dimension",
			fields=[
		...
		saashq.local.inventory_dimensions = dimensions

	return saashq.local.inventory_dimensions


@saashq.whitelist()
def delete_dimension(dimension):
	doc = saashq.get_doc("Inventory Dimension", dimension)
	doc.delete()


@saashq.whitelist()
def get_parent_fields(child_doctype, dimension_name):
	parent_doctypes = saashq.get_all("DocField", fields=["parent"], filters={"options": child_doctype})
