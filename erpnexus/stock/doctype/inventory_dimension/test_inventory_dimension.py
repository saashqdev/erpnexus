...
from saashq.utils import nowdate, nowtime
...
from erpnexus.stock.doctype.stock_ledger_entry.stock_ledger_entry import InventoryDimensionNegativeStockError
...
class TestInventoryDimension(IntegrationTestCase):
	...
	def test_delete_inventory_dimension(self):
		...
		custom_field = saashq.db.get_value(
	...
	def test_inventory_dimension(self):
		saashq.local.document_wise_inventory_dimensions = {}
		...
		sle_shelf = saashq.db.get_value("Stock Ledger Entry", {"voucher_no": outward.name}, "shelf")
	...
	def test_check_standard_dimensions(self):
		...
		self.assertFalse(
			saashq.db.get_value("Custom Field", {"fieldname": "project", "dt": "Stock Ledger Entry"}, "name")
		)
	...
	def test_validate_negative_stock_for_inventory_dimension(self):
		saashq.local.inventory_dimensions = {}
...
def create_store_dimension():
	if not saashq.db.exists("DocType", "Store"):
		saashq.get_doc(
			{
				"doctype": "DocType",
				"name": "Store",
	...
	for store in ["Store 1", "Store 2"]:
		if not saashq.db.exists("Store", store):
			saashq.get_doc({"doctype": "Store", "store_name": store}).insert(ignore_permissions=True)


def prepare_test_data():
	if not saashq.db.exists("DocType", "Shelf"):
		saashq.get_doc(
			{
				"doctype": "DocType",
				"name": "Shelf",
	...
	for shelf in ["Shelf 1", "Shelf 2"]:
		if not saashq.db.exists("Shelf", shelf):
			saashq.get_doc({"doctype": "Shelf", "shelf_name": shelf}).insert(ignore_permissions=True)

	create_warehouse("Shelf Warehouse")

	if not saashq.db.exists("DocType", "Rack"):
		saashq.get_doc(
			{
				"doctype": "DocType",
				"name": "Rack",
	...
	for rack in ["Rack 1"]:
		if not saashq.db.exists("Rack", rack):
			saashq.get_doc({"doctype": "Rack", "rack_name": rack}).insert(ignore_permissions=True)

	create_warehouse("Rack Warehouse")

	if not saashq.db.exists("DocType", "Pallet"):
		saashq.get_doc(
			{
				"doctype": "DocType",
				"name": "Pallet",
	...
	if not saashq.db.exists("DocType", "Inv Site"):
		saashq.get_doc(
			{
				"doctype": "DocType",
				"name": "Inv Site",
	...
	for site in ["Site 1", "Site 2"]:
		if not saashq.db.exists("Inv Site", site):
			saashq.get_doc({"doctype": "Inv Site", "site_name": site}).insert(ignore_permissions=True)


def create_inventory_dimension(**args):
	args = saashq._dict(args)

	if saashq.db.exists("Inventory Dimension", args.dimension_name):
		return saashq.get_doc("Inventory Dimension", args.dimension_name)

	doc = saashq.new_doc("Inventory Dimension")
	doc.update(args)
...
def prepare_data_for_internal_transfer():
	...
	cost_center = saashq.db.get_value("Company", company, "cost_center") or saashq.db.get_value(
		"Cost Center", {"company": company}, "name"
	)

	expene_account = saashq.db.get_value(
		"Company", company, "stock_adjustment_account"
	) or saashq.db.get_value("Account", {"company": company, "account_type": "Expense Account"}, "name")
	...
	return saashq._dict(
		{
			...
			"store_warehouse": saashq.db.get_value(
