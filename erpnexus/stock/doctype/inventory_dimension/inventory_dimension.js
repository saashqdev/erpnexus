...
saashq.ui.form.on("Inventory Dimension", {
	setup(frm) {
	...
	set_query_on_fields(frm) {
		frm.set_query("reference_document", () => {
			let invalid_doctypes = saashq.model.core_doctypes_list;
		...
		frm.set_query("document_type", () => {
			return {
				query: "erpnexus.stock.doctype.inventory_dimension.inventory_dimension.get_inventory_documents",
			};
		});
	},
	...
	set_parent_fields(frm) {
		if (frm.doc.apply_to_all_doctypes) {
			...
			saashq.call({
				method: "erpnexus.stock.doctype.inventory_dimension.inventory_dimension.get_parent_fields",
				args: {
	...
	delete_dimension(frm) {
		...
		saashq.confirm(__(msg, [frm.doc.name.bold()]), () => {
			saashq.call({
				method: "erpnexus.stock.doctype.inventory_dimension.inventory_dimension.delete_dimension",
				args: {
					dimension: frm.doc.name,
				},
				callback: function () {
					saashq.set_route("List", "Inventory Dimension");
				},
			});
		});
	},
