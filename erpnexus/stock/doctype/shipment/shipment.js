...
saashq.ui.form.on("Shipment", {
	...
	onload: function (frm) {
		frm.set_query("delivery_address_name", () => {
			let delivery_to = `delivery_${saashq.model.scrub(frm.doc.delivery_to_type)}`;
		...
		frm.set_query("pickup_address_name", () => {
			let pickup_from = `pickup_${saashq.model.scrub(frm.doc.pickup_from_type)}`;
			return frm.events.address_query(
				frm,
	...
	pickup_from_type: function (frm) {
		if (frm.doc.pickup_from_type == "Company") {
			frm.set_value("pickup_company", saashq.defaults.get_default("company"));
	...
	delivery_to_type: function (frm) {
		if (frm.doc.delivery_to_type == "Company") {
			frm.set_value("delivery_company", saashq.defaults.get_default("company"));
	...
	delivery_address_name: function (frm) {
		if (frm.doc.delivery_to_type == "Company") {
			erpnexus.utils.get_address_display(frm, "delivery_address_name", "delivery_address", true);
		} else {
			erpnexus.utils.get_address_display(frm, "delivery_address_name", "delivery_address", false);
		}
	},
	pickup_address_name: function (frm) {
		if (frm.doc.pickup_from_type == "Company") {
			erpnexus.utils.get_address_display(frm, "pickup_address_name", "pickup_address", true);
		} else {
			erpnexus.utils.get_address_display(frm, "pickup_address_name", "pickup_address", false);
		}
	},
	get_contact_display: function (frm, contact_name, contact_type) {
		saashq.call({
			method: "saashq.contacts.doctype.contact.contact.get_contact_details",
	...
	pickup_contact_person: function (frm) {
		if (frm.doc.pickup_contact_person) {
			saashq.call({
				method: "erpnexus.stock.doctype.shipment.shipment.get_company_contact",
	...
	set_company_contact: function (frm, delivery_type) {
				...
				if (!(r.last_name && r.email && (r.phone || r.mobile_no))) {
					...
					saashq.throw(
						__("Last Name, Email or Phone/Mobile of the user are mandatory to continue.") +
							...
							` <a href="/app/user/${saashq.session.user}">${saashq.session.user}</a>`
					);
				}
		...
		frm.set_value("pickup_contact_person", saashq.session.user);
	},
	...
	set_address_name: function (frm, ref_doctype, ref_docname, delivery_type) {
		saashq.call({
			method: "erpnexus.stock.doctype.shipment.shipment.get_address_name",
			args: {
	...
	set_contact_name: function (frm, ref_doctype, ref_docname, delivery_type) {
		saashq.call({
			method: "erpnexus.stock.doctype.shipment.shipment.get_contact_name",
			args: {
				ref_doctype: ref_doctype,
				docname: ref_docname,
			},
	...
	add_template: function (frm) {
		if (frm.doc.parcel_template) {
			saashq.model.with_doc("Shipment Parcel Template", frm.doc.parcel_template, () => {
				let parcel_template = saashq.model.get_doc(
					"Shipment Parcel Template",
					frm.doc.parcel_template
				);
				let row = saashq.model.add_child(frm.doc, "Shipment Parcel", "shipment_parcel");
	...
	pickup_date: function (frm) {
		if (frm.doc.pickup_date < saashq.datetime.get_today()) {
			saashq.throw(__("Pickup Date cannot be before this day"));
		}
	},
...
saashq.ui.form.on("Shipment Delivery Note", {
