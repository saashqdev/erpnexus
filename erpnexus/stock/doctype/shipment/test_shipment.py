...
from erpnexus.stock.doctype.delivery_note.delivery_note import make_shipment
...
def create_test_shipment(delivery_notes=None):
	...
	shipment = saashq.new_doc("Shipment")
...
def get_shipment_customer_contact(customer_name):
	...
	contacts = saashq.get_all("Contact", fields=["name"], filters={"name": customer_name})
	if len(contacts):
		return contacts[0]
	else:
...
def get_shipment_customer_address(customer_name):
	...
	customer_address = saashq.get_all("Address", fields=["name"], filters={"address_title": address_title})
...
def get_shipment_customer():
	...
	customer = saashq.get_all("Customer", fields=["name"], filters={"name": customer_name})
	if len(customer):
		return customer[0]
	else:
...
def get_shipment_company_address(company_name):
	...
	addresses = saashq.get_all("Address", fields=["name"], filters={"address_title": address_title})
	if len(addresses):
		return addresses[0]
	else:
...
def get_shipment_company():
	return saashq.get_doc("Company", "_Test Company")
...
def create_customer_contact(fname, lname):
	customer = saashq.new_doc("Contact")
...
def create_material_receipt(item, company):
	...
	stock = saashq.new_doc("Stock Entry")
