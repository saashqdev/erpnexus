...
from saashq.utils import cint, cstr, flt, get_link_to_form, getdate, new_line_sep, nowdate
...
from erpnexus.stock.stock_balance import get_indented_qty, update_bin_qty
...
class MaterialRequest(BuyingController):
	...
	if TYPE_CHECKING:
		...
		from erpnexus.stock.doctype.material_request_item.material_request_item import MaterialRequestItem
	...
	def validate_qty_against_so(self):
		...
		for so_no in so_items.keys():
			for item in so_items[so_no].keys():
				already_indented = saashq.db.sql(
					"""select sum(qty)
	...
	def validate(self):
		...
		from erpnexus.controllers.status_updater import validate_status
	...
	def on_submit(self):
		...
		if self.material_request_type == "Purchase" and saashq.db.exists(
	...
	def check_modified_date(self):
		mod_db = saashq.db.sql("""select modified from `tabMaterial Request` where name = %s""", self.name)
		date_diff = saashq.db.sql(f"""select TIMEDIFF('{mod_db[0][0]}', '{cstr(self.modified)}')""")

		if date_diff and date_diff[0][0]:
			saashq.throw(_("{0} {1} has been modified. Please refresh.").format(_(self.doctype), self.name))
	...
	def get_mr_items_ordered_qty(self, mr_items):
		...
		if self.material_request_type in ("Material Issue", "Material Transfer", "Customer Provided"):
			doctype = saashq.qb.DocType("Stock Entry Detail")
			qty_field = doctype.transfer_qty
		elif self.material_request_type == "Manufacture":
			doctype = saashq.qb.DocType("Work Order")
		...
		if doctype and qty_field:
			...
			mr_items_ordered_qty = saashq._dict(query.run())
	...
	def update_completed_qty(self, mr_items=None, update_modified=True):
		...
		mr_qty_allowance = saashq.db.get_single_value("Stock Settings", "mr_qty_allowance")
		...
		for d in self.get("items"):
			...
			if d.name in mr_items:
				...
				saashq.db.set_value(d.doctype, d.name, "ordered_qty", d.ordered_qty)
	...
	def update_requested_qty(self, mr_item_rows=None):
		...
		for d in self.get("items"):
			if (
				...
				and d.warehouse
				and saashq.db.get_value("Item", d.item_code, "is_stock_item") == 1
			):
	...
	def update_requested_qty_in_production_plan(self):
		...
		for production_plan in production_plans:
			doc = saashq.get_doc("Production Plan", production_plan)
			doc.set_status()
	...
	if stock_entry.doctype == "Stock Entry":
		...
		for mr, mr_item_rows in material_request_map.items():
			if mr and mr_item_rows:
				mr_obj = saashq.get_doc("Material Request", mr)
...
def update_status(name, status):
	material_request = saashq.get_doc("Material Request", name)
...
def make_purchase_order(source_name, target_doc=None, args=None):
	...
	def postprocess(source, target_doc):
		if saashq.flags.args and saashq.flags.args.default_supplier:
			# items only for given default supplier
			supplier_items = []
			for d in target_doc.items:
				default_supplier = get_item_defaults(d.item_code, target_doc.company).get("default_supplier")
				if saashq.flags.args.default_supplier == default_supplier:
...
def get_material_requests_based_on_supplier(doctype, txt, searchfield, start, page_len, filters):
	...
	if not supplier_items:
		saashq.throw(_("{0} is not the default supplier for any items.").format(supplier))

	material_requests = saashq.db.sql(
...
def get_default_supplier_query(doctype, txt, searchfield, start, page_len, filters):
	doc = saashq.get_doc("Material Request", filters.get("doc"))
	item_list = []
	for d in doc.items:
...
def make_stock_entry(source_name, target_doc=None):
	...
	def set_missing_values(source, target):
		...
		if source.job_card:
			job_card_details = saashq.get_all(
...
def raise_work_orders(material_request):
	...
	errors = []
	work_orders = []
	default_wip_warehouse = saashq.db.get_single_value("Manufacturing Settings", "default_wip_warehouse")

	for d in mr.items:
		if (d.stock_qty - d.ordered_qty) > 0:
			if saashq.db.exists("BOM", {"item": d.item_code, "is_default": 1}):
		...
		if len(work_orders) > 1:
			msgprint(
				_("The following {0} were created: {1}").format(
					saashq.bold(_("Work Orders")), "<br>" + ", ".join(work_orders_list)
				)
			)
		else:
			msgprint(
				_("The {0} {1} created successfully").format(
					saashq.bold(_("Work Order")), work_orders_list[0]
				)
			)
