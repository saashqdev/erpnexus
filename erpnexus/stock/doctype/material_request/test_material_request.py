...
from erpnexus.stock.doctype.material_request.material_request import (
...
class TestMaterialRequest(IntegrationTestCase):
	def test_material_request_qty(self):
		mr = saashq.copy_doc(self.globalTestRecords["Material Request"][0])
		mr.items[0].qty = 0
	...
	def test_make_purchase_order(self):
		mr = saashq.copy_doc(self.globalTestRecords["Material Request"][0]).insert()

		self.assertRaises(saashq.ValidationError, make_purchase_order, mr.name)

		mr = saashq.get_doc("Material Request", mr.name)
		mr.submit()
	...
	def test_make_supplier_quotation(self):
		...
		self.assertRaises(saashq.ValidationError, make_supplier_quotation, mr.name)
	...
	def test_make_stock_entry(self):
		...
		self.assertRaises(saashq.ValidationError, make_stock_entry, mr.name)
	...
	def test_cannot_stop_cancelled_material_request(self):
		...
		mr.load_from_db()
		mr.cancel()
		self.assertRaises(saashq.ValidationError, mr.update_status, "Stopped")
	...
	def test_cannot_submit_cancelled_mr(self):
		...
		mr.load_from_db()
		mr.cancel()
		self.assertRaises(saashq.ValidationError, mr.submit)
	...
	def test_cannot_change_cancelled_mr(self):
		...
		self.assertRaises(saashq.InvalidStatusError, mr.update_status, "Draft")
		self.assertRaises(saashq.InvalidStatusError, mr.update_status, "Stopped")
		self.assertRaises(saashq.InvalidStatusError, mr.update_status, "Ordered")
		self.assertRaises(saashq.InvalidStatusError, mr.update_status, "Issued")
		self.assertRaises(saashq.InvalidStatusError, mr.update_status, "Transferred")
		self.assertRaises(saashq.InvalidStatusError, mr.update_status, "Pending")
	...
	def test_cannot_delete_submitted_mr(self):
		...
		self.assertRaises(saashq.ValidationError, mr.delete)
	...
	def test_completed_qty_for_purchase(self):
		...
		po = saashq.copy_doc(po_doc)
		po.insert()
		po.load_from_db()
		mr.update_status("Stopped")
		self.assertRaises(saashq.InvalidStatusError, po.submit)
		po.db_set("docstatus", 1)
		self.assertRaises(saashq.InvalidStatusError, po.cancel)
	...
	def test_completed_qty_for_transfer(self):
		...
		# check for stopped status of Material Request
		se = saashq.copy_doc(se_doc)
		se.insert()
		mr.update_status("Stopped")
		self.assertRaises(saashq.InvalidStatusError, se.submit)
		...
		se.submit()
		mr.update_status("Stopped")
		self.assertRaises(saashq.InvalidStatusError, se.cancel)
	...
	def test_over_transfer_qty_allowance(self):
		mr = saashq.new_doc("Material Request")
		...
		saashq.db.set_single_value("Stock Settings", "mr_qty_allowance", 20)
		...
		se = saashq.copy_doc(se_doc)
		se.insert()
		self.assertRaises(saashq.ValidationError)
		se.items[0].qty = 12
		se.submit()
	...
	def test_incorrect_mapping_of_stock_entry(self):
		...
		# check for stopped status of Material Request
		se = saashq.copy_doc(se_doc)
		self.assertRaises(saashq.MappingMismatchError, se.insert)
	...
	def _get_requested_qty(self, item_code, warehouse):
		return flt(
			saashq.db.get_value("Bin", {"item_code": item_code, "warehouse": warehouse}, "indented_qty")
		)
	...
	def test_material_request_type_manufacture(self):
		mr = saashq.copy_doc(self.globalTestRecords["Material Request"][1]).insert()
		mr = saashq.get_doc("Material Request", mr.name)
		mr.submit()
		completed_qty = mr.items[0].ordered_qty
		requested_qty = saashq.db.sql(
		...
		po = saashq.get_doc("Work Order", prod_order[0])
		...
		new_requested_qty = saashq.db.sql(
			"""select indented_qty from `tabBin` where \
			item_code= %s and warehouse= %s """,
			(mr.items[0].item_code, mr.items[0].warehouse),
		)[0][0]
	...
	def test_requested_qty_multi_uom(self):
		...
		wo = saashq.get_doc("Work Order", work_order[0])
		wo.qty = 50
	...
	def test_multi_uom_for_purchase(self):
		...
		if not saashq.db.get_value("UOM Conversion Detail", {"parent": item.item_code, "uom": "Kg"}):
			item_doc = saashq.get_doc("Item", item.item_code)
	...
	def test_auto_email_users_with_company_user_permissions(self):
		from erpnexus.stock.reorder_item import get_email_list
		...
		comapnywise_mr_list = saashq._dict({})
...
def get_in_transit_warehouse(company):
	...
	in_transit_warehouse = saashq.db.exists("Warehouse", {"warehouse_type": "Transit", "company": company})
