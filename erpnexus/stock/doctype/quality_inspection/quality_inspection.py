...
from saashq.utils import cint, cstr, flt, get_number_format_info

from erpnexus.stock.doctype.quality_inspection_template.quality_inspection_template import (
...
class QualityInspection(Document):
	...
	if TYPE_CHECKING:
		...
		from erpnexus.stock.doctype.quality_inspection_reading.quality_inspection_reading import (
	...
	def validate(self):
		...
		if self.inspection_type == "In Process" and self.reference_type == "Job Card":
			item_qi_template = saashq.db.get_value("Item", self.item_code, "quality_inspection_template")
	...
	def get_item_specification_details(self):
		if not self.quality_inspection_template:
			self.quality_inspection_template = saashq.db.get_value(
	...
	def get_quality_inspection_template(self):
		template = ""
		if self.bom_no:
			template = saashq.db.get_value("BOM", self.bom_no, "quality_inspection_template")

		if not template:
			template = saashq.db.get_value("BOM", self.item_code, "quality_inspection_template")
	...
	def validate_readings_status_mandatory(self):
		for reading in self.readings:
			if not reading.status:
				saashq.throw(_("Row #{0}: Status is mandatory").format(reading.idx))
	...
	def set_status_based_on_acceptance_formula(self, reading):
		...
		try:
			result = saashq.safe_eval(condition, None, data)
			reading.status = "Accepted" if result else "Rejected"
		except NameError as e:
			field = saashq.bold(e.args[0].split()[1])
			saashq.throw(
				_(
...
def item_query(doctype, txt, searchfield, start, page_len, filters):
	...
	if not from_doctype or not saashq.db.exists("DocType", from_doctype):
		return []
...
def make_quality_inspection(source_name, target_doc=None):
	def postprocess(source, doc):
		doc.inspected_by = saashq.session.user
