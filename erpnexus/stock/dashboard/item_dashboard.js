...
erpnexus.stock.ItemDashboard = class ItemDashboard {
	constructor(opts) {
		$.extend(this, opts);
		this.make();
	}
	make() {
		...
		this.content = $(saashq.render_template("item_dashboard")).appendTo(this.parent);
		...
		this.content.on("click", ".btn-edit", function () {
			...
			saashq.db.get_value(
				...
				"name",
				(r) => {
					saashq.set_route("Form", "Putaway Rule", r.name);
				}
			);
		});
		...
		function handle_move_add(element, action) {
			...
			if (disable_quick_entry) {
				...
				if (action === "Add") {
					...
					erpnexus.stock.move_item(item, null, warehouse, actual_qty, rate, stock_uom, function () {
						me.refresh();
					});
				} else {
					erpnexus.stock.move_item(item, warehouse, null, actual_qty, null, stock_uom, function () {
						me.refresh();
					});
				}
			}
		...
		function open_stock_entry(item, warehouse, entry_type) {
			saashq.model.with_doctype("Stock Entry", function () {
				var doc = saashq.model.get_new_doc("Stock Entry");
				if (entry_type) {
					doc.stock_entry_type = entry_type;
				}

				var row = saashq.model.add_child(doc, "items");
				row.item_code = item;
	...
	render(data) {
		...
		if (context.data.length > 0) {
			...
			$(saashq.render_template(this.template, context)).appendTo(this.result);
		} else {
	...
	get_item_dashboard_data(data, max_count, show_item) {
		...
		let can_write = 0;
		if (saashq.boot.user.can_write.indexOf("Stock Entry") >= 0) {
			can_write = 1;
		}
	...
	get_capacity_dashboard_data(data) {
		...
		let can_write = 0;
		if (saashq.boot.user.can_write.indexOf("Putaway Rule") >= 0) {
			can_write = 1;
		}
...
erpnexus.stock.move_item = function (item, source, target, actual_qty, rate, stock_uom, callback) {
	...
	dialog.set_primary_action(__("Create Stock Entry"), function () {
		if (source && (dialog.get_value("qty") == 0 || dialog.get_value("qty") > actual_qty)) {
			saashq.msgprint(__("Quantity must be greater than zero, and less or equal to {0}", [actual_qty]));
			return;
		}

		if (dialog.get_value("source") === dialog.get_value("target")) {
			saashq.msgprint(__("Source and target warehouse must be different"));
			return;
		}

		saashq.model.with_doctype("Stock Entry", function () {
			let doc = saashq.model.get_new_doc("Stock Entry");
			doc.from_warehouse = dialog.get_value("source");
			doc.to_warehouse = dialog.get_value("target");
			doc.stock_entry_type = doc.from_warehouse ? "Material Transfer" : "Material Receipt";
			let row = saashq.model.add_child(doc, "items");
