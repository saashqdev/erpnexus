...
def get_data(
	...
	try:
		...
		if DatabaseQuery("Warehouse", user=saashq.session.user).build_match_conditions():
			filters.append(["warehouse", "in", [w.name for w in saashq.get_list("Warehouse")]])
	except saashq.PermissionError:
		# user does not have access on warehouse
		return []

	items = saashq.db.get_all(
		"Bin",
		fields=[
			"item_code",
			"warehouse",
	...
	for item in items:
		item.update(
			{
				"item_name": saashq.get_cached_value("Item", item.item_code, "item_name"),
				"stock_uom": saashq.get_cached_value("Item", item.item_code, "stock_uom"),
				"disable_quick_entry": saashq.get_cached_value("Item", item.item_code, "has_batch_no")
				or saashq.get_cached_value("Item", item.item_code, "has_serial_no"),
