...
from saashq.query_builder.functions import CombineDatetime, Sum, Timestamp
from saashq.utils import add_days, cint, cstr, flt, get_link_to_form, now, nowtime, today
from pypika import Order

from erpnexus.stock.deprecated_serial_batch import (
	DeprecatedBatchNoValuation,
	DeprecatedSerialNoValuation,
)
from erpnexus.stock.valuation import round_off_if_near_zero
...
class SerialBatchBundle:
	...
	def set_item_details(self):
		...
		self.item_details = saashq.get_cached_value("Item", self.sle.item_code, fields, as_dict=1)
	...
	def is_material_transfer(self):
		...
		if (
			...
			and saashq.get_cached_value("Stock Entry", self.sle.voucher_no, "purpose") in allowed_types
		):
			return True

	def make_serial_batch_no_bundle_for_material_transfer(self):
		from erpnexus.controllers.stock_controller import make_bundle_for_material_transfer

		bundle = saashq.db.get_value(
	...
	def validate_actual_qty(self, sn_doc):
		...
		if not condition:
			...
			saashq.throw(_(msg), title=_("Incorrect Type of Transaction"))
	...
	def validate_item(self):
		...
		if self.sle.actual_qty > 0:
			...
			if (
				...
				and not saashq.db.get_single_value("Stock Settings", "naming_series_prefix")
			):
			...
			if not saashq.db.get_single_value(
	...
	def set_serial_and_batch_bundle(self, sn_doc):
		...
		if sn_doc.is_rejected:
			...
			saashq.db.set_value(self.child_doctype, self.sle.voucher_detail_no, values_to_update)
	...
	def validate_item_and_warehouse(self):
		if self.sle.serial_and_batch_bundle and not saashq.db.exists(
	...
	def delink_serial_and_batch_bundle(self):
		...
		saashq.db.set_value(self.child_doctype, self.sle.voucher_detail_no, update_values)
	...
	def post_process(self):
		...
		if self.sle.serial_and_batch_bundle:
			docstatus = saashq.get_cached_value(
	...
	def cancel_serial_and_batch_bundle(self):
		...
		saashq.get_cached_doc("Serial and Batch Bundle", self.sle.serial_and_batch_bundle).cancel()
	...
	def submit_serial_and_batch_bundle(self):
		doc = saashq.get_doc("Serial and Batch Bundle", self.sle.serial_and_batch_bundle)
	...
	def set_warehouse_and_status_in_serial_nos(self):
		from erpnexus.stock.doctype.serial_no.serial_no import get_serial_nos as get_parsed_serial_nos
		...
		if status == "Delivered":
			warranty_period = saashq.get_cached_value("Item", self.sle.item_code, "warranty_period")
			if warranty_period:
	...
	def update_batch_qty(self):
		from erpnexus.stock.doctype.batch.batch import get_available_batches

		batches = get_batch_nos(self.sle.serial_and_batch_bundle)
		if not self.sle.serial_and_batch_bundle and self.sle.batch_no:
			batches = saashq._dict({self.sle.batch_no: self.sle.actual_qty})

		batches_qty = get_available_batches(
			saashq._dict({"item_code": self.item_code, "batch_no": list(batches.keys())})
		)

		for batch_no in batches:
			saashq.db.set_value("Batch", batch_no, "batch_qty", batches_qty.get(batch_no, 0))
...
def get_serial_nos(serial_and_batch_bundle, serial_nos=None):
	...
	serial_nos = saashq.get_all("Serial and Batch Entry", filters=filters, order_by="idx", pluck="serial_no")
...
def get_serial_or_batch_nos(bundle):
	...
	bundle_data = saashq.get_cached_value(
	...
	data = saashq.get_all("Serial and Batch Entry", fields=fields, filters={"parent": bundle})
...
class SerialNoValuation(DeprecatedSerialNoValuation):
	...
	def calculate_stock_value_change(self):
		if flt(self.sle.actual_qty) > 0:
			self.stock_value_change = saashq.get_cached_value(
	...
	def get_incoming_rate_from_bundle(self, serial_no) -> float:
		bundle = saashq.qb.DocType("Serial and Batch Bundle")
		bundle_child = saashq.qb.DocType("Serial and Batch Entry")

		query = (
			saashq.qb.from_(bundle)
...
def is_rejected(voucher_type, voucher_detail_no, warehouse):
	if voucher_type in ["Purchase Receipt", "Purchase Invoice"]:
		return warehouse == saashq.get_cached_value(
...
class SerialBatchCreation:
	...
	def set_item_details(self):
		...
		item_details = saashq.get_cached_value("Item", self.item_code, fields, as_dict=1)
	...
	def duplicate_package(self):
		...
		package = saashq.get_doc("Serial and Batch Bundle", id)
		new_package = saashq.copy_doc(package)
	...
	def make_serial_and_batch_bundle(self):
		doc = saashq.new_doc("Serial and Batch Bundle")
		...
		if doc.voucher_no and saashq.get_cached_value(doc.voucher_type, doc.voucher_no, "docstatus") == 2:
			doc.voucher_no = ""
	...
	def update_serial_and_batch_entries(self):
		doc = saashq.get_doc("Serial and Batch Bundle", self.serial_and_batch_bundle)
	...
	def validate_qty(self, doc):
		if doc.type_of_transaction == "Outward" and self.actual_qty and doc.total_qty:
			...
			if required_qty - total_qty > 0:
				...
				saashq.throw(msg, title=_("Insufficient Stock"))

	def set_auto_serial_batch_entries_for_outward(self):
		from erpnexus.stock.doctype.batch.batch import get_available_batches
		from erpnexus.stock.doctype.serial_no.serial_no import get_serial_nos_for_outward
	...
	def set_auto_serial_batch_entries_for_inward(self):
		...
		if self.has_serial_no:
			...
			self.batches = saashq._dict({self.batch_no: abs(self.actual_qty)})

	def make_serial_no_if_not_exists(self):
		non_exists_serial_nos = []
		for row in self.serial_nos:
			if not saashq.db.exists("Serial No", row):
	...
	def set_serial_batch_entries(self, doc):
		...
		if self.get("serial_nos"):
			serial_no_wise_batch = saashq._dict({})
			if self.has_batch_no:
	...
	def create_batch(self):
		...
		if self.is_rejected:
			...
			if bundle:
				if batch_no := saashq.db.get_value("Serial and Batch Entry", {"parent": bundle}, "batch_no"):
					return batch_no
...
def get_serial_or_batch_items(items):
	serial_or_batch_items = saashq.get_all(
		"Item",
