...
def setup_taxes_and_charges(company_name: str, country: str):
	if not saashq.db.exists("Company", company_name):
		saashq.throw(_("Company {} does not exist yet. Taxes setup aborted.").format(company_name))
...
def from_detailed_data(company_name, data):
	...
	coa_name = saashq.db.get_value("Company", company_name, "chart_of_accounts")
...
def update_regional_tax_settings(country, company):
	path = saashq.get_app_path("erpnexus", "regional", saashq.scrub(country))
	if os.path.exists(path.encode("utf-8")):
		try:
			module_name = f"erpnexus.regional.{saashq.scrub(country)}.setup.update_regional_tax_settings"
			saashq.get_attr(module_name)(country, company)
		except (ImportError, AttributeError):
			pass
		except Exception:
			# Log error and ignore if failed to setup regional tax settings
			saashq.log_error("Unable to setup regional tax settings")
...
def make_taxes_and_charges_template(company_name, doctype, template):
	...
	if saashq.db.exists(doctype, {"title": template.get("title"), "company": company_name}):
		return
	...
	for tax_row in template.get("taxes"):
		...
		tax_row_defaults = {
			...
			"cost_center": saashq.db.get_value("Company", company_name, "cost_center"),
		}
...
def get_or_create_account(company_name, account):
	...
	existing_accounts = saashq.get_all(
		"Account",
	...
	if existing_accounts:
		return saashq.get_doc("Account", existing_accounts[0].name)
...
def get_or_create_tax_group(company_name, root_type):
	...
	tax_group_name = saashq.db.get_value(
		"Account",
	...
	tax_group_account = saashq.get_doc(
		{
			"doctype": "Account",
			"company": company_name,
			"is_group": 1,
...
def make_tax_category(tax_category):
	...
	if not saashq.db.exists(doctype, tax_category["title"]):
		doc = saashq.get_doc(tax_category)
