...
from erpnexus.accounts.doctype.account.account import RootNotEditable
from erpnexus.regional.address_template.setup import set_up_address_templates
...
def install(country=None):
	...
	response = saashq.read_file(os.path.join(base_path, "delivery_trip/dispatch_notification_template.html"))
	...
	from erpnexus.buying.doctype.supplier_scorecard.supplier_scorecard import make_default_records
...
def update_buying_defaults():
	buying_settings = saashq.get_doc("Buying Settings")
...
def add_uom_data():
	# add UOMs
	uoms = json.loads(
		open(saashq.get_app_path("erpnexus", "setup", "setup_wizard", "data", "uom_data.json")).read()
	)
	for d in uoms:
		if not saashq.db.exists("UOM", d.get("uom_name")):
			doc = saashq.new_doc("UOM")
			doc.update(d)
			doc.save()
	...
	uom_conversions = json.loads(
		open(
			saashq.get_app_path("erpnexus", "setup", "setup_wizard", "data", "uom_conversion_data.json")
		).read()
	)
	for d in uom_conversions:
		if not saashq.db.exists("UOM Category", d.get("category")):
			saashq.get_doc({"doctype": "UOM Category", "category_name": d.get("category")}).db_insert()
...
def add_sale_stages():
	...
	for sales_stage in records:
		saashq.get_doc(sales_stage).db_insert()
...
def install_defaults(args=None):  # nosemgrep
	...
	saashq.db.set_value("Currency", args.get("currency"), "enabled", 1)
	saashq.db.set_single_value("Stock Settings", "email_footer_address", args.get("company_name"))
...
def set_global_defaults(args):
	global_defaults = saashq.get_doc("Global Defaults", "Global Defaults")
...
def update_stock_settings():
	...
	stock_settings.default_warehouse = saashq.db.get_value("Warehouse", {"warehouse_name": _("Stores")})
...
def create_bank_account(args):
	...
	bank_account_group = saashq.db.get_value(
		"Account",
	...
	if bank_account_group:
		...
		try:
			...
			saashq.throw(_("Bank account cannot be named as {0}").format(args.get("bank_account")))
