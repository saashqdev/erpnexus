...
def setup_demo_data():
	...
	capture("demo_data_creation_started", "erpnexus")
	try:
		company = create_demo_company()
		process_masters()
		make_transactions(company)
		saashq.cache.delete_keys("bootinfo")
		saashq.publish_realtime("demo_data_complete")
	except Exception:
		saashq.log_error("Failed to create demo data")
		capture("demo_data_creation_failed", "erpnexus", properties={"exception": saashq.get_traceback()})
		raise
	capture("demo_data_creation_completed", "erpnexus")
...
def clear_demo_data():
	...
	capture("demo_data_erased", "erpnexus")
	try:
		company = saashq.db.get_single_value("Global Defaults", "demo_company")
		create_transaction_deletion_record(company)
		clear_masters()
		delete_company(company)
		default_company = saashq.db.get_single_value("Global Defaults", "default_company")
		saashq.db.set_default("company", default_company)
	except Exception:
		saashq.db.rollback()
		saashq.log_error("Failed to erase demo data")
		saashq.throw(
...
def create_demo_company():
	company = saashq.db.get_all("Company")[0].name
	company_doc = saashq.get_doc("Company", company)

	# Make a dummy company
	new_company = saashq.new_doc("Company")
	...
	saashq.db.set_single_value("Global Defaults", "demo_company", new_company.name)
	saashq.db.set_default("company", new_company.name)

	bank_account = create_bank_account({"company_name": new_company.name})
	saashq.db.set_value("Company", new_company.name, "default_bank_account", bank_account.name)

	return new_company.name


def process_masters():
	for doctype in saashq.get_hooks("demo_master_doctypes"):
...
def create_demo_record(doctype):
	saashq.get_doc(doctype).insert(ignore_permissions=True)


def make_transactions(company):
	saashq.db.set_single_value("Stock Settings", "allow_negative_stock", 1)
	from erpnexus.accounts.utils import FiscalYearError

	try:
		start_date = get_fiscal_year(date=getdate())[1]
	except FiscalYearError:
		# User might have setup fiscal year for previous or upcoming years
		active_fiscal_years = saashq.db.get_all("Fiscal Year", filters={"disabled": 0}, as_list=1)
		if active_fiscal_years:
			start_date = saashq.db.get_value("Fiscal Year", active_fiscal_years[0][0], "year_start_date")
		else:
			saashq.throw(_("There are no active Fiscal Years for which Demo Data can be generated."))

	for doctype in saashq.get_hooks("demo_transaction_doctypes"):
		data = read_data_file_using_hooks(doctype)
		if data:
			for item in json.loads(data):
...
def create_transaction_deletion_record(company):
	transaction_deletion_record = saashq.new_doc("Transaction Deletion Record")
...
def clear_masters():
	for doctype in saashq.get_hooks("demo_master_doctypes")[::-1]:
		data = read_data_file_using_hooks(doctype)
		if data:
			for item in json.loads(data):
...
def clear_demo_record(document):
	...
	valid_columns = saashq.get_meta(document_type).get_valid_columns()
	...
	try:
		doc = saashq.get_doc(document_type, filters)
...
def delete_company(company):
	saashq.db.set_single_value("Global Defaults", "demo_company", "")
	saashq.delete_doc("Company", company, ignore_permissions=True)
...
def get_warehouse(company):
	warehouses = saashq.db.get_all("Warehouse", {"company": company, "is_group": 0})
