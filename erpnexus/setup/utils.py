...
from saashq.utils import add_days, flt, get_datetime_str, nowdate
...
def before_tests():
	...
	if not saashq.db.a_row_exists("Company"):
...
def get_exchange_rate(from_currency, to_currency, transaction_date=None, args=None):
	...
	currency_settings = saashq.get_doc("Accounts Settings").as_dict()
	...
	if saashq.get_cached_value("Currency Exchange Settings", "Currency Exchange Settings", "disabled"):
		return 0.00
	...
	try:
		...
		if not value:
			...
			settings = saashq.get_cached_doc("Currency Exchange Settings")
			req_params = {
		...
		saashq.log_error("Unable to fetch exchange rate")
		saashq.msgprint(
			_(
...
def _enable_all_roles_for_admin():
	...
	all_roles = set(saashq.db.get_values("Role", pluck="name"))
	admin_roles = set(
		saashq.db.get_values("Has Role", {"parent": "Administrator"}, fieldname="role", pluck="role")
	)
...
def set_defaults_for_tests():
	...
	saashq.db.set_single_value("Selling Settings", defaults)
	for key, value in defaults.items():
		saashq.db.set_default(key, value)
...
def welcome_email():
	site_name = get_default_company() or "ERPNexus"
