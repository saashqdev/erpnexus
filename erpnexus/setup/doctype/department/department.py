...
class Department(NestedSet):
	...
	def before_rename(self, old, new, merge=False):
		...
		if saashq.get_cached_value("Company", self.company, "abbr") not in new:
			new = get_abbreviated_name(new, self.company)

		return new

	def on_update(self):
		if not (saashq.local.flags.ignore_update_nsm or saashq.flags.in_setup_wizard):
			super().on_update()
...
def on_doctype_update():
	saashq.db.add_index("Department", ["lft", "rgt"])


def get_abbreviated_name(name, company):
	abbr = saashq.get_cached_value("Company", company, "abbr")
...
def get_children(doctype, parent=None, company=None, is_root=False, include_disabled=False):
	...
	return saashq.get_all("Department", fields=fields, filters=filters, order_by="name")
