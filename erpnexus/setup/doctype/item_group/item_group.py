...
class ItemGroup(NestedSet):
	...
	def validate(self):
		if not self.parent_item_group and not saashq.flags.in_test:
			if saashq.db.exists("Item Group", _("All Item Groups")):
	...
	def delete_child_item_groups_key(self):
		saashq.cache().hdel("child_item_groups", self.name)

	def validate_item_group_defaults(self):
		from erpnexus.stock.doctype.item.item import validate_item_default_company_links

		validate_item_default_company_links(self.item_group_defaults)


def get_child_item_groups(item_group_name):
	item_group = saashq.get_cached_value("Item Group", item_group_name, ["lft", "rgt"], as_dict=1)
...
def get_item_group_defaults(item, company):
	...
	item_group = saashq.get_cached_doc("Item Group", item.item_group)
