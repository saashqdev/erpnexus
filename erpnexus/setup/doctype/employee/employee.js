...
erpnexus.setup.EmployeeController = class EmployeeController extends saashq.ui.form.Controller {
	setup() {
		this.frm.fields_dict.user_id.get_query = function (doc, cdt, cdn) {
			return {
				query: "saashq.core.doctype.user.user.user_query",
...
saashq.ui.form.on("Employee", {
	...
	update_contact: function (frm) {
		var prefered_email_fieldname = saashq.model.scrub(frm.doc.prefered_contact_email) || "user_id";
	...
	create_user: function (frm) {
		if (!frm.doc.prefered_email) {
			saashq.throw(__("Please enter Preferred Contact Email"));
		}
		saashq.call({
			method: "erpnexus.setup.doctype.employee.employee.create_user",
			args: {
...
cur_frm.cscript = new erpnexus.setup.EmployeeController({
	frm: cur_frm,
});

saashq.tour["Employee"] = [
	{
