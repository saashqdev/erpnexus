...
from saashq.desk.page.setup_wizard.setup_wizard import make_records
from saashq.utils import cint, formatdate, get_link_to_form, get_timestamp, today
from saashq.utils.nestedset import NestedSet, rebuild_tree

from erpnexus.accounts.doctype.account.account import get_account_currency
from erpnexus.setup.setup_wizard.operations.taxes_setup import setup_taxes_and_charges
...
class Company(NestedSet):
	...
	def validate_abbr(self):
		...
		if not self.abbr.strip():
			saashq.throw(_("Abbreviation is mandatory"))

		if saashq.db.sql("select abbr from tabCompany where name!=%s and abbr=%s", (self.name, self.abbr)):
			saashq.throw(_("Abbreviation already used for another company"))
	...
	def validate_default_accounts(self):
		...
		for account in accounts:
			if self.get(account[1]):
				for_company = saashq.db.get_value("Account", self.get(account[1]), "company")
				...
				if get_account_currency(self.get(account[1])) != self.default_currency:
					...
					).format(saashq.bold(account[0]))
					saashq.throw(error_message)

	def validate_advance_account_currency(self):
		if (
			self.default_advance_received_account
			and saashq.get_cached_value("Account", self.default_advance_received_account, "account_currency")
			!= self.default_currency
		):
			saashq.throw(
				_("'{0}' should be in company currency {1}.").format(
					saashq.bold(_("Default Advance Received Account")), saashq.bold(self.default_currency)
				)
			)

		if (
			self.default_advance_paid_account
			and saashq.get_cached_value("Account", self.default_advance_paid_account, "account_currency")
			!= self.default_currency
		):
			saashq.throw(
				_("'{0}' should be in company currency {1}.").format(
					saashq.bold(_("Default Advance Paid Account")), saashq.bold(self.default_currency)
				)
			)
	...
	def validate_currency(self):
		if self.is_new():
			return
		self.previous_default_currency = saashq.get_cached_value("Company", self.name, "default_currency")
		if (
			self.default_currency
	...
	def on_update(self):
		...
		if not saashq.db.sql(
			"""select name from tabAccount
				where company=%s and docstatus<2 limit 1""",
			self.name,
		):
			if not saashq.local.flags.ignore_chart_of_accounts:
				saashq.flags.country_change = True
				self.create_default_accounts()
				self.create_default_warehouses()

		if not saashq.db.get_value("Cost Center", {"is_group": 0, "company": self.name}):
			self.create_default_cost_center()

		if saashq.flags.country_change:
			install_country_fixtures(self.name, self.country)
			self.create_default_tax_template()

		if not saashq.db.get_value("Department", {"company": self.name}):
		...
		if self.default_currency:
			saashq.db.set_value("Currency", self.default_currency, "enabled", 1)

		if (
			hasattr(saashq.local, "enable_perpetual_inventory")
			and self.name in saashq.local.enable_perpetual_inventory
		):
			saashq.local.enable_perpetual_inventory[self.name] = self.enable_perpetual_inventory

		if saashq.flags.parent_company_changed:
	...
	def create_default_warehouses(self):
		for wh_detail in [
			...
			if not saashq.db.exists("Warehouse", "{} - {}".format(wh_detail["warehouse_name"], self.abbr)):
				warehouse = saashq.get_doc(
					{
						"doctype": "Warehouse",
	...
	def create_default_accounts(self):
		from erpnexus.accounts.doctype.account.chart_of_accounts.chart_of_accounts import create_charts

		saashq.local.flags.ignore_root_company_validation = True
		...
		self.db_set(
			...
			saashq.db.get_value("Account", {"company": self.name, "account_type": "Payable", "is_group": 0}),
		)
	...
	def create_default_departments(self):
		records = [
			# Department
			{
				...
				"is_group": 1,
				"parent_department": "",
				"__condition": lambda: not saashq.db.exists("Department", _("All Departments")),
			},
			{
				"doctype": "Department",
		...
		saashq.local.flags.ignore_update_nsm = True
		make_records(records)
		saashq.local.flags.ignore_update_nsm = False
	...
	def validate_coa_input(self):
		if self.create_chart_of_accounts_based_on == "Existing Company":
			...
			if not self.existing_company:
				saashq.throw(_("Please select Existing Company for creating Chart of Accounts"))
			...
			if (
				...
				saashq.throw(
					_("Set default {0} account for non stock items").format(
						saashq.bold(_("Provisional Account"))
					)
				)
	...
	def check_country_change(self):
		saashq.flags.country_change = False

		if not self.is_new() and self.country != saashq.get_cached_value("Company", self.name, "country"):
			saashq.flags.country_change = True
	...
	def validate_parent_company(self):
		if self.parent_company:
			is_group = saashq.get_value("Company", self.parent_company, "is_group")

			if not is_group:
				saashq.throw(_("Parent Company must be a group company"))
	...
	def set_default_accounts(self):
		...
		if not self.default_income_account:
			income_account = saashq.db.get_all(
				"Account",
		...
		if not self.write_off_account:
			write_off_acct = saashq.db.get_value(
		...
		if not self.exchange_gain_loss_account:
			exchange_gain_loss_acct = saashq.db.get_value(
		...
		if not self.disposal_account:
			disposal_acct = saashq.db.get_value(
				"Account",
	...
	def set_mode_of_payment_account(self):
		cash = saashq.db.get_value("Mode of Payment", {"type": "Cash"}, "name")
		if (
			cash
			and self.default_cash_account
			and not saashq.db.get_value("Mode of Payment Account", {"company": self.name, "parent": cash})
		):
			mode_of_payment = saashq.get_doc("Mode of Payment", cash, for_update=True)
	...
	def on_trash(self):
		...
		rec = saashq.db.sql("SELECT name from `tabGL Entry` where company = %s", self.name)
		if not rec:
			saashq.db.sql(
			...
			for doctype in ["Account", "Cost Center", "Budget", "Party Account"]:
				saashq.db.sql(f"delete from `tab{doctype}` where company = %s", self.name)

		if not saashq.db.get_value("Stock Ledger Entry", {"company": self.name}):
			saashq.db.sql("""delete from `tabWarehouse` where company=%s""", self.name)

		saashq.defaults.clear_default("company", value=self.name)
		for doctype in ["Mode of Payment Account", "Item Default"]:
			saashq.db.sql(f"delete from `tab{doctype}` where company = %s", self.name)

		# clear default accounts, warehouses from item
		warehouses = saashq.db.sql_list("select name from tabWarehouse where company=%s", self.name)
		if warehouses:
			saashq.db.sql(
		...
		# delete BOMs
		boms = saashq.db.sql_list("select name from tabBOM where company=%s", self.name)
		if boms:
			saashq.db.sql("delete from tabBOM where company=%s", self.name)
		...
		saashq.db.sql("delete from tabEmployee where company=%s", self.name)
		saashq.db.sql("delete from tabDepartment where company=%s", self.name)
		saashq.db.sql("delete from `tabTax Withholding Account` where company=%s", self.name)
		saashq.db.sql("delete from `tabTransaction Deletion Record` where company=%s", self.name)

		# delete tax templates
		saashq.db.sql("delete from `tabSales Taxes and Charges Template` where company=%s", self.name)
		saashq.db.sql("delete from `tabPurchase Taxes and Charges Template` where company=%s", self.name)
		saashq.db.sql("delete from `tabItem Tax Template` where company=%s", self.name)

		# delete Process Deferred Accounts if no GL Entry found
		if not saashq.db.get_value("GL Entry", {"company": self.name}):
			saashq.db.sql("delete from `tabProcess Deferred Accounting` where company=%s", self.name)

	def check_parent_changed(self):
		saashq.flags.parent_company_changed = False

		if not self.is_new() and self.parent_company != saashq.db.get_value(
			"Company", self.name, "parent_company"
		):
			saashq.flags.parent_company_changed = True
...
def install_country_fixtures(company, country):
	try:
		module_name = f"erpnexus.regional.{saashq.scrub(country)}.setup.setup"
		saashq.get_attr(module_name)(company, False)
	except ImportError:
		pass
	except Exception:
		saashq.log_error("Unable to set country fixtures")
		saashq.throw(
...
def update_company_current_month_sales(company):
	...
	results = saashq.db.sql(
		...
		WHERE
			...
			AND docstatus = 1
			AND company = {saashq.db.escape(company)}
		GROUP BY
			month_year
	""",
		as_dict=True,
	...
	saashq.db.set_value("Company", company, "total_monthly_sales", monthly_total)
...
def update_company_monthly_sales(company):
	...
	from saashq.utils.goal import get_monthly_results

	filter_str = f"company = {saashq.db.escape(company)} and status != 'Draft' and docstatus=1"
	month_to_value_dict = get_monthly_results(
		"Sales Invoice", "base_grand_total", "posting_date", filter_str, "sum"
	)

	saashq.db.set_value("Company", company, "sales_monthly_history", json.dumps(month_to_value_dict))
...
def update_transactions_annual_history(company, commit=False):
	...
	saashq.db.set_value("Company", company, "transactions_annual_history", json.dumps(transactions_history))
...
def cache_companies_monthly_sales_history():
	companies = [d["name"] for d in saashq.get_list("Company")]
...
def get_children(doctype, parent=None, company=None, is_root=False):
	...
	return saashq.db.sql(
		...
		from
			`tabCompany` comp
		where
			ifnull(parent_company, "")={saashq.db.escape(parent)}
		""",
		as_dict=1,
	)
...
def get_timeline_data(doctype, name):
	...
	history = saashq.get_cached_value("Company", name, "transactions_annual_history")
...
def create_transaction_deletion_request(company):
	from erpnexus.setup.doctype.transaction_deletion_record.transaction_deletion_record import (
