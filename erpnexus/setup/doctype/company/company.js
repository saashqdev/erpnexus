...
saashq.provide("erpnexus.company");

saashq.ui.form.on("Company", {
	onload: function (frm) {
		if (frm.doc.__islocal && frm.doc.parent_company) {
			saashq.db.get_value("Company", frm.doc.parent_company, "is_group", (r) => {
				if (!r.is_group) {
	...
	date_of_commencement: function (frm) {
		if (frm.doc.date_of_commencement < frm.doc.date_of_incorporation) {
			saashq.throw(__("Date of Commencement should be greater than Date of Incorporation"));
		}
	...
	refresh: function (frm) {
		erpnexus.company.setup_queries(frm);
		...
		if (!frm.is_new()) {
			...
			if (saashq.perm.has_perm("Cost Center", 0, "read")) {
				frm.add_custom_button(
					__("Cost Centers"),
					function () {
						saashq.set_route("Tree", "Cost Center", { company: frm.doc.name });
					},
					__("View")
				);
			}

			if (saashq.perm.has_perm("Account", 0, "read")) {
				frm.add_custom_button(
					__("Chart of Accounts"),
					function () {
						saashq.set_route("Tree", "Account", { company: frm.doc.name });
					},
					__("View")
				);
			}

			if (saashq.perm.has_perm("Sales Taxes and Charges Template", 0, "read")) {
				frm.add_custom_button(
					__("Sales Tax Template"),
					function () {
						saashq.set_route("List", "Sales Taxes and Charges Template", {
			...
			if (saashq.perm.has_perm("Purchase Taxes and Charges Template", 0, "read")) {
				frm.add_custom_button(
					__("Purchase Tax Template"),
					function () {
						saashq.set_route("List", "Purchase Taxes and Charges Template", {
							company: frm.doc.name,
						});
					},
					__("View")
		...
		erpnexus.company.set_chart_of_accounts_options(frm.doc);
	},
	...
	make_default_tax_template: function (frm) {
		frm.call({
			...
			freeze: true,
			callback: function () {
				saashq.msgprint(__("Default tax templates for sales, purchase and items are created."));
			},
		});
	},
	...
	delete_company_transactions: function (frm) {
		saashq.call({
			method: "erpnexus.setup.doctype.transaction_deletion_record.transaction_deletion_record.is_deletion_doc_running",
			args: {
				company: frm.doc.name,
			},
			freeze: true,
						...
						var d = saashq.prompt(
							...
							function (data) {
								if (data.company_name !== frm.doc.name) {
									saashq.msgprint(__("Company name not same"));
									return;
								}
								saashq.call({
									method: "erpnexus.setup.doctype.company.company.create_transaction_deletion_request",
									args: {
									...
									onerror: function () {
										saashq.msgprint(__("Wrong Password"));
									},
								});
							},
...
erpnexus.company.set_chart_of_accounts_options = function (doc) {
	var selected_value = doc.chart_of_accounts;
	if (doc.country) {
		return saashq.call({
			method: "erpnexus.accounts.doctype.account.chart_of_accounts.chart_of_accounts.get_charts_for_country",
			args: {
				country: doc.country,
				with_standard: true,
			},
...
erpnexus.company.setup_queries = function (frm) {
	$.each(
		[
		...
		function (i, v) {
			erpnexus.company.set_custom_query(frm, v);
		}
	);
...
erpnexus.company.set_custom_query = function (frm, v) {
	var filters = {
		company: frm.doc.name,
		is_group: 0,
	};
