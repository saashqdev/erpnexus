...
import saashq
import saashq.desk.notifications
from dateutil.relativedelta import relativedelta
from saashq import _
from saashq.core.doctype.user.user import STANDARD_USERS
...
from erpnexus.accounts.utils import get_balance_on, get_count_on, get_fiscal_year
...
class EmailDigest(Document):
	...
	if TYPE_CHECKING:
		...
		from erpnexus.setup.doctype.email_digest_recipient.email_digest_recipient import (
	...
	def __init__(self, *args, **kwargs):
		...
		self.set_dates()
		self._accounts = {}
		self.currency = saashq.db.get_value("Company", self.company, "default_currency")

	@saashq.whitelist()
	def get_users(self):
		"""get list of users"""
		user_list = saashq.db.sql(
			"""
		...
		saashq.response["user_list"] = user_list
	...
	def get_msg_html(self):
		...
		from erpnexus.setup.doctype.email_digest.quotes import get_random_quote
		...
		if self.get("purchase_orders_items_overdue"):
			...
			if not context.purchase_order_list:
				saashq.throw(_("No items to be received are overdue"))

		if not context:
			return None

		saashq.flags.ignore_account_permission = False

		# style
		return saashq.render_template(
			"erpnexus/setup/doctype/email_digest/templates/default.html", context, is_path=True
		)
	...
	def get_notifications(self):
		...
		notifications = saashq.desk.notifications.get_notifications()
	...
	def get_todo_list(self, user_id=None):
		"""Get to-do list"""
		if not user_id:
			user_id = saashq.session.user

		todo_list = saashq.db.sql(
			"""select *
	...
	def get_issue_list(self, user_id=None):
		...
		meta = saashq.get_meta("Issue")
		role_permissions = saashq.permissions.get_role_permissions(meta, user_id)
		if not role_permissions.get("read"):
			return None

		issue_list = saashq.db.sql(
			"""select *
	...
	def set_accounting_cards(self, context):
		...
		for key in (
			...
			if self.get(key):
				...
				if card:
					card = saashq.safe_eval(card)

				else:
					card = saashq._dict(getattr(self, "get_" + key)())
	...
	def get_expenses_booked(self):
		...
		expense_account = saashq.db.get_all(
			"Account",
			fields=["name"],
	...
	def get_summary_of_pending(self, doc_type, fieldname, getfield):
		value, count, billed_value, delivered_value = saashq.db.sql(
	...
	def get_purchase_orders_items_overdue_list(self):
		...
		purchase_order_list = saashq.db.sql(sql_po, as_dict=True)
		purchase_order_items_overdue_list = saashq.db.sql(sql_poi, as_dict=True)
...
def send():
	...
	for ed in saashq.db.sql(
		"""select name from `tabEmail Digest`
			where enabled=1 and docstatus<2""",
		as_list=1,
	):
		ed_obj = saashq.get_doc("Email Digest", ed[0])
...
def get_digest_msg(name):
	return saashq.get_doc("Email Digest", name).get_msg_html()
