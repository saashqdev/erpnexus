...
from saashq.utils import formatdate, getdate, today
...
class HolidayList(Document):
	...
	if TYPE_CHECKING:
		...
		from erpnexus.setup.doctype.holiday.holiday import Holiday
			...
			if row.holiday_date in unique_dates:
				saashq.throw(
					_("Holiday Date {0} added multiple times").format(
						saashq.bold(formatdate(row.holiday_date))
					)
				)
...
def is_holiday(holiday_list, date=None):
	...
	if date is None:
		date = today()
	if holiday_list:
		return bool(saashq.db.exists("Holiday", {"parent": holiday_list, "holiday_date": date}, cache=True))
	else:
		return False
...
def local_country_name(country_code: str) -> str:
	...
	return Locale.parse(saashq.local.lang, sep="-").territories.get(country_code, country_code)
