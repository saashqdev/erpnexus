...
from erpnexus.setup.doctype.holiday_list.holiday_list import local_country_name
...
class TestHolidayList(IntegrationTestCase):
	def test_holiday_list(self):
		...
		fetched_holiday_list = saashq.get_value("Holiday List", holiday_list.name)
		self.assertEqual(holiday_list.name, fetched_holiday_list)

	def test_weekly_off(self):
		holiday_list = saashq.new_doc("Holiday List")
...
def make_holiday_list(name, from_date=None, to_date=None, holiday_dates=None):
	...
	saashq.delete_doc_if_exists("Holiday List", name, force=1)
	doc = saashq.get_doc(
		{
