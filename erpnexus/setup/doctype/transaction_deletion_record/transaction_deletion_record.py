...
from saashq.utils import cint, comma_and, create_batch, get_link_to_form
from saashq.utils.background_jobs import get_job, is_job_enqueued
...
class TransactionDeletionRecord(Document):
	...
	if TYPE_CHECKING:
		...
		from erpnexus.accounts.doctype.transaction_deletion_record_details.transaction_deletion_record_details import (
			TransactionDeletionRecordDetails,
		)
		from erpnexus.setup.doctype.transaction_deletion_record_item.transaction_deletion_record_item import (
	...
	def before_submit(self):
		if queued_docs := saashq.db.get_all(
			...
			if task := getattr(self, method, None):
				try:
					...
					if traceback:
						...
						saashq.db.set_value(self.doctype, self.name, "error_log", message)
					saashq.db.set_value(self.doctype, self.name, "status", "Failed")
	...
	def delete_lead_addresses(self):
		...
		if not self.delete_leads_and_addresses:
			leads = saashq.get_all("Lead", filters={"company": self.company})
			leads = ["'%s'" % row.get("name") for row in leads]
			addresses = []
			if leads:
				addresses = saashq.db.sql_list(
					"""select parent from `tabDynamic Link` where link_name
					in ({leads})""".format(leads=",".join(leads))
				)

				if addresses:
					addresses = ["%s" % saashq.db.escape(addr) for addr in addresses]
	...
	def reset_company_values(self):
		...
		if not self.reset_company_default_values:
			company_obj = saashq.get_doc("Company", self.company)
			...
			for docfield in self.doctypes:
				if docfield.doctype_name != self.doctype and not docfield.done:
					...
					if no_of_docs > 0:
						...
						saashq.db.set_value(docfield.doctype, docfield.name, "no_of_docs", processed)
					else:
						# reset naming series
						naming_series = saashq.db.get_value("DocType", docfield.doctype_name, "autoname")
						if naming_series:
							if "#" in naming_series:
								self.update_naming_series(naming_series, docfield.doctype_name)
						saashq.db.set_value(docfield.doctype, docfield.name, "done", 1)

			pending_doctypes = saashq.db.get_all(
	...
	def get_doctypes_to_be_ignored_list(self):
		singles = saashq.get_all("DocType", filters={"issingle": 1}, pluck="name")
	...
	def get_all_child_doctypes(self):
		return saashq.get_all("DocType", filters={"istable": 1}, pluck="name")

	def get_number_of_docs_linked_with_specified_company(self, doctype, company_fieldname):
		return saashq.db.count(doctype, {company_fieldname: self.company})
	...
	def delete_child_tables(self, doctype, reference_doc_names):
		...
		for table in child_tables:
			saashq.db.delete(table, {"parent": ["in", reference_doc_names]})

	def delete_docs_linked_with_specified_company(self, doctype, reference_doc_names):
		saashq.db.delete(doctype, {"name": ("in", reference_doc_names)})
	...
	def update_naming_series(self, naming_series, doctype_name):
		...
		saashq.db.sql("""update `tabSeries` set current = %s where name=%s""", (last, prefix))
	...
	def delete_communications(self, doctype, reference_doc_names):
		...
		for batch in create_batch(communication_names, self.batch_size):
			saashq.delete_doc("Communication", batch, ignore_permissions=True)
...
def get_doctypes_to_be_ignored():
	...
	doctypes_to_be_ignored.extend(saashq.get_hooks("company_data_to_be_ignored") or [])
...
def is_deletion_doc_running(company: str | None = None, err_msg: str | None = None):
	if company:
		if running_deletion_jobs := saashq.db.get_all(
			"Transaction Deletion Record",
