...
def save_new_records(test_records):
	for record in test_records:
		...
		try:
			saashq.set_value(**kwargs)
		except saashq.DoesNotExistError:
			curr_exchange = saashq.new_doc(record.get("doctype"))
...
class TestCurrencyExchange(IntegrationTestCase):
	...
	def tearDown(self):
		saashq.db.set_single_value("Accounts Settings", "allow_stale", 1)
		self.clear_cache()
	...
	def test_exchange_rate_via_exchangerate_host(self, mock_get):
		...
		settings = saashq.get_single("Currency Exchange Settings")
	...
	def test_exchange_rate_strict(self, mock_get):
		...
		saashq.db.set_single_value("Accounts Settings", "allow_stale", 0)
		saashq.db.set_single_value("Accounts Settings", "stale_days", 1)
