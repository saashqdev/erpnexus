...
def update_address_template(country, html, is_default=0):
	...
	if not saashq.db.exists("Country", country):
		saashq.log_error(f"Country {country} for regional Address Template does not exist.")
		return

	if saashq.db.exists("Address Template", country):
		saashq.db.set_value("Address Template", country, "template", html)
		saashq.db.set_value("Address Template", country, "is_default", is_default)
	else:
		saashq.get_doc(
