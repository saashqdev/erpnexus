...
from erpnexus.regional.address_template.setup import get_address_templates, update_address_template


def ensure_country(country):
	if saashq.db.exists("Country", country):
		return saashq.get_doc("Country", country)
	else:
		c = saashq.get_doc({"doctype": "Country", "country_name": country})
		c.insert()
		return c
