...
from erpnexus.regional.report.irs_1099.irs_1099 import execute as execute_1099_report


class TestUnitedStates(IntegrationTestCase):
	def test_irs_1099_custom_field(self):
		if not saashq.db.exists("Supplier", "_US 1099 Test Supplier"):
			doc = saashq.new_doc("Supplier")
			...
			doc.save()
			saashq.db.commit()
			supplier = saashq.get_doc("Supplier", "_US 1099 Test Supplier")
			self.assertEqual(supplier.irs_1099, 1)

	def test_irs_1099_report(self):
		make_payment_entry_to_irs_1099_supplier()
		filters = saashq._dict({"fiscal_year": "_Test Fiscal Year 2016", "company": "_Test Company 1"})
...
def make_payment_entry_to_irs_1099_supplier():
	saashq.db.sql("delete from `tabGL Entry` where party='_US 1099 Test Supplier'")
	saashq.db.sql("delete from `tabGL Entry` where against='_US 1099 Test Supplier'")
	saashq.db.sql("delete from `tabPayment Entry` where party='_US 1099 Test Supplier'")
