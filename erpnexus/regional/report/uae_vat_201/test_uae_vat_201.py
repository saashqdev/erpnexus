...
from erpnexus.regional.report.uae_vat_201.uae_vat_201 import (
	get_exempt_total,
...
from erpnexus.stock.doctype.warehouse.test_warehouse import get_warehouse_account
...
class TestUaeVat201(TestCase):
	def setUp(self):
		...
		saashq.db.sql("delete from `tabSales Invoice` where company='_Test Company UAE VAT'")
		saashq.db.sql("delete from `tabPurchase Invoice` where company='_Test Company UAE VAT'")
...
def make_company(company_name, abbr):
	if not saashq.db.exists("Company", company_name):
	...
	if not saashq.db.get_value("Cost Center", {"is_group": 0, "company": company.name}):
...
def set_vat_accounts():
	if not saashq.db.exists("UAE VAT Settings", "_Test Company UAE VAT"):
		vat_accounts = saashq.get_all(
			"Account",
			fields=["name"],
...
def make_customer():
	if not saashq.db.exists("Customer", "_Test UAE Customer"):
...
def make_supplier():
	if not saashq.db.exists("Supplier", "_Test UAE Supplier"):
		saashq.get_doc(
			{
...
def create_warehouse(warehouse_name, properties=None, company=None):
	...
	if not saashq.db.exists("Warehouse", warehouse_id):
		warehouse = saashq.new_doc("Warehouse")
