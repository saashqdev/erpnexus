...
from erpnexus.regional.report.vat_audit_report.vat_audit_report import execute
...
class TestVATAuditReport(TestCase):
	...
	def tearDown(self):
		saashq.db.sql("delete from `tabSales Invoice` where company='_Test Company SA VAT'")
		saashq.db.sql("delete from `tabPurchase Invoice` where company='_Test Company SA VAT'")
...
def set_sa_vat_accounts():
	if not saashq.db.exists("South Africa VAT Settings", "_Test Company SA VAT"):
...
def make_customer():
	if not saashq.db.exists("Customer", "_Test SA Customer"):
		saashq.get_doc(
			{
				"doctype": "Customer",
...
def make_supplier():
	if not saashq.db.exists("Supplier", "_Test SA Supplier"):
		saashq.get_doc(
			{
				"doctype": "Supplier",
