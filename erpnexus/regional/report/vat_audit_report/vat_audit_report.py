...
from saashq.utils import formatdate, get_link_to_form
...
class VATAuditReport:
	...
	def get_sa_vat_accounts(self):
		self.sa_vat_accounts = saashq.get_all(
			"South Africa VAT Account", filters={"parent": self.filters.company}, pluck="account"
		)
		if not self.sa_vat_accounts and not saashq.flags.in_test and not saashq.flags.in_migrate:
			link_to_settings = get_link_to_form(
				"South Africa VAT Settings", "", label="South Africa VAT Settings"
			)
			saashq.throw(_("Please set VAT Accounts in {0}").format(link_to_settings))

	def get_invoice_data(self, doctype):
		conditions = self.get_conditions()
		self.invoices = saashq._dict()

		invoice_data = saashq.db.sql(
			f"""
			SELECT
				{self.select_columns}
			FROM
	...
	def get_invoice_items(self, doctype):
		self.invoice_items = saashq._dict()
	...
	def get_items_based_on_tax_rate(self, doctype):
		self.items_based_on_tax_rate = saashq._dict()
		self.item_tax_rate = saashq._dict()
		self.tax_doctype = (
			"Purchase Taxes and Charges" if doctype == "Purchase Invoice" else "Sales Taxes and Charges"
		)

		self.tax_details = saashq.db.sql(
			"""
			SELECT
	...
	def get_data(self, doctype):
		...
		for rate, section in consolidated_data.items():
			rate = int(rate)
			label = saashq.bold(section_name + "- " + "Rate" + " " + str(rate) + "%")
			...
			total = {
				"posting_date": saashq.bold(_("Total")),
