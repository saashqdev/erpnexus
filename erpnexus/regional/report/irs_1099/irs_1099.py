...
from saashq.utils import cstr, nowdate
from saashq.utils.data import fmt_money
from saashq.utils.jinja import render_template
from saashq.utils.pdf import get_pdf
from saashq.utils.print_format import read_multi_pdf
...
def execute(filters=None):
	filters = filters if isinstance(filters, saashq._dict) else saashq._dict(filters)
	if not filters:
		filters.setdefault("fiscal_year", get_fiscal_year(nowdate())[0])
		filters.setdefault("company", saashq.db.get_default("company"))

	region = saashq.db.get_value("Company", filters={"name": filters.company}, fieldname=["country"])
	...
	if filters.supplier_group:
		conditions += "AND s.supplier_group = %s" % saashq.db.escape(filters.get("supplier_group"))
...
def irs_1099_print(filters):
	if not filters:
		saashq._dict(
			{
				"company": saashq.db.get_default("Company"),
				"fiscal_year": saashq.db.get_default("Fiscal Year"),
			}
		)
	else:
	...
	company_tin = saashq.db.get_value("Company", filters.company, "tax_id")

	columns, data = execute(filters)
	template = saashq.get_doc("Print Format", "IRS 1099 Form").html
	output = PdfWriter()
...
def get_payer_address_html(company):
	...
	if address_list:
		...
		address_display = saashq.get_doc("Address", company_address).get_display()
...
def get_street_address_html(party_type, party):
	...
	if address_list:
		...
		doc = saashq.get_doc("Address", supplier_address)
