...
saashq.ui.form.on("Rename Tool", {
	onload: function (frm) {
		return saashq.call({
			method: "erpnexus.utilities.doctype.rename_tool.rename_tool.get_doctypes",
			callback: function (r) {
	...
	refresh: function (frm) {
		...
		frm.page.set_primary_action(__("Rename"), function () {
			...
			saashq.call({
				method: "erpnexus.utilities.doctype.rename_tool.rename_tool.upload",
				args: {
				...
				callback: function (r) {
					...
					if (r.exc) {
						r.exc = saashq.utils.parse_json(r.exc);
