...
from saashq.utils.data import get_system_timezone
...
class Video(Document):
	...
	def set_youtube_statistics(self):
		api_key = saashq.db.get_single_value("Video Settings", "api_key")
...
def is_tracking_enabled():
	return saashq.db.get_single_value("Video Settings", "enable_youtube_tracking")
...
def update_youtube_data():
	...
	video_settings = saashq.get_cached_doc("Video Settings")
...
def get_id_from_url(url):
	...
	if not isinstance(url, str):
		saashq.throw(_("URL can only be a string"), title=_("Invalid URL"))
...
def batch_update_youtube_data():
	def get_youtube_statistics(video_ids):
		...
		try:
			...
			saashq.log_error("Unable to update YouTube statistics")
	...
	def set_youtube_data(entries):
		for entry in entries:
			...
			saashq.db.set_value("Video", video_id, stats)

	video_list = saashq.get_all("Video", fields=["youtube_video_id"])
