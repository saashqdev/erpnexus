...
from saashq.utils import get_link_to_form, today
...
def transaction_processing(data, from_doctype, to_doctype, args=None):
	...
	saashq.msgprint(_("Started a background job to create {1} {0}").format(to_doctype, length_of_data))
	saashq.enqueue(
		job,
...
def retry(date: str | None = None):
	...
	if date:
		...
		if not failed_docs:
			saashq.msgprint(_("There are no Failed transactions"))
		else:
			job = saashq.enqueue(
...
def retry_failed_transactions(failed_docs: list | None):
	if failed_docs:
		for log in failed_docs:
			try:
				saashq.db.savepoint("before_creation_state")
				task(log.transaction_name, log.from_doctype, log.to_doctype)
			except Exception:
				saashq.db.rollback(save_point="before_creation_state")
				update_log(log.name, "Failed", 1, str(saashq.get_traceback(with_context=True)))
			else:
				update_log(log.name, "Success", 1)


def update_log(log_name, status, retried, err=None):
	saashq.db.set_value("Bulk Transaction Log Detail", log_name, "transaction_status", status)
	saashq.db.set_value("Bulk Transaction Log Detail", log_name, "retried", retried)
	if err:
		saashq.db.set_value("Bulk Transaction Log Detail", log_name, "error_description", err)
...
def job(deserialized_data, from_doctype, to_doctype, args):
	...
	for d in deserialized_data:
		try:
			...
			create_log(
				doc_name,
				str(saashq.get_traceback(with_context=True)),
				from_doctype,
				to_doctype,
				status="Failed",
...
def task(doc_name, from_doctype, to_doctype):
	from erpnexus.accounts.doctype.payment_entry import payment_entry
	from erpnexus.accounts.doctype.purchase_invoice import purchase_invoice
	from erpnexus.accounts.doctype.sales_invoice import sales_invoice
	from erpnexus.buying.doctype.purchase_order import purchase_order
	from erpnexus.buying.doctype.supplier_quotation import supplier_quotation
	from erpnexus.selling.doctype.quotation import quotation
	from erpnexus.selling.doctype.sales_order import sales_order
	from erpnexus.stock.doctype.delivery_note import delivery_note
	from erpnexus.stock.doctype.purchase_receipt import purchase_receipt
	...
	hooks = saashq.get_hooks("bulk_transaction_task_mapper")
	for hook in hooks:
		mapper.update(saashq.get_attr(hook)())

	saashq.flags.bulk_transaction = True
	...
	del obj
	del saashq.flags.bulk_transaction


def create_log(doc_name, e, from_doctype, to_doctype, status, log_date=None, restarted=0):
	transaction_log = saashq.new_doc("Bulk Transaction Log Detail")
