...
from saashq.utils import cint, flt, get_time, now_datetime

from erpnexus.controllers.status_updater import StatusUpdater


class UOMMustBeIntegerError(saashq.ValidationError):
	pass
...
class TransactionBase(StatusUpdater):
	def validate_posting_time(self):
		...
		if saashq.flags.in_import and self.posting_date:
		...
		if not getattr(self, "set_posting_time", None):
			...
			try:
				...
				saashq.throw(_("Invalid Posting Time"))
				...
				if ref_dn:
					if is_child:
						...
						if ref_dn not in item_ref_dn:
							...
							saashq.throw(_("Duplicate row {0} with same {1}").format(d.idx, key))
					elif ref_dn:
		...
		for reference_doctype, ref_dn_list in ref_doc.items():
			...
			for reference_name in ref_dn_list:
				...
				if not prevdoc_values:
					saashq.throw(_("Invalid reference {0} {1}").format(reference_doctype, reference_name))
	...
	def validate_rate_with_reference_doc(self, ref_details):
		...
		if self.doctype in buying_doctypes:
			action, role_allowed_to_override = saashq.get_cached_value(
			...
			for d in self.get("items"):
				if d.get(ref_link_field):
					...
					if abs(flt(d.rate - ref_rate, d.precision("rate"))) >= 0.01:
						if action == "Stop":
							if role_allowed_to_override not in saashq.get_roles():
								stop_actions.append(
		...
		if stop_actions:
			saashq.throw(stop_actions, as_list=True)
	...
	def validate_currency_for_receivable_payable_and_advance_account(self):
		if self.doctype in ["Customer", "Supplier"]:
			...
			for x in self.accounts:
				company_default_currency = saashq.get_cached_value("Company", x.company, "default_currency")
				receivable_payable_account_currency = None
				advance_account_currency = None

				if x.account:
					receivable_payable_account_currency = saashq.get_cached_value(
				...
				if receivable_payable_account_currency and (
					...
					saashq.throw(
						_(
							...
							saashq.bold(receivable_payable_account_currency),
							saashq.bold(self.default_currency),
							saashq.bold(company_default_currency),
						)
					)
...
def delete_events(ref_type, ref_name):
	...
	if events:
		saashq.delete_doc("Event", events, for_reload=True)
...
def validate_uom_is_integer(doc, uom_field, qty_fields, child_dt=None):
	...
	integer_uoms = list(
		filter(
			lambda uom: saashq.db.get_value("UOM", uom, "must_be_whole_number", cache=True) or None,
			distinct_uoms,
		)
	)
