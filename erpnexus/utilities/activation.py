...
def get_level():
	...
	for doctype, min_count in doctypes.items():
		count = saashq.db.count(doctype)
		if count > min_count:
			activation_level += 1
		sales_data.append({doctype: count})

	if saashq.db.get_single_value("System Settings", "setup_complete"):
		activation_level += 1

	communication_number = saashq.db.count("Communication", dict(communication_medium="Email"))
	if communication_number > 10:
		activation_level += 1
	sales_data.append({"Communication": communication_number})

	# recent login
	if saashq.db.sql("select name from tabUser where last_login > date_sub(now(), interval 2 day) limit 1"):
		activation_level += 1
...
def get_help_messages():
	...
	domain = saashq.get_cached_value("Company", erpnexus.get_default_company(), "domain")
	messages = []
	...
	for m in message_settings:
		if not m.domain or domain in m.domain:
			m.count = saashq.db.count(m.doctype)
			if m.count < m.target:
				messages.append(m)
