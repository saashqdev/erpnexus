...
def update_doctypes():
	for d in saashq.db.sql(
		...
		dt = saashq.get_doc("DocType", d.parent)
...
def get_site_info(site_info):
	...
	if not company:
		company = saashq.db.sql("select name from `tabCompany` order by creation asc")
		company = company[0][0] if company else None

	if company:
		domain = saashq.get_cached_value("Company", cstr(company), "domain")
...
def payment_app_import_guard():
	marketplace_link = '<a href="https://saashqcloud.com/marketplace/apps/payments">Marketplace</a>'
	github_link = '<a href="https://github.com/saashqdev/payments/">GitHub</a>'
	...
	try:
		...
		saashq.throw(msg, title=_("Missing Payments App"))
