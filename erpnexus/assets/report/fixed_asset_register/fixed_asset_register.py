...
from saashq.utils import add_months, cstr, flt, formatdate, getdate, nowdate, today
...
def execute(filters=None):
	filters = saashq._dict(filters or {})
...
def get_conditions(filters):
	...
	date_field = saashq.scrub(filters.date_based_on or "Purchase Date")
...
def get_data(filters):
	...
	group_by = saashq.scrub(filters.get("group_by"))
	...
	assets_record = saashq.db.get_all("Asset", filters=conditions, fields=fields)
...
def prepare_chart_data(data, filters):
	...
	if filters.filter_based_on not in ("Date Range", "Fiscal Year"):
		...
		date_field = saashq.scrub(filters.date_based_on)
	...
	for d in period_list:
		labels_values_map.setdefault(
			d.get("label"), saashq._dict({"asset_value": 0, "depreciated_amount": 0})
		)
...
def get_assets_linked_to_fb(filters):
	afb = saashq.qb.DocType("Asset Finance Book")

	query = saashq.qb.from_(afb).select(
		afb.parent,
	)
	...
	if filters.include_default_book_assets:
		...
		if filters.finance_book and company_fb and cstr(filters.finance_book) != cstr(company_fb):
			saashq.throw(_("To use a different finance book, please uncheck 'Include Default FB Assets'"))
...
def get_group_by_data(group_by, conditions, assets_linked_to_fb, depreciation_amount_map):
	...
	assets = saashq.db.get_all("Asset", filters=conditions, fields=fields)
		...
		return [
			{
				...
				"fieldtype": "Link",
				"fieldname": saashq.scrub(filters.get("group_by")),
