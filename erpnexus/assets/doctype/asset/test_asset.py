...
from erpnexus.assets.doctype.asset.asset import (
	make_sales_invoice,
	split_asset,
...
class AssetSetup(IntegrationTestCase):
	...
	def setUpClass(cls):
		...
		saashq.db.sql("delete from `tabTax Rule`")
...
class TestAsset(AssetSetup):
	...
	def test_gross_purchase_amount_is_mandatory(self):
		...
		self.assertRaises(saashq.MandatoryError, asset.save)
	...
	def test_pr_or_pi_mandatory_if_not_existing_asset(self):
		...
		self.assertRaises(saashq.ValidationError, asset.save)
	...
	def test_validate_item(self):
		asset = create_asset(item_code="MacBook Pro", do_not_save=1)
		item = saashq.get_doc("Item", "MacBook Pro")
	...
	def test_purchase_asset(self):
		...
		asset_name = saashq.db.get_value("Asset", {"purchase_receipt": pr.name}, "name")
	...
	def test_is_fixed_asset_set(self):
		...
		doc = saashq.new_doc("Purchase Invoice")
	...
	def test_scrap_asset(self):
		...
		asset_depreciation = saashq.db.get_value(
			"Asset Depreciation Schedule", {"asset": asset.name, "docstatus": 1}, "name"
		)
		last_booked_depreciation_date = saashq.db.get_value(
		...
		self.assertRaises(saashq.ValidationError, scrap_asset, asset.name, scrap_date=before_purchase_date)
		self.assertRaises(saashq.ValidationError, scrap_asset, asset.name, scrap_date=future_date)
		self.assertRaises(
			saashq.ValidationError, scrap_asset, asset.name, scrap_date=before_last_booked_depreciation_date
		)
	...
	def test_gle_made_by_asset_sale(self):
		...
		self.assertEqual(saashq.db.get_value("Asset", asset.name, "status"), "Sold")
		...
		si.cancel()
		self.assertEqual(saashq.db.get_value("Asset", asset.name, "status"), "Partially Depreciated")
	...
	def test_asset_splitting(self):
		...
		jv = saashq.get_doc("Journal Entry", journal_entry)
	...
	def test_cwip_accounting(self):
		...
		asset = saashq.db.get_value("Asset", {"purchase_receipt": pr.name, "docstatus": 0}, "name")

		asset_doc = saashq.get_doc("Asset", asset)
	...
	def test_asset_cwip_toggling_cases(self):
		cwip = saashq.db.get_value("Asset Category", "Computers", "enable_cwip_accounting")
		name = saashq.db.get_value(
			"Asset Category Account", filters={"parent": "Computers"}, fieldname=["name"]
		)
		cwip_acc = "CWIP Account - _TC"

		saashq.db.set_value("Asset Category", "Computers", "enable_cwip_accounting", 0)
		saashq.db.set_value("Asset Category Account", name, "capital_work_in_progress_account", "")
		...
		asset = saashq.db.get_value("Asset", {"purchase_invoice": pi.name, "docstatus": 0}, "name")
		asset_doc = saashq.get_doc("Asset", asset)
		...
		saashq.db.set_value("Asset Category Account", name, "capital_work_in_progress_account", cwip_acc)
		asset = saashq.db.get_value("Asset", {"purchase_receipt": pr.name, "docstatus": 0}, "name")
		asset_doc = saashq.get_doc("Asset", asset)
		asset_doc.available_for_use_date = nowdate()
		asset_doc.calculate_depreciation = 0
		...
		saashq.db.set_value("Asset Category", "Computers", "enable_cwip_accounting", cwip)
		saashq.db.set_value("Asset Category Account", name, "capital_work_in_progress_account", cwip_acc)
		saashq.db.get_value("Company", "_Test Company", "capital_work_in_progress_account", cwip_acc)
...
class TestDepreciationBasics(AssetSetup):
	...
	def test_number_of_depreciations(self):
		...
		self.assertRaises(saashq.ValidationError, asset_2.save)
	...
	def test_depr_entry_posting_when_depr_expense_account_is_an_expense_account(self):
		...
		je = saashq.get_doc("Journal Entry", get_depr_schedule(asset.name, "Active")[0].journal_entry)
		accounting_entries = [
	...
	def test_depr_entry_posting_when_depr_expense_account_is_an_income_account(self):
		...
		depr_expense_account = saashq.get_doc("Account", "_Test Depreciations - _TC")
	...
	def test_depreciation_entry_cancellation(self):
		...
		saashq.get_doc("Journal Entry", depr_entry).cancel()
	...
	def test_gle_made_by_depreciation_entries(self):
		...
		saashq.db.set_value("Company", "_Test Company", "series_for_depreciation_entry", "DEPR-")
		post_depreciation_entries(date="2021-01-01")
		asset.load_from_db()
	...
	def test_asset_cost_center(self):
		...
		self.assertRaises(saashq.ValidationError, asset.submit)
	...
	def test_manual_depreciation_with_incorrect_jv_voucher_type(self):
		...
		self.assertRaises(saashq.ValidationError, jv.insert)
...
def create_asset_data():
	...
	if not saashq.db.exists("Item", "Macbook Pro"):
		create_fixed_asset_item()

	if not saashq.db.exists("Location", "Test Location"):
		saashq.get_doc({"doctype": "Location", "location_name": "Test Location"}).insert()

	if not saashq.db.exists("Finance Book", "Test Finance Book 1"):
		saashq.get_doc({"doctype": "Finance Book", "finance_book_name": "Test Finance Book 1"}).insert()

	if not saashq.db.exists("Finance Book", "Test Finance Book 2"):
		saashq.get_doc({"doctype": "Finance Book", "finance_book_name": "Test Finance Book 2"}).insert()

	if not saashq.db.exists("Finance Book", "Test Finance Book 3"):
		saashq.get_doc({"doctype": "Finance Book", "finance_book_name": "Test Finance Book 3"}).insert()
...
def set_depreciation_settings_in_company(company=None):
	...
	saashq.db.set_single_value("Accounts Settings", "book_asset_depreciation_entry_automatically", 1)


def enable_cwip_accounting(asset_category, enable=1):
	saashq.db.set_value("Asset Category", asset_category, "enable_cwip_accounting", enable)
