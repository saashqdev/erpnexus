...
from saashq.query_builder.functions import Max, Min
...
from erpnexus.accounts.doctype.journal_entry.journal_entry import make_reverse_journal_entry
...
def post_depreciation_entries(date=None):
	...
	if not cint(
		saashq.db.get_single_value("Accounts Settings", "book_asset_depreciation_entry_automatically")
	):
		return
...
def get_depreciable_asset_depr_schedules_data(date):
	a = saashq.qb.DocType("Asset")
	ads = saashq.qb.DocType("Asset Depreciation Schedule")
	ds = saashq.qb.DocType("Depreciation Schedule")
...
def get_acc_frozen_upto():
	...
	if frozen_accounts_modifier not in saashq.get_roles() or saashq.session.user == "Administrator":
...
def get_depreciation_cost_center_and_depreciation_series_for_company():
	company_names = saashq.db.get_all("Company", pluck="name")
...
def make_depreciation_entry(
	...
	saashq.has_permission("Journal Entry", throw=True)

	if not date:
		date = today()

	asset_depr_schedule_doc = saashq.get_doc("Asset Depreciation Schedule", asset_depr_schedule_name)

	asset = saashq.get_doc("Asset", asset_depr_schedule_doc.asset)
...
def get_depreciation_accounts(asset_category, company):
	...
	if not accumulated_depreciation_account or not depreciation_expense_account:
		accounts = saashq.get_cached_value(
...
def get_credit_and_debit_accounts(accumulated_depreciation_account, depreciation_expense_account):
	root_type = saashq.get_value("Account", depreciation_expense_account, "root_type")
	...
	if root_type == "Expense":
		...
		saashq.throw(_("Depreciation Expense Account should be an Income or Expense Account."))
...
def set_depr_entry_posting_status_for_failed_assets(failed_asset_names):
	for asset_name in failed_asset_names:
		saashq.db.set_value("Asset", asset_name, "depr_entry_posting_status", "Failed")
...
def scrap_asset(asset_name, scrap_date=None):
	...
	if asset.docstatus != 1:
		saashq.throw(_("Asset {0} must be submitted").format(asset.name))
	elif asset.status in ("Cancelled", "Sold", "Scrapped", "Capitalized", "Decapitalized"):
		saashq.throw(_("Asset {0} cannot be scrapped, as it is already {1}").format(asset.name, asset.status))
	...
	depreciation_series = saashq.get_cached_value("Company", asset.company, "series_for_depreciation_entry")
	...
	saashq.db.set_value("Asset", asset_name, "disposal_date", date)
	saashq.db.set_value("Asset", asset_name, "journal_entry_for_scrap", je.name)
	asset.set_status("Scrapped")

	add_asset_activity(asset_name, _("Asset scrapped"))

	saashq.msgprint(_("Asset scrapped via Journal Entry {0}").format(je.name))


def validate_scrap_date(scrap_date, today_date, purchase_date, calculate_depreciation, asset_name):
	if scrap_date > today_date:
		saashq.throw(_("Future date is not allowed"))
	elif scrap_date < purchase_date:
		saashq.throw(_("Scrap date cannot be before purchase date"))

	if calculate_depreciation:
		asset_depreciation_schedules = saashq.db.get_all(
		...
		for depreciation_schedule in asset_depreciation_schedules:
			...
			if (
				...
				saashq.throw(_("Asset cannot be scrapped before the last depreciation entry."))
...
def restore_asset(asset_name):
	...
	saashq.get_doc("Journal Entry", je).cancel()
...
def modify_depreciation_schedule_for_asset_repairs(asset, notes):
	asset_repairs = saashq.get_all(
		"Asset Repair", filters={"asset": asset.name}, fields=["name", "increase_in_asset_life"]
	)

	for repair in asset_repairs:
		if repair.increase_in_asset_life:
			asset_repair = saashq.get_doc("Asset Repair", repair.name)
		...
		for schedule_idx, schedule in enumerate(asset_depr_schedule_doc.get("depreciation_schedule")):
			if schedule.schedule_date == date and schedule.journal_entry:
				if not disposal_was_made_on_original_schedule_date(
					...
					saashq.flags.is_reverse_depr_entry = True
					reverse_journal_entry.submit()

					saashq.flags.is_reverse_depr_entry = False
...
def get_disposal_account_and_cost_center(company):
	disposal_account, depreciation_cost_center = saashq.get_cached_value(
		"Company", company, ["disposal_account", "depreciation_cost_center"]
	)

	if not disposal_account:
		saashq.throw(_("Please set 'Gain/Loss Account on Asset Disposal' in Company {0}").format(company))
	if not depreciation_cost_center:
		saashq.throw(_("Please set 'Asset Depreciation Cost Center' in Company {0}").format(company))
