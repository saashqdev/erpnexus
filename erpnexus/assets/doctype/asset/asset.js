...
saashq.provide("erpnexus.asset");
saashq.provide("erpnexus.accounts.dimensions");

saashq.ui.form.on("Asset", {
	onload: function (frm) {
	...
	setup: function (frm) {
		...
		frm.make_methods = {
			"Asset Movement": () => {
				saashq.call({
					method: "erpnexus.assets.doctype.asset.asset.make_asset_movement",
					freeze: true,
					args: {
		...
		frm.set_query("purchase_receipt", (doc) => {
			return {
				query: "erpnexus.controllers.queries.get_purchase_receipts",
				filters: { item_code: doc.item_code },
			};
		});
		frm.set_query("purchase_invoice", (doc) => {
			return {
				query: "erpnexus.controllers.queries.get_purchase_invoices",
				filters: { item_code: doc.item_code },
			};
		});
	},
	...
	refresh: function (frm) {
		saashq.ui.form.trigger("Asset", "is_existing_asset");
		...
		if (frm.doc.docstatus == 1) {
			if (["Submitted", "Partially Depreciated", "Fully Depreciated"].includes(frm.doc.status)) {
				frm.add_custom_button(
					__("Transfer Asset"),
					function () {
						erpnexus.asset.transfer_asset(frm);
					},
					__("Manage")
				);
				...
				frm.add_custom_button(
					__("Scrap Asset"),
					function () {
						erpnexus.asset.scrap_asset(frm);
					},
					__("Manage")
				);
				...
				frm.add_custom_button(
					__("Restore Asset"),
					function () {
						erpnexus.asset.restore_asset(frm);
					},
					__("Manage")
				);
			}
		...
		if (frm.doc.docstatus == 0) {
			...
			if (frm.doc.is_composite_asset) {
				saashq.call({
					method: "erpnexus.assets.doctype.asset.asset.has_active_capitalization",
					args: {
						asset: frm.doc.name,
					},
					callback: function (r) {
	...
	make_journal_entry: function (frm) {
		saashq.call({
			method: "erpnexus.assets.doctype.asset.asset.make_journal_entry",
			args: {
	...
	render_depreciation_schedule_view: function (frm, asset_depr_schedule_doc) {
		...
		asset_depr_schedule_doc.depreciation_schedule.forEach((sch) => {
			const row = [
				sch["idx"],
				saashq.format(sch["schedule_date"], { fieldtype: "Date" }),
				saashq.format(sch["depreciation_amount"], { fieldtype: "Currency" }),
				saashq.format(sch["accumulated_depreciation_amount"], { fieldtype: "Currency" }),
				sch["journal_entry"] || "",
			];
		...
		let datatable = new saashq.DataTable(wrapper.get(0), {
			columns: columns,
			data: data,
			layout: "fluid",
	...
	setup_chart_and_depr_schedule_view: async function (frm) {
		...
		var x_intervals = [saashq.format(frm.doc.purchase_date, { fieldtype: "Date" })];
		var asset_values = [frm.doc.gross_purchase_amount];

		if (frm.doc.calculate_depreciation) {
			if (frm.doc.opening_accumulated_depreciation) {
				var depreciation_date = saashq.datetime.add_months(
					frm.doc.finance_books[0].depreciation_start_date,
					-1 * frm.doc.finance_books[0].frequency_of_depreciation
				);
				x_intervals.push(saashq.format(depreciation_date, { fieldtype: "Date" }));
				asset_values.push(
					flt(
			...
			let asset_depr_schedule_doc = (
				await saashq.call(
					"erpnexus.assets.doctype.asset_depreciation_schedule.asset_depreciation_schedule.get_asset_depr_schedule_doc",
					{
						asset_name: frm.doc.name,
						status: "Active",
			...
			$.each(asset_depr_schedule_doc.depreciation_schedule || [], function (i, v) {
				x_intervals.push(saashq.format(v.schedule_date, { fieldtype: "Date" }));
				var asset_value = flt(
			...
			if (frm.doc.opening_accumulated_depreciation) {
				x_intervals.push(saashq.format(frm.doc.creation.split(" ")[0], { fieldtype: "Date" }));
				asset_values.push(
					flt(
						frm.doc.gross_purchase_amount - frm.doc.opening_accumulated_depreciation,
						precision("gross_purchase_amount")
			...
			$.each(depr_entries || [], function (i, v) {
				x_intervals.push(saashq.format(v.posting_date, { fieldtype: "Date" }));
		...
		if (["Scrapped", "Sold"].includes(frm.doc.status)) {
			x_intervals.push(saashq.format(frm.doc.disposal_date, { fieldtype: "Date" }));
			asset_values.push(0);
		}
	...
	set_finance_book: function (frm) {
		saashq.call({
			method: "erpnexus.assets.doctype.asset.asset.get_item_details",
			args: {
	...
	make_sales_invoice: function (frm) {
		saashq.call({
			...
			method: "erpnexus.assets.doctype.asset.asset.make_sales_invoice",
			callback: function (r) {
				var doclist = saashq.model.sync(r.message);
				saashq.set_route("Form", doclist[0].doctype, doclist[0].name);
			},
	...
	create_asset_maintenance: function (frm) {
		saashq.call({
			...
			method: "erpnexus.assets.doctype.asset.asset.create_asset_maintenance",
			callback: function (r) {
				var doclist = saashq.model.sync(r.message);
				saashq.set_route("Form", doclist[0].doctype, doclist[0].name);
			},
	...
	create_asset_repair: function (frm) {
		saashq.call({
			...
			method: "erpnexus.assets.doctype.asset.asset.create_asset_repair",
			callback: function (r) {
				var doclist = saashq.model.sync(r.message);
				saashq.set_route("Form", doclist[0].doctype, doclist[0].name);
			},
	...
	create_asset_capitalization: function (frm) {
		saashq.call({
			...
			method: "erpnexus.assets.doctype.asset.asset.create_asset_capitalization",
			callback: function (r) {
				var doclist = saashq.model.sync(r.message);
				saashq.set_route("Form", doclist[0].doctype, doclist[0].name);
	...
	split_asset: function (frm) {
		...
		dialog.set_primary_action(__("Split"), function () {
			...
			saashq.call({
				...
				method: "erpnexus.assets.doctype.asset.asset.split_asset",
				callback: function (r) {
					let doclist = saashq.model.sync(r.message);
					saashq.set_route("Form", doclist[0].doctype, doclist[0].name);
				},
			});
	...
	create_asset_value_adjustment: function (frm) {
		saashq.call({
			...
			method: "erpnexus.assets.doctype.asset.asset.create_asset_value_adjustment",
			freeze: 1,
			callback: function (r) {
				var doclist = saashq.model.sync(r.message);
				saashq.set_route("Form", doclist[0].doctype, doclist[0].name);
	...
	purchase_receipt: (frm) => {
		...
		if (frm.doc.purchase_receipt) {
			if (frm.doc.item_code) {
				saashq.db.get_doc("Purchase Receipt", frm.doc.purchase_receipt).then((pr_doc) => {
	...
	purchase_invoice: (frm) => {
		...
		if (frm.doc.purchase_invoice) {
			if (frm.doc.item_code) {
				saashq.db.get_doc("Purchase Invoice", frm.doc.purchase_invoice).then((pi_doc) => {
	...
	set_values_from_purchase_doc: function (frm, doctype, purchase_doc) {
		...
		if (!item) {
			let doctype_field = saashq.scrub(doctype);
		...
		saashq.db.get_value("Item", item.item_code, "is_grouped_asset", (r) => {
	...
	set_depreciation_rate: function (frm, row) {
		if (
			...
			saashq.call({
				...
				callback: function (r) {
					if (r.message) {
						saashq.flags.dont_change_rate = true;
	...
	set_salvage_value_percentage_or_expected_value_after_useful_life: function (
		...
		if (expected_value_after_useful_life_changed) {
			saashq.flags.from_set_salvage_value_percentage_or_expected_value_after_useful_life = true;
			...
			saashq.flags.from_set_salvage_value_percentage_or_expected_value_after_useful_life = false;
...
saashq.ui.form.on("Asset Finance Book", {
	...
	expected_value_after_useful_life: function (frm, cdt, cdn) {
		const row = locals[cdt][cdn];
		if (!saashq.flags.from_set_salvage_value_percentage_or_expected_value_after_useful_life) {
	...
	rate_of_depreciation: function (frm, cdt, cdn) {
		if (!saashq.flags.dont_change_rate) {
			saashq.model.set_value(cdt, cdn, "expected_value_after_useful_life", 0);
		}

		saashq.flags.dont_change_rate = false;
	},

	depreciation_start_date: function (frm, cdt, cdn) {
		const book = locals[cdt][cdn];
		if (frm.doc.available_for_use_date && book.depreciation_start_date < frm.doc.available_for_use_date) {
			saashq.msgprint(__("Depreciation Posting Date cannot be before Available-for-use Date"));
...
erpnexus.asset.scrap_asset = function (frm) {
	var scrap_dialog = new saashq.ui.Dialog({
		...
		primary_action(values) {
			saashq.call({
				...
				method: "erpnexus.assets.doctype.asset.depreciation.scrap_asset",
				callback: function (r) {
					frm.reload_doc();
					scrap_dialog.hide();
				},
...
erpnexus.asset.restore_asset = function (frm) {
	saashq.confirm(__("Do you really want to restore this scrapped asset?"), function () {
		saashq.call({
			args: {
				asset_name: frm.doc.name,
			},
			method: "erpnexus.assets.doctype.asset.depreciation.restore_asset",
			callback: (r) => frm.reload_doc(),
		});
	});
};

erpnexus.asset.transfer_asset = function (frm) {
	saashq.call({
		method: "erpnexus.assets.doctype.asset.asset.make_asset_movement",
		freeze: true,
		args: {
