...
from saashq.utils import cint, flt, getdate, now_datetime
...
class TestAssetCapitalization(IntegrationTestCase):
	...
	def test_capitalization_with_perpetual_inventory(self):
		...
		target_asset = saashq.get_doc("Asset", asset_capitalization.target_asset)
	...
	def test_capitalization_with_periodical_inventory(self):
		...
		# Test General Ledger Entries
		default_expense_account = saashq.db.get_value("Company", company, "default_expense_account")
		expected_gle = {
...
def create_asset_capitalization(**args):
	...
	now = now_datetime()
	target_asset = saashq.get_doc("Asset", args.target_asset) if args.target_asset else saashq._dict()
				...
				saashq._dict(
					{
						...
						"company": saashq.get_cached_value("Warehouse", source_warehouse, "company"),
