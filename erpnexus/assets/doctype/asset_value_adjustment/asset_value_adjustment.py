...
from saashq.utils import flt, formatdate, get_link_to_form, getdate
...
from erpnexus.assets.doctype.asset.depreciation import get_depreciation_accounts
...
class AssetValueAdjustment(Document):
	...
	def on_cancel(self):
		saashq.get_doc("Journal Entry", self.journal_entry).cancel()
		self.update_asset()
		add_asset_activity(
			self.asset,
	...
	def validate_date(self):
		asset_purchase_date = saashq.db.get_value("Asset", self.asset, "purchase_date")
	...
	def set_value_after_depreciation(self):
		saashq.db.set_value("Asset", self.asset, "value_after_depreciation", self.new_asset_value)
	...
	def make_depreciation_entry(self):
		asset = saashq.get_doc("Asset", self.asset)
		(
			fixed_asset_account,
...
def get_value_of_accounting_dimensions(asset_name):
	dimension_fields = [*saashq.get_list("Accounting Dimension", pluck="fieldname"), "cost_center"]
	return saashq.db.get_value("Asset", asset_name, fieldname=dimension_fields, as_dict=True)
