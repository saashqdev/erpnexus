...
class AssetMovement(Document):
	...
	if TYPE_CHECKING:
		...
		from erpnexus.assets.doctype.asset_movement_item.asset_movement_item import AssetMovementItem
	...
	def validate_asset(self):
		for d in self.assets:
			status, company = saashq.db.get_value("Asset", d.asset, ["status", "company"])
			if self.purpose == "Transfer" and status in ("Draft", "Scrapped", "Sold"):
				saashq.throw(_("{0} asset cannot be transferred").format(status))

			if company != self.company:
				saashq.throw(_("Asset {0} does not belong to company {1}").format(d.asset, self.company))

			if not (d.source_location or d.target_location or d.from_employee or d.to_employee):
				saashq.throw(_("Either location or employee must be required"))

	def validate_location(self):
		for d in self.assets:
			if self.purpose in ["Transfer", "Issue"]:
				current_location = saashq.db.get_value("Asset", d.asset, "location")
			...
			if self.purpose == "Issue":
				...
				if not d.to_employee:
					saashq.throw(_("Employee is required while issuing Asset {0}").format(d.asset))
			...
			if self.purpose == "Transfer":
				...
				if d.source_location == d.target_location:
					saashq.throw(_("Source and Target Location cannot be same"))
	...
	def validate_employee(self):
		for d in self.assets:
			if d.from_employee:
				current_custodian = saashq.db.get_value("Asset", d.asset, "custodian")
			...
			if d.to_employee and saashq.db.get_value("Employee", d.to_employee, "company") != self.company:
				saashq.throw(
	...
	def set_latest_location_and_custodian_in_asset(self):
		...
		for d in self.assets:
			...
			latest_movement_entry = saashq.db.sql(
				f"""
			...
			saashq.db.set_value("Asset", d.asset, "location", current_location, update_modified=False)
			saashq.db.set_value("Asset", d.asset, "custodian", current_employee, update_modified=False)
