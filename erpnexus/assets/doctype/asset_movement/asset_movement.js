...
saashq.ui.form.on("Asset Movement", {
	setup: (frm) => {
...
saashq.ui.form.on("Asset Movement Item", {
			...
			saashq.db
				...
				.then((asset_doc) => {
					if (asset_doc.location)
						saashq.model.set_value(cdt, cdn, "source_location", asset_doc.location);
					if (asset_doc.custodian)
						saashq.model.set_value(cdt, cdn, "from_employee", asset_doc.custodian);
				})
				.catch((err) => {
