...
from saashq.utils import flt, nowdate, nowtime, today

from erpnexus.assets.doctype.asset.asset import (
	get_asset_account,
	get_asset_value_after_depreciation,
)
from erpnexus.assets.doctype.asset.test_asset import (
	create_asset,
	create_asset_data,
...
class TestAssetRepair(IntegrationTestCase):
	...
	def test_update_status(self):
		...
		asset_repair.save()
		asset_status = saashq.db.get_value("Asset", asset_repair.asset, "status")
	...
	def test_decrease_stock_quantity(self):
		...
		stock_entry = saashq.get_last_doc("Stock Entry")
	...
	def test_serialized_item_consumption(self):
		...
		asset_repair.repair_status = "Completed"
		self.assertRaises(saashq.ValidationError, asset_repair.submit)
	...
	def test_gl_entries_with_perpetual_inventory(self):
		...
		asset_category = saashq.get_doc("Asset Category", "Computers")
		...
		stock_entry_expense_account = (
			saashq.get_doc("Stock Entry", {"asset_repair": asset_repair.name}).get("items")[0].expense_account
		)
	...
	def test_gl_entries_with_periodical_inventory(self):
		saashq.db.set_value("Company", "_Test Company", "default_expense_account", "Cost of Goods Sold - _TC")
		asset_repair = create_asset_repair(
			capitalize_repair_cost=1,
			stock_consumption=1,
			item="_Test Non Stock Item",
...
def create_asset_repair(**args):
	...
	if args.submit:
		asset_repair.repair_status = "Completed"
		asset_repair.cost_center = saashq.db.get_value("Company", asset.company, "cost_center")
