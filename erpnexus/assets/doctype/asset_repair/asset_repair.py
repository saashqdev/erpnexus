...
from saashq.utils import add_months, cint, flt, get_link_to_form, getdate, time_diff_in_hours

import erpnexus
from erpnexus.accounts.general_ledger import make_gl_entries
from erpnexus.assets.doctype.asset.asset import get_asset_account
...
class AssetRepair(AccountsController):
	...
	if TYPE_CHECKING:
		...
		from erpnexus.assets.doctype.asset_repair_consumed_item.asset_repair_consumed_item import (
			AssetRepairConsumedItem,
		)
		from erpnexus.assets.doctype.asset_repair_purchase_invoice.asset_repair_purchase_invoice import (
	...
	def validate(self):
		self.asset_doc = saashq.get_doc("Asset", self.asset)
		self.validate_dates()
	...
	def validate_purchase_invoice(self):
		...
		for pi in self.invoices:
			if pi.purchase_invoice not in purchase_invoice_list:
				saashq.throw(_("Expense item not present in Purchase Invoice"))

	def validate_purchase_invoice_repair_cost(self):
		for pi in self.invoices:
			if flt(pi.repair_cost) > saashq.db.get_value(
				"Purchase Invoice", pi.purchase_invoice, "base_net_total"
			):
				saashq.throw(_("Repair cost cannot be greater than purchase invoice base net total"))

	def validate_purchase_invoice_expense_account(self):
		for pi in self.invoices:
			if pi.expense_account not in saashq.db.get_all(
	...
	def update_status(self):
		if self.repair_status == "Pending" and self.asset_doc.status != "Out of Order":
			saashq.db.set_value("Asset", self.asset, "status", "Out of Order")
			add_asset_activity(
				self.asset,
	...
	def after_delete(self):
		saashq.get_doc("Asset", self.asset).set_status()

	def check_repair_status(self):
		if self.repair_status == "Pending":
			saashq.throw(_("Please update Repair Status."))

	def check_for_stock_items_and_warehouse(self):
		if not self.get("stock_items"):
			saashq.throw(_("Please enter Stock Items consumed during the Repair."), title=_("Missing Items"))
	...
	def validate_serial_no(self, stock_item):
		if not stock_item.serial_and_batch_bundle and saashq.get_cached_value(
			"Item", stock_item.item_code, "has_serial_no"
		):
			msg = f"Serial No Bundle is mandatory for Item {stock_item.item_code}"
			saashq.throw(msg, title=_("Missing Serial No Bundle"))
	...
	def get_gl_entries_for_consumed_items(self, gl_entries, fixed_asset_account):
		...
		stock_entry = saashq.get_doc("Stock Entry", {"asset_repair": self.name})

		default_expense_account = None
		if not erpnexus.is_perpetual_inventory_enabled(self.company):
			default_expense_account = saashq.get_cached_value(
				"Company", self.company, "default_expense_account"
			)
			if not default_expense_account:
				saashq.throw(_("Please set default Expense Account in Company {0}").format(self.company))
...
def expense_item_pi_query(company):
	...
	query = (
		saashq.qb.from_(PurchaseInvoice)
