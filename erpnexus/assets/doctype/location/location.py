...
from saashq.utils.nestedset import NestedSet, update_nsm
...
class Location(NestedSet):
	...
	def update_ancestor_location_features(self):
		...
		for ancestor in self.get_ancestors():
			ancestor_doc = saashq.get_doc("Location", ancestor)
...
def get_children(doctype, parent=None, location=None, is_root=False):
	...
	return saashq.db.sql(
		...
		from
			`tabLocation` comp
		where
			ifnull(parent_location, "")={saashq.db.escape(parent)}
		""",
		as_dict=1,
	)
...
def on_doctype_update():
	saashq.db.add_index("Location", ["lft", "rgt"])
