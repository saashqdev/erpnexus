...
class AssetDepreciationSchedule(Document):
	...
	if TYPE_CHECKING:
		...
		from erpnexus.assets.doctype.depreciation_schedule.depreciation_schedule import (
	...
	def validate_another_asset_depr_schedule_does_not_exist(self):
		...
		asset_depr_schedule = saashq.db.exists(
	...
	def update_shift_depr_schedule(self):
		...
		asset_doc = saashq.get_doc("Asset", self.asset)
	...
	def prepare_draft_asset_depr_schedule_data_from_asset_name_and_fb_name(self, asset_name, fb_name):
		asset_doc = saashq.get_doc("Asset", asset_name)

		finance_book_filter = ["finance_book", "is", "not set"]
		if fb_name:
			finance_book_filter = ["finance_book", "=", fb_name]

		asset_finance_book_name = saashq.db.get_value(
			doctype="Asset Finance Book",
			filters=[["parent", "=", asset_name], finance_book_filter],
		)
		asset_finance_book_doc = saashq.get_doc("Asset Finance Book", asset_finance_book_name)
				...
				if flt(depreciation_amount, asset_doc.precision("gross_purchase_amount")) <= 0:
					saashq.throw(
						_(
							...
							saashq.bold(asset_doc.gross_purchase_amount),
							saashq.bold(row.total_number_of_depreciations),
							saashq.bold(row.frequency_of_depreciation),
						)
					)
	...
	def add_depr_schedule_row(self, schedule_date, depreciation_amount, schedule_idx):
		if self.shift_based:
			shift = (
				...
				else saashq.get_cached_value("Asset Shift Factor", {"default": 1}, "shift_name")
			)
		else:
			shift = None
...
def get_daily_depr_amount(asset, row, schedule_idx, amount):
	if cint(saashq.db.get_single_value("Accounts Settings", "calculate_depr_using_total_days")):
		total_days = (
			date_diff(
				get_last_day(
					add_months(
...
def get_asset_shift_factors_map():
	return dict(saashq.db.get_all("Asset Shift Factor", ["shift_name", "shift_factor"], as_list=True))
...
def make_draft_asset_depr_schedule(asset_doc, row):
	asset_depr_schedule_doc = saashq.new_doc("Asset Depreciation Schedule")
...
def get_temp_asset_depr_schedule_doc(
	...
	temp_asset_depr_schedule_doc = saashq.copy_doc(current_asset_depr_schedule_doc)
...
def get_asset_depr_schedule_doc(asset_name, status, finance_book=None):
	...
	asset_depr_schedule_doc = saashq.get_doc("Asset Depreciation Schedule", asset_depr_schedule[0].name)
