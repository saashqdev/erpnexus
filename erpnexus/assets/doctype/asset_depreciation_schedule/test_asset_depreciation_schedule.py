...
class TestAssetDepreciationSchedule(IntegrationTestCase):
	...
	def test_throw_error_if_another_asset_depr_schedule_exist(self):
		...
		second_asset_depr_schedule = saashq.get_doc(
			{"doctype": "Asset Depreciation Schedule", "asset": asset.name, "finance_book": None}
		)

		self.assertRaises(saashq.ValidationError, second_asset_depr_schedule.insert)
	...
	def test_schedule_for_slm_for_existing_asset_daily_pro_rata_enabled(self):
		saashq.db.set_single_value("Accounts Settings", "calculate_depr_using_total_days", 1)
		asset = create_asset(
			calculate_depreciation=1,
			depreciation_method="Straight Line",
		...
		self.assertEqual(schedules, expected_schedules)
		saashq.db.set_single_value("Accounts Settings", "calculate_depr_using_total_days", 0)
