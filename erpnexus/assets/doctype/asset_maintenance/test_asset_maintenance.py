...
from saashq.utils import add_days, get_last_day, nowdate
...
class TestAssetMaintenance(IntegrationTestCase):
	def setUp(self):
		...
		self.asset_name = saashq.db.get_value("Asset", {"purchase_receipt": self.pr.name}, "name")
		self.asset_doc = saashq.get_doc("Asset", self.asset_name)
	...
	def test_create_asset_maintenance_with_log(self):
		...
		asset_maintenance = saashq.get_doc(
			{
		...
		asset_maintenance_log = saashq.db.get_value(
			"Asset Maintenance Log",
			{"asset_maintenance": asset_maintenance.name, "task_name": "Change Oil"},
			"name",
		)

		asset_maintenance_log_doc = saashq.get_doc("Asset Maintenance Log", asset_maintenance_log)
