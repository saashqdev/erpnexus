...
from saashq import _, qb, throw
from saashq.model.meta import get_field_precision
from saashq.query_builder import AliasedQuery, Criterion, Table
from saashq.query_builder.functions import Count, Round, Sum
...
# imported to enable erpnexus.accounts.utils.get_account_currency
from erpnexus.accounts.doctype.account.account import get_account_currency
from erpnexus.accounts.doctype.accounting_dimension.accounting_dimension import get_dimensions
from erpnexus.stock import get_warehouse_account_map
from erpnexus.stock.utils import get_stock_value_on

if TYPE_CHECKING:
	from erpnexus.stock.doctype.repost_item_valuation.repost_item_valuation import RepostItemValuation


class FiscalYearError(saashq.ValidationError):
	pass


class PaymentEntryUnlinkError(saashq.ValidationError):
	pass
...
def get_fiscal_years(
	...
	if raise_on_missing:
		...
		if company:
			error_msg = _("""{0} for {1}""").format(error_msg, saashq.bold(company))

		if verbose == 1:
			saashq.msgprint(error_msg)
...
def _get_fiscal_years(company=None):
	fiscal_years = saashq.cache().hget("fiscal_years", company) or []
	...
	if not fiscal_years:
		...
		query = (
			saashq.qb.from_(FY).select(FY.name, FY.year_start_date, FY.year_end_date).where(FY.disabled == 0)
		)

		if company:
			FYC = DocType("Fiscal Year Company")
			query = query.where(
				ExistsCriterion(saashq.qb.from_(FYC).select(FYC.name).where(FYC.parent == FY.name)).negate()
				| ExistsCriterion(
					saashq.qb.from_(FYC)
					.select(FYC.company)
		...
		saashq.cache().hset("fiscal_years", company, fiscal_years)
	return fiscal_years
...
def get_balance_on(
	...
	if not account and saashq.form_dict.get("account"):
		account = saashq.form_dict.get("account")
	if not date and saashq.form_dict.get("date"):
		date = saashq.form_dict.get("date")
	if not party_type and saashq.form_dict.get("party_type"):
		party_type = saashq.form_dict.get("party_type")
	if not party and saashq.form_dict.get("party"):
		party = saashq.form_dict.get("party")
	if not cost_center and saashq.form_dict.get("cost_center"):
		cost_center = saashq.form_dict.get("cost_center")

	cond = ["is_cancelled=0"]
	if start_date:
		cond.append("posting_date >= %s" % saashq.db.escape(cstr(start_date)))
	if date:
		cond.append("posting_date <= %s" % saashq.db.escape(cstr(date)))
	else:
	...
	if cost_center and report_type == "Profit and Loss":
		cc = saashq.get_doc("Cost Center", cost_center)
		if cc.is_group:
			cond.append(
				f""" exists (
			...
			cond.append(f"""gle.cost_center = {saashq.db.escape(cost_center)} """)

	if account:
		if not (saashq.flags.ignore_account_permission or ignore_account_permission):
		...
		if acc.is_group:
			...
			if acc.account_currency == saashq.get_cached_value("Company", acc.company, "default_currency"):
				in_account_currency = False
		else:
			cond.append(f"""gle.account = {saashq.db.escape(account)} """)

	if account_type:
		accounts = saashq.db.get_all(
			"Account",
		...
		cond.append(
			...
			gle.account in (%s)
		"""
			% (", ".join([saashq.db.escape(account) for account in accounts]))
		)

	if party_type and party:
		cond.append(
			f"""gle.party_type = {saashq.db.escape(party_type)} and gle.party = {saashq.db.escape(party)} """
		)

	if company:
		cond.append("""gle.company = %s """ % (saashq.db.escape(company)))
...
def get_count_on(account, fieldname, date):
	...
	if account:
		...
		if not saashq.flags.ignore_account_permission:
			acc.check_permission("read")
...
def add_cc(args=None):
	...
	if args.parent_cost_center == args.company:
		args.parent_cost_center = "{} - {}".format(
			args.parent_cost_center, saashq.get_cached_value("Company", args.company, "abbr")
		)
...
def reconcile_against_document(
	...
	for key, entries in reconciled_entries.items():
		...
		doc = saashq.get_doc(voucher_type, voucher_no)
		saashq.flags.ignore_party_validation = True
		...
		doc = saashq.get_doc(entry.voucher_type, entry.voucher_no)
		...
		if voucher_type == "Payment Entry" and doc.book_advance_payments_in_separate_party_account:
			...
			from erpnexus.accounts.general_ledger import process_debit_credit_difference
		...
		if update_advance_paid:
			for t, n in update_advance_paid:
				saashq.get_doc(t, n).set_total_advance_paid()

		saashq.flags.ignore_party_validation = False
...
def check_if_advance_entry_modified(args):
	...
	if args.voucher_type == "Journal Entry":
		...
		precision = saashq.get_precision("Payment Entry", "unallocated_amount")
...
def validate_allocated_amount(args):
	precision = args.get("precision") or saashq.db.get_single_value("System Settings", "currency_precision")
...
def update_reference_in_journal_entry(d, journal_entry, do_not_save=False):
	...
	[
		...
		for field in saashq.get_meta("Journal Entry Account").get_fieldnames_with_value()
	]
...
def update_reference_in_payment_entry(
	...
	if not skip_ref_details_update_for_pe:
		reference_exchange_details = saashq._dict()
	...
	payment_entry.make_exchange_gain_loss_journal(
		saashq._dict({"difference_posting_date": d.difference_posting_date}), dimensions_dict
	)
...
def cancel_exchange_gain_loss_journal(
	...
	if parent_doc.doctype in ["Sales Invoice", "Purchase Invoice", "Payment Entry", "Journal Entry"]:
		...
		for doc in gain_loss_journals:
			gain_loss_je = saashq.get_doc("Journal Entry", doc)
...
def get_linked_exchange_gain_loss_journal(referenced_dt: str, referenced_dn: str, je_docstatus: int) -> list:
	...
	if journals := saashq.db.get_all(
		...
		gain_loss_journals = saashq.db.get_all(
			"Journal Entry",
			{
				"name": ["in", journals],
...
def cancel_common_party_journal(self):
	...
	if not saashq.db.get_single_value("Accounts Settings", "enable_common_party_accounting"):
		return
	...
	journal_entry = saashq.db.get_value(
		"Journal Entry Account",
		filters={
	...
	common_party_journal = saashq.db.get_value(
		"Journal Entry",
		filters={
			"name": journal_entry,
	...
	common_party_je = saashq.get_doc("Journal Entry", common_party_journal)
...
def update_accounting_ledgers_after_reference_removal(
	...
	gle_update_query = (
		...
		.set(gle.modified_by, saashq.session.user)
	...
	ple_update_query = (
		...
		.set(ple.modified_by, saashq.session.user)
		.where(
...
def remove_ref_doc_link_from_jv(
	...
	if linked_jv:
		update_query = (
			...
			.set(jea.modified_by, saashq.session.user)
		...
		saashq.msgprint(_("Journal Entries {0} are un-linked").format("\n".join(linked_jv)))
...
def remove_ref_doc_link_from_pe(
	...
	if linked_pe:
		update_query = (
			...
			.set(per.modified_by, saashq.session.user)
		...
		for pe in linked_pe:
			try:
				pe_doc = saashq.get_doc("Payment Entry", pe)
				pe_doc.set_amounts()
				...
				saashq.throw(msg, exc=PaymentEntryUnlinkError, title=_("Payment Unlink Error"))
			...
			qb.update(pay).set(pay.total_allocated_amount, pe_doc.total_allocated_amount).set(
				...
				pay.modified_by, saashq.session.user
			).where(pay.name == pe).run()

		saashq.msgprint(_("Payment Entries {0} are un-linked").format("\n".join(linked_pe)))


@saashq.whitelist()
def get_company_default(company, fieldname, ignore_validation=False):
	value = saashq.get_cached_value("Company", company, fieldname)

	if not ignore_validation and not value:
		throw(
			_("Please set default {0} in Company {1}").format(
				saashq.get_meta("Company").get_label(fieldname), company
			)
		)
...
def get_currency_precision():
	...
	if not precision:
		number_format = saashq.db.get_default("number_format") or "#,###.##"
...
def get_outstanding_invoices(
	...
	precision = saashq.get_precision("Sales Invoice", "outstanding_amount") or 2

	if account:
		root_type, account_type = saashq.get_cached_value(
			"Account", account[0], ["root_type", "account_type"]
		)
		party_account_type = "Receivable" if root_type == "Asset" else "Payable"
		party_account_type = account_type or party_account_type
	else:
		party_account_type = erpnexus.get_party_account_type(party_type)
...
def get_account_name(account_type=None, root_type=None, is_group=None, account_currency=None, company=None):
	...
	return saashq.db.get_value(
		"Account",
		{
			...
			"account_currency": account_currency or saashq.defaults.get_defaults().currency,
			"company": company or saashq.defaults.get_defaults().company,
		},
		"name",
	)
...
def get_companies():
	...
	return [d.name for d in saashq.get_list("Company", fields=["name"], order_by="name")]
...
def get_children(doctype, parent, company, is_root=False, include_disabled=False):
	...
	from erpnexus.accounts.report.financial_statements import sort_accounts
	...
	acc = saashq.get_list(doctype, fields=fields, filters=filters)
...
def create_payment_gateway_account(gateway, payment_channel="Email"):
	from erpnexus.setup.setup_wizard.operations.install_fixtures import create_bank_account

	company = saashq.get_cached_value("Global Defaults", "Global Defaults", "default_company")
	if not company:
		return

	# NOTE: we translate Payment Gateway account name because that is going to be used by the end user
	bank_account = saashq.db.get_value(
		"Account",
	...
	if not bank_account:
		saashq.msgprint(_("Payment Gateway Account not created, please create one manually."))
		return
...
def update_cost_center(docname, cost_center_name, cost_center_number, company, merge):
	...
	if cost_center_number:
		saashq.db.set_value("Cost Center", docname, "cost_center_number", cost_center_number.strip())
	else:
		saashq.db.set_value("Cost Center", docname, "cost_center_number", "")

	saashq.db.set_value("Cost Center", docname, "cost_center_name", cost_center_name.strip())

	new_name = get_autoname_with_number(cost_center_number, cost_center_name, company)
	if docname != new_name:
		saashq.rename_doc("Cost Center", docname, new_name, force=1, merge=merge)
		return new_name
...
def validate_field_number(doctype_name, docname, number_value, company, field_name):
	...
	if number_value:
		...
		doctype_with_same_number = saashq.db.get_value(doctype_name, filters)
...
def get_autoname_with_number(number_value, doc_title, company):
	...
	company_abbr = saashq.get_cached_value("Company", company, "abbr")
...
def get_coa(doctype, parent, is_root=None, chart=None):
	...
	chart = chart if chart else saashq.flags.chart
	saashq.flags.chart = chart
...
def repost_gle_for_stock_vouchers(
	...
	from erpnexus.accounts.general_ledger import toggle_debit_credit_if_negative
	...
	precision = get_field_precision(saashq.get_meta("GL Entry").get_field("debit")) or 2
	...
	for stock_vouchers_chunk in create_batch(stock_vouchers, GL_REPOSTING_CHUNK):
		...
		for voucher_type, voucher_no in stock_vouchers_chunk:
			...
			voucher_obj = saashq.get_doc(voucher_type, voucher_no)
...
def get_future_stock_vouchers(posting_date, posting_time, for_warehouses=None, for_items=None, company=None):
	...
	future_stock_vouchers = saashq.db.sql(
...
def get_stock_accounts(company, voucher_type=None, voucher_no=None, accounts=None):
	stock_accounts = [
		d.name
		for d in saashq.db.get_all("Account", {"account_type": "Stock", "company": company, "is_group": 0})
	]
...
def get_stock_and_account_balance(account=None, posting_date=None, company=None):
	...
	account_table = saashq.qb.DocType("Account")
	query = (
		saashq.qb.from_(account_table)
	...
	precision = saashq.get_precision("Journal Entry Account", "debit_in_account_currency")
...
def check_and_delete_linked_reports(report):
	...
	icons = saashq.get_all("Desktop Icon", fields=["name"], filters={"_report": report})
	if icons:
		for icon in icons:
			saashq.delete_doc("Desktop Icon", icon)


def create_err_and_its_journals(companies: list | None = None) -> None:
	if companies:
		for company in companies:
			err = saashq.new_doc("Exchange Rate Revaluation")
			...
			if err.accounts:
				...
				if company.submit_err_jv:
					...
					jv and saashq.get_doc("Journal Entry", jv).submit()
...
def update_voucher_outstanding(voucher_type, voucher_no, account, party_type, party):
	...
	vouchers = [saashq._dict({"voucher_type": voucher_type, "voucher_no": voucher_no})]
	common_filter = []
	if account:
	...
	if (
		...
		ref_doc = saashq.get_doc(voucher_type, voucher_no)
...
def create_gain_loss_journal(
	...
	journal_entry = saashq.new_doc("Journal Entry")
	...
	party_account_currency = saashq.get_cached_value("Account", party_account, "account_currency")

	if not gain_loss_account:
		saashq.throw(_("Please set default Exchange Gain/Loss Account in Company {}").format(company))
	gain_loss_account_currency = get_account_currency(gain_loss_account)
	company_currency = saashq.get_cached_value("Company", company, "default_currency")

	if gain_loss_account_currency != company_currency:
		saashq.throw(_("Currency for {0} must be {1}").format(gain_loss_account, company_currency))

	journal_account = saashq._dict(
		{
			...
			"exchange_rate": 0,
			"cost_center": cost_center or erpnexus.get_default_cost_center(company),
...
def get_party_types_from_account_type(account_type):
	return saashq.db.get_all("Party Type", {"account_type": account_type}, pluck="name")


def run_ledger_health_checks():
	health_monitor_settings = saashq.get_doc("Ledger Health Monitor")
	if health_monitor_settings.enable_health_monitor:
		...
		if health_monitor_settings.debit_credit_mismatch:
			for x in health_monitor_settings.companies:
				...
				voucher_wise = saashq.get_doc("Report", "Voucher-wise Balance")
				res = voucher_wise.execute_script_report(filters=filters)
				for x in res[1]:
					doc = saashq.new_doc("Ledger Health")
		...
		if health_monitor_settings.general_and_payment_ledger_mismatch:
			for x in health_monitor_settings.companies:
				...
				gl_pl_comparison = saashq.get_doc("Report", "General and Payment Ledger Comparison")
