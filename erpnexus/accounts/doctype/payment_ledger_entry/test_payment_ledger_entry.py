...
class TestPaymentLedgerEntry(IntegrationTestCase):
	...
	def create_company(self):
		...
		# create bank account
		if saashq.db.exists("Account", "HDFC - _PL"):
			self.bank = "HDFC - _PL"
		else:
	...
	def test_multi_payment_unlink_on_invoice_cancellation(self):
		...
		si.delete()
		self.assertRaises(saashq.DoesNotExistError, saashq.get_doc, si.doctype, si.name)
	...
	def test_advance_payment_unlink_on_order_cancellation(self):
		...
		# with references removed, deletion should be possible
		so.delete()
		self.assertRaises(saashq.DoesNotExistError, saashq.get_doc, so.doctype, so.name)
