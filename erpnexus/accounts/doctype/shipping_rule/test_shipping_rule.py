...
from erpnexus.accounts.doctype.shipping_rule.shipping_rule import (
...
class TestShippingRule(IntegrationTestCase):
	def test_from_greater_than_to(self):
		shipping_rule = saashq.copy_doc(self.globalTestRecords["Shipping Rule"][0])
...
def create_shipping_rule(shipping_rule_type, shipping_rule_name):
	if saashq.db.exists("Shipping Rule", shipping_rule_name):
		return saashq.get_doc("Shipping Rule", shipping_rule_name)

	sr = saashq.new_doc("Shipping Rule")
