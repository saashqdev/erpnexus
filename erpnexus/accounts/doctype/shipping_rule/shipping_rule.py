...
class OverlappingConditionError(saashq.ValidationError):
	pass


class FromGreaterThanToError(saashq.ValidationError):
	pass


class ManyBlankToValuesError(saashq.ValidationError):
	pass
...
class ShippingRule(Document):
	...
	if TYPE_CHECKING:
		...
		from erpnexus.accounts.doctype.shipping_rule_condition.shipping_rule_condition import (
			ShippingRuleCondition,
		)
		from erpnexus.accounts.doctype.shipping_rule_country.shipping_rule_country import (
			ShippingRuleCountry,
		)
	...
	def add_shipping_rule_to_tax_table(self, doc, shipping_amount):
		...
		if self.shipping_rule_type == "Selling":
			...
			if doc.meta.get_field("taxes").options != "Sales Taxes and Charges":
				saashq.throw(_("Shipping rule only applicable for Selling"))
			...
			if doc.meta.get_field("taxes").options != "Purchase Taxes and Charges":
				saashq.throw(_("Shipping rule only applicable for Buying"))
