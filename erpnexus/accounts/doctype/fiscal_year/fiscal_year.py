...
from saashq.utils import add_days, add_years, cstr, getdate
...
class FiscalYear(Document):
	...
	if TYPE_CHECKING:
		...
		from erpnexus.accounts.doctype.fiscal_year_company.fiscal_year_company import FiscalYearCompany
	...
	def validate(self):
		...
		if not self.is_new():
			year_start_end_dates = saashq.db.sql(
	...
	def on_update(self):
		...
		saashq.cache().delete_value("fiscal_years")

	def on_trash(self):
		saashq.cache().delete_value("fiscal_years")

	def validate_overlap(self):
		existing_fiscal_years = saashq.db.sql(
		...
		if existing_fiscal_years:
			for existing in existing_fiscal_years:
				company_for_existing = saashq.db.sql_list(
...
def auto_create_fiscal_year():
	for d in saashq.db.sql(
		...
		try:
			current_fy = saashq.get_doc("Fiscal Year", d[0])

			new_fy = saashq.copy_doc(current_fy, ignore_no_copy=False)
...
def get_from_and_to_date(fiscal_year):
	...
	cached_results = saashq.get_cached_value("Fiscal Year", fiscal_year, fields, as_dict=1)
