...
saashq.ui.form.on("Ledger Merge", {
	setup: function (frm) {
		saashq.realtime.on("ledger_merge_refresh", ({ ledger_merge }) => {
			if (ledger_merge !== frm.doc.name) return;
			saashq.model.clear_doc(frm.doc.doctype, frm.doc.name);
			saashq.model.with_doc(frm.doc.doctype, frm.doc.name).then(() => {
				frm.refresh();
			});
		});

		saashq.realtime.on("ledger_merge_progress", (data) => {
		...
		frm.set_query("account", function (doc) {
			if (!doc.company) saashq.throw(__("Please set Company"));
			if (!doc.root_type) saashq.throw(__("Please set Root Type"));
			return {
				filters: {
		...
		frm.set_query("account", "merge_accounts", function (doc) {
			if (!doc.company) saashq.throw(__("Please set Company"));
			if (!doc.root_type) saashq.throw(__("Please set Root Type"));
			if (!doc.account) saashq.throw(__("Please set Account"));
			let acc = [doc.account];
...
saashq.ui.form.on("Ledger Merge Accounts", {
