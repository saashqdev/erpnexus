...
from saashq.utils import cint, cstr, flt, formatdate, get_link_to_form, getdate, nowdate
...
from erpnexus.accounts.party import get_due_date, get_party_account
from erpnexus.accounts.utils import get_account_currency, get_fiscal_year
from erpnexus.assets.doctype.asset.asset import is_cwip_accounting_enabled
from erpnexus.assets.doctype.asset_category.asset_category import get_asset_category_account
...
from erpnexus.controllers.buying_controller import BuyingController
...
class WarehouseMissingError(saashq.ValidationError):
	pass
...
class PurchaseInvoice(BuyingController):
	...
	if TYPE_CHECKING:
		...
		from erpnexus.accounts.doctype.advance_tax.advance_tax import AdvanceTax
		...
		from erpnexus.accounts.doctype.purchase_invoice_advance.purchase_invoice_advance import (
			PurchaseInvoiceAdvance,
		)
		from erpnexus.accounts.doctype.purchase_invoice_item.purchase_invoice_item import PurchaseInvoiceItem
		from erpnexus.accounts.doctype.purchase_taxes_and_charges.purchase_taxes_and_charges import (
			PurchaseTaxesandCharges,
		)
		from erpnexus.accounts.doctype.tax_withheld_vouchers.tax_withheld_vouchers import TaxWithheldVouchers
		from erpnexus.buying.doctype.purchase_receipt_item_supplied.purchase_receipt_item_supplied import (
	...
	def onload(self):
		super().onload()
		supplier_tds = saashq.db.get_value("Supplier", self.supplier, "tax_withholding_category")
	...
	def validate_release_date(self):
		if self.release_date and getdate(nowdate()) >= getdate(self.release_date):
			saashq.throw(_("Release date must be in the future"))

	def validate_cash(self):
		if not self.cash_bank_account and flt(self.paid_amount):
			saashq.throw(_("Cash or Bank Account is mandatory for making payment entry"))
	...
	def set_missing_values(self, for_validate=False):
		if not self.credit_to:
			...
			self.party_account_currency = saashq.get_cached_value(
		...
		tds_category = saashq.db.get_value("Supplier", self.supplier, "tax_withholding_category")
	...
	def validate_credit_to_acc(self):
		...
		if account.report_type != "Balance Sheet":
			saashq.throw(
				...
				).format(saashq.bold(_("Credit To"))),
		...
		if self.supplier and account.account_type != "Payable":
			saashq.throw(
				...
				).format(saashq.bold(_("Credit To")), saashq.bold(self.credit_to)),
				title=_("Invalid Account"),
			)
	...
	def validate_with_previous_doc(self):
		...
		if (
			cint(saashq.db.get_single_value("Buying Settings", "maintain_same_rate"))
	...
	def validate_item_code(self):
		for d in self.get("items"):
			if not d.item_code:
				saashq.msgprint(_("Item Code required at Row No {0}").format(d.idx), raise_exception=True)
	...
	def set_expense_account(self, for_validate=False):
		...
		for item in self.get("items"):
			...
			if (
				...
				and (
					not item.po_detail
					or not saashq.db.get_value("Purchase Order Item", item.po_detail, "delivered_by_supplier")
				)
			):
				if self.update_stock and item.warehouse and (not item.from_warehouse):
					if (
						...
						msg = _(
							...
							item.idx,
							saashq.bold(warehouse_account[item.warehouse]["account"]),
							saashq.bold(item.expense_account),
							saashq.bold(item.warehouse),
						)
						saashq.msgprint(msg, title=_("Expense Head Changed"))
					item.expense_account = warehouse_account[item.warehouse]["account"]
				else:
					# check if 'Stock Received But Not Billed' account is credited in Purchase receipt or not
					if item.purchase_receipt:
						negative_expense_booked_in_pr = saashq.db.sql(
						...
						if negative_expense_booked_in_pr:
							if (
								...
								msg = _(
									...
									item.idx,
									saashq.bold(stock_not_billed_account),
									saashq.bold(item.purchase_receipt),
								)
								saashq.msgprint(msg, title=_("Expense Head Changed"))
						...
						if (
							...
							msg = _(
								...
								item.idx, saashq.bold(stock_not_billed_account), saashq.bold(item.item_code)
							)
							msg += "<br>"
							msg += _(
				...
				if not item.pr_detail and item.po_detail:
					receipt_item = saashq.get_cached_value(
						"Purchase Receipt Item",
						{
				...
				if item.pr_detail:
					...
					arbnb_booked_in_pr = saashq.db.get_value(
						"GL Entry",
						{
	...
	def force_set_against_expense_account(self):
		self.set_against_expense_account()
		saashq.db.set_value(self.doctype, self.name, "against_expense_account", self.against_expense_account)

	def po_required(self):
		if saashq.db.get_single_value("Buying Settings", "po_required") == "Yes":
			if saashq.get_value(
			...
			for d in self.get("items"):
				if not d.purchase_order:
					msg = _("Purchase Order Required for item {}").format(saashq.bold(d.item_code))
					msg += "<br><br>"
					msg += _(
						"To submit the invoice without purchase order please set {0} as {1} in {2}"
					).format(
						saashq.bold(_("Purchase Order Required")),
						saashq.bold(_("No")),
	...
	def pr_required(self):
		stock_items = self.get_stock_items()
		if saashq.db.get_single_value("Buying Settings", "pr_required") == "Yes":
			if saashq.get_value(
			...
			for d in self.get("items"):
				if not d.purchase_receipt and d.item_code in stock_items:
					msg = _("Purchase Receipt Required for item {}").format(saashq.bold(d.item_code))
					msg += "<br><br>"
					msg += _(
						"To submit the invoice without purchase receipt please set {0} as {1} in {2}"
					).format(
						saashq.bold(_("Purchase Receipt Required")),
						saashq.bold(_("No")),
						get_link_to_form("Buying Settings", "Buying Settings", "Buying Settings"),
					)
	...
	def check_prev_docstatus(self):
		for d in self.get("items"):
			if d.purchase_order:
				submitted = saashq.db.sql(
					"select name from `tabPurchase Order` where docstatus = 1 and name = %s", d.purchase_order
				)
				if not submitted:
					saashq.throw(_("Purchase Order {0} is not submitted").format(d.purchase_order))
			if d.purchase_receipt:
				...
				if not submitted:
					saashq.throw(_("Purchase Receipt {0} is not submitted").format(d.purchase_receipt))
	...
	def on_submit(self):
		...
		if saashq.db.get_single_value("Buying Settings", "project_update_frequency") == "Each Transaction":
			self.update_project()
	...
	def get_gl_entries(self, warehouse_account=None):
		self.auto_accounting_for_stock = erpnexus.is_perpetual_inventory_enabled(self.company)
	...
	def check_asset_cwip_enabled(self):
		...
		for item in self.get("items"):
			if item.item_code and item.is_fixed_asset:
				asset_category = saashq.get_cached_value("Item", item.item_code, "asset_category")
	...
	def make_item_gl_entries(self, gl_entries):
		...
		if self.update_stock:
			stock_ledger_entries = saashq.get_all(
				"Stock Ledger Entry",
		...
		for item in self.get("items"):
			if flt(item.base_net_amount):
				...
				if item.item_code:
					saashq.get_cached_value("Item", item.item_code, "asset_category")
	...
	def get_provisional_accounts(self):
		...
		provisional_gl_entries = saashq.get_all(
			"GL Entry",
			filters={
				"voucher_type": "Purchase Receipt",
	...
	def make_provisional_gl_entry(self, gl_entries, item):
		if item.purchase_receipt:
			...
			if pr_item.get("has_provisional_entry"):
				purchase_receipt_doc = saashq.get_cached_doc("Purchase Receipt", item.purchase_receipt)
	...
	def update_project(self):
		...
		pj = saashq.qb.DocType("Project")
		for proj, value in projects.items():
			res = saashq.qb.from_(pj).select(pj.total_purchase_cost).where(pj.name == proj).for_update().run()
			current_purchase_cost = res and res[0][0] or 0
			# saashq.db.set_value("Project", proj, "total_purchase_cost", current_purchase_cost + value)
			project_doc = saashq.get_doc("Project", proj)
	...
	def validate_supplier_invoice(self):
		if self.bill_date:
			if getdate(self.bill_date) > getdate(self.posting_date):
				saashq.throw(_("Supplier Invoice Date cannot be greater than Posting Date"))

		if self.bill_no:
			if cint(saashq.db.get_single_value("Accounts Settings", "check_supplier_invoice_uniqueness")):
				...
				if pi:
					pi = pi[0][0]
					saashq.throw(_("Supplier Invoice No exists in Purchase Invoice {0}").format(pi))
	...
	def update_billing_status_in_pr(self, update_modified=True):
		...
		adjust_incoming_rate = saashq.db.get_single_value(
			"Buying Settings", "set_landed_cost_based_on_purchase_invoice_rate"
		)

		for pr in set(updated_pr):
			from erpnexus.stock.doctype.purchase_receipt.purchase_receipt import update_billing_percentage

			pr_doc = saashq.get_doc("Purchase Receipt", pr)
	...
	def get_pr_details_billed_amt(self):
		...
		if pr_details:
			doctype = saashq.qb.DocType("Purchase Invoice Item")
			query = (
			...
			pr_details_billed_amt = saashq._dict(query.run(as_list=1))
	...
	def set_tax_withholding(self):
		...
		if self.apply_tds and not self.get("tax_withholding_category"):
			self.tax_withholding_category = saashq.db.get_value(
	...
	def update_advance_tax_references(self, cancel=0):
		for tax in self.get("advance_tax"):
			at = saashq.qb.DocType("Advance Taxes and Charges").as_("at")

			if cancel:
				saashq.qb.update(at).set(
...
def get_purchase_document_details(doc):
	...
	exchange_rate_map = saashq._dict(
		saashq.get_all(
			parent_doctype,
	...
	net_rate_map = saashq._dict(
		saashq.get_all(child_doctype, filters={"name": ("in", items)}, fields=["name", "net_rate"], as_list=1)
	)
...
def change_release_date(name, release_date=None):
	if saashq.db.exists("Purchase Invoice", name):
		pi = saashq.get_doc("Purchase Invoice", name)
