...
from saashq.utils import add_days, cint, flt, getdate, nowdate, today

import erpnexus
from erpnexus.accounts.doctype.account.test_account import create_account, get_inventory_account
from erpnexus.accounts.doctype.payment_entry.payment_entry import get_payment_entry
from erpnexus.buying.doctype.purchase_order.purchase_order import get_mapped_purchase_invoice
from erpnexus.buying.doctype.purchase_order.purchase_order import make_purchase_invoice as make_pi_from_po
...
from erpnexus.controllers.accounts_controller import InvalidQtyError, get_payment_terms
...
from erpnexus.stock.doctype.material_request.material_request import make_purchase_order
from erpnexus.stock.doctype.material_request.test_material_request import make_material_request
from erpnexus.stock.doctype.purchase_receipt.purchase_receipt import (
	make_purchase_invoice as create_purchase_invoice_from_receipt,
)
from erpnexus.stock.doctype.purchase_receipt.test_purchase_receipt import (
	get_taxes,
...
from erpnexus.stock.doctype.stock_entry.test_stock_entry import get_qty_after_transaction
from erpnexus.stock.tests.test_utils import StockTestMixin
...
class TestPurchaseInvoice(IntegrationTestCase, StockTestMixin):
	...
	def test_update_received_qty_in_material_request(self):
		from erpnexus.buying.doctype.purchase_order.purchase_order import make_purchase_invoice
	...
	def test_gl_entries_without_perpetual_inventory(self):
		...
		pi = saashq.copy_doc(self.globalTestRecords["Purchase Invoice"][0])
		self.assertTrue(not cint(erpnexus.is_perpetual_inventory_enabled(pi.company)))
		pi.insert()
		pi.submit()
	...
	def test_gl_entries_with_perpetual_inventory(self):
		...
		self.assertTrue(cint(erpnexus.is_perpetual_inventory_enabled(pi.company)), 1)

		self.check_gle_for_pi(pi.name)

	def test_terms_added_after_save(self):
		pi = saashq.copy_doc(self.globalTestRecords["Purchase Invoice"][1])
		pi.insert()
	...
	def test_payment_entry_unlink_against_purchase_invoice(self):
		...
		pi_doc = saashq.get_doc("Purchase Invoice", pi_doc.name)
		pi_doc.load_from_db()
		self.assertTrue(pi_doc.status, "Paid")

		self.assertRaises(saashq.LinkExistsError, pi_doc.cancel)
		unlink_payment_on_cancel_of_invoice()
	...
	def test_purchase_invoice_for_blocked_supplier(self):
		...
		self.assertRaises(saashq.ValidationError, make_purchase_invoice)
	...
	def test_purchase_invoice_blocked_invoice_must_be_in_future(self):
		...
		self.assertRaises(saashq.ValidationError, pi.save)
		pi.release_date = ""
		pi.save()
	...
	def test_purchase_invoice_temporary_blocked(self):
		...
		self.assertRaises(saashq.ValidationError, pe.save)
	...
	def test_purchase_invoice_with_exchange_rate_difference(self):
		...
		exchange_gain_loss_account = saashq.db.get_value("Company", pi.company, "exchange_gain_loss_account")
	...
	def test_purchase_invoice_change_naming_series(self):
		...
		self.assertRaises(saashq.CannotChangeConstantError, pi.save)
	...
	def test_purchase_invoice_with_advance(self):
		jv = saashq.copy_doc(self.globalTestRecords["Journal Entry"][1])
		jv.insert()
		jv.submit()
	...
	def test_total_purchase_cost_for_project(self):
		if not saashq.db.exists("Project", {"project_name": "_Test Project for Purchase"}):
			project = make_project({"project_name": "_Test Project for Purchase"})
		else:
			project = saashq.get_doc("Project", {"project_name": "_Test Project for Purchase"})

		existing_purchase_cost = saashq.db.sql(
		...
		self.assertEqual(
			saashq.db.get_value("Project", project.name, "total_purchase_cost"),
		...
		pi.cancel()
		self.assertEqual(
			saashq.db.get_value("Project", project.name, "total_purchase_cost"), existing_purchase_cost
		)
	...
	def test_purchase_invoice_update_stock_gl_entry_with_perpetual_inventory(self):
		pi = make_purchase_invoice(
			update_stock=1,
			posting_date=saashq.utils.nowdate(),
			posting_time=saashq.utils.nowtime(),
	...
	def test_auto_batch(self):
		item_code = saashq.db.get_value("Item", {"has_batch_no": 1, "create_new_batch": 1}, "name")
		...
		self.assertTrue(saashq.db.get_value("Batch", {"item": item_code, "reference_name": pi.name}))

	def test_update_stock_and_purchase_return(self):
		actual_qty_0 = get_qty_after_transaction()

		pi = make_purchase_invoice(
			update_stock=1, posting_date=saashq.utils.nowdate(), posting_time=saashq.utils.nowtime()
		)
	...
	def test_rejected_serial_no(self):
		...
		self.assertEqual(
			saashq.db.get_value("Serial No", rejected_serial_no, "warehouse"),
	...
	def test_outstanding_amount_after_advance_payment_entry_cancelation(self):
		...
		# added to avoid Document has been modified exception
		pe = saashq.get_doc("Payment Entry", pe.name)
		pe.cancel()
	...
	def test_duplicate_due_date_in_terms(self):
		...
		self.assertRaises(saashq.ValidationError, pi.insert)

	def test_debit_note(self):
		from erpnexus.accounts.doctype.payment_entry.test_payment_entry import get_payment_entry
		from erpnexus.accounts.doctype.sales_invoice.test_sales_invoice import get_outstanding_amount
		...
		pi_doc = saashq.get_doc("Purchase Invoice", pi.name)
	...
	def test_deferred_expense_via_journal_entry(self):
		...
		gl_entries = gl_entries = saashq.db.sql(
	...
	def test_gain_loss_with_advance_entry(self):
		unlink_enabled = saashq.db.get_single_value(
			"Accounts Settings", "unlink_payment_on_cancellation_of_invoice"
		)

		saashq.db.set_single_value("Accounts Settings", "unlink_payment_on_cancellation_of_invoice", 1)

		original_account = saashq.db.get_value("Company", "_Test Company", "exchange_gain_loss_account")
		saashq.db.set_value(
		...
		total_debit_amount = saashq.db.get_all(
		...
		self.assertEqual(
			saashq.db.get_value("Journal Entry", jea_parent.parent, "voucher_type"), "Exchange Gain Or Loss"
		)
		...
		jea_parent_2 = saashq.db.get_all(
			"Journal Entry Account",
			filters={
				"account": creditors_account,
				"docstatus": 1,
		...
		self.assertEqual(
			saashq.db.get_value("Journal Entry", jea_parent_2.parent, "voucher_type"),
		...
		self.assertEqual(saashq.db.get_value("Journal Entry", jea_parent.parent, "docstatus"), 2)

		pi_2.reload()
		pi_2.cancel()

		self.assertEqual(saashq.db.get_value("Journal Entry", jea_parent_2.parent, "docstatus"), 2)
		...
		saashq.db.set_value("Company", "_Test Company", "exchange_gain_loss_account", original_account)
	...
	def test_purchase_invoice_advance_taxes(self):
		...
		tds_account_args = {
			...
			"account_type": "Tax",
			"parent_account": saashq.db.get_value(
	...
	def test_provisional_accounting_entry_for_over_billing(self):
		...
		saashq.db.set_single_value("Buying Settings", "maintain_same_rate", 0)
	...
	def test_adjust_incoming_rate(self):
		...
		saashq.db.set_single_value("Buying Settings", "set_landed_cost_based_on_purchase_invoice_rate", 1)
		...
		saashq.db.set_single_value("Buying Settings", "set_landed_cost_based_on_purchase_invoice_rate", 0)
		...
		saashq.db.set_single_value("Buying Settings", "maintain_same_rate", 1)

	def test_item_less_defaults(self):
		pi = saashq.new_doc("Purchase Invoice")
	...
	def test_batch_expiry_for_purchase_invoice(self):
		...
		saashq.db.set_value("Batch", batch_no, "expiry_date", add_days(nowdate(), -1))
	...
	def test_offsetting_entries_for_accounting_dimensions(self):
		...
		from erpnexus.accounts.report.trial_balance.test_trial_balance import (
		...
		branch1 = saashq.new_doc("Branch")
		branch1.branch = "Location 1"
		branch1.insert(ignore_if_duplicate=True)
		branch2 = saashq.new_doc("Branch")
	...
	def test_repost_accounting_entries(self):
		...
		settings = saashq.get_doc("Repost Accounting Ledger Settings")
	...
	def test_purchase_invoice_without_supplier_group(self):
		...
		if not saashq.db.exists("Supplier", test_supplier_name):
	...
	def test_debit_note_with_account_mismatch(self):
		...
		self.assertRaises(saashq.ValidationError, dr_note.save)
	...
	def test_make_pr_and_pi_from_po(self):
		from erpnexus.assets.doctype.asset.test_asset import create_asset_category

		if not saashq.db.exists("Asset Category", "Computers"):
	...
	def test_opening_invoice_rounding_adjustment_validation(self):
		...
		pi.is_opening = "Yes"
		pi.save()
		self.assertRaises(saashq.ValidationError, pi.submit)
...
def toggle_provisional_accounting_setting(**args):
	args = saashq._dict(args)
	company = saashq.get_doc("Company", args.company or "_Test Company")
