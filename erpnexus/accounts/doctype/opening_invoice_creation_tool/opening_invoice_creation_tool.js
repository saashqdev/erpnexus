...
saashq.ui.form.on("Opening Invoice Creation Tool", {
	setup: function (frm) {
		...
		saashq.realtime.on("opening_invoice_creation_progress", (data) => {
				...
				setTimeout(
					() => {
						...
						if (frm.doc.invoice_type == "Sales") {
							saashq.msgprint(__("Opening Sales Invoices have been created."));
						} else {
							saashq.msgprint(__("Opening Purchase Invoices have been created."));
						}
					},
					1500,
					data.title
	...
	company: function (frm) {
		if (frm.doc.company) {
			...
			saashq.call({
				method: "erpnexus.accounts.doctype.opening_invoice_creation_tool.opening_invoice_creation_tool.get_temporary_opening_account",
				args: {
	...
	make_dashboard: function (frm) {
		...
		if (!$.isEmptyObject(opening_invoices_summary)) {
			let section = frm.dashboard.add_section(
				saashq.render_template("opening_invoice_creation_tool_dashboard", {
			...
			section.on("click", ".invoice-link", function () {
				...
				saashq.set_route("List", doctype, { is_opening: "Yes", company: company, docstatus: 1 });
			});
			frm.dashboard.show();
		}
	},
...
saashq.ui.form.on("Opening Invoice Creation Tool Item", {
	invoices_add: (frm) => {
		frm.trigger("update_invoice_table");
	},
});
