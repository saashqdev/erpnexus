...
from saashq.utils import cint, flt, getdate, today

from erpnexus.accounts.doctype.loyalty_program.loyalty_program import (
	get_loyalty_details,
	get_loyalty_program_details_with_points,
)
from erpnexus.accounts.party import get_dashboard_info
...
class TestLoyaltyProgram(IntegrationTestCase):
	...
	def test_loyalty_points_earned_single_tier(self):
		...
		customer = saashq.get_doc("Customer", {"customer_name": "Test Loyalty Customer"})
	...
	def test_loyalty_points_earned_multiple_tier(self):
		saashq.db.set_value("Customer", "Test Loyalty Customer", "loyalty_program", "Test Multiple Loyalty")
		# assign multiple tier program to the customer
		customer = saashq.get_doc("Customer", {"customer_name": "Test Loyalty Customer"})
		customer.loyalty_program = saashq.get_doc(
	...
	def test_cancel_sales_invoice(self):
		...
		si.cancel()
		lpe = saashq.db.exists("Loyalty Point Entry", lpe.name)
	...
	def test_sales_invoice_return(self):
		...
		si_original = saashq.get_doc("Sales Invoice", lpe_original.invoice)

		earned_points = get_points_earned(si_original)
		lpe_after_return = saashq.get_doc(
			"Loyalty Point Entry",
			{
				"invoice_type": "Sales Invoice",
				"invoice": si_original.name,
	...
	return saashq.get_doc(
		{
			...
			"customer": saashq.get_doc("Customer", {"customer_name": "Test Loyalty Customer"}).name,
			"company": "_Test Company",
			"due_date": today(),
			"posting_date": today(),
			"currency": "INR",
			...
			"items": [
				{
					...
					"item_code": saashq.get_doc("Item", {"item_name": "Loyal Item"}).name,
					"qty": qty,
					"rate": 10000,
...
def create_records():
	...
	if not saashq.db.exists("Account", "Loyalty - _TC"):
		saashq.get_doc(
			{
				"doctype": "Account",
	...
	if not saashq.db.exists("Loyalty Program", "Test Single Loyalty"):
		saashq.get_doc(
			{
				"doctype": "Loyalty Program",
	...
	if not saashq.db.exists("Customer", "Test Loyalty Customer"):
		saashq.get_doc(
			{
	...
	if not saashq.db.exists("Loyalty Program", "Test Multiple Loyalty"):
		saashq.get_doc(
			{
				"doctype": "Loyalty Program",
	...
	# create an item
	if not saashq.db.exists("Item", "Loyal Item"):
		saashq.get_doc(
			{
				"doctype": "Item",
	...
	# create item price
	if not saashq.db.exists("Item Price", {"price_list": "Standard Selling", "item_code": "Loyal Item"}):
		saashq.get_doc(
			{
				"doctype": "Item Price",
