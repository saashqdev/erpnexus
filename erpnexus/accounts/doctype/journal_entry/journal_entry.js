...
saashq.provide("erpnexus.journal_entry");

saashq.ui.form.on("Journal Entry", {
	setup: function (frm) {
	...
	refresh: function (frm) {
		...
		if (frm.doc.docstatus == 1) {
			frm.add_custom_button(
				...
				function () {
					return erpnexus.journal_entry.reverse_journal_entry(frm);
				},
				__("Actions")
			);
		}
		...
		if (frm.doc.__islocal) {
			frm.add_custom_button(__("Quick Entry"), function () {
				return erpnexus.journal_entry.quick_entry(frm);
			});
		}

		// hide /unhide fields based on currency
		erpnexus.journal_entry.toggle_fields_based_on_currency(frm);
	...
	make_inter_company_journal_entry: function (frm) {
		...
		d.set_primary_action(__("Create"), function () {
			...
			saashq.call({
				...
				method: "erpnexus.accounts.doctype.journal_entry.journal_entry.make_inter_company_journal_entry",
				callback: function (r) {
					if (r.message) {
	...
	posting_date: function (frm) {
		...
		$.each(frm.doc.accounts || [], function (i, row) {
			erpnexus.journal_entry.set_exchange_rate(frm, row.doctype, row.name);
		});
	},
			...
			callback: function (r) {
				if (r.message) {
					$.each(frm.doc.accounts || [], function (i, jvd) {
						saashq.model.set_value(jvd.doctype, jvd.name, "cost_center", r.message.cost_center);
					});
				}
			},
		});
	...
	voucher_type: function (frm) {
		...
		if (
			...
			if (["Bank Entry", "Cash Entry"].includes(frm.doc.voucher_type)) {
				return saashq.call({
					type: "GET",
					method: "erpnexus.accounts.doctype.journal_entry.journal_entry.get_default_bank_cash_account",
					args: {
						account_type:
	...
	from_template: function (frm) {
		if (frm.doc.from_template) {
			saashq.db.get_doc("Journal Entry Template", frm.doc.from_template).then((doc) => {
				saashq.model.clear_table(frm.doc, "accounts");
				frm.set_value({
					company: doc.company,
...
var update_jv_details = function (doc, r) {
	$.each(r, function (i, d) {
		var row = saashq.model.add_child(doc, "Journal Entry Account", "accounts");
		saashq.model.set_value(row.doctype, row.name, "account", d.account);
	});
	refresh_field("accounts");
};

erpnexus.accounts.JournalEntry = class JournalEntry extends saashq.ui.form.Controller {
	onload() {
		this.load_defaults();
		this.setup_queries();
		erpnexus.accounts.dimensions.setup_dimension_filters(this.frm, this.frm.doctype);
	}
	...
	load_defaults() {
		...
		if (this.frm.doc.__islocal && this.frm.doc.company) {
			saashq.model.set_default_values(this.frm.doc);
			$.each(this.frm.doc.accounts || [], function (i, jvd) {
				saashq.model.set_default_values(jvd);
			});
			var posting_date = this.frm.doc.posting_date;
			if (!this.frm.doc.amended_from)
				this.frm.set_value("posting_date", posting_date || saashq.datetime.get_today());
		}
	}
	...
	setup_queries() {
		...
		me.frm.set_query("account", "accounts", function (doc, cdt, cdn) {
			return erpnexus.journal_entry.account_query(me.frm);
		});
		...
		me.frm.set_query("reference_name", "accounts", function (doc, cdt, cdn) {
			var jvd = saashq.get_doc(cdt, cdn);

			// journal entry
			if (jvd.reference_type === "Journal Entry") {
				saashq.model.validate_missing(jvd, "account");
				return {
					query: "erpnexus.accounts.doctype.journal_entry.journal_entry.get_against_jv",
					filters: {
						account: jvd.account,
						party: jvd.party,
					},
			...
			if (["Sales Order", "Purchase Order"].includes(jvd.reference_type)) {
				...
				saashq.model.validate_missing(jvd, "party_type");
				saashq.model.validate_missing(jvd, "party");
	...
	get_outstanding(doctype, docname, company, child) {
		...
		return saashq.call({
			method: "erpnexus.accounts.doctype.journal_entry.journal_entry.get_outstanding",
			args: { args: args },
			callback: function (r) {
				if (r.message) {
					$.each(r.message, function (field, value) {
						saashq.model.set_value(child.doctype, child.name, field, value);
					});
				}
			},
		});
	...
	accounts_add(doc, cdt, cdn) {
		...
		erpnexus.accounts.dimensions.copy_dimension_from_first_row(this.frm, cdt, cdn, "accounts");
	}
};

cur_frm.script_manager.make(erpnexus.accounts.JournalEntry);
...
saashq.ui.form.on("Journal Entry Account", {
	party: function (frm, cdt, cdn) {
		var d = saashq.get_doc(cdt, cdn);
		if (!d.account && d.party_type && d.party) {
			if (!frm.doc.company) saashq.throw(__("Please select Company"));
			return frm.call({
				method: "erpnexus.accounts.doctype.journal_entry.journal_entry.get_party_account_and_currency",
				child: d,
				args: {
					company: frm.doc.company,
	...
	cost_center: function (frm, dt, dn) {
		...
		if (frm.doc.voucher_type != "Exchange Gain Or Loss") {
			erpnexus.journal_entry.set_account_details(frm, dt, dn);
		}
	},
	...
	debit_in_account_currency: function (frm, cdt, cdn) {
		erpnexus.journal_entry.set_exchange_rate(frm, cdt, cdn);
	},
	...
	exchange_rate: function (frm, cdt, cdn) {
		...
		if (row.account_currency == company_currency || !frm.doc.multi_currency) {
			saashq.model.set_value(cdt, cdn, "exchange_rate", 1);
		}

		erpnexus.journal_entry.set_debit_credit_in_company_currency(frm, cdt, cdn);
	},
});

saashq.ui.form.on("Journal Entry Account", "accounts_remove", function (frm) {
	frm.cscript.update_totals(frm.doc);
});

$.extend(erpnexus.journal_entry, {
	...
	set_exchange_rate: function (frm, cdt, cdn) {
		...
		if (row.account_currency == company_currency || !frm.doc.multi_currency) {
			...
			saashq.call({
				method: "erpnexus.accounts.doctype.journal_entry.journal_entry.get_exchange_rate",
				args: {
	...
	quick_entry: function (frm) {
		...
		var dialog = new saashq.ui.Dialog({
			...
			fields: [
				{ fieldtype: "Currency", fieldname: "debit", label: __("Amount"), reqd: 1 },
				{
					...
					options: "Account",
					get_query: function () {
						return erpnexus.journal_entry.account_query(frm);
					},
				},
				{
					fieldtype: "Link",
		...
		dialog.set_primary_action(__("Save"), function () {
			...
			saashq.model.set_value(debit_row.doctype, debit_row.name, "account", values.debit_account);
			saashq.model.set_value(
				debit_row.doctype,
				debit_row.name,
			...
			saashq.model.set_value(credit_row.doctype, credit_row.name, "account", values.credit_account);
			saashq.model.set_value(
				credit_row.doctype,
				credit_row.name,
	...
	account_query: function (frm) {
		...
		if (!frm.doc.multi_currency) {
			$.extend(filters, {
				account_currency: [
					"in",
					[saashq.get_doc(":Company", frm.doc.company).default_currency, null],
				],
			});
		}
	...
	reverse_journal_entry: function (frm) {
		saashq.model.open_mapped_doc({
			method: "erpnexus.accounts.doctype.journal_entry.journal_entry.make_reverse_journal_entry",
			frm: frm,
		});
	},
});
...
$.extend(erpnexus.journal_entry, {
	set_account_details: function (frm, dt, dn) {
		var d = locals[dt][dn];
		if (d.account) {
			if (!frm.doc.company) saashq.throw(__("Please select Company first"));
			if (!frm.doc.posting_date) saashq.throw(__("Please select Posting Date first"));

			return saashq.call({
				method: "erpnexus.accounts.doctype.journal_entry.journal_entry.get_account_details_and_party_type",
				args: {
					account: d.account,
				...
				callback: function (r) {
					if (r.message) {
						$.extend(d, r.message);
						erpnexus.journal_entry.set_amount_on_last_row(frm, dt, dn);
						erpnexus.journal_entry.set_debit_credit_in_company_currency(frm, dt, dn);
						refresh_field("accounts");
					}
				},
			});
