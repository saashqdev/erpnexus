...
from erpnexus.accounts.doctype.journal_entry.journal_entry import StockAccountInvalidTransaction
...
class TestJournalEntry(IntegrationTestCase):
	...
	def test_journal_entry_with_against_jv(self):
		jv_invoice = saashq.copy_doc(self.globalTestRecords["Journal Entry"][2])
		base_jv = saashq.copy_doc(self.globalTestRecords["Journal Entry"][0])
	...
	def test_jv_against_purchase_order(self):
		...
		base_jv = saashq.copy_doc(self.globalTestRecords["Journal Entry"][1])
	...
	def jv_against_voucher_testcase(self, base_jv, test_voucher):
		...
		submitted_voucher = saashq.get_doc(test_voucher.doctype, test_voucher.name)
	...
	def cancel_against_voucher_testcase(self, test_voucher):
		if test_voucher.doctype == "Journal Entry":
			...
			submitted_voucher = saashq.get_doc(test_voucher.doctype, test_voucher.name)
			self.assertRaises(saashq.LinkExistsError, submitted_voucher.cancel)
	...
	def test_jv_with_project(self):
		...
		if not saashq.db.exists("Project", {"project_name": "Journal Entry Project"}):
			...
			project_name = saashq.get_value("Project", {"project_name": "_Test Project"})
	...
	def test_negative_debit_and_credit_with_same_account_head(self):
		...
		# Create JV with defaut cost center - _Test Cost Center
		saashq.db.set_single_value("Accounts Settings", "merge_similar_account_heads", 0)
