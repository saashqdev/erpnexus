...
import saashq
from saashq import _, msgprint, scrub
from saashq.utils import comma_and, cstr, flt, fmt_money, formatdate, get_link_to_form, nowdate

import erpnexus
from erpnexus.accounts.deferred_revenue import get_deferred_booking_accounts
...
class StockAccountInvalidTransaction(saashq.ValidationError):
	pass
...
class JournalEntry(AccountsController):
	...
	if TYPE_CHECKING:
		...
		from erpnexus.accounts.doctype.journal_entry_account.journal_entry_account import JournalEntryAccount
	...
	def validate(self):
		...
		if not saashq.flags.is_reverse_depr_entry:
	...
	def validate_advance_accounts(self):
		...
		advance_accounts.add(
			saashq.get_cached_value("Company", self.company, "default_advance_received_account")
		)
		advance_accounts.add(saashq.get_cached_value("Company", self.company, "default_advance_paid_account"))
		if advance_accounts_used := journal_accounts & advance_accounts:
			saashq.msgprint(
				...
				).format(saashq.bold(comma_and(advance_accounts_used)))
			)
	...
	def update_advance_paid(self):
		...
		for voucher_type, order_list in advance_paid.items():
			for voucher_no in list(set(order_list)):
				saashq.get_doc(voucher_type, voucher_no).set_total_advance_paid()

	def validate_inter_company_accounts(self):
		if self.voucher_type == "Inter Company Journal Entry" and self.inter_company_journal_entry_reference:
			doc = saashq.get_doc("Journal Entry", self.inter_company_journal_entry_reference)
			account_currency = saashq.get_cached_value("Company", self.company, "default_currency")
			previous_account_currency = saashq.get_cached_value("Company", doc.company, "default_currency")
			if account_currency == previous_account_currency:
				if self.total_credit != doc.total_debit or self.total_debit != doc.total_credit:
					saashq.throw(_("Total Credit/ Debit Amount should be same as linked Journal Entry"))
	...
	def validate_depr_entry_voucher_type(self):
		if (
			...
			saashq.throw(_("Journal Entry type should be set as Depreciation Entry for asset depreciation"))
	...
	def apply_tax_withholding(self):
		from erpnexus.accounts.report.general_ledger.general_ledger import get_account_type_map
		...
		if len(parties) > 1:
			saashq.throw(_("Cannot apply TDS against multiple parties in one entry"))
	...
	def update_asset_value(self):
		...
		for d in self.get("accounts"):
			if (
				...
				asset = saashq.get_doc("Asset", d.reference_name)
	...
	def update_invoice_discounting(self):
		...
		for inv_disc in invoice_discounting_list:
			inv_disc_doc = saashq.get_doc("Invoice Discounting", inv_disc)
			status = None
	...
	def update_booked_depreciation(self, cancel=0):
		for d in self.get("accounts"):
			if (
				...
				and d.reference_name
				and saashq.get_cached_value("Account", d.account, "root_type") == "Expense"
				and d.debit
			):
				asset = saashq.get_doc("Asset", d.reference_name)
				for fb_row in asset.get("finance_books"):
	...
	def unlink_advance_entry_reference(self):
		for d in self.get("accounts"):
			if d.is_advance == "Yes" and d.reference_type in ("Sales Invoice", "Purchase Invoice"):
				doc = saashq.get_doc(d.reference_type, d.reference_name)
	...
	def unlink_asset_reference(self):
		for d in self.get("accounts"):
			if (
				...
				journal_entry_for_scrap = saashq.db.get_value(
	...
	def unlink_inter_company_jv(self):
		if self.voucher_type == "Inter Company Journal Entry" and self.inter_company_journal_entry_reference:
			...
			saashq.db.set_value("Journal Entry", self.name, "inter_company_journal_entry_reference", "")
	...
	def validate_party(self):
		for d in self.get("accounts"):
			account_type = saashq.get_cached_value("Account", d.account, "account_type")
			if account_type in ["Receivable", "Payable"]:
				if not (d.party_type and d.party):
					...
					d.party_type
					and saashq.db.get_value("Party Type", d.party_type, "account_type") != account_type
				):
					saashq.throw(
			...
			if d.reference_type not in ("Sales Invoice", "Purchase Invoice", "Journal Entry"):
				...
				if d.is_advance == "Yes":
					if d.party_type == "Customer" and flt(d.debit) > 0:
						saashq.throw(_("Row {0}: Advance against Customer must be credit").format(d.idx))
					elif d.party_type == "Supplier" and flt(d.credit) > 0:
						saashq.throw(_("Row {0}: Advance against Supplier must be debit").format(d.idx))
	...
	def validate_against_jv(self):
		for d in self.get("accounts"):
			if d.reference_type == "Journal Entry":
				account_root_type = saashq.get_cached_value("Account", d.account, "root_type")
				if (
				...
				if d.reference_name == self.name:
					saashq.throw(_("You can not enter current voucher in 'Against Journal Entry' column"))

				against_entries = saashq.db.sql(
	...
	def validate_orders(self):
		...
		for reference_name, total in self.reference_totals.items():
			...
			if reference_type in ("Sales Order", "Purchase Order"):
				order = saashq.get_doc(reference_type, reference_name)

				if order.docstatus != 1:
					saashq.throw(_("{0} {1} is not submitted").format(reference_type, reference_name))

				if flt(order.per_billed) >= 100:
					saashq.throw(_("{0} {1} is fully billed").format(reference_type, reference_name))

				if cstr(order.status) == "Closed":
					saashq.throw(_("{0} {1} is closed").format(reference_type, reference_name))
	...
	def set_against_account(self):
		...
		if self.voucher_type in ("Deferred Revenue", "Deferred Expense"):
			for d in self.get("accounts"):
				...
				d.against_account = saashq.db.get_value(d.reference_type, d.reference_name, field)
		else:
			for d in self.get("accounts"):
				if flt(d.debit) > 0:
		...
		if not (self.voucher_type == "Exchange Gain Or Loss" and self.multi_currency):
			for d in self.get("accounts"):
				if not flt(d.debit) and not flt(d.credit):
					saashq.throw(_("Row {0}: Both Debit and Credit values cannot be zero").format(d.idx))
	...
	def set_total_debit_credit(self):
		...
		for d in self.get("accounts"):
			if d.debit and d.credit:
				saashq.throw(_("You cannot credit and debit same account at the same time"))
	...
	def validate_multi_currency(self):
		...
		if alternate_currency:
			if not self.multi_currency:
				saashq.throw(_("Please check Multi Currency option to allow accounts with other currency"))
	...
	def set_exchange_rate(self):
		for d in self.get("accounts"):
			...
			if not d.exchange_rate:
				saashq.throw(_("Row {0}: Exchange Rate is mandatory").format(d.idx))
	...
	def create_remarks(self):
		...
		if self.cheque_no:
			if self.cheque_date:
				...
				msgprint(_("Please enter Reference date"), raise_exception=saashq.MandatoryError)
	...
	def set_print_format_fields(self):
		...
		for d in self.get("accounts"):
			...
			elif saashq.get_cached_value("Account", d.account, "account_type") in ["Bank", "Cash"]:
				bank_amount += flt(d.debit_in_account_currency) or flt(d.credit_in_account_currency)
				bank_account_currency = d.account_currency

		if party_type and pay_to_recd_from:
			self.pay_to_recd_from = saashq.db.get_value(
	...
	def build_gl_map(self):
		...
		company_currency = erpnexus.get_company_currency(self.company)
	...
	def make_gl_entries(self, cancel=0, adv_adj=0):
		...
		merge_entries = saashq.db.get_single_value("Accounts Settings", "merge_similar_account_heads")
				...
				if not blank_row:
					blank_row = self.append(
						"accounts",
						{
							...
							"cost_center": erpnexus.get_default_cost_center(self.company),
						},
					)
	...
	def validate_credit_debit_note(self):
		if self.stock_entry:
			if saashq.db.get_value("Stock Entry", self.stock_entry, "docstatus") != 1:
				saashq.throw(_("Stock Entry {0} is not submitted").format(self.stock_entry))
	...
	def validate_empty_accounts_table(self):
		if not self.get("accounts"):
			saashq.throw(_("Accounts table cannot be blank."))
...
def get_default_bank_cash_account(
	...
	from erpnexus.accounts.doctype.sales_invoice.sales_invoice import get_bank_cash_account
	...
	if not account:
		...
		if account_type == "Bank":
			account = saashq.get_cached_value("Company", company, "default_bank_account")
			if not account:
				account_list = saashq.get_all(
			...
			account = saashq.get_cached_value("Company", company, "default_cash_account")
			if not account:
				account_list = saashq.get_all(
	...
	if account:
		account_details = saashq.get_cached_value(
...
def get_payment_entry_against_order(
	...
	ref_doc = saashq.get_doc(dt, dn)

	if flt(ref_doc.per_billed, 2) > 0:
		saashq.throw(_("Can only make payment against unbilled {0}").format(dt))
...
def get_payment_entry(ref_doc, args):
	cost_center = ref_doc.get("cost_center") or saashq.get_cached_value(
	...
	party_row = je.append(
		"accounts",
		{
			...
			"account_type": saashq.get_cached_value("Account", args.get("party_account"), "account_type"),
...
def get_against_jv(doctype, txt, searchfield, start, page_len, filters):
	if not saashq.db.has_column("Journal Entry", searchfield):
		return []
...
def get_outstanding(args):
	if not saashq.has_permission("Account"):
		saashq.msgprint(_("No Permission"), raise_exception=1)

	if isinstance(args, str):
		args = json.loads(args)

	company_currency = erpnexus.get_company_currency(args.get("company"))
	due_date = None

	if args.get("doctype") == "Journal Entry":
		condition = " and party=%(party)s" if args.get("party") else ""

		against_jv_amount = saashq.db.sql(
			f"""
...
def get_party_account_and_currency(company, party_type, party):
	...
	return {
		"account": account,
		"account_currency": saashq.get_cached_value("Account", account, "account_currency"),
	}
...
def get_account_details_and_party_type(account, date, company, debit=None, credit=None, exchange_rate=None):
	...
	company_currency = erpnexus.get_company_currency(company)
	account_details = saashq.get_cached_value(
...
def get_exchange_rate(
	...
	if not account_details:
		saashq.throw(_("Please select correct account"))
	...
	if account_currency != company_currency:
		if reference_type in ("Sales Invoice", "Purchase Invoice") and reference_name:
			exchange_rate = saashq.db.get_value(reference_type, reference_name, "conversion_rate")
