...
from erpnexus.accounts.doctype.subscription_plan.subscription_plan import get_plan_rate
from erpnexus.accounts.party import get_party_account, get_party_bank_account
from erpnexus.accounts.utils import get_account_currency, get_currency_precision
from erpnexus.utilities import payment_app_import_guard
...
class PaymentRequest(Document):
	...
	if TYPE_CHECKING:
		...
		from erpnexus.accounts.doctype.subscription_plan_detail.subscription_plan_detail import (
	...
	def validate_reference_document(self):
		if not self.reference_doctype or not self.reference_name:
			saashq.throw(_("To create a Payment Request reference document is required"))
	...
	def validate_payment_request_amount(self):
		...
		ref_doc = saashq.get_doc(self.reference_doctype, self.reference_name)
		if not hasattr(ref_doc, "order_type") or ref_doc.order_type != "Shopping Cart":
			ref_amount = get_amount(ref_doc, self.payment_account)
			if not ref_amount:
				saashq.throw(_("Payment Entry is already created"))
	...
	def validate_currency(self):
		ref_doc = saashq.get_doc(self.reference_doctype, self.reference_name)
		if self.payment_account and ref_doc.currency != saashq.get_cached_value(
			"Account", self.payment_account, "account_currency"
		):
			saashq.throw(_("Transaction currency must be same as Payment Gateway currency"))

	def validate_subscription_details(self):
		if self.is_a_subscription:
			amount = 0
			for subscription_plan in self.subscription_plans:
				payment_gateway = saashq.db.get_value(
	...
	def get_request_amount(self):
		data_of_completed_requests = saashq.get_all(
			"Integration Request",
			filters={
	...
	def get_payment_url(self):
		if self.reference_doctype != "Fees":
			...
			data.update({"company": saashq.defaults.get_defaults().company})
		...
		return controller.get_payment_url(
			**{
				...
				"reference_doctype": "Payment Request",
				"reference_docname": self.name,
				"payer_email": self.email_to or saashq.session.user,
	...
	def create_payment_entry(self, submit=True):
		"""create entry"""
		saashq.flags.ignore_account_permission = True
	...
	def get_message(self):
		...
		if self.message:
			return saashq.render_template(self.message, context)
	...
	def check_if_payment_entry_exists(self):
		if self.status == "Paid":
			if saashq.get_all(
				...
				saashq.throw(_("Payment Entry already exists"), title=_("Error"))
...
def make_payment_request(**args):
	...
	if args.dt not in ALLOWED_DOCTYPES_FOR_PAYMENT_REQUEST:
		saashq.throw(_("Payment Requests cannot be created against: {0}").format(saashq.bold(args.dt)))

	ref_doc = args.ref_doc or saashq.get_doc(args.dt, args.dn)

	gateway_account = get_gateway_details(args) or saashq._dict()
	...
	draft_payment_request = saashq.db.get_value(
		"Payment Request",
	...
	if existing_payment_request_amount:
		...
		if not grand_total:
			saashq.throw(_("Payment Request is already created"))

	if draft_payment_request:
		saashq.db.set_value(
			"Payment Request", draft_payment_request, "grand_total", grand_total, update_modified=False
		)
		pr = saashq.get_doc("Payment Request", draft_payment_request)
	else:
		pr = saashq.new_doc("Payment Request")
		...
		if saashq.db.get_single_value("Accounts Settings", "create_pr_in_draft_status", cache=True):
	...
	if args.order_type == "Shopping Cart":
		...
		saashq.local.response["location"] = pr.get_payment_url()
...
def get_print_format_list(ref_doctype):
	...
	print_format_list.extend(
		[p.name for p in saashq.get_all("Print Format", filters={"doc_type": ref_doctype})]
	)
...
def resend_payment_email(docname):
	return saashq.get_doc("Payment Request", docname).send_email()


@saashq.whitelist()
def make_payment_entry(docname):
	doc = saashq.get_doc("Payment Request", docname)
...
def update_payment_requests_as_per_pe_references(references=None, cancel=False):
	...
	referenced_payment_requests = saashq.get_all(
		"Payment Request",
...
def get_subscription_details(reference_doctype, reference_name):
	if reference_doctype == "Sales Invoice":
		...
		for subscription in subscriptions:
			plans = saashq.get_doc("Subscription", subscription.sub_name).plans
			for plan in plans:
...
def validate_payment(doc, method=None):
	if doc.reference_doctype != "Payment Request" or (
		saashq.db.get_value(doc.reference_doctype, doc.reference_docname, "status") != "Paid"
	):
		return
...
def get_open_payment_requests_query(doctype, txt, searchfield, start, page_len, filters):
	...
	open_payment_requests = saashq.get_list(
		"Payment Request",
		filters={
