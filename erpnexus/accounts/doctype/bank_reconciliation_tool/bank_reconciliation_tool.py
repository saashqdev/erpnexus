...
from erpnexus import get_default_cost_center
from erpnexus.accounts.doctype.bank_transaction.bank_transaction import get_total_allocated_amount
from erpnexus.accounts.report.bank_reconciliation_statement.bank_reconciliation_statement import (
	get_amounts_not_reflected_in_system,
	get_entries,
)
from erpnexus.accounts.utils import get_account_currency, get_balance_on
...
def get_account_balance(bank_account, till_date):
	...
	account = saashq.db.get_value("Bank Account", bank_account, "account")
	filters = saashq._dict({"account": account, "report_date": till_date, "include_pos_transactions": 1})
...
def update_bank_transaction(bank_transaction_name, reference_number, party_type=None, party=None):
	...
	bank_transaction = saashq.get_doc("Bank Transaction", bank_transaction_name)
...
def create_journal_entry_bts(
	...
	bank_transaction = saashq.db.get_values(
		"Bank Transaction",
		bank_transaction_name,
		fieldname=["name", "deposit", "withdrawal", "bank_account", "currency"],
		as_dict=True,
	)[0]
	company_account = saashq.get_value("Bank Account", bank_transaction.bank_account, "account")
	account_type = saashq.db.get_value("Account", second_account, "account_type")
	...
	company = saashq.get_value("Account", company_account, "company")
	company_default_currency = saashq.get_cached_value("Company", company, "default_currency")
	company_account_currency = saashq.get_cached_value("Account", company_account, "account_currency")
	second_account_currency = saashq.get_cached_value("Account", second_account, "account_currency")
...
def auto_reconcile_vouchers(
	...
	saashq.flags.auto_reconcile_vouchers = True
	...
	saashq.msgprint(title=_("Auto Reconciliation"), msg=alert_message, indicator=indicator)

	saashq.flags.auto_reconcile_vouchers = False
...
def reconcile_vouchers(bank_transaction_name, vouchers):
	...
	transaction = saashq.get_doc("Bank Transaction", bank_transaction_name)
...
def get_linked_payments(
	...
	transaction = saashq.get_doc("Bank Transaction", bank_transaction_name)
	bank_account = saashq.db.get_values(
...
def get_queries(
	...
	for method_name in saashq.get_hooks("get_matching_queries"):
		queries.extend(
			saashq.get_attr(method_name)(
				bank_account,
				company,
				transaction,
				document_types,
...
def get_bt_matching_query(exact_match, transaction):
	...
	bt = saashq.qb.DocType("Bank Transaction")

	field = "deposit" if transaction.withdrawal > 0.0 else "withdrawal"
	amount_equality = getattr(bt, field) == transaction.unallocated_amount
	amount_rank = saashq.qb.terms.Case().when(amount_equality, 1).else_(0)
	amount_condition = amount_equality if exact_match else getattr(bt, field) > 0.0

	ref_rank = saashq.qb.terms.Case().when(bt.reference_number == transaction.reference_number, 1).else_(0)
	unallocated_rank = (
		saashq.qb.terms.Case().when(bt.unallocated_amount == transaction.unallocated_amount, 1).else_(0)
	)

	party_condition = (
		(bt.party_type == transaction.party_type) & (bt.party == transaction.party) & bt.party.isnotnull()
	)
	party_rank = saashq.qb.terms.Case().when(party_condition, 1).else_(0)
...
def get_pe_matching_query(
	...
	payment_type = "Receive" if transaction.deposit > 0.0 else "Pay"
	pe = saashq.qb.DocType("Payment Entry")

	ref_condition = pe.reference_no == transaction.reference_number
	ref_rank = saashq.qb.terms.Case().when(ref_condition, 1).else_(0)
	...
	if saashq.flags.auto_reconcile_vouchers is True:
...
def get_je_matching_query(
	...
	je = saashq.qb.DocType("Journal Entry")
	jea = saashq.qb.DocType("Journal Entry Account")
...
def get_si_matching_query(exact_match, currency, common_filters):
	...
	si = saashq.qb.DocType("Sales Invoice")
	sip = saashq.qb.DocType("Sales Invoice Payment")
...
def get_pi_matching_query(exact_match, currency, common_filters):
	...
	query = (
		saashq.qb.from_(purchase_invoice)
		.select(
			(party_rank + amount_rank + 1).as_("rank"),
