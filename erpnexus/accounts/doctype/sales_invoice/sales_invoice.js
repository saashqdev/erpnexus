...
saashq.provide("erpnexus.accounts");

cur_frm.cscript.tax_table = "Sales Taxes and Charges";

erpnexus.accounts.taxes.setup_tax_validations("Sales Invoice");
erpnexus.accounts.payment_triggers.setup("Sales Invoice");
erpnexus.accounts.pos.setup("Sales Invoice");
erpnexus.accounts.taxes.setup_tax_filters("Sales Taxes and Charges");
erpnexus.sales_common.setup_selling_controller();
erpnexus.accounts.SalesInvoiceController = class SalesInvoiceController extends (
	erpnexus.selling.SellingController
) {
	setup(doc) {
	...
	company() {
		super.company();
		erpnexus.accounts.dimensions.update_dimension(this.frm, this.frm.doctype);
	}
	onload() {
		var me = this;
		super.onload();
		...
		erpnexus.queries.setup_queries(this.frm, "Warehouse", function () {
			return erpnexus.queries.warehouse(me.frm.doc);
		});
		...
		erpnexus.queries.setup_warehouse_query(this.frm);
	}
	...
	refresh(doc, dt, dn) {
		...
		erpnexus.accounts.ledger_preview.show_accounting_ledger_preview(this.frm);

		if (doc.update_stock) {
			this.show_stock_ledger();
			erpnexus.accounts.ledger_preview.show_stock_ledger_preview(this.frm);
		}
		...
		erpnexus.accounts.unreconcile_payment.add_unreconcile_btn(me.frm);
	}

	make_maintenance_schedule() {
		saashq.model.open_mapped_doc({
			method: "erpnexus.accounts.doctype.sales_invoice.sales_invoice.make_maintenance_schedule",
			frm: this.frm,
		});
	}
	...
	on_submit(doc, dt, dn) {
		...
		doc.items.forEach((row) => {
			if (row.delivery_note) saashq.model.clear_doc("Delivery Note", row.delivery_note);
		});
	}
		...
		this.$sales_order_btn = this.frm.add_custom_button(
			...
			function () {
				erpnexus.utils.map_current_doc({
					method: "erpnexus.selling.doctype.sales_order.sales_order.make_sales_invoice",
	...
	quotation_btn() {
		...
		this.$quotation_btn = this.frm.add_custom_button(
			...
			function () {
				erpnexus.utils.map_current_doc({
					method: "erpnexus.selling.doctype.quotation.quotation.make_sales_invoice",
			...
			function () {
				erpnexus.utils.map_current_doc({
					...
					get_query: function () {
						...
						return {
							query: "erpnexus.controllers.queries.get_delivery_notes_to_be_billed",
							filters: filters,
						};
					},
				});
	...
	customer() {
		...
		erpnexus.utils.get_party_details(
			this.frm,
			"erpnexus.accounts.party.get_party_details",
			{
		...
		if (this.frm.doc.customer) {
			saashq.call({
				method: "erpnexus.accounts.doctype.sales_invoice.sales_invoice.get_loyalty_programs",
				args: {
	...
	make_inter_company_invoice() {
		let me = this;
		saashq.model.open_mapped_doc({
			method: "erpnexus.accounts.doctype.sales_invoice.sales_invoice.make_inter_company_purchase_invoice",
			frm: me.frm,
		});
	}
	...
	write_off_outstanding_amount_automatically() {
		if (cint(this.frm.doc.write_off_outstanding_amount_automatically)) {
			saashq.model.round_floats_in(this.frm.doc, ["grand_total", "paid_amount"]);
	...
	make_sales_return() {
		saashq.model.open_mapped_doc({
			method: "erpnexus.accounts.doctype.sales_invoice.sales_invoice.make_sales_return",
			frm: this.frm,
		});
	}
	...
	asset(frm, cdt, cdn) {
		...
		if (row.asset) {
			saashq.call({
				method: erpnexus.assets.doctype.asset.depreciation.get_disposal_account_and_cost_center,
				args: {
					company: frm.doc.company,
				},
				callback: function (r, rt) {
					saashq.model.set_value(cdt, cdn, "income_account", r.message[0]);
					saashq.model.set_value(cdt, cdn, "cost_center", r.message[1]);
				},
			});
		}
	}
	...
	set_pos_data() {
		if (this.frm.doc.is_pos) {
			...
			if (!this.frm.doc.company) {
				...
				saashq.msgprint(__("Please specify Company to proceed"));
			} else {
				var me = this;
				...
				return this.frm.call({
					...
					callback: function (r) {
						if (!r.exc) {
							...
							saashq.model.set_default_values(me.frm.doc);
							me.set_dynamic_labels();
	...
	currency() {
		...
		if (this.frm.doc.timesheets) {
			this.frm.doc.timesheets.forEach((d) => {
				let row = saashq.get_doc(d.doctype, d.name);
...
extend_cscript(cur_frm.cscript, new erpnexus.accounts.SalesInvoiceController({ frm: cur_frm }));

cur_frm.cscript["Make Delivery Note"] = function () {
	saashq.model.open_mapped_doc({
		method: "erpnexus.accounts.doctype.sales_invoice.sales_invoice.make_delivery_note",
		frm: cur_frm,
	});
};

cur_frm.cscript.income_account = function (doc, cdt, cdn) {
	erpnexus.utils.copy_value_in_all_rows(doc, cdt, cdn, "items", "income_account");
};

cur_frm.cscript.expense_account = function (doc, cdt, cdn) {
	erpnexus.utils.copy_value_in_all_rows(doc, cdt, cdn, "items", "expense_account");
};

cur_frm.cscript.cost_center = function (doc, cdt, cdn) {
	erpnexus.utils.copy_value_in_all_rows(doc, cdt, cdn, "items", "cost_center");
};
...
saashq.ui.form.on("Sales Invoice", {
	setup: function (frm) {
		...
		frm.set_query("income_account", "items", function (doc) {
			return {
				query: "erpnexus.controllers.queries.get_income_account",
				filters: {
					company: doc.company,
					disabled: 0,
				},
		...
		frm.set_query("time_sheet", "timesheets", function (doc, cdt, cdn) {
			return {
				query: "erpnexus.projects.doctype.timesheet.timesheet.get_timesheet",
		...
		frm.set_query("company_address", function (doc) {
			if (!doc.company) {
				saashq.throw(__("Please set Company"));
			}

			return {
				query: "saashq.contacts.doctype.address.address.address_query",
				filters: {
		...
		frm.set_query("pos_profile", function (doc) {
			...
			return {
				query: "erpnexus.accounts.doctype.pos_profile.pos_profile.pos_profile_query",
				filters: {
					company: doc.company,
				},
			};
	...
	company: function (frm) {
		if (frm.doc.company) {
			saashq.call({
				method: "erpnexus.setup.doctype.company.company.get_default_company_address",
	...
	loyalty_points: function (frm) {
		if (frm.redemption_conversion_factor) {
			...
			saashq.call({
				method: "erpnexus.accounts.doctype.loyalty_program.loyalty_program.get_redeemption_factor",
				args: {
	...
	hide_fields: function (frm) {
		...
		if (cint(doc.is_pos) == 1) {
			...
			for (var i in parent_fields) {
				var docfield = saashq.meta.docfield_map[doc.doctype][parent_fields[i]];
	...
	get_loyalty_details: function (frm) {
		if (frm.doc.customer && frm.doc.redeem_loyalty_points) {
			saashq.call({
				method: "erpnexus.accounts.doctype.loyalty_program.loyalty_program.get_loyalty_program_details",
				args: {
	...
	set_loyalty_points: function (frm) {
		if (frm.redemption_conversion_factor) {
			...
			if (frm.doc.grand_total && remaining_amount < loyalty_amount) {
				...
				saashq.throw(__("You can only redeem max {0} points in this order.", [redeemable_points]));
			}
	...
	async get_timesheet_data(frm, kwargs) {
		return saashq
			.call({
				method: "erpnexus.projects.doctype.timesheet.timesheet.get_projectwise_timesheet_data",
				args: kwargs,
			})
			.then((r) => {
	...
	create_invoice_discounting: function (frm) {
		saashq.model.open_mapped_doc({
			method: "erpnexus.accounts.doctype.sales_invoice.sales_invoice.create_invoice_discounting",
			frm: frm,
		});
	},
	...
	create_dunning: function (frm) {
		saashq.model.open_mapped_doc({
			method: "erpnexus.accounts.doctype.sales_invoice.sales_invoice.create_dunning",
			frm: frm,
		});
	},
});

saashq.ui.form.on("Sales Invoice Timesheet", {
...
var set_timesheet_detail_rate = function (cdt, cdn, currency, timelog) {
	saashq.call({
		method: "erpnexus.projects.doctype.timesheet.timesheet.get_timesheet_detail_rate",
		args: {
			timelog: timelog,
			currency: currency,
		},
		callback: function (r) {
			if (!r.exc && r.message) {
				saashq.model.set_value(cdt, cdn, "billing_amount", r.message);
			}
		},
	});
};
