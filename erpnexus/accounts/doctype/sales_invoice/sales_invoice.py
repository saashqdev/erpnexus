...
from saashq.contacts.doctype.address.address import get_address_display
from saashq.model.mapper import get_mapped_doc
from saashq.model.utils import get_fetch_values
from saashq.utils import add_days, cint, cstr, flt, formatdate, get_link_to_form, getdate, nowdate
from saashq.utils.data import comma_and

import erpnexus
from erpnexus.accounts.deferred_revenue import validate_service_stop_date
...
from erpnexus.accounts.doctype.tax_withholding_category.tax_withholding_category import (
	get_party_tax_withholding_details,
)
from erpnexus.accounts.general_ledger import get_round_off_account_and_cost_center
from erpnexus.accounts.party import get_due_date, get_party_account, get_party_details
from erpnexus.accounts.utils import cancel_exchange_gain_loss_journal, get_account_currency
from erpnexus.assets.doctype.asset.depreciation import (
	depreciate_asset,
...
from erpnexus.assets.doctype.asset_activity.asset_activity import add_asset_activity
from erpnexus.controllers.accounts_controller import validate_account_head
from erpnexus.controllers.selling_controller import SellingController
from erpnexus.projects.doctype.timesheet.timesheet import get_projectwise_timesheet_data
from erpnexus.setup.doctype.company.company import update_company_current_month_sales
from erpnexus.stock.doctype.delivery_note.delivery_note import update_billed_amount_based_on_so
...
class SalesInvoice(SellingController):
	...
	if TYPE_CHECKING:
		...
		from erpnexus.accounts.doctype.payment_schedule.payment_schedule import PaymentSchedule
		from erpnexus.accounts.doctype.pricing_rule_detail.pricing_rule_detail import PricingRuleDetail
		from erpnexus.accounts.doctype.sales_invoice_advance.sales_invoice_advance import SalesInvoiceAdvance
		from erpnexus.accounts.doctype.sales_invoice_item.sales_invoice_item import SalesInvoiceItem
		from erpnexus.accounts.doctype.sales_invoice_payment.sales_invoice_payment import SalesInvoicePayment
		from erpnexus.accounts.doctype.sales_invoice_timesheet.sales_invoice_timesheet import (
			SalesInvoiceTimesheet,
		)
		from erpnexus.accounts.doctype.sales_taxes_and_charges.sales_taxes_and_charges import (
			SalesTaxesandCharges,
		)
		from erpnexus.selling.doctype.sales_team.sales_team import SalesTeam
		from erpnexus.stock.doctype.packed_item.packed_item import PackedItem
	...
	def validate_fixed_asset(self):
		for d in self.get("items"):
			if d.is_fixed_asset and d.meta.get_field("asset") and d.asset:
				asset = saashq.get_doc("Asset", d.asset)
				if self.doctype == "Sales Invoice" and self.docstatus == 1:
					if self.update_stock:
						saashq.throw(_("'Update Stock' cannot be checked for fixed asset sale"))
	...
	def on_submit(self):
		...
		if saashq.db.get_single_value("Selling Settings", "sales_update_frequency") == "Each Transaction":
		...
		if (
			...
			against_si_doc = saashq.get_doc("Sales Invoice", self.return_against)
	...
	def validate_pos_return(self):
		...
		if self.is_pos and self.is_return:
			...
			if total_amount_in_payments < invoice_total:
				saashq.throw(_("Total payments amount can't be greater than {}").format(-invoice_total))

	def validate_pos_paid_amount(self):
		if len(self.payments) == 0 and self.is_pos:
			saashq.throw(_("At least one mode of payment is required for POS invoice."))
	...
	def check_if_consolidated_invoice(self):
		...
		if self.doctype == "Sales Invoice" and self.is_consolidated:
			...
			pos_closing_entry = saashq.get_all(
			...
			if pos_closing_entry and pos_closing_entry[0]:
				msg = _("To cancel a {} you need to cancel the POS Closing Entry {}.").format(
					saashq.bold(_("Consolidated Sales Invoice")),
					get_link_to_form("POS Closing Entry", pos_closing_entry[0]),
				)
				saashq.throw(msg, title=_("Not Allowed"))
	...
	def check_credit_limit(self):
		from erpnexus.selling.doctype.customer.customer import check_credit_limit

		validate_against_credit_limit = False
		bypass_credit_limit_check_at_sales_order = saashq.db.get_value(
	...
	def unlink_sales_invoice_from_timesheets(self):
		for row in self.timesheets:
			timesheet = saashq.get_doc("Timesheet", row.time_sheet)
	...
	def set_missing_values(self, for_validate=False):
		...
		if not self.debit_to:
			...
			self.party_account_currency = saashq.db.get_value(
		...
		if not print_format and not cint(saashq.db.get_value("Print Format", "POS Invoice", "disabled")):
	...
	def update_time_sheet(self, sales_invoice):
		for d in self.timesheets:
			if d.time_sheet:
				timesheet = saashq.get_doc("Timesheet", d.time_sheet)
	...
	def validate_time_sheets_are_submitted(self):
		for data in self.timesheets:
			if data.time_sheet:
				status = saashq.db.get_value("Timesheet", data.time_sheet, "status")
				if status not in ["Submitted", "Payslip"]:
					saashq.throw(_("Timesheet {0} is already completed or cancelled").format(data.time_sheet))
	...
	def set_pos_fields(self, for_validate=False):
		...
		if not self.account_for_change_amount:
			self.account_for_change_amount = saashq.get_cached_value(
				"Company", self.company, "default_cash_account"
			)

		from erpnexus.stock.get_item_details import get_pos_profile, get_pos_profile_item_details
		...
		pos = {}
		if self.pos_profile:
			pos = saashq.get_doc("POS Profile", self.pos_profile)
		...
		if pos:
			...
			if self.customer:
				customer_price_list, customer_group = saashq.get_value(
					"Customer", self.customer, ["default_price_list", "customer_group"]
				)
				customer_group_price_list = saashq.get_value(
			...
			for item in self.get("items"):
				if item.get("item_code"):
					profile_details = get_pos_profile_item_details(
						pos, saashq._dict(item.as_dict()), pos, update_data=True
					)
			...
			if self.tc_name and not self.terms:
				self.terms = saashq.db.get_value("Terms and Conditions", self.tc_name, "terms")
	...
	def get_company_abbr(self):
		return saashq.db.sql("select abbr from tabCompany where name=%s", self.company)[0][0]
	...
	def validate_debit_to_acc(self):
		...
		account = saashq.get_cached_value(
			"Account", self.debit_to, ["account_type", "report_type", "account_currency"], as_dict=True
		)

		if not account:
			saashq.throw(_("Debit To is required"), title=_("Account Missing"))

		if account.report_type != "Balance Sheet":
			msg = (
				_("Please ensure {} account is a Balance Sheet account.").format(saashq.bold(_("Debit To")))
				+ " "
			)
			msg += _(
				"You can change the parent account to a Balance Sheet account or select a different account."
			)
			saashq.throw(msg, title=_("Invalid Account"))

		if self.customer and account.account_type != "Receivable":
			msg = (
				_("Please ensure {} account {} is a Receivable account.").format(
					saashq.bold(_("Debit To")), saashq.bold(self.debit_to)
				)
				+ " "
			)
	...
	def validate_with_previous_doc(self):
		...
		if (
			cint(saashq.db.get_single_value("Selling Settings", "maintain_same_sales_rate"))
	...
	def force_set_against_income_account(self):
		self.set_against_income_account()
		saashq.db.set_value(self.doctype, self.name, "against_income_account", self.against_income_account)
	...
	def so_dn_required(self):
		...
		for key, value in prev_doc_field_map.items():
			if saashq.db.get_single_value("Selling Settings", value[0]) == "Yes":
				if saashq.get_value("Customer", self.customer, value[0]):
					continue
	...
	def validate_pos(self):
		if self.is_return:
			...
			if flt(self.paid_amount) + flt(self.write_off_amount) - flt(invoice_total) > 1.0 / (
				...
				saashq.throw(_("Paid amount + Write Off Amount can not be greater than Grand Total"))
	...
	def validate_warehouse(self):
		...
		for d in self.get_item_list():
			if (
				not d.warehouse
				and d.item_code
				and saashq.get_cached_value("Item", d.item_code, "is_stock_item")
			):
				saashq.throw(_("Warehouse required for stock Item {0}").format(d.item_code))
	...
	def validate_write_off_account(self):
		if flt(self.write_off_amount) and not self.write_off_account:
			self.write_off_account = saashq.get_cached_value("Company", self.company, "write_off_account")
	...
	def set_billing_hours_and_amount(self):
		if not self.project:
			for timesheet in self.timesheets:
				ts_doc = saashq.get_doc("Timesheet", timesheet.time_sheet)
	...
	def get_warehouse(self):
		user_pos_profile = saashq.db.sql(
			"""select name, warehouse from `tabPOS Profile`
			where ifnull(user,'') = %s and company = %s""",
			(saashq.session["user"], self.company),
		)
		warehouse = user_pos_profile[0][1] if user_pos_profile else None

		if not warehouse:
			global_pos_profile = saashq.db.sql(
				"""select name, warehouse from `tabPOS Profile`
	...
	def check_prev_docstatus(self):
		for d in self.get("items"):
			if (
				d.sales_order
				and saashq.db.get_value("Sales Order", d.sales_order, "docstatus", cache=True) != 1
			):
				saashq.throw(_("Sales Order {0} is not submitted").format(d.sales_order))

			if (
				d.delivery_note
				and saashq.db.get_value("Delivery Note", d.delivery_note, "docstatus", cache=True) != 1
			):
				throw(_("Delivery Note {0} is not submitted").format(d.delivery_note))

	def make_gl_entries(self, gl_entries=None, from_repost=False):
		from erpnexus.accounts.general_ledger import make_gl_entries, make_reverse_gl_entries

		auto_accounting_for_stock = erpnexus.is_perpetual_inventory_enabled(self.company)
		if not gl_entries:
		...
		if gl_entries:
			...
			if self.docstatus == 1:
				...
				cancel_exchange_gain_loss_journal(saashq._dict(doctype=self.doctype, name=self.name))
				make_reverse_gl_entries(voucher_type=self.doctype, voucher_no=self.name)

			if update_outstanding == "No":
				from erpnexus.accounts.doctype.gl_entry.gl_entry import update_outstanding_amt
	...
	def get_gl_entries(self, warehouse_account=None):
		from erpnexus.accounts.general_ledger import merge_similar_entries
	...
	def make_item_gl_entries(self, gl_entries):
				...
				if item.is_fixed_asset:
					...
					if self.is_return:
						...
						if asset.calculate_depreciation:
							posting_date = saashq.db.get_value(
		...
		if cint(self.update_stock) and erpnexus.is_perpetual_inventory_enabled(self.company):
			gl_entries += super().get_gl_entries()

	def get_asset(self, item):
		if item.get("asset"):
			asset = saashq.get_doc("Asset", item.asset)
		else:
			saashq.throw(
	...
	def make_pos_gl_entries(self, gl_entries):
		if cint(self.is_pos):
			skip_change_gl_entries = not cint(
				saashq.db.get_single_value("Accounts Settings", "post_change_gl_entries")
			)
	...
	def get_gle_for_change_amount(self) -> list[dict]:
		...
		if not self.account_for_change_amount:
			saashq.throw(_("Please set Account for Change Amount"), title=_("Mandatory Field"))
	...
	def make_write_off_gl_entry(self, gl_entries):
		...
		if (
			...
			default_cost_center = saashq.get_cached_value("Company", self.company, "cost_center")
				...
				if not round_off_for_opening:
					saashq.throw(
						_(
							...
							saashq.bold(self.rounding_adjustment),
							saashq.bold("Round Off for Opening"),
							get_link_to_form("Company", self.company),
							saashq.bold("Disable Rounded Total"),
						)
					)
				else:
	...
	def update_billing_status_in_dn(self, update_modified=True):
		...
		for dn in set(updated_delivery_notes):
			saashq.get_doc("Delivery Note", dn).update_billing_percentage(update_modified=update_modified)
	...
	def update_project(self):
		...
		for p in unique_projects:
			project = saashq.get_doc("Project", p)
	...
	def verify_payment_amount_is_positive(self):
		for entry in self.payments:
			if entry.amount < 0:
				saashq.throw(_("Row #{0} (Payment Table): Amount must be positive").format(entry.idx))

	def verify_payment_amount_is_negative(self):
		for entry in self.payments:
			if entry.amount > 0:
				saashq.throw(_("Row #{0} (Payment Table): Amount must be negative").format(entry.idx))
	...
	def delete_loyalty_point_entry(self):
		...
		if against_lp_entry:
			...
			saashq.db.sql("""delete from `tabLoyalty Point Entry` where invoice=%s""", (self.name))
			# Set loyalty program
			self.set_loyalty_program_tier()
	...
	def apply_loyalty_points(self):
		from erpnexus.accounts.doctype.loyalty_point_entry.loyalty_point_entry import (
	...
	def set_status(self, update=False, status=None, update_modified=True):
		...
		if not status:
			if self.docstatus == 2:
				...
				elif self.is_return == 0 and saashq.db.get_value(
...
def get_discounting_status(sales_invoice):
	...
	invoice_discounting_list = saashq.db.sql(
		"""
		select status
...
def validate_inter_company_party(doctype, party, company, inter_company_reference):
	...
	if inter_company_reference:
		doc = saashq.get_doc(ref_doc, inter_company_reference)
		ref_party = doc.supplier if doctype in ["Sales Invoice", "Sales Order"] else doc.customer
		if saashq.db.get_value(partytype, {"represents_company": doc.company}, "name") != party:
			saashq.throw(_("Invalid {0} for Inter Company Transaction.").format(_(partytype)))
		if saashq.get_cached_value(ref_partytype, ref_party, "represents_company") != company:
			saashq.throw(_("Invalid Company for Inter Company Transaction."))

	elif saashq.db.get_value(partytype, {"name": party, internal: 1}, "name") == party:
		companies = saashq.get_all(
...
def update_linked_doc(doctype, name, inter_company_reference):
	...
	if inter_company_reference:
		saashq.db.set_value(doctype, inter_company_reference, ref_field, name)
...
def unlink_inter_company_doc(doctype, name, inter_company_reference):
	...
	if inter_company_reference:
		saashq.db.set_value(doctype, name, ref_field, "")
		saashq.db.set_value(ref_doc, inter_company_reference, ref_field, "")


def get_list_context(context=None):
	from erpnexus.controllers.website_list_for_contact import get_list_context
...
def get_inter_company_details(doc, doctype):
	if doctype in ["Sales Invoice", "Sales Order", "Delivery Note"]:
		...
		company = saashq.get_cached_value("Supplier", doc.supplier, "represents_company")
...
def validate_inter_company_transaction(doc, doctype):
	...
	valid_price_list = saashq.db.get_value("Price List", {"name": price_list, "buying": 1, "selling": 1})
	if not valid_price_list and not doc.is_internal_transfer():
		saashq.throw(_("Selected Price List should have buying and selling fields checked."))

	party = details.get("party")
	if not party:
		partytype = "Supplier" if doctype in ["Sales Invoice", "Sales Order"] else "Customer"
		saashq.throw(_("No {0} found for Inter Company Transactions.").format(partytype))

	company = details.get("company")
	default_currency = saashq.get_cached_value("Company", company, "default_currency")
...
def make_inter_company_transaction(doctype, source_name, target_doc=None):
	if doctype in ["Sales Invoice", "Sales Order"]:
		source_doc = saashq.get_doc(doctype, source_name)
	...
	def update_details(source_doc, target_doc, source_parent):
		...
		if target_doc.doctype in ["Purchase Invoice", "Purchase Order"]:
			currency = saashq.db.get_value("Supplier", details.get("party"), "default_currency")
			...
			currency = saashq.db.get_value("Customer", details.get("party"), "default_currency")
			target_doc.company = details.get("company")
...
def get_received_items(reference_name, doctype, reference_fieldname):
	...
	target_doctypes = saashq.get_all(
		doctype,
		filters=filters,
		as_list=True,
	)
	...
	received_items_map = saashq._dict(
		saashq.get_all(
			doctype + " Item",
	...
	for item in doc.get("items"):
		...
		if doc.update_stock:
			...
			if not item.warehouse and item.get("purchase_order") and item.get("purchase_order_item"):
				item.warehouse = saashq.db.get_value(
...
def update_pr_items(doc, sales_item_map, purchase_item_map, parent_child_map, warehouse_map):
	for item in doc.get("items"):
		...
		if not item.warehouse and item.get("purchase_order") and item.get("purchase_order_item"):
			item.warehouse = saashq.db.get_value("Purchase Order Item", item.purchase_order_item, "warehouse")


def get_delivery_note_details(internal_reference):
	si_item_details = saashq.get_all(
...
def get_pd_details(doctype, sd_detail_map, sd_detail_field):
	...
	pd_item_details = saashq.get_all(
		doctype,
...
def get_loyalty_programs(customer):
	...
	from erpnexus.selling.doctype.customer.customer import get_loyalty_programs

	customer = saashq.get_doc("Customer", customer)
...
def create_invoice_discounting(source_name, target_doc=None):
	invoice = saashq.get_doc("Sales Invoice", source_name)
	invoice_discounting = saashq.new_doc("Invoice Discounting")
...
def create_dunning(source_name, target_doc=None, ignore_permissions=False):
	...
	def postprocess_dunning(source, target):
		from erpnexus.accounts.doctype.dunning.dunning import get_dunning_letter_text

		dunning_type = saashq.db.exists("Dunning Type", {"is_default": 1, "company": source.company})
		if dunning_type:
			dunning_type = saashq.get_doc("Dunning Type", dunning_type)
...
def check_if_return_invoice_linked_with_payment_entry(self):
	...
	if not saashq.db.get_single_value("Accounts Settings", "unlink_payment_on_cancellation_of_invoice"):
		return
	...
	payment_entries = saashq.db.sql_list(
		"""
		SELECT
			t1.name
		FROM
	...
	if payment_entries:
		for payment in payment_entries:
			payment_entry = saashq.get_doc("Payment Entry", payment)
