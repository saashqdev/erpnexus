...
import saashq
from saashq import qb
from saashq.model.dynamic_links import get_dynamic_link_map
from saashq.tests import IntegrationTestCase, UnitTestCase
from saashq.utils import add_days, flt, format_date, getdate, nowdate, today

import erpnexus
from erpnexus.accounts.doctype.account.test_account import create_account, get_inventory_account
from erpnexus.accounts.doctype.pos_profile.test_pos_profile import make_pos_profile
from erpnexus.accounts.doctype.purchase_invoice.purchase_invoice import WarehouseMissingError
from erpnexus.accounts.doctype.purchase_invoice.test_purchase_invoice import (
	unlink_payment_on_cancel_of_invoice,
)
from erpnexus.accounts.doctype.sales_invoice.sales_invoice import make_inter_company_transaction
from erpnexus.accounts.utils import PaymentEntryUnlinkError
from erpnexus.assets.doctype.asset.depreciation import post_depreciation_entries
from erpnexus.assets.doctype.asset.test_asset import create_asset, create_asset_data
from erpnexus.assets.doctype.asset_depreciation_schedule.asset_depreciation_schedule import (
	get_depr_schedule,
)
from erpnexus.controllers.accounts_controller import InvalidQtyError, update_invoice_status
from erpnexus.controllers.taxes_and_totals import get_itemised_tax_breakup_data
from erpnexus.exceptions import InvalidAccountCurrency, InvalidCurrency
from erpnexus.selling.doctype.customer.test_customer import get_customer_dict
from erpnexus.stock.doctype.delivery_note.delivery_note import make_sales_invoice
from erpnexus.stock.doctype.item.test_item import create_item
from erpnexus.stock.doctype.purchase_receipt.test_purchase_receipt import make_purchase_receipt
from erpnexus.stock.doctype.serial_and_batch_bundle.test_serial_and_batch_bundle import (
	get_batch_from_bundle,
	get_serial_nos_from_bundle,
	make_serial_batch_bundle,
)
from erpnexus.stock.doctype.stock_entry.test_stock_entry import (
	get_qty_after_transaction,
	make_stock_entry,
)
from erpnexus.stock.doctype.stock_reconciliation.test_stock_reconciliation import (
	create_stock_reconciliation,
)
from erpnexus.stock.utils import get_incoming_rate, get_stock_balance
...
class TestSalesInvoice(IntegrationTestCase):
	def setUp(self):
		from erpnexus.stock.doctype.stock_ledger_entry.test_stock_ledger_entry import create_items
	...
	def make(self):
		w = saashq.copy_doc(self.globalTestRecords["Sales Invoice"][0])
		w.is_pos = 0
		w.insert()
		w.submit()
		return w
	...
	def test_timestamp_change(self):
		...
		w2 = saashq.get_doc(w.doctype, w.name)
		...
		time.sleep(1)
		self.assertRaises(saashq.TimestampMismatchError, w2.save)

	def test_sales_invoice_change_naming_series(self):
		si = saashq.copy_doc(self.globalTestRecords["Sales Invoice"][2])
		si.insert()
		si.naming_series = "TEST-"

		self.assertRaises(saashq.CannotChangeConstantError, si.save)

		si = saashq.copy_doc(self.globalTestRecords["Sales Invoice"][1])
		si.insert()
		si.naming_series = "TEST-"
	...
	def test_payment_entry_unlink_against_invoice(self):
		from erpnexus.accounts.doctype.payment_entry.test_payment_entry import get_payment_entry

		si = saashq.copy_doc(self.globalTestRecords["Sales Invoice"][0])
		si.is_pos = 0
		si.insert()
		si.submit()
		...
		unlink_payment_on_cancel_of_invoice(0)
		si = saashq.get_doc("Sales Invoice", si.name)
		self.assertRaises(saashq.LinkExistsError, si.cancel)
		unlink_payment_on_cancel_of_invoice()
	...
	def test_sales_invoice_discount_amount(self):
		si = saashq.copy_doc(self.globalTestRecords["Sales Invoice"][3])
	...
	def test_discount_amount_gl_entry(self):
		saashq.db.set_value("Company", "_Test Company", "round_off_account", "Round Off - _TC")
		si = saashq.copy_doc(self.globalTestRecords["Sales Invoice"][3])
		si.discount_amount = 104.94
		si.append(
			"taxes",
	...
	def test_inclusive_rate_validations(self):
		...
		self.assertRaises(saashq.ValidationError, si.insert)
	...
	def test_sales_invoice_calculation_export_currency_with_tax_inclusive_price(self):
		...
		if saashq.db.get_value("Currency", "USD", "smallest_currency_fraction_value") < 0.01:
	...
	def test_payment(self):
		w = self.make()
		jv = saashq.get_doc(saashq.copy_doc(self.globalTestRecords["Journal Entry"][0]))
		jv.get("accounts")[0].reference_type = w.doctype
		jv.get("accounts")[0].reference_name = w.name
		jv.insert()
		jv.submit()

		self.assertEqual(saashq.db.get_value("Sales Invoice", w.name, "outstanding_amount"), 162.0)
		...
		jv.cancel()
		self.assertEqual(saashq.db.get_value("Sales Invoice", w.name, "outstanding_amount"), 562.0)

	def test_outstanding_on_cost_center_allocation(self):
		# setup cost centers
		from erpnexus.accounts.doctype.cost_center.test_cost_center import create_cost_center
		from erpnexus.accounts.doctype.cost_center_allocation.test_cost_center_allocation import (
	...
	def test_pos_returns_with_repayment(self):
		from erpnexus.accounts.doctype.sales_invoice.sales_invoice import make_sales_return
	...
	def test_pos_with_no_gl_entry_for_change_amount(self):
		saashq.db.set_single_value("Accounts Settings", "post_change_gl_entries", 0)
		...
		saashq.db.set_single_value("Accounts Settings", "post_change_gl_entries", 1)
	...
	def validate_pos_gl_entry(self, si, pos, cash_amount, validate_without_change_gle=False):
		...
		saashq.db.sql("delete from `tabPOS Profile`")

	def test_bin_details_of_packed_item(self):
		from erpnexus.selling.doctype.product_bundle.test_product_bundle import make_product_bundle
		from erpnexus.stock.doctype.item.test_item import make_item

		# test Update Items with product bundle
		if not saashq.db.exists("Item", "_Test Product Bundle Item New"):
		...
		bin_details = saashq.db.get_value(
			"Bin",
	...
	def test_pos_si_without_payment(self):
		...
		self.assertRaises(saashq.ValidationError, si.submit)
	...
	def _insert_purchase_receipt(self):
		pr = saashq.copy_doc(self.globalTestRecords["Purchase Receipt"][0])
		pr.naming_series = "_T-Purchase Receipt-"
		pr.insert()
		pr.submit()

	def _insert_delivery_note(self):
		dn = saashq.copy_doc(self.globalTestRecords["Delivery Note"][0])
	...
	def test_sales_invoice_with_advance(self):
		jv = saashq.copy_doc(self.globalTestRecords["Journal Entry"][0])
		jv.insert()
		jv.submit()
	...
	def test_serialized(self):
		from erpnexus.stock.doctype.stock_entry.test_stock_entry import make_serialized_item
		...
		self.assertFalse(saashq.db.get_value("Serial No", serial_nos[0], "warehouse"))
	...
	def test_serialized_cancel(self):
		...
		self.assertEqual(
			saashq.db.get_value("Serial No", serial_nos[0], "warehouse"), "_Test Warehouse - _TC"
		)
	...
	def test_serial_numbers_against_delivery_note(self):
		...
		"""
		from erpnexus.stock.doctype.delivery_note.test_delivery_note import create_delivery_note
		from erpnexus.stock.doctype.stock_entry.test_stock_entry import make_serialized_item
	...
	def test_return_sales_invoice(self):
		...
		incoming_rate, stock_value_difference = saashq.db.get_value(
			"Stock Ledger Entry",
		...
		# Check gl entry
		gle_warehouse_amount = saashq.db.get_value(
			"GL Entry",
		...
		party_credited = saashq.db.get_value(
			"GL Entry",
			{
		...
		self.assertEqual(saashq.db.get_value("Sales Invoice", si1.name, "outstanding_amount"), -1000)
		self.assertEqual(saashq.db.get_value("Sales Invoice", si.name, "outstanding_amount"), 2500)
	...
	def test_zero_qty_return_invoice_with_stock_effect(self):
		...
		self.assertRaises(saashq.ValidationError, cr_note.save)
	...
	def test_gle_made_when_asset_is_returned(self):
		...
		disposal_account = saashq.get_cached_value("Company", "_Test Company", "disposal_account")

		# Asset value is 100,000 but it was sold for 90,000, so there should be a loss of 10,000
		loss_for_si = saashq.get_all(
			"GL Entry",
			filters={"voucher_no": si.name, "account": disposal_account},
			fields=["credit", "debit"],
		)[0]

		loss_for_return_si = saashq.get_all(
			"GL Entry",
	...
	def test_incoming_rate_for_stand_alone_credit_note(self):
		...
		incoming_rate = saashq.db.get_value(
			"Stock Ledger Entry", {"voucher_no": return_si.name}, "incoming_rate"
		)
		debit_amount = saashq.db.get_value(
	...
	def test_invoice_exchange_rate(self):
		...
		self.assertRaises(saashq.ValidationError, si.save)
	...
	def test_outstanding_amount_after_advance_jv_cancellation(self):
		...
		jv = saashq.get_doc("Journal Entry", jv.name)
		jv.cancel()
	...
	def test_multiple_uom_in_selling(self):
		...
		item_price = saashq.new_doc("Item Price")
	...
	def test_item_wise_tax_breakup(self):
		saashq.flags.country = "United States"
	...
	def test_company_monthly_sales(self):
		existing_current_month_sales = saashq.get_cached_value(
			"Company", "_Test Company", "total_monthly_sales"
		)

		si = create_sales_invoice()
		current_month_sales = saashq.get_cached_value("Company", "_Test Company", "total_monthly_sales")
	...
	def test_rounding_adjustment_2(self):
		...
		round_off_account = saashq.get_cached_value("Company", "_Test Company", "round_off_account")
		expected_values = {
	...
	def test_rounding_adjustment_3(self):
		from erpnexus.accounts.doctype.accounting_dimension.test_accounting_dimension import (
			create_dimension,
			disable_dimension,
		)
		...
		round_off_gle = saashq.db.get_value(
			"GL Entry",
	...
	def test_sales_invoice_with_shipping_rule(self):
		from erpnexus.accounts.doctype.shipping_rule.test_shipping_rule import create_shipping_rule
	...
	def test_credit_note(self):
		...
		si_doc = saashq.get_doc("Sales Invoice", si.name)
	...
	def test_sales_invoice_with_project_link(self):
		from erpnexus.projects.doctype.project.test_project import make_project
	...
	def test_fixed_deferred_revenue(self):
		...
		acc_settings = saashq.get_doc("Accounts Settings", "Accounts Settings")
	...
	def test_inter_company_transaction_without_default_warehouse(self):
		...
		# setup
		old_negative_stock = saashq.db.get_single_value("Stock Settings", "allow_negative_stock")
		saashq.db.set_single_value("Stock Settings", "allow_negative_stock", 1)

		old_perpetual_inventory = erpnexus.is_perpetual_inventory_enabled("_Test Company 1")
		saashq.local.enable_perpetual_inventory["_Test Company 1"] = 1
		...
		# tear down
		saashq.local.enable_perpetual_inventory["_Test Company 1"] = old_perpetual_inventory
		saashq.db.set_single_value("Stock Settings", "allow_negative_stock", old_negative_stock)
	...
	def test_internal_transfer_gl_precision_issues(self):
		...
		if get_stock_balance("_Test Internal Transfer Item", "Stores - TCP1", "2022-04-10"):
			create_stock_reconciliation(
				...
				if saashq.get_all("Stock Ledger Entry")
	...
	def test_additional_discount_for_sales_invoice_with_discount_accounting_enabled(self):
		from erpnexus.accounts.doctype.repost_accounting_ledger.test_repost_accounting_ledger import (
	...
	def test_depreciation_on_return_of_sold_asset(self):
		from erpnexus.controllers.sales_and_purchase_return import make_return_doc
	...
	def test_sales_invoice_against_supplier(self):
		from erpnexus.accounts.doctype.opening_invoice_creation_tool.test_opening_invoice_creation_tool import (
			make_customer,
		)
		from erpnexus.accounts.doctype.party_link.party_link import create_party_link
		from erpnexus.buying.doctype.supplier.test_supplier import create_supplier
		...
		saashq.db.set_single_value("Accounts Settings", "enable_common_party_accounting", 1)
		...
		party_link.delete()
		saashq.db.set_single_value("Accounts Settings", "enable_common_party_accounting", 0)
	...
	def test_sales_invoice_against_supplier_usd_with_dimensions(self):
		...
		cust_doc = saashq.get_doc("Customer", customer)
		cust_doc.default_currency = "USD"
		cust_doc.save()
		# create a supplier
		supplier = create_supplier(supplier_name="_Test Common Supplier USD").name
		supp_doc = saashq.get_doc("Supplier", supplier)
		...
		if not saashq.get_all("Accounting Dimension", filters={"document_type": "Department"}):
			dim = saashq.get_doc(
				{
			...
			dim = saashq.get_doc(
				...
				saashq.get_all("Accounting Dimension", filters={"document_type": "Department"})[0],
			)
			dim.disabled = False
	...
	def test_sales_invoice_cancel_with_common_party_advance_jv(self):
		...
		jv_status = saashq.db.get_value("Journal Entry", jv, "docstatus")
	...
	def test_sales_commission(self):
		...
		saashq.db.set_value("Item", si.get("items")[0].item_code, "grant_commission", 1)
		saashq.db.set_value("Item", si.get("items")[1].item_code, "grant_commission", 0)
	...
	def test_over_billing_case_against_delivery_note(self):
		...
		over_billing_allowance = saashq.db.get_single_value("Accounts Settings", "over_billing_allowance")
		saashq.db.set_single_value("Accounts Settings", "over_billing_allowance", 0)
		...
		item_copy = saashq.copy_doc(si.items[0])
		si.append("items", item_copy)
		si.save()

		with self.assertRaises(saashq.ValidationError) as err:
			si.submit()

		self.assertTrue("cannot overbill" in str(err.exception).lower())

		saashq.db.set_single_value("Accounts Settings", "over_billing_allowance", over_billing_allowance)
	...
	def test_multi_currency_deferred_revenue_via_journal_entry(self):
		...
		saashq.db.set_single_value("Accounts Settings", "acc_frozen_upto", getdate("2019-01-31"))
	...
	def test_sales_invoice_with_disabled_account(self):
		try:
			...
			account = saashq.get_doc("Account", account_name)
			account.disabled = 1
			account.save()
	...
	def test_gain_loss_with_advance_entry(self):
		from erpnexus.accounts.doctype.journal_entry.test_journal_entry import make_journal_entry
		...
		je_type = saashq.get_cached_value("Journal Entry", journals[0], "voucher_type")
	...
	def test_batch_expiry_for_sales_invoice_return(self):
		...
		saashq.db.set_value("Batch", batch_no, "expiry_date", add_days(today(), -1))
	...
	def test_advance_entries_as_liability(self):
		from erpnexus.accounts.doctype.payment_entry.test_payment_entry import create_payment_entry
	...
	def test_sales_invoice_without_customer_group_and_territory(self):
		# create a customer
		if not saashq.db.exists("Customer", "_Test Simple Customer"):
			customer_dict = get_customer_dict("_Test Simple Customer")
			customer_dict.pop("customer_group")
			customer_dict.pop("territory")
			customer = saashq.get_doc(customer_dict).insert(ignore_permissions=True)
	...
	def test_partial_allocation_on_advance_as_liability(self):
		...
		pr = saashq.get_doc("Payment Reconciliation")
		pr.company = company
		...
		pr.allocate_entries(saashq._dict({"invoices": invoices, "payments": payments}))
	...
	def test_loyalty_points_redemption_with_shopping_cart(self):
		from erpnexus.accounts.doctype.loyalty_program.test_loyalty_program import (
			create_records,
			create_sales_invoice_record,
		)
		from erpnexus.selling.doctype.sales_order.sales_order import make_sales_invoice
		from erpnexus.selling.doctype.sales_order.test_sales_order import make_sales_order

		# Set up loyalty program
		create_records()
		saashq.db.set_value("Customer", "Test Loyalty Customer", "loyalty_program", "Test Single Loyalty")
		...
		from saashq.model.trace import traced_field_context
		...
		loyalty_account = saashq.db.get_value("Loyalty Program", "Test Single Loyalty", "expense_account")
		expected_gl_entries = [
	...
	def test_taxes_merging_from_delivery_note(self):
		...
		from saashq.model.mapper import map_docs

		map_docs(
			method="erpnexus.stock.doctype.delivery_note.delivery_note.make_sales_invoice",
	...
	def _create_opening_roundoff_account(self, company_name):
		liability_root = saashq.db.get_all(
			"Account",
		...
		if acc := saashq.db.exists(
			...
			saashq.db.set_value("Company", company_name, "round_off_for_opening", acc)
		else:
			...
			acc.save()
			saashq.db.set_value("Company", company_name, "round_off_for_opening", acc.name)
	...
	def test_common_party_with_foreign_currency_jv(self):
		from erpnexus.accounts.doctype.account.test_account import create_account
		from erpnexus.accounts.doctype.opening_invoice_creation_tool.test_opening_invoice_creation_tool import (
			make_customer,
		)
		from erpnexus.accounts.doctype.party_link.party_link import create_party_link
...
def create_sales_invoice(**args):
	si = saashq.new_doc("Sales Invoice")
	...
	if si.update_stock and (args.get("batch_no") or args.get("serial_no")):
		...
		if args.get("batch_no"):
			batches = saashq._dict({args.batch_no: qty})
...
def create_internal_parties():
	from erpnexus.selling.doctype.customer.test_customer import create_internal_customer
	...
	saashq.db.set_value("Company", "_Test Company", "unrealized_profit_loss_account", account)
...
def create_internal_supplier(supplier_name, represents_company, allowed_to_interact_with):
	if not saashq.db.exists("Supplier", supplier_name):
		...
		supplier_name = saashq.db.exists("Supplier", supplier_name)
