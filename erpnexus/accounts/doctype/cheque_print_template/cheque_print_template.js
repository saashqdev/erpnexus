...
saashq.provide("erpnexus.cheque_print");

saashq.ui.form.on("Cheque Print Template", {
	refresh: function (frm) {
		if (!frm.doc.__islocal) {
			frm.add_custom_button(
				frm.doc.has_print_format ? __("Update Print Format") : __("Create Print Format"),
				function () {
					erpnexus.cheque_print.view_cheque_print(frm);
				}
			...
			var template =
				...
				<div id="cheque_preview" style="width: {{ cheque_width }}cm; \
					...
					<span style="top: {{ date_dist_from_top_edge }}cm;\
						...
						position: absolute;"> {{ saashq.datetime.obj_to_user() }} </span>\
			...
			$(saashq.render(template, frm.doc)).appendTo(frm.fields_dict.cheque_print_preview.wrapper);
...
erpnexus.cheque_print.view_cheque_print = function (frm) {
	saashq.call({
		method: "erpnexus.accounts.doctype.cheque_print_template.cheque_print_template.create_or_update_cheque_print_format",
		args: {
		...
		callback: function (r) {
			if (!r.exe && !frm.doc.has_print_format) {
				...
				saashq.msgprint(__("Print settings updated in respective print format"));
			}
		},
	});
};
