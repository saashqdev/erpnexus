...
from erpnexus.setup.doctype.customer_group.customer_group import get_parent_customer_groups


class IncorrectCustomerGroup(saashq.ValidationError):
	pass


class IncorrectSupplierType(saashq.ValidationError):
	pass


class ConflictingTaxRule(saashq.ValidationError):
	pass
...
class TaxRule(Document):
	...
	def validate_tax_template(self):
		...
		if not (self.sales_tax_template or self.purchase_tax_template):
			saashq.throw(_("Tax Template is mandatory."))
	...
	def validate_filters(self):
		...
		for d in filters:
			if conds:
				conds += " and "
			conds += f"""ifnull({d}, '') = {saashq.db.escape(cstr(filters[d]))}"""
		...
		if tax_rule:
			if tax_rule[0].priority == self.priority:
				saashq.throw(_("Tax Rule Conflicts with {0}").format(tax_rule[0].name), ConflictingTaxRule)
...
def get_party_details(party, party_type, args=None):
	...
	if args:
		if args.get("billing_address"):
			billing_address = saashq.get_doc("Address", args.get("billing_address"))
		if args.get("shipping_address"):
			shipping_address = saashq.get_doc("Address", args.get("shipping_address"))
	else:
		...
		if billing_address_name:
			billing_address = saashq.get_doc("Address", billing_address_name)
		if shipping_address_name:
			shipping_address = saashq.get_doc("Address", shipping_address_name)
...
def get_tax_template(posting_date, args):
	...
	conditions.append(
		"ifnull(tax_category, '') = {}".format(saashq.db.escape(cstr(args.get("tax_category")), False))
	)
	...
	for key, value in args.items():
		if key == "use_for_shopping_cart":
			...
			conditions.append(f"ifnull({key}, '') in ('', {saashq.db.escape(cstr(value))})")
	...
	if saashq.db.get_value(doctype, tax_template, "disabled") == 1:
		return None
...
def get_customer_group_condition(customer_group):
	condition = ""
	customer_groups = ["%s" % (saashq.db.escape(d.name)) for d in get_parent_customer_groups(customer_group)]
	if customer_groups:
