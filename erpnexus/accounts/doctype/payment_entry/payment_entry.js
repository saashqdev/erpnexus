...
erpnexus.accounts.taxes.setup_tax_validations("Payment Entry");
erpnexus.accounts.taxes.setup_tax_filters("Advance Taxes and Charges");

saashq.ui.form.on("Payment Entry", {
	onload: function (frm) {
	...
	setup: function (frm) {
		frm.set_query("paid_from", function () {
			...
			var account_types = ["Pay", "Internal Transfer"].includes(frm.doc.payment_type)
				? ["Bank", "Cash"]
				: [saashq.boot.party_account_types[frm.doc.party_type]];
		...
		frm.set_query("party_type", function () {
			...
			return {
				filters: {
					name: ["in", Object.keys(saashq.boot.party_account_types)],
				},
			};
		});
		...
		frm.set_query("contact_person", function () {
			if (frm.doc.party) {
				return {
					query: "saashq.contacts.doctype.contact.contact.contact_query",
					filters: {
		...
		frm.set_query("payment_term", "references", function (frm, cdt, cdn) {
			...
			if (
				...
				return {
					query: "erpnexus.controllers.queries.get_payment_terms_for_references",
					filters: {
		...
		frm.set_query("payment_request", "references", function (doc, cdt, cdn) {
			const row = saashq.get_doc(cdt, cdn);
			return {
				query: "erpnexus.accounts.doctype.payment_request.payment_request.get_open_payment_requests_query",
				filters: {
	...
	refresh: function (frm) {
		erpnexus.hide_company(frm);
		frm.events.hide_unhide_fields(frm);
		frm.events.set_dynamic_labels(frm);
		frm.events.show_general_ledger(frm);
		erpnexus.accounts.ledger_preview.show_accounting_ledger_preview(frm);
		if (
			frm.doc.references &&
			...
			frm.add_custom_button(
				...
				function () {
					saashq.set_route("List", "Journal Entry", {
		...
		erpnexus.accounts.unreconcile_payment.add_unreconcile_btn(frm);
		saashq.flags.allocate_payment_amount = true;
	},
	...
	hide_unhide_fields: function (frm) {
		var company_currency = frm.doc.company
			? saashq.get_doc(":Company", frm.doc.company)?.default_currency
			: "";
	...
	mode_of_payment: function (frm) {
		erpnexus.accounts.pos.get_payment_mode_account(frm, frm.doc.mode_of_payment, function (account) {
	...
	party_type: function (frm) {
		let party_types = Object.keys(saashq.boot.party_account_types);
		if (frm.doc.party_type && !party_types.includes(frm.doc.party_type)) {
			frm.set_value("party_type", "");
			saashq.throw(__("Party can only be one of {0}", [party_types.join(", ")]));
		}

		frm.set_query("party", function () {
			if (frm.doc.party_type == "Employee") {
				return {
					query: "erpnexus.controllers.queries.employee_query",
				};
	...
	party: function (frm) {
		...
		if (frm.doc.payment_type && frm.doc.party_type && frm.doc.party && frm.doc.company) {
			if (!frm.doc.posting_date) {
				saashq.msgprint(__("Please select Posting Date before selecting Party"));
				frm.set_value("party", "");
				return;
			}
			frm.set_party_account_based_on_party = true;

			let company_currency = saashq.get_doc(":Company", frm.doc.company).default_currency;

			return saashq.call({
				method: "erpnexus.accounts.doctype.payment_entry.payment_entry.get_party_details",
				args: {
					company: frm.doc.company,
					party_type: frm.doc.party_type,
					party: frm.doc.party,
	...
	apply_tax_withholding_amount: function (frm) {
		if (!frm.doc.apply_tax_withholding_amount) {
			...
			saashq.db.get_value("Supplier", frm.doc.party, "tax_withholding_category", (values) => {
	...
	set_account_currency_and_balance: function (
		...
		var company_currency = saashq.get_doc(":Company", frm.doc.company).default_currency;
		if (frm.doc.posting_date && account) {
			saashq.call({
				method: "erpnexus.accounts.doctype.payment_entry.payment_entry.get_account_details",
				args: {
					account: account,
					date: frm.doc.posting_date,
					cost_center: frm.doc.cost_center,
	...
	source_exchange_rate: function (frm) {
		...
		frm.set_df_property("source_exchange_rate", "read_only", erpnexus.stale_rate_allowed() ? 0 : 1);
	},
	...
	target_exchange_rate: function (frm) {
		...
		// Make read only if Accounts Settings doesn't allow stale rates
		frm.set_df_property("target_exchange_rate", "read_only", erpnexus.stale_rate_allowed() ? 0 : 1);
	},
	...
	get_outstanding_invoices_or_orders: function (frm, get_outstanding_invoices, get_orders_to_be_billed) {
		const today = saashq.datetime.get_today();
		let fields = [
			{ fieldtype: "Section Break", label: __("Posting Date") },
			{
				...
				default: saashq.datetime.add_days(today, -30),
			},
	...
	validate_filters_data: function (frm, filters) {
		...
		for (let key in fields) {
			...
			if (filters[from_field] && !filters[to_field]) {
				saashq.throw(__("Error: {0} is mandatory field", [to_field.replace(/_/g, " ")]));
	...
	get_outstanding_documents: function (frm, filters, get_outstanding_invoices, get_orders_to_be_billed) {
		...
		saashq.flags.allocate_payment_amount = filters["allocate_payment_amount"];

		return saashq.call({
			method: "erpnexus.accounts.doctype.payment_entry.payment_entry.get_outstanding_reference_documents",
			args: {
				args: args,
			},
			callback: function (r, rt) {
	...
	allocate_party_amount_against_ref_docs: async function (frm, paid_amount, paid_amount_change) {
		await frm.call("allocate_amount_to_references", {
			...
			allocate_payment_amount: saashq.flags.allocate_payment_amount ?? false,
		});
	...
	set_unallocated_amount: function (frm) {
		...
		var total_deductions = saashq.utils.sum(
	...
	check_mandatory_to_fetch: function (frm) {
		$.each(["Company", "Party Type", "Party", "payment_type"], function (i, field) {
			if (!frm.doc[saashq.model.scrub(field)]) {
				saashq.msgprint(__("Please select {0} first", [field]));
				return false;
			}
		});
	},
	...
	validate_reference_document: function (frm, row) {
		var _validate = function (i, row) {
			...
			if (
				...
				saashq.model.set_value(row.doctype, row.name, "reference_doctype", null);
				saashq.msgprint(
					__(
			...
			if (
				...
				saashq.model.set_value(row.doctype, row.name, "against_voucher_type", null);
				saashq.msgprint(
					__(
	...
	set_deductions_entry: function (frm, account) {
		if (frm.doc.difference_amount) {
			saashq.call({
				method: "erpnexus.accounts.doctype.payment_entry.payment_entry.get_company_defaults",
				args: {
					company: frm.doc.company,
				},
				callback: function (r, rt) {
					if (r.message) {
						...
						const add_deductions = (details) => {
							...
							if (row) {
								...
								saashq.msgprint(__("No gain or loss in the exchange rate"));
							}
	...
	bank_account: function (frm) {
		...
		if (frm.doc.bank_account && ["Pay", "Receive"].includes(frm.doc.payment_type)) {
			saashq.call({
				method: "erpnexus.accounts.doctype.bank_account.bank_account.get_bank_account_details",
				args: {
	...
	cost_center: function (frm) {
		if (frm.doc.posting_date && (frm.doc.paid_from || frm.doc.paid_to)) {
			return saashq.call({
				method: "erpnexus.accounts.doctype.payment_entry.payment_entry.get_party_and_account_balance",
				args: {
					company: frm.doc.company,
					date: frm.doc.posting_date,
	...
	after_save: function (frm) {
		const { matched_payment_requests } = saashq.last_response;
...
saashq.ui.form.on("Payment Entry Reference", {
	...
	reference_name: function (frm, cdt, cdn) {
		...
		if (row.reference_name && row.reference_doctype) {
			return saashq.call({
				method: "erpnexus.accounts.doctype.payment_entry.payment_entry.get_reference_details",
				args: {
					reference_doctype: row.reference_doctype,
					reference_name: row.reference_name,
					party_account_currency:
				...
				callback: function (r, rt) {
					if (r.message) {
						$.each(r.message, function (field, value) {
							saashq.model.set_value(cdt, cdn, field, value);
						});
						...
						saashq.model.set_value(cdt, cdn, "allocated_amount", allocated_amount);
						frm.refresh_fields();
					}
				},
			});
...
saashq.ui.form.on("Advance Taxes and Charges", {
	rate: function (frm) {
		frm.events.apply_taxes(frm);
		frm.events.set_unallocated_amount(frm);
	},
...
saashq.ui.form.on("Payment Entry Deduction", {
	amount: function (frm) {
		frm.events.set_unallocated_amount(frm);
	},
