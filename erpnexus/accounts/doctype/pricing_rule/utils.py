...
from saashq.utils import cint, flt, fmt_money, get_link_to_form, getdate, today

from erpnexus.setup.doctype.item_group.item_group import get_child_item_groups
...
class MultiplePricingRuleConflict(saashq.ValidationError):
	pass
...
def get_pricing_rules(args, doc=None):
	...
	if not saashq.db.exists("Pricing Rule", {"disable": 0, args.transaction_type: 1}):
		return
	...
	if doc:
		for pricing_rule in pricing_rules:
			if pricing_rule.condition:
				try:
					if saashq.safe_eval(pricing_rule.condition, None, doc.as_dict()):
...
def _get_pricing_rules(apply_on, args, values):
	apply_on_field = saashq.scrub(apply_on)
	...
	if apply_on_field in ["item_code", "brand"]:
		...
		if apply_on_field == "item_code":
			...
			if "variant_of" not in args:
				args.variant_of = saashq.get_cached_value("Item", args.item_code, "variant_of")
...
def _get_tree_conditions(args, parenttype, table, allow_blank=True):
	field = saashq.scrub(parenttype)
	condition = ""
	if args.get(field):
		if not saashq.flags.tree_conditions:
			saashq.flags.tree_conditions = {}
		key = (parenttype, args.get(field))
		if key in saashq.flags.tree_conditions:
			return saashq.flags.tree_conditions[key]

		try:
			lft, rgt = saashq.db.get_value(parenttype, args.get(field), ["lft", "rgt"])
		except TypeError:
			saashq.throw(_("Invalid {0}").format(args.get(field)))

		parent_groups = saashq.db.sql_list(
		...
		if parenttype in ["Customer Group", "Item Group", "Territory"]:
			parent_field = f"parent_{saashq.scrub(parenttype)}"
			root_name = saashq.db.get_list(
				parenttype,
		...
		if parent_groups:
			...
			condition = "ifnull({table}.{field}, '') in ({parent_groups})".format(
				table=table, field=field, parent_groups=", ".join(saashq.db.escape(d) for d in parent_groups)
			)

			saashq.flags.tree_conditions[key] = condition
	return condition
...
def filter_pricing_rules(args, pricing_rules, doc=None):
	...
	if pricing_rules:
		...
		pr_doc = saashq.get_cached_doc("Pricing Rule", pricing_rules[0].name)
		...
		if pricing_rules[0].mixed_conditions and doc:
			...
			items = [args.get(saashq.scrub(pr_doc.get("apply_on")))]
...
def get_qty_and_rate_for_mixed_conditions(doc, pr_doc, args):
	...
	apply_on = saashq.scrub(pr_doc.get("apply_on"))
...
def get_qty_amount_data_for_cumulative(pr_doc, doc, items=None):
	...
	date_field = (
		"transaction_date" if saashq.get_meta(doctype).has_field("transaction_date") else "posting_date"
	)
			...
			if d.price_or_product_discount == "Price":
				...
				for field in ["additional_discount_percentage", "discount_amount"]:
					...
					if (
						...
						saashq.msgprint(_("User has not applied rule on the invoice {0}").format(doc.name))
					else:
						if not d.coupon_code_based:
							doc.set(field, d.get(pr_field))
						elif doc.get("coupon_code"):
							# coupon code based pricing rule
							coupon_code_pricing_rule = saashq.db.get_value(
				...
				item_details = saashq._dict({"parenttype": doc.doctype, "free_item_data": []})
...
def get_pricing_rule_items(pr_doc, other_items=False) -> list:
	...
	if pr_doc.apply_rule_on_other and other_items:
		apply_on = saashq.scrub(pr_doc.apply_rule_on_other)
...
def validate_coupon_code(coupon_name):
	coupon = saashq.get_doc("Coupon Code", coupon_name)
	if coupon.valid_from and coupon.valid_from > getdate(today()):
		saashq.throw(_("Sorry, this coupon code's validity has not started"))
	elif coupon.valid_upto and coupon.valid_upto < getdate(today()):
		saashq.throw(_("Sorry, this coupon code's validity has expired"))
	elif coupon.maximum_use and coupon.used >= coupon.maximum_use:
		saashq.throw(_("Sorry, this coupon code is no longer valid"))
