...
from saashq.utils import add_days, add_years, flt, getdate, nowdate, today
...
class TestPaymentReconciliation(IntegrationTestCase):
	...
	def create_company(self):
		company = None
		if saashq.db.exists("Company", "_Test Payment Reconciliation"):
			company = saashq.get_doc("Company", "_Test Payment Reconciliation")
		else:
		...
		# create bank account
		if saashq.db.exists("Account", "HDFC - _PR"):
			self.bank = "HDFC - _PR"
		else:
			bank_acc = saashq.get_doc(
				{
					"doctype": "Account",
					"account_name": "HDFC",
	...
	def create_account(self):
		...
		for x in accounts:
			...
			if not saashq.db.get_value(
				...
				acc = saashq.get_doc("Account", name)
	...
	def create_payment_reconciliation(self, party_is_customer=True):
		pr = saashq.new_doc("Payment Reconciliation")
	...
	def create_cost_center(self):
		...
		self.main_cc = saashq.get_doc("Cost Center", get_default_cost_center(self.company))

		cc_exists = saashq.db.get_list("Cost Center", filters={"cost_center_name": cc_name})
		if cc_exists:
			self.sub_cc = saashq.get_doc("Cost Center", cc_exists[0].name)
		else:
			sub_cc = saashq.new_doc("Cost Center")
	...
	def test_difference_amount_via_journal_entry(self):
		...
		pr.reconcile()
		total_credit_amount = saashq.db.get_all(
			"Journal Entry Account",
	...
	def test_no_difference_amount_for_base_currency_accounts(self):
		...
		exc_gain_loss_journals = saashq.db.get_all(
			"Journal Entry Account",
	...
	def test_reconciliation_purchase_invoice_against_return(self):
		...
		pi_return = saashq.get_doc(pi.as_dict())
		pi_return.name = None
		...
		# Should not raise saashq.exceptions.ValidationError: Total Debit must be equal to Total Credit.
		pr.reconcile()
	...
	def test_rounding_of_unallocated_amount(self):
		...
		# Should not raise saashq.exceptions.ValidationError: Payment Entry has been modified after you pulled it. Please pull it again.
		pr.reconcile()
	...
	def test_reconciliation_on_closed_period_payment(self):
		...
		first_fy_start_date = saashq.db.get_value("Fiscal Year", {"disabled": 0}, "min(year_start_date)")
		...
		je_1 = saashq.new_doc("Journal Entry")
...
def make_supplier(supplier_name, currency=None):
	if not saashq.db.exists("Supplier", supplier_name):
		supplier = saashq.new_doc("Supplier")
...
def create_fiscal_year(company, year_start_date, year_end_date):
	...
	if not fy_docname:
		...
		fy_doc = saashq.get_doc("Fiscal Year", fy_docname)
		if not saashq.db.exists("Fiscal Year Company", {"parent": fy_docname, "company": company}):
