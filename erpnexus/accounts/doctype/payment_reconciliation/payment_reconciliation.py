...
from saashq.utils import flt, fmt_money, get_link_to_form, getdate, nowdate, today

import erpnexus
from erpnexus.accounts.doctype.accounting_dimension.accounting_dimension import get_dimensions
from erpnexus.accounts.doctype.process_payment_reconciliation.process_payment_reconciliation import (
	is_any_doc_running,
)
...
from erpnexus.controllers.accounts_controller import get_advance_payment_entries_for_regional
...
class PaymentReconciliation(Document):
	...
	if TYPE_CHECKING:
		...
		from erpnexus.accounts.doctype.payment_reconciliation_allocation.payment_reconciliation_allocation import (
			PaymentReconciliationAllocation,
		)
		from erpnexus.accounts.doctype.payment_reconciliation_invoice.payment_reconciliation_invoice import (
			PaymentReconciliationInvoice,
		)
		from erpnexus.accounts.doctype.payment_reconciliation_payment.payment_reconciliation_payment import (
	...
	def get_jv_entries(self):
		...
		dr_or_cr = (
			...
			if erpnexus.get_party_account_type(self.party_type) == "Receivable"
	...
	def get_return_invoices(self):
		...
		conditions.append(doc[saashq.scrub(self.party_type)] == self.party)
	...
	def get_difference_amount(self, payment_entry, invoice, allocated_amount):
		...
		) != saashq.get_cached_value("Company", self.company, "default_currency"):
	...
	def is_auto_process_enabled(self):
		return saashq.db.get_single_value("Accounts Settings", "auto_reconcile_payments")
	...
	def allocate_entries(self, args):
		...
		default_exchange_gain_loss_account = saashq.get_cached_value(
	...
	@saashq.whitelist()
	def reconcile(self):
		if saashq.db.get_single_value("Accounts Settings", "auto_reconcile_payments"):
	...
	def check_mandatory_to_fetch(self):
		for fieldname in ["company", "party_type", "party", "receivable_payable_account"]:
			if not self.get(fieldname):
				saashq.throw(_("Please select {0} first").format(self.meta.get_label(fieldname)))

	def validate_entries(self):
		if not self.get("invoices"):
			saashq.throw(_("No records found in the Invoices table"))

		if not self.get("payments"):
			saashq.throw(_("No records found in the Payments table"))
	...
	def get_invoice_exchange_map(self, invoices, payments):
		...
		invoice_exchange_map = saashq._dict()

		if sales_invoices:
			sales_invoice_map = saashq._dict(
				saashq.db.get_all(
					"Sales Invoice",
		...
		if purchase_invoices:
			purchase_invoice_map = saashq._dict(
				saashq.db.get_all(
					"Purchase Invoice",
	...
	def validate_allocation(self):
		unreconciled_invoices = saashq._dict()
		...
		if not invoices_to_reconcile:
			saashq.throw(_("No records found in Allocation table"))
				...
				"accounts": [
					{
						...
						"cost_center": inv.cost_center or erpnexus.get_default_cost_center(company),
...
def get_queries_for_dimension_filters(company: str | None = None):
	...
	for d in get_dimensions()[0]:
		filters = {}
		meta = saashq.get_meta(d.document_type)
