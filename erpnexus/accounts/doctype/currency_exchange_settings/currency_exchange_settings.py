...
class CurrencyExchangeSettings(Document):
	...
	if TYPE_CHECKING:
		...
		from erpnexus.accounts.doctype.currency_exchange_settings_details.currency_exchange_settings_details import (
			CurrencyExchangeSettingsDetails,
		)
		from erpnexus.accounts.doctype.currency_exchange_settings_result.currency_exchange_settings_result import (
	...
	def validate(self):
		...
		if saashq.flags.in_test or saashq.flags.in_install or saashq.flags.in_setup_wizard:
			return
			...
			if not self.access_key:
				saashq.throw(
					_("Access Key is required for Service Provider: {0}").format(
						saashq.bold(self.service_provider)
					)
				)
	...
	def validate_result(self, response, value):
		try:
			...
			saashq.throw(_("Invalid result key. Response:") + " " + response.text)
		if not isinstance(value, int | float):
			saashq.throw(_("Returned exchange rate is neither integer not float."))
