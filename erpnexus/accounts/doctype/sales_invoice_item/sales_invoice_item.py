...
from erpnexus.assets.doctype.asset.depreciation import get_disposal_account_and_cost_center
from erpnexus.stock.doctype.serial_no.serial_no import get_delivery_note_serial_no, get_serial_nos
...
class SalesInvoiceItem(Document):
	...
	def validate_cost_center(self, company: str):
		cost_center_company = saashq.get_cached_value("Cost Center", self.cost_center, "company")
		if cost_center_company != company:
			saashq.throw(
				_("Row #{0}: Cost Center {1} does not belong to company {2}").format(
					saashq.bold(self.idx), saashq.bold(self.cost_center), saashq.bold(company)
				)
			)
	...
	def validate_serial_against_delivery_note(self):
		...
		serial_nos = saashq.db.get_value("Delivery Note Item", self.dn_detail, "serial_no") or ""
		...
		if serial_no_diff:
			dn_link = saashq.utils.get_link_to_form("Delivery Note", self.delivery_note)
			msg = (
				_("Row #{0}: The following serial numbers are not present in Delivery Note {1}:").format(
					self.idx, dn_link
				)
				+ " "
				+ ", ".join(saashq.bold(d) for d in serial_no_diff)
			)

			saashq.throw(msg=msg, title=_("Serial Nos Mismatch"))
