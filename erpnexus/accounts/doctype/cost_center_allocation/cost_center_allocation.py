...
from saashq.utils import add_days, flt, format_date, getdate


class MainCostCenterCantBeChild(saashq.ValidationError):
	pass


class InvalidMainCostCenter(saashq.ValidationError):
	pass


class InvalidChildCostCenter(saashq.ValidationError):
	pass


class WrongPercentageAllocation(saashq.ValidationError):
	pass


class InvalidDateError(saashq.ValidationError):
	pass
...
class CostCenterAllocation(Document):
	...
	if TYPE_CHECKING:
		...
		from erpnexus.accounts.doctype.cost_center_allocation_percentage.cost_center_allocation_percentage import (
	...
	def validate_total_allocation_percentage(self):
		...
		if total_percentage != 100:
			saashq.throw(_("Total percentage against cost centers should be 100"), WrongPercentageAllocation)

	def validate_from_date_based_on_existing_gle(self):
		# Check if GLE exists against the main cost center
		# If exists ensure from date is set after posting date of last GLE

		last_gle_date = saashq.db.get_value(
			"GL Entry",
	...
	def validate_backdated_allocation(self):
		...
		future_allocation = saashq.db.get_value(
		...
		if future_allocation:
			saashq.msgprint(
				_(
					...
					saashq.bold(future_allocation.name),
					saashq.bold(format_date(future_allocation.valid_from)),
					saashq.bold(format_date(add_days(future_allocation.valid_from, -1))),
				),
				title=_("Warning!"),
				indicator="orange",
				alert=1,
	...
	def validate_child_cost_centers(self):
		...
		main_cost_centers = [
			d.main_cost_center
			for d in saashq.get_all("Cost Center Allocation", {"docstatus": 1}, "main_cost_center")
		]
