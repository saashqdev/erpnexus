...
from erpnexus.accounts.doctype.finance_book.test_finance_book import create_finance_book
...
class TestPeriodClosingVoucher(IntegrationTestCase):
	def test_closing_entry(self):
		saashq.db.sql("delete from `tabGL Entry` where company='Test PCV Company'")
		saashq.db.sql("delete from `tabPeriod Closing Voucher` where company='Test PCV Company'")
	...
	def test_gl_entries_restrictions(self):
		...
		self.assertRaises(saashq.ValidationError, jv1.submit)

	def test_closing_balance_with_dimensions_and_test_reposting_entry(self):
		saashq.db.sql("delete from `tabGL Entry` where company='Test PCV Company'")
		saashq.db.sql("delete from `tabPeriod Closing Voucher` where company='Test PCV Company'")
		saashq.db.sql("delete from `tabAccount Closing Balance` where company='Test PCV Company'")
		...
		closing_balance = saashq.db.get_value(
		...
		cc1_closing_balance = saashq.db.get_value(
			"Account Closing Balance",
			{
				"account": "Sales - TPC",
				"cost_center": cost_center1,
		...
		cc2_closing_balance = saashq.db.get_value(
			"Account Closing Balance",
			{
				"account": "Sales - TPC",
				"cost_center": cost_center2,
		...
		warehouse = saashq.db.get_value("Warehouse", {"company": company}, "name")
		...
		self.assertRaises(saashq.ValidationError, repost_doc.save)
