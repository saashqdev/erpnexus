...
from erpnexus.accounts.doctype.bank_transaction.test_bank_transaction import create_bank_account
...
class TestAutoMatchParty(IntegrationTestCase):
	...
	def setUpClass(cls):
		create_bank_account()
		saashq.db.set_single_value("Accounts Settings", "enable_party_matching", 1)
		saashq.db.set_single_value("Accounts Settings", "enable_fuzzy_matching", 1)
		return super().setUpClass()

	@classmethod
	def tearDownClass(cls):
		saashq.db.set_single_value("Accounts Settings", "enable_party_matching", 0)
		saashq.db.set_single_value("Accounts Settings", "enable_fuzzy_matching", 0)
...
def create_supplier_for_match(supplier_name="John Doe & Co.", iban=None, account_no=None):
	if saashq.db.exists("Supplier", {"supplier_name": supplier_name}):
	...
	if not saashq.db.exists("Bank", "TestBank"):
		bank = saashq.new_doc("Bank")
		bank.bank_name = "TestBank"
		bank.insert(ignore_if_duplicate=True)

	if not saashq.db.exists("Bank Account", supplier.name + " - " + "TestBank"):
		bank_account = saashq.new_doc("Bank Account")
...
def create_bank_transaction(
	...
	doc = saashq.new_doc("Bank Transaction")
	doc.update(
		{
