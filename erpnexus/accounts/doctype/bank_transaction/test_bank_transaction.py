...
import saashq
from saashq import utils
from saashq.model.docstatus import DocStatus
from saashq.tests import IntegrationTestCase, UnitTestCase

from erpnexus.accounts.doctype.bank_reconciliation_tool.bank_reconciliation_tool import (
	get_linked_payments,
	reconcile_vouchers,
)
...
from erpnexus.tests.utils import if_lending_app_installed
...
class TestBankTransaction(IntegrationTestCase):
	def setUp(self):
		...
		uniq_identifier = saashq.generate_hash(length=10)
	...
	# This test checks if ERPNexus is able to provide a linked payment for a bank transaction based on the amount of the bank transaction.
	def test_linked_payments(self):
		bank_transaction = saashq.get_doc(
			"Bank Transaction",
	...
	def test_reconcile(self):
		...
		payment = saashq.get_doc("Payment Entry", dict(party="Mr G", paid_amount=1700))
		vouchers = json.dumps(
			[
				{
		...
		unallocated_amount = saashq.db.get_value(
			"Bank Transaction", bank_transaction.name, "unallocated_amount"
		)
		self.assertTrue(unallocated_amount == 0)

		clearance_date = saashq.db.get_value("Payment Entry", payment.name, "clearance_date")
	...
	# Check if ERPNexus can correctly filter a linked payments based on the debit/credit amount
	...
	def test_already_reconciled(self):
		...
		payment = saashq.get_doc("Payment Entry", dict(party="Mr G", paid_amount=1200))
		vouchers = json.dumps(
			[
				{
					"payment_doctype": "Payment Entry",
	...
	def test_clear_sales_invoice(self):
		...
		payment = saashq.get_doc("Sales Invoice", dict(customer="Fayva", status=["=", "Paid"]))
		vouchers = json.dumps(
			[
				{
		...
		self.assertEqual(
			saashq.db.get_value("Bank Transaction", bank_transaction.name, "unallocated_amount"), 0
		)
		self.assertTrue(
			saashq.db.get_value("Sales Invoice Payment", dict(parent=payment.name), "clearance_date")
			is not None
		)
...
def add_vouchers(gl_account="_Test Bank - _TC"):
	...
	mode_of_payment = saashq.get_doc({"doctype": "Mode of Payment", "name": "Cash"})

	if not saashq.db.get_value("Mode of Payment Account", {"company": "_Test Company", "parent": "Cash"}):
