...
from erpnexus.accounts.doctype.account.account import (
	InvalidAccountMergeError,
	merge_account,
	update_account_number,
)
from erpnexus.stock import get_company_default_inventory_account, get_warehouse_account
...
class TestAccount(IntegrationTestCase):
	def test_rename_account(self):
		if not saashq.db.exists("Account", "1210 - Debtors - _TC"):
		...
		account_number, account_name = saashq.db.get_value(
		...
		saashq.delete_doc("Account", "1211-11-4 - 6 - Debtors 1 - Test - - _TC")
	...
	def test_merge_account(self):
		...
		parent = saashq.db.get_value("Account", "Earnest Money - _TC", "parent_account")
		...
		self.assertFalse(saashq.db.exists("Account", "Securities and Deposits - _TC"))
	...
	def test_account_sync(self):
		saashq.local.flags.pop("ignore_root_company_validation", None)
	...
	def test_add_account_to_a_group(self):
		saashq.db.set_value("Account", "Office Rent - _TC3", "is_group", 1)

		acc = saashq.new_doc("Account")
		acc.account_name = "Test Group Account"
		acc.parent_account = "Office Rent - _TC3"
		acc.company = "_Test Company 3"
		self.assertRaises(saashq.ValidationError, acc.insert)

		saashq.db.set_value("Account", "Office Rent - _TC3", "is_group", 0)
	...
	def test_account_rename_sync(self):
		...
		saashq.delete_doc("Account", "1234 - Test Rename Sync Account - _TC3")
		saashq.delete_doc("Account", "1234 - Test Rename Sync Account - _TC4")
		saashq.delete_doc("Account", "1234 - Test Rename Sync Account - _TC5")
	...
	def test_account_currency_sync(self):
		...
		saashq.delete_doc("Account", "_Test Bank JPY - _TC6")
		saashq.delete_doc("Account", "_Test Bank JPY - _TC7")
	...
	def test_child_company_account_rename_sync(self):
		...
		self.assertTrue(
			saashq.db.exists("Account", {"account_name": "Test Group Account", "company": "_Test Company 4"})
		)
		self.assertTrue(
			saashq.db.exists("Account", {"account_name": "Test Group Account", "company": "_Test Company 5"})
		)
		...
		self.assertRaises(saashq.ValidationError, update_account_number, acc_tc_5, "Test Modified Account")

		# Rename child company account with allow_account_creation_against_child_company enabled
		saashq.db.set_value("Company", "_Test Company 5", "allow_account_creation_against_child_company", 1)
		...
		saashq.db.set_value("Company", "_Test Company 5", "allow_account_creation_against_child_company", 0)
	...
	def test_validate_account_currency(self):
		...
		if not saashq.db.get_value("Account", "Test Currency Account - _TC"):
			acc = saashq.new_doc("Account")
			...
			acc = saashq.get_doc("Account", "Test Currency Account - _TC")
		...
		self.assertRaises(saashq.ValidationError, acc.save)

	def test_account_balance(self):
		from erpnexus.accounts.utils import get_balance_on

		if not saashq.db.exists("Account", "Test Percent Account %5 - _TC"):
			acc = saashq.new_doc("Account")
...
def get_inventory_account(company, warehouse=None):
	account = None
	if warehouse:
		account = get_warehouse_account(saashq.get_doc("Warehouse", warehouse))
	else:
...
def create_account(**kwargs):
	...
	if account:
		account = saashq.get_doc("Account", account)
		account.update(
			dict(
