					...
					account = saashq.get_doc(
						{
							...
							or saashq.get_cached_value("Company", company, "default_currency"),
							"tax_rate": child.get("tax_rate"),
						}
					)

					if root_account or saashq.local.flags.allow_unverified_charts:
...
def get_chart(chart_template, existing_company=None):
	...
	if existing_company:
		...
		from erpnexus.accounts.doctype.account.chart_of_accounts.verified import (
...
def get_charts_for_country(country, with_standard=False):
	...
	def _get_chart_name(content):
		if content:
			...
			) or saashq.local.flags.allow_unverified_charts:
				charts.append(content["name"])

	country_code = saashq.get_cached_value("Country", country, "code")
	if country_code:
		folders = ("verified",)
		if saashq.local.flags.allow_unverified_charts:
			folders = ("verified", "unverified")
