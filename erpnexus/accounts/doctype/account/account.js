...
saashq.ui.form.on("Account", {
	...
	add_toolbar_buttons: function (frm) {
		...
		} else if (cint(frm.doc.is_group) == 0 && saashq.boot.user.can_read.indexOf("GL Entry") !== -1) {
			frm.add_custom_button(
				__("General Ledger"),
				function () {
	...
	merge_account: function (frm) {
		var d = new saashq.ui.Dialog({
			...
			primary_action: function () {
				...
				saashq.call({
					method: "erpnexus.accounts.doctype.account.account.merge_account",
					args: {
						old: frm.doc.name,
						new: data.name,
					},
					callback: function (r) {
						if (!r.exc) {
							if (r.message) {
								saashq.set_route("Form", "Account", r.message);
							}
							d.hide();
						}
					},
	...
	update_account_number: function (frm) {
		var d = new saashq.ui.Dialog({
			...
			primary_action: function () {
				...
				saashq.call({
					method: "erpnexus.accounts.doctype.account.account.update_account_number",
					args: {
