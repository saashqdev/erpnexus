...
saashq.ui.form.on("Cost Center", {
	...
	refresh: function (frm) {
		...
		if (!frm.doc.__islocal) {
			frm.add_custom_button(__("Chart of Cost Centers"), function () {
				saashq.set_route("Tree", "Cost Center");
			});

			frm.add_custom_button(__("Budget"), function () {
				saashq.set_route("List", "Budget", { cost_center: frm.doc.name });
			});
		}
	},
	update_cost_center_number: function (frm) {
		var d = new saashq.ui.Dialog({
			...
			primary_action: function () {
				...
				saashq.dom.freeze();
				saashq.call({
					method: "erpnexus.accounts.utils.update_cost_center",
					args: {
					...
					callback: function (r) {
						...
						if (!r.exc) {
							if (r.message) {
								saashq.set_route("Form", "Cost Center", r.message);
							} else {
	...
	parent_cost_center(frm) {
		if (!frm.doc.company) {
			saashq.msgprint(__("Please enter company name first"));
		}
	},
