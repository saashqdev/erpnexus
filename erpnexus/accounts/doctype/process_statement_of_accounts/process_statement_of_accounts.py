...
from saashq.utils import add_days, add_months, format_date, getdate, today
from saashq.utils.jinja import validate_template
from saashq.utils.pdf import get_pdf
from saashq.www.printview import get_print_style

from erpnexus import get_company_currency
from erpnexus.accounts.party import get_party_account_currency
from erpnexus.accounts.report.accounts_receivable.accounts_receivable import execute as get_ar_soa
from erpnexus.accounts.report.accounts_receivable_summary.accounts_receivable_summary import (
	execute as get_ageing,
)
from erpnexus.accounts.report.general_ledger.general_ledger import execute as get_soa
...
class ProcessStatementOfAccounts(Document):
	...
	if TYPE_CHECKING:
		...
		from erpnexus.accounts.doctype.process_statement_of_accounts_customer.process_statement_of_accounts_customer import (
			ProcessStatementOfAccountsCustomer,
		)
		from erpnexus.accounts.doctype.psoa_cost_center.psoa_cost_center import PSOACostCenter
		from erpnexus.accounts.doctype.psoa_project.psoa_project import PSOAProject
	...
	def validate(self):
		...
		if not self.customers:
			saashq.throw(_("Customers not selected."))
...
def get_statement_dict(doc, get_statement_dict=False):
	...
	for entry in doc.customers:
		...
		tax_id = saashq.get_doc("Customer", entry.customer).tax_id
...
def get_html(doc, filters, entry, col, res, ageing):
	base_template_path = "saashq/www/printview.html"
	template_path = (
		"erpnexus/accounts/doctype/process_statement_of_accounts/process_statement_of_accounts.html"
		if doc.report == "General Ledger"
		else "erpnexus/accounts/doctype/process_statement_of_accounts/process_statement_of_accounts_accounts_receivable.html"
	)

	if doc.letter_head:
		from saashq.www.printview import get_letter_head

		letter_head = get_letter_head(doc, 0)

	html = saashq.render_template(
		template_path,
		{
			"filters": filters,
			"data": res,
			...
			"terms_and_conditions": saashq.db.get_value(
...
def get_customers_based_on_territory_or_customer_group(customer_collection, collection_name):
	...
	collection = saashq.get_doc(customer_collection, collection_name)
	selected = [
		customer.name
		for customer in saashq.get_list(
			customer_collection,
...
def get_customers_based_on_sales_person(sales_person):
	lft, rgt = saashq.db.get_value("Sales Person", sales_person, ["lft", "rgt"])
	...
	sales_person_records = saashq._dict()
	for d in records:
...
def get_recipients_and_cc(customer, doc):
	...
	if doc.cc_to != "":
		try:
			cc = [saashq.get_value("User", doc.cc_to, "email")]
		except Exception:
			pass
...
def fetch_customers(customer_collection, collection_name, primary_mandatory):
	...
	if customer_collection == "Sales Person":
		...
		if not bool(customers):
			saashq.throw(_("No Customers found with selected options."))
	else:
		if customer_collection == "Sales Partner":
			customers = saashq.get_list(
				"Customer",
				fields=["name", "customer_name", "email_id"],
...
def get_customer_emails(customer_name, primary_mandatory, billing_and_primary=True):
	...
	if len(billing_email) == 0 or (billing_email[0][0] is None):
		if billing_and_primary:
			saashq.throw(_("No billing email found for customer: {0}").format(customer_name))
		else:
			return ""

	if billing_and_primary:
		primary_email = saashq.get_value("Customer", customer_name, "email_id")
		if primary_email is None and int(primary_mandatory):
			saashq.throw(_("No primary email found for customer: {0}").format(customer_name))
...
def download_statements(document_name):
	doc = saashq.get_doc("Process Statement Of Accounts", document_name)
	report = get_report_pdf(doc)
	if report:
		saashq.local.response.filename = doc.name + ".pdf"
		saashq.local.response.filecontent = report
...
def send_emails(document_name, from_scheduler=False, posting_date=None):
	...
	if report:
		for customer, report_pdf in report.items():
			...
			filename = saashq.render_template(doc.pdf_name, context)
			...
			subject = saashq.render_template(doc.subject, context)
			message = saashq.render_template(doc.body, context)

			if doc.sender:
				sender_email = saashq.db.get_value("Email Account", doc.sender, "email_id")
			else:
				sender_email = saashq.session.user
		...
		if doc.enable_auto_email and from_scheduler:
			...
			doc.add_comment("Comment", "Emails sent on: " + saashq.utils.format_datetime(saashq.utils.now()))
			if doc.report == "General Ledger":
