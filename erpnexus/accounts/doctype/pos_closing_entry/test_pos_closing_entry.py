...
from erpnexus.accounts.doctype.pos_closing_entry.pos_closing_entry import (
	make_closing_entry_from_opening,
)
from erpnexus.accounts.doctype.pos_invoice.pos_invoice import make_sales_return
from erpnexus.accounts.doctype.pos_invoice.test_pos_invoice import create_pos_invoice
from erpnexus.accounts.doctype.pos_opening_entry.test_pos_opening_entry import create_opening_entry
from erpnexus.accounts.doctype.pos_profile.test_pos_profile import make_pos_profile
from erpnexus.selling.page.point_of_sale.point_of_sale import get_items
...
class TestPOSClosingEntry(IntegrationTestCase):
	...
	def test_cancelling_of_pos_closing_entry(self):
		...
		self.assertRaises(saashq.ValidationError, pos_inv1.cancel)

		si_doc = saashq.get_doc("Sales Invoice", pos_inv1.consolidated_invoice)
		self.assertRaises(saashq.ValidationError, si_doc.cancel)

		pcv_doc.load_from_db()
		pcv_doc.cancel()

		cancelled_invoice = saashq.db.get_value(
			"POS Invoice Merge Log", {"pos_closing_entry": pcv_doc.name}, "consolidated_invoice"
		)
		docstatus = saashq.db.get_value("Sales Invoice", cancelled_invoice, "docstatus")
	...
	def test_pos_closing_for_required_accounting_dimension_in_pos_profile(self):
		...
		self.assertRaises(saashq.ValidationError, pos_profile.insert)

		pos_profile.location = "Block 1"
		pos_profile.insert()
		self.assertTrue(saashq.db.exists("POS Profile", pos_profile.name))
		...
		accounting_dimension_department = saashq.get_doc("Accounting Dimension", {"name": "Department"})
		...
		self.assertRaises(saashq.ValidationError, pcv_doc.submit)

		accounting_dimension_department = saashq.get_doc(
	...
	def test_merging_into_sales_invoice_for_batched_item(self):
		saashq.flags.print_message = False
		...
		batch_qty = saashq.db.get_value("Batch", batch_no, "batch_qty")
		...
		piv_merge = saashq.db.get_value("POS Invoice Merge Log", {"pos_closing_entry": pcv_doc.name}, "name")

		self.assertTrue(piv_merge)
		piv_merge_doc = saashq.get_doc("POS Invoice Merge Log", piv_merge)
		...
		saashq.flags.print_message = True
