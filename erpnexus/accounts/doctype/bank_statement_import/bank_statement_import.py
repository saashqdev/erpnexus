...
from saashq.core.doctype.data_import.data_import import DataImport
from saashq.core.doctype.data_import.importer import Importer, ImportFile
from saashq.utils.background_jobs import enqueue
from saashq.utils.xlsxutils import ILLEGAL_CHARACTERS_RE, handle_html
...
class BankStatementImport(DataImport):
	...
	def validate(self):
		...
		if (
			...
			column_to_field_map = {}
			bank = saashq.get_doc("Bank", self.bank)
	...
	def start_import(self):
		preview = saashq.get_doc("Bank Statement Import", self.name).get_preview_from_template(
			self.import_file, self.google_sheets_url
		)

		if "Bank Account" not in json.dumps(preview["columns"]):
			saashq.throw(_("Please add the Bank Account column"))
...
def get_preview_from_template(data_import, import_file=None, google_sheets_url=None):
	return saashq.get_doc("Bank Statement Import", data_import).get_preview_from_template(
...
def form_start_import(data_import):
	return saashq.get_doc("Bank Statement Import", data_import).start_import()


@saashq.whitelist()
def download_errored_template(data_import_name):
	data_import = saashq.get_doc("Bank Statement Import", data_import_name)
	data_import.export_errored_rows()


@saashq.whitelist()
def download_import_log(data_import_name):
	return saashq.get_doc("Bank Statement Import", data_import_name).download_import_log()
...
def start_import(data_import, bank_account, import_file_path, google_sheets_url, bank, template_options):
	...
	data_import = saashq.get_doc("Bank Statement Import", data_import)
...
def get_import_status(docname):
	...
	data_import = saashq.get_doc("Bank Statement Import", docname)
...
def get_import_logs(docname: str):
	saashq.has_permission("Bank Statement Import")
...
def upload_bank_statement(**args):
	...
	bsi = saashq.new_doc("Bank Statement Import")
