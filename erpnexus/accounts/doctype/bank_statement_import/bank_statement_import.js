...
saashq.ui.form.on("Bank Statement Import", {
	onload(frm) {
	...
	setup(frm) {
		saashq.realtime.on("data_import_refresh", ({ data_import }) => {
			...
			saashq.model.clear_doc("Bank Statement Import", frm.doc.name);
			saashq.model.with_doc("Bank Statement Import", frm.doc.name).then(() => {
				frm.refresh();
			});
		});
	...
	show_import_status(frm) {
		...
		saashq.call({
			method: "erpnexus.accounts.doctype.bank_statement_import.bank_statement_import.get_import_status",
			args: {
	...
	// method: 'saashq.core.doctype.data_import.data_import.get_preview_from_template',
	...
	export_errored_rows(frm) {
		open_url_post(
			"/api/method/erpnexus.accounts.doctype.bank_statement_import.bank_statement_import.download_errored_template",
			{
	...
	export_import_log(frm) {
		open_url_post(
			"/api/method/erpnexus.accounts.doctype.bank_statement_import.bank_statement_import.download_import_log",
			{
				data_import_name: frm.doc.name,
			}
		);
	...
	render_import_log(frm) {
		saashq.call({
			method: "erpnexus.accounts.doctype.bank_statement_import.bank_statement_import.get_import_logs",
			args: {
				docname: frm.doc.name,
			},
			callback: function (r) {
