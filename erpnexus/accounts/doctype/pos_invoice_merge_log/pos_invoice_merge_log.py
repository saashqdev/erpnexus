...
from saashq.model.mapper import map_child_doc, map_doc
from saashq.utils import cint, flt, get_time, getdate, nowdate, nowtime
from saashq.utils.background_jobs import enqueue, is_job_enqueued
from saashq.utils.scheduler import is_scheduler_inactive

from erpnexus.accounts.doctype.pos_profile.pos_profile import required_accounting_dimensions
...
class POSInvoiceMergeLog(Document):
	...
	if TYPE_CHECKING:
		...
		from erpnexus.accounts.doctype.pos_invoice_reference.pos_invoice_reference import (
			POSInvoiceReference,
		)
	...
	def validate_duplicate_pos_invoices(self):
		...
		for key, value in pos_occurences.items():
			if len(value) > 1:
				error_list.append(
					_("{0} is added multiple times on rows: {1}").format(saashq.bold(key), saashq.bold(value))
				)

		if error_list:
			saashq.throw(error_list, title=_("Duplicate POS Invoices found"), as_list=True)
	...
	def validate_pos_invoice_status(self):
		for d in self.pos_invoices:
			status, docstatus, is_return, return_against = saashq.db.get_value(
				"POS Invoice", d.pos_invoice, ["status", "docstatus", "is_return", "return_against"]
			)

			bold_pos_invoice = saashq.bold(d.pos_invoice)
			bold_status = saashq.bold(status)
			if docstatus != 1:
				saashq.throw(
			...
			if (
				...
				bold_return_against = saashq.bold(return_against)
				return_against_status = saashq.db.get_value("POS Invoice", return_against, "status")
	...
	def on_submit(self):
		pos_invoice_docs = [saashq.get_cached_doc("POS Invoice", d.pos_invoice) for d in self.pos_invoices]
	...
	def merge_pos_invoice_into(self, invoice, data):
		...
		invoice.disable_rounded_total = cint(
			saashq.db.get_value("POS Profile", invoice.pos_profile, "disable_rounded_total")
		)
			...
			if not dimension_value:
				saashq.throw(
					_("Please set Accounting Dimension {} in {}").format(
						...
						saashq.get_desk_link("POS Profile", invoice.pos_profile),
					)
				)
	...
	def get_new_sales_invoice(self):
		sales_invoice = saashq.new_doc("Sales Invoice")
	...
	def serial_and_batch_bundle_reference_for_pos_invoice(self):
		for d in self.pos_invoices:
			pos_invoice = saashq.get_doc("POS Invoice", d.pos_invoice)
	...
	def cancel_linked_invoices(self):
		for si_name in [self.consolidated_invoice, self.consolidated_credit_note]:
			if not si_name:
				continue
			si = saashq.get_doc("Sales Invoice", si_name)
...
def get_all_unconsolidated_invoices():
	...
	pos_invoices = saashq.db.get_all(
		"POS Invoice",
		filters=filters,
		fields=[
			"name as pos_invoice",
...
def consolidate_pos_invoices(pos_invoices=None, closing_entry=None):
	...
	if saashq.flags.in_test and not invoices:
...
def split_invoices(invoices):
	...
	special_invoices = []
	pos_return_docs = [
		saashq.get_cached_doc("POS Invoice", d.pos_invoice)
		for d in invoices
	...
	for pos_invoice in pos_return_docs:
		for item in pos_invoice.items:
			...
			return_against_is_consolidated = (
				saashq.db.get_value("POS Invoice", pos_invoice.return_against, "status", cache=True)
				== "Consolidated"
			)
...
def create_merge_logs(invoice_by_customer, closing_entry=None):
	try:
		...
		saashq.db.rollback()
		message_log = saashq.message_log.pop() if saashq.message_log else str(e)
		...
		saashq.db.commit()
		saashq.publish_realtime("closing_process_complete", user=saashq.session.user)


def cancel_merge_logs(merge_logs, closing_entry=None):
	try:
		for log in merge_logs:
			merge_log = saashq.get_doc("POS Invoice Merge Log", log)
...
def enqueue_job(job, **kwargs):
	...
	if not is_job_enqueued(job_id):
		...
		saashq.msgprint(msg, alert=1)


def check_scheduler_status():
	if is_scheduler_inactive() and not saashq.flags.in_test:
		saashq.throw(_("Scheduler is inactive. Cannot enqueue job."), title=_("Scheduler Inactive"))
