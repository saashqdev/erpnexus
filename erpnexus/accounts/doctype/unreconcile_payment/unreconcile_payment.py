...
class UnreconcilePayment(Document):
	...
	if TYPE_CHECKING:
		...
		from erpnexus.accounts.doctype.unreconcile_payment_entries.unreconcile_payment_entries import (
	...
	def validate(self):
		...
		if self.voucher_type not in self.supported_types:
			saashq.throw(_("Only {0} are supported").format(comma_and(self.supported_types)))
	...
	def on_submit(self):
		...
		for alloc in self.allocations:
			doc = saashq.get_doc(alloc.reference_doctype, alloc.reference_name)
			unlink_ref_doc_from_payment_entries(doc, self.voucher_no)
			cancel_exchange_gain_loss_journal(doc, self.voucher_type, self.voucher_no)
			update_voucher_outstanding(
				alloc.reference_doctype, alloc.reference_name, alloc.account, alloc.party_type, alloc.party
			)
			if doc.doctype in saashq.get_hooks("advance_payment_payable_doctypes") + saashq.get_hooks(
				"advance_payment_receivable_doctypes"
			):
				doc.set_total_advance_paid()

			saashq.db.set_value("Unreconcile Payment Entries", alloc.name, "unlinked", True)
...
def create_unreconcile_doc_for_selection(selections=None):
	if selections:
		...
		for row in selections:
			unrecon = saashq.new_doc("Unreconcile Payment")
