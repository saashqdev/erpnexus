...
saashq.ui.form.on("Accounting Dimension Filter", {
	...
	onload: function (frm) {
		...
		saashq.db.get_list("Accounting Dimension", { fields: ["document_type"] }).then((res) => {
	...
	setup_filters: function (frm) {
		...
		if (frm.doc.accounting_dimension) {
			saashq.model.with_doctype(frm.doc.accounting_dimension, function () {
				if (saashq.model.is_tree(frm.doc.accounting_dimension)) {
					filters["is_group"] = 0;
				}

				if (saashq.meta.has_field(frm.doc.accounting_dimension, "company")) {
...
saashq.ui.form.on("Allowed Dimension", {
