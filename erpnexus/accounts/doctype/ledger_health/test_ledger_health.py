...
from erpnexus.accounts.utils import run_ledger_health_checks
...
class TestLedgerHealth(AccountsTestMixin, IntegrationTestCase):
	...
	def configure_monitoring_tool(self):
		monitor_settings = saashq.get_doc("Ledger Health Monitor")
	...
	def test_debit_credit_mismatch(self):
		...
		saashq.db.set_value("GL Entry", gle.name, "credit", 8000)
	...
	def test_gl_and_pl_mismatch(self):
		...
		ple = saashq.db.get_all("Payment Ledger Entry", filters={"voucher_no": self.je.name})[0]
		saashq.db.set_value("Payment Ledger Entry", ple.name, "amount", 11000)
