...
from erpnexus.accounts.doctype.pos_profile.pos_profile import (
	get_child_nodes,
)
from erpnexus.stock.get_item_details import get_pos_profile
...
class TestPOSProfile(IntegrationTestCase):
	def test_pos_profile(self):
		...
		if pos_profile:
			doc = saashq.get_doc("POS Profile", pos_profile.get("name"))
...
def make_pos_profile(**args):
	saashq.db.sql("delete from `tabPOS Payment Method`")
	saashq.db.sql("delete from `tabPOS Profile`")

	args = saashq._dict(args)

	pos_profile = saashq.get_doc(
		{
			...
			"customer_group": saashq.db.get_value("Customer Group", {"is_group": 0}, "name"),
	...
	mode_of_payment = saashq.get_doc("Mode of Payment", "Cash")
	company = args.company or "_Test Company"
	default_account = args.income_account or "Sales - _TC"

	if not saashq.db.get_value("Mode of Payment Account", {"company": company, "parent": "Cash"}):
	...
	if not saashq.db.exists("POS Profile", args.name or "_Test POS Profile"):
