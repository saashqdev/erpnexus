...
class RepostAccountingLedger(Document):
	...
	if TYPE_CHECKING:
		...
		from erpnexus.accounts.doctype.repost_accounting_ledger_items.repost_accounting_ledger_items import (
		...
		if self.vouchers:
			...
			for vtype in self._allowed_types:
				if names := [x.voucher_no for x in self.vouchers if x.voucher_type == vtype]:
					latest_voucher = saashq.db.get_all(
						vtype,
					...
					if latest_voucher and latest_pcv[0] >= latest_voucher:
						saashq.throw(_("Cannot Resubmit Ledger entries for vouchers in Closed fiscal year."))
	...
	def get_existing_ledger_entries(self):
		...
		self.gles = saashq._dict({})

		for gle in existing_gles:
			self.gles.setdefault((gle.voucher_type, gle.voucher_no), saashq._dict({})).setdefault(
				"existing", []
			).append(gle.update({"old": True}))

	def generate_preview_data(self):
		saashq.flags.through_repost_accounting_ledger = True
		self.gl_entries = []
		self.get_existing_ledger_entries()
		for x in self.vouchers:
			doc = saashq.get_doc(x.voucher_type, x.voucher_no)
	...
	def generate_preview(self):
		...
		rendered_page = saashq.render_template(
			"erpnexus/accounts/doctype/repost_accounting_ledger/repost_accounting_ledger.html",
	...
	def on_submit(self):
		if len(self.vouchers) > 5:
			...
			saashq.enqueue(
				method="erpnexus.accounts.doctype.repost_accounting_ledger.repost_accounting_ledger.start_repost",
				account_repost_doc=self.name,
				is_async=True,
				job_name=job_name,
			)
			saashq.msgprint(_("Repost has started in the background"))
		else:
...
def start_repost(account_repost_doc=str) -> None:
	...
	if account_repost_doc:
		repost_doc = saashq.get_doc("Repost Accounting Ledger", account_repost_doc)
...
def validate_docs_for_deferred_accounting(sales_docs, purchase_docs):
	docs_with_deferred_revenue = saashq.db.get_all(
		"Sales Invoice Item",
	...
	docs_with_deferred_expense = saashq.db.get_all(
	...
	if disallowed_types := voucher_types.difference(allowed_types):
		...
		saashq.throw(
			_("{0} {1} not allowed to be reposted. Modify {2} to enable reposting.").format(
				saashq.bold(comma_and(list(disallowed_types))),
				message,
				saashq.bold(
					saashq.utils.get_link_to_form(
...
def get_repost_allowed_types(doctype, txt, searchfield, start, page_len, filters):
	...
	if allowed_types := saashq.db.get_all(
