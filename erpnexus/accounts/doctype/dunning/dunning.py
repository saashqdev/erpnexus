...
class Dunning(AccountsController):
	...
	if TYPE_CHECKING:
		...
		from erpnexus.accounts.doctype.overdue_payment.overdue_payment import OverduePayment
	...
	def validate_same_currency(self):
		...
		for row in self.overdue_payments:
			invoice_currency = saashq.get_value("Sales Invoice", row.sales_invoice, "currency")
	...
	for reference in doc.references:
		...
		if reference.reference_doctype == "Sales Invoice" and (
			...
			for dunning in dunnings:
				resolve = True
				dunning = saashq.get_doc("Dunning", dunning.get("name"))
				for overdue_payment in dunning.overdue_payments:
					outstanding_inv = saashq.get_value(
						"Sales Invoice", overdue_payment.sales_invoice, "outstanding_amount"
					)
					outstanding_ps = saashq.get_value(
...
def get_linked_dunnings_as_per_state(sales_invoice, state):
	dunning = saashq.qb.DocType("Dunning")
	overdue_payment = saashq.qb.DocType("Overdue Payment")

	return (
		saashq.qb.from_(dunning)
...
def get_dunning_letter_text(dunning_type: str, doc: str | dict, language: str | None = None) -> dict:
	...
	if language:
		letter_text = saashq.db.get_value(
	...
	return {
		"body_text": saashq.render_template(letter_text.body_text, doc),
		"closing_text": saashq.render_template(letter_text.closing_text, doc),
