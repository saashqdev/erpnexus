...
from erpnexus import get_default_company, get_default_cost_center
...
class InvoiceCancelled(saashq.ValidationError):
	pass


class InvoiceNotCancelled(saashq.ValidationError):
	pass
...
class Subscription(Document):
	...
	def validate_plans_billing_cycle(billing_cycle_data: list[dict[str, str]]) -> None:
		...
		if billing_cycle_data and len(billing_cycle_data) != 1:
			saashq.throw(_("You can only have Plans with the same billing cycle in a Subscription"))
	...
	def get_billing_cycle_and_interval(self) -> list[dict[str, str]]:
		...
		subscription_plan = saashq.qb.DocType("Subscription Plan")
		billing_info = (
			saashq.qb.from_(subscription_plan)
	...
	def get_status_for_past_grace_period(self) -> str:
		cancel_after_grace = cint(saashq.get_value("Subscription Settings", None, "cancel_after_grace"))
		status = "Unpaid"
	...
	def is_past_grace_period(self, posting_date: DateTimeLikeObject | None = None) -> bool:
		...
		grace_period = cint(saashq.get_value("Subscription Settings", None, "grace_period"))
	...
	def validate_party_billing_currency(self):
		...
		"""
		if self.party:
			party_billing_currency = saashq.get_cached_value(
				self.party_type, self.party, "default_currency"
			) or saashq.get_cached_value("Company", self.company, "default_currency")

			plans = [x.plan for x in self.plans]
			subscription_plan_currencies = saashq.db.get_all(
			...
			if unsupported_plans:
				unsupported_plans = [
					...
					).format(saashq.bold(party_billing_currency)),
					*unsupported_plans,
				]

				saashq.throw(
					unsupported_plans, saashq.ValidationError, "Unsupported Subscription Plans", as_list=True
				)
	...
	def validate_trial_period(self) -> None:
		...
		if self.trial_period_start and self.trial_period_end:
			if getdate(self.trial_period_end) < getdate(self.trial_period_start):
				saashq.throw(_("Trial Period End Date Cannot be before Trial Period Start Date"))

		if self.trial_period_start and not self.trial_period_end:
			saashq.throw(_("Both Trial Period Start Date and Trial Period End Date must be set"))

		if self.trial_period_start and getdate(self.trial_period_start) > getdate(self.start_date):
			saashq.throw(_("Trial Period Start date cannot be after Subscription Start Date"))
	...
	def validate_to_follow_calendar_months(self) -> None:
		...
		if not self.end_date:
			saashq.throw(_("Subscription End Date is mandatory to follow calendar months"))

		if billing_info[0]["billing_interval"] != "Month":
			saashq.throw(_("Billing Interval in Subscription Plan must be Month to follow calendar months"))
	...
	def create_invoice(
		...
		invoice = saashq.new_doc(self.invoice_document_type)
		...
		if self.invoice_document_type == "Sales Invoice":
			...
			if saashq.db.get_value("Supplier", self.party, "tax_withholding_category"):
				invoice.apply_tds = 1
	...
	def get_items_from_plans(self, plans: list[dict[str, str]], prorate: bool | None = None) -> list[dict]:
		...
		party = self.party
		for plan in plans:
			plan_doc = saashq.get_doc("Subscription Plan", plan.plan)
			...
			deferred = saashq.db.get_value("Item", item_code, deferred_field)
	...
	def get_current_invoice(self) -> Document | None:
		...
		if invoice:
			return saashq.get_doc(self.invoice_document_type, invoice[0])
	...
	def cancel_subscription(self) -> None:
		...
		if self.status == "Cancelled":
			saashq.throw(_("subscription is already cancelled."), InvoiceCancelled)
	...
	def restart_subscription(self, posting_date: DateTimeLikeObject | None = None) -> None:
		...
		"""
		if self.status != "Cancelled":
			saashq.throw(_("You cannot restart a Subscription that is not cancelled."), InvoiceNotCancelled)
	...
	def force_fetch_subscription_updates(self):
		...
		if nowdate() < self.current_invoice_start:
			saashq.msgprint(_("Subscription for Future dates cannot be processed."))
			return
...
def is_prorate() -> int:
	return cint(saashq.db.get_single_value("Subscription Settings", "prorate"))
...
def process_all(subscription: str | None = None, posting_date: DateTimeLikeObject | None = None) -> None:
	...
	for subscription in saashq.get_all("Subscription", filters, pluck="name"):
		try:
			subscription = saashq.get_doc("Subscription", subscription)
