...
from erpnexus.accounts.doctype.subscription.subscription import get_prorata_factor
...
class TestSubscription(IntegrationTestCase):
	...
	def test_create_subscription_trial_with_wrong_dates(self):
		...
		self.assertRaises(saashq.ValidationError, subscription.save)
	...
	def test_subscription_cancel_after_grace_period(self):
		settings = saashq.get_single("Subscription Settings")
	...
	def test_restart_active_subscription(self):
		subscription = create_subscription()
		self.assertRaises(saashq.ValidationError, subscription.restart_subscription)
	...
	def test_subscription_with_follow_calendar_months(self):
		subscription = saashq.new_doc("Subscription")
	...
	def test_multi_currency_subscription(self):
		...
		saashq.db.set_value("Customer", party, "default_currency", "USD")
		subscription = create_subscription(
			start_date="2018-01-01",
			generate_invoice_at="Beginning of the current subscription period",
		...
		currency = saashq.db.get_value("Sales Invoice", subscription.invoices[0].name, "currency")
	...
	def test_subscription_recovery(self):
		...
		self.assertEqual(len(subscription.invoices), 2)
		self.assertEqual(
			getdate(saashq.db.get_value("Sales Invoice", subscription.invoices[0].name, "from_date")),
			getdate("2021-01-01"),
		)
		self.assertEqual(
			getdate(saashq.db.get_value("Sales Invoice", subscription.invoices[1].name, "from_date")),
			getdate("2021-02-01"),
		)
...
def create_plan(**kwargs):
	if not saashq.db.exists("Subscription Plan", kwargs.get("plan_name")):
		plan = saashq.new_doc("Subscription Plan")
...
def create_parties():
	if not saashq.db.exists("Supplier", "_Test Supplier"):
		supplier = saashq.new_doc("Supplier")
		supplier.supplier_name = "_Test Supplier"
		supplier.supplier_group = "All Supplier Groups"
		supplier.insert()

	if not saashq.db.exists("Customer", "_Test Subscription Customer"):
	...
	if not saashq.db.exists("Customer", "_Test Subscription Customer John Doe"):
		customer = saashq.new_doc("Customer")
