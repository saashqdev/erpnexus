...
class TestTaxWithholdingCategory(IntegrationTestCase):
	...
	def test_single_threshold_tds(self):
		...
		gl_entries = saashq.db.get_all("GL Entry", filters={"voucher_no": pi.name}, fields=["*"])
	...
	def test_tax_withholding_category_checks(self):
		invoices = []
		saashq.db.set_value("Supplier", "Test TDS Supplier3", "tax_withholding_category", "New TDS Category")
			...
			[
				{
					...
					"item_code": saashq.db.get_value("Item", {"item_name": "TDS Item"}, "name"),
					"qty": 1,
					"rate": 20000,
					"cost_center": "Main - _TC",
	...
	def set_previous_fy_and_tax_category(self):
		...
		if prev_fiscal_year := get_fiscal_year(add_days(fiscal_year[1], -10)):
			self.prev_fy = saashq.get_doc("Fiscal Year", prev_fiscal_year[0])
		...
		cat = saashq.get_doc("Tax Withholding Category", category)
		cat.append(
			"rates",
			{
...
def cancel_invoices():
	purchase_invoices = saashq.get_all(
		"Purchase Invoice",
		{
	...
	sales_invoices = saashq.get_all(
		"Sales Invoice", {"customer": "Test TCS Customer", "docstatus": 1}, pluck="name"
	)

	for d in purchase_invoices:
		saashq.get_doc("Purchase Invoice", d).cancel()

	for d in sales_invoices:
		saashq.get_doc("Sales Invoice", d).cancel()


def create_purchase_invoice(**args):
	# return sales invoice doc object
	item = saashq.db.get_value("Item", {"item_name": "TDS Item"}, "name")
...
def create_sales_invoice(**args):
	# return sales invoice doc object
	item = saashq.db.get_value("Item", {"item_name": "TCS Item"}, "name")
...
def create_records():
	...
	for name in [
		...
		if saashq.db.exists("Supplier", name):
			continue
	...
	# create item
	if not saashq.db.exists("Item", "TDS Item"):
		saashq.get_doc(
			{
				"doctype": "Item",
				"item_code": "TDS Item",
	...
	if not saashq.db.exists("Item", "TCS Item"):
		saashq.get_doc(
			{
				"doctype": "Item",
				"item_code": "TCS Item",
	...
	# create tds account
	if not saashq.db.exists("Account", "TDS - _TC"):
		saashq.get_doc(
			{
				"doctype": "Account",
				"company": "_Test Company",
	...
	# create tcs account
	if not saashq.db.exists("Account", "TCS - _TC"):
		saashq.get_doc(
			{
				"doctype": "Account",
				"company": "_Test Company",
...
def create_tax_withholding_category(
	...
	if not saashq.db.exists("Tax Withholding Category", category_name):
		saashq.get_doc(
			{
...
def create_lower_deduction_certificate(supplier, tax_withholding_category, tax_rate, certificate_no, limit):
	fiscal_year = get_fiscal_year(today(), company="_Test Company")
	if not saashq.db.exists("Lower Deduction Certificate", certificate_no):
		saashq.get_doc(
			{
