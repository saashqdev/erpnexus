saashq.ui.form.on("Chart of Accounts Importer", {
	onload: function (frm) {
		...
		var d = new saashq.ui.Dialog({
			...
			primary_action: function () {
				...
				if (!data.template_type) {
					saashq.throw(__("Please select <b>Template Type</b> to download template"));
				}

				open_url_post(
					"/api/method/erpnexus.accounts.doctype.chart_of_accounts_importer.chart_of_accounts_importer.download_template",
					{
	...
	company: function (frm) {
		if (frm.doc.company) {
			...
			saashq.call({
				method: "erpnexus.accounts.doctype.chart_of_accounts_importer.chart_of_accounts_importer.validate_company",
				args: {
...
var create_import_button = function (frm) {
	frm.page
		.set_primary_action(__("Import"), function () {
			return saashq.call({
				method: "erpnexus.accounts.doctype.chart_of_accounts_importer.chart_of_accounts_importer.import_coa",
				args: {
...
var validate_coa = function (frm) {
	if (frm.doc.import_file) {
		...
		return saashq.call({
			method: "erpnexus.accounts.doctype.chart_of_accounts_importer.chart_of_accounts_importer.get_coa",
			args: {
				file_name: frm.doc.import_file,
				parent: parent,
