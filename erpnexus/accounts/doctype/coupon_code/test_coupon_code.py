...
def test_create_test_data():
	...
	# create test item
	if not saashq.db.exists("Item", "_Test Tesla Car"):
		item = saashq.get_doc(
			{
	...
	if not saashq.db.exists("Pricing Rule", {"title": "_Test Pricing Rule for _Test Item"}):
		item_pricing_rule = saashq.get_doc(
			{
	...
	if not saashq.db.exists("Sales Partner", "_Test Coupon Partner"):
		sales_partner = saashq.get_doc(
			{
	...
	if not saashq.db.exists("Coupon Code", "SAVE30"):
		pricing_rule = saashq.db.get_value(
			"Pricing Rule", {"title": "_Test Pricing Rule for _Test Item"}, ["name"]
		)
		coupon_code = saashq.get_doc(
			{
				"doctype": "Coupon Code",
				"coupon_name": "SAVE30",
				"coupon_code": "SAVE30",
...
class TestCouponCode(IntegrationTestCase):
	...
	def test_sales_order_with_coupon_code(self):
		saashq.db.set_value("Coupon Code", "SAVE30", "used", 0)
		...
		so.submit()
		self.assertEqual(saashq.db.get_value("Coupon Code", "SAVE30", "used"), 1)
