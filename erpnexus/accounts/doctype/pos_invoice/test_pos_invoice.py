...
from erpnexus.accounts.doctype.pos_invoice.pos_invoice import make_sales_return
...
class TestPOSInvoice(IntegrationTestCase):
	...
	def tearDown(self):
		...
		if saashq.db.get_single_value("Selling Settings", "validate_selling_price"):
			saashq.db.set_single_value("Selling Settings", "validate_selling_price", 0)
	...
	def test_change_naming_series(self):
		...
		self.assertRaises(saashq.CannotChangeConstantError, inv.save)
	...
	def test_without_payment(self):
		...
		inv.payments = []
		self.assertRaises(saashq.ValidationError, inv.insert)
	...
	def test_serialized_item_transaction(self):
		...
		pos2.insert()
		self.assertRaises(saashq.ValidationError, pos2.submit)
	...
	def test_invalid_serial_no_validation(self):
		...
		self.assertRaises(saashq.ValidationError, pos.insert)
	...
	def test_value_error_on_serial_no_validation(self):
		...
		saashq.db.set_value("POS Invoice", pos.name, "docstatus", 2)
	...
	def test_loyalty_points(self):
		...
		from erpnexus.accounts.doctype.loyalty_program.test_loyalty_program import create_records
	...
	def test_merging_into_sales_invoice_with_discount(self):
		from erpnexus.accounts.doctype.pos_closing_entry.test_pos_closing_entry import (
		...
		saashq.db.sql("delete from `tabPOS Invoice`")
		...
		rounded_total = saashq.db.get_value("Sales Invoice", pos_inv.consolidated_invoice, "rounded_total")
	...
	def test_merging_with_validate_selling_price(self):
		...
		if not saashq.db.get_single_value("Selling Settings", "validate_selling_price"):
			saashq.db.set_single_value("Selling Settings", "validate_selling_price", 1)
		...
		self.assertRaises(saashq.ValidationError, pos_inv.submit)
		...
		rounded_total = saashq.db.get_value("Sales Invoice", pos_inv2.consolidated_invoice, "rounded_total")
	...
	def test_pos_batch_reservation(self):
		...
		batches = get_auto_batch_nos(
			saashq._dict({"item_code": "_BATCH ITEM Test For Reserve", "warehouse": "_Test Warehouse - _TC"})
		)
	...
	def test_pos_batch_item_qty_validation(self):
		...
		item = saashq.get_doc("Item", "_BATCH ITEM")
		...
		sn_doc = SerialBatchCreation(
			{
				...
				"qty": 2,
				"avg_rate": 300,
				"batches": saashq._dict({"TestBatch 01": 2}),
	...
	def test_ignore_pricing_rule(self):
		from erpnexus.accounts.doctype.pricing_rule.test_pricing_rule import make_pricing_rule
	...
	def test_delivered_serial_no_case(self):
		from erpnexus.accounts.doctype.pos_invoice_merge_log.test_pos_invoice_merge_log import (
			init_user_and_profile,
		)
		from erpnexus.stock.doctype.delivery_note.test_delivery_note import create_delivery_note
		from erpnexus.stock.doctype.stock_entry.test_stock_entry import make_serialized_item

		saashq.db.savepoint("before_test_delivered_serial_no_case")
		try:
			...
			saashq.db.rollback(save_point="before_test_delivered_serial_no_case")
			saashq.set_user("Administrator")
...
def create_pos_invoice(**args):
	...
	pos_inv = saashq.new_doc("POS Invoice")
	pos_inv.update(args)
	...
	pos_inv.posting_date = args.posting_date or saashq.utils.nowdate()
...
def make_batch_item(item_name):
	...
	if not saashq.db.exists(item_name):
