...
from saashq.utils import cint, flt, get_link_to_form, getdate, nowdate
from saashq.utils.nestedset import get_descendants_of

from erpnexus.accounts.doctype.loyalty_program.loyalty_program import validate_loyalty_points
...
from erpnexus.controllers.queries import item_query as _item_query
...
class POSInvoice(SalesInvoice):
	...
	if TYPE_CHECKING:
		...
		from erpnexus.accounts.doctype.pos_invoice_item.pos_invoice_item import POSInvoiceItem
		from erpnexus.accounts.doctype.pricing_rule_detail.pricing_rule_detail import PricingRuleDetail
		from erpnexus.accounts.doctype.sales_invoice_advance.sales_invoice_advance import (
			SalesInvoiceAdvance,
		)
		from erpnexus.accounts.doctype.sales_invoice_payment.sales_invoice_payment import (
			SalesInvoicePayment,
		)
	...
	def validate(self):
		if not cint(self.is_pos):
			saashq.throw(
				_("POS Invoice should have the field {0} checked.").format(saashq.bold(_("Include Payment")))
			)
	...
	def on_submit(self):
		...
		if not self.is_return and self.loyalty_program:
			...
			against_psi_doc = saashq.get_doc("POS Invoice", self.return_against)
	...
	def before_cancel(self):
		if (
			...
			and saashq.db.get_value("Sales Invoice", self.consolidated_invoice, "docstatus") == 1
		):
	...
	def delink_serial_and_batch_bundle(self):
		for row in self.items:
			if row.serial_and_batch_bundle:
				...
				saashq.get_doc("Serial and Batch Bundle", row.serial_and_batch_bundle).cancel()
				row.db_set("serial_and_batch_bundle", None)

	def submit_serial_batch_bundle(self, table_name):
		for item in self.get(table_name):
			if item.serial_and_batch_bundle:
				doc = saashq.get_doc("Serial and Batch Bundle", item.serial_and_batch_bundle)
	...
	def validate_stock_availablility(self):
		...
		if self.docstatus.is_draft() and not saashq.db.get_value(
	...
	def validate_serialised_or_batched_item(self):
		...
		if error_msg:
			saashq.throw(error_msg, title=_("Serial / Batch Bundle Missing"), as_list=True)
		...
		for d in self.get("items"):
			if d.get("qty") > 0:
				saashq.throw(
					...
					).format(d.idx, saashq.bold(d.item_code)),
					title=_("Invalid Item"),
				)
			if d.get("serial_no"):
				serial_nos = get_serial_nos(d.serial_no)
				for sr in serial_nos:
					serial_no_exists = saashq.db.sql(
						"""
						SELECT name
					...
					if not serial_no_exists:
						bold_return_against = saashq.bold(self.return_against)
						bold_serial_no = saashq.bold(sr)
						saashq.throw(
							_(
	...
	def validate_change_account(self):
		if (
			...
			and saashq.get_cached_value("Account", self.account_for_change_amount, "company") != self.company
		):
			saashq.throw(
	...
	def validate_change_amount(self):
		...
		if flt(self.change_amount) and not self.account_for_change_amount:
			saashq.msgprint(_("Please enter Account for Change Amount"), raise_exception=1)
	...
	def validate_company_with_pos_company(self):
		if self.company != saashq.db.get_value("POS Profile", self.pos_profile, "company"):
			saashq.throw(
				_("Company {} does not match with POS Profile Company {}").format(
					self.company, saashq.db.get_value("POS Profile", self.pos_profile, "company")
				)
			)
	...
	def validate_loyalty_transaction(self):
		if self.redeem_loyalty_points and (
			...
			expense_account, cost_center = saashq.db.get_value(
	...
	def set_pos_fields(self, for_validate=False):
		...
		if not self.pos_profile:
			...
			if not pos_profile:
				saashq.throw(_("No POS Profile found. Please create a New POS Profile first"))
			self.pos_profile = pos_profile.get("name")

		profile = {}
		if self.pos_profile:
			profile = saashq.get_doc("POS Profile", self.pos_profile)
		...
		if profile:
			...
			if self.customer:
				customer_price_list, customer_group, customer_currency = saashq.db.get_value(
					"Customer", self.customer, ["default_price_list", "customer_group", "default_currency"]
				)
				customer_group_price_list = saashq.get_cached_value(
			...
			for item in self.get("items"):
				if item.get("item_code"):
					profile_details = get_pos_profile_item_details(
						profile.get("company"), saashq._dict(item.as_dict()), profile
					)
	...
	def reset_mode_of_payments(self):
		if self.pos_profile:
			pos_profile = saashq.get_cached_doc("POS Profile", self.pos_profile)
	...
	def create_payment_request(self):
		for pay in self.payments:
			if pay.type == "Phone":
				if pay.amount <= 0:
					saashq.throw(_("Payment amount cannot be less than or equal to 0"))

				if not self.contact_mobile:
					saashq.throw(_("Please enter the phone number first"))
	...
	def get_new_payment_request(self, mop):
		payment_gateway_account = saashq.db.get_value(
	...
	def get_existing_payment_request(self, pay):
		...
		pr = saashq.db.get_value("Payment Request", filters=filters)
		if pr:
			return saashq.get_doc("Payment Request", pr)


@saashq.whitelist()
def get_stock_availability(item_code, warehouse):
	if saashq.db.get_value("Item", item_code, "is_stock_item"):
		is_stock_item = True
		...
		is_stock_item = True
		if saashq.db.exists("Product Bundle", {"name": item_code, "disabled": 0}):
...
def get_bundle_availability(bundle_item_code, warehouse):
	product_bundle = saashq.get_doc("Product Bundle", bundle_item_code)
	...
	for item in product_bundle.items:
		...
		if bundle_bin_qty > max_available_bundles and saashq.get_value(
...
def get_pos_reserved_qty(item_code, warehouse):
	p_inv = saashq.qb.DocType("POS Invoice")
	p_item = saashq.qb.DocType("POS Invoice Item")

	reserved_qty = (
		saashq.qb.from_(p_inv)
		.from_(p_item)
...
def make_merge_log(invoices):
	...
	if len(invoices) == 0:
		saashq.throw(_("At least one invoice has to be selected."))

	merge_log = saashq.new_doc("POS Invoice Merge Log")
	merge_log.posting_date = getdate(nowdate())
	for inv in invoices:
		inv_data = saashq.db.get_values(
