...
class TransactionExists(saashq.ValidationError):
	pass
...
class PromotionalScheme(Document):
	...
	if TYPE_CHECKING:
		...
		from erpnexus.accounts.doctype.campaign_item.campaign_item import CampaignItem
		from erpnexus.accounts.doctype.customer_group_item.customer_group_item import CustomerGroupItem
		from erpnexus.accounts.doctype.customer_item.customer_item import CustomerItem
		from erpnexus.accounts.doctype.pricing_rule_brand.pricing_rule_brand import PricingRuleBrand
		from erpnexus.accounts.doctype.pricing_rule_item_code.pricing_rule_item_code import (
			PricingRuleItemCode,
		)
		from erpnexus.accounts.doctype.pricing_rule_item_group.pricing_rule_item_group import (
			PricingRuleItemGroup,
		)
		from erpnexus.accounts.doctype.promotional_scheme_price_discount.promotional_scheme_price_discount import (
			PromotionalSchemePriceDiscount,
		)
		from erpnexus.accounts.doctype.promotional_scheme_product_discount.promotional_scheme_product_discount import (
			PromotionalSchemeProductDiscount,
		)
		from erpnexus.accounts.doctype.sales_partner_item.sales_partner_item import SalesPartnerItem
		from erpnexus.accounts.doctype.supplier_group_item.supplier_group_item import SupplierGroupItem
		from erpnexus.accounts.doctype.supplier_item.supplier_item import SupplierItem
		from erpnexus.accounts.doctype.territory_item.territory_item import TerritoryItem
	...
	def validate(self):
		if not self.selling and not self.buying:
			saashq.throw(_("Either 'Selling' or 'Buying' must be selected"), title=_("Mandatory"))
		if not (self.price_discount_slabs or self.product_discount_slabs):
			saashq.throw(_("Price or product discount slabs are required"))
	...
	def validate_applicable_for(self):
		if self.applicable_for:
			applicable_for = saashq.scrub(self.applicable_for)

			if not self.get(applicable_for):
				msg = f"The field {saashq.bold(self.applicable_for)} is required"
				saashq.throw(_(msg))
	...
	def validate_pricing_rules(self):
		...
		for doc in invalid_pricing_rule:
			saashq.delete_doc("Pricing Rule", doc)
	...
	def get_invalid_pricing_rules(self):
		pr = saashq.qb.DocType("Pricing Rule")
		conditions = []
		...
		return saashq.qb.from_(pr).select(pr.name).where(Criterion.all(conditions)).run(pluck=True)
	...
	def update_pricing_rules(self, pricing_rules):
		...
		for doc in docs:
			...
			if doc.get("__islocal"):
				...
				doc.save()
				saashq.msgprint(_("Pricing Rule {0} is updated").format(doc.name))

		if count:
			saashq.msgprint(_("New {0} pricing rules are created").format(count))

	def on_trash(self):
		for rule in saashq.get_all("Pricing Rule", {"promotional_scheme": self.name}):
			saashq.delete_doc("Pricing Rule", rule.name)


def raise_for_transaction_exists(name):
	msg = f"""You can't change the {saashq.bold(_('Applicable For'))}
		because transactions are present against the Promotional Scheme {saashq.bold(name)}. """
	msg += "Kindly disable this Promotional Scheme and create new for new Applicable For."

	saashq.throw(_(msg), TransactionExists)
...
def _get_pricing_rules(doc, child_doc, discount_fields, rules=None):
	...
	applicable_for = saashq.scrub(doc.get("applicable_for"))
...
def get_pricing_rule_docname(
	...
	docname = saashq.get_all("Pricing Rule", fields=fields, filters=filters)
...
def prepare_pricing_rule(
	...
	if docname:
		pr = saashq.get_doc("Pricing Rule", docname)
	else:
		pr = saashq.new_doc("Pricing Rule")
...
def set_args(args, pr, doc, child_doc, discount_fields, child_doc_fields):
	...
	for field in ["items", "item_groups", "brands"]:
		...
		apply_on = saashq.scrub(doc.get("apply_on"))
