...
saashq.ui.form.on("Process Deferred Accounting", {
	setup: function (frm) {
	...
	end_date: function (frm) {
		if (frm.doc.end_date && frm.doc.end_date < frm.doc.start_date) {
			saashq.throw(__("End date cannot be before start date"));
		}
	},

	onload: function (frm) {
		if (frm.doc.posting_date && frm.doc.docstatus === 0) {
			frm.set_value("start_date", saashq.datetime.add_months(frm.doc.posting_date, -1));
