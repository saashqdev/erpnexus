...
from saashq.utils import add_to_date, formatdate, get_link_to_form, getdate, nowdate
from saashq.utils.dashboard import cache_source
from saashq.utils.dateutils import get_from_date_from_timespan, get_period_ending
...
def get(
	...
	if chart_name:
		chart = saashq.get_doc("Dashboard Chart", chart_name)
	else:
	...
	if not saashq.db.exists("Account", account) and chart_name:
		saashq.throw(
...
def build_result(account, dates, gl_entries):
	...
	root_type = saashq.get_cached_value("Account", account, "root_type")
