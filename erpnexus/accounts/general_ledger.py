...
from saashq.utils import cint, flt, formatdate, get_link_to_form, getdate, now
...
from erpnexus.accounts.doctype.accounting_dimension_filter.accounting_dimension_filter import (
	get_dimension_filter_map,
)
from erpnexus.accounts.doctype.accounting_period.accounting_period import ClosedAccountingPeriod
from erpnexus.accounts.doctype.budget.budget import validate_expense_against_budget
from erpnexus.accounts.utils import create_payment_ledger_entry
from erpnexus.exceptions import InvalidAccountDimensionError, MandatoryAccountDimensionError
...
def get_accounting_dimensions_for_offsetting_entry(gl_map, company):
	acc_dimension = saashq.qb.DocType("Accounting Dimension")
	dimension_detail = saashq.qb.DocType("Accounting Dimension Detail")

	acc_dimensions = (
		saashq.qb.from_(acc_dimension)
...
def validate_disabled_accounts(gl_map):
	...
	disabled_accounts = saashq.get_all(
		"Account",
	...
	if used_disabled_accounts:
		account_list = "<br>"
		account_list += ", ".join([saashq.bold(d) for d in used_disabled_accounts])
		saashq.throw(
...
def validate_accounting_period(gl_map):
	accounting_periods = saashq.db.sql(
		""" SELECT
			ap.name as name
		FROM
	...
	if accounting_periods:
		saashq.throw(
			...
			).format(saashq.bold(accounting_periods[0].name)),
...
def get_cost_center_allocation_data(company, posting_date, cost_center):
	cost_center_allocation = saashq.db.get_value(
...
def merge_similar_entries(gl_map, precision=None):
	...
	company = gl_map[0].company if gl_map else erpnexus.get_default_company()
	company_currency = erpnexus.get_company_currency(company)

	if not precision:
		precision = get_field_precision(saashq.get_meta("GL Entry").get_field("debit"), company_currency)
	...
	merged_gl_map = filter(
		...
		or (
			...
			and saashq.get_cached_value("Journal Entry", x.voucher_no, "voucher_type")
...
def validate_cwip_accounts(gl_map):
	...
	cwip_enabled = any(
		...
		for ac in saashq.db.get_all("Asset Category", "enable_cwip_accounting")
	)
	if cwip_enabled:
		cwip_accounts = [
			d[0]
...
def process_debit_credit_difference(gl_map):
	precision = get_field_precision(
		saashq.get_meta("GL Entry").get_field("debit"),
		currency=saashq.get_cached_value("Company", gl_map[0].company, "default_currency"),
	)
	...
	if abs(debit_credit_diff) > allowance:
		if not (
			...
			and saashq.get_cached_value("Journal Entry", voucher_no, "voucher_type")
			== "Exchange Gain Or Loss"
		):
		...
		if not round_off_for_opening:
			saashq.throw(
				_("Please set '{0}' in Company: {1}").format(
					saashq.bold("Round Off for Opening"), get_link_to_form("Company", gl_map[0].company)
				)
			)
...
def update_accounting_dimensions(round_off_gle):
	...
	meta = saashq.get_meta(round_off_gle["voucher_type"])
	...
	if dimensions and has_all_dimensions:
		dimension_values = saashq.db.get_value(
...
def get_round_off_account_and_cost_center(company, voucher_type, voucher_no, use_company_default=False):
	round_off_account, round_off_cost_center, round_off_for_opening = saashq.get_cached_value(
		"Company", company, ["round_off_account", "round_off_cost_center", "round_off_for_opening"]
	) or [None, None, None]

	# Use expense account as fallback
	if not round_off_account:
		round_off_account = saashq.get_cached_value("Company", company, "default_expense_account")

	meta = saashq.get_meta(voucher_type)

	# Give first preference to parent cost center for round off GLE
	if not use_company_default and meta.has_field("cost_center"):
		parent_cost_center = saashq.db.get_value(voucher_type, voucher_no, "cost_center")
	...
	if not round_off_account:
		saashq.throw(
			_("Please mention '{0}' in Company: {1}").format(
				saashq.bold("Round Off Account"), get_link_to_form("Company", company)
			)
		)
	...
	if not round_off_cost_center:
		saashq.throw(
			_("Please mention '{0}' in Company: {1}").format(
				saashq.bold("Round Off Cost Center"), get_link_to_form("Company", company)
			)
		)
	...
	if gl_entries:
		...
		for entry in gl_entries:
			...
			if immutable_ledger_enabled:
				...
				new_gle["posting_date"] = saashq.form_dict.get("posting_date") or getdate()
	...
	if not adv_adj:
		...
		if acc_frozen_upto:
			...
			if getdate(posting_date) <= getdate(acc_frozen_upto) and (
				frozen_accounts_modifier not in saashq.get_roles() or saashq.session.user == "Administrator"
			):
				saashq.throw(
...
def validate_against_pcv(is_opening, posting_date, company):
	if is_opening and saashq.db.exists("Period Closing Voucher", {"docstatus": 1, "company": company}):
		saashq.throw(
			_("Opening Entry can not be created after Period Closing Voucher is created."),
			title=_("Invalid Opening Entry"),
		)

	last_pcv_date = saashq.db.get_value(
	...
	if last_pcv_date and getdate(posting_date) <= getdate(last_pcv_date):
		...
		saashq.throw(message, title=_("Period Closed"))
			...
			if value["is_mandatory"] and not gl_entry.get(dimension):
				saashq.throw(
					_("{0} is mandatory for account {1}").format(
						saashq.bold(saashq.unscrub(dimension)), saashq.bold(gl_entry.account)
					),
				...
				if gl_entry.get(dimension) and gl_entry.get(dimension) not in value["allowed_dimensions"]:
					saashq.throw(
						_("Invalid value {0} for {1} against account {2}").format(
							saashq.bold(gl_entry.get(dimension)),
							saashq.bold(saashq.unscrub(dimension)),
							saashq.bold(gl_entry.account),
						),
...
def is_immutable_ledger_enabled():
	return saashq.db.get_single_value("Accounts Settings", "enable_immutable_ledger")
