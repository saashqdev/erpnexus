...
import saashq
from saashq import _, msgprint, qb, scrub
from saashq.contacts.doctype.address.address import get_company_address, get_default_address
from saashq.core.doctype.user_permission.user_permission import get_permitted_documents
from saashq.model.utils import get_fetch_values
from saashq.query_builder.functions import Abs, Count, Date, Sum
...
from erpnexus.exceptions import InvalidAccountCurrency, PartyDisabled, PartyFrozen
...
class DuplicatePartyAccountError(saashq.ValidationError):
	pass
...
def get_party_details(
	...
	if not saashq.db.exists(party_type, party):
		saashq.throw(_("{0}: {1} does not exists").format(party_type, party))
...
def _get_party_details(
	...
	party_details = saashq._dict(
		set_account_and_due_date(party, account, party_type, company, posting_date, bill_date, doctype)
	)
	party = party_details[party_type.lower()]
	party = saashq.get_doc(party_type, party)

	if not ignore_permissions:
		ptype = "select" if saashq.only_has_select_perm(party_type) else "read"
		saashq.has_permission(party_type, ptype, party, throw=True)
	...
	if party_type == "Supplier" and party:
		party_details["supplier_tds"] = saashq.get_value(party_type, party.name, "tax_withholding_category")

	if not party_details.get("tax_category") and pos_profile:
		party_details["tax_category"] = saashq.get_value("POS Profile", pos_profile, "tax_category")
...
def complete_contact_details(party_details):
	if not party_details.contact_person:
		...
		contact_details = saashq.db.get_value("Contact", party_details.contact_person, fields, as_dict=True)
...
def get_default_price_list(party):
	...
	if party.doctype == "Customer":
		return saashq.get_cached_value("Customer Group", party.customer_group, "default_price_list")
...
def set_price_list(party_details, party, party_type, given_price_list, pos=None):
	...
	if price_list and len(price_list) == 1:
		...
		customer_price_list = saashq.get_value("Customer", party.name, "default_price_list")

		if customer_price_list:
			price_list = customer_price_list
		else:
			pos_price_list = saashq.get_value("POS Profile", pos, "selling_price_list")
			price_list = pos_price_list or given_price_list
	else:
		price_list = get_default_price_list(party) or given_price_list

	if price_list:
		party_details.price_list_currency = saashq.db.get_value(
...
def get_party_account(party_type, party=None, company=None, include_advance=False):
	...
	if not party and party_type in ["Customer", "Supplier"]:
		...
		return saashq.get_cached_value("Company", company, default_account_name)
	...
	if not account and party_type in ["Customer", "Supplier"]:
		...
		group = saashq.get_cached_value(party_type, party, scrub(party_group_doctype))
		account = saashq.db.get_value(
			"Party Account",
	...
	if not account and party_type in ["Customer", "Supplier"]:
		...
		account = saashq.get_cached_value("Company", company, default_account_name)
...
def get_party_advance_account(party_type, party, company):
	...
	if not account:
		...
		account = saashq.get_cached_value("Company", company, account_name)
...
def get_party_bank_account(party_type, party):
	return saashq.db.get_value("Bank Account", {"party_type": party_type, "party": party, "is_default": 1})


def get_party_account_currency(party_type, party, company):
	def generator():
		party_account = get_party_account(party_type, party, company)
		return saashq.get_cached_value("Account", party_account, "account_currency")

	return saashq.local_cache("party_account_currency", (party_type, party, company), generator)
...
def get_party_gle_account(party_type, party, company):
	def generator():
		existing_gle_account = saashq.db.sql(
	...
	if existing_gle_currency and party_account_currency != existing_gle_currency:
		saashq.throw(
			_(
				...
				saashq.bold(party_type),
				saashq.bold(party),
				saashq.bold(existing_gle_currency),
				saashq.bold(company),
			),
...
def validate_party_accounts(doc):
	...
	for account in doc.get("accounts"):
		...
		party_account_currency = saashq.get_cached_value("Account", account.account, "account_currency")
		if saashq.db.get_default("Company"):
			company_default_currency = saashq.get_cached_value(
				"Company", saashq.db.get_default("Company"), "default_currency"
			)
		else:
			company_default_currency = saashq.get_cached_value("Company", account.company, "default_currency")
	...
	if (bill_date or posting_date) and party:
		...
		if template_name:
			...
			if party_type == "Supplier":
				supplier_group = saashq.get_cached_value(party_type, party, "supplier_group")
				template_name = saashq.get_cached_value("Supplier Group", supplier_group, "payment_terms")
				if template_name:
...
def get_due_date_from_template(template_name, posting_date, bill_date):
	...
	template = saashq.get_doc("Payment Terms Template", template_name)
...
def validate_due_date(posting_date, due_date, bill_date=None, template_name=None):
	if getdate(due_date) < getdate(posting_date):
		saashq.throw(_("Due Date cannot be before Posting / Supplier Invoice Date"))
	else:
		...
		if default_due_date != posting_date and getdate(due_date) > getdate(default_due_date):
			is_credit_controller = (
				saashq.db.get_single_value("Accounts Settings", "credit_controller") in saashq.get_roles()
			)
...
def get_address_tax_category(tax_category=None, billing_address=None, shipping_address=None):
	addr_tax_category_from = saashq.db.get_single_value(
		"Accounts Settings", "determine_address_tax_category_from"
	)
	if addr_tax_category_from == "Shipping Address":
		if shipping_address:
			tax_category = saashq.db.get_value("Address", shipping_address, "tax_category") or tax_category
	else:
		if billing_address:
			tax_category = saashq.db.get_value("Address", billing_address, "tax_category") or tax_category
...
def set_taxes(
	...
	from erpnexus.accounts.doctype.tax_rule.tax_rule import get_party_details, get_tax_template
	...
	if party_type in ("Customer", "Lead", "Prospect"):
		...
		if party_type in ["Lead", "Prospect"]:
			args["customer"] = None
			del args[saashq.scrub(party_type)]
	else:
...
def get_payment_terms_template(party_name, party_type, company=None):
	...
	if party_type == "Customer":
		customer = saashq.get_cached_value(
			"Customer", party_name, fieldname=["payment_terms", "customer_group"], as_dict=1
		)
		template = customer.payment_terms

		if not template and customer.customer_group:
			template = saashq.get_cached_value("Customer Group", customer.customer_group, "payment_terms")
	else:
		supplier = saashq.get_cached_value(
			"Supplier", party_name, fieldname=["payment_terms", "supplier_group"], as_dict=1
		)
		template = supplier.payment_terms
		if not template and supplier.supplier_group:
			template = saashq.get_cached_value("Supplier Group", supplier.supplier_group, "payment_terms")

	if not template and company:
		template = saashq.get_cached_value("Company", company, fieldname="payment_terms")
	return template


def validate_party_frozen_disabled(party_type, party_name):
	if saashq.flags.ignore_party_validation:
		return

	if party_type and party_name:
		if party_type in ("Customer", "Supplier"):
			party = saashq.get_cached_value(party_type, party_name, ["is_frozen", "disabled"], as_dict=True)
			if party.disabled:
				saashq.throw(_("{0} {1} is disabled").format(party_type, party_name), PartyDisabled)
			elif party.get("is_frozen"):
				frozen_accounts_modifier = saashq.db.get_single_value(
					"Accounts Settings", "frozen_accounts_modifier"
				)
				if frozen_accounts_modifier not in saashq.get_roles():
					saashq.throw(_("{0} {1} is frozen").format(party_type, party_name), PartyFrozen)

		elif party_type == "Employee":
			if saashq.db.get_value("Employee", party_name, "status") != "Active":
				saashq.msgprint(_("{0} {1} is not active").format(party_type, party_name), alert=True)
...
def get_dashboard_info(party_type, party, loyalty_program=None):
	...
	company_wise_grand_total = saashq.get_all(
		doctype,
		filters={
			"docstatus": 1,
			party_type.lower(): party,
	...
	if party_type == "Customer":
		loyalty_point_details = saashq._dict(
			saashq.get_all(
				"Loyalty Point Entry",
				filters={
					"customer": party,
	...
	company_wise_billing_this_year = saashq._dict()
	...
	company_wise_total_unpaid = saashq._dict(
		saashq.db.sql(
			"""
	...
	for d in companies:
		company_default_currency = saashq.get_cached_value("Company", d.company, "default_currency")
...
def get_party_shipping_address(doctype: str, name: str) -> str | None:
	...
	:return: String
	"""
	shipping_addresses = saashq.get_all(
		"Address",
		filters=[
...
def get_partywise_advanced_payment_amount(
	...
	account_type = saashq.get_cached_value("Party Type", party_type, "account_type")

	ple = saashq.qb.DocType("Payment Ledger Entry")
	acc = saashq.qb.DocType("Account")
	...
	if invoice_doctypes := saashq.get_hooks("invoice_doctypes"):
...
def add_party_account(party_type, party, company, account):
	doc = saashq.get_doc(party_type, party)
...
def render_address(address, check_permissions=True):
	try:
		from saashq.contacts.doctype.address.address import render_address as _render
	except ImportError:
		# Older saashq versions where this function is not available
		from saashq.contacts.doctype.address.address import get_address_display as _render

	return saashq.call(_render, address, check_permissions=check_permissions)
