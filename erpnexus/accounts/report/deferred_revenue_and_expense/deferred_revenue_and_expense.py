...
from saashq.query_builder import Column, functions
from saashq.utils import add_days, date_diff, flt, get_first_day, get_last_day, getdate, rounded
...
class Deferred_Item:
	...
	def report_data(self):
		...
		"""
		ret_data = saashq._dict({"name": self.item_name})
	...
	def make_dummy_gle(self, name, date, amount):
		"""
		return - saashq._dict() of a dummy gle entry
		"""
		entry = saashq._dict(
	...
	def calculate_item_revenue_expense_for_period(self):
		...
		for period in self.period_list:
			...
			self.period_total.append(saashq._dict({"key": period.key, "total": period_sum, "actual": actual}))
...
class Deferred_Invoice:
	def __init__(self, invoice, items, filters, period_list):
		...
		items - list : saashq._dict() with item details. Refer Deferred_Item for required fields
		"""
		self.name = invoice
	...
	def calculate_invoice_revenue_expense_for_period(self):
		...
		for period in self.period_list:
			self.period_total.append(saashq._dict({"key": period.key, "total": 0, "actual": 0}))
	...
	def report_data(self):
		...
		"""
		ret_data = []
		inv_total = saashq._dict({"name": self.name})
...
class Deferred_Revenue_and_Expense_Report:
	def __init__(self, filters=None):
		...
		if not filters:
			fiscal_year = saashq.get_doc("Fiscal Year", get_fiscal_year(date=getdate()))
			self.filters = saashq._dict(
				{
					"company": saashq.defaults.get_user_default("Company"),
	...
	def generate_report_data(self):
		...
		# add total row
		if self.filters.type == "Revenue":
			total_row = saashq._dict({"name": "Total Deferred Income"})
		elif self.filters.type == "Expense":
			total_row = saashq._dict({"name": "Total Deferred Expense"})
