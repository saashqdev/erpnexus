...
from saashq.utils import add_days, add_months, cint, cstr, flt, formatdate, get_first_day, getdate
...
def get_period_list(
	...
	for i in range(cint(math.ceil(months / months_to_add))):
		period = saashq._dict({"from_date": start_date})
...
def validate_fiscal_year(fiscal_year, from_fiscal_year, to_fiscal_year):
	if not fiscal_year.get("year_start_date") or not fiscal_year.get("year_end_date"):
		saashq.throw(_("Start Year and End Year are mandatory"))

	if getdate(fiscal_year.get("year_end_date")) < getdate(fiscal_year.get("year_start_date")):
		saashq.throw(_("End Year cannot be before Start Year"))


def validate_dates(from_date, to_date):
	if not from_date or not to_date:
		saashq.throw(_("From Date and To Date are mandatory"))

	if to_date < from_date:
		saashq.throw(_("To Date cannot be less than From Date"))
...
def get_accounting_entries(
	...
	gl_entry = saashq.qb.DocType(doctype)
	query = (
...
def get_account_filter_query(root_lft, root_rgt, root_type, gl_entry):
	...
	exists_query = (
		saashq.qb.from_(acc).select(acc.name).where(acc.name == gl_entry.account).where(acc.is_group == 0)
	)
...
def get_cost_centers_with_children(cost_centers):
	...
	for d in cost_centers:
		if saashq.db.exists("Cost Center", d):
			lft, rgt = saashq.db.get_value("Cost Center", d, ["lft", "rgt"])
			children = saashq.get_all("Cost Center", filters={"lft": [">=", lft], "rgt": ["<=", rgt]})
			all_cost_centers += [c.name for c in children]
		else:
			saashq.throw(_("Cost Center: {0} does not exist").format(d))
