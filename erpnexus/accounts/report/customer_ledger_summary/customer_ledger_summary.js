...
saashq.query_reports["Customer Ledger Summary"] = {
	filters: [
		{
			fieldname: "company",
			label: __("Company"),
		...
		},
		{
			...
			on_change: () => {
				...
				if (party) {
					saashq.db.get_value("Customer", party, ["tax_id", "customer_name"], function (value) {
						saashq.query_report.set_filter_value("tax_id", value["tax_id"]);
						saashq.query_report.set_filter_value("customer_name", value["customer_name"]);
					});
				} else {
					saashq.query_report.set_filter_value("tax_id", "");
					saashq.query_report.set_filter_value("customer_name", "");
				}
			},
		},
		{
