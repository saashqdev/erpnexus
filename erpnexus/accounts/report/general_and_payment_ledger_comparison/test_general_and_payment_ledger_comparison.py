...
from saashq.utils import add_days

from erpnexus.accounts.doctype.sales_invoice.test_sales_invoice import create_sales_invoice
from erpnexus.accounts.report.general_and_payment_ledger_comparison.general_and_payment_ledger_comparison import (
	execute,
)
...
class TestGeneralAndPaymentLedger(IntegrationTestCase, AccountsTestMixin):
	...
	def test_01_basic_report_functionality(self):
		...
		ple = saashq.db.get_all("Payment Ledger Entry", filters={"voucher_no": sinv.name, "delinked": 0})[0]
		saashq.db.set_value("Payment Ledger Entry", ple.name, "amount", sinv.grand_total - 1)

		filters = saashq._dict({"company": self.company})
		...
		# account filter
		filters = saashq._dict({"company": self.company, "account": self.debit_to})
		columns, data = execute(filters=filters)
		self.assertEqual(len(data), 1)
		self.assertEqual(expected, data[0])

		filters = saashq._dict({"company": self.company, "account": self.creditors})
		columns, data = execute(filters=filters)
		self.assertEqual([], data)

		# voucher_no filter
		filters = saashq._dict({"company": self.company, "voucher_no": sinv.name})
		columns, data = execute(filters=filters)
		self.assertEqual(len(data), 1)
		self.assertEqual(expected, data[0])

		filters = saashq._dict({"company": self.company, "voucher_no": sinv.name + "-1"})
		columns, data = execute(filters=filters)
		self.assertEqual([], data)
