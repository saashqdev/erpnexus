...
from saashq.utils import flt, formatdate, get_datetime_str, get_table_name
...
from erpnexus.accounts.doctype.fiscal_year.fiscal_year import get_from_and_to_date
...
def get_invoiced_item_gross_margin(sales_invoice=None, item_code=None, company=None, with_item_data=False):
	from erpnexus.accounts.report.gross_profit.gross_profit import GrossProfitGenerator

	sales_invoice = sales_invoice or saashq.form_dict.get("sales_invoice")
	item_code = item_code or saashq.form_dict.get("item_code")
	company = company or saashq.get_cached_value("Sales Invoice", sales_invoice, "company")
...
def get_party_details(party_type, party_list):
	party_details = {}
	party = saashq.qb.DocType(party_type)
	query = saashq.qb.from_(party).select(party.name, party.tax_id).where(party.name.isin(party_list))
...
def get_taxes_query(invoice_list, doctype, parenttype):
	taxes = saashq.qb.DocType(doctype)

	query = (
		saashq.qb.from_(taxes)
...
def get_journal_entries(filters, args):
	...
	journal_account = saashq.qb.DocType("Journal Entry Account")
	query = (
		saashq.qb.from_(je)
...
def apply_common_conditions(filters, query, doctype, child_doctype=None, payments=False):
	...
	if child_doctype:
		child_doc = saashq.qb.DocType(child_doctype)
...
def get_advance_taxes_and_charges(invoice_list):
	adv_taxes = saashq.qb.DocType("Advance Taxes and Charges")
	return (
		saashq.qb.from_(adv_taxes)
		.select(
			adv_taxes.parent,
