# Copyright (c) 2015, Saashq Technologies Pvt. Ltd.
...
import saashq
from saashq import _, qb, query_builder, scrub
from saashq.query_builder import Criterion
from saashq.query_builder.functions import Date, Substring, Sum
from saashq.utils import cint, cstr, flt, getdate, nowdate

from erpnexus.accounts.doctype.accounting_dimension.accounting_dimension import (
	get_accounting_dimensions,
	get_dimension_with_children,
)
from erpnexus.accounts.utils import get_currency_precision, get_party_types_from_account_type
...
class ReceivablePayableReport:
	...
	def run(self, args):
		...
		self.set_defaults()
		self.party_naming_by = saashq.db.get_single_value(args.get("naming_by")[0], args.get("naming_by")[1])
		self.get_columns()
		self.get_data()
		self.get_chart_data()
	...
	def set_defaults(self):
		if not self.filters.get("company"):
			self.filters.company = saashq.db.get_single_value("Global Defaults", "default_company")
		self.company_currency = saashq.get_cached_value(
	...
	def get_invoice_details(self):
		self.invoice_details = saashq._dict()
		if self.account_type == "Receivable":
			# nosemgrep
			si_list = saashq.db.sql(
				"""
	...
	def get_payment_terms(self, row):
		...
		# nosemgrep
		payment_terms_details = saashq.db.sql(
			f"""
			select
		...
		for d in payment_terms_details:
			term = saashq._dict(original_row)
		...
		for key in ("paid", "credit_note"):
			if row[key] > 0:
				if not additional_row:
					additional_row = saashq._dict(row)
	...
	def get_future_payments(self):
		if self.filters.show_future_payments:
			self.future_payments = saashq._dict()
	...
	def get_future_payments_from_payment_entry(self):
		...
		pe_ref = saashq.qb.DocType("Payment Entry Reference")
	...
	def get_return_entries(self):
		...
		self.return_entries = saashq._dict(
			saashq.get_all(
	...
	def get_ple_entries(self):
		...
		if self.filters.get("show_remarks"):
			if remarks_length := saashq.db.get_single_value(
	...
	def get_sales_invoices_or_customers_based_on_sales_person(self):
		if self.filters.get("sales_person"):
			lft, rgt = saashq.db.get_value("Sales Person", self.filters.get("sales_person"), ["lft", "rgt"])
			...
			self.sales_person_records = saashq._dict()
			for d in records:
	...
	def get_cost_center_conditions(self):
		lft, rgt = saashq.db.get_value("Cost Center", self.filters.cost_center, ["lft", "rgt"])
		cost_center_list = [
			center.name
			for center in saashq.get_list("Cost Center", filters={"lft": (">=", lft), "rgt": ("<=", rgt)})
		]
	...
	def get_hierarchical_filters(self, doctype, key):
		lft, rgt = saashq.db.get_value(doctype, self.filters.get(key), ["lft", "rgt"])
	...
	def get_party_details(self, party):
		if party not in self.party_details:
			if self.account_type == "Receivable":
				...
				self.party_details[party] = saashq.db.get_value(
					"Customer",
					party,
					fields,
					as_dict=True,
...
def get_customer_group_with_children(customer_groups):
	...
	for d in customer_groups:
		if saashq.db.exists("Customer Group", d):
			lft, rgt = saashq.db.get_value("Customer Group", d, ["lft", "rgt"])
			children = saashq.get_all("Customer Group", filters={"lft": [">=", lft], "rgt": ["<=", rgt]})
			all_customer_groups += [c.name for c in children]
		else:
			saashq.throw(_("Customer Group: {0} does not exist").format(d))
