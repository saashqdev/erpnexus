...
from saashq.utils import add_days, flt, getdate, today
...
from erpnexus.accounts.report.accounts_receivable.accounts_receivable import execute
...
class TestAccountsReceivable(AccountsTestMixin, IntegrationTestCase):
	...
	def test_exchange_revaluation_for_party(self):
		...
		company_doc = saashq.get_doc("Company", self.company)
		...
		je = saashq.get_doc("Journal Entry", err_journals.get("revaluation_jv"))
		je = je.submit()
	...
	def test_sales_person(self):
		sales_person = (
			saashq.get_doc({"doctype": "Sales Person", "sales_person_name": "John Clark", "enabled": True})
			.insert()
			.submit()
		)
	...
	def test_customer_group_filter(self):
		self.create_sales_invoice()
		cus_group = saashq.db.get_value("Customer", self.customer, "customer_group")
		filters = {
			"company": self.company,
			"report_date": today(),
			"range": "30, 60, 90, 120",
	...
	def test_report_output_if_party_is_missing(self):
		...
		if not saashq.db.get_value("Account", filters={"account_name": acc_name, "company": self.company}):
			additional_receivable_acc = saashq.get_doc(
				{
					"doctype": "Account",
