...
saashq.query_reports["Profitability Analysis"] = {
	filters: [
		{
			fieldname: "company",
			label: __("Company"),
	...
	formatter: function (value, row, column, data, default_formatter) {
		if (column.fieldname == "account") {
			...
			column.link_onclick =
				"saashq.query_reports['Profitability Analysis'].open_profit_and_loss_statement(" +
				JSON.stringify(data) +
				")";
			column.is_tree = true;
		}
	...
	open_profit_and_loss_statement: function (data) {
		...
		if (data.based_on == "Cost Center") {
			saashq.route_options["cost_center"] = data.account;
		} else {
			saashq.route_options["project"] = data.account;
		}

		saashq.set_route("query-report", "Profit and Loss Statement");
	},
	tree: true,
	name_field: "account",
