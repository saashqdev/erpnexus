...
from saashq.utils import add_months, today

from erpnexus.accounts.report.purchase_register.purchase_register import execute


class TestPurchaseRegister(IntegrationTestCase):
	def test_purchase_register(self):
		saashq.db.sql("delete from `tabPurchase Invoice` where company='_Test Company 6'")
		saashq.db.sql("delete from `tabGL Entry` where company='_Test Company 6'")

		filters = saashq._dict(company="_Test Company 6", from_date=add_months(today(), -1), to_date=today())

		pi = make_purchase_invoice()

		report_results = execute(filters)
		first_row = saashq._dict(report_results[1][0])
	...
	def test_purchase_register_ledger_view(self):
		...
		report_results = execute(filters)
		first_row = saashq._dict(report_results[1][2])
