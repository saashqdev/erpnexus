...
from erpnexus.accounts.report.sales_payment_summary.sales_payment_summary import (
...
class TestSalesPaymentSummary(IntegrationTestCase):
	...
	def setUpClass(cls):
		super().setUpClass()
		create_records()
		pes = saashq.get_all("Payment Entry")
		jes = saashq.get_all("Journal Entry")
		sis = saashq.get_all("Sales Invoice")
		for pe in pes:
			saashq.db.set_value("Payment Entry", pe.name, "docstatus", 2)
		for je in jes:
			saashq.db.set_value("Journal Entry", je.name, "docstatus", 2)
		for si in sis:
			saashq.db.set_value("Sales Invoice", si.name, "docstatus", 2)
	...
	def test_get_mode_of_payments(self):
		...
		payment_entries = saashq.get_all(
			"Payment Entry",
			filters={"mode_of_payment": "Cash", "docstatus": 1},
			fields=["name", "docstatus"],
		)
		for payment_entry in payment_entries:
			pe = saashq.get_doc("Payment Entry", payment_entry.name)
			pe.cancel()
	...
	return saashq.get_doc(
		{
			...
			"customer": saashq.get_doc("Customer", {"customer_name": "Prestiga-Biz"}).name,
			...
			"items": [
				{
					...
					"item_code": saashq.get_doc("Item", {"item_name": "Consulting"}).name,
					"qty": qty,
					"rate": 10000,
...
def create_records():
	if saashq.db.exists("Customer", "Prestiga-Biz"):
		return
