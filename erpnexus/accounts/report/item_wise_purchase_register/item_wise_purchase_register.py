...
import erpnexus
from erpnexus.accounts.report.item_wise_sales_register.item_wise_sales_register import (
	add_sub_total_row,
	add_total_row,
...
def get_items(filters, additional_table_columns):
	doctype = "Purchase Invoice"
	pi = saashq.qb.DocType(doctype)
	pii = saashq.qb.DocType(f"{doctype} Item")
...
def get_aii_accounts():
	return dict(saashq.db.sql("select name, stock_received_but_not_billed from tabCompany"))


def get_purchase_receipts_against_purchase_order(item_list):
	po_pr_map = saashq._dict()
	po_item_rows = list(set(d.po_detail for d in item_list))

	if po_item_rows:
		purchase_receipts = saashq.db.sql(
			"""
