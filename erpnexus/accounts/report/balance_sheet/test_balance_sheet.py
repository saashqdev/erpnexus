...
from erpnexus.accounts.report.balance_sheet.balance_sheet import execute
...
class TestBalanceSheet(IntegrationTestCase):
	def test_balance_sheet(self):
		saashq.db.sql(f"delete from `tabJournal Entry` where company='{COMPANY}'")
		saashq.db.sql(f"delete from `tabGL Entry` where company='{COMPANY}'")
...
def create_account(account_name: str, parent_account: str, company: str):
	if saashq.db.exists("Account", {"account_name": account_name, "company": company}):
		return
