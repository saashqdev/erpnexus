...
from erpnexus.accounts.report.trial_balance.trial_balance import execute
...
class TestTrialBalance(IntegrationTestCase):
	...
	def test_offsetting_entries_for_accounting_dimensions(self):
		...
		saashq.db.sql("delete from `tabSales Invoice` where company='Trial Balance Company'")
		saashq.db.sql("delete from `tabGL Entry` where company='Trial Balance Company'")
...
def create_accounting_dimension(**args):
	...
	if saashq.db.exists("Accounting Dimension", document_type):
		accounting_dimension = saashq.get_doc("Accounting Dimension", document_type)
		accounting_dimension.disabled = 0
	else:
		accounting_dimension = saashq.new_doc("Accounting Dimension")
...
def disable_dimension(**args):
	args = saashq._dict(args)
	document_type = args.document_type or "Branch"
	dimension = saashq.get_doc("Accounting Dimension", document_type)
	dimension.disabled = 1
	dimension.save()


def clear_dimension_defaults(dimension_name):
	accounting_dimension = saashq.get_doc("Accounting Dimension", dimension_name)
