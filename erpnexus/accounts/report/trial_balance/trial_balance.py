...
from saashq.utils import add_days, cstr, flt, formatdate, getdate
...
def validate_filters(filters):
	if not filters.fiscal_year:
		saashq.throw(_("Fiscal Year {0} is required").format(filters.fiscal_year))

	fiscal_year = saashq.get_cached_value(
		"Fiscal Year", filters.fiscal_year, ["year_start_date", "year_end_date"], as_dict=True
	)
	if not fiscal_year:
		saashq.throw(_("Fiscal Year {0} does not exist").format(filters.fiscal_year))
	else:
	...
	if filters.from_date > filters.to_date:
		saashq.throw(_("From Date cannot be greater than To Date"))
...
def get_data(filters):
	...
	company_currency = filters.presentation_currency or erpnexus.get_company_currency(filters.company)
...
def get_rootwise_opening_balances(filters, report_type):
	...
	ignore_closing_balances = saashq.db.get_single_value(
		"Accounts Settings", "ignore_account_closing_balance"
	)

	if not ignore_closing_balances:
		last_period_closing_voucher = saashq.db.get_all(
...
def get_opening_balance(
	...
	closing_balance = saashq.qb.DocType(doctype)
	account = saashq.qb.DocType("Account")

	opening_balance = (
		saashq.qb.from_(closing_balance)
		.select(
		...
		.where(
			...
			& (
				closing_balance.account.isin(
					saashq.qb.from_(account).select("name").where(account.report_type == report_type)
				)
			)
		)
	...
	if filters.cost_center:
		lft, rgt = saashq.db.get_value("Cost Center", filters.cost_center, ["lft", "rgt"])
		cost_center = saashq.qb.DocType("Cost Center")
		opening_balance = opening_balance.where(
			closing_balance.cost_center.isin(
				saashq.qb.from_(cost_center)
				.select("name")
	...
	if filters.get("include_default_book_entries"):
		company_fb = saashq.get_cached_value("Company", filters.company, "default_finance_book")

		if filters.finance_book and company_fb and cstr(filters.finance_book) != cstr(company_fb):
			saashq.throw(_("To use a different finance book, please uncheck 'Include Default FB Entries'"))
