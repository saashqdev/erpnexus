...
import erpnexus
from erpnexus.accounts.report.financial_statements import (
	filter_accounts,
	filter_out_zero_value_rows,
)
from erpnexus.accounts.report.trial_balance.trial_balance import validate_filters
...
def get_data(filters, dimension_list):
	company_currency = erpnexus.get_company_currency(filters.company)
	...
	min_lft, max_rgt = saashq.db.sql(
		"""select min(lft), max(rgt) from `tabAccount`
		where company=%s""",
		(filters.company),
	)[0]

	account = saashq.db.sql_list(
	...
	format_gl_entries(
		gl_entries_by_account, accounts_by_name, dimension_list, saashq.scrub(filters.get("dimension"))
	)
...
def set_gl_entries_by_account(dimension_list, filters, account, gl_entries_by_account):
	...
	if account:
		condition += " and account in ({})".format(", ".join([saashq.db.escape(d) for d in account]))
	...
	if filters.get("include_default_book_entries"):
		gl_filters["company_fb"] = saashq.get_cached_value("Company", filters.company, "default_finance_book")
	...
	gl_entries = saashq.db.sql(
		...
		order by account, posting_date""".format(
			dimension=saashq.scrub(filters.get("dimension")), condition=condition
		),
		gl_filters,
		as_dict=True,
	)  # nosec
		...
		for entry in entries:
			...
			for dimension in dimension_list:
				if dimension == entry.get(dimension_type):
					d[saashq.scrub(dimension)] = (
						d.get(saashq.scrub(dimension), 0.0) + flt(entry.debit) - flt(entry.credit)
					)
...
def prepare_data(accounts, filters, company_currency, dimension_list):
	...
	for d in accounts:
		...
		for dimension in dimension_list:
			row[saashq.scrub(dimension)] = flt(d.get(saashq.scrub(dimension), 0.0), 3)

			if abs(row[saashq.scrub(dimension)]) >= 0.005:
				# ignore zero values
				has_value = True
				total += flt(d.get(saashq.scrub(dimension), 0.0), 3)
...
def accumulate_values_into_parents(accounts, accounts_by_name, dimension_list):
	...
	for d in reversed(accounts):
		if d.parent_account:
			for dimension in dimension_list:
				accounts_by_name[d.parent_account][saashq.scrub(dimension)] = accounts_by_name[
					d.parent_account
				].get(saashq.scrub(dimension), 0.0) + d.get(saashq.scrub(dimension), 0.0)


def get_condition(dimension):
	conditions = []

	conditions.append(f"{saashq.scrub(dimension)} in %(dimensions)s")

	return " and {}".format(" and ".join(conditions)) if conditions else ""


def get_dimensions(filters):
	meta = saashq.get_meta(filters.get("dimension"), cached=False)
	query_filters = {}

	if meta.has_field("company"):
		query_filters = {"company": filters.get("company")}

	return saashq.get_all(filters.get("dimension"), filters=query_filters, pluck="name")
...
def get_columns(dimension_list):
	...
	for dimension in dimension_list:
		columns.append(
			{
				"fieldname": saashq.scrub(dimension),
				"label": dimension,
				"fieldtype": "Currency",
				"options": "currency",
				"width": 150,
