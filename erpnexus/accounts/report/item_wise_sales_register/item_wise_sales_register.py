...
from erpnexus.accounts.report.sales_register.sales_register import get_mode_of_payments
from erpnexus.accounts.report.utils import get_query_columns, get_values_for_columns
from erpnexus.selling.report.item_wise_sales_history.item_wise_sales_history import (
...
def _execute(filters=None, additional_table_columns=None, additional_conditions=None):
	...
	company_currency = saashq.get_cached_value("Company", filters.get("company"), "default_currency")
	...
	if item_list:
		...
		for tax in tax_columns:
			scrubbed_tax_fields.update(
				{
					tax + " Rate": saashq.scrub(tax + " Rate"),
					tax + " Amount": saashq.scrub(tax + " Amount"),
				}
			)
...
def apply_conditions(query, si, sii, filters, additional_conditions=None):
	...
	if filters.get("mode_of_payment"):
		sales_invoice = saashq.db.get_all(
	...
	if filters.get("warehouse"):
		if saashq.db.get_value("Warehouse", filters.get("warehouse"), "is_group"):
			lft, rgt = saashq.db.get_all(
				"Warehouse", filters={"name": filters.get("warehouse")}, fields=["lft", "rgt"], as_list=True
			)[0]
			warehouses = saashq.db.get_all("Warehouse", {"lft": (">", lft), "rgt": ("<", rgt)}, pluck="name")
...
def apply_group_by_conditions(query, si, ii, filters):
	if filters.get("group_by") == "Invoice":
		...
		query = query.orderby(si[saashq.scrub(filters.get("group_by"))])
...
def get_items(filters, additional_query_columns, additional_conditions=None):
	...
	si = saashq.qb.DocType(doctype)
	sii = saashq.qb.DocType(f"{doctype} Item")
	...
	if additional_query_columns:
		for column in additional_query_columns:
			if column.get("_doctype"):
				table = saashq.qb.DocType(column.get("_doctype"))
...
def get_tax_accounts(
	...
	tax_amount_precision = (
		get_field_precision(saashq.get_meta(tax_doctype).get_field("tax_amount"), currency=company_currency)
		or 2
	)
	...
	account_doctype = saashq.qb.DocType("Account")

	query = (
		saashq.qb.from_(account_doctype)
				...
				for item_code, tax_data in item_wise_tax_detail.items():
					...
					for d in item_row_map.get(parent, {}).get(item_code, []):
						...
						if item_tax_amount:
							...
							itemised_tax.setdefault(d.name, {})[description] = saashq._dict(
								{
									"tax_rate": tax_rate,
	...
	for desc in tax_columns:
		columns.append(
			{
				"label": _(desc + " Rate"),
				"fieldname": saashq.scrub(desc + " Rate"),
				"fieldtype": "Float",
				"width": 100,
			}
		)
		...
		columns.append(
			{
				"label": _(desc + " Amount"),
				"fieldname": saashq.scrub(desc + " Amount"),
				"fieldtype": "Currency",
				"options": "currency",
				"width": 100,
			}
...
def get_display_value(filters, group_by_field, item):
	if filters.get("group_by") == "Item":
		...
		party = saashq.scrub(filters.get("group_by"))
...
def get_group_by_and_display_fields(filters):
	if filters.get("group_by") == "Item":
		...
		group_by_field = saashq.scrub(filters.get("group_by"))
		subtotal_display_field = "item_code"
...
def add_sub_total_row(item, total_row_map, group_by_value, tax_columns):
	...
	for tax in tax_columns:
		total_row.setdefault(saashq.scrub(tax + " Amount"), 0.0)
		total_row[saashq.scrub(tax + " Amount")] += flt(item[saashq.scrub(tax + " Amount")])
