...
def execute(filters=None):
	if filters.get("party_type") == "Customer":
		party_naming_by = saashq.db.get_single_value("Selling Settings", "cust_master_name")
	else:
		party_naming_by = saashq.db.get_single_value("Buying Settings", "supp_master_name")
	...
	for name, details in gle_map.items():
		for entry in details:
			...
			if tax_amount:
				...
				row = {
					"pan" if saashq.db.has_column(filters.party_type, "pan") else "tax_id": party_map.get(
						party, {}
					).get("pan"),
...
def get_party_pan_map(party_type):
	...
	if saashq.db.has_column(party_type, "pan"):
		fields.append("pan")

	party_details = saashq.db.get_all(party_type, fields=fields)
...
def get_columns(filters):
	pan = "pan" if saashq.db.has_column(filters.party_type, "pan") else "tax_id"
	columns = [
		{
		...
		{"label": _(saashq.unscrub(pan)), "fieldname": pan, "fieldtype": "Data", "width": 60},
	]
...
def get_tds_docs(filters):
	...
	payment_entries = []
	journal_entries = []
	tax_category_map = saashq._dict()
	net_total_map = saashq._dict()
	journal_entry_party_map = saashq._dict()
	bank_accounts = saashq.get_all("Account", {"is_group": 0, "account_type": "Bank"}, pluck="name")

	_tds_accounts = saashq.get_all(
...
def get_tds_docs_query(filters, bank_accounts, tds_accounts):
	if not tds_accounts:
		saashq.throw(
			_("No {0} Accounts found for this company.").format(saashq.bold(_("Tax Withholding"))),
	...
	if filters.get("party"):
		...
		party = saashq.get_all(filters.get("party_type"), pluck="name")
		jv_condition = gle.against.isin(party) | (
