...
import saashq
from saashq import _, qb, scrub
from saashq.query_builder import Order
from saashq.utils import cint, flt, formatdate
...
from erpnexus.controllers.queries import get_match_cond
from erpnexus.stock.report.stock_ledger.stock_ledger import get_item_group_condition
...
def execute(filters=None):
	...
	filters.currency = saashq.get_cached_value("Company", filters.company, "default_currency")

	gross_profit_data = GrossProfitGenerator(filters)

	data = []

	group_wise_columns = saashq._dict(
		{
			"invoice": [
				"invoice_or_item",
				"customer",
...
class GrossProfitGenerator:
	def __init__(self, filters=None):
		...
		self.filters = saashq._dict(filters)
	...
	def process(self):
		...
		self.currency_precision = cint(saashq.db.get_default("currency_precision")) or 3
		self.float_precision = cint(saashq.db.get_default("float_precision")) or 2
		...
		for row in reversed(self.si_list):
			...
			product_bundles = []
			if row.update_stock:
				product_bundles = self.product_bundles.get(row.parenttype, {}).get(row.parent, saashq._dict())
			elif row.dn_detail:
				product_bundles = self.product_bundles.get("Delivery Note", {}).get(
					row.delivery_note, saashq._dict()
				)
	...
	def get_returned_invoice_items(self):
		returned_invoices = saashq.db.sql(
			"""
			select
		...
		self.returned_invoices = saashq._dict()
		for inv in returned_invoices:
			self.returned_invoices.setdefault(inv.return_against, saashq._dict()).setdefault(
				inv.item_code, []
			).append(inv)
	...
	def get_buying_amount_from_so_dn(self, sales_order, so_detail, item_code):
		from saashq.query_builder.functions import Avg

		delivery_note_item = saashq.qb.DocType("Delivery Note Item")

		query = (
			saashq.qb.from_(delivery_note_item)
	...
	def get_last_purchase_rate(self, item_code, row):
		...
		query = query.orderby(purchase_invoice.posting_date, order=saashq.qb.desc).limit(1)
	...
	def load_invoice_items(self):
		...
		if self.filters.get("cost_center"):
			self.filters.cost_center = saashq.parse_json(self.filters.get("cost_center"))
			self.filters.cost_center = get_cost_centers_with_children(self.filters.cost_center)
			conditions += " and `tabSales Invoice Item`.cost_center in %(cost_center)s"

		if self.filters.get("project"):
			self.filters.project = saashq.parse_json(self.filters.get("project"))
		...
		if self.filters.get("warehouse"):
			warehouse_details = saashq.db.get_value(
				"Warehouse", self.filters.get("warehouse"), ["lft", "rgt"], as_dict=1
			)
			if warehouse_details:
				conditions += f" and `tabSales Invoice Item`.warehouse in (select name from `tabWarehouse` wh where wh.lft >= {warehouse_details.lft} and wh.rgt <= {warehouse_details.rgt} and warehouse = wh.name)"

		self.si_list = saashq.db.sql(
			"""
			select
	...
	def get_delivery_notes(self):
		self.delivery_notes = saashq._dict({})
		if self.si_list:
	...
	def group_items_by_invoice(self):
		...
		for row in self.si_list:
			...
			if saashq.db.exists("Product Bundle", row.item_code):
		...
		return saashq._dict(
			{
				...
				"base_net_amount": saashq.db.get_value("Sales Invoice", row.parent, "base_net_total"),
			}
		)
	...
	def get_bundle_item_details(self, item_code):
		return saashq.db.get_value("Item", item_code, ["item_name", "description", "item_group", "brand"])
	...
	def load_product_bundle(self):
		...
		for d in pki_query.run(as_dict=True):
			self.product_bundles.setdefault(d.parenttype, saashq._dict()).setdefault(
				d.parent, saashq._dict()
			).setdefault(d.parent_item, []).append(d)

	def load_non_stock_items(self):
		self.non_stock_items = saashq.db.sql_list(
