...
from erpnexus.accounts.report.gross_profit.gross_profit import execute
...
class TestGrossProfit(IntegrationTestCase):
	...
	def test_invoice_without_only_delivery_note(self):
		...
		expected_entry_without_dn = {
			...
			"customer": self.customer,
			"posting_date": saashq.utils.datetime.date.fromisoformat(nowdate()),
			"item_code": self.item,
			"item_name": self.item,
	...
	def test_crnote_against_invoice_with_multiple_instances_of_same_item(self):
		...
		sinv.append("items", saashq.copy_doc(sinv.items[0], ignore_no_copy=False))
		sinv = sinv.save().submit()
