...
def set_default_role(doc, method):
	...
	if saashq.flags.setting_role or saashq.flags.in_migrate:
		return

	roles = saashq.get_roles(doc.name)

	contact_name = saashq.get_value("Contact", dict(email_id=doc.email))
	if contact_name:
		contact = saashq.get_doc("Contact", contact_name)
		for link in contact.links:
			saashq.flags.setting_role = True
...
def create_customer_or_supplier():
	...
	if saashq.db.get_value("User", user, "user_type") != "Website User":
		return

	user_roles = saashq.get_roles()
	portal_settings = saashq.get_single("Portal Settings")
	...
	party = saashq.new_doc(doctype)
	fullname = saashq.utils.get_fullname(user)
...
def party_exists(doctype, user):
	...
	contact_name = saashq.db.get_value("Contact", {"email_id": user})
	if contact_name:
		contact = saashq.get_doc("Contact", contact_name)
