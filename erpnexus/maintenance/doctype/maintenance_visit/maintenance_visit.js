...
saashq.provide("erpnexus.maintenance");
saashq.ui.form.on("Maintenance Visit", {
	setup: function (frm) {
	...
	onload: function (frm) {
		...
		if (frm.doc.maintenance_type === "Scheduled") {
			...
			saashq
				.call({
					method: "erpnexus.maintenance.doctype.maintenance_schedule.maintenance_schedule.get_serial_nos_from_schedule",
					args: {
		...
		if (frm.doc.__islocal) {
			frm.set_value({ mntc_date: saashq.datetime.get_today() });
		}
	},
	...
	customer_address: function (frm) {
		erpnexus.utils.get_address_display(frm, "customer_address", "address_display");
	},
...
erpnexus.maintenance.MaintenanceVisit = class MaintenanceVisit extends saashq.ui.form.Controller {
	refresh() {
			...
			this.frm.add_custom_button(
				...
				function () {
					if (!me.frm.doc.customer) {
						saashq.msgprint(__("Please select Customer first"));
						return;
					}
					erpnexus.utils.map_current_doc({
						method: "erpnexus.maintenance.doctype.maintenance_schedule.maintenance_schedule.make_maintenance_visit",
...
extend_cscript(cur_frm.cscript, new erpnexus.maintenance.MaintenanceVisit({ frm: cur_frm }));
