...
from saashq.utils import format_date, get_datetime
...
class MaintenanceVisit(TransactionBase):
	...
	if TYPE_CHECKING:
		...
		from erpnexus.maintenance.doctype.maintenance_visit_purpose.maintenance_visit_purpose import (
	...
	def validate_serial_no(self):
		for d in self.get("purposes"):
			if d.serial_no and not saashq.db.exists("Serial No", d.serial_no):
				saashq.throw(_("Serial No {0} does not exist").format(d.serial_no))

	def validate_purpose_table(self):
		if not self.purposes:
			saashq.throw(_("Add Items in the Purpose Table"), title=_("Purposes Required"))

	def validate_maintenance_date(self):
		if self.maintenance_type == "Scheduled":
			if self.maintenance_schedule_detail:
				item_ref = saashq.db.get_value(
					"Maintenance Schedule Detail", self.maintenance_schedule_detail, "item_reference"
				)
				if item_ref:
					start_date, end_date = saashq.db.get_value(
		...
		if not self.maintenance_schedule:
			for d in self.get("purposes"):
				if d.prevdoc_docname and d.prevdoc_doctype == "Warranty Claim":
					...
					wc_doc = saashq.get_doc("Warranty Claim", d.prevdoc_docname)
					wc_doc.update(
						{
