...
saashq.ui.form.on("Maintenance Schedule", {
	setup: function (frm) {
	...
	generate_schedule: function (frm) {
		if (frm.is_new()) {
			saashq.msgprint(__("Please save first"));
		} else {
...
erpnexus.maintenance.MaintenanceSchedule = class MaintenanceSchedule extends saashq.ui.form.Controller {
	refresh() {
...
extend_cscript(cur_frm.cscript, new erpnexus.maintenance.MaintenanceSchedule({ frm: cur_frm }));
