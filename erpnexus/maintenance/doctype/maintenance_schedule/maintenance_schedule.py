...
from saashq.utils import add_days, cint, cstr, date_diff, formatdate, getdate

from erpnexus.setup.doctype.employee.employee import get_holiday_list_for_employee
from erpnexus.stock.doctype.serial_no.serial_no import get_serial_nos
from erpnexus.utilities.transaction_base import TransactionBase, delete_events
...
class MaintenanceSchedule(TransactionBase):
	...
	if TYPE_CHECKING:
		...
		from erpnexus.maintenance.doctype.maintenance_schedule_detail.maintenance_schedule_detail import (
			MaintenanceScheduleDetail,
		)
		from erpnexus.maintenance.doctype.maintenance_schedule_item.maintenance_schedule_item import (
	...
	def on_submit(self):
		...
		for d in self.get("items"):
			...
			if d.sales_person and d.sales_person not in email_map:
				sp = saashq.get_doc("Sales Person", d.sales_person)
				try:
			...
			scheduled_date = saashq.db.get_all(
			...
			for key in scheduled_date:
				description = saashq._("Reference: {0}, Item Code: {1} and Customer: {2}").format(
	...
	def validate_schedule_date_for_holiday_list(self, schedule_date, sales_person):
		...
		employee = saashq.db.get_value("Sales Person", sales_person, "employee")
		if employee:
			holiday_list = get_holiday_list_for_employee(employee)
		else:
			holiday_list = saashq.get_cached_value("Company", self.company, "default_holiday_list")

		holidays = saashq.db.sql_list(
	...
	def validate_serial_no(self, item_code, serial_nos, amc_start_date):
		for serial_no in serial_nos:
			...
			if not sr_details:
				saashq.throw(_("Serial No {0} not found").format(serial_no))

			if sr_details.get("item_code") != item_code:
				saashq.throw(
					_("Serial No {0} does not belong to Item {1}").format(
						saashq.bold(serial_no), saashq.bold(item_code)
					),
					title=_("Invalid"),
				)
...
def get_serial_nos_from_schedule(item_code, schedule=None):
	serial_nos = []
	if schedule:
		serial_nos = saashq.db.get_value(
...
def make_maintenance_visit(source_name, target_doc=None, item_name=None, s_id=None):
	...
	def update_serial(source, target, parent):
		if source.item_reference:
			if sbb := saashq.db.get_value(
				"Maintenance Schedule Item", source.item_reference, "serial_and_batch_bundle"
			):
				serial_nos = saashq.get_doc("Serial and Batch Bundle", sbb).get_serial_nos()
