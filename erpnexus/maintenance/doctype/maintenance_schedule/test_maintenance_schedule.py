...
from saashq.utils import format_date
from saashq.utils.data import add_days, formatdate, today

from erpnexus.maintenance.doctype.maintenance_schedule.maintenance_schedule import (
...
class TestMaintenanceSchedule(IntegrationTestCase):
	...
	def test_make_schedule(self):
		...
		visit = saashq.new_doc("Maintenance Visit")
		...
		visit.save()
		visit.submit()
		ms = saashq.get_doc("Maintenance Schedule", ms.name)
...
def make_serial_item_with_serial(self, item_code):
	...
	active_serials = saashq.db.get_all("Serial No", {"status": "Active", "item_code": item_code})
...
def make_maintenance_schedule(**args):
	ms = saashq.new_doc("Maintenance Schedule")
