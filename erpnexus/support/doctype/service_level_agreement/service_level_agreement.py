...
from saashq.utils.nestedset import get_ancestors_of
from saashq.utils.safe_exec import get_safe_globals

from erpnexus.support.doctype.issue.issue import get_holidays
...
class ServiceLevelAgreement(Document):
	...
	if TYPE_CHECKING:
		...
		from erpnexus.support.doctype.pause_sla_on_status.pause_sla_on_status import PauseSLAOnStatus
		from erpnexus.support.doctype.service_day.service_day import ServiceDay
		from erpnexus.support.doctype.service_level_priority.service_level_priority import (
			ServiceLevelPriority,
		)
		from erpnexus.support.doctype.sla_fulfilled_on_status.sla_fulfilled_on_status import (
	...
	def check_support_and_resolution(self):
		...
		if len(set(support_days)) != len(support_days):
			...
			saashq.throw(_("Workday {0} has been repeated.").format(repeated_days))

	def validate_doc(self):
		if (
			self.enabled
			and self.document_type == "Issue"
			and not saashq.db.get_single_value("Support Settings", "track_service_level_agreement")
		):
			saashq.throw(
				_("{0} is not enabled in {1}").format(
					saashq.bold(_("Track Service Level Agreement")),
					get_link_to_form("Support Settings", "Support Settings"),
				)
			)

		if self.default_service_level_agreement and saashq.db.exists(
		...
		if (
			...
			saashq.throw(
				_("Service Level Agreement for {0} {1} already exists.").format(
					saashq.bold(self.entity_type), saashq.bold(self.entity)
				)
			)

	def validate_selected_doctype(self):
		invalid_doctypes = list(saashq.model.core_doctypes_list)
		invalid_doctypes.extend(["Cost Center", "Company"])
		valid_document_types = saashq.get_all(
			"DocType",
			{
				"issingle": 0,
				"istable": 0,
		...
		if self.document_type not in valid_document_types:
			saashq.throw(msg=_("Please select valid document type."), title=_("Invalid Document Type"))
	...
	def validate_status_field(self):
		...
		if not meta.get_field("status"):
			saashq.throw(
				...
				).format(saashq.bold(self.document_type))
			)
	...
	def validate_condition(self):
		...
		if self.condition:
			try:
				saashq.safe_eval(self.condition, None, get_context(temp_doc))
			except Exception:
				saashq.throw(_("The Condition '{0}' is invalid").format(self.condition))

	def get_service_level_agreement_priority(self, priority):
		priority = saashq.get_doc("Service Level Priority", {"priority": priority, "parent": self.name})
	...
	def before_insert(self):
		...
		meta = saashq.get_meta(self.document_type, cached=False)
	...
	def create_docfields(self, meta, service_level_agreement_fields):
		...
		saashq.get_doc("DocType", self.document_type).save(ignore_permissions=True)
	...
	def reset_field_properties(self, field, field_dt, sla_field):
		field = saashq.get_doc(field_dt, {"fieldname": field.fieldname})
...
def check_agreement_status():
	service_level_agreements = saashq.get_all(
		"Service Level Agreement",
	...
	for service_level_agreement in service_level_agreements:
		...
		if doc.end_date and getdate(doc.end_date) < getdate(saashq.utils.getdate()):
			saashq.db.set_value("Service Level Agreement", service_level_agreement.name, "enabled", 0)


def get_active_service_level_agreement_for(doc):
	if not saashq.db.get_single_value("Support Settings", "track_service_level_agreement"):
		return
	...
	for agreement in agreements:
		...
		if not condition or (condition and saashq.safe_eval(condition, None, get_context(doc))):
...
def get_context(doc):
	return {
		"doc": doc.as_dict(),
		"nowdate": nowdate,
		"saashq": saashq._dict(utils=get_safe_globals().get("saashq").get("utils")),
	}


def get_customer_group(customer):
	customer_groups = []
	customer_group = saashq.db.get_value("Customer", customer, "customer_group") if customer else None
	if customer_group:
...
def get_customer_territory(customer):
	...
	customer_territory = saashq.db.get_value("Customer", customer, "territory") if customer else None
...
def get_service_level_agreement_filters(doctype, name, customer=None):
	...
	return {
		"priority": [
			priority.priority
			for priority in saashq.get_all(
				"Service Level Priority", filters={"parent": name}, fields=["priority"]
			)
		],
		"service_level_agreements": [
			d.name for d in saashq.get_all("Service Level Agreement", filters=filters, or_filters=or_filters)
		],
	}
...
def get_documents_with_active_service_level_agreement():
	sla_doctypes = saashq.cache().hget("service_level_agreement", "active")
...
def set_documents_with_active_service_level_agreement():
	...
	saashq.cache().hset("service_level_agreement", "active", active)
	return active
...
def handle_status_change(doc, apply_sla_for_resolution):
	now_time = saashq.flags.current_time or now_datetime(doc.get("owner"))
	prev_status = saashq.db.get_value(doc.doctype, doc.name, "status")
...
def get_fulfillment_statuses(service_level_agreement):
	return [
		entry.status
		for entry in saashq.db.get_all(
...
def get_allotted_seconds(parameter, service_level):
	...
	if parameter == "response":
		...
		saashq.throw(_("{0} parameter is invalid").format(parameter))
...
def get_support_days(service_level):
	...
	for service in service_level.get("support_and_resolution"):
		support_days[service.workday] = saashq._dict(
			{
...
def change_service_level_agreement_and_priority(self):
	if (
		...
		and saashq.db.exists("Issue", self.name)
		and saashq.db.get_single_value("Support Settings", "track_service_level_agreement")
	):
		if self.priority != saashq.db.get_value("Issue", self.name, "priority"):
			self.set_response_and_resolution_time(
				priority=self.priority, service_level_agreement=self.service_level_agreement
			)
			saashq.msgprint(_("Priority has been changed to {0}.").format(self.priority))

		if self.service_level_agreement != saashq.db.get_value("Issue", self.name, "service_level_agreement"):
			self.set_response_and_resolution_time(
				priority=self.priority, service_level_agreement=self.service_level_agreement
			)
			saashq.msgprint(
...
def get_response_and_resolution_duration(doc):
	sla = saashq.get_doc("Service Level Agreement", doc.service_level_agreement)
...
def reset_service_level_agreement(doctype: str, docname: str, reason, user):
	if not saashq.db.get_single_value("Support Settings", "allow_resetting_service_level_agreement"):
		saashq.throw(_("Allow Resetting Service Level Agreement from Support Settings."))
...
def on_communication_update(doc, status):
	...
	for_resolution = saashq.db.get_value(
...
def get_tz(user):
	return saashq.db.get_value("User", user, "time_zone") or get_system_timezone()
...
def get_sla_doctypes():
	doctypes = []
	data = saashq.get_all("Service Level Agreement", {"enabled": 1}, ["document_type"], distinct=1)
