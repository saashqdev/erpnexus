...
from erpnexus.support.doctype.issue_priority.test_issue_priority import make_priorities
from erpnexus.support.doctype.service_level_agreement.service_level_agreement import (
...
class TestServiceLevelAgreement(IntegrationTestCase):
	def setUp(self):
		...
		saashq.qb.from_(lead).delete().where(lead.company == self.company).run()

	def create_company(self):
		name = "_Test Support SLA"
		company = None
		if saashq.db.exists("Company", name):
			company = saashq.get_doc("Company", name)
		else:
	...
	def test_custom_field_creation_for_sla_on_standard_dt(self):
		...
		for field in sla_fields:
			self.assertTrue(
				saashq.db.exists("Custom Field", {"dt": doctype, "fieldname": field.get("fieldname")})
			)
	...
	def test_docfield_creation_for_sla_on_custom_dt(self):
		...
		for field in sla_fields:
			self.assertTrue(
				saashq.db.exists("DocField", {"fieldname": field.get("fieldname"), "parent": doctype.name})
			)
	...
	def test_sla_application(self):
		...
		saashq.flags.current_time = datetime.datetime(2019, 3, 4, 15, 0)
		lead.reload()
	...
	def test_hold_time(self):
		...
		saashq.flags.current_time = datetime.datetime(2020, 3, 4, 4, 15)
		lead.reload()
		lead.status = "Replied"
		lead.save()

		lead.reload()
		self.assertEqual(lead.on_hold_since, saashq.flags.current_time)

		saashq.flags.current_time = datetime.datetime(2020, 3, 4, 5, 5)
		lead.reload()
		lead.status = "Converted"
		lead.save()
	...
	def test_failed_sla_for_response_only(self):
		...
		saashq.flags.current_time = datetime.datetime(2019, 3, 4, 16, 5)
		lead.reload()
		lead.status = "Replied"
		lead.save()
	...
	def test_fulfilled_sla_for_response_only(self):
		...
		saashq.flags.current_time = datetime.datetime(2019, 3, 4, 15, 30)
		lead.reload()
		lead.status = "Replied"
		lead.save()
	...
	def test_service_level_agreement_filters(self):
		...
		applied_sla = saashq.db.get_value("Lead", lead.name, "service_level_agreement")
		self.assertFalse(applied_sla)

		source = saashq.new_doc(doctype="UTM Source")
	...
	def tearDown(self):
		for d in saashq.get_all("Service Level Agreement"):
			saashq.delete_doc("Service Level Agreement", d.name, force=1)
...
def get_service_level_agreement(
	...
	service_level_agreement = saashq.get_doc("Service Level Agreement", filters)
...
def create_service_level_agreement(
	...
	service_level_agreement = saashq._dict(
		{
			...
			"entity": entity,
			"start_date": saashq.utils.getdate(),
			"end_date": saashq.utils.add_to_date(saashq.utils.getdate(), days=100),
	...
	sla = saashq.db.exists("Service Level Agreement", filters)
	if sla:
		saashq.delete_doc("Service Level Agreement", sla, force=1)

	return saashq.get_doc(service_level_agreement).insert(ignore_permissions=True, ignore_if_duplicate=True)
...
def create_customer():
	...
	if not saashq.db.exists("Customer", "_Test Customer"):
		customer.insert(ignore_permissions=True)
		return customer.name
	else:
		return saashq.db.exists("Customer", "_Test Customer")


def create_customer_group():
	customer_group = saashq.get_doc(
		{"doctype": "Customer Group", "customer_group_name": "_Test SLA Customer Group"}
	)

	if not saashq.db.exists("Customer Group", {"customer_group_name": "_Test SLA Customer Group"}):
		customer_group.insert()
		return customer_group.name
	else:
		return saashq.db.exists("Customer Group", {"customer_group_name": "_Test SLA Customer Group"})


def create_territory():
	territory = saashq.get_doc(
		{
			"doctype": "Territory",
	...
	if not saashq.db.exists("Territory", {"territory_name": "_Test SLA Territory"}):
		territory.insert()
		return territory.name
	else:
		return saashq.db.exists("Territory", {"territory_name": "_Test SLA Territory"})
...
def make_holiday_list():
	holiday_list = saashq.db.exists("Holiday List", "__Test Holiday List")
	if not holiday_list:
		holiday_list = saashq.get_doc(
			{
...
def create_custom_doctype():
	if not saashq.db.exists("DocType", "Test SLA on Custom Dt"):
		doc = saashq.get_doc(
			{
				"doctype": "DocType",
				"module": "Support",
		...
		return saashq.get_doc("DocType", "Test SLA on Custom Dt")
