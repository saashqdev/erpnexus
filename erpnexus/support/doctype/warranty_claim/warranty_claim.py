...
class WarrantyClaim(TransactionBase):
	...
	def validate(self):
		if session["user"] != "Guest" and not self.customer:
			saashq.throw(_("Customer is required"))

		if (
			self.status == "Closed"
			and not self.resolution_date
			and saashq.db.get_value("Warranty Claim", self.name, "status") != "Closed"
		):
	...
	def on_cancel(self):
		...
		if lst:
			...
			saashq.throw(_("Cancel Material Visit {0} before cancelling this Warranty Claim").format(lst1))
		else:
...
def make_maintenance_visit(source_name, target_doc=None):
	from saashq.model.mapper import get_mapped_doc, map_child_doc
	...
	if not visit:
		...
		source_doc = saashq.get_doc("Warranty Claim", source_name)
