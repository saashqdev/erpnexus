saashq.ui.form.on("Issue", {
			...
			frm.add_custom_button(
				...
				function () {
					saashq.model.open_mapped_doc({
						method: "erpnexus.support.doctype.issue.issue.make_task",
						frm: frm,
					});
				},
				__("Create")
	...
	reset_service_level_agreement: function (frm) {
		let reset_sla = new saashq.ui.Dialog({
			...
			primary_action: (values) => {
				...
				saashq.call(
					"erpnexus.support.doctype.service_level_agreement.service_level_agreement.reset_service_level_agreement",
					{
						reason: values.reason,
						user: saashq.session.user_email,
					...
					() => {
						...
						frm.refresh();
						saashq.msgprint(__("Service Level Agreement was reset."));
					}
				);
			},
		});
	...
	timeline_refresh: function (frm) {
		...
		// if (saashq.model.can_create("Help Article")) {
		...
		// 			${saashq.utils.icon('solid-info', 'sm')}
		// 		</a>
		// 	`);
		...
		// 			const doc = saashq.model.get_new_doc("Help Article");
		// 			doc.title = frm.doc.subject;
		// 			doc.content = content;
		// 			saashq.set_route("Form", "Help Article", doc.name);
		// 		});
		// 	}
