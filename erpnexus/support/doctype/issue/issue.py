...
from saashq.utils import date_diff, get_datetime, now_datetime, time_diff_in_seconds
...
class Issue(Document):
	...
	def validate(self):
		...
		if not self.raised_by:
			self.raised_by = saashq.session.user
	...
	def set_lead_contact(self, email_id):
		...
		if email_id:
			if not self.lead:
				self.lead = saashq.db.get_value("Lead", {"email_id": email_id})

			if not self.contact and not self.customer:
				self.contact = saashq.db.get_value("Contact", {"email_id": email_id})

				if self.contact:
					contact = saashq.get_doc("Contact", self.contact)
					self.customer = contact.get_link_for("Customer")

			if not self.company:
				self.company = saashq.db.get_value("Lead", self.lead, "company") or saashq.db.get_default(
					"Company"
				)
	...
	def split_issue(self, subject, communication_id):
		...
		saashq.get_doc(replicated_issue).insert()

		# Replicate linked Communications
		# TODO: get all communications in timeline before this, and modify them to append them to new doc
		comm_to_split_from = saashq.get_doc("Communication", communication_id)
		...
		for communication in communications:
			doc = saashq.get_doc("Communication", communication.name)
		...
		saashq.get_doc(
			{
				...
				"content": " - Split the Issue from <a href='/app/Form/Issue/{}'>{}</a>".format(
					self.name, saashq.bold(self.name)
				),
			}
...
def get_issue_list(doctype, txt, filters, limit_start, limit_page_length=20, order_by=None):
	...
	contact = saashq.db.get_value("Contact", {"user": user}, "name")
	customer = None
...
def set_multiple_status(names, status):
	for name in json.loads(names):
		saashq.db.set_value("Issue", name, "status", status)
...
def auto_close_tickets():
	...
	auto_close_after_days = saashq.db.get_single_value("Support Settings", "close_issue_after_days") or 7

	table = saashq.qb.DocType("Issue")
	issues = (
	...
	for issue in issues:
		doc = saashq.get_doc("Issue", issue)
		doc.status = "Closed"
...
def has_website_permission(doc, ptype, user, verbose=False):
	from erpnexus.controllers.website_list_for_contact import has_website_permission
...
def update_issue(contact, method):
	...
	saashq.db.sql("""UPDATE `tabIssue` set contact='' where contact=%s""", contact.name)
...
def calculate_first_response_time(issue, first_responded_on):
	...
	support_hours = saashq.get_cached_doc(
...
def get_working_hours(date, support_hours):
	if is_work_day(date, support_hours):
		weekday = saashq.utils.get_weekday(date)
...
def get_holidays(holiday_list_name):
	holiday_list = saashq.get_cached_doc("Holiday List", holiday_list_name)
