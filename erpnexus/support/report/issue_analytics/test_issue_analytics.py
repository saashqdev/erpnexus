...
from erpnexus.support.doctype.issue.test_issue import create_customer, make_issue
from erpnexus.support.doctype.service_level_agreement.test_service_level_agreement import (
	create_service_level_agreements_for_issues,
)
from erpnexus.support.report.issue_analytics.issue_analytics import execute
...
class TestIssueAnalytics(IntegrationTestCase):
	...
	def setUpClass(cls):
		super().setUpClass()
		saashq.db.sql("delete from `tabIssue` where company='_Test Company'")
...
def create_issue_types():
	for entry in ["Bug", "Service Request", "Discomfort"]:
		if not saashq.db.exists("Issue Type", entry):
			saashq.get_doc({"doctype": "Issue Type", "__newname": entry}).insert()
