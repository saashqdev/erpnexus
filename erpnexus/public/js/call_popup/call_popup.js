class CallPopup {
	...
	make() {
		saashq.utils.play_sound("incoming-call");
		...
		saashq.utils.bind_actions_with_object(this.dialog.$body, this);
	...
	close_modal() {
		this.dialog.hide();
		delete erpnexus.call_popup;
	}

	call_ended(call_log, missed) {
		saashq.utils.play_sound("call-disconnect");
	...
	setup_listener() {
		saashq.realtime.on(`call_${this.call_log.id}_ended`, (call_log) => {
			this.call_ended(call_log);
		});

		saashq.realtime.on(`call_${this.call_log.id}_missed`, (call_log) => {
			this.call_ended(call_log, true);
		});
	}

	clear_listeners() {
		saashq.realtime.off(`call_${this.call_log.id}_ended`);
		saashq.realtime.off(`call_${this.call_log.id}_missed`);
	}

	setup_call_details() {
		this.caller_info = $(`<div></div>`);
		this.call_details = new saashq.ui.FieldGroup({
			fields: [
				{
					fieldname: "name",
					label: "Name",
				...
				},
				{
					...
					click: () => saashq.set_route("Form", "Contact", this.get_contact_link().link_name),
				...
				},
				{
					...
					click: () => saashq.new_doc("Lead", { mobile_no: this.caller_number }),
					depends_on: () => !this.get_caller_name(),
				},
				{
				...
				},
				{
					...
					click: () => {
						...
						saashq
							.xcall(
								"erpnexus.telephony.doctype.call_log.call_log.add_call_summary_and_call_type",
								{
	...
	create_new_customer() {
		...
		const new_customer = saashq.model.get_new_doc("Customer");
		new_customer.mobile_no = this.caller_number;
		saashq.set_route("Form", new_customer.doctype, new_customer.name);
	}

	create_new_contact() {
		// TODO: fix new_doc, it should accept child table values
		const new_contact = saashq.model.get_new_doc("Contact");
		const phone_no = saashq.model.add_child(new_contact, "Contact Phone", "phone_nos");
		phone_no.phone = this.caller_number;
		phone_no.is_primary_mobile_no = 1;
		saashq.set_route("Form", new_contact.doctype, new_contact.name);
	}
}

$(document).on("app_ready", function () {
	saashq.realtime.on("show_call_popup", (call_log) => {
		let call_popup = erpnexus.call_popup;
		if (call_popup && call_log.name === call_popup.call_log.name) {
			call_popup.update_call_log(call_log);
			call_popup.dialog.show();
		} else {
			erpnexus.call_popup = new CallPopup(call_log);
		}
	});
});
