saashq.provide("erpnexus.financial_statements");

erpnexus.financial_statements = {
	filters: get_filters(),
	baseData: null,
	formatter: function (value, row, column, data, default_formatter, filter) {
		if (
			saashq.query_report.get_filter_value("selected_view") == "Growth" &&
			data &&
			column.colIndex >= 3
		) {
			...
			return value;
		} else if (saashq.query_report.get_filter_value("selected_view") == "Margin" && data) {
		...
		if (data && column.fieldname == "stub") {
			...
			if (data.account || data.accounts) {
				column.link_onclick =
					"erpnexus.financial_statements.open_general_ledger(" + JSON.stringify(data) + ")";
			}
			column.is_tree = true;
		}
	...
	open_general_ledger: function (data) {
		...
		let project = $.grep(saashq.query_report.filters, function (e) {
		...
		if (["Payable", "Receivable"].includes(data.account_type)) {
			...
			saashq.route_options["party_account"] = data.account;
			saashq.route_options["report_date"] = data.year_end_date;
		}
	...
	onload: function (report) {
		...
		erpnexus.financial_statements.filters = get_filters();

		let fiscal_year = erpnexus.utils.get_fiscal_year(saashq.datetime.get_today());
		...
		if (report.page) {
			...
			report.page.add_custom_menu_item(views_menu, __("Balance Sheet"), function () {
				var filters = report.get_values();
				saashq.set_route("query-report", "Balance Sheet", { company: filters.company });
			});

			report.page.add_custom_menu_item(views_menu, __("Profit and Loss"), function () {
				var filters = report.get_values();
				saashq.set_route("query-report", "Profit and Loss Statement", { company: filters.company });
			});

			report.page.add_custom_menu_item(views_menu, __("Cash Flow Statement"), function () {
				var filters = report.get_values();
				saashq.set_route("query-report", "Cash Flow", { company: filters.company });
			});
		}
	},
};
...
function get_filters() {
	let filters = [
		...
		},
		{
			...
			reqd: 1,
			on_change: function () {
				let filter_based_on = saashq.query_report.get_filter_value("filter_based_on");
				saashq.query_report.toggle_filter_display(
					"from_fiscal_year",
					filter_based_on === "Date Range"
				);
				saashq.query_report.toggle_filter_display("to_fiscal_year", filter_based_on === "Date Range");
				saashq.query_report.toggle_filter_display(
					"period_start_date",
	...
	let fiscal_year = erpnexus.utils.get_fiscal_year(saashq.datetime.get_today(), false, false);
	if (fiscal_year) {
