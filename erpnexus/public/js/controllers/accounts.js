...
// get tax rate
saashq.provide("erpnexus.taxes");

erpnexus.accounts.taxes = {
			...
			onload: function(frm) {
				if(frm.get_field("taxes")) {
					frm.set_query("account_head", "taxes", function(doc) {
						...
						return {
							query: "erpnexus.controllers.queries.tax_account_query",
							filters: {
		...
		saashq.ui.form.on(doctype, {
			account_head: function(frm, cdt, cdn) {
				...
				if(!d.charge_type && d.account_head){
					saashq.msgprint(__("Please select Charge Type first"));
					saashq.model.set_value(cdt, cdn, "account_head", "");
				} else if (d.account_head) {
					saashq.call({
						type:"GET",
						method: "erpnexus.controllers.accounts_controller.get_tax_rate",
						args: {"account_head":d.account_head},
						callback: function(r) {
							if (d.charge_type!=="Actual") {
								saashq.model.set_value(cdt, cdn, "rate", r.message.tax_rate || 0);
							}
							saashq.model.set_value(cdt, cdn, "description", r.message.account_name);
						}
					})
				}
			},
			...
			included_in_print_rate: function(frm, cdt, cdn) {
				let tax = saashq.get_doc(cdt, cdn);
				try {
					me.validate_taxes_and_charges(cdt, cdn);
	...
	validate_inclusive_tax: function(tax, frm) {
		...
		if(cint(tax.included_in_print_rate)) {
			if(tax.charge_type == "Actual") {
				...
				saashq.throw(__("Valuation type charges can not marked as Inclusive"));
			}
		}
	}
}

erpnexus.accounts.payment_triggers = {
...
erpnexus.accounts.pos = {
	...
	get_payment_mode_account: function(frm, mode_of_payment, callback) {
		...
		return  saashq.call({
			method: "erpnexus.accounts.doctype.sales_invoice.sales_invoice.get_bank_cash_account",
			args: {
