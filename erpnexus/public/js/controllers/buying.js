...
erpnexus.buying = {
	setup_buying_controller: function() {
		erpnexus.buying.BuyingController = class BuyingController extends erpnexus.TransactionController {
			setup() {
				super.setup();
			...
			onload(doc, cdt, cdn) {
				...
				if (this.frm.doc.__islocal
					&& saashq.meta.has_field(this.frm.doc.doctype, "disable_rounded_total")) {

						var df = saashq.meta.get_docfield(this.frm.doc.doctype, "disable_rounded_total");
						var disable = cint(df.default) || cint(saashq.sys_defaults.disable_rounded_total);
				...
				if(this.frm.get_field('shipping_address')) {
					this.frm.set_query("shipping_address", () => {
						if(this.frm.doc.customer) {
							...
							return erpnexus.queries.company_address_query(this.frm.doc)
					});
				}
			}
			...
			setup_queries(doc, cdt, cdn) {
				...
				me.frm.set_query('supplier', erpnexus.queries.supplier);
				me.frm.set_query('contact_person', erpnexus.queries.contact_query);
				me.frm.set_query('supplier_address', erpnexus.queries.address_query);

				me.frm.set_query('billing_address', erpnexus.queries.company_address_query);
				...
				this.frm.set_query("manufacturer", "items", function(doc, cdt, cdn) {
					...
					return {
						query: "erpnexus.controllers.queries.item_manufacturer_query",
			...
			supplier_address() {
				erpnexus.utils.get_address_display(this.frm);
				erpnexus.utils.set_taxes_from_address(this.frm, "supplier_address", "supplier_address", "supplier_address");
			}
			...
			calculate_received_qty(doc, cdt, cdn){
				var item = saashq.get_doc(cdt, cdn);
				saashq.model.round_floats_in(item, ["qty", "rejected_qty"]);
				...
				saashq.model.set_value(cdt, cdn, "received_qty", received_qty);
				saashq.model.set_value(cdt, cdn, "received_stock_qty", received_stock_qty);
			}
			...
			validate_negative_quantity(cdt, cdn, item, fieldnames){
				...
				for(var i = 0; i<fieldnames.length; i++) {
					if(item[fieldnames[i]] < 0){
						saashq.msgprint(__("Row #{0}: {1} can not be negative for item {2}", [item.idx,__(saashq.meta.get_label(cdt, fieldnames[i], cdn)), item.item_code]));
			...
			set_from_product_bundle() {
				...
				this.frm.add_custom_button(__("Product Bundle"), function() {
					erpnexus.buying.get_items_from_product_bundle(me.frm);
				}, __("Get Items From"));
			}

			shipping_address(){
				var me = this;
				erpnexus.utils.get_address_display(this.frm, "shipping_address",
					"shipping_address_display", true);
			}

			billing_address() {
				erpnexus.utils.get_address_display(this.frm, "billing_address",
					...
					saashq.call({
						...
						callback: function(r){
							if (r.message=="success") {
								saashq.show_alert({message:__("Auto repeat document updated"), indicator:'green'});
							} else {
								saashq.show_alert({message:__("An error occurred during the update process"), indicator:'red'});
							}
						}
					})
				}
			...
			manufacturer(doc, cdt, cdn) {
				...
				if(row.manufacturer) {
					saashq.call({
						method: "erpnexus.stock.doctype.item_manufacturer.item_manufacturer.get_item_manufacturer_part_no",
						args: {
						...
						callback: function(r) {
							if (r.message) {
								saashq.model.set_value(cdt, cdn, 'manufacturer_part_no', r.message);
							}
						}
					});
				}
			...
			manufacturer_part_no(doc, cdt, cdn) {
				...
				if (row.manufacturer_part_no) {
					saashq.model.get_value('Item Manufacturer',
						{
							'item_code': row.item_code,
							'manufacturer': row.manufacturer,
			...
			add_serial_batch_bundle(doc, cdt, cdn) {
				...
				saashq.db.get_value("Item", item.item_code, fields)
					.then((r) => {
						if (r.message && (r.message.has_batch_no || r.message.has_serial_no)) {
							...
							new erpnexus.SerialBatchPackageSelector(
								me.frm, item, (r) => {
									if (r) {
										...
										saashq.model.set_value(item.doctype, item.name, update_values);
									}
								}
							);
						}
...
erpnexus.buying.link_to_mrs = function(frm) {
	saashq.call({
			...
			for (let item of frm.doc.items) {
				...
				(r.message[0] || []).forEach(function(d) {
					if (d.qty > 0 && qty > 0 && item.item_code == d.item_code && !item.material_request_item)
					{
						...
						saashq.msgprint("Assigning " + d.mr_name + " to " + d.item_code + " (row " + item.idx + ")");
						if (qty > 0)
						{
							saashq.msgprint("Splitting " + qty + " units of " + d.item_code);
							var newrow = saashq.model.add_child(frm.doc, item.doctype, "items");
							item_length++;
...
erpnexus.buying.get_default_bom = function(frm) {
	$.each(frm.doc["items"] || [], function(i, d) {
		if (d.item_code && d.bom === "") {
			return saashq.call({
				type: "GET",
				method: "erpnexus.stock.get_item_details.get_default_bom",
				args: {
					"item_code": d.item_code,
				},
				callback: function(r) {
					if(r) {
						saashq.model.set_value(d.doctype, d.name, "bom", r.message);
					}
				}
			})
		}
...
erpnexus.buying.get_items_from_product_bundle = function(frm) {
	var dialog = new saashq.ui.Dialog({
		...
		primary_action(args){
			...
			return saashq.call({
				type: "GET",
				method: "erpnexus.stock.doctype.packed_item.packed_item.get_items_from_product_bundle",
				args: {
					row: {
				...
				callback: function(r) {
					...
					if(!r.exc && r.message) {
						...
						for (var i=0; i< r.message.length; i++) {
							...
							if(saashq.meta.get_docfield(d.doctype, "price_list_rate", d.name)) {
