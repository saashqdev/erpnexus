...
erpnexus.taxes_and_totals = class TaxesAndTotals extends erpnexus.payments {
	setup() {
	...
	apply_pricing_rule_on_item(item) {
		...
		saashq.model.set_value(item.doctype, item.name, "rate", item_rate);
	}
	...
	calculate_discount_amount() {
		if (saashq.meta.get_docfield(this.frm.doc.doctype, "discount_amount")) {
	...
	validate_conversion_rate() {
		...
		var conversion_rate_label = saashq.meta.get_label(this.frm.doc.doctype, "conversion_rate",
			this.frm.doc.name);
	...
	calculate_item_values() {
		...
		if (!this.discount_amount_applied) {
			for (const item of this.frm.doc.items || []) {
				saashq.model.round_floats_in(item);
	...
	initialize_taxes() {
		...
		$.each(this.frm.doc["taxes"] || [], function(i, tax) {
			...
			if (!this.discount_amount_applied) {
				erpnexus.accounts.taxes.validate_taxes_and_charges(tax.doctype, tax.name);
				erpnexus.accounts.taxes.validate_inclusive_tax(tax, this.frm);
			}
			saashq.model.round_floats_in(tax);
		});
	}

	fetch_round_off_accounts() {
		let me = this;
		saashq.flags.round_off_applicable_accounts = [];

		if (me.frm.doc.company) {
			saashq.call({
				"method": "erpnexus.controllers.taxes_and_totals.get_round_off_applicable_accounts",
				"args": {
					"company": me.frm.doc.company,
					"account_list": saashq.flags.round_off_applicable_accounts
				},
				callback(r) {
					if (r.message) {
						saashq.flags.round_off_applicable_accounts.push(...r.message);
					}
				}
			});
		}
		...
		saashq.call({
			method: "erpnexus.controllers.taxes_and_totals.get_rounding_tax_settings",
			callback: function(r) {
				saashq.flags.round_off_settings = r.message;
			}
		});
	}
	...
	calculate_net_total() {
		...
		saashq.model.round_floats_in(this.frm.doc, ["total", "base_total", "net_total", "base_net_total"]);
	}
	...
	calculate_shipping_charges() {
		...
		saashq.model.round_floats_in(this.frm.doc, ["total", "base_total", "net_total", "base_net_total"]);
		if (saashq.meta.get_docfield(this.frm.doc.doctype, "shipping_rule", this.frm.doc.name)) {
	...
	calculate_taxes() {
		...
		$.each(this.frm._items || [], function(n, item) {
			...
			$.each(me.frm.doc["taxes"] || [], function(i, tax) {
				...
				if (saashq.flags.round_row_wise_tax) {
	...
	round_off_totals(tax) {
		if (saashq.flags.round_off_applicable_accounts.includes(tax.account_head)) {
	...
	*/
	...
	adjust_grand_total_for_inclusive_tax() {
		...
		if (this.frm.doc["taxes"] && this.frm.doc["taxes"].length) {
			...
			if (any_inclusive_tax) {
				...
				var non_inclusive_tax_amount = saashq.utils.sum($.map(this.frm.doc.taxes || [],
					function(d) {
	...
	calculate_totals() {
		...
		saashq.model.round_floats_in(this.frm.doc, ["grand_total", "base_grand_total"]);
	...
	set_rounded_total() {
		...
		if(saashq.meta.get_docfield(this.frm.doc.doctype, "disable_rounded_total", this.frm.doc.name)) {
			disable_rounded_total = this.frm.doc.disable_rounded_total;
		} else if (saashq.sys_defaults.disable_rounded_total) {
			disable_rounded_total = saashq.sys_defaults.disable_rounded_total;
		}
		...
		if(saashq.meta.get_docfield(this.frm.doc.doctype, "rounded_total", this.frm.doc.name)) {
	...
	_cleanup() {
		...
		if(items && items.length) {
			if(!saashq.meta.get_docfield(items[0].doctype, "item_tax_amount", this.frm.doctype)) {
		...
		if(this.frm.doc["taxes"] && this.frm.doc["taxes"].length) {
			...
			if(!saashq.meta.get_docfield(this.frm.doc["taxes"][0].doctype, "tax_amount_after_discount_amount", this.frm.doctype)) {
	...
	set_discount_amount() {
		if(this.frm.doc.additional_discount_percentage) {
			this.frm.doc.discount_amount = flt(flt(this.frm.doc[saashq.scrub(this.frm.doc.apply_discount_on)])
	...
	apply_discount_amount() {
		...
		if (this.frm.doc.discount_amount) {
			if(!this.frm.doc.apply_discount_on)
				saashq.throw(__("Please select Apply Discount On"));
	...
	calculate_total_advance(update_paid_amount) {
		var total_allocated_amount = saashq.utils.sum($.map(this.frm.doc["advances"] || [], function(adv) {
	...
	calculate_outstanding_amount(update_paid_amount) {
		...
		saashq.model.round_floats_in(this.frm.doc, ["grand_total", "total_advance", "write_off_amount"]);
		...
		if(["Sales Invoice", "POS Invoice", "Purchase Invoice"].includes(this.frm.doc.doctype)) {
			...
			saashq.model.round_floats_in(this.frm.doc, ["paid_amount"]);
		...
		if(this.frm.doc.is_pos && (update_paid_amount===undefined || update_paid_amount)) {
			$.each(this.frm.doc['payments'] || [], function(index, data) {
				if(data.default && payment_status && total_amount_to_pay > 0) {
					...
					saashq.model.set_value(data.doctype, data.name, "base_amount", base_amount);
					saashq.model.set_value(data.doctype, data.name, "amount", amount);
					payment_status = false;

				} else if(me.frm.doc.paid_amount) {
					saashq.model.set_value(data.doctype, data.name, "amount", 0.0);
				}
			});
		}
	}
