saashq.provide("erpnexus.timesheet");

erpnexus.timesheet.timer = function (frm, row, timestamp = 0) {
	...
	erpnexus.timesheet.control_timer(frm, dialog, row, timestamp);
	dialog.show();
};

erpnexus.timesheet.control_timer = function (frm, dialog, row, timestamp = 0) {
	...
	$btn_start.click(function (e) {
		if (!initialized) {
			...
			row = saashq.model.add_child(frm.doc, "Timesheet Detail", "time_logs");
			row.activity_type = args.activity_type;
			row.from_time = saashq.datetime.get_datetime_as_string();
			...
			if (row.expected_hours) {
				...
				row.to_time = d.format(saashq.defaultDatetimeFormat);
			}
	...
	$btn_complete.click(function () {
		...
		grid_row.doc.to_time = saashq.datetime.now_datetime();
		grid_row.refresh();
		frm.dirty();
		frm.save();
		reset();
	...
	function updateStopwatch(increment) {
		...
		if (cur_dialog && cur_dialog.get_value("expected_hours") > 0) {
			if (flag && currentIncrement >= cur_dialog.get_value("expected_hours") * 3600) {
				saashq.utils.play_sound("alert");
				saashq.msgprint(__("Timer exceeded the given hours."));
				flag = false;
			}
		}
