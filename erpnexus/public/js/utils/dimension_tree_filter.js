...
erpnexus.accounts.dimensions = {
	...
	setup_filters(frm, doctype) {
		...
		if (this.accounting_dimensions) {
			this.accounting_dimensions.forEach((dimension) => {
				saashq.model.with_doctype(dimension["document_type"], () => {
					let parent_fields = [];
					saashq.meta.get_docfields(doctype).forEach((df) => {
						...
						if (saashq.meta.has_field(doctype, dimension["fieldname"])) {
	...
	setup_child_filters(frm, doctype, parentfield, dimension) {
		...
		if (saashq.meta.has_field(doctype, dimension)) {
			saashq.model.with_doctype(doctype, () => {
				...
				frm.set_query(dimension, parentfield, function (doc, cdt, cdn) {
					...
					return erpnexus.queries.get_filtered_dimensions(row, fields, dimension, doc.company);
				});
			});
		}
	},
	...
	setup_account_filters(frm, dimension, fields) {
		frm.set_query(dimension, function (doc) {
			return erpnexus.queries.get_filtered_dimensions(doc, fields, dimension, doc.company);
		});
	},
	...
	copy_dimension_from_first_row(frm, cdt, cdn, fieldname) {
		if (saashq.meta.has_field(frm.doctype, fieldname) && this.accounting_dimensions) {
			this.accounting_dimensions.forEach((dimension) => {
