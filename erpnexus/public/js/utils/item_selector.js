erpnexus.ItemSelector = class ItemSelector {
	constructor(opts) {
		$.extend(this, opts);
		...
		if (!this.item_query) {
			this.item_query = erpnexus.queries.item().query;
		}
	...
	add_item(item_code) {
		...
		$.each(this.frm.doc.items || [], (i, d) => {
			if (d[this.item_field] === item_code) {
				saashq.model.set_value(d.doctype, d.name, "qty", d.qty + 1);
				saashq.show_alert({ message: __("Added {0} ({1})", [item_code, d.qty]), indicator: "green" });
				added = true;
				return false;
			}
		});
		...
		if (!added) {
			...
			saashq.run_serially([
				...
				() => saashq.model.set_value(d.doctype, d.name, this.item_field, item_code),
				() => saashq.timeout(0.1),
				() => {
					saashq.model.set_value(d.doctype, d.name, "qty", 1);
					saashq.show_alert({ message: __("Added {0} ({1})", [item_code, 1]), indicator: "green" });
				},
			]);
		}
	}
	...
	render_items() {
		...
		var me = this;
		saashq.link_search("Item", args, function (results) {
			$.each(results, function (i, d) {
				if (!d.image) {
					d.abbr = saashq.get_abbr(d.item_name);
					d.color = saashq.get_palette(d.item_name);
				}
			});
			me.dialog.results.html(saashq.render_template("item_selector", { data: results }));
		});
	}
};
