...
saashq.provide("erpnexus.utils");
saashq.provide("erpnexus.stock.utils");

$.extend(erpnexus, {
	get_currency: function (company) {
		if (!company && cur_frm) company = cur_frm.doc.company;
		if (company)
			return saashq.get_doc(":Company", company)?.default_currency || saashq.boot.sysdefaults.currency;
		else return saashq.boot.sysdefaults.currency;
	},

	get_presentation_currency_list: () => {
		const docs = saashq.boot.docs;
	...
	hide_company: function (frm) {
		if (frm?.fields_dict.company) {
			...
			} else if (erpnexus.last_selected_company) {
				if (!frm.doc.company) frm.set_value("company", erpnexus.last_selected_company);
			}
		}
	},
	...
	stale_rate_allowed: () => {
		return cint(saashq.boot.sysdefaults.allow_stale);
	},
	...
	route_to_adjustment_jv: (args) => {
		saashq.model.with_doctype("Journal Entry", () => {
			// route to adjustment Journal Entry to handle Account Balance and Stock Value mismatch
			let journal_entry = saashq.model.get_new_doc("Journal Entry");

			args.accounts.forEach((je_account) => {
				let child_row = saashq.model.add_child(journal_entry, "accounts");
			...
			saashq.set_route("Form", "Journal Entry", journal_entry.name);
		});
	},

	route_to_pending_reposts: (args) => {
		saashq.set_route("List", "Repost Item Valuation", args);
	},
});

$.extend(erpnexus.utils, {
	set_party_dashboard_indicators: function (frm) {
		if (frm.doc.__onload && frm.doc.__onload.dashboard_info) {
			var company_wise_info = frm.doc.__onload.dashboard_info;
			if (company_wise_info.length > 1) {
				company_wise_info.forEach(function (info) {
					erpnexus.utils.add_indicator_for_multicompany(frm, info);
				});
		...
		if (bundle_ids?.length) {
			frm.add_custom_button(
				...
				() => {
					...
					saashq.set_route("query-report", "Serial and Batch Summary");
				},
				__("View")
			);
		}
	...
	get_terms: function (tc_name, doc, callback) {
		if (tc_name) {
			return saashq.call({
				method: "erpnexus.setup.doctype.terms_and_conditions.terms_and_conditions.get_terms_and_conditions",
				args: {
	...
	make_bank_account: function (doctype, docname) {
		saashq.call({
			method: "erpnexus.accounts.doctype.bank_account.bank_account.make_bank_account",
			args: {
				doctype: doctype,
				docname: docname,
			},
	...
	add_dimensions: function (report_name, index) {
		...
		saashq.call({
			method: "erpnexus.accounts.doctype.accounting_dimension.accounting_dimension.get_dimensions",
			callback: function (r) {
					...
					if (!found) {
						filters.splice(index, 0, {
							...
							get_data: function (txt) {
								return saashq.db.get_link_options(dimension["document_type"], txt);
							},
						});
					}
				});
	...
	add_inventory_dimensions: function (report_name, index) {
		...
		saashq.call({
			method: "erpnexus.stock.doctype.inventory_dimension.inventory_dimension.get_inventory_dimensions",
			callback: function (r) {
					...
					r.message.forEach((dimension) => {
						...
						if (!existing_filter.length) {
							filters.splice(index, 0, {
								...
								fieldtype: "MultiSelectList",
								get_data: function (txt) {
									return saashq.db.get_link_options(dimension["doctype"], txt);
								},
							});
						} else {
	...
	make_pricing_rule: function (doctype, docname) {
		saashq.call({
			method: "erpnexus.accounts.doctype.pricing_rule.pricing_rule.make_pricing_rule",
			args: {
				doctype: doctype,
				docname: docname,
			},
	...
	get_tree_options: function (option) {
		...
		let unscrub_option = saashq.model.unscrub(option);
		let user_permission = saashq.defaults.get_user_permissions();
		let options;
	...
	get_tree_default: function (option) {
		...
		if (options.includes(saashq.defaults.get_default(option))) {
			return saashq.defaults.get_default(option);
		} else {
			return options[0];
		}
	},
	overrides_parent_value_in_all_rows: function (doc, dt, dn, table_fieldname, fieldname, parent_fieldname) {
		if (doc[parent_fieldname]) {
			...
			saashq.refresh_field(table_fieldname);
		}
	},
	create_new_doc: function (doctype, update_fields) {
		saashq.model.with_doctype(doctype, function () {
			var new_doc = saashq.model.get_new_doc(doctype);
	...
	check_payments_app: () => {
		if (saashq.boot.versions && !saashq.boot.versions.payments) {
			const marketplace_link =
				'<a href="https://saashqcloud.com/marketplace/apps/payments">Marketplace</a>';
			const github_link = '<a href="https://github.com/saashqdev/payments/">GitHub</a>';
	...
	pick_serial_and_batch_bundle(frm, cdt, cdn, type_of_transaction, warehouse_field) {
		let item_row = saashq.get_doc(cdt, cdn);
		item_row.type_of_transaction = type_of_transaction;

		saashq.db.get_value("Item", item_row.item_code, ["has_batch_no", "has_serial_no"]).then((r) => {
	...
	get_fiscal_year: function (date, with_dates = false, raise_on_missing = true) {
		...
		if (!date) {
			date = saashq.datetime.get_today();
		}

		let fiscal_year = "";
		if (
			saashq.boot.current_fiscal_year &&
			date >= saashq.boot.current_fiscal_year[1] &&
			date <= saashq.boot.current_fiscal_year[2]
		) {
			if (with_dates) fiscal_year = saashq.boot.current_fiscal_year;
			else fiscal_year = saashq.boot.current_fiscal_year[0];
		} else {
			saashq.call({
				method: "erpnexus.accounts.utils.get_fiscal_year",
...
erpnexus.utils.select_alternate_items = function (opts) {
	const frm = opts.frm;
	...
	const dialog = new saashq.ui.Dialog({
					...
					},
					{
						...
						onchange: function () {
							...
							if (item_code && warehouse) {
								saashq.call({
									method: "erpnexus.stock.utils.get_latest_stock_qty",
									args: {
										item_code: item_code,
										warehouse: warehouse,
									},
						...
						get_query: (e) => {
							return {
								query: "erpnexus.stock.doctype.item_alternative.item_alternative.get_alternative_items",
								filters: {
		...
		primary_action: function () {
			...
			alternative_items.forEach((d) => {
				let row = saashq.get_doc(opts.child_doctype, d.docname);
				let qty = null;
				...
				saashq.model.set_value(row.doctype, row.name, "qty", qty);
				saashq.model.set_value(row.doctype, row.name, opts.original_item_field, d.item_code);
...
erpnexus.utils.update_child_items = function (opts) {
	const frm = opts.frm;
	const cannot_add_row = typeof opts.cannot_add_row === "undefined" ? true : opts.cannot_add_row;
	const child_docname = typeof opts.cannot_add_row === "undefined" ? "items" : opts.child_docname;
	const child_meta = saashq.get_meta(`${frm.doc.doctype} Item`);
	...
	let dialog = new saashq.ui.Dialog({
		...
		update_items: function () {
			...
			saashq.call({
				method: "erpnexus.controllers.accounts_controller.update_child_qty_rate",
				freeze: true,
				args: {
...
erpnexus.utils.map_current_doc = function (opts) {
	function _map() {
		if ($.isArray(cur_frm.doc.items) && cur_frm.doc.items.length > 0) {
			...
			var items_doctype = saashq.meta.get_docfield(cur_frm.doctype, "items").options;

			// find the link fieldname from items table for the given
			// source_doctype
			var link_fieldname = null;
			saashq.get_meta(items_doctype).fields.forEach(function (d) {
			...
			if (already_set) {
				opts.source_name.forEach(function (src) {
					saashq.model.with_doc(opts.source_doctype, src, function (r) {
						var source_doc = saashq.model.get_doc(opts.source_doctype, src);
		...
		return saashq.call({
			...
			type: "POST",
			method: "saashq.model.mapper.map_docs",
			args: {
				method: opts.method,
	...
	if (opts.source_doctype) {
		...
		if (["Purchase Receipt", "Delivery Note"].includes(opts.source_doctype)) {
			let target_meta = saashq.get_meta(cur_frm.doc.doctype);
		...
		const d = new saashq.ui.form.MultiSelectDialog({
			...
			action: function (selections, args) {
				...
				if (values.length === 0) {
					saashq.msgprint(__("Please select {0}", [opts.source_doctype]));
					return;
				}
...
saashq.form.link_formatters["Item"] = function (value, doc, df) {
	return add_link_title(value, doc, df, "item_name");
};

saashq.form.link_formatters["Employee"] = function (value, doc, df) {
	return add_link_title(value, doc, df, "employee_name");
};

saashq.form.link_formatters["Project"] = function (value, doc, df) {
...
$(document).on("app_ready", function () {
	if (!saashq.datetime.is_timezone_same()) {
		$.each(
			[
			...
			function (i, d) {
				saashq.ui.form.on(d, "onload", function (frm) {
					cur_frm.set_df_property("posting_time", "description", saashq.sys_defaults.time_zone);
				});
			}
		);
	}
...
// Show SLA dashboard
$(document).on("app_ready", function () {
	$.each(saashq.boot.service_level_agreement_doctypes, function (_i, d) {
		saashq.ui.form.on(d, {
			onload: function (frm) {
				if (!frm.doc.service_level_agreement) return;

				saashq.call({
					method: "erpnexus.support.doctype.service_level_agreement.service_level_agreement.get_service_level_agreement_filters",
					args: {
...
function get_time_left(timestamp, agreement_status) {
	const diff = moment(timestamp).diff(saashq.datetime.system_datetime(true));
...
$.extend(erpnexus.stock.utils, {
	set_item_details_using_barcode(frm, child_row, callback) {
		const barcode_scanner = new erpnexus.utils.BarcodeScanner({ frm: frm });
