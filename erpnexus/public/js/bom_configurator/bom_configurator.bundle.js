class BOMConfigurator {
	...
	make() {
		...
		saashq.views.trees["BOM Configurator"] = new saashq.views.TreeView(options);
		let node = saashq.views.trees["BOM Configurator"].tree.root_node;
		saashq.views.trees["BOM Configurator"].tree.show_toolbar(node);
		saashq.views.trees["BOM Configurator"].tree.load_children(node, true);
		this.tree_view = saashq.views.trees["BOM Configurator"];
	}

	bind_events() {
		saashq.views.trees["BOM Configurator"].events = {
			frm: this.frm,
	...
	tree_options() {
		return {
			...
			get_tree_nodes: "erpnexus.manufacturing.doctype.bom_creator.bom_creator.get_children",
	...
	tree_methods() {
		let frm_obj = this;
		let view = saashq.views.trees["BOM Configurator"];
		...
		return {
			...
			onrender(node) {
				...
				amount = saashq.format(amount, { fieldtype: "Currency", currency: frm_obj.frm.doc.currency });
			...
			toolbar:
				this.frm?.doc.docstatus === 0
					? [
							{
								label: __(saashq.utils.icon("edit", "sm") + " BOM"),
								click: function (node) {
									let view = saashq.views.trees["BOM Configurator"];
							...
							{
								label: __(saashq.utils.icon("add", "sm") + " Raw Material"),
								click: function (node) {
									let view = saashq.views.trees["BOM Configurator"];
							...
							{
								label: __(saashq.utils.icon("add", "sm") + " Sub Assembly"),
								click: function (node) {
									let view = saashq.views.trees["BOM Configurator"];
							...
							{
								label: __(saashq.utils.icon("move", "sm") + " Sub Assembly"),
								click: function (node) {
									let view = saashq.views.trees["BOM Configurator"];
							...
							{
								label: __(saashq.utils.icon("delete", "sm") + " Item"),
								click: function (node) {
									let view = saashq.views.trees["BOM Configurator"];
	...
	add_item(node, view) {
		saashq.prompt(
			...
			(data) => {
				...
				saashq.call({
					method: "erpnexus.manufacturing.doctype.bom_creator.bom_creator.add_item",
					args: {
	...
	add_sub_assembly(node, view) {
		...
		dialog.set_primary_action(__("Add"), () => {
			...
			saashq.call({
				method: "erpnexus.manufacturing.doctype.bom_creator.bom_creator.add_sub_assembly",
				args: {
					parent: node.data.parent_id,
					fg_item: node.data.value,
					fg_reference_id: node.data.name || this.frm.doc.name,
	...
	convert_to_sub_assembly(node, view) {
		...
		dialog.set_primary_action(__("Add"), () => {
			...
			if (!bom_item.item_code) {
				saashq.throw(__("Sub Assembly Item is mandatory"));
			}

			bom_item.items.forEach((d) => {
				if (!d.item_code) {
					saashq.throw(__("Item is mandatory in Raw Materials table."));
				}
			});
	...
	delete_node(node, view) {
		saashq.confirm(__("Are you sure you want to delete this Item?"), () => {
			saashq.call({
				method: "erpnexus.manufacturing.doctype.bom_creator.bom_creator.delete_node",
				args: {
					parent: node.data.parent_id,
					fg_item: node.data.value,
	...
	edit_bom(node, view) {
		...
		this.frm.edit_bom_dialog = saashq.prompt(
			fields,
			(data) => {
				...
				saashq.call({
					method: "erpnexus.manufacturing.doctype.bom_creator.bom_creator.edit_bom_creator",
					args: {
						doctype: doctype,
						docname: docname,
						data: data,
	...
	load_tree(response, node) {
		...
		saashq.views.trees["BOM Configurator"].tree.load_children(node);
		...
		while (node) {
			...
			total_amount = saashq.format(total_amount, {
				fieldtype: "Currency",
...
saashq.ui.BOMConfigurator = BOMConfigurator;
