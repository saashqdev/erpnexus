class VisualPlantFloor {
	...
	prepare_data() {
		saashq.call({
			method: "erpnexus.manufacturing.doctype.workstation.workstation.get_workstations",
			args: {
	...
	add_filter() {
		this.plant_floor = saashq.ui.form.make_control({
			df: {
				fieldtype: "Link",
				options: "Plant Floor",
		...
		this.workstation_type = saashq.ui.form.make_control({
			df: {
				fieldtype: "Link",
		...
		this.workstation = saashq.ui.form.make_control({
			df: {
				fieldtype: "Link",
				options: "Workstation",
		...
		this.workstation_status = saashq.ui.form.make_control({
			df: {
				fieldtype: "Select",
				options: "\nProduction\nOff\nIdle\nProblem\nMaintenance\nSetup",
	...
	render_workstations() {
		...
		let template = saashq.render_template("visual_plant_floor_template", {
	...
	update_status(data) {
		...
		if (data.status_image) {
			...
			workstation_card$
				.find(".workstation-image-container")
				.append(
					`<div class="workstation-image-cls workstation-abbr" style="margin:6px; height:82px">${saashq.get_abbr(
						data.name,
						2
					)}</div>`
				);
...
saashq.ui.VisualPlantFloor = VisualPlantFloor;
