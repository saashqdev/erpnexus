...
erpnexus.SMSManager = function SMSManager(doc) {
	var me = this;
	this.setup = function () {
		var default_msg = {
			Lead: "",
	...
	this.get_contact_number = function (contact, ref_doctype, ref_name) {
		saashq.call({
			method: "saashq.core.doctype.sms_settings.sms_settings.get_contact_number",
			args: {
	...
	this.make_dialog = function () {
		...
		d.fields_dict.send.input.onclick = function () {
			...
			if (v) {
				$(btn).set_working();
				saashq.call({
					method: "saashq.core.doctype.sms_settings.sms_settings.send_sms",
					args: {
