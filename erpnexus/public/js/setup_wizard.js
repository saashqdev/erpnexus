saashq.provide("erpnexus.setup");

saashq.pages["setup-wizard"].on_page_load = function (wrapper) {
	if (saashq.sys_defaults.company) {
		saashq.set_route("desk");
		return;
	}
};

saashq.setup.on("before_load", function () {
	erpnexus.setup.slides_settings.map(saashq.setup.add_slide);
});

erpnexus.setup.slides_settings = [
	{
		// Organization
		name: "organization",
		...
		validate: function () {
			...
			if ((this.values.company_name || "").toLowerCase() == "company") {
				saashq.msgprint(__("Company Name cannot be Company"));
				return false;
			}
		...
		validate_fy_dates: function () {
			...
			if (invalid || start_greater_than_end) {
				saashq.msgprint(__("Please enter valid Financial Year Start and End Dates"));
				return false;
			}
		...
		set_fy_dates: function (slide) {
			var country = saashq.wizard.values.country;

			if (country) {
				let fy = erpnexus.setup.fiscal_years[country];
				...
				if (year_start_date > saashq.datetime.get_today()) {
					next_year = current_year;
					current_year -= 1;
				}
		...
		load_chart_of_accounts: function (slide) {
			let country = saashq.wizard.values.country;
		...
		bind_events: function (slide) {
			...
			slide.get_input("fy_start_date").on("change", function () {
				...
				var year_end_date = saashq.datetime.add_days(saashq.datetime.add_months(start_date, 12), -1);
			...
			slide
				...
				.val(saashq.boot.sysdefaults.company_name || "")
				.trigger("change");
			...
			slide
				...
				.on("change", function () {
					...
					if (abbr.length > 10) {
						saashq.msgprint(__("Company Abbreviation cannot have more than 5 characters"));
						abbr = abbr.slice(0, 10);
					}
					slide.get_field("company_abbr").set_value(abbr);
				})
				.val(saashq.boot.sysdefaults.company_abbr || "")
				.trigger("change");
		},
		...
		charts_modal: function (slide, chart_template) {
			...
			let coa_tree = new saashq.ui.Tree({
				parent: $(dialog.body),
				label: parent,
				expandable: true,
				method: "erpnexus.accounts.utils.get_coa",
				args: {
			...
			const buttons = $(form_container).find(".saashq-control");
