...
erpnexus.accounts.bank_reconciliation.NumberCardManager = class NumberCardManager {
	constructor(opts) {
	...
	make_cards() {
		...
		chart_data.forEach((summary) => {
			let number_card = new erpnexus.accounts.NumberCard(summary);
...
erpnexus.accounts.NumberCard = class NumberCard {
	constructor(options) {
		this.$card = saashq.utils.build_summary_item(options);
	}
