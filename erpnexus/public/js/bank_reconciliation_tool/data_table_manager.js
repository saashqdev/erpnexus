...
erpnexus.accounts.bank_reconciliation.DataTableManager = class DataTableManager {
	constructor(opts) {
		Object.assign(this, opts);
		this.dialog_manager = new erpnexus.accounts.bank_reconciliation.DialogManager(
			this.company,
			this.bank_account,
	...
	make_dt() {
		var me = this;
		saashq.call({
			method: "erpnexus.accounts.doctype.bank_reconciliation_tool.bank_reconciliation_tool.get_bank_transactions",
			args: {
	...
	format_row(row) {
		return [
			row["date"],
			row["party_type"],
			saashq.form.formatters.Link(row["party"], { options: row["party_type"] }),
			row["description"],
			row["deposit"],
			row["withdrawal"],
	...
	get_datatable() {
		...
		this.datatable = new saashq.DataTable(this.$reconciliation_tool_dt.get(0), datatable_options);
