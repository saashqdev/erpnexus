...
erpnexus.payments = class payments extends erpnexus.stock.StockController {
	make_payment() {
		var me = this;
	...
	make_keyboard() {
		var me = this;
		$(this.$body).empty();
		$(this.$body).html(saashq.render_template("pos_payment", this.frm.doc));
	...
	make_multimode_payment() {
		...
		this.payments = saashq.model.add_child(this.frm.doc, "Multi Mode Payment", "payments");
	...
	show_payment_details() {
		...
		if (this.frm.doc.payments.length) {
			$.each(this.frm.doc.payments, function (index, data) {
				$(
					saashq.render_template("payment_details", {
		...
		$(this.$body)
			...
			.text(
				format_currency(
					...
					saashq.get_doc(":Company", this.frm.doc.company).default_currency
				)
			);
		this.update_invoice();
	}
