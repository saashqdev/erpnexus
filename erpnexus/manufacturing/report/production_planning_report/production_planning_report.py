...
class ProductionPlanReport:
	...
	def get_open_orders(self):
		...
		parent = saashq.qb.DocType(doctype)
		query = None
		...
		if doctype == "Work Order":
			...
			child = saashq.qb.DocType(f"{doctype} Item")
			query = (
				saashq.qb.from_(parent)
				.from_(child)
				.select(
	...
	def get_raw_materials(self):
		...
		if self.filters.based_on == "Work Order":
			...
			for d in self.orders:
				bom_no = d.bom_no or saashq.get_cached_value("Item", d.production_item, "default_bom")
			...
			bom_item = saashq.qb.DocType(bom_item_doctype)
	...
	def get_purchase_details(self):
		...
		purchased_items = saashq.get_all(
			"Purchase Order Item",
