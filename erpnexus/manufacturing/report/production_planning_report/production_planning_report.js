...
saashq.query_reports["Production Planning Report"] = {
	filters: [
		{
			fieldname: "company",
			label: __("Company"),
		...
		},
		{
			...
			reqd: 1,
			on_change: function () {
				let filters = saashq.query_report.filters;
				let based_on = saashq.query_report.get_filter_value("based_on");
				let options = {
		...
		},
		{
			...
			get_data: function (txt) {
				...
				return saashq.db.get_link_options(based_on, txt);
			},
			get_query: function () {
