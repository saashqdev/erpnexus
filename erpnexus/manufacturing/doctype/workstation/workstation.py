...
class WorkstationHolidayError(saashq.ValidationError):
	pass


class NotInWorkingHoursError(saashq.ValidationError):
	pass
...
class Workstation(Document):
	...
	if TYPE_CHECKING:
		...
		from erpnexus.manufacturing.doctype.workstation_working_hour.workstation_working_hour import (
	...
	def validate_working_hours(self, row):
		if not (row.start_time and row.end_time):
			saashq.throw(_("Row #{0}: Start Time and End Time are required").format(row.idx))

		if get_time(row.start_time) >= get_time(row.end_time):
			saashq.throw(_("Row #{0}: Start Time must be before End Time").format(row.idx))
	...
	def set_data_based_on_workstation_type(self):
		if self.workstation_type:
			...
			data = saashq.get_cached_value("Workstation Type", self.workstation_type, fields, as_dict=True)
	...
	def validate_workstation_holiday(self, schedule_date, skip_holiday_list_check=False):
		if not skip_holiday_list_check and (
			...
			or cint(saashq.db.get_single_value("Manufacturing Settings", "allow_production_on_holidays"))
		):
...
def get_job_cards(workstation, job_card=None):
	if saashq.has_permission("Job Card", "read"):
		jc_data = saashq.get_all(
			"Job Card",
			fields=[
				"name",
				"production_item",
		...
		allow_excess_transfer = saashq.db.get_single_value(
			"Manufacturing Settings", "job_card_excess_transfer"
		)

		user_employee = saashq.db.get_value("Employee", {"user_id": saashq.session.user}, "name")
		...
		for row in jc_data:
			...
			row.fg_uom = saashq.get_cached_value("Item", item_code, "stock_uom")
...
def get_default_holiday_list():
	return saashq.get_cached_value(
		"Company", saashq.defaults.get_user_default("Company"), "default_holiday_list"
	)


def check_if_within_operating_hours(workstation, operation, from_datetime, to_datetime):
	if from_datetime and to_datetime:
		if not saashq.db.get_single_value("Manufacturing Settings", "allow_production_on_holidays"):
			check_workstation_for_holiday(workstation, from_datetime, to_datetime)

		if not cint(saashq.db.get_single_value("Manufacturing Settings", "allow_overtime")):
...
def is_within_operating_hours(workstation, operation, from_datetime, to_datetime):
	...
	workstation = saashq.get_doc("Workstation", workstation)
...
def get_workstations(**kwargs):
	...
	_workstation = saashq.qb.DocType("Workstation")

	query = (
		saashq.qb.from_(_workstation)
		.select(
			_workstation.name,
...
def update_job_card(job_card, method, **kwargs):
	...
	if kwargs.get("employees"):
		kwargs.employees = saashq.parse_json(kwargs.employees)
...
def validate_job_card(job_card, status):
	job_card_details = saashq.db.get_value("Job Card", job_card, ["status", "for_quantity"], as_dict=1)
