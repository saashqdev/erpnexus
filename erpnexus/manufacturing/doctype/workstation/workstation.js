...
saashq.ui.form.on("Workstation", {
	...
	onload(frm) {
		if (frm.is_new()) {
			saashq.call({
				type: "GET",
				method: "erpnexus.manufacturing.doctype.workstation.workstation.get_default_holiday_list",
				callback: function (r) {
					if (!r.exe && r.message) {
...
class WorkstationDashboard {
	...
	prepapre_dashboard() {
		saashq.call({
			method: "erpnexus.manufacturing.doctype.workstation.workstation.get_job_cards",
			args: {
	...
	render_job_cards() {
		this.template = saashq.render_template("workstation_job_card", {
			data: this.job_cards,
		});
	...
	setup_qrcode_fields() {
		this.start_job_qrcode = saashq.ui.form.make_control({
			df: {
				label: __("Start Job"),
				fieldtype: "Data",
				options: "Barcode",
		...
		this.complete_job_qrcode = saashq.ui.form.make_control({
			df: {
	...
	validate_job_card(job_card, status, callback) {
		saashq.call({
			method: "erpnexus.manufacturing.doctype.workstation.workstation.validate_job_card",
			args: {
				job_card: job_card,
				status: status,
			},
	...
	toggle_job_card() {
		this.$wrapper.find(".collapse-indicator-job").on("click", (e) => {
			...
			if (
				...
				$(e.currentTarget).html(saashq.utils.icon("es-line-down", "sm", "mb-1"));
			else $(e.currentTarget).html(saashq.utils.icon("es-line-up", "sm", "mb-1"));
		});
	}
	...
	bind_events() {
		...
		this.$wrapper.find(".btn-pause").on("click", (e) => {
			...
			me.update_job_card(job_card, "pause_job", {
				end_time: saashq.datetime.now_datetime(),
			});
		});
		...
		this.$wrapper.find(".btn-resume").on("click", (e) => {
			...
			me.update_job_card(job_card, "resume_job", {
				start_time: saashq.datetime.now_datetime(),
			});
		});
	...
	start_job(job_card) {
		...
		this.employee_dialog = saashq.prompt(fields, (values) => {
	...
	complete_job(job_card, for_quantity) {
		saashq.prompt(
			...
			(data) => {
				if (flt(data.qty) <= 0) {
					saashq.throw(__("Quantity should be greater than 0"));
				}
	...
	update_job_card(job_card, method, data) {
		...
		saashq.call({
			method: "erpnexus.manufacturing.doctype.workstation.workstation.update_job_card",
			args: {
				job_card: job_card,
				method: method,
	...
	make_material_request(job_card) {
		let me = this;
		saashq.call({
			method: "erpnexus.manufacturing.doctype.workstation.workstation.get_raw_materials",
			args: {
				job_card: job_card,
			},
			callback: (r) => {
				if (r.message) {
					me.prepare_materials_modal(r.message, job_card, (job_card) => {
						saashq.call({
							method: "erpnexus.manufacturing.doctype.job_card.job_card.make_stock_entry",
							args: {
	...
	prepare_materials_modal(raw_materials, job_card, callback) {
		...
		this.materials_dialog = new saashq.ui.Dialog({
			title: "Raw Materials",
			fields: fields,
			size: "large",
	...
	get_current_time(data) {
		...
		data.time_logs.forEach((d) => {
			if (d.to_time) {
				...
				current_time += this.get_seconds_diff(saashq.datetime.now_datetime(), d.from_time);
			}
		});
