...
from saashq.query_builder.functions import IfNull, Max, Min, Sum
...
from erpnexus.manufacturing.doctype.workstation_type.workstation_type import get_workstations
from erpnexus.subcontracting.doctype.subcontracting_bom.subcontracting_bom import (
	get_subcontracting_boms_for_finished_goods,
)


class OverlapError(saashq.ValidationError):
	pass


class OperationMismatchError(saashq.ValidationError):
	pass


class OperationSequenceError(saashq.ValidationError):
	pass


class JobCardCancelError(saashq.ValidationError):
	pass


class JobCardOverTransferError(saashq.ValidationError):
	pass
...
class JobCard(Document):
	...
	if TYPE_CHECKING:
		...
		from erpnexus.manufacturing.doctype.job_card_item.job_card_item import JobCardItem
		from erpnexus.manufacturing.doctype.job_card_operation.job_card_operation import JobCardOperation
		from erpnexus.manufacturing.doctype.job_card_scheduled_time.job_card_scheduled_time import (
			JobCardScheduledTime,
		)
		from erpnexus.manufacturing.doctype.job_card_scrap_item.job_card_scrap_item import JobCardScrapItem
		from erpnexus.manufacturing.doctype.job_card_time_log.job_card_time_log import JobCardTimeLog
	...
	def onload(self):
		excess_transfer = saashq.db.get_single_value("Manufacturing Settings", "job_card_excess_transfer")
		self.set_onload("job_card_excess_transfer", excess_transfer)
		self.set_onload("work_order_closed", self.is_work_order_closed())
		self.set_onload("has_stock_entry", self.has_stock_entry())

	def has_stock_entry(self):
		return saashq.db.exists("Stock Entry", {"job_card": self.name, "docstatus": ["!=", 2]})
	...
	def set_manufactured_qty(self):
		...
		table = saashq.qb.DocType(table_name)
		query = saashq.qb.from_(table).where((table.job_card == self.name) & (table.docstatus == 1))
	...
	def validate_job_card_qty(self):
		...
		wo_qty = flt(saashq.get_cached_value("Work Order", self.work_order, "qty"))

		completed_qty = flt(saashq.db.get_value("Work Order Operation", self.operation_id, "completed_qty"))
		...
		if job_card_qty and ((job_card_qty - completed_qty) > wo_qty):
			...
			saashq.throw(_(msg), title=_("Extra Job Card Quantity"))
		...
		if self.get("time_logs"):
			for d in self.get("time_logs"):
				if d.to_time and get_datetime(d.from_time) > get_datetime(d.to_time):
					saashq.throw(_("Row {0}: From time must be less than to time").format(d.idx))
	...
	def get_overlap_for(self, args, open_job_cards=None):
		...
		if self.workstation:
			production_capacity = (
				saashq.get_cached_value("Workstation", self.workstation, "production_capacity") or 1
			)
	...
	def get_time_logs(self, args, doctype, open_job_cards=None):
		jc = saashq.qb.DocType("Job Card")
		jctl = saashq.qb.DocType(doctype)
	...
	def get_open_job_cards(self, employee):
		jc = saashq.qb.DocType("Job Card")
		jctl = saashq.qb.DocType("Job Card Time Log")
	...
	def schedule_time_logs(self, row):
		...
		while row.remaining_time_in_mins > 0:
			args = saashq._dict({"from_time": row.planned_start_time, "to_time": row.planned_end_time})
	...
	def check_workstation_time(self, row):
		workstation_doc = saashq.get_cached_doc("Workstation", self.workstation)
		if not workstation_doc.working_hours or cint(
			saashq.db.get_single_value("Manufacturing Settings", "allow_overtime")
		):
	...
	def get_required_items(self):
		...
		doc = saashq.get_doc("Work Order", self.get("work_order"))
			...
			if self.get("operation") == d.operation or self.operation_row_id == d.operation_row_id:
				self.append(
					"items",
					{
						...
						"uom": saashq.db.get_value("Item", d.item_code, "stock_uom"),
	...
	def validate_job_card(self):
		...
		if self.work_order and saashq.get_cached_value("Work Order", self.work_order, "status") == "Stopped":
			saashq.throw(
	...
	def update_semi_finished_good_details(self):
		if self.operation_id:
			...
			if (
				self.finished_good
				and saashq.get_cached_value("Work Order", self.work_order, "production_item")
				== self.finished_good
			):
				_wo_doc = saashq.get_doc("Work Order", self.work_order)
	...
	def validate_produced_quantity(self, for_quantity, process_loss_qty, wo):
		...
		if wo.produced_qty > for_quantity + process_loss_qty:
			...
			).format(saashq.bold(_("Job Card")), saashq.bold(self.name), saashq.bold(self.production_item))

			second_part_msg = _(
				"Kindly cancel the Manufacturing Entries first against the work order {0}."
			).format(saashq.bold(get_link_to_form("Work Order", self.work_order)))
	...
	def update_work_order_data(self, for_quantity, process_loss_qty, time_in_mins, wo):
		workstation_hour_rate = saashq.get_value("Workstation", self.workstation, "hour_rate")
		jc = saashq.qb.DocType("Job Card")
		jctl = saashq.qb.DocType("Job Card Time Log")
	...
	def set_transferred_qty_in_job_card_item(self, ste_doc):
		def _get_job_card_items_transferred_qty(ste_doc):
			...
			if job_card_items:
				...
				sed = saashq.qb.DocType("Stock Entry Detail")
				...
				job_card_items_transferred_qty = saashq._dict(query.run(as_list=True))
		...
		def _validate_over_transfer(row, transferred_qty):
			...
			required_qty = saashq.db.get_value("Job Card Item", row.job_card_item, "required_qty")
			...
			if is_excess:
				saashq.throw(
					_(
						...
						row.idx, saashq.bold(required_qty), saashq.bold(row.item_code), ste_doc.job_card
					),
		...
		allow_excess = saashq.db.get_single_value("Manufacturing Settings", "job_card_excess_transfer")
		...
		for row in ste_doc.items:
			...
			saashq.db.set_value("Job Card Item", row.job_card_item, "transferred_qty", flt(transferred_qty))
	...
	def set_transferred_qty(self, update_status=False):
		...
		if self.work_order and not saashq.get_cached_value(
	...
	def set_transferred_qty_in_work_order(self):
		doc = saashq.get_doc("Work Order", self.work_order)
	...
	def validate_operation_id(self):
		if (
			...
			and self.operation
			and self.work_order
			and saashq.get_cached_value("Work Order Operation", self.operation_row_number, "name")
			!= self.operation_id
		):
	...
	def validate_work_order(self):
		if self.is_work_order_closed():
			saashq.throw(_("You can't make any changes to Job Card since Work Order is closed."))

	def is_work_order_closed(self):
		if self.work_order:
			status = saashq.get_value("Work Order", self.work_order)
	...
	def update_status_in_workstation(self, status):
		...
		saashq.db.set_value("Workstation", self.workstation, "status", status)
	...
	def update_workstation_status(self):
		...
		if not job_cards:
			saashq.db.set_value("Workstation", self.workstation, "status", "Off")

		for row in job_cards:
			saashq.db.set_value("Workstation", self.workstation, "status", status_map.get(row.status))
			return
	...
	def make_stock_entry_for_semi_fg_item(self, auto_submit=False):
		from erpnexus.stock.doctype.stock_entry_type.stock_entry_type import ManufactureEntry
		...
		ste = ManufactureEntry(
			{
				...
				"project": saashq.db.get_value("Work Order", self.work_order, "project"),
			}
		)
...
def make_time_log(kwargs):
	...
	kwargs = saashq._dict(kwargs)
	doc = saashq.get_doc("Job Card", kwargs.job_card_id)
...
def get_operations(doctype, txt, searchfield, start, page_len, filters):
	if not filters.get("work_order"):
		saashq.msgprint(_("Please select a Work Order first."))
		return []
...
def make_stock_entry(source_name, target_doc=None):
	...
	def set_missing_values(source, target):
		...
		wo_allows_alternate_item = saashq.db.get_value(
			"Work Order", target.work_order, "allow_alternative_item"
		)
		for item in target.items:
			item.allow_alternative_item = int(
				wo_allows_alternate_item
				and saashq.get_cached_value("Item", item.item_code, "allow_alternative_item")
			)
