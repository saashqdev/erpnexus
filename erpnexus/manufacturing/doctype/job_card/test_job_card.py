...
from saashq.utils.data import add_to_date, now, today

from erpnexus.manufacturing.doctype.job_card.job_card import (
...
from erpnexus.manufacturing.doctype.work_order.work_order import WorkOrder
...
class TestJobCard(IntegrationTestCase):
	...
	def test_job_card_operations(self):
		...
		if job_cards:
			...
			saashq.db.set_value("Job Card", job_card.name, "operation_row_number", job_card.operation_id)

			doc = saashq.get_doc("Job Card", job_card.name)
	...
	def test_job_card_with_different_work_station(self):
		...
		if job_card:
			workstation = saashq.db.get_value(
			...
			completed_qty = saashq.db.get_value(
	...
	def test_job_card_overlap(self):
		...
		jc1 = saashq.get_last_doc("Job Card", {"work_order": self.work_order.name})
		jc2 = saashq.get_last_doc("Job Card", {"work_order": wo2.name})
	...
	def test_job_card_overlap_with_capacity(self):
		...
		workstation = make_workstation(workstation_name=random_string(5)).name
		saashq.db.set_value("Workstation", workstation, "production_capacity", 1)
		...
		saashq.db.set_value("Workstation", workstation, "production_capacity", 2)
		jc2.load_from_db()
	...
	def test_job_card_multiple_materials_transfer(self):
		...
		job_card_name = saashq.db.get_value("Job Card", {"work_order": self.work_order.name})
		job_card = saashq.get_doc("Job Card", job_card_name)
	...
	def test_job_card_excess_material_transfer(self):
		...
		job_card = saashq.get_last_doc("Job Card", {"work_order": self.work_order.name})
	...
	def test_job_card_excess_material_transfer_with_no_reference(self):
		...
		self.assertRaises(saashq.ValidationError, transfer_entry_1.insert)
	...
	def test_job_card_material_transfer_correctness(self):
		...
		job_card_name = saashq.db.get_value(
	...
	def test_corrective_costing(self):
		...
		corrective_action = saashq.get_doc(
			doctype="Operation", is_corrective_operation=1, name=saashq.generate_hash()
		).insert()
	...
	def test_job_card_material_request_and_bom_details(self):
		from erpnexus.stock.doctype.material_request.material_request import make_stock_entry

		create_bom_with_multiple_operations()
		work_order = make_wo_with_transfer_against_jc()

		job_card_name = saashq.db.get_value("Job Card", {"work_order": work_order.name}, "name")
	...
	def test_job_card_proccess_qty_and_completed_qty(self):
		from erpnexus.manufacturing.doctype.routing.test_routing import (
			create_routing,
			setup_bom,
			setup_operations,
		)
		...
		jc = saashq.get_doc("Job Card", first_job_card)
		...
		second_job_card = saashq.get_all(
			"Job Card",
		...
		jc2 = saashq.get_doc("Job Card", second_job_card)
		...
		self.assertRaises(saashq.ValidationError, jc2.save)
		...
		s = saashq.get_doc(make_stock_entry_for_wo(wo_doc.name, "Manufacture", 10))
		s.submit()
...
def create_bom_with_multiple_operations():
	...
	test_record = saashq.get_test_records("BOM")[2]
	bom_doc = saashq.get_doc(test_record)
