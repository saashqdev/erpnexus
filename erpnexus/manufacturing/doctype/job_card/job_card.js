...
saashq.ui.form.on("Job Card", {
	setup: function (frm) {
		frm.set_query("operation", function () {
			return {
				query: "erpnexus.manufacturing.doctype.job_card.job_card.get_operations",
				filters: {
	...
	refresh: function (frm) {
		...
		if (
			...
			if (!frm.doc.time_logs?.length) {
				frm.add_custom_button(__("Start Job"), () => {
					let from_time = saashq.datetime.now_datetime();
		...
		if (frm.doc.work_order) {
			saashq.db.get_value("Work Order", frm.doc.work_order, "transfer_material_against").then((r) => {
		...
		if (frm.doc.docstatus === 1 && frm.doc.for_quantity > frm.doc.manufactured_qty) {
			frm.add_custom_button(__("Make Subcontracting PO"), () => {
				saashq.model.open_mapped_doc({
					method: "erpnexus.manufacturing.doctype.job_card.job_card.make_subcontracting_po",
					frm: frm,
				});
			}).addClass("btn-primary");
		}
	...
	make_corrective_job_card: function (frm, operation, for_operation) {
		saashq.call({
			method: "erpnexus.manufacturing.doctype.job_card.job_card.make_corrective_job_card",
			args: {
	...
	operation: function (frm) {
		...
		if (frm.doc.operation && frm.doc.work_order) {
			saashq.call({
				method: "erpnexus.manufacturing.doctype.job_card.job_card.get_operation_details",
				args: {
					work_order: frm.doc.work_order,
	...
	make_time_log: function (frm, args) {
		...
		saashq.call({
			method: "erpnexus.manufacturing.doctype.job_card.job_card.make_time_log",
			args: {
				args: args,
			},
			freeze: true,
	...
	get_current_time(frm) {
		...
		frm.doc.time_logs.forEach((d) => {
			if (d.to_time) {
				...
				current_time += get_seconds_diff(saashq.datetime.now_datetime(), d.from_time);
			}
		});
	...
	make_material_request: function (frm) {
		saashq.model.open_mapped_doc({
			method: "erpnexus.manufacturing.doctype.job_card.job_card.make_material_request",
			frm: frm,
	...
	source_warehouse(frm) {
		if (frm.doc.source_warehouse) {
			frm.doc.items.forEach((d) => {
				saashq.model.set_value(d.doctype, d.name, "source_warehouse", frm.doc.source_warehouse);
			});
		}
	},
});

saashq.ui.form.on("Job Card Time Log", {
