...
class Operation(Document):
	...
	if TYPE_CHECKING:
		...
		from erpnexus.manufacturing.doctype.sub_operation.sub_operation import SubOperation
	...
	def duplicate_sub_operation(self):
		...
		for row in self.sub_operations:
			if row.operation in operation_list:
				saashq.throw(
					_("The operation {0} can not add multiple times").format(saashq.bold(row.operation))
				)

			if self.name == row.operation:
				saashq.throw(
					_("The operation {0} can not be the sub operation").format(saashq.bold(row.operation))
				)
