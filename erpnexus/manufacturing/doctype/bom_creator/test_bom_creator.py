...
from erpnexus.manufacturing.doctype.bom_creator.bom_creator import (
	add_item,
	add_sub_assembly,
)
...
class TestBOMCreator(IntegrationTestCase):
	...
	def test_make_boms_from_bom_creator(self):
		...
		data = saashq.get_all("BOM", filters={"bom_creator": doc.name, "docstatus": 1})
...
def make_bom_creator(**kwargs):
	...
	doc = saashq.new_doc("BOM Creator")
	doc.update(kwargs)
	doc.save()
