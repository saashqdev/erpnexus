...
saashq.ui.form.on("Plant Floor", {
	...
	add_workstation(frm) {
		frm.add_custom_button(__("Create Workstation"), () => {
			var doc = saashq.model.get_new_doc("Workstation");
	...
	setup_queries(frm) {
		frm.set_query("warehouse", (doc) => {
			if (!doc.company) {
				saashq.throw(__("Please select Company first"));
			}
	...
	update_realtime_status(frm) {
		saashq.realtime.on("update_workstation_status", (data) => {
			saashq.visual_plant_floor.update_status(data);
		});
	},
	...
	prepare_stock_dashboard(frm) {
		...
		saashq.visual_stock = new VisualStock({
			wrapper: wrapper,
			frm: frm,
		});
	},
...
class VisualStock {
	...
	prepare_filters() {
		...
		this.item_filter = saashq.ui.form.make_control({
			df: {
				fieldtype: "Link",
				fieldname: "item_code",
				placeholder: __("Item"),
		...
		this.item_group_filter = saashq.ui.form.make_control({
			df: {
				fieldtype: "Link",
	...
	async get_stock_summary(start, item_code, item_group) {
		let stock_summary = await saashq.call({
			method: "erpnexus.manufacturing.doctype.plant_floor.plant_floor.get_stock_summary",
			args: {
	...
	render_stock_summary() {
		let template = saashq.render_template("stock_summary_template", {
	...
	update_values() {
		if (!this.values.qty) {
			saashq.throw(__("Quantity is required"));
		}
