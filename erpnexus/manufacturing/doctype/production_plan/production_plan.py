...
from erpnexus.manufacturing.doctype.bom.bom import get_children as get_bom_children
from erpnexus.manufacturing.doctype.bom.bom import validate_bom_no
from erpnexus.manufacturing.doctype.work_order.work_order import get_item_details
from erpnexus.setup.doctype.item_group.item_group import get_item_group_defaults
from erpnexus.stock.get_item_details import get_conversion_factor
from erpnexus.stock.utils import get_or_make_bin
...
class ProductionPlan(Document):
	...
	if TYPE_CHECKING:
		...
		from erpnexus.manufacturing.doctype.material_request_plan_item.material_request_plan_item import (
			MaterialRequestPlanItem,
		)
		from erpnexus.manufacturing.doctype.production_plan_item.production_plan_item import (
			ProductionPlanItem,
		)
		from erpnexus.manufacturing.doctype.production_plan_item_reference.production_plan_item_reference import (
			ProductionPlanItemReference,
		)
		from erpnexus.manufacturing.doctype.production_plan_material_request.production_plan_material_request import (
			ProductionPlanMaterialRequest,
		)
		from erpnexus.manufacturing.doctype.production_plan_material_request_warehouse.production_plan_material_request_warehouse import (
			ProductionPlanMaterialRequestWarehouse,
		)
		from erpnexus.manufacturing.doctype.production_plan_sales_order.production_plan_sales_order import (
			ProductionPlanSalesOrder,
		)
		from erpnexus.manufacturing.doctype.production_plan_sub_assembly_item.production_plan_sub_assembly_item import (
	...
	def validate_data(self):
		for d in self.get("po_items"):
			if not d.bom_no:
				saashq.throw(_("Please select BOM for Item in Row {0}").format(d.idx))
			else:
				validate_bom_no(d.item_code, d.bom_no)

			if not flt(d.planned_qty):
				saashq.throw(_("Please enter Planned Qty for Item {0} at row {1}").format(d.item_code, d.idx))
	...
	def get_open_sales_orders(self):
		...
		if open_so:
			...
			saashq.msgprint(_("Sales orders are not available for production"))
	...
	def get_pending_material_requests(self):
		...
		mr = saashq.qb.DocType("Material Request")
		mr_item = saashq.qb.DocType("Material Request Item")
	...
	def get_bom_item_condition(self):
		...
		has_bom = saashq.db.exists({"doctype": "BOM", "item": self.item_code, "docstatus": 1})

		if not has_bom:
			bom = saashq.qb.DocType("BOM")
			template_item = saashq.db.get_value("Item", self.item_code, ["variant_of"])
	...
	def get_so_items(self):
		...
		if not self.get("sales_orders") or not self.get_so_mr_list("sales_order", "sales_orders"):
			saashq.throw(_("Please fill the Sales Orders table"), title=_("Sales Orders Required"))

		so_list = self.get_so_mr_list("sales_order", "sales_orders")

		bom = saashq.qb.DocType("BOM")
		so_item = saashq.qb.DocType("Sales Order Item")

		items_subquery = saashq.qb.from_(bom).select(bom.name).where(bom.is_active == 1)
		items_query = (
			saashq.qb.from_(so_item)
			.select(
				so_item.parent,
				so_item.item_code,
				so_item.warehouse,
		...
		if self.item_code and saashq.db.exists("Item", self.item_code):
		...
		pi = saashq.qb.DocType("Packed Item")
	...
	def get_mr_items(self):
		...
		if not self.get("material_requests") or not self.get_so_mr_list(
			...
			saashq.throw(_("Please fill the Material Requests table"), title=_("Material Requests Required"))
	...
	def get_so_wise_planned_qty(sales_orders):
		so_wise_planned_qty = saashq._dict()
		data = saashq.get_all(
	...
	def update_bin_qty(self):
		for d in self.mr_items:
			if d.warehouse:
				...
				bin = saashq.get_doc("Bin", bin_name, for_update=True)
	...
	def delete_draft_work_order(self):
		for d in saashq.get_all(
			...
			saashq.delete_doc("Work Order", d.name)
	...
	def get_production_items(self):
		...
		for d in self.po_items:
			...
			if not item_details["project"] and d.sales_order:
				item_details["project"] = saashq.get_cached_value("Sales Order", d.sales_order, "project")
	...
	def make_work_order(self):
		from erpnexus.manufacturing.doctype.work_order.work_order import get_default_warehouse
		...
		if not wo_list:
			saashq.msgprint(_("No Work Orders were created"))
	...
	def show_list_created_message(self, doctype, doc_list=None):
		...
		saashq.flags.mute_messages = False
		if doc_list:
			doc_list = [get_link_to_form(doctype, p) for p in doc_list]
			msgprint(_("{0} created").format(comma_and(doc_list)))

	def create_work_order(self, item):
		from erpnexus.manufacturing.doctype.work_order.work_order import OverProductionError

		if flt(item.get("qty")) <= 0:
			return

		wo = saashq.new_doc("Work Order")
		wo.update(item)
	...
	def make_material_request(self):
		...
		for item in self.mr_items:
			item_doc = saashq.get_cached_doc("Item", item.item_code)
			...
			if key not in material_request_map:
				...
				material_request_map[key] = saashq.new_doc("Material Request")
			...
			material_request.append(
				"items",
				{
					...
					"project": saashq.db.get_value("Sales Order", item.sales_order, "project")
					if item.sales_order
					else None,
				},
			)
	...
	def get_sub_assembly_items(self, manufacturing_type=None):
		...
		for row in self.po_items:
			if self.skip_available_sub_assembly_item and not self.sub_assembly_warehouse:
				saashq.throw(_("Row #{0}: Please select the Sub Assembly Warehouse").format(row.idx))

			if not row.item_code:
				saashq.throw(_("Row #{0}: Please select Item Code in Assembly Items").format(row.idx))

			if not row.bom_no:
				saashq.throw(_("Row #{0}: Please select the BOM No in Assembly Items").format(row.idx))
		...
		if not sub_assembly_items_store and self.skip_available_sub_assembly_item:
			...
			saashq.msgprint(message, title=_("Note"))
	...
	def set_sub_assembly_items_based_on_level(self, row, bom_data, manufacturing_type=None):
		...
		is_group_warehouse = saashq.db.get_value("Warehouse", self.sub_assembly_warehouse, "is_group")
...
def download_raw_materials(doc, warehouses=None):
	if isinstance(doc, str):
		doc = saashq._dict(json.loads(doc))
	...
	doc.warehouse = None
	saashq.flags.show_qty_in_stock_uom = 1
...
def get_exploded_items(item_details, company, bom_no, include_non_stock_items, planned_qty=1, doc=None):
	bei = saashq.qb.DocType("BOM Explosion Item")
	bom = saashq.qb.DocType("BOM")
	item = saashq.qb.DocType("Item")
	item_default = saashq.qb.DocType("Item Default")
	item_uom = saashq.qb.DocType("UOM Conversion Detail")
...
def get_subitems(
	...
	bom_item = saashq.qb.DocType("BOM Item")
	bom = saashq.qb.DocType("BOM")
	item = saashq.qb.DocType("Item")
	item_default = saashq.qb.DocType("Item Default")
	item_uom = saashq.qb.DocType("UOM Conversion Detail")
...
def get_material_request_items(
	...
	if row["purchase_uom"] != row["stock_uom"]:
		if not (row["conversion_factor"] or saashq.flags.show_qty_in_stock_uom):
			saashq.throw(
	...
	if saashq.db.get_value("UOM", row["purchase_uom"], "must_be_whole_number"):
		required_qty = ceil(required_qty)

	if include_safety_stock:
		required_qty += flt(row["safety_stock"])

	item_details = saashq.get_cached_value("Item", row.item_code, ["purchase_uom", "stock_uom"], as_dict=1)
...
def get_sales_orders(self):
	...
	open_so_subquery1 = saashq.qb.from_(bom).select(bom.name).where(bom.is_active == 1)
...
def get_bin_details(row, company, for_warehouse=None, all_warehouse=False):
	if isinstance(row, str):
		row = saashq._dict(json.loads(row))

	bin = saashq.qb.DocType("Bin")
	wh = saashq.qb.DocType("Warehouse")

	subquery = saashq.qb.from_(wh).select(wh.name).where(wh.company == company)
...
def get_warehouse_list(warehouses):
	...
	for row in warehouses:
		child_warehouses = saashq.db.get_descendants("Warehouse", row.get("warehouse"))
		if child_warehouses:
...
def get_items_for_material_requests(doc, warehouses=None, get_parent_warehouse_data=None):
	...
	for data in po_items:
		...
		if data.get("bom") or data.get("bom_no"):
			...
			if not planned_qty:
				saashq.throw(_("For row {0}: Enter Planned Qty").format(data.get("idx")))
		...
		for item_code, details in item_details.items():
			so_item_details.setdefault(sales_order, saashq._dict())
	...
	if not mr_items:
		to_enable = saashq.bold(_("Ignore Existing Projected Quantity"))
		warehouse = saashq.bold(doc.get("for_warehouse"))
		message = (
			_(
...
def get_materials_from_other_locations(item, warehouses, new_mr_items, company):
	from erpnexus.stock.doctype.pick_list.pick_list import get_available_item_locations

	stock_uom, purchase_uom = saashq.db.get_value(
	...
	precision = saashq.get_precision("Material Request Plan Item", "quantity")
	if flt(required_qty, precision) > 0:
		required_qty = required_qty

		if saashq.db.get_value("UOM", purchase_uom, "must_be_whole_number"):
			required_qty = ceil(required_qty)
...
def get_sub_assembly_items(bom_no, bom_data, to_produce_qty, company, warehouse=None, indent=0):
	...
	for d in data:
		if d.expandable:
			parent_item_code = saashq.get_cached_value("BOM", bom_no, "item")
...
def get_reserved_qty_for_production_plan(item_code, warehouse):
	from erpnexus.manufacturing.doctype.work_order.work_order import get_reserved_qty_for_production

	table = saashq.qb.DocType("Production Plan")
	child = saashq.qb.DocType("Material Request Plan Item")
...
def get_non_completed_production_plans():
	table = saashq.qb.DocType("Production Plan")
	child = saashq.qb.DocType("Production Plan Item")
...
def get_raw_materials_of_sub_assembly_items(
	...
	bei = saashq.qb.DocType("BOM Item")
	bom = saashq.qb.DocType("BOM")
	item = saashq.qb.DocType("Item")
	item_default = saashq.qb.DocType("Item Default")
	item_uom = saashq.qb.DocType("UOM Conversion Detail")
...
def sales_order_query(doctype=None, txt=None, searchfield=None, start=None, page_len=None, filters=None):
	saashq.has_permission("Production Plan", throw=True)

	if not filters:
		filters = {}

	so_table = saashq.qb.DocType("Sales Order")
	table = saashq.qb.DocType("Sales Order Item")

	query = (
		saashq.qb.from_(so_table)
		.join(table)
...
def get_reserved_qty_for_sub_assembly(item_code, warehouse):
	table = saashq.qb.DocType("Production Plan")
	child = saashq.qb.DocType("Production Plan Sub Assembly Item")
