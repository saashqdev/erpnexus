...
from saashq.utils import add_to_date, flt, getdate, now_datetime, nowdate
...
from erpnexus.manufacturing.doctype.work_order.work_order import make_stock_entry as make_se_from_wo
...
class TestProductionPlan(IntegrationTestCase):
	def setUp(self):
		for item in [
			...
			sr = saashq.db.get_value(
				"Stock Reconciliation Item", {"item_code": item, "docstatus": 1}, "parent"
			)
			if sr:
				sr_doc = saashq.get_doc("Stock Reconciliation", sr)
				sr_doc.cancel()
	...
	def test_production_plan_mr_creation(self):
		...
		material_requests = saashq.get_all(
		...
		for row in material_requests:
			mr_schedule_date = getdate(saashq.db.get_value("Material Request", row[0], "schedule_date"))
		...
		for name in material_requests:
			mr = saashq.get_doc("Material Request", name[0])
			if mr.docstatus != 0:
				mr.cancel()

		for name in work_orders:
			mr = saashq.delete_doc("Work Order", name[0])

		pln = saashq.get_doc("Production Plan", pln.name)
		pln.cancel()
	...
	def test_production_plan_start_date(self):
		...
		for wo in work_orders:
			saashq.delete_doc("Work Order", wo.name)
	...
	def test_production_plan_sales_orders(self):
		...
		pln = saashq.new_doc("Production Plan")
	...
	def test_donot_allow_to_make_multiple_pp_against_same_so(self):
		...
		pln.get_so_items()
		self.assertRaises(saashq.ValidationError, pln.save)
	...
	def test_so_based_bill_of_material(self):
		...
		pln1 = saashq.new_doc("Production Plan")
		...
		pln2 = saashq.new_doc("Production Plan")
	...
	def test_production_plan_combine_items(self):
		...
		for so_item in so_items:
			so_wo_qty = saashq.db.get_value("Sales Order Item", so_item, "work_order_qty")
	...
	def test_production_plan_subassembly_default_supplier(self):
		...
		item_doc = saashq.get_doc("Item", "Test Motherboard")
	...
	def test_production_plan_for_subcontracting_po(self):
		from erpnexus.manufacturing.doctype.bom.test_bom import create_nested_bom
		from erpnexus.subcontracting.doctype.subcontracting_bom.test_subcontracting_bom import (
		...
		item_doc = saashq.get_doc("Item", fg_item)
		company = "_Test Company"
		...
		po = saashq.db.get_value("Purchase Order Item", {"production_plan": plan.name}, "parent")
		po_doc = saashq.get_doc("Purchase Order", po)
	...
	def test_production_plan_combine_subassembly(self):
		...
		saashq.db.set_value("BOM Item", parent_bom_2.items[0].name, "bom_no", subassembly_bom)
		...
		plan.append(
			...
			{  # Add Green-Car to Prod Plan
				...
				"item_code": "Green-Car",
				"bom_no": saashq.db.get_value("Item", "Green-Car", "default_bom"),
				"planned_qty": 1,
	...
	def test_pp_to_mr_customer_provided(self):
		...
		material_request = saashq.db.get_value(
			"Material Request Item",
			{"production_plan": production_plan.name, "item_code": "CUST-0987"},
			"parent",
		)
		mr = saashq.get_doc("Material Request", material_request)
	...
	def test_production_plan_with_multi_level_bom(self):
		...
		if not saashq.db.get_value("BOM", {"item": "Test BOM 3"}):
			make_bom(item="Test BOM 3", raw_materials=["Test RM BOM 1"], rm_qty=3)

		if not saashq.db.get_value("BOM", {"item": "Test BOM 2"}):
			make_bom(item="Test BOM 2", raw_materials=["Test BOM 3"], rm_qty=3)

		if not saashq.db.get_value("BOM", {"item": "Test BOM 1"}):
		...
		pln.append(
			...
			{
				"item_code": item_code,
				"bom_no": saashq.db.get_value("BOM", {"item": "Test BOM 1"}),
				"planned_qty": 3,
			},
		)
	...
	def test_get_sales_order_with_variant(self):
		...
		if not saashq.db.exists("Item", {"item_code": "PIV"}):
		...
		if not saashq.db.exists("BOM", {"item": "PIV"}):
			parent_bom = make_bom(item="PIV", raw_materials=[rm_item.item_code])
		else:
			parent_bom = saashq.get_doc("BOM", {"item": "PIV"})

		if not saashq.db.exists("Item", {"item_code": "PIV-RED"}):
			variant = create_variant("PIV", {"Colour": "Red"})
			variant.save()
			variant_bom = make_bom(item=variant.item_code, raw_materials=[rm_item.item_code])
		else:
			variant = saashq.get_doc("Item", "PIV-RED")
		if not saashq.db.exists("BOM", {"item": "PIV-RED"}):
			variant_bom = make_bom(item=variant.item_code, raw_materials=[rm_item.item_code])
		...
		variant_bom.cancel()
		saashq.delete_doc("BOM", variant_bom.name)
	...
	def test_multiple_work_order_for_production_plan_item(self):
		...
		def create_work_order(item, pln, qty):
			...
			for _key, item in items_data.items():
				...
				wo_doc = saashq.get_doc("Work Order", wo_name)
				wo_doc.update(
		...
		wo1 = saashq.get_doc("Work Order", wo_list[0])
		wo1.cancel()
		pln.reload()
		self.assertEqual(pln.po_items[0].ordered_qty, 2)

		# Cancel 2nd Work Order
		wo2 = saashq.get_doc("Work Order", wo_list[1])
		wo2.cancel()
		pln.reload()
	...
	def test_production_plan_pending_qty_with_sales_order(self):
		...
		se = saashq.get_doc(make_se_from_wo(wo.name, "Manufacture", 1))
		se.submit()
	...
	def test_qty_based_status(self):
		pp = saashq.new_doc("Production Plan")
		pp.po_items = [saashq._dict(planned_qty=5, produce_qty=4)]
		self.assertFalse(pp.all_items_completed())

		pp.po_items = [
			saashq._dict(planned_qty=5, produce_qty=10),
			saashq._dict(planned_qty=5, produce_qty=4),
		]
		self.assertFalse(pp.all_items_completed())
	...
	def test_temporary_name_relinking(self):
		...
		for _ in range(10):
			po_item = pp.append(
				"po_items",
				{
					...
					"temporary_name": saashq.generate_hash(length=10),
				},
			)
		...
		pp.append("sub_assembly_items", {"production_plan_item": saashq.generate_hash(length=16)})
		pp._rename_temporary_references()
	...
	def test_produced_qty_for_multi_level_bom_item(self):
		...
		pln.make_work_order()
		work_orders = saashq.get_all("Work Order", filters={"production_plan": pln.name}, pluck="name")
		sa_wo = fg_wo = None
		...
		se = saashq.get_doc(make_se_from_wo(sa_wo, "Manufacture"))
		se.submit()

		se = saashq.get_doc(make_se_from_wo(fg_wo, "Manufacture"))
		se.submit()
	...
	def test_material_request_item_for_purchase_uom(self):
		...
		if not saashq.db.exists("UOM Conversion Detail", {"parent": bom_item, "uom": "Nos"}):
			doc = saashq.get_doc("Item", bom_item)
	...
	def test_resered_qty_for_production_plan_for_material_requests(self):
		...
		before_qty = flt(saashq.db.get_value("Bin", bin_name, "reserved_qty_for_production_plan"))

		pln = create_production_plan(item_code="Test Production Item 1")

		bin_name = get_or_make_bin("Raw Material Item 1", "_Test Warehouse - _TC")
		after_qty = flt(saashq.db.get_value("Bin", bin_name, "reserved_qty_for_production_plan"))
	...
	def test_resered_qty_for_production_plan_for_work_order(self):
		...
		work_orders = []
		for row in saashq.get_all("Work Order", filters={"production_plan": pln.name}, fields=["name"]):
			wo_doc = saashq.get_doc("Work Order", row.name)
		...
		if rm_work_order:
			s = saashq.get_doc(make_se_from_wo(rm_work_order.name, "Material Transfer for Manufacture", 1))
			s.submit()
			bin_name = get_or_make_bin("Raw Material Item 1", "_Test Warehouse - _TC")
			after_qty = flt(saashq.db.get_value("Bin", bin_name, "reserved_qty_for_production_plan"))
	...
	def test_resered_qty_for_production_plan_for_material_requests_with_multi_UOM(self):
		...
		for row in pln.mr_items:
			...
			reserved_qty = flt(saashq.db.get_value("Bin", bin_name, "reserved_qty_for_production_plan"))
			self.assertEqual(reserved_qty - before_qty, 100.0)

		pln.submit_material_request = 1
		pln.make_work_order()

		for work_order in saashq.get_all(
			"Work Order",
			fields=["name"],
			filters={"production_plan": pln.name},
		):
			wo_doc = saashq.get_doc("Work Order", work_order.name)
			wo_doc.source_warehouse = "_Test Warehouse - _TC"
			wo_doc.wip_warehouse = "_Test Warehouse 1 - _TC"
			wo_doc.fg_warehouse = "_Test Warehouse - _TC"
			wo_doc.submit()

		reserved_qty_after_mr = flt(saashq.db.get_value("Bin", bin_name, "reserved_qty_for_production_plan"))
	...
	def test_transfer_and_purchase_mrp_for_purchase_uom(self):
		...
		parent_bom = create_nested_bom(bom_tree, prefix="")
		if not saashq.db.exists("UOM Conversion Detail", {"parent": "Test RM Item INK", "uom": "Kg"}):
			doc = saashq.get_doc("Item", "Test RM Item INK")
			doc.purchase_uom = "Kg"
	...
	def test_mr_qty_for_complex_bom(self):
		...
		def set_bom_qty(item_code, qtys):
			...
			bom = saashq.get_doc("BOM", {"item": item_code})
	...
	def test_reserve_sub_assembly_items(self):
		...
		for d in work_orders:
			wo_doc = saashq.get_doc("Work Order", d.name)
			...
			if d.production_item == "Frame Assembly":
				...
				se_doc = saashq.get_doc(make_se_from_wo(wo_doc.name, "Manufacture", 2))
				se_doc.submit()
	...
	if not args.get("sales_order"):
		pln.append(
			"po_items",
			{
				...
				"bom_no": saashq.db.get_value("Item", args.item_code, "default_bom"),
