...
def get_mins_between_operations():
	return relativedelta(
		minutes=cint(saashq.db.get_single_value("Manufacturing Settings", "mins_between_operations")) or 10
	)


@saashq.whitelist()
def is_material_consumption_enabled():
	if not hasattr(saashq.local, "material_consumption"):
		saashq.local.material_consumption = cint(
			saashq.db.get_single_value("Manufacturing Settings", "material_consumption")
		)

	return saashq.local.material_consumption
