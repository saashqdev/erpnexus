...
class BlanketOrder(Document):
	...
	if TYPE_CHECKING:
		...
		from erpnexus.manufacturing.doctype.blanket_order_item.blanket_order_item import BlanketOrderItem
	...
	def validate_duplicate_items(self):
		...
		for item in self.items:
			if item.item_code in item_list:
				saashq.throw(_("Note: Item {0} added multiple times").format(saashq.bold(item.item_code)))
			item_list.append(item.item_code)

	def update_ordered_qty(self):
		ref_doctype = "Sales Order" if self.blanket_order_type == "Selling" else "Purchase Order"

		trans = saashq.qb.DocType(ref_doctype)
		trans_item = saashq.qb.DocType(f"{ref_doctype} Item")

		item_ordered_qty = saashq._dict(
			(
				saashq.qb.from_(trans_item)
				.from_(trans)
...
def make_order(source_name):
	doctype = saashq.flags.args.doctype
	...
	if order_doc.doctype in ("Sales Order", "Purchase Order"):
		...
		if order_data:
			...
			for bo_name, item_data in order_data.items():
				bo_doc = saashq.get_doc("Blanket Order", bo_name)
