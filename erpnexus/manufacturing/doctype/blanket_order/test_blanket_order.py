...
class TestBlanketOrder(IntegrationTestCase):
	...
	def test_sales_order_creation(self):
		...
		saashq.flags.args.doctype = "Sales Order"
		...
		bo = saashq.get_doc("Blanket Order", bo.name)
	...
	def test_purchase_order_creation(self):
		...
		saashq.flags.args.doctype = "Purchase Order"
	...
	def test_blanket_order_allowance(self):
		...
		saashq.db.set_single_value("Selling Settings", "blanket_order_allowance", 10)
		so.submit()
		...
		po.schedule_date = today()
		po.items[0].qty = 110
		self.assertRaises(saashq.ValidationError, po.submit)

		saashq.db.set_single_value("Buying Settings", "blanket_order_allowance", 10)
		po.submit()
	...
	def test_party_item_code(self):
		...
		if not saashq.db.exists("Item Customer Detail", {"customer_name": customer, "parent": item_code}):
			item_doc.append("customer_items", {"customer_name": customer, "ref_code": "CUST-REF-1"})
			item_doc.save()

		if not saashq.db.exists("Item Supplier", {"supplier": supplier, "parent": item_code}):
...
def make_blanket_order(**args):
	...
	bo = saashq.new_doc("Blanket Order")
