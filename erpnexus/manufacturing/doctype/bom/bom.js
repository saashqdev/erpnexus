...
saashq.ui.form.on("BOM", {
	setup(frm) {
		...
		frm.set_query("bom_no", "operations", function (doc, cdt, cdn) {
			...
			return {
				query: "erpnexus.controllers.queries.bom",
				filters: {
					currency: frm.doc.currency,
					company: frm.doc.company,
		...
		frm.set_query("item", function () {
			return {
				query: "erpnexus.manufacturing.doctype.bom.bom.item_query",
				filters: {
					is_stock_item: 1,
				},
			};
	...
	default_target_warehouse(frm) {
		if (frm.doc.default_source_warehouse) {
			frm.doc.operations.forEach((d) => {
				saashq.model.set_value(d.doctype, d.name, "fg_warehouse", frm.doc.default_target_warehouse);
			});
		}
	},
	...
	refresh(frm) {
		...
		if (!frm.is_new() && !frm.doc.docstatus == 0) {
			frm.add_custom_button(
				__("New Version"),
				function () {
					let new_bom = saashq.model.copy_doc(frm.doc);
					saashq.set_route("Form", "BOM", new_bom.name);
				},
				__("Actions")
			);
		}
		...
		if (frm.doc.has_variants) {
			...
			frm.$wrapper.find(".variants-intro").on("click", () => {
				saashq.set_route("List", "Item", { variant_of: frm.doc.item });
			});
		}
	},
	...
	make_work_order(frm) {
		frm.events.setup_variant_prompt(frm, "Work Order", (frm, item, data, variant_items) => {
			saashq.call({
				method: "erpnexus.manufacturing.doctype.work_order.work_order.make_work_order",
				args: {
					bom_no: frm.doc.name,
					item: item,
					qty: data.qty || 0.0,
				...
				callback(r) {
					if (r.message) {
						let doc = saashq.model.sync(r.message)[0];
	...
	make_variant_bom(frm) {
		frm.events.setup_variant_prompt(
			...
			(frm, item, data, variant_items) => {
				saashq.call({
					method: "erpnexus.manufacturing.doctype.bom.bom.make_variant_bom",
					args: {
			...
			fields.push({
				...
				fields: [
					...
					},
					{
						...
						get_query(data) {
							if (!data.item_code) {
								saashq.throw(__("Select template item"));
							}
			...
			(data) => {
				...
				variant_items.forEach((d) => {
					if (!d.variant_item_code) {
						saashq.throw(__("Select variant item code for the template item {0}", [d.item_code]));
					}
				});
	...
	make_quality_inspection(frm) {
		saashq.model.open_mapped_doc({
			method: "erpnexus.stock.doctype.quality_inspection.quality_inspection.make_quality_inspection",
			frm: frm,
		});
	},
		...
		if (frm.doc.routing) {
			saashq.call({
				...
				callback(r) {
					if (!r.exc) {
						frm.refresh_fields();
						erpnexus.bom.calculate_op_cost(frm.doc);
						erpnexus.bom.calculate_total(frm.doc);
					}
				},
			});
		}
...
saashq.ui.form.on("BOM Operation", {
	bom_no(frm, cdt, cdn) {
		let row = locals[cdt][cdn];
...
erpnexus.bom.BomController = class BomController extends erpnexus.TransactionController {
	conversion_rate(doc) {
...
extend_cscript(cur_frm.cscript, new erpnexus.bom.BomController({ frm: cur_frm }));

cur_frm.cscript.hour_rate = function (doc) {
	erpnexus.bom.calculate_op_cost(doc);
	erpnexus.bom.calculate_total(doc);
};
	...
	if (d.item_code) {
		return saashq.call({
			...
			callback: function (r) {
				...
				erpnexus.bom.calculate_rm_cost(doc);
				erpnexus.bom.calculate_scrap_materials_cost(doc);
				erpnexus.bom.calculate_total(doc);
			},
			freeze: true,
		});
...
cur_frm.cscript.rate = function (doc, cdt, cdn) {
	...
	if (d.bom_no) {
		saashq.msgprint(__("You cannot change the rate if BOM is mentioned against any Item."));
...
erpnexus.bom.update_cost = function (doc) {
	erpnexus.bom.calculate_op_cost(doc);
	erpnexus.bom.calculate_rm_cost(doc);
	erpnexus.bom.calculate_scrap_materials_cost(doc);
	erpnexus.bom.calculate_total(doc);
...
erpnexus.bom.calculate_op_cost = function (doc) {
	...
	if (doc.with_operations) {
		doc.operations.forEach((item) => {
			...
			saashq.model.set_value("BOM Operation", item.name, {
...
// rm : raw material
erpnexus.bom.calculate_rm_cost = function (doc) {
	var rm = doc.items || [];
	var total_rm_cost = 0;
	...
	for (var i = 0; i < rm.length; i++) {
		...
		saashq.model.set_value("BOM Item", rm[i].name, "amount", amount);
		saashq.model.set_value("BOM Item", rm[i].name, "base_amount", base_amount);
		saashq.model.set_value(
			"BOM Item",
			rm[i].name,
...
// sm : scrap material
erpnexus.bom.calculate_scrap_materials_cost = function (doc) {
	...
	for (var i = 0; i < sm.length; i++) {
		...
		saashq.model.set_value("BOM Scrap Item", sm[i].name, "base_rate", base_rate);
		saashq.model.set_value("BOM Scrap Item", sm[i].name, "amount", amount);
		saashq.model.set_value("BOM Scrap Item", sm[i].name, "base_amount", base_amount);
...
erpnexus.bom.calculate_total = function (doc) {
...
saashq.ui.form.on("BOM Operation", "operation", function (frm, cdt, cdn) {
	var d = locals[cdt][cdn];
	...
	saashq.call({
		...
		callback: function (data) {
			if (data.message.description) {
				saashq.model.set_value(d.doctype, d.name, "description", data.message.description);
			}
			if (data.message.workstation) {
				saashq.model.set_value(d.doctype, d.name, "workstation", data.message.workstation);
			}
		},
	});
});

saashq.ui.form.on("BOM Operation", "workstation", function (frm, cdt, cdn) {
	var d = locals[cdt][cdn];
	...
	saashq.call({
		...
		callback: function (data) {
			saashq.model.set_value(d.doctype, d.name, "base_hour_rate", data.message.hour_rate);
			saashq.model.set_value(
				d.doctype,
				d.name,
				"hour_rate",
...
saashq.ui.form.on("BOM Item", {
	do_not_explode: function (frm, cdt, cdn) {
		get_bom_material_detail(frm.doc, cdt, cdn, false);
	},
});

saashq.ui.form.on("BOM Item", "qty", function (frm, cdt, cdn) {
	var d = locals[cdt][cdn];
	d.stock_qty = d.qty * d.conversion_factor;
	refresh_field("stock_qty", d.name, d.parentfield);
});

saashq.ui.form.on("BOM Item", "item_code", function (frm, cdt, cdn) {
	var d = locals[cdt][cdn];
	saashq.db.get_value("Item", { name: d.item_code }, "allow_alternative_item", (r) => {
		d.allow_alternative_item = r.allow_alternative_item;
	});
	refresh_field("allow_alternative_item", d.name, d.parentfield);
});

saashq.ui.form.on("BOM Item", "sourced_by_supplier", function (frm, cdt, cdn) {
	var d = locals[cdt][cdn];
...
saashq.ui.form.on("BOM Item", "rate", function (frm, cdt, cdn) {
	var d = locals[cdt][cdn];
	if (d.sourced_by_supplier) {
		d.rate = 0;
		refresh_field("rate", d.name, d.parentfield);
...
saashq.ui.form.on("BOM Operation", "operations_remove", function (frm) {
	erpnexus.bom.calculate_op_cost(frm.doc);
	erpnexus.bom.calculate_total(frm.doc);
});

saashq.ui.form.on("BOM Item", "items_remove", function (frm) {
	erpnexus.bom.calculate_rm_cost(frm.doc);
	erpnexus.bom.calculate_total(frm.doc);
});
...
saashq.ui.form.on("BOM Scrap Item", {
...
saashq.ui.form.on("BOM", {
	_prompt_for_raw_materials(frm, row) {
		...
		frm._bom_rm_dialog = new saashq.ui.Dialog({
