...
from saashq.utils import cint, cstr, flt, parse_json, today
from saashq.website.website_generator import WebsiteGenerator

import erpnexus
from erpnexus.setup.utils import get_exchange_rate
from erpnexus.stock.doctype.item.item import get_item_details
from erpnexus.stock.get_item_details import get_conversion_factor, get_price_list_rate

form_grid_templates = {"items": "templates/form_grid/item_grid.html"}


class BOMRecursionError(saashq.ValidationError):
	pass
...
class BOMTree:
	...
	def __create_tree(self):
		bom = saashq.get_cached_doc("BOM", self.name)
...
class BOM(WebsiteGenerator):
	...
	if TYPE_CHECKING:
		...
		from erpnexus.manufacturing.doctype.bom_explosion_item.bom_explosion_item import BOMExplosionItem
		from erpnexus.manufacturing.doctype.bom_item.bom_item import BOMItem
		from erpnexus.manufacturing.doctype.bom_operation.bom_operation import BOMOperation
		from erpnexus.manufacturing.doctype.bom_scrap_item.bom_scrap_item import BOMScrapItem
	...
	def autoname(self):
		...
		if saashq.db.exists("BOM", name):
			conflicting_bom = saashq.get_doc("BOM", name)

			if conflicting_bom.item != self.item:
				msg = _("A BOM with name {0} already exists for item {1}.").format(
					saashq.bold(name), saashq.bold(conflicting_bom.item)
				)
	...
	def validate(self):
		self.route = saashq.scrub(self.name).replace("_", "-")

		if not self.company:
			saashq.throw(_("Please select a Company first."), title=_("Mandatory"))
	...
	def on_update(self):
		saashq.cache().hdel("bom_children", self.name)
	...
	def update_bom_creator_status(self):
		...
		doc = saashq.get_doc("BOM Creator", self.bom_creator)
	...
	def get_item_det(self, item_code):
		...
		if not item:
			saashq.throw(_("Item: {0} does not exist in the system").format(item_code))
	...
	def get_bom_material_detail(self, args=None):
		...
		if not args:
			args = saashq.form_dict.get("args")
	...
	def validate_bom_currency(self, item):
		if item.get("bom_no") and saashq.db.get_value("BOM", item.get("bom_no"), "currency") != self.currency:
			saashq.throw(
	...
	def get_rm_rate(self, arg):
		...
		if arg.get("scrap_items"):
			...
			if not saashq.db.get_value("Item", arg["item_code"], "is_customer_provided_item") and not arg.get(
				"sourced_by_supplier"
			):
	...
	def update_cost(self, update_parent=True, from_child_bom=False, update_hour_rate=True, save=True):
		...
		if self.total_cost != existing_bom_cost and update_parent:
			parent_boms = saashq.db.sql_list(
			...
			for bom in parent_boms:
				saashq.get_doc("BOM", bom).update_cost(from_child_bom=True)
		...
		if not from_child_bom:
			...
			saashq.msgprint(_(msg), alert=True)
	...
	def manage_default_bom(self):
		...
		if self.is_default and self.is_active:
			from saashq.model.utils import set_default

			set_default(self, "item")
			item = saashq.get_doc("Item", self.item)
			if item.default_bom != self.name:
				saashq.db.set_value("Item", self.item, "default_bom", self.name)
		elif (
			not saashq.db.exists(dict(doctype="BOM", docstatus=1, item=self.item, is_default=1))
			and self.is_active
		):
			...
			item = saashq.get_doc("Item", self.item)
			if item.default_bom == self.name:
				saashq.db.set_value("Item", self.item, "default_bom", None)
	...
	def validate_main_item(self):
		...
		if not item:
			saashq.throw(_("Item {0} does not exist in the system or has expired").format(self.item))
		else:
			ret = saashq.db.get_value("Item", self.item, ["description", "stock_uom", "item_name"])
			self.description = ret[0]
			self.uom = ret[1]
			self.item_name = ret[2]

		if not self.quantity:
			saashq.throw(_("Quantity should be greater than 0"))

	def validate_currency(self):
		if self.rm_cost_as_per == "Price List":
			price_list_currency = saashq.db.get_value("Price List", self.buying_price_list, "currency")
	...
	def validate_materials(self):
		...
		if not self.get("items"):
			saashq.throw(_("Raw Materials cannot be blank."))
		...
		for m in self.get("items"):
			...
			if flt(m.qty) <= 0:
				saashq.throw(_("Quantity required for Item {0} in row {1}").format(m.item_code, m.idx))
			check_list.append(m)
	...
	def add_materials_from_bom(self, finished_good, bom_no, operation_row_id, qty=None):
		if not saashq.db.exists("BOM", {"item": finished_good, "name": bom_no, "docstatus": 1}):
			saashq.throw(_("BOM {0} not found for the item {1}").format(bom_no, finished_good))
	...
	def traverse_tree(self, bom_list=None):
		def _get_children(bom_no):
			children = saashq.cache().hget("bom_children", bom_no)
			if children is None:
				...
				saashq.cache().hset("bom_children", bom_no, children)
			return children
	...
	def update_rate_and_time(self, row, update_hour_rate=False):
		if not row.hour_rate or update_hour_rate:
			hour_rate = flt(saashq.get_cached_value("Workstation", row.workstation, "hour_rate"))
	...
	def get_rm_rate_map(self) -> dict[str, float]:
		...
		for item in self.get("items"):
			if item.bom_no:
				...
				explosion_items = saashq.get_all(
					"BOM Explosion Item",
	...
	def company_currency(self):
		return erpnexus.get_company_currency(self.company)
	...
	def add_exploded_items(self, save=True):
		...
		if save:
			saashq.db.sql("""delete from `tabBOM Explosion Item` where parent=%s""", self.name)
	...
	def validate_bom_links(self):
		if not self.is_active:
			...
			if act_pbom and act_pbom[0][0]:
				saashq.throw(_("Cannot deactivate or cancel BOM as it is linked with other BOMs"))
	...
	def validate_operations(self):
		if self.with_operations and not self.get("operations") and self.docstatus == 1:
			saashq.throw(_("Operations cannot be left blank"))

		if self.with_operations:
			for d in self.operations:
				if not d.description:
					d.description = saashq.db.get_value("Operation", d.operation, "description")
	...
	def validate_scrap_items(self):
		must_be_whole_number = saashq.get_value("UOM", self.uom, "must_be_whole_number")

		if self.process_loss_percentage and self.process_loss_percentage > 100:
			saashq.throw(_("Process Loss Percentage cannot be greater than 100"))

		if self.process_loss_qty and must_be_whole_number and self.process_loss_qty % 1 != 0:
			msg = f"Item: {saashq.bold(self.item)} with Stock UOM: {saashq.bold(self.uom)} can't have fractional process loss qty as UOM {saashq.bold(self.uom)} is a whole Number."
			saashq.throw(msg, title=_("Invalid Process Loss Configuration"))
...
def get_bom_item_rate(args, bom_doc):
	if bom_doc.rm_cost_as_per == "Valuation Rate":
		...
		rate = (
			...
			or flt(saashq.db.get_value("Item", args["item_code"], "last_purchase_rate"))
		) * (args.get("conversion_factor") or 1)
	elif bom_doc.rm_cost_as_per == "Price List":
		if not bom_doc.buying_price_list:
			saashq.throw(_("Please select Price List"))
		bom_args = saashq._dict(
			{
				"doctype": "BOM",
		...
		item_doc = saashq.get_cached_doc("Item", args.get("item_code"))
...
def get_valuation_rate(data):
	...
	"""
	from saashq.query_builder.functions import Count, IfNull, Sum
	from pypika import Case

	item_code, company = data.get("item_code"), data.get("company")
	valuation_rate = 0.0

	bin_table = saashq.qb.DocType("Bin")
	wh_table = saashq.qb.DocType("Warehouse")
	item_valuation = (
		saashq.qb.from_(bin_table)
		.join(wh_table)
	...
	if (valuation_rate is not None) and valuation_rate <= 0:
		...
		last_val_rate = (
			...
			.orderby(sle.posting_datetime, order=saashq.qb.desc)
			.orderby(sle.creation, order=saashq.qb.desc)
			.limit(1)
		).run(as_dict=True)
...
def get_bom_items_as_dict(
	...
	if saashq.get_cached_value("BOM", bom, "track_semi_finished_goods"):
		fetch_exploded = 0
	...
	if cint(fetch_exploded):
		...
		items = saashq.db.sql(query, {"qty": qty, "bom": bom, "company": company}, as_dict=True)
	else:
		query = query.format(
			table="BOM Item",
			where_conditions="",
	...
	for item, item_details in item_dict.items():
		for d in [
			...
			company_in_record = saashq.db.get_value(d[0], item_details.get(d[1]), "company")
			if not item_details.get(d[1]) or (company_in_record and company != company_in_record):
				item_dict[item][d[1]] = saashq.get_cached_value("Company", company, d[2]) if d[2] else None
...
def validate_bom_no(item, bom_no):
	...
	if not bom.is_active:
		saashq.throw(_("BOM {0} must be active").format(bom_no))
	if bom.docstatus != 1:
		if not getattr(saashq.flags, "in_test", False):
			saashq.throw(_("BOM {0} must be submitted").format(bom_no))
	if item:
		...
		if (
			...
			or bom.item.lower() == cstr(saashq.db.get_value("Item", item, "variant_of")).lower()
		):
			rm_item_exists = True
		if not rm_item_exists:
			saashq.throw(_("BOM {0} does not belong to Item {1}").format(bom_no, item))


@saashq.whitelist()
def get_children(parent=None, is_root=False, **filters):
	if not parent or parent == "BOM":
		saashq.msgprint(_("Please select a BOM"))
		return

	if parent:
		saashq.form_dict.parent = parent

	if saashq.form_dict.parent:
		bom_doc = saashq.get_cached_doc("BOM", saashq.form_dict.parent)
		saashq.has_permission("BOM", doc=bom_doc, throw=True)

		bom_items = saashq.get_all(
			"BOM Item",
			fields=["item_code", "bom_no as value", "stock_qty"],
			filters=[["parent", "=", saashq.form_dict.parent]],
			order_by="idx",
		)

		item_names = tuple(d.get("item_code") for d in bom_items)

		items = saashq.get_list(
			"Item",
		...
		for bom_item in bom_items:
			...
			bom_item.image = saashq.db.escape(bom_item.image)
...
def add_additional_cost(stock_entry, work_order):
	...
	company_account = saashq.db.get_value(
		"Company",
		work_order.company,
...
def add_non_stock_items_cost(stock_entry, work_order, expense_account):
	bom = saashq.get_doc("BOM", work_order.bom_no)
	...
	non_stock_items = saashq.get_all(
		"Item",
		fields="name",
...
def add_operations_cost(stock_entry, work_order=None, expense_account=None):
	from erpnexus.stock.doctype.stock_entry.stock_entry import get_operating_cost_per_unit
...
def get_bom_diff(bom1, bom2):
	from saashq.model import table_fields

	if bom1 == bom2:
		saashq.throw(
			_("BOM 1 {0} and BOM 2 {1} should not be same").format(saashq.bold(bom1), saashq.bold(bom2))
		)

	doc1 = saashq.get_doc("BOM", bom1)
	doc2 = saashq.get_doc("BOM", bom2)
...
def item_query(doctype, txt, searchfield, start, page_len, filters):
	meta = saashq.get_meta("Item", cached=True)
	...
	if filters and filters.get("item_code"):
		has_variants = saashq.get_cached_value("Item", filters.get("item_code"), "has_variants")
		if not has_variants:
...
def make_variant_bom(source_name, bom_no, item, variant_items, target_doc=None):
	from erpnexus.manufacturing.doctype.work_order.work_order import add_variant_item
...
def get_op_cost_from_sub_assemblies(bom_no, op_cost=0):
	...
	for row in bom_items:
		...
		if cost := saashq.get_cached_value("BOM", row.bom_no, "operating_cost_per_bom_quantity"):
			op_cost += flt(cost)
