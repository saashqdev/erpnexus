...
from saashq.utils import cstr, flt

from erpnexus.controllers.tests.test_subcontracting_controller import (
	set_backflush_based_on,
)
from erpnexus.manufacturing.doctype.bom.bom import BOMRecursionError, item_query, make_variant_bom
...
class TestBOM(IntegrationTestCase):
	...
	def test_get_items_list(self):
		from erpnexus.manufacturing.doctype.bom.bom import get_bom_items
	...
	def test_default_bom(self):
		def _get_default_bom_in_item():
			return cstr(saashq.db.get_value("Item", "_Test FG Item 2", "default_bom"))

		bom = saashq.get_doc("BOM", {"item": "_Test FG Item 2", "is_default": 1})
	...
	def test_update_bom_cost_in_all_boms(self):
		...
		bom_rates = saashq.db.get_values(
			"BOM Item",
			{
	...
	def test_bom_cost(self):
		bom = saashq.copy_doc(self.globalTestRecords["BOM"][2])
		bom.insert()
	...
	def test_bom_cost_multi_uom_multi_currency_based_on_price_list(self):
		saashq.db.set_value("Price List", "_Test Price List", "price_not_uom_dependent", 1)
	...
	def test_bom_cost_multi_uom_based_on_valuation_rate(self):
		...
		reset_item_valuation_rate(
			...
			warehouse_list=saashq.get_all("Warehouse", {"is_group": 0, "company": bom.company}, pluck="name"),
			qty=200,
			rate=200,
		)
	...
	def test_bom_cost_with_fg_based_operating_cost(self):
		bom = saashq.copy_doc(self.globalTestRecords["BOM"][4])
		bom.insert()
	...
	def test_subcontractor_sourced_item(self):
		...
		if not saashq.db.exists("Item", "Test Extra Item 1"):
			make_item("Test Extra Item 1", {"is_stock_item": 1, "stock_uom": "Nos"})

		if not saashq.db.exists("Item", "Test Extra Item 2"):
			make_item("Test Extra Item 2", {"is_stock_item": 1, "stock_uom": "Nos"})

		if not saashq.db.exists("Item", "Test Extra Item 3"):
	...
	def test_bom_recursion_1st_level(self):
		...
		bom.item = item_code
		bom.append("items", saashq._dict(item_code=item_code))
		bom.save()
	...
	def test_bom_recursion_transitive(self):
		...
		bom1 = saashq.new_doc("BOM")
		bom1.item = item1
		bom1.append("items", saashq._dict(item_code=item2))
		bom1.save()

		bom2 = saashq.new_doc("BOM")
		bom2.item = item2
		bom2.append("items", saashq._dict(item_code=item1))
		bom2.save()
	...
	def test_bom_with_process_loss_item(self):
		...
		#  PL can't be > 100
		self.assertRaises(saashq.ValidationError, bom_doc.submit)
	...
	def test_exclude_exploded_items_from_bom(self):
		...
		new_bom = saashq.copy_doc(saashq.get_doc("BOM", bom_no))
	...
	def test_valid_transfer_defaults(self):
		bom_with_op = saashq.db.get_value(
			"BOM", {"item": "_Test FG Item 2", "with_operations": 1, "is_active": 1}
		)
		bom = saashq.copy_doc(saashq.get_doc("BOM", bom_with_op), ignore_no_copy=False)
	...
	def test_bom_versioning(self):
		bom_tree = {saashq.generate_hash(length=10): {saashq.generate_hash(length=10): {}}}
		...
		version = saashq.copy_doc(amendment)
		version.docstatus = 0
	...
	def test_clear_inpection_quality(self):
		bom = saashq.copy_doc(self.globalTestRecords["BOM"][2], ignore_no_copy=True)
		bom.docstatus = 0
		bom.is_default = 0
	...
	def test_bom_pricing_based_on_lpp(self):
		...
		parent = saashq.generate_hash(length=10)
		child = saashq.generate_hash(length=10)
	...
	def test_set_default_bom_for_item_having_single_bom(self):
		...
		bom.save()
		bom.submit()
		self.assertEqual(saashq.get_value("Item", fg_item.item_code, "default_bom"), bom.name)

		# Step 2: Uncheck is_active field
		bom.is_active = 0
		bom.save()
		bom.reload()
		self.assertIsNone(saashq.get_value("Item", fg_item.item_code, "default_bom"))
	...
	def test_do_not_include_manufacturing_and_fixed_items(self):
		from erpnexus.manufacturing.doctype.bom.bom import item_query

		if not saashq.db.exists("Asset Category", "Computers-Test"):
			doc = saashq.get_doc({"doctype": "Asset Category", "asset_category_name": "Computers-Test"})
...
def get_default_bom(item_code="_Test FG Item 2"):
	return saashq.db.get_value("BOM", {"item": item_code, "is_active": 1, "is_default": 1})
...
def create_nested_bom(tree, prefix="_Test bom ", submit=True):
	...
	def create_items(bom_tree):
		for item_code, subtree in bom_tree.items():
			...
			if not saashq.db.exists("Item", bom_item_code):
				saashq.get_doc(
	...
	for item in order_of_creating_bom:
		...
		if child_items:
			...
			bom = saashq.get_doc(doctype="BOM", item=bom_item_code)
...
def reset_item_valuation_rate(item_code, warehouse_list=None, qty=None, rate=None):
	...
	if not warehouse_list:
		warehouse_list = saashq.db.sql_list(
			"""
...
def create_process_loss_bom_item(item_tuple):
	...
	if saashq.db.exists("Item", item_code) is None:
