...
from erpnexus.manufacturing.doctype.manufacturing_settings.manufacturing_settings import (
	get_mins_between_operations,
)
from erpnexus.stock.doctype.batch.batch import make_batch
from erpnexus.stock.doctype.item.item import get_item_defaults, validate_end_of_life
from erpnexus.stock.doctype.serial_no.serial_no import get_available_serial_nos, get_serial_nos
from erpnexus.stock.stock_balance import get_planned_qty, update_bin_qty
from erpnexus.stock.utils import get_bin, get_latest_stock_qty, validate_warehouse_company
from erpnexus.utilities.transaction_base import validate_uom_is_integer


class OverProductionError(saashq.ValidationError):
	pass


class CapacityError(saashq.ValidationError):
	pass


class StockOverProductionError(saashq.ValidationError):
	pass


class OperationTooLongError(saashq.ValidationError):
	pass


class ItemHasVariantError(saashq.ValidationError):
	pass


class SerialNoQtyError(saashq.ValidationError):
	pass
...
class WorkOrder(Document):
	...
	if TYPE_CHECKING:
		...
		from erpnexus.manufacturing.doctype.work_order_item.work_order_item import WorkOrderItem
		from erpnexus.manufacturing.doctype.work_order_operation.work_order_operation import (
			WorkOrderOperation,
		)
	...
	def onload(self):
		ms = saashq.get_doc("Manufacturing Settings")
	...
	def show_create_job_card_button(self):
		operation_details = saashq._dict(
			saashq.get_all(
				"Job Card",
	...
	def validate_sales_order(self):
		if self.sales_order:
			...
			if len(so):
				...
				saashq.throw(_("Sales Order {0} is not valid").format(self.sales_order))

	def check_sales_order_on_hold_or_close(self):
		status = saashq.db.get_value("Sales Order", self.sales_order, "status")
		if status in ("Closed", "On Hold"):
			saashq.throw(_("Sales Order {0} is {1}").format(self.sales_order, status))

	def set_default_warehouse(self):
		if not self.wip_warehouse and not self.skip_transfer:
			self.wip_warehouse = saashq.db.get_single_value("Manufacturing Settings", "default_wip_warehouse")
		if not self.fg_warehouse:
			self.fg_warehouse = saashq.db.get_single_value("Manufacturing Settings", "default_fg_warehouse")
	...
	def validate_work_order_against_so(self):
		# already ordered qty
		ordered_qty_against_so = saashq.db.sql(
		...
		allowance_percentage = flt(
			saashq.db.get_single_value("Manufacturing Settings", "overproduction_percentage_for_sales_order")
		)
	...
	def get_transferred_or_manufactured_qty(self, purpose):
		table = saashq.qb.DocType("Stock Entry")
		query = saashq.qb.from_(table).where(
	...
	def update_production_plan_status(self):
		production_plan = saashq.get_doc("Production Plan", self.production_plan)
		produced_qty = 0
		if self.production_plan_item:
			total_qty = saashq.get_all(
				"Work Order",
	...
	def validate_warehouse(self):
		...
		if not self.wip_warehouse and not self.skip_transfer:
			saashq.throw(_("Work-in-Progress Warehouse is required before Submit"))
		if not self.fg_warehouse:
			saashq.throw(_("Target Warehouse is required before Submit"))
	...
	def on_submit(self):
		...
		if self.production_plan and saashq.db.exists(
	...
	def create_serial_no_batch_no(self):
		...
		if not cint(
			saashq.db.get_single_value("Manufacturing Settings", "make_serial_no_batch_from_work_order")
		):
			return
	...
	def create_batch_for_finished_good(self):
		...
		batch_auto_creation = saashq.get_cached_value("Item", self.production_item, "create_new_batch")
		if not batch_auto_creation:
			saashq.msgprint(
				_("Batch not created for item {} since it does not have a batch series.").format(
					saashq.bold(self.production_item)
				),
				alert=True,
				indicator="orange",
			)
	...
	def delete_auto_created_batch_and_serial_no(self):
		for row in saashq.get_all("Serial No", filters={"work_order": self.name}):
			saashq.delete_doc("Serial No", row.name)

		for row in saashq.get_all("Batch", filters={"reference_name": self.name}):
	...
	def make_serial_nos(self, args):
		...
		saashq.db.bulk_insert("Serial No", fields=fields, values=set(serial_nos_details))

	def create_job_card(self):
		manufacturing_settings_doc = saashq.get_doc("Manufacturing Settings")
	...
	def set_operation_start_end_time(self, idx, row):
		...
		if row.planned_start_time == row.planned_end_time:
			saashq.throw(_("Capacity Planning Error, planned start time can not be same as end time"))

	def validate_cancel(self):
		if self.status == "Stopped":
			saashq.throw(_("Stopped Work Order cannot be cancelled, Unstop it first to cancel"))

		# Check whether any stock entry exists against this Work Order
		stock_entry = saashq.db.sql(
		...
		if stock_entry:
			saashq.throw(
				_("Cannot cancel because submitted Stock Entry {0} exists").format(
					saashq.utils.get_link_to_form("Stock Entry", stock_entry[0][0])
				)
			)
	...
	def update_planned_qty(self):
		...
		if self.material_request:
			mr_obj = saashq.get_doc("Material Request", self.material_request)
			mr_obj.update_requested_qty([self.material_request_item])

	def set_produced_qty_for_sub_assembly_item(self):
		table = saashq.qb.DocType("Work Order")
	...
	def update_ordered_qty(self):
		if self.production_plan and self.production_plan_item and not self.production_plan_sub_assembly_item:
			...
			saashq.db.set_value("Production Plan Item", self.production_plan_item, "ordered_qty", qty)

			doc = saashq.get_doc("Production Plan", self.production_plan)
			doc.set_status()
	...
	def update_work_order_qty_in_so(self):
		...
		if self.product_bundle_item:
			total_bundle_qty = saashq.db.sql(
				""" select sum(qty) from
				`tabProduct Bundle Item` where parent = %s""",
				(saashq.db.escape(self.product_bundle_item)),
			)[0][0]
	...
	def update_work_order_qty_in_combined_so(self):
		...
		prod_plan = saashq.get_doc("Production Plan", self.production_plan)
		item_reference = saashq.get_value(
	...
	def update_completed_qty_in_material_request(self):
		if self.material_request and self.material_request_item:
			saashq.get_doc("Material Request", self.material_request).update_completed_qty(
	...
	def set_work_order_operations(self):
		...
		self.set("operations", [])
		if not self.bom_no or not saashq.get_cached_value("BOM", self.bom_no, "with_operations"):
			return

		operations = []

		if self.use_multi_level_bom:
			bom_tree = saashq.get_doc("BOM", self.bom_no).get_tree_representation()
		...
		bom_qty = saashq.get_cached_value("BOM", self.bom_no, "quantity")
	...
	def get_holidays(self, workstation):
		holiday_list = saashq.db.get_value("Workstation", workstation, "holiday_list")
	...
	def update_operation_status(self):
		...
		for d in self.get("operations"):
			...
			if not qty:
				...
				saashq.throw(_("Completed Qty cannot be greater than 'Qty to Manufacture'"))
	...
	def delete_job_card(self):
		for d in saashq.get_all("Job Card", ["name"], {"work_order": self.name}):
			saashq.delete_doc("Job Card", d.name)

	def validate_production_item(self):
		if saashq.get_cached_value("Item", self.production_item, "has_variants"):
			saashq.throw(_("Work Order cannot be raised against a Item Template"), ItemHasVariantError)
	...
	def validate_qty(self):
		if self.qty <= 0:
			saashq.throw(_("Quantity to Manufacture must be greater than 0."))

		if (
			self.stock_uom
			and saashq.get_cached_value("UOM", self.stock_uom, "must_be_whole_number")
			...
			saashq.throw(
				_(
					...
					saashq.bold(_("Must be Whole Number")),
					saashq.bold(self.stock_uom),
				),
			)

		if self.production_plan and self.production_plan_item and not self.production_plan_sub_assembly_item:
			qty_dict = saashq.db.get_value(
	...
	def validate_operation_time(self):
		for d in self.operations:
			if d.time_in_mins <= 0:
				saashq.throw(_("Operation Time must be greater than 0 for Operation {0}").format(d.operation))
	...
	def update_transferred_qty_for_required_items(self):
		ste = saashq.qb.DocType("Stock Entry")
		ste_child = saashq.qb.DocType("Stock Entry Detail")
		...
		transferred_items = saashq._dict({d.original_item or d.item_code: d.qty for d in data})
	...
	def update_returned_qty(self):
		...
		data = query.run(as_dict=1) or []
		returned_dict = saashq._dict({d.original_item or d.item_code: d.qty for d in data})
...
def get_bom_operations(doctype, txt, searchfield, start, page_len, filters):
	...
	return saashq.get_all("BOM Operation", filters=filters, fields=["operation"], as_list=1)
...
def get_item_details(item, project=None, skip_bom_info=False, throw=True):
	...
	res["bom_no"] = saashq.db.get_value("BOM", filters=filters)

	if not res["bom_no"]:
		variant_of = saashq.db.get_value("Item", item, "variant_of")

		if variant_of:
			res["bom_no"] = saashq.db.get_value("BOM", filters={"item": variant_of, "is_default": 1})
	...
	if not res["bom_no"]:
		if project:
			...
			saashq.msgprint(msg, raise_exception=throw, indicator="yellow", alert=(not throw))

			return res

	bom_data = saashq.db.get_value(
		"BOM",
		res["bom_no"],
...
def make_work_order(bom_no, item, qty=0, project=None, variant_items=None):
	if not saashq.has_permission("Work Order", "write"):
		saashq.throw(_("Not permitted"), saashq.PermissionError)

	item_details = get_item_details(item, project)

	wo_doc = saashq.new_doc("Work Order")
	wo_doc.track_semi_finished_goods = saashq.db.get_value("BOM", bom_no, "track_semi_finished_goods")
...
def add_variant_item(variant_items, wo_doc, bom_no, table_name="items"):
	...
	for item in variant_items:
		...
		bom_doc = saashq.get_cached_doc("BOM", bom_no)
...
def set_work_order_ops(name):
	po = saashq.get_doc("Work Order", name)
...
def make_stock_entry(work_order_id, purpose, qty=None, target_warehouse=None):
	work_order = saashq.get_doc("Work Order", work_order_id)
	...
	if work_order.bom_no:
		stock_entry.inspection_required = saashq.db.get_value("BOM", work_order.bom_no, "inspection_required")
...
def get_default_warehouse():
	doc = saashq.get_cached_doc("Manufacturing Settings")
...
def stop_unstop(work_order, status):
	...
	pro_order = saashq.get_doc("Work Order", work_order)

	if pro_order.status == "Closed":
		saashq.throw(_("Closed Work Order can not be stopped or Re-opened"))

	pro_order.update_status(status)
	pro_order.update_planned_qty()
	saashq.msgprint(_("Work Order has been {0}").format(status))
...
def split_qty_based_on_batch_size(wo_doc, row, qty):
	if not cint(saashq.db.get_value("Operation", row.operation, "create_job_card_based_on_batch_size")):
...
def create_job_card(work_order, row, enable_capacity_planning=False, auto_create=False):
	...
	if auto_create:
		...
		doc.insert()
		saashq.msgprint(_("Job card {0} created").format(get_link_to_form("Job Card", doc.name)), alert=True)
...
def create_pick_list(source_name, target_doc=None, for_qty=None):
	...
	max_finished_goods_qty = saashq.db.get_value("Work Order", source_name, "qty")
	...
	def update_item_quantity(source, target, source_parent):
		...
		if qty:
			...
			target.uom = saashq.get_value("Item", source.item_code, "stock_uom")
...
def get_reserved_qty_for_production(
	...
	wo = saashq.qb.DocType("Work Order")
	wo_item = saashq.qb.DocType("Work Order Item")
...
def make_stock_return_entry(work_order):
	from erpnexus.stock.doctype.stock_entry.stock_entry import get_available_materials

	non_consumed_items = get_available_materials(work_order)
	if not non_consumed_items:
		return

	wo_doc = saashq.get_cached_doc("Work Order", work_order)
