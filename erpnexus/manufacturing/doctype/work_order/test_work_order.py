...
from saashq.utils import add_days, add_months, add_to_date, cint, flt, now, today

from erpnexus.manufacturing.doctype.job_card.job_card import JobCardCancelError
from erpnexus.manufacturing.doctype.production_plan.test_production_plan import make_bom
from erpnexus.manufacturing.doctype.work_order.work_order import (
	CapacityError,
	ItemHasVariantError,
	OverProductionError,
...
from erpnexus.stock.doctype.item.test_item import create_item, make_item
...
from erpnexus.stock.doctype.stock_entry import test_stock_entry
from erpnexus.stock.doctype.warehouse.test_warehouse import create_warehouse
from erpnexus.stock.utils import get_bin
...
class TestWorkOrder(IntegrationTestCase):
	...
	def check_planned_qty(self):
		...
		# from stores to wip
		s = saashq.get_doc(make_stock_entry(wo_order.name, "Material Transfer for Manufacture", 4))
		...
		# from wip to fg
		s = saashq.get_doc(make_stock_entry(wo_order.name, "Manufacture", 4))
		s.insert()
		s.submit()

		self.assertEqual(saashq.db.get_value("Work Order", wo_order.name, "produced_qty"), 4)

		planned2 = saashq.db.get_value(
			"Bin", {"item_code": "_Test FG Item", "warehouse": "_Test Warehouse 1 - _TC"}, "planned_qty"
		)
	...
	def test_over_production(self):
		...
		s = saashq.get_doc(make_stock_entry(wo_doc.name, "Manufacture", 7))
		s.insert()
	...
	def test_reserved_qty_for_partial_completion(self):
		...
		s = saashq.get_doc(make_stock_entry(wo_order.name, "Manufacture", 1))
		s.submit()
	...
	def test_production_item(self):
		...
		saashq.db.set_value("Item", "_Test FG Item", "end_of_life", "2000-1-1")

		self.assertRaises(saashq.ValidationError, wo_order.save)

		saashq.db.set_value("Item", "_Test FG Item", "end_of_life", None)
		saashq.db.set_value("Item", "_Test FG Item", "disabled", 1)

		self.assertRaises(saashq.ValidationError, wo_order.save)

		saashq.db.set_value("Item", "_Test FG Item", "disabled", 0)
	...
	def test_reserved_qty_for_production_on_stock_entry(self):
		...
		s = saashq.get_doc(make_stock_entry(self.wo_order.name, "Material Transfer for Manufacture", 2))
		...
		s = saashq.get_doc(make_stock_entry(self.wo_order.name, "Manufacture", 2))
	...
	def test_backflush_qty_for_overpduction_manufacture(self):
		...
		s = saashq.get_doc(make_stock_entry(wo_order.name, "Material Transfer for Manufacture", 60))
		s.submit()
		cancel_stock_entry.append(s.name)

		s = saashq.get_doc(make_stock_entry(wo_order.name, "Manufacture", 60))
		s.submit()
		cancel_stock_entry.append(s.name)
		...
		s1 = saashq.get_doc(make_stock_entry(wo_order.name, "Manufacture", 50))
		s1.submit()
		...
		for ste in cancel_stock_entry:
			doc = saashq.get_doc("Stock Entry", ste)
			doc.cancel()
	...
	def test_reserved_qty_for_stopped_production(self):
		...
		s = saashq.get_doc(make_stock_entry(self.wo_order.name, "Material Transfer for Manufacture", 1))
	...
	def test_scrap_material_qty(self):
		...
		s = saashq.get_doc(make_stock_entry(wo_order.name, "Material Transfer for Manufacture", 2))
		for d in s.get("items"):
			d.s_warehouse = "Stores - _TC"
		s.insert()
		s.submit()

		s = saashq.get_doc(make_stock_entry(wo_order.name, "Manufacture", 2))
		s.insert()
		s.submit()

		wo_order_details = saashq.db.get_value(
	...
	def test_allow_overproduction(self):
		...
		s = saashq.get_doc(make_stock_entry(wo_order.name, "Material Transfer for Manufacture", 3))
		s.insert()
		self.assertRaises(StockOverProductionError, s.submit)
	...
	def test_work_order_with_non_stock_item(self):
		...
		if not saashq.db.get_value("Item Price", {"item_code": "_Test FG Non Stock Item"}):
			saashq.get_doc(
				{
					"doctype": "Item Price",
		...
		if not saashq.db.get_value("BOM", {"item": fg_item, "docstatus": 1}):
			bom = make_bom(
				item=fg_item,
				rate=1000,
		...
		se = saashq.get_doc(make_stock_entry(wo.name, "Material Transfer for Manufacture", 1))
		se.insert()
		se.submit()

		ste = saashq.get_doc(make_stock_entry(wo.name, "Manufacture", 1))
		ste.insert()
	...
	def test_job_card(self):
		stock_entries = []
		bom = saashq.get_doc("BOM", {"docstatus": 1, "with_operations": 1, "company": "_Test Company"})
		...
		ste = saashq.get_doc(make_stock_entry(work_order.name, "Material Transfer for Manufacture", 1))
		ste.submit()
		...
		ste1 = saashq.get_doc(make_stock_entry(work_order.name, "Manufacture", 1))
		ste1.submit()
	...
	def test_capcity_planning(self):
		...
		if data:
			...
			saashq.db.set_single_value("Manufacturing Settings", {"capacity_planning_for_days": 30})
	...
	def test_work_order_with_non_transfer_item(self):
		saashq.db.set_single_value("Manufacturing Settings", "backflush_raw_materials_based_on", "BOM")
		...
		if not saashq.db.get_value("BOM", {"item": fg_item}):
			make_bom(item=fg_item, raw_materials=["_Test FG Item", "_Test FG Item 1"])

		wo = make_wo_order_test_record(production_item=fg_item)
		ste = saashq.get_doc(make_stock_entry(wo.name, "Material Transfer for Manufacture", 1))
		ste.insert()
		ste.submit()
		self.assertEqual(len(ste.items), 1)
		ste1 = saashq.get_doc(make_stock_entry(wo.name, "Manufacture", 1))
	...
	def test_operation_time_with_batch_size(self):
		...
		bom_name = saashq.db.get_value("BOM", {"item": fg_item, "is_active": 1, "with_operations": 1}, "name")
	...
	def test_batch_size_for_fg_item(self):
		...
		saashq.db.set_single_value("Manufacturing Settings", "make_serial_no_batch_from_work_order", 0)
		...
		saashq.db.set_single_value("Manufacturing Settings", "make_serial_no_batch_from_work_order", 1)
		ste1 = saashq.get_doc(make_stock_entry(work_order.name, "Manufacture", 1))
		for row in ste1.get("items"):
			if row.is_finished_item:
				self.assertEqual(row.item_code, fg_item)
		...
		batches = saashq.get_all("Batch", filters={"reference_name": work_order.name})
		self.assertEqual(len(batches), 3)
		batches = [batch.name for batch in batches]

		ste1 = saashq.get_doc(make_stock_entry(work_order.name, "Manufacture", 10))
		for row in ste1.get("items"):
			if row.is_finished_item:
				self.assertEqual(row.item_code, fg_item)
				self.assertEqual(row.qty, 10)

				bundle_id = saashq.get_doc("Serial and Batch Bundle", row.serial_and_batch_bundle)
		...
		remaining_batches = []
		ste1 = saashq.get_doc(make_stock_entry(work_order.name, "Manufacture", 20))
		for row in ste1.get("items"):
			if row.is_finished_item:
				self.assertEqual(row.item_code, fg_item)
	...
	def test_partial_material_consumption(self):
		saashq.db.set_single_value("Manufacturing Settings", "material_consumption", 1)
		...
		ste1 = saashq.get_doc(make_stock_entry(wo_order.name, "Manufacture", 2))
		ste1.submit()
		ste_cancel_list.append(ste1)

		ste3 = saashq.get_doc(make_stock_entry(wo_order.name, "Material Consumption for Manufacture", 2))
		...
		saashq.db.set_single_value("Manufacturing Settings", "material_consumption", 0)
	...
	def test_extra_material_transfer(self):
		...
		ste3 = saashq.get_doc(make_stock_entry(wo_order.name, "Manufacture", 2))
		...
		ste2 = saashq.get_doc(make_stock_entry(wo_order.name, "Manufacture", 2))
	...
	def test_make_stock_entry_for_customer_provided_item(self):
		...
		if not saashq.db.exists("BOM", {"item": finished_item}):
		...
		ste = saashq.get_doc(make_stock_entry(wo.name, purpose="Material Transfer for Manufacture"))
		ste.insert()
	...
	def test_valuation_rate_missing_on_make_stock_entry(self):
		...
		if not saashq.db.get_value("BOM", {"item": item_name}):
		...
		stock_entry = saashq.get_doc(make_stock_entry(wo.name, "Material Transfer for Manufacture"))
		self.assertRaises(saashq.ValidationError, stock_entry.save)

	def test_wo_completion_with_pl_bom(self):
		from erpnexus.manufacturing.doctype.bom.test_bom import (
		...
		if not saashq.db.exists("BOM", bom_no):
		...
		se = saashq.get_doc(make_stock_entry(wo.name, "Material Transfer for Manufacture", qty))
		se.get("items")[0].s_warehouse = "Stores - _TC"
		se.insert()
		se.submit()

		se = saashq.get_doc(make_stock_entry(wo.name, "Manufacture", qty))
		se.insert()
		se.submit()
	...
	def test_job_card_scrap_item(self):
		...
		for job_card in job_cards:
			if job_card.docstatus == 1:
				saashq.get_doc("Job Card", job_card.name).cancel()

			saashq.delete_doc("Job Card Time Log", job_card.name)
		...
		if not saashq.db.get_value("BOM", {"item": item}):
			bom = make_bom(
		...
		job_card = saashq.db.get_value("Job Card", {"work_order": wo_order.name}, "name")
		update_job_card(job_card)

		stock_entry = saashq.get_doc(make_stock_entry(wo_order.name, "Manufacture", 10))
		...
		job_card = saashq.db.get_value("Job Card", {"work_order": wo_order.name, "docstatus": 0}, "name")
	...
	def test_close_work_order(self):
		...
		job_cards = saashq.db.get_value("Job Card", {"work_order": wo_order.name}, "name")

		if len(job_cards) == len(bom.operations):
			for jc in job_cards:
				job_card_doc = saashq.get_doc("Job Card", jc)
				job_card_doc.append(
					"time_logs",
	...
	def test_partial_manufacture_entries(self):
		...
		sm = saashq.get_doc(make_stock_entry(wo_order.name, "Material Transfer for Manufacture", 100))
		...
		s = saashq.get_doc(make_stock_entry(wo_order.name, "Manufacture", 90))
		...
		s1 = saashq.get_doc(make_stock_entry(wo_order.name, "Manufacture", 5))
		...
		s2 = saashq.get_doc(make_stock_entry(wo_order.name, "Manufacture", 5))
	...
	def test_auto_batch_creation(self):
		from erpnexus.manufacturing.doctype.bom.test_bom import create_nested_bom

		fg_item = saashq.generate_hash(length=20)
		child_item = saashq.generate_hash(length=20)

		bom_tree = {fg_item: {child_item: {}}}

		create_nested_bom(bom_tree, prefix="")

		item = saashq.get_doc("Item", fg_item)
		item.has_batch_no = 1
		...
		try:
			...
			for row in stock_entry.items:
				if row.item_code == fg_item:
					...
					sn_doc = saashq.get_doc("Serial and Batch Bundle", row.serial_and_batch_bundle)
	...
	def get_serial_nos_for_fg(self, work_order):
		serial_nos = []
		for row in saashq.get_all("Serial No", filters={"work_order": work_order}):
	...
	def test_manufacture_entry_mapped_idx_with_exploded_bom(self):
		...
		ste_manu = saashq.get_doc(make_stock_entry(wo_order.name, "Manufacture", 1))
	...
	def test_work_order_multiple_material_transfer(self):
		...
		final_transfer_entry = saashq.get_doc(  # transfer last RM with For Quantity = 0
	...
	def test_backflushed_batch_raw_materials_based_on_transferred(self):
		...
		transferred_ste_doc = saashq.get_doc(
			...
			saashq._dict(
				{
					...
					"batches": saashq._dict({batch_no: 4}),
		...
		manufacture_ste_doc1 = saashq.get_doc(make_stock_entry(wo_doc.name, "Manufacture", 1))
		...
		manufacture_ste_doc2 = saashq.get_doc(make_stock_entry(wo_doc.name, "Manufacture", 2))
	...
	def test_backflushed_serial_no_raw_materials_based_on_transferred(self):
		...
		# Second Manufacture stock entry
		manufacture_ste_doc2 = saashq.get_doc(make_stock_entry(wo_doc.name, "Manufacture", 3))
	...
	def test_backflushed_serial_no_batch_raw_materials_based_on_transferred(self):
		...
		for row in ste_doc.items:
			bundle_doc = saashq.get_doc("Serial and Batch Bundle", row.serial_and_batch_bundle)
		...
		bundle_doc = saashq.get_doc("Serial and Batch Bundle", bundle)
		for d in bundle_doc.entries:
			self.assertTrue(d.serial_no)
			self.assertTrue(d.batch_no)
			batch_no = saashq.get_cached_value("Serial No", d.serial_no, "batch_no")
			self.assertEqual(d.batch_no, batch_no)
	...
	def test_backflushed_batch_raw_materials_based_on_transferred_autosabb(self):
		...
		manufacture_ste_doc = saashq.get_doc(make_stock_entry(wo_doc.name, "Manufacture", 4))
	...
	def test_non_consumed_material_return_against_work_order(self):
		...
		ste_doc = saashq.get_doc(make_stock_entry(wo_doc.name, "Material Transfer for Manufacture", 5))
		for row in ste_doc.items:
			row.qty += 2
			row.transfer_qty += 2
			test_stock_entry.make_stock_entry(
		...
		ste_doc = saashq.get_doc(make_stock_entry(wo_doc.name, "Manufacture", 5))
		for row in ste_doc.items:
	...
	def test_job_card_extra_qty(self):
		...
		job_card = saashq.db.get_value("Job Card", {"work_order": wo_order.name}, "name")
		job_card_doc = saashq.get_doc("Job Card", job_card)
		...
		job_card2 = saashq.copy_doc(job_card_doc)
		job_card2.append(
			"time_logs",
			{
				"from_time": row.from_time,
		...
		self.assertRaises(saashq.ValidationError, job_card2.save)

		saashq.db.set_single_value("Manufacturing Settings", "overproduction_percentage_for_work_order", 100)
	...
	def test_operating_cost_account(self):
		...
		if not saashq.db.exists("Account", operating_cost_account):
			saashq.get_doc(
				{
					"doctype": "Account",
		...
		saashq.db.set_value("Company", company, "default_operating_cost_account", operating_cost_account)

		for item in ["TEST RM OP COST Item 1", "TEST FG OP COST Item"]:
			if not saashq.db.exists("Item", item):
		...
		if not saashq.db.exists("Workstation", workstation):
		...
		job_cards = saashq.get_all("Job Card", filters={"work_order": wo.name})
		for job_card in job_cards:
			job_card_doc = saashq.get_doc("Job Card", job_card.name)
		...
		se_doc = saashq.get_doc(make_stock_entry(wo.name, "Manufacture", 1))
		se_doc.save()
	...
	def test_op_cost_and_scrap_based_on_sub_assemblies(self):
		...
		saashq.db.set_single_value("Manufacturing Settings", "set_op_cost_and_scrape_from_sub_assemblies", 1)
		...
		se_doc = saashq.get_doc(make_stock_entry(wo_order.name, "Manufacture", 10))
		se_doc.save()
		...
		saashq.db.set_single_value("Manufacturing Settings", "set_op_cost_and_scrape_from_sub_assemblies", 0)
	...
	def test_get_rm_cost_from_consumption_entry(self):
		...
		mte = saashq.get_doc(make_stock_entry(wo.name, "Material Transfer for Manufacture", 10))
		mte.items[0].s_warehouse = "Stores - _TC"
		mte.insert().submit()

		mce = saashq.get_doc(make_stock_entry(wo.name, "Material Consumption for Manufacture", 10))
		mce.insert().submit()

		me = saashq.get_doc(make_stock_entry(wo.name, "Manufacture", 10))
		me.insert().submit()
	...
	def test_partial_material_consumption_with_batch(self):
		...
		stock_entry = saashq.get_doc(make_stock_entry(wo_order.name, "Material Transfer for Manufacture", 10))
		stock_entry.submit()
		stock_entry.reload()
	...
	def test_disassemby_order(self):
		...
		stock_entry = saashq.get_doc(make_stock_entry(wo.name, "Manufacture", 1))
		for row in stock_entry.items:
		...
		stock_entry = saashq.get_doc(make_stock_entry(wo.name, "Disassemble", 1))
		stock_entry.save()
	...
	def test_components_qty_for_bom_based_manufacture_entry(self):
		saashq.db.set_single_value("Manufacturing Settings", "backflush_raw_materials_based_on", "BOM")
		saashq.db.set_single_value("Manufacturing Settings", "validate_components_quantities_per_bom", 1)
		...
		transfer_entry = saashq.get_doc(make_stock_entry(wo.name, "Material Transfer for Manufacture", 10))
		transfer_entry.save()
		for row in transfer_entry.items:
			row.qty = 5

		self.assertRaises(saashq.ValidationError, transfer_entry.save)
		...
		manufacture_entry = saashq.get_doc(make_stock_entry(wo.name, "Manufacture", 10))
		...
		self.assertRaises(saashq.ValidationError, manufacture_entry.save)
		manufacture_entry.reload()
		manufacture_entry.submit()

		saashq.db.set_single_value("Manufacturing Settings", "validate_components_quantities_per_bom", 0)


def make_operation(**kwargs):
	kwargs = saashq._dict(kwargs)

	operation_doc = saashq.get_doc(
		{
			"doctype": "Operation",
...
def make_workstation(**kwargs):
	...
	workstation_doc = saashq.get_doc(
		{
			"doctype": "Workstation",
...
def prepare_boms_for_sub_assembly_test():
	if not saashq.db.exists("BOM", {"item": "Test Final SF Item 1"}):
		bom = make_bom(
	...
	if not saashq.db.exists("BOM", {"item": "Test Final SF Item 2"}):
		bom = make_bom(
	...
	if not saashq.db.exists("BOM", {"item": "Test Final FG Item"}):
		bom = make_bom(
...
def prepare_data_for_workstation_type_check():
	from erpnexus.manufacturing.doctype.operation.test_operation import make_operation
	from erpnexus.manufacturing.doctype.workstation.test_workstation import make_workstation
	from erpnexus.manufacturing.doctype.workstation_type.test_workstation_type import (
	...
	if not saashq.db.exists("BOM", {"item": fg_item.name}):
		bom_doc = make_bom(
			item=fg_item.name,
			source_warehouse="Stores - _TC",
		...
		for index, operation in enumerate(operations):
			if not saashq.db.exists("BOM Operation", {"parent": bom_doc.name, "operation": operation}):
				bom_doc.append(
					"operations",
					{
						"operation": operation,
...
def update_job_card(job_card, jc_qty=None, days=None):
	employee = saashq.db.get_value("Employee", {"status": "Active"}, "name")
	job_card_doc = saashq.get_doc("Job Card", job_card)
	job_card_doc.set(
		"scrap_items",
		[
...
def allow_overproduction(fieldname, percentage):
	doc = saashq.get_doc("Manufacturing Settings")
