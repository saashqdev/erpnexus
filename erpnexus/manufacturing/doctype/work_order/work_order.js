...
saashq.ui.form.on("Work Order", {
	setup: function (frm) {
		...
		frm.set_query("bom_no", function () {
			if (frm.doc.production_item) {
				...
				saashq.msgprint(__("Please enter Production Item first"));
			}
		});
		...
		frm.set_query("operation", "required_items", function () {
			return {
				query: "erpnexus.manufacturing.doctype.work_order.work_order.get_bom_operations",
				filters: {
	...
	onload: function (frm) {
		...
		if (frm.doc.__islocal) {
			...
			erpnexus.work_order.set_default_warehouse(frm);
		}
	},
	...
	refresh: function (frm) {
		...
		erpnexus.work_order.set_custom_buttons(frm);
		frm.set_intro("");
	...
	create_stock_return_entry: function (frm) {
		saashq.call({
			method: "erpnexus.manufacturing.doctype.work_order.work_order.make_stock_return_entry",
			args: {
				work_order: frm.doc.name,
			},
			callback: function (r) {
				if (!r.exc) {
					let doc = saashq.model.sync(r.message);
	...
	make_job_card: function (frm) {
		...
		const dialog = saashq.prompt(
			...
			function (data) {
				saashq.call({
					method: "erpnexus.manufacturing.doctype.work_order.work_order.make_job_card",
					freeze: true,
					args: {
						work_order: frm.doc.name,
	...
	make_disassembly_order(frm) {
		erpnexus.work_order
			...
			.then((data) => {
				return saashq.xcall("erpnexus.manufacturing.doctype.work_order.work_order.make_stock_entry", {
			...
			.then((stock_entry) => {
				saashq.model.sync(stock_entry);
				saashq.set_route("Form", stock_entry.doctype, stock_entry.name);
			});
	},
	...
	production_item: function (frm) {
		if (frm.doc.production_item) {
			saashq.call({
				method: "erpnexus.manufacturing.doctype.work_order.work_order.get_item_details",
				args: {
				...
				callback: function (r) {
					if (r.message) {
						...
						erpnexus.in_production_item_onchange = true;
						...
						erpnexus.in_production_item_onchange = false;
					}
				},
			});
		}
	...
	project: function (frm) {
		if (!erpnexus.in_production_item_onchange && !frm.doc.bom_no) {
	...
	set_sales_order: function (frm) {
		if (frm.doc.production_item) {
			saashq.call({
				method: "erpnexus.manufacturing.doctype.work_order.work_order.query_sales_order",
	...
	additional_operating_cost: function (frm) {
		erpnexus.work_order.calculate_cost(frm.doc);
		erpnexus.work_order.calculate_total_cost(frm);
	},
});

saashq.ui.form.on("Work Order Item", {
	source_warehouse: function (frm, cdt, cdn) {
		var row = locals[cdt][cdn];
		if (!row.item_code) {
			saashq.throw(__("Please set the Item Code first"));
	...
	item_code: function (frm, cdt, cdn) {
		...
		if (row.item_code) {
			saashq.call({
				method: "erpnexus.stock.doctype.item.item.get_item_details",
				args: {
					item_code: row.item_code,
					company: frm.doc.company,
				},
...
saashq.ui.form.on("Work Order Operation", {
		...
		if (d.workstation) {
			saashq.call({
				...
				callback: function (data) {
					saashq.model.set_value(d.doctype, d.name, "hour_rate", data.message.hour_rate);
					erpnexus.work_order.calculate_cost(frm.doc);
					erpnexus.work_order.calculate_total_cost(frm);
				},
			});
...
erpnexus.work_order = {
	set_custom_buttons: function (frm) {
		...
		if (doc.status !== "Closed") {
			frm.add_custom_button(
				__("Close"),
				function () {
					saashq.confirm(__("Once the Work Order is Closed. It can't be resumed."), () => {
						erpnexus.work_order.change_work_order_status(frm, "Closed");
					});
				},
				__("Status")
			);
		...
		if (doc.docstatus === 1 && !["Closed", "Completed"].includes(doc.status)) {
			if (doc.status != "Stopped" && doc.status != "Completed") {
				frm.add_custom_button(
					__("Stop"),
					function () {
						erpnexus.work_order.change_work_order_status(frm, "Stopped");
					},
					__("Status")
				);
				...
				frm.add_custom_button(
					__("Re-open"),
					function () {
						erpnexus.work_order.change_work_order_status(frm, "Resumed");
					},
					__("Status")
				);
			}
				...
				if (show_start_btn) {
					...
					if (pending_to_transfer && frm.doc.status != "Stopped") {
						...
						frm.add_custom_button(__("Create Pick List"), function () {
							erpnexus.work_order.create_pick_list(frm);
						});

						var start_btn = frm.add_custom_button(__("Start"), function () {
							erpnexus.work_order.make_se(frm, "Material Transfer for Manufacture");
						});
						...
						if (counter > 0) {
							var consumption_btn = frm.add_custom_button(
								...
								function () {
									...
									erpnexus.work_order.make_consumption_se(
										frm,
					...
					if (flt(doc.material_transferred_for_manufacturing) > 0) {
						if (flt(doc.produced_qty) < flt(doc.material_transferred_for_manufacturing)) {
							...
							let finish_btn = frm.add_custom_button(__("Finish"), function () {
								erpnexus.work_order.make_se(frm, "Manufacture");
							});
	...
	set_default_warehouse: function (frm) {
		if (!(frm.doc.wip_warehouse || frm.doc.fg_warehouse)) {
			saashq.call({
				method: "erpnexus.manufacturing.doctype.work_order.work_order.get_default_warehouse",
				callback: function (r) {
					if (!r.exe) {
		...
		return new Promise((resolve, reject) => {
			saashq.prompt(
				...
				(data) => {
					...
					if (data.qty > max) {
						saashq.msgprint(__("Quantity must not be more than {0}", [max]));
						reject();
					}
	...
	create_pick_list: function (frm, purpose = "Material Transfer for Manufacture") {
		this.show_prompt_for_qty_input(frm, purpose)
			.then((data) => {
				return saashq.xcall("erpnexus.manufacturing.doctype.work_order.work_order.create_pick_list", {
					source_name: frm.doc.name,
					for_qty: data.qty,
				});
			})
			.then((pick_list) => {
				saashq.model.sync(pick_list);
				saashq.set_route("Form", pick_list.doctype, pick_list.name);
			});
	},
	...
	make_consumption_se: function (frm, backflush_raw_materials_based_on) {
		...
		saashq.call({
			method: "erpnexus.manufacturing.doctype.work_order.work_order.make_stock_entry",
			args: {
				work_order_id: frm.doc.name,
	...
	change_work_order_status: function (frm, status) {
		...
		saashq.call({
			method: `erpnexus.manufacturing.doctype.work_order.work_order.${method_name}`,
			freeze: true,
