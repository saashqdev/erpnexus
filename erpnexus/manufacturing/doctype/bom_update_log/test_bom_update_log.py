...
from erpnexus.manufacturing.doctype.bom_update_log.bom_update_log import (
	BOMMissingError,
	resume_bom_cost_update_jobs,
)
from erpnexus.manufacturing.doctype.bom_update_tool.bom_update_tool import (
	enqueue_replace_bom,
	enqueue_update_cost,
)
...
class TestBOMUpdateLog(IntegrationTestCase):
	...
	def test_bom_update_log_validate(self):
		...
		with self.assertRaises(saashq.ValidationError):
			enqueue_replace_bom(boms=saashq._dict(current_bom=self.boms.new_bom, new_bom=self.boms.new_bom))

		with self.assertRaises(saashq.ValidationError):
			enqueue_replace_bom(boms=saashq._dict(current_bom=self.boms.new_bom, new_bom="Dummy BOM"))
	...
	def test_bom_replace_for_root_bom(self):
		...
		old_bom = saashq.db.get_value("BOM", {"item": "B-Item E"}, "name")
		bom_tree = {"B-Item E": {"B-Item G": {}}}

		new_bom = create_nested_bom(bom_tree, prefix="")
		enqueue_replace_bom(boms=saashq._dict(current_bom=old_bom, new_bom=new_bom.name))
...
def remove_bom(item_code):
	boms = saashq.get_all("BOM", fields=["docstatus", "name"], filters={"item": item_code})

	for row in boms:
		if row.docstatus == 1:
			saashq.get_doc("BOM", row.name).cancel()

		saashq.delete_doc("BOM", row.name)
