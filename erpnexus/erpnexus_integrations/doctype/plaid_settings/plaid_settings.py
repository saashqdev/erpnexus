...
from saashq.utils import add_months, formatdate, getdate, sbool, today
from plaid.errors import ItemError

from erpnexus.erpnexus_integrations.doctype.plaid_settings.plaid_connector import PlaidConnector
...
def get_plaid_configuration():
	if saashq.db.get_single_value("Plaid Settings", "enabled"):
...
def add_institution(token, response):
	...
	if not saashq.db.exists("Bank", response["institution"]["name"]):
		try:
			bank = saashq.get_doc(
				{
					"doctype": "Bank",
			...
			saashq.log_error("Plaid Link Error")
	else:
		bank = saashq.get_doc("Bank", response["institution"]["name"])
...
def add_bank_accounts(response, bank, company):
	...
	parent_gl_account = saashq.db.get_all(
	...
	if not parent_gl_account:
		saashq.throw(
			...
			).format(saashq.bold(_("Bank")), company)
		)

	for account in response["accounts"]:
		acc_type = saashq.db.get_value("Bank Account Type", account["type"])
		if not acc_type:
			add_account_type(account["type"])

		acc_subtype = saashq.db.get_value("Bank Account Subtype", account["subtype"])
		if not acc_subtype:
			add_account_subtype(account["subtype"])

		bank_account_name = "{} - {}".format(account["name"], bank["bank_name"])
		existing_bank_account = saashq.db.exists("Bank Account", bank_account_name)

		if not existing_bank_account:
			try:
				gl_account = saashq.get_doc(
					{
						"doctype": "Account",
			...
			try:
				existing_account = saashq.get_doc("Bank Account", existing_bank_account)
...
def add_account_type(account_type):
	try:
		saashq.get_doc({"doctype": "Bank Account Type", "account_type": account_type}).insert()
	except Exception:
		saashq.throw(saashq.get_traceback())


def add_account_subtype(account_subtype):
	try:
		saashq.get_doc({"doctype": "Bank Account Subtype", "account_subtype": account_subtype}).insert()
	except Exception:
		saashq.throw(saashq.get_traceback())
...
def sync_transactions(bank, bank_account):
	...
	last_transaction_date = saashq.db.get_value("Bank Account", bank_account, "last_integration_date")
	...
	try:
		...
		if result:
			last_transaction_date = saashq.db.get_value("Bank Transaction", result.pop(), "date")

			saashq.logger().info(
				f"Plaid added {len(result)} new Bank Transactions from '{bank_account}' between {start_date} and {end_date}"
			)

			saashq.db.set_value("Bank Account", bank_account, "last_integration_date", last_transaction_date)
	except Exception:
		saashq.log_error(saashq.get_traceback(), _("Plaid transactions sync error"))
...
def get_transactions(bank, bank_account=None, start_date=None, end_date=None):
	...
	if bank_account:
		related_bank = saashq.db.get_values(
			"Bank Account", bank_account, ["bank", "integration_id"], as_dict=True
		)
		access_token = saashq.db.get_value("Bank", related_bank[0].bank, "plaid_access_token")
		account_id = related_bank[0].integration_id
	else:
		access_token = saashq.db.get_value("Bank", bank, "plaid_access_token")
		account_id = None
	...
	try:
		...
		if e.code == "ITEM_LOGIN_REQUIRED":
			...
			saashq.log_error(message=msg, title=_("Plaid Link Refresh Required"))
...
def new_bank_transaction(transaction):
	...
	bank_account = saashq.db.get_value("Bank Account", dict(integration_id=transaction["account_id"]))
	...
	if not saashq.db.exists(
		...
		try:
			new_transaction = saashq.get_doc(
				{
			...
			saashq.throw(_("Bank transaction creation error"))

	return result


def automatic_synchronization():
	settings = saashq.get_doc("Plaid Settings", "Plaid Settings")
...
def enqueue_synchronization():
	plaid_accounts = saashq.get_all(
		"Bank Account", filters={"integration_id": ["!=", ""]}, fields=["name", "bank"]
	)

	for plaid_account in plaid_accounts:
		saashq.enqueue(
			"erpnexus.erpnexus_integrations.doctype.plaid_settings.plaid_settings.sync_transactions",
...
def get_company(bank_account_name):
	from saashq.defaults import get_user_default

	company_names = saashq.db.get_all("Company", pluck="name")
	if len(company_names) == 1:
		return company_names[0]
	if saashq.db.exists("Bank Account", bank_account_name):
		return saashq.db.get_value("Bank Account", bank_account_name, "company")
	company_default = get_user_default("Company")
	if company_default:
		return company_default
	saashq.throw(_("Could not detect the Company for updating Bank Accounts"))
...
def update_bank_account_ids(response):
	...
	bank = saashq.get_doc("Bank", institution_name).as_dict()
