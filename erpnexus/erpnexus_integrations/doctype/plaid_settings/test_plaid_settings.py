...
from erpnexus.erpnexus_integrations.doctype.plaid_settings.plaid_settings import (
	add_account_subtype,
	add_account_type,
	add_bank_accounts,
...
class TestPlaidSettings(IntegrationTestCase):
	...
	def tearDown(self):
		for bt in saashq.get_all("Bank Transaction"):
			doc = saashq.get_doc("Bank Transaction", bt.name)
			doc.cancel()
			doc.delete()

		for doctype in ("Bank Account", "Bank Account Type", "Bank Account Subtype"):
			for d in saashq.get_all(doctype):
				saashq.delete_doc(doctype, d.name, force=True)

	def test_plaid_disabled(self):
		saashq.db.set_single_value("Plaid Settings", "enabled", 0)
		self.assertTrue(get_plaid_configuration() == "disabled")

	def test_add_account_type(self):
		add_account_type("brokerage")
		self.assertEqual(saashq.get_doc("Bank Account Type", "brokerage").name, "brokerage")

	def test_add_account_subtype(self):
		add_account_subtype("loan")
		self.assertEqual(saashq.get_doc("Bank Account Subtype", "loan").name, "loan")

	def test_new_transaction(self):
		if not saashq.db.exists("Bank", "Citi"):
			saashq.get_doc({"doctype": "Bank", "bank_name": "Citi"}).insert()
		...
		bank = json.dumps(saashq.get_doc("Bank", "Citi").as_dict(), default=json_handler)
		...
		self.assertTrue(len(saashq.get_all("Bank Transaction")) == 1)
