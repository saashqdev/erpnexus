...
from saashq.utils.data import format_datetime
...
from erpnexus.accounts.doctype.chart_of_accounts_importer.chart_of_accounts_importer import (
	unset_existing_data,
)
...
class TallyMigration(Document):
	...
	def get_collection(self, data_file):
		...
		master_file = saashq.get_doc("File", {"file_url": data_file})
	...
	def set_account_defaults(self):
		self.default_cost_center, self.default_round_off_account = saashq.db.get_value(
			"Company", self.erpnexus_company, ["cost_center", "round_off_account"]
		)
		self.default_warehouse = saashq.db.get_single_value("Stock Settings", "default_warehouse")
				...
				items.append(
					{
						...
						"item_defaults": [{"company": self.erpnexus_company}],
					}
				)
	...
	def _import_master_data(self):
		def create_company_and_coa(coa_file_url):
			coa_file = saashq.get_doc("File", {"file_url": coa_file_url})
			saashq.local.flags.ignore_chart_of_accounts = True

			try:
				company = saashq.get_doc(
					{
						"doctype": "Company",
						"company_name": self.erpnexus_company,
						"default_currency": "INR",
						"enable_perpetual_inventory": 0,
					}
				).insert()
			except saashq.DuplicateEntryError:
				company = saashq.get_doc("Company", self.erpnexus_company)
				unset_existing_data(self.erpnexus_company)

			saashq.local.flags.ignore_chart_of_accounts = False
		...
		def create_parties_and_addresses(parties_file_url, addresses_file_url):
			parties_file = saashq.get_doc("File", {"file_url": parties_file_url})
			for party in json.loads(parties_file.get_content()):
				try:
					party_doc = saashq.get_doc(party)
					party_doc.insert()
				except Exception:
					self.log(party_doc)
			addresses_file = saashq.get_doc("File", {"file_url": addresses_file_url})
			for address in json.loads(addresses_file.get_content()):
				try:
					address_doc = saashq.get_doc(address)
					address_doc.insert(ignore_mandatory=True)
				except Exception:
					self.log(address_doc)

		def create_items_uoms(items_file_url, uoms_file_url):
			uoms_file = saashq.get_doc("File", {"file_url": uoms_file_url})
			for uom in json.loads(uoms_file.get_content()):
				if not saashq.db.exists(uom):
					try:
						uom_doc = saashq.get_doc(uom)
						uom_doc.insert()
					except Exception:
						self.log(uom_doc)

			items_file = saashq.get_doc("File", {"file_url": items_file_url})
			for item in json.loads(items_file.get_content()):
				try:
					item_doc = saashq.get_doc(item)
					item_doc.insert()
				except Exception:
					self.log(item_doc)
	...
	def _process_day_book_data(self):
		...
		def voucher_to_journal_entry(voucher):
			...
			for entry in ledger_entries:
				account = {
					"account": encode_company_abbr(entry.LEDGERNAME.string.strip(), self.erpnexus_company),
		...
		def voucher_to_invoice(voucher):
			if voucher.VOUCHERTYPENAME.string.strip() in ["Sales", "Credit Note"]:
				...
				account_name = encode_company_abbr(self.tally_debtors_account, self.erpnexus_company)
				price_list_field = "selling_price_list"
			elif voucher.VOUCHERTYPENAME.string.strip() in ["Purchase", "Debit Note"]:
				doctype = "Purchase Invoice"
				party_field = "supplier"
				account_field = "credit_to"
				account_name = encode_company_abbr(self.tally_creditors_account, self.erpnexus_company)
		...
		def get_voucher_taxes(voucher):
			...
			for entry in ledger_entries:
				if entry.ISPARTYLEDGER.string.strip() == "No":
					tax_account = encode_company_abbr(entry.LEDGERNAME.string.strip(), self.erpnexus_company)
					taxes.append(
						{
							"charge_type": "Actual",
		...
		def get_party(party):
			if saashq.db.exists({"doctype": "Supplier", "supplier_name": party}):
				return "Supplier", encode_company_abbr(self.tally_creditors_account, self.erpnexus_company)
			elif saashq.db.exists({"doctype": "Customer", "customer_name": party}):
				return "Customer", encode_company_abbr(self.tally_debtors_account, self.erpnexus_company)
	...
	def _import_day_book_data(self):
		...
		try:
			saashq.db.set_value(
				"Account",
				encode_company_abbr(self.tally_creditors_account, self.erpnexus_company),
				"account_type",
				"Payable",
			)
			saashq.db.set_value(
				"Account",
				encode_company_abbr(self.tally_debtors_account, self.erpnexus_company),
				"account_type",
				"Receivable",
			)
			saashq.db.set_value(
				"Company", self.erpnexus_company, "round_off_account", self.default_round_off_account
			)

			vouchers_file = saashq.get_doc("File", {"file_url": self.vouchers})
	...
	def _import_vouchers(self, start, total, is_last=False):
		...
		for index, voucher in enumerate(chunk, start=start):
			try:
				voucher_doc = saashq.get_doc(voucher)
				voucher_doc.insert()
				voucher_doc.submit()
		...
		if is_last:
			...
			self.save()
			saashq.db.set_value("Price List", "Tally Price List", "enabled", 0)
		saashq.flags.in_migrate = False

	@saashq.whitelist()
	def process_master_data(self):
		self.set_status("Processing Master Data")
		saashq.enqueue_doc(self.doctype, self.name, "_process_master_data", queue="long", timeout=3600)

	@saashq.whitelist()
	def import_master_data(self):
		self.set_status("Importing Master Data")
		saashq.enqueue_doc(self.doctype, self.name, "_import_master_data", queue="long", timeout=3600)

	@saashq.whitelist()
	def process_day_book_data(self):
		self.set_status("Processing Day Book Data")
		saashq.enqueue_doc(self.doctype, self.name, "_process_day_book_data", queue="long", timeout=3600)

	@saashq.whitelist()
	def import_day_book_data(self):
		self.set_status("Importing Day Book Data")
		saashq.enqueue_doc(self.doctype, self.name, "_import_day_book_data", queue="long", timeout=3600)

	def log(self, data=None):
		if isinstance(data, saashq.model.document.Document):
			if sys.exc_info()[1].__class__ != saashq.DuplicateEntryError:
				failed_import_log = json.loads(self.failed_import_log)
				doc = data.as_dict()
			...
			return saashq.log_error(title="Tally Migration Error", message=message)
