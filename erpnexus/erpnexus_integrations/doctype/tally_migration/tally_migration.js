...
saashq.provide("erpnexus.tally_migration");

saashq.ui.form.on("Tally Migration", {
	onload: function (frm) {
		let reload_status = true;
		saashq.realtime.on("tally_migration_progress_update", function (data) {
			if (reload_status) {
				saashq.model.with_doc(frm.doc.doctype, frm.doc.name, () => {
					frm.refresh_header();
				});
				reload_status = false;
			}
	...
	refresh: function (frm) {
		...
		erpnexus.tally_migration.failed_import_log = JSON.parse(frm.doc.failed_import_log);
		erpnexus.tally_migration.fixed_errors_log = JSON.parse(frm.doc.fixed_errors_log);
	...
	erpnexus_company: function (frm) {
		saashq.db.exists("Company", frm.doc.erpnexus_company).then((exists) => {
			if (exists) {
				saashq.msgprint(
					__(
						"Company {0} already exists. Continuing will overwrite the Company and Chart of Accounts",
						[frm.doc.erpnexus_company]
					)
				);
			}
		});
	...
	render_html_table(frm, shown_logs, hidden_logs, field) {
		...
		let rows = erpnexus.tally_migration.get_html_rows(shown_logs, field);
	...
	show_error_summary(frm) {
		let summary = erpnexus.tally_migration.failed_import_log.reduce((summary, row) => {
			if (row.doc) {
	...
	show_errored_import_log(frm) {
		let import_log = erpnexus.tally_migration.failed_import_log;
	...
	show_fixed_errors_log(frm) {
		let completed_log = erpnexus.tally_migration.fixed_errors_log;
...
erpnexus.tally_migration.getError = (traceback) => {
...
erpnexus.tally_migration.cleanDoc = (obj) => {
	...
	$.each(temp, function (key, value) {
		if (value === "" || value === null) {
			...
			erpnexus.tally_migration.cleanDoc(value);
		} else if ($.isArray(value)) {
			$.each(value, function (k, v) {
				erpnexus.tally_migration.cleanDoc(v);
			});
		}
	});
	return temp;
...
erpnexus.tally_migration.unresolve = (document) => {
	/* Mark document migration as unresolved ie. move to failed error log */
	let frm = cur_frm;
	let failed_log = erpnexus.tally_migration.failed_import_log;
	let fixed_log = erpnexus.tally_migration.fixed_errors_log;

	let modified_fixed_log = fixed_log.filter((row) => {
		if (!saashq.utils.deep_equal(erpnexus.tally_migration.cleanDoc(row.doc), document)) {
			return row;
		}
	});
...
erpnexus.tally_migration.resolve = (document) => {
...
erpnexus.tally_migration.create_new_doc = (document) => {
	/* Mark as resolved and create new document */
	erpnexus.tally_migration.resolve(document);
	return saashq.call({
		type: "POST",
		method: "erpnexus.erpnexus_integrations.doctype.tally_migration.tally_migration.new_doc",
		args: {
			document,
		},
		freeze: true,
		callback: function (r) {
			if (!r.exc) {
				...
				saashq.get_doc(r.message.doctype, r.message.name).__run_link_triggers = true;
...
erpnexus.tally_migration.get_html_rows = (logs, field) => {
	let index = 0;
	let rows = logs
		.map(({ doc, exc }) => {
			...
			let error_message = erpnexus.tally_migration.getError(traceback);
			index++;
			...
			let show_doc = `
				...
				<div class="collapse margin-top" id="${id}-doc">
					<div class="well">
						<pre style="font-size: smaller;">${JSON.stringify(erpnexus.tally_migration.cleanDoc(doc), null, 1)}</pre>
					</div>
				</div>`;

			let create_button = `
				<button class='btn btn-default btn-xs m-3' type='button' onclick='erpnexus.tally_migration.create_new_doc(${JSON.stringify(
					doc
				)})'>
					${__("Create Document")}
				</button>`;
			...
			let mark_as_unresolved = `
				<button class='btn btn-default btn-xs m-3' type='button' onclick='erpnexus.tally_migration.unresolve(${JSON.stringify(
					doc
				)})'>
