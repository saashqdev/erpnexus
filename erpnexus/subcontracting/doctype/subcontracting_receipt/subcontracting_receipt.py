...
class SubcontractingReceipt(SubcontractingController):
	...
	if TYPE_CHECKING:
		...
		from erpnexus.subcontracting.doctype.subcontracting_receipt_item.subcontracting_receipt_item import (
			SubcontractingReceiptItem,
		)
		from erpnexus.subcontracting.doctype.subcontracting_receipt_supplied_item.subcontracting_receipt_supplied_item import (
	...
	def validate(self):
		...
		if getdate(self.posting_date) > getdate(nowdate()):
			saashq.throw(_("Posting Date cannot be future date"))

		super().validate()

		if self.is_new() and self.get("_action") == "save" and not saashq.flags.in_test:
	...
	def update_job_card(self):
		for row in self.get("items"):
			if row.job_card:
				doc = saashq.get_doc("Job Card", row.job_card)
				doc.set_manufactured_qty()

	def get_manufactured_qty(self, job_card):
		table = saashq.qb.DocType("Subcontracting Receipt Item")
		query = (
	...
	def get_scrap_items(self, recalculate_rate=False):
		...
		for item in list(self.items):
			if item.bom:
				bom = saashq.get_doc("BOM", item.bom)
	...
	def set_available_qty_for_consumption(self):
		...
		sco_supplied_item = saashq.qb.DocType("Subcontracting Order Supplied Item")
		for item in self.get("items"):
			supplied_items = (
				saashq.qb.from_(sco_supplied_item)
				.select(
	...
	def update_status(self, status=None, update_modified=False):
		if not status:
			...
			if self.is_return:
				saashq.get_doc("Subcontracting Receipt", self.return_against).update_status(
	...
	def get_gl_entries(self, warehouse_account=None):
		...
		if not erpnexus.is_perpetual_inventory_enabled(self.company):
			return []
		...
		for item in self.items:
			if flt(item.rate) and flt(item.qty):
				if warehouse_account.get(item.warehouse):
					stock_value_diff = saashq.db.get_value(
						"Stock Ledger Entry",
						{
	...
	def auto_create_purchase_receipt(self):
		if saashq.db.get_single_value("Buying Settings", "auto_create_purchase_receipt"):
...
def make_purchase_receipt(source_name, target_doc=None, save=False, submit=False, notify=False):
	if isinstance(source_name, str):
		source_doc = saashq.get_doc("Subcontracting Receipt", source_name)
	else:
		...
		target.update(
			{
				...
				"currency": saashq.get_cached_value("Company", target.company, "default_currency"),
			}
		)
