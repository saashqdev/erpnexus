...
erpnexus.landed_cost_taxes_and_charges.setup_triggers("Subcontracting Receipt");

saashq.ui.form.on("Subcontracting Receipt", {
	setup: (frm) => {
	...
	refresh: (frm) => {
		...
		if (!frm.doc.is_return && frm.doc.docstatus === 1 && frm.doc.per_returned < 100) {
			frm.add_custom_button(
				...
				() => {
					saashq.model.open_mapped_doc({
						method: "erpnexus.subcontracting.doctype.subcontracting_receipt.subcontracting_receipt.make_subcontract_return",
						frm: frm,
					});
				},
				__("Create")
...
saashq.ui.form.on("Subcontracting Receipt Item", {
	item_code(frm) {
		set_missing_values(frm);
	},
...
saashq.ui.form.on("Subcontracting Receipt Supplied Item", {
	consumed_qty(frm) {
		set_missing_values(frm);
	},
