...
from erpnexus.buying.doctype.purchase_order.purchase_order import is_subcontracting_order_created
...
class SubcontractingOrder(SubcontractingController):
	...
	if TYPE_CHECKING:
		...
		from erpnexus.subcontracting.doctype.subcontracting_order_item.subcontracting_order_item import (
			SubcontractingOrderItem,
		)
		from erpnexus.subcontracting.doctype.subcontracting_order_service_item.subcontracting_order_service_item import (
			SubcontractingOrderServiceItem,
		)
		from erpnexus.subcontracting.doctype.subcontracting_order_supplied_item.subcontracting_order_supplied_item import (
	...
	def onload(self):
		self.set_onload(
			...
			saashq.db.get_single_value("Buying Settings", "over_transfer_allowance"),
		)
	...
	def validate_purchase_order_for_subcontracting(self):
		if self.purchase_order:
			...
			po = saashq.get_doc("Purchase Order", self.purchase_order)

			if not po.is_subcontracted:
				saashq.throw(_("Please select a valid Purchase Order that is configured for Subcontracting."))

			if po.is_old_subcontracting_flow:
				saashq.throw(_("Please select a valid Purchase Order that has Service Items."))
			...
			saashq.throw(_("Please select a Subcontracting Purchase Order."))

	def validate_service_items(self):
		for item in self.service_items:
			if saashq.get_value("Item", item.item_code, "is_stock_item"):
	...
	def update_ordered_qty_for_subcontracting(self, sco_item_rows=None):
		...
		for item in self.get("items"):
			if (
				...
				and saashq.get_cached_value("Item", item.item_code, "is_stock_item")
				and item.warehouse
			):
	...
	def get_ordered_qty(item_code, warehouse):
		table = saashq.qb.DocType("Subcontracting Order")
		child = saashq.qb.DocType("Subcontracting Order Item")
	...
	def populate_items_table(self):
		...
		for si in self.service_items:
			if si.fg_item:
				item = saashq.get_doc("Item", si.fg_item)
				bom = (
					saashq.db.get_value(
						"Subcontracting BOM",
...
def update_subcontracting_order_status(sco, status=None):
	if isinstance(sco, str):
		sco = saashq.get_doc("Subcontracting Order", sco)
