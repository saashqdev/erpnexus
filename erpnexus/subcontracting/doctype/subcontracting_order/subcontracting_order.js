...
erpnexus.landed_cost_taxes_and_charges.setup_triggers("Subcontracting Order");

saashq.ui.form.on("Subcontracting Order", {
	setup: (frm) => {
	...
	set_queries: (frm) => {
		...
		frm.set_query("supplier_address", erpnexus.queries.address_query);

		frm.set_query("billing_address", erpnexus.queries.company_address_query);

		frm.set_query("shipping_address", () => {
			return erpnexus.queries.company_address_query(frm.doc);
		});
	},
	...
	refresh: function (frm) {
		saashq.dynamic_link = { doc: frm.doc, fieldname: "supplier", doctype: "Supplier" };
	...
	update_subcontracting_order_status(frm, status) {
		saashq.call({
			method: "erpnexus.subcontracting.doctype.subcontracting_order.subcontracting_order.update_subcontracting_order_status",
			args: {
				sco: frm.doc.name,
				status: status,
			},
...
erpnexus.buying.SubcontractingOrderController = class SubcontractingOrderController {
	setup() {
	...
	make_subcontracting_receipt() {
		saashq.model.open_mapped_doc({
			method: "erpnexus.subcontracting.doctype.subcontracting_order.subcontracting_order.make_subcontracting_receipt",
			frm: cur_frm,
...
extend_cscript(cur_frm.cscript, new erpnexus.buying.SubcontractingOrderController({ frm: cur_frm }));
