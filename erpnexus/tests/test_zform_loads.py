...
class TestFormLoads(IntegrationTestCase):
	...
	def test_load(self):
		erpnexus_modules = saashq.get_all("Module Def", filters={"app_name": "erpnexus"}, pluck="name")
		doctypes = saashq.get_all(
			"DocType",
			{"istable": 0, "issingle": 0, "is_virtual": 0, "module": ("in", erpnexus_modules)},
			pluck="name",
		)

		for doctype in doctypes:
			last_doc = saashq.db.get_value(doctype, {}, "name", order_by="creation desc")
			if not last_doc:
				continue
	...
	def assertFormLoad(self, doctype, docname):
		...
		saashq.response = saashq._dict({"docs": []})
		saashq.response.docinfo = None
		...
		self.assertTrue(
			saashq.response.docs, msg=f"expected document in reponse, found: {saashq.response.docs}"
		)
		self.assertTrue(
			saashq.response.docinfo, msg=f"expected docinfo in reponse, found: {saashq.response.docinfo}"
		)
