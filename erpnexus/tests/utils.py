...
import saashq
from saashq.core.doctype.report.report import get_report_module_dotted_path
...
def create_test_contact_and_address():
	saashq.db.sql("delete from tabContact")
	saashq.db.sql("delete from `tabContact Email`")
	saashq.db.sql("delete from `tabContact Phone`")
	saashq.db.sql("delete from tabAddress")
	saashq.db.sql("delete from `tabDynamic Link`")
...
def execute_script_report(
	...
	test_filters = []
	report_execute_fn = saashq.get_attr(get_report_module_dotted_path(module, report_name) + ".execute")
	report_filters = saashq._dict(default_filters).copy().update(filters)
...
def if_lending_app_not_installed(function):
	...
	def wrapper(*args, **kwargs):
		if "lending" not in saashq.get_installed_apps():
			return function(*args, **kwargs)
		return
