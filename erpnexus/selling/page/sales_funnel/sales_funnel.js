...
saashq.pages["sales-funnel"].on_page_load = function (wrapper) {
	...
	wrapper.sales_funnel = new erpnexus.SalesFunnel(wrapper);

	saashq.breadcrumbs.add("Selling");
};

erpnexus.SalesFunnel = class SalesFunnel {
	constructor(wrapper) {
		var me = this;
	...
	setup(wrapper) {
		...
		(this.company_field = wrapper.page.add_field({
			...
			change: function () {
				me.company = this.value || saashq.defaults.get_user_default("company");
				me.get_data();
			},
		})),
			(this.elements = {
		...
		this.company = saashq.defaults.get_user_default("company");
		this.options = {
			from_date: saashq.datetime.add_months(saashq.datetime.get_today(), -1),
			to_date: saashq.datetime.get_today(),
			chart: "sales_funnel",
		};
		...
		$.each(this.options, function (k, v) {
			if (["from_date", "to_date"].includes(k)) {
				me.elements[k].val(saashq.datetime.str_to_user(v));
			} else {
				me.elements[k].val(v);
			}
			...
			me.elements[k].on("change", function () {
				if (["from_date", "to_date"].includes(k)) {
					me.options[k] =
						saashq.datetime.user_to_str($(this).val()) != "Invalid date"
							? saashq.datetime.user_to_str($(this).val())
	...
	get_data(btn) {
		var me = this;
		if (!this.company) {
			saashq.throw(__("Please Select a Company."));
		}

		const method_map = {
			sales_funnel: "erpnexus.selling.page.sales_funnel.sales_funnel.get_funnel_data",
			opp_by_utm_source: "erpnexus.selling.page.sales_funnel.sales_funnel.get_opp_by_utm_source",
			opp_by_utm_campaign: "erpnexus.selling.page.sales_funnel.sales_funnel.get_opp_by_utm_campaign",
			opp_by_utm_medium: "erpnexus.selling.page.sales_funnel.sales_funnel.get_opp_by_utm_medium",
			sales_pipeline: "erpnexus.selling.page.sales_funnel.sales_funnel.get_pipeline_data",
		};
		saashq.call({
