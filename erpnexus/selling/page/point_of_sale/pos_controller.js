erpnexus.PointOfSale.Controller = class {
	constructor(wrapper) {
	...
	fetch_opening_entry() {
		return saashq.call("erpnexus.selling.page.point_of_sale.point_of_sale.check_opening_entry", {
	...
	create_opening_voucher() {
		...
		const fetch_pos_payment_methods = () => {
			...
			saashq.db.get_doc("POS Profile", pos_profile).then(({ payments }) => {
		...
		const dialog = new saashq.ui.Dialog({
			...
			primary_action: async function ({ company, pos_profile, balance_details }) {
				if (!balance_details.length) {
					...
					return saashq.utils.play_sound("error");
				}

				// filter balance details for empty rows
				balance_details = balance_details.filter((d) => d.mode_of_payment);

				const method = "erpnexus.selling.page.point_of_sale.point_of_sale.create_opening_voucher";
	...
	async prepare_app_defaults(data) {
		...
		saashq.db.get_value("Stock Settings", undefined, "allow_negative_stock").then(({ message }) => {
			this.allow_negative_stock = flt(message.allow_negative_stock) || false;
		});

		saashq.call({
			method: "erpnexus.selling.page.point_of_sale.point_of_sale.get_pos_profile_data",
	...
	open_form_view() {
		saashq.model.sync(this.frm.doc);
		saashq.set_route("Form", this.frm.doc.doctype, this.frm.doc.name);
	}
	...
	close_pos() {
		...
		let voucher = saashq.model.get_new_doc("POS Closing Entry");
		voucher.pos_profile = this.frm.doc.pos_profile;
		voucher.user = saashq.session.user;
		voucher.company = this.frm.doc.company;
		voucher.pos_opening_entry = this.pos_opening;
		voucher.period_end_date = saashq.datetime.now_datetime();
		voucher.posting_date = saashq.datetime.now_date();
		voucher.posting_time = saashq.datetime.now_time();
		saashq.set_route("Form", "POS Closing Entry", voucher.name);
	}

	init_item_selector() {
		this.item_selector = new erpnexus.PointOfSale.ItemSelector({
	...
	init_item_cart() {
		this.cart = new erpnexus.PointOfSale.ItemCart({
			wrapper: this.$components_wrapper,
			settings: this.settings,
			events: {
				get_frm: () => this.frm,
	...
	init_item_details() {
		this.item_details = new erpnexus.PointOfSale.ItemDetails({
			wrapper: this.$components_wrapper,
			settings: this.settings,
			events: {
				get_frm: () => this.frm,
				...
				form_updated: (item, field, value) => {
					const item_row = saashq.model.get_doc(item.doctype, item.name);
	...
	init_payments() {
		this.payment = new erpnexus.PointOfSale.Payment({
			wrapper: this.$components_wrapper,
			events: {
				get_frm: () => this.frm || {},
	...
	init_recent_order_list() {
		this.recent_order_list = new erpnexus.PointOfSale.PastOrderList({
			wrapper: this.$components_wrapper,
			events: {
				open_invoice_data: (name) => {
					saashq.db.get_doc("POS Invoice", name).then((doc) => {
	...
	init_order_summary() {
		this.order_summary = new erpnexus.PointOfSale.PastOrderSummary({
			wrapper: this.$components_wrapper,
			events: {
				get_frm: () => this.frm,
				...
				delete_order: (name) => {
					saashq.model.delete_doc(this.frm.doc.doctype, name, () => {
	...
	get_new_frm(_frm) {
		const doctype = "POS Invoice";
		const page = $("<div>");
		const frm = _frm || new saashq.ui.form.Form(doctype, page, false);
		const name = saashq.model.make_new_doc_and_get_name(doctype, true);
		frm.refresh(name);
	...
	async make_return_invoice(doc) {
		...
		this.frm.doc.items = [];
		return saashq.call({
			method: "erpnexus.accounts.doctype.pos_invoice.pos_invoice.make_sales_return",
			args: {
			...
			callback: (r) => {
				...
				saashq.get_doc(r.message.doctype, r.message.name).__run_link_triggers = false;
		...
		try {
			...
			if (item_row_exists) {
				...
				if (this.is_current_item_being_edited(item_row) || from_selector) {
					await saashq.model.set_value(item_row.doctype, item_row.name, field, value);
	...
	async check_stock_availability(item_row, qty_needed, warehouse) {
		...
		if (!(available_qty > 0)) {
			if (is_stock_item) {
				saashq.model.clear_doc(item_row.doctype, item_row.name);
				saashq.throw({
	...
	async check_serial_no_availablilty(item_code, warehouse, serial_no) {
		const method = "erpnexus.stock.doctype.serial_no.serial_no.get_pos_reserved_serial_nos";
		const args = { filters: { item_code, warehouse } };
		const res = await saashq.call({ method, args });
	...
	get_available_stock(item_code, warehouse) {
		const me = this;
		return saashq.call({
			method: "erpnexus.accounts.doctype.pos_invoice.pos_invoice.get_stock_availability",
			args: {
				item_code: item_code,
				warehouse: warehouse,
			},
	...
	remove_item_from_cart() {
		...
		return saashq.model
			...
			.then(() => {
				saashq.model.clear_doc(doctype, name);
