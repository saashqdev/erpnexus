erpnexus.PointOfSale.PastOrderSummary = class {
	...
	init_email_print_dialog() {
		const email_dialog = new saashq.ui.Dialog({
			title: "Email Receipt",
			fields: [
		...
		const print_dialog = new saashq.ui.Dialog({
			title: "Print Receipt",
	...
	print_receipt() {
		...
		saashq.utils.print(
			...
			this.doc.language || saashq.boot.lang
		);
	}
	...
	send_email() {
		...
		saashq.call({
			...
			args: {
				...
				send_email: 1,
				print_format,
				sender_full_name: saashq.user.full_name(),
				_lang: doc.language,
			},
			callback: (r) => {
				if (!r.exc) {
					...
					saashq.msgprint(__("There were errors while sending email. Please try again."));
				}
			},
		});
	}
	...
	attach_document_info(doc) {
		saashq.db.get_value("Customer", this.doc.customer, "email_id").then(({ message }) => {
