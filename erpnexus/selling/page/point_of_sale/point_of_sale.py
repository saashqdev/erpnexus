...
from saashq.utils.nestedset import get_root_of

from erpnexus.accounts.doctype.pos_invoice.pos_invoice import get_stock_availability
from erpnexus.accounts.doctype.pos_profile.pos_profile import get_child_nodes, get_item_groups
from erpnexus.stock.utils import scan_barcode
...
def get_items(start, page_length, price_list, item_group, pos_profile, search_term=""):
	warehouse, hide_unavailable_items = saashq.db.get_value(
	...
	if not saashq.db.exists("Item Group", item_group):
		item_group = get_root_of("Item Group")

	condition = get_conditions(search_term)
	condition += get_item_group_condition(pos_profile)

	lft, rgt = saashq.db.get_value("Item Group", item_group, ["lft", "rgt"])
	...
	for item in items_data:
		uoms = saashq.get_doc("Item", item.item_code).get("uoms", [])

		item.actual_qty, _ = get_stock_availability(item.item_code, warehouse)
		item.uom = item.stock_uom

		item_price = saashq.get_all(
			"Item Price",
...
def get_conditions(search_term):
	...
	condition += """item.name like {search_term}
		or item.item_name like {search_term}""".format(search_term=saashq.db.escape("%" + search_term + "%"))
...
def add_search_fields_condition(search_term):
	condition = ""
	search_fields = saashq.get_all("POS Search Fields", fields=["fieldname"])
	if search_fields:
		for field in search_fields:
			condition += " or item.`{}` like {}".format(
				field["fieldname"], saashq.db.escape("%" + search_term + "%")
			)
	return condition
...
def check_opening_entry(user):
	open_vouchers = saashq.db.get_all(
		"POS Opening Entry",
...
def create_opening_voucher(pos_profile, company, balance_details):
	...
	new_pos_opening = saashq.get_doc(
		{
			"doctype": "POS Opening Entry",
			"period_start_date": saashq.utils.get_datetime(),
			"posting_date": saashq.utils.getdate(),
...
def get_past_order_list(search_term, status, limit=20):
	...
	if search_term and status:
		invoices_by_customer = saashq.db.get_all(
			"POS Invoice",
			filters={"customer": ["like", f"%{search_term}%"], "status": status},
			fields=fields,
			page_length=limit,
		)
		invoices_by_name = saashq.db.get_all(
			"POS Invoice",
		...
		invoice_list = saashq.db.get_all(
...
def set_customer_info(fieldname, customer, value=""):
	if fieldname == "loyalty_program":
		saashq.db.set_value("Customer", customer, "loyalty_program", value)

	contact = saashq.get_cached_value("Customer", customer, "customer_primary_contact")
	if not contact:
		contact = saashq.db.sql(
			"""
	...
	if not contact:
		new_contact = saashq.new_doc("Contact")
		new_contact.is_primary_contact = 1
		new_contact.first_name = customer
		new_contact.set("links", [{"link_doctype": "Customer", "link_name": customer}])
		new_contact.save()
		contact = new_contact.name
		saashq.db.set_value("Customer", customer, "customer_primary_contact", contact)

	contact_doc = saashq.get_doc("Contact", contact)
	if fieldname == "email_id":
		contact_doc.set("email_ids", [{"email_id": value, "is_primary": 1}])
		saashq.db.set_value("Customer", customer, "email_id", value)
	elif fieldname == "mobile_no":
		contact_doc.set("phone_nos", [{"phone": value, "is_primary_mobile_no": 1}])
		saashq.db.set_value("Customer", customer, "mobile_no", value)
	contact_doc.save()


@saashq.whitelist()
def get_pos_profile_data(pos_profile):
	pos_profile = saashq.get_doc("POS Profile", pos_profile)
