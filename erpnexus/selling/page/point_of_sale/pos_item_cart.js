erpnexus.PointOfSale.ItemCart = class {
	...
	make_cart_numpad() {
		...
		this.number_pad = new erpnexus.PointOfSale.NumberPad({
	...
	bind_events() {
		...
		saashq.ui.form.on("POS Invoice", "paid_amount", (frm) => {
	...
	attach_shortcuts() {
		for (let row of this.number_pad.keys) {
			for (let btn of row) {
				...
				let shortcut_key = `ctrl+${saashq.scrub(String(btn))[0]}`;
				...
				let shortcut_label = shortcut_key.split("+").map(saashq.utils.to_title_case).join("+");
				shortcut_label = saashq.utils.is_mac() ? shortcut_label.replace("Ctrl", "⌘") : shortcut_label;
				this.$numpad_section
					.find(`.numpad-btn[data-button-value="${fieldname}"]`)
					.attr("title", shortcut_label);

				saashq.ui.keys.on(`${shortcut_key}`, () => {
		...
		const ctrl_label = saashq.utils.is_mac() ? "⌘" : "Ctrl";
	...
	make_customer_selector() {
		...
		this.customer_field = saashq.ui.form.make_control({
			df: {
				label: __("Customer"),
				fieldtype: "Link",
				options: "Customer",
				...
				onchange: function () {
					if (this.value) {
						...
						saashq.dom.freeze();
						saashq.model.set_value(frm.doc.doctype, frm.doc.name, "customer", this.value);
			...
			return new Promise((resolve) => {
				saashq.db
					...
					.then(({ message }) => {
						...
						if (loyalty_program) {
							saashq.call({
								method: "erpnexus.accounts.doctype.loyalty_program.loyalty_program.get_loyalty_program_details_with_points",
	...
	show_discount_control() {
		...
		this.discount_field = saashq.ui.form.make_control({
			df: {
				label: __("Discount"),
				fieldtype: "Data",
	...
	get_customer_image() {
		...
		if (image) {
			...
			return `<div class="customer-image customer-abbr">${saashq.get_abbr(customer)}</div>`;
		}
	}
	...
	update_totals_section(frm) {
		...
		const grand_total = cint(saashq.sys_defaults.disable_rounded_total)
			? frm.doc.grand_total
	...
	render_cart_item(item_data, $item_to_update) {
		...
		function get_description_html() {
			if (item_data.description) {
				...
				item_data.description = saashq.ellipsis(item_data.description, 45);
				...
				return `
					<div class="item-image">
						<img
							...
							src="${image}" alt="${saashq.get_abbr(item_name)}"">
					</div>`;
			} else {
				return `<div class="item-image item-abbr">${saashq.get_abbr(item_name)}</div>`;
			}
		}
	}
	...
	render_customer_fields() {
		...
		const me = this;
		dfs.forEach((df) => {
			this[`customer_${df.fieldname}_field`] = saashq.ui.form.make_control({
		...
		function handle_customer_field_change() {
			...
			if (this.value && current_value != this.value && this.df.fieldname != "loyalty_points") {
				saashq.call({
					method: "erpnexus.selling.page.point_of_sale.point_of_sale.set_customer_info",
					args: {
