...
erpnexus.PointOfSale.Payment = class {
	...
	make_invoice_fields_control() {
		saashq.db.get_doc("POS Settings", undefined).then((doc) => {
			...
			fields.forEach((df) => {
				...
				this[`${df.fieldname}_field`] = saashq.ui.form.make_control({
					df: {
						...df,
						...df_events,
					},
	...
	bind_events() {
		...
		saashq.ui.form.on("POS Invoice", "contact_mobile", (frm) => {
		...
		saashq.ui.form.on("POS Invoice", "coupon_code", (frm) => {
		...
		this.$component.on("click", ".submit-order-btn", () => {
			...
			if (!items.length || (paid_amount == 0 && doc.additional_discount_percentage != 100)) {
				...
				saashq.show_alert({ message, indicator: "orange" });
		...
		saashq.ui.form.on("POS Invoice", "loyalty_amount", (frm) => {
			const formatted_currency = format_currency(frm.doc.loyalty_amount, frm.doc.currency);
			this.$payment_modes.find(`.loyalty-amount-amount`).html(formatted_currency);
		});

		saashq.ui.form.on("Sales Invoice Payment", "amount", (frm, cdt, cdn) => {
	...
	setup_listener_for_payments() {
		saashq.realtime.on("process_phone_payment", (data) => {
			const doc = this.events.get_frm().doc;
			...
			saashq.msgprint({ message: message, title: title });
		});
	}
	...
	toggle_remarks_control() {
		if (this.$remarks.find(".saashq-control").length) {
			this.$remarks.html("+ Add Remark");
		} else {
			this.$remarks.html("");
			this[`remark_control`] = saashq.ui.form.make_control({
				df: {
					label: __("Remark"),
					fieldtype: "Data",
					onchange: function () {},
	...
	render_payment_mode_dom() {
		...
		payments.forEach((p) => {
			const mode = p.mode_of_payment.replace(/ +/g, "_").toLowerCase();
			const me = this;
			this[`${mode}_control`] = saashq.ui.form.make_control({
				df: {
					label: p.mode_of_payment,
					fieldtype: "Currency",
					placeholder: __("Enter {0} amount.", [__(p.mode_of_payment)]),
					onchange: function () {
						const current_value = saashq.model.get_value(p.doctype, p.name, "amount");
	...
	render_loyalty_points_payment_mode() {
		...
		this["loyalty-amount_control"] = saashq.ui.form.make_control({
			df: {
