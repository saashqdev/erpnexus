...
saashq.ui.form.on("Customer", {
	setup: function (frm) {
		...
		frm.make_methods = {
			Quotation: () =>
				saashq.model.open_mapped_doc({
					method: "erpnexus.selling.doctype.customer.customer.make_quotation",
					frm: frm,
				}),
			"Sales Order": () =>
				saashq.model.with_doctype("Sales Order", function () {
					var so = saashq.model.get_new_doc("Sales Order");
					so.customer = frm.doc.name; // Set the current customer as the SO customer
					saashq.set_route("Form", "Sales Order", so.name);
				}),
			Opportunity: () =>
				saashq.model.open_mapped_doc({
					method: "erpnexus.selling.doctype.customer.customer.make_opportunity",
					frm: frm,
				}),
			"Pricing Rule": () => erpnexus.utils.make_pricing_rule(frm.doc.doctype, frm.doc.name),
		};
		...
		frm.set_query("customer_primary_contact", function (doc) {
			return {
				query: "erpnexus.selling.doctype.customer.customer.get_customer_primary_contact",
				filters: {
					customer: doc.name,
				},
			};
	...
	refresh: function (frm) {
		if (saashq.defaults.get_default("cust_master_name") != "Naming Series") {
		...
		if (!frm.doc.__islocal) {
			...
			frm.add_custom_button(
				...
				function () {
					saashq.set_route("query-report", "Accounts Receivable", {
						party_type: "Customer",
						party: frm.doc.name,
					});
				},
			...
			if (cint(saashq.defaults.get_default("enable_common_party_accounting"))) {
				frm.add_custom_button(
			...
			// indicator
			erpnexus.utils.set_party_dashboard_indicators(frm);
		} else {
	...
	validate: function (frm) {
		if (frm.doc.lead_name) saashq.model.clear_doc("Lead", frm.doc.lead_name);
	},
	...
	show_party_link_dialog: function (frm) {
		const dialog = new saashq.ui.Dialog({
			...
			primary_action: function ({ supplier }) {
				saashq.call({
					method: "erpnexus.accounts.doctype.party_link.party_link.create_party_link",
					args: {
