...
from saashq import _, msgprint, qb
from saashq.contacts.address_and_contact import (
	delete_contact_and_address,
	load_address_and_contact,
)
from saashq.model.mapper import get_mapped_doc
from saashq.model.naming import set_name_by_naming_series, set_name_from_naming_options
from saashq.model.utils.rename_doc import update_linked_doctypes
from saashq.utils import cint, cstr, flt, get_formatted_email, today
from saashq.utils.user import get_users_with_role

from erpnexus.accounts.party import get_dashboard_info, validate_party_accounts
from erpnexus.controllers.website_list_for_contact import add_role_for_portal_user
...
class Customer(TransactionBase):
	...
	if TYPE_CHECKING:
		...
		from erpnexus.accounts.doctype.allowed_to_transact_with.allowed_to_transact_with import (
			AllowedToTransactWith,
		)
		from erpnexus.accounts.doctype.party_account.party_account import PartyAccount
		from erpnexus.selling.doctype.customer_credit_limit.customer_credit_limit import (
			CustomerCreditLimit,
		)
		from erpnexus.selling.doctype.sales_team.sales_team import SalesTeam
		from erpnexus.utilities.doctype.portal_user.portal_user import PortalUser
	...
	def autoname(self):
		cust_master_name = saashq.defaults.get_global_default("cust_master_name")
		if cust_master_name == "Customer Name":
			self.name = self.get_customer_name()
		elif cust_master_name == "Naming Series":
			set_name_by_naming_series(self)
		else:
			set_name_from_naming_options(saashq.get_meta(self.doctype).autoname, self)

	def get_customer_name(self):
		if saashq.db.get_value("Customer", self.customer_name) and not saashq.flags.in_import:
			count = saashq.db.sql(
	...
	def validate(self):
		...
		if saashq.db.exists("Customer", self.name):
			customer = saashq.get_doc("Customer", self.name)
			if self.loyalty_program == customer.loyalty_program and not self.loyalty_program_tier:
				self.loyalty_program_tier = customer.loyalty_program_tier

		if self.sales_team:
			if sum(member.allocated_percentage or 0 for member in self.sales_team) != 100:
				saashq.throw(_("Total contribution percentage should be equal to 100"))

	@saashq.whitelist()
	def get_customer_group_details(self):
		doc = saashq.get_doc("Customer Group", self.customer_group)
		self.accounts = []
		self.credit_limits = []
	...
	def check_customer_group_change(self):
		saashq.flags.customer_group_changed = False

		if not self.get("__islocal"):
			if self.customer_group != saashq.db.get_value("Customer", self.name, "customer_group"):
				saashq.flags.customer_group_changed = True

	def validate_default_bank_account(self):
		if self.default_bank_account:
			is_company_account = saashq.db.get_value(
				"Bank Account", self.default_bank_account, "is_company_account"
			)
			if not is_company_account:
				saashq.throw(
					_("{0} is not a company bank account").format(saashq.bold(self.default_bank_account))
				)
	...
	def validate_internal_customer(self):
		...
		internal_customer = saashq.db.get_value(
			"Customer",
			{
		...
		if internal_customer:
			saashq.throw(
				_("Internal Customer for company {0} already exists").format(
					saashq.bold(self.represents_company)
				)
			)
	...
	def update_customer_groups(self):
		...
		if saashq.flags.customer_group_changed:
	...
	def create_primary_contact(self):
		if not self.customer_primary_contact and not self.lead_name:
			...
			saashq.set_value("Contact", self.customer_primary_contact, "is_primary_contact", 1)  # ensure
	...
	def create_primary_address(self):
		...
		if self.flags.is_new_doc and self.get("address_line1"):
			...
			saashq.set_value("Address", self.customer_primary_address, "is_primary_address", 1)  # ensure

	def update_lead_status(self):
		"""If Customer created from Lead, update lead status to "Converted"
		update Customer link in Quotation, Opportunity"""
		if self.lead_name:
			saashq.db.set_value("Lead", self.lead_name, "status", "Converted")

	def link_lead_address_and_contact(self):
		if self.lead_name:
			# assign lead address and contact to customer (if already not set)
			linked_contacts_and_addresses = saashq.get_all(
				"Dynamic Link",
				filters=[
			...
			for row in linked_contacts_and_addresses:
				linked_doc = saashq.get_doc(row.doctype, row.name)
	...
	def copy_communication(self):
		if not self.lead_name or not saashq.db.get_single_value(
			"CRM Settings", "carry_forward_communication_and_comments"
		):
			return

		from erpnexus.crm.utils import copy_comments, link_communications

		copy_comments("Lead", self.lead_name, self)
		link_communications("Lead", self.lead_name, self)

	def validate_name_with_customer_group(self):
		if saashq.db.exists("Customer Group", self.name):
			saashq.throw(
				_(
	...
	def on_trash(self):
		...
		if self.lead_name:
			saashq.db.sql("update `tabLead` set status='Interested' where name=%s", self.lead_name)

	def after_rename(self, olddn, newdn, merge=False):
		if saashq.defaults.get_global_default("cust_master_name") == "Customer Name":
		...
		if len(loyalty_program) == 1:
			...
			saashq.msgprint(
				_("Multiple Loyalty Programs found for Customer {}. Please select manually.").format(
					saashq.bold(self.customer_name)
				)
			)
...
def make_quotation(source_name, target_doc=None):
	...
	price_list, currency = saashq.db.get_value(
...
def get_loyalty_programs(doc):
	...
	lp_details = []
	loyalty_programs = saashq.get_all(
		"Loyalty Program",
...
def get_nested_links(link_doctype, link_name, ignore_permissions=False):
	from saashq.desk.treeview import _get_children
...
def check_credit_limit(customer, company, ignore_outstanding_sales_order=False, extra_amount=0):
	...
	if credit_limit > 0 and flt(customer_outstanding) > credit_limit:
		...
		credit_controller_role = saashq.db.get_single_value("Accounts Settings", "credit_controller")
		if not credit_controller_role or credit_controller_role not in saashq.get_roles():
			...
			saashq.msgprint(
				...
				primary_action={
					"label": "Send Email",
					"server_action": "erpnexus.selling.doctype.customer.customer.send_emails",
...
def send_emails(customer, customer_outstanding, credit_limit, credit_controller_users_list):
	...
	saashq.sendmail(recipients=credit_controller_users_list, subject=subject, message=message)
...
def get_customer_outstanding(customer, company, ignore_outstanding_sales_order=False, cost_center=None):
	...
	cond = ""
	if cost_center:
		lft, rgt = saashq.get_cached_value("Cost Center", cost_center, ["lft", "rgt"])

		cond = f""" and cost_center in (select name from `tabCost Center` where
			lft >= {lft} and rgt <= {rgt})"""

	outstanding_based_on_gle = saashq.db.sql(
		f"""
	...
	if not ignore_outstanding_sales_order:
		outstanding_based_on_so = saashq.db.sql(
			"""
	...
	unmarked_delivery_note_items = saashq.db.sql(
		"""select
	...
	si_amounts = saashq.db.sql(
		...
		GROUP BY dn_detail""".format(
			", ".join(saashq.db.escape(dn_item.name) for dn_item in unmarked_delivery_note_items)
		)
	)
...
def get_credit_limit(customer, company):
	...
	if customer:
		credit_limit = saashq.db.get_value(
			"Customer Credit Limit",
		...
		if not credit_limit:
			customer_group = saashq.get_cached_value("Customer", customer, "customer_group")
	...
	if not credit_limit:
		credit_limit = saashq.get_cached_value("Company", company, "credit_limit")
