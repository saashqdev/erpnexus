...
from erpnexus.exceptions import PartyDisabled, PartyFrozen
from erpnexus.selling.doctype.customer.customer import (
	get_credit_limit,
	get_customer_outstanding,
	parse_full_name,
)
from erpnexus.tests.utils import create_test_contact_and_address
...
class TestCustomer(IntegrationTestCase):
	...
	def test_get_customer_group_details(self):
		doc = saashq.new_doc("Customer Group")
	...
	def test_party_details_tax_category(self):
		...
		saashq.delete_doc_if_exists("Address", "_Test Address With Tax Category-Shipping")

		# Tax Category without Address
		details = get_party_details("_Test Customer With Tax Category")
		self.assertEqual(details.tax_category, "_Test Tax Category 1")

		billing_address = saashq.get_doc(
			dict(
				doctype="Address",
		...
		shipping_address = saashq.get_doc(
			dict(
				doctype="Address",
				address_title="_Test Address With Tax Category",
		...
		settings = saashq.get_single("Accounts Settings")
	...
	def test_rename(self):
		...
		# add comments
		comment = saashq.get_doc("Customer", "_Test Customer 1").add_comment(
			"Comment", "Test Comment for Rename"
		)

		# rename
		saashq.rename_doc("Customer", "_Test Customer 1", new_name)

		# check if customer renamed
		self.assertTrue(saashq.db.exists("Customer", new_name))
		self.assertFalse(saashq.db.exists("Customer", "_Test Customer 1"))
		...
		saashq.rename_doc("Customer", new_name, "_Test Customer 1")

		saashq.db.rollback()

	def test_freezed_customer(self):
		saashq.db.set_value("Customer", "_Test Customer", "is_frozen", 1)
		...
		saashq.db.set_value("Customer", "_Test Customer", "is_frozen", 0)

		so.save()

	def test_delete_customer_contact(self):
		customer = saashq.get_doc(get_customer_dict("_Test Customer for delete")).insert(
		...
		saashq.delete_doc("Customer", customer.name)

	def test_disabled_customer(self):
		saashq.db.set_value("Customer", "_Test Customer", "disabled", 1)
		...
		saashq.db.set_value("Customer", "_Test Customer", "disabled", 0)

		so.save()

	def test_duplicate_customer(self):
		saashq.db.sql("delete from `tabCustomer` where customer_name='_Test Customer 1'")

		if not saashq.db.get_value("Customer", "_Test Customer 1"):
			test_customer_1 = saashq.get_doc(get_customer_dict("_Test Customer 1")).insert(
				ignore_permissions=True
			)
		else:
			test_customer_1 = saashq.get_doc("Customer", "_Test Customer 1")

		duplicate_customer = saashq.get_doc(get_customer_dict("_Test Customer 1")).insert(
			ignore_permissions=True
		)
	...
	def test_customer_credit_limit(self):
		...
		self.assertRaises(saashq.ValidationError, so.submit)

		# Delivery Note
		dn = create_delivery_note(do_not_submit=True)
		self.assertRaises(saashq.ValidationError, dn.submit)
	...
	def test_customer_credit_limit_after_submit(self):
		from erpnexus.controllers.accounts_controller import update_child_qty_rate
		from erpnexus.selling.doctype.sales_order.test_sales_order import make_sales_order
	...
	def test_customer_credit_limit_on_change(self):
		outstanding_amt = self.get_customer_outstanding_amount()
		customer = saashq.get_doc("Customer", "_Test Customer")
		customer.append(
		...
		self.assertRaises(saashq.ValidationError, customer.save)
	...
	def test_customer_payment_terms(self):
		...
		saashq.db.set_value("Customer", "_Test Customer With Template", "payment_terms", "")
...
def create_internal_customer(customer_name=None, represents_company=None, allowed_to_interact_with=None):
	...
	existing_representative = saashq.db.get_value("Customer", {"represents_company": represents_company})
	if existing_representative:
		return existing_representative

	if not saashq.db.exists("Customer", customer_name):
		customer = saashq.get_doc(
			{
				"doctype": "Customer",
				"customer_group": "_Test Customer Group",
				"customer_name": customer_name,
		...
		customer_name = saashq.db.get_value("Customer", customer_name)
