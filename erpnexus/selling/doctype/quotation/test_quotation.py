...
from saashq.utils import add_days, add_months, flt, getdate, nowdate
...
class TestQuotation(IntegrationTestCase):
	...
	def test_make_sales_order_terms_copied(self):
		from erpnexus.selling.doctype.quotation.quotation import make_sales_order

		quotation = saashq.copy_doc(self.globalTestRecords["Quotation"][0])
	...
	def test_gross_profit(self):
		...
		from erpnexus.stock.get_item_details import insert_item_price
		...
		selling_price_list = saashq.get_all("Price List", filters={"selling": 1}, limit=1)[0].name
			...
			saashq._dict(
				{
					...
					"conversion_factor": 1,
					"discount_amount": 0.0,
					"currency": saashq.db.get_value("Price List", selling_price_list, "currency"),
	...
	def test_maintain_rate_in_sales_cycle_is_enforced(self):
		...
		maintain_rate = saashq.db.get_single_value("Selling Settings", "maintain_same_sales_rate")
		...
		self.assertRaises(saashq.ValidationError, sales_order.save)

		saashq.db.set_single_value("Selling Settings", "maintain_same_sales_rate", maintain_rate)
	...
	def test_make_sales_order(self):
		...
		self.assertRaises(saashq.ValidationError, make_sales_order, quotation.name)
		quotation.submit()
	...
	def test_valid_till_before_transaction_date(self):
		...
		self.assertRaises(saashq.ValidationError, quotation.validate)

	def test_so_from_expired_quotation(self):
		from erpnexus.selling.doctype.quotation.quotation import make_sales_order

		saashq.db.set_single_value("Selling Settings", "allow_sales_order_creation_for_expired_quotation", 0)
		...
		saashq.db.set_single_value("Selling Settings", "allow_sales_order_creation_for_expired_quotation", 1)
	...
	def test_create_quotation_with_margin(self):
		...
		quotation = saashq.copy_doc(test_record)
		quotation.transaction_date = nowdate()
		quotation.valid_till = add_months(quotation.transaction_date, 1)
		quotation.insert()
	...
	def test_quotation_expiry(self):
		from erpnexus.selling.doctype.quotation.quotation import set_expired_status
	...
	def test_alternative_items_sales_order_mapping_with_stock_items(self):
		...
		saashq.flags.args = saashq._dict()
		item_list = []
		stock_items = {
			"_Test Simple Item 1": 100,
			"_Test Alt 1": 120,
		...
		saashq.flags.args.selected_items = [quotation.items[2]]
		sales_order = make_sales_order(quotation.name)
	...
	def test_uom_validation(self):
		...
		if not saashq.db.exists("UOM", "lbs"):
			saashq.get_doc({"doctype": "UOM", "uom_name": "lbs", "must_be_whole_number": 1}).insert()
		else:
			saashq.db.set_value("UOM", "lbs", "must_be_whole_number", 1)

		quotation = make_quotation(item_code=item, qty=1, rate=100, do_not_submit=1)
		quotation.items[0].uom = "lbs"
		quotation.items[0].conversion_factor = 2.23
		self.assertRaises(saashq.ValidationError, quotation.save)

	def test_item_tax_template_for_quotation(self):
		from erpnexus.stock.doctype.item.test_item import make_item

		if not saashq.db.exists("Account", {"account_name": "_Test Vat", "company": "_Test Company"}):
			saashq.get_doc(
				{
					"doctype": "Account",
		...
		if not saashq.db.exists("Item Tax Template", "Vat Template - _TC"):
			saashq.get_doc(
				{
...
def enable_calculate_bundle_price(enable=1):
	selling_settings = saashq.get_doc("Selling Settings")
