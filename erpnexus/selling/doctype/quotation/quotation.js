...
erpnexus.accounts.taxes.setup_tax_validations("Sales Taxes and Charges Template");
erpnexus.accounts.taxes.setup_tax_filters("Sales Taxes and Charges");
erpnexus.pre_sales.set_as_lost("Quotation");
erpnexus.sales_common.setup_selling_controller();

saashq.ui.form.on("Quotation", {
	setup: function (frm) {
...
erpnexus.selling.QuotationController = class QuotationController extends erpnexus.selling.SellingController {
	onload(doc, dt, dn) {
	...
	refresh(doc, dt, dn) {
		...
		if (doc.__islocal && !doc.valid_till) {
			if (saashq.boot.sysdefaults.quotation_valid_till) {
				this.frm.set_value(
					"valid_till",
					saashq.datetime.add_days(
						doc.transaction_date,
						saashq.boot.sysdefaults.quotation_valid_till
					)
				);
			} else {
				this.frm.set_value("valid_till", saashq.datetime.add_months(doc.transaction_date, 1));
			}
		}

		if (doc.docstatus == 1 && !["Lost", "Ordered"].includes(doc.status)) {
			if (
				saashq.boot.sysdefaults.allow_sales_order_creation_for_expired_quotation ||
				!doc.valid_till ||
				saashq.datetime.get_diff(doc.valid_till, saashq.datetime.get_today()) >= 0
			) {
	...
	get_lead_details() {
		...
		saashq.call({
			method: "erpnexus.crm.doctype.lead.lead.get_lead_details",
			args: {
...
cur_frm.script_manager.make(erpnexus.selling.QuotationController);
...
saashq.ui.form.on("Quotation Item", "stock_balance", function (frm, cdt, cdn) {
	var d = saashq.model.get_doc(cdt, cdn);
	saashq.route_options = { item_code: d.item_code };
