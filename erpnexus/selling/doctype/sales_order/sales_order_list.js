saashq.listview_settings["Sales Order"] = {
	add_fields: [
		"base_grand_total",
		"customer_name",
		"currency",
	...
	get_indicator: function (doc) {
		if (doc.status === "Closed") {
			...
			if (saashq.datetime.get_diff(doc.delivery_date) < 0) {
	...
	onload: function (listview) {
		var method = "erpnexus.selling.doctype.sales_order.sales_order.close_or_unclose_sales_orders";
		...
		listview.page.add_action_item(__("Sales Invoice"), () => {
			erpnexus.bulk_transaction_processing.create(listview, "Sales Order", "Sales Invoice");
		});

		listview.page.add_action_item(__("Delivery Note"), () => {
			saashq.call({
				method: "erpnexus.selling.doctype.sales_order.sales_order.is_enable_cutoff_date_on_bulk_delivery_note_creation",
				callback: (r) => {
					if (r.message) {
							...
							fields: [
								{
									...
									default: saashq.datetime.add_days(saashq.datetime.nowdate(), 1),
								},
							],
						});
						dialog.set_primary_action(__("Select"), function (values) {
							var until_delivery_date = values.delivery_date;
							erpnexus.bulk_transaction_processing.create(
								listview,
								"Sales Order",
								"Delivery Note",
								{
						...
						erpnexus.bulk_transaction_processing.create(listview, "Sales Order", "Delivery Note");
					}
				},
			});
		});
		...
		listview.page.add_action_item(__("Advance Payment"), () => {
			erpnexus.bulk_transaction_processing.create(listview, "Sales Order", "Payment Entry");
		});
	},
};
