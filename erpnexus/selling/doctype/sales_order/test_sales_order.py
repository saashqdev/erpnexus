...
from saashq.utils import add_days, flt, getdate, nowdate, today

from erpnexus.accounts.test.accounts_mixin import AccountsTestMixin
from erpnexus.controllers.accounts_controller import InvalidQtyError, update_child_qty_rate
from erpnexus.maintenance.doctype.maintenance_schedule.test_maintenance_schedule import (
	make_maintenance_schedule,
)
from erpnexus.maintenance.doctype.maintenance_visit.test_maintenance_visit import (
	make_maintenance_visit,
)
from erpnexus.manufacturing.doctype.blanket_order.test_blanket_order import make_blanket_order
from erpnexus.selling.doctype.product_bundle.test_product_bundle import make_product_bundle
from erpnexus.selling.doctype.sales_order.sales_order import (
	WarehouseRequired,
	create_pick_list,
	make_delivery_note,
...
from erpnexus.stock.get_item_details import get_bin_details
...
class TestSalesOrder(AccountsTestMixin, IntegrationTestCase):
	...
	def setUpClass(cls):
		...
		cls.unlink_setting = int(
			saashq.db.get_single_value("Accounts Settings", "unlink_advance_payment_on_cancelation_of_order")
		)
	...
	def test_sales_order_qty(self):
		...
		self.assertRaises(saashq.NonNegativeError, so.save)
	...
	def test_make_material_request(self):
		...
		self.assertRaises(saashq.ValidationError, make_material_request, so.name)
	...
	def test_make_delivery_note(self):
		...
		self.assertRaises(saashq.ValidationError, make_delivery_note, so.name)
	...
	def test_make_sales_invoice(self):
		...
		self.assertRaises(saashq.ValidationError, make_sales_invoice, so.name)
	...
	def test_reserved_qty_for_over_delivery(self):
		...
		saashq.db.set_value("Item", "_Test Item", "over_delivery_receipt_allowance", 50)
	...
	def test_reserved_qty_for_over_delivery_via_sales_invoice(self):
		...
		# set over-delivery allowance
		saashq.db.set_value("Item", "_Test Item", "over_delivery_receipt_allowance", 50)
		saashq.db.set_value("Item", "_Test Item", "over_billing_allowance", 20)
	...
	def test_sales_order_on_hold(self):
		...
		si = make_sales_invoice(so.name)
		self.assertRaises(saashq.ValidationError, create_dn_against_so, so.name)
	...
	def test_reserved_qty_for_over_delivery_with_packing_list(self):
		...
		# set over-delivery allowance
		saashq.db.set_value("Item", "_Test Product Bundle Item", "over_delivery_receipt_allowance", 50)
	...
	def test_update_child_removing_item(self):
		...
		self.assertRaises(saashq.ValidationError, update_child_qty_rate, "Sales Order", trans_item, so.name)
	...
	def test_update_child_with_precision(self):
		...
		precision = get_field_precision(saashq.get_meta("Sales Order Item").get_field("rate"))
	...
	def test_update_child_qty_rate_with_workflow(self):
		from saashq.model.workflow import apply_workflow
		...
		test_user = saashq.get_doc("User", user)
		...
		test_user2 = saashq.get_doc("User", user2)
		test_user2.add_roles("Sales User", "Test Approver")
		saashq.set_user(user2)
	...
	def test_material_request_for_product_bundle(self):
		...
		if not saashq.db.exists("Item", "_Test Product Bundle Item New 1"):
	...
	def test_update_child_product_bundle(self):
		# test Update Items with product bundle
		if not saashq.db.exists("Item", "_Product Bundle Item"):
	...
	def test_update_child_with_tax_template(self):
		...
		"""
		if not saashq.db.exists("Item", "Test Item with Tax"):
			make_item(
				"Test Item with Tax",
				{
					"is_stock_item": 1,
		...
		if not saashq.db.exists("Item Tax Template", {"title": "Test Update Items Template"}):
			saashq.get_doc(
				{
		...
		new_item_with_tax = saashq.get_doc("Item", "Test Item with Tax")
		...
		if not saashq.db.exists("Item Tax", {"parent": item, "item_tax_template": tax_template}):
			item_doc = saashq.get_doc("Item", item)
		...
		old_stock_settings_value = saashq.db.get_single_value("Stock Settings", "default_warehouse")
		saashq.db.set_single_value("Stock Settings", "default_warehouse", "_Test Warehouse - _TC")
		...
		saashq.get_doc("Item Tax Template", "Test Update Items Template - _TC").delete()
		saashq.db.set_single_value("Stock Settings", "default_warehouse", old_stock_settings_value)

	def test_warehouse_user(self):
		test_user = create_user("test_so_warehouse_user@example.com", "Sales User", "Stock User")

		test_user_2 = saashq.get_doc("User", "test2@example.com")
		test_user_2.add_roles("Sales User", "Stock User")
		test_user_2.remove_roles("Sales Manager")

		saashq.permissions.add_user_permission("Warehouse", "_Test Warehouse 1 - _TC", test_user.name)
		saashq.permissions.add_user_permission("Warehouse", "_Test Warehouse 2 - _TC1", test_user_2.name)
		saashq.permissions.add_user_permission("Company", "_Test Company 1", test_user_2.name)
		...
		self.assertRaises(saashq.PermissionError, so.insert)

		saashq.set_user(test_user_2.name)
		so.insert()

		saashq.set_user("Administrator")
		saashq.permissions.remove_user_permission("Warehouse", "_Test Warehouse 1 - _TC", test_user.name)
		saashq.permissions.remove_user_permission("Warehouse", "_Test Warehouse 2 - _TC1", test_user_2.name)
		saashq.permissions.remove_user_permission("Company", "_Test Company 1", test_user_2.name)
	...
	def test_block_delivery_note_against_cancelled_sales_order(self):
		...
		self.assertRaises(saashq.CancelledLinkError, dn.submit)
	...
	def test_auto_insert_price(self):
		...
		saashq.db.set_single_value("Stock Settings", "auto_insert_price_list_rate_if_missing", 1)

		item_price = saashq.db.get_value(
			"Item Price", {"price_list": "_Test Price List", "item_code": "_Test Item for Auto Price List"}
		)
		if item_price:
			saashq.delete_doc("Item Price", item_price)
		...
		saashq.db.set_single_value("Stock Settings", "auto_insert_price_list_rate_if_missing", 0)
	...
	def test_drop_shipping(self):
		from erpnexus.buying.doctype.purchase_order.purchase_order import update_status
		from erpnexus.selling.doctype.sales_order.sales_order import (
			make_purchase_order_for_default_supplier,
		)
		from erpnexus.selling.doctype.sales_order.sales_order import update_status as so_update_status
		...
		if saashq.db.get_value("Item", "_Test Regular Item", "is_stock_item") == 1:
		...
		self.assertEqual(flt(saashq.db.get_value("Sales Order", so.name, "per_delivered"), 2), 100.00)
		po.load_from_db()
	...
	def test_drop_shipping_full_for_default_suppliers(self):
		...
		if not saashq.db.exists("Item", "_Test Item for Drop Shipping 1"):
			make_item("_Test Item for Drop Shipping 1", {"is_stock_item": 1, "delivered_by_supplier": 1})

		if not saashq.db.exists("Item", "_Test Item for Drop Shipping 2"):
	...
	def test_product_bundles_in_so_are_replaced_with_bundle_items_in_po(self):
		...
		"""
		from erpnexus.selling.doctype.sales_order.sales_order import make_purchase_order
	...
	def test_make_work_order(self):
		from erpnexus.selling.doctype.sales_order.sales_order import get_work_order_items
	...
	def test_advance_payment_entry_unlink_against_sales_order(self):
		...
		saashq.db.set_single_value("Accounts Settings", "unlink_advance_payment_on_cancelation_of_order", 0)
		...
		so_doc = saashq.get_doc("Sales Order", so.name)

		self.assertRaises(saashq.LinkExistsError, so_doc.cancel)
	...
	def test_cancel_sales_order_after_cancel_payment_entry(self):
		...
		po_doc = saashq.get_doc("Payment Entry", pe.name)
		po_doc.cancel()
	...
	def test_work_order_pop_up_from_sales_order(self):
		...
		from erpnexus.controllers.item_variant import create_variant
	...
	def test_request_for_raw_materials(self):
		...
		mr_doc = saashq.get_doc("Material Request", mr.get("name"))
	...
	def test_so_optional_blanket_order(self):
		...
		so_doc = saashq.get_doc("Sales Order", so.get("name"))
	...
	def test_so_cancellation_when_si_drafted(self):
		...
		self.assertRaises(saashq.ValidationError, so.cancel)
	...
	def test_so_cancellation_after_si_submission(self):
		...
		so.load_from_db()
		self.assertRaises(saashq.LinkExistsError, so.cancel)
	...
	def test_so_cancellation_after_work_order_submission(self):
		...
		Expected result: Sales Order should not get cancelled
		"""
		from erpnexus.manufacturing.doctype.work_order.test_work_order import make_wo_order_test_record
	...
	def test_so_back_updated_from_wo_via_mr(self):
		...
		from erpnexus.stock.doctype.material_request.material_request import raise_work_orders
		...
		wo = saashq.get_doc("Work Order", wo_name)
		...
		se = saashq.get_doc(make_se_from_wo(wo.name, "Manufacture", 2))
		se.submit()  # Finish WO
	...
	def test_sales_order_partial_advance_payment(self):
		...
		saashq.get_doc(customer).insert()
	...
	def test_packed_items_for_partial_sales_order(self):
		...
		for product_bundle in [
			...
			if not saashq.db.exists("Item", product_bundle):
	...
	@patch(
		...
		"erpnexus.accounts.doctype.payment_request.payment_request.PaymentRequest.get_payment_url",
		return_value=None,
	)
	def test_sales_order_advance_payment_status(self, mocked_get_payment_url):
		from erpnexus.accounts.doctype.payment_entry.test_payment_entry import get_payment_entry
		from erpnexus.accounts.doctype.payment_request.payment_request import make_payment_request
		...
		so.submit()
		self.assertEqual(saashq.db.get_value(so.doctype, so.name, "advance_payment_status"), "Not Requested")

		pr = make_payment_request(
			dt=so.doctype, dn=so.name, order_type="Shopping Cart", submit_doc=True, return_doc=True
		)
		self.assertEqual(saashq.db.get_value(so.doctype, so.name, "advance_payment_status"), "Requested")
		...
		self.assertEqual(saashq.db.get_value(so.doctype, so.name, "advance_payment_status"), "Fully Paid")
		...
		pr.cancel()
		self.assertEqual(
			saashq.db.get_value(so.doctype, so.name, "advance_payment_status"), "Not Requested"
		...
		pe.cancel()
		self.assertEqual(
			saashq.db.get_value(so.doctype, so.name, "advance_payment_status"), "Requested"
		)  # here: reset
	...
	def test_pick_list_for_batch(self):
		from erpnexus.stock.doctype.pick_list.pick_list import create_delivery_note
	...
	def test_auto_update_price_list(self):
		...
		item_price = saashq.db.get_value("Item Price", {"item_code": item.name}, "price_list_rate")
		...
		saashq.db.set_single_value("Stock Settings", "update_existing_price_list_rate", 1)
		so = make_sales_order(
		...
		saashq.db.set_single_value("Stock Settings", "update_existing_price_list_rate", 0)
		saashq.db.set_single_value("Stock Settings", "auto_insert_price_list_rate_if_missing", 0)
	...
	def test_credit_limit_on_so_reopning(self):
		...
		self.assertRaises(saashq.ValidationError, so1.update_status, "Draft")
...
def automatically_fetch_payment_terms(enable=1):
	accounts_settings = saashq.get_doc("Accounts Settings")
...
def get_reserved_qty(item_code="_Test Item", warehouse="_Test Warehouse - _TC"):
	return flt(saashq.db.get_value("Bin", {"item_code": item_code, "warehouse": warehouse}, "reserved_qty"))
...
def make_sales_order_workflow():
	if saashq.db.exists("Workflow", "SO Test Workflow"):
		doc = saashq.get_doc("Workflow", "SO Test Workflow")
		doc.set("is_active", 1)
		doc.save()
		return doc

	saashq.get_doc(dict(doctype="Role", role_name="Test Junior Approver")).insert(ignore_if_duplicate=True)
	saashq.get_doc(dict(doctype="Role", role_name="Test Approver")).insert(ignore_if_duplicate=True)
	saashq.cache().hdel("roles", saashq.session.user)

	workflow = saashq.get_doc(
		{
			"doctype": "Workflow",
