...
erpnexus.accounts.taxes.setup_tax_validations("Sales Order");
erpnexus.sales_common.setup_selling_controller();

saashq.ui.form.on("Sales Order", {
	setup: function (frm) {
	...
	refresh: function (frm) {
		if (frm.doc.docstatus === 1) {
			if (
				...
				frm.add_custom_button(__("Update Items"), () => {
					erpnexus.utils.update_child_items({
						frm: frm,
						child_docname: "items",
			...
			if (
				...
				saashq.model.can_cancel("Stock Reservation Entry")
			) {
				frm.add_custom_button(
					__("Unreserve"),
			...
			frm.doc.items.forEach((item) => {
				if (flt(item.stock_reserved_qty) > 0 && saashq.model.can_read("Stock Reservation Entry")) {
					frm.add_custom_button(
						__("Reserved Stock"),
		...
		if (frm.doc.docstatus === 0) {
			...
			if (frm.doc.docstatus === 0) {
				saashq.call({
					method: "erpnexus.selling.doctype.sales_order.sales_order.get_stock_reservation_status",
					callback: function (r) {
						if (!r.message) {
	...
	get_items_from_internal_purchase_order(frm) {
		if (!saashq.model.can_read("Purchase Order")) {
			return;
		}
		...
		frm.add_custom_button(
			...
			() => {
				erpnexus.utils.map_current_doc({
					method: "erpnexus.buying.doctype.purchase_order.purchase_order.make_inter_company_sales_order",
	...
	onload: function (frm) {
		if (!frm.doc.transaction_date) {
			frm.set_value("transaction_date", saashq.datetime.get_today());
		}
		erpnexus.queries.setup_queries(frm, "Warehouse", function () {
			return {
		...
		frm.set_query("warehouse", "items", function (doc, cdt, cdn) {
			...
			if (row.item_code) {
				query.query = "erpnexus.controllers.queries.warehouse_query";
		...
		const dialog = new saashq.ui.Dialog({
			...
			fields: [
				...
				{ fieldtype: "Column Break" },
				{
					...
					get_query: () => {
						return {
							query: "erpnexus.controllers.queries.get_filtered_child_rows",
							filters: {
			...
			primary_action: () => {
				...
				if (data.items && data.items.length > 0) {
					...
					saashq.msgprint(__("Please select items to reserve."));
				}
			},
		});
	...
	cancel_stock_reservation_entries(frm) {
		const dialog = new saashq.ui.Dialog({
			...
			primary_action: () => {
				...
				if (data.sr_entries && data.sr_entries.length > 0) {
					...
					saashq.msgprint(__("Please select items to unreserve."));
				}
			},
		});
		...
		saashq
			.call({
				method: "erpnexus.stock.doctype.stock_reservation_entry.stock_reservation_entry.get_stock_reservation_entries_for_voucher",
				args: {
	...
	show_reserved_stock(frm) {
		...
		saashq.set_route("query-report", "Reserved Stock");
	},
});

saashq.ui.form.on("Sales Order Item", {
	...
	delivery_date: function (frm, cdt, cdn) {
		if (!frm.doc.delivery_date) {
			erpnexus.utils.copy_value_in_all_rows(frm.doc, cdt, cdn, "items", "delivery_date");
		}
	},
});

erpnexus.selling.SalesOrderController = class SalesOrderController extends erpnexus.selling.SellingController {
	onload(doc, dt, dn) {
	...
	refresh(doc, dt, dn) {
			...
			if (doc.status !== "Closed") {
				if (doc.status !== "On Hold") {
					...
					if (
						...
						saashq.model.can_create("Pick List")
					) {
						this.frm.add_custom_button(
							__("Pick List"),
					...
					if (
						...
						if (saashq.model.can_create("Delivery Note")) {
							this.frm.add_custom_button(
								__("Delivery Note"),
						...
						if (saashq.model.can_create("Work Order")) {
							this.frm.add_custom_button(
								__("Work Order"),
					...
					// sales invoice
					if (flt(doc.per_billed, 2) < 100 && saashq.model.can_create("Sales Invoice")) {
						this.frm.add_custom_button(
							__("Sales Invoice"),
					...
					if (
						(!doc.order_type ||
							((order_is_a_sale || order_is_a_custom_sale) &&
								flt(doc.per_delivered, 2) < 100)) &&
						saashq.model.can_create("Material Request")
					) {
						this.frm.add_custom_button(
							__("Material Request"),
					...
					// Make Purchase Order
					if (!this.frm.doc.is_internal_customer && saashq.model.can_create("Purchase Order")) {
						this.frm.add_custom_button(
							__("Purchase Order"),
					...
					if (flt(doc.per_delivered, 2) < 100 && (order_is_maintenance || order_is_a_custom_sale)) {
						if (saashq.model.can_create("Maintenance Visit")) {
							this.frm.add_custom_button(
						...
						if (saashq.model.can_create("Maintenance Schedule")) {
							this.frm.add_custom_button(
					...
					// project
					if (flt(doc.per_delivered, 2) < 100 && saashq.model.can_create("Project")) {
					...
					if (
						...
						saashq.model.can_create("Purchase Order")
					) {
						let me = this;
				...
				if (
					...
					100 + saashq.boot.sysdefaults.over_billing_allowance
				) {
					this.frm.add_custom_button(
						__("Payment Request"),
					...
					if (saashq.model.can_create("Payment Entry")) {
						this.frm.add_custom_button(
							__("Payment"),
		...
		if (this.frm.doc.docstatus === 0 && saashq.model.can_read("Quotation")) {
			this.frm.add_custom_button(
				__("Quotation"),
				function () {
					let d = erpnexus.utils.map_current_doc({
						method: "erpnexus.selling.doctype.quotation.quotation.make_sales_order",
	...
	create_pick_list() {
		saashq.model.open_mapped_doc({
			method: "erpnexus.selling.doctype.sales_order.sales_order.create_pick_list",
			frm: this.frm,
		});
	}
	...
	make_work_order() {
		var me = this;
		me.frm.call({
			method: "erpnexus.selling.doctype.sales_order.sales_order.get_work_order_items",
			args: {
				...
				if (!r.message) {
					...
					var d = new saashq.ui.Dialog({
						...
						primary_action: function () {
							...
							if (!data) {
								saashq.throw(__("Please select items"));
							}
							me.frm.call({
	...
	make_material_request() {
		saashq.model.open_mapped_doc({
			method: "erpnexus.selling.doctype.sales_order.sales_order.make_material_request",
			frm: this.frm,
		});
	}
	...
	make_raw_material_request_dialog(r) {
		...
		var d = new saashq.ui.Dialog({
			...
			primary_action: function () {
				...
				me.frm.call({
					method: "erpnexus.selling.doctype.sales_order.sales_order.make_raw_material_request",
					args: {
						items: data,
						company: me.frm.doc.company,
						sales_order: me.frm.docname,
						...
						<div class="list-item">
							<div class="list-item__content list-item__content--flex-2">
								...
								<input
									type="checkbox"
									data-date="${date}"
									${saashq.datetime.get_day_diff(new Date(date), today) > 0 ? "" : 'checked="checked"'}
								/>
								${saashq.datetime.str_to_user(date)}
								</label>
							</div>
						</div>
					`
	...
	make_delivery_note(delivery_dates, for_reserved_stock = false) {
		saashq.model.open_mapped_doc({
			method: "erpnexus.selling.doctype.sales_order.sales_order.make_delivery_note",
			frm: this.frm,
			args: {
				delivery_dates,
	...
	make_maintenance_schedule() {
		saashq.model.open_mapped_doc({
			method: "erpnexus.selling.doctype.sales_order.sales_order.make_maintenance_schedule",
			frm: this.frm,
		});
	}
	...
	make_project() {
		saashq.model.open_mapped_doc({
			method: "erpnexus.selling.doctype.sales_order.sales_order.make_project",
			frm: this.frm,
		});
	}
	...
	make_inter_company_order() {
		saashq.model.open_mapped_doc({
			method: "erpnexus.selling.doctype.sales_order.sales_order.make_inter_company_purchase_order",
			frm: this.frm,
		});
	}
	...
	make_maintenance_visit() {
		saashq.model.open_mapped_doc({
			method: "erpnexus.selling.doctype.sales_order.sales_order.make_maintenance_visit",
			frm: this.frm,
		});
	}
		...
		var dialog = new saashq.ui.Dialog({
			...
			primary_action(args) {
				...
				var method = args.against_default_supplier
					...
					: "make_purchase_order";
				return saashq.call({
					method: "erpnexus.selling.doctype.sales_order.sales_order." + method,
					...
					callback: function (r) {
						if (!r.exc) {
							if (!args.against_default_supplier) {
								...
								saashq.set_route("List", "Purchase Order");
							}
						}
					},
				});
	...
	hold_sales_order() {
		...
		var d = new saashq.ui.Dialog({
			...
			primary_action: function () {
				var data = d.get_values();
				saashq.call({
					method: "saashq.desk.form.utils.add_comment",
					args: {
	...
	update_status(label, status) {
		...
		saashq.call({
			method: "erpnexus.selling.doctype.sales_order.sales_order.update_status",
...
extend_cscript(cur_frm.cscript, new erpnexus.selling.SalesOrderController({ frm: cur_frm }));
