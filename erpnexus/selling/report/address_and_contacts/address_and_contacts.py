...
def get_columns(filters):
	...
	columns = [
		...
		f"{saashq.unscrub(str(party_type_value))}::150",
		"Address Line 1",
		"Address Line 2",
		"City",
		"State",
	...
	if filters.get("party_type") == "Supplier" and saashq.db.get_single_value(
		"Buying Settings", "supp_master_name"
	) == ["Naming Series", "Auto Name"]:
		columns.insert(1, "Supplier Name:Data:150")
	if filters.get("party_type") == "Customer" and saashq.db.get_single_value(
...
def get_party_addresses_and_contact(party_type, party, party_group, filters):
	...
	fetch_party_list = saashq.get_list(
	...
	for d in party_list:
		party_details.setdefault(d, saashq._dict())
...
def get_party_details(party_type, party_list, doctype, party_details):
	...
	for d in records:
		...
		details.setdefault(saashq.scrub(doctype), []).append(d[1:])
