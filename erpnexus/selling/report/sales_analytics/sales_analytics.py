...
from saashq.utils import add_days, add_to_date, flt, getdate
...
class Analytics:
	def __init__(self, filters=None):
		...
		if self.filters.doc_type == "Payment Entry" and self.filters.value_quantity == "Quantity":
			saashq.throw(_("Only Value available for Payment Entry"))
		self.date_field = (
			"transaction_date"
		...
		for d in reversed(self.group_entries):
			...
			for end_date in self.periodic_daterange:
				...
				if d.parent and (self.filters.tree_type != "Order Type" or d.parent == "Order Types"):
					self.entity_periodic_data.setdefault(d.parent, saashq._dict()).setdefault(period, 0.0)
	...
	def get_periodic_data(self):
		self.entity_periodic_data = saashq._dict()

		for d in self.entries:
			if self.filters.tree_type == "Supplier Group":
				d.entity = self.parent_child_map.get(d.entity)
			period = self.get_period(d.get(self.date_field))
			self.entity_periodic_data.setdefault(d.entity, saashq._dict()).setdefault(period, 0.0)
	...
	def get_groups(self):
		...
		self.depth_map = saashq._dict()

		self.group_entries = saashq.db.sql(
	...
	def get_supplier_parent_child_map(self):
		self.parent_child_map = saashq._dict(
			saashq.db.sql(""" select name, supplier_group from `tabSupplier`""")
		)
