...
saashq.query_reports["Sales Analytics"] = {
	filters: [
		{
			fieldname: "tree_type",
			label: __("Tree Type"),
		...
		},
		{
			...
			fieldtype: "Date",
			default:
				saashq.defaults.get_user_default("sales_start_date") ||
				erpnexus.utils.get_fiscal_year(saashq.datetime.get_today(), true)[1],
			reqd: 1,
		},
		{
			fieldname: "to_date",
			...
			fieldtype: "Date",
			default:
				saashq.defaults.get_user_default("sales_end_date") ||
				erpnexus.utils.get_fiscal_year(saashq.datetime.get_today(), true)[2],
			reqd: 1,
		},
		{
			fieldname: "company",
		...
		return Object.assign(options, {
			...
			events: {
				onCheckRow: function (data) {
					...
					const tree_type = saashq.query_report.filters[0].value;
					if (data_doctype != tree_type) return;

					const row_name = data[2].content;
					const raw_data = saashq.query_report.chart.data;
					...
					const new_options = Object.assign({}, saashq.query_report.chart_options, {
						data: new_data,
					});
					saashq.query_report.render_chart(new_options);

					saashq.query_report.raw_chart_data = new_data;
				},
			},
		});
	},
