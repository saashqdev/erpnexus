...
def get_data(filters=None):
	...
	for territory in saashq.get_all("Territory"):
...
def get_opportunities(filters):
	...
	if filters.get("transaction_date"):
		conditions = " WHERE transaction_date between {} and {}".format(
			saashq.db.escape(filters["transaction_date"][0]),
			saashq.db.escape(filters["transaction_date"][1]),
		)
