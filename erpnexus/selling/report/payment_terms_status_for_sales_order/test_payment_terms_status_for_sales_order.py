...
from saashq.utils import add_days, add_months, nowdate
...
from erpnexus.selling.report.payment_terms_status_for_sales_order.payment_terms_status_for_sales_order import (
	execute,
)
...
class TestPaymentTermsStatusForSalesOrder(IntegrationTestCase):
	...
	def create_payment_terms_template(self):
		...
		template = None
		if saashq.db.exists("Payment Terms Template", "_Test 50-50"):
			template = saashq.get_doc("Payment Terms Template", "_Test 50-50")
		else:
			...
			doc = saashq.get_doc(
				{
					...
					"date": date,
					"from_currency": "USD",
					"to_currency": saashq.get_cached_value("Company", "_Test Company", "default_currency"),
					"exchange_rate": 70,
					"for_buying": True,
					"for_selling": True,
				}
	...
	def test_03_group_filters(self):
		...
		for idx, g in enumerate(group_filters, 0):
			# build filter
			filters = saashq._dict({}).update(base_filters).update(g)
