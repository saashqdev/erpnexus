...
import saashq
from saashq import _, qb, query_builder
from saashq.query_builder import Criterion, functions
from saashq.utils.dateutils import getdate
...
def get_conditions(filters):
	...
	"""
	filters = saashq._dict(filters) if filters else saashq._dict({})
	conditions = saashq._dict({})

	conditions.company = filters.company or saashq.defaults.get_user_default("company")
	conditions.end_date = filters.period_end_date or saashq.utils.today()
	conditions.start_date = filters.period_start_date or saashq.utils.add_months(conditions.end_date, -1)
...
def set_payment_terms_statuses(sales_orders, invoices, filters):
	...
	for so in sales_orders:
		so.currency = saashq.get_cached_value("Company", filters.get("company"), "default_currency")
		so.invoices = ""
