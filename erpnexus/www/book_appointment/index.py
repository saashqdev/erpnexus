...
def get_context(context):
	is_enabled = saashq.db.get_single_value("Appointment Booking Settings", "enable_scheduling")
	if is_enabled:
		return context
	else:
		saashq.redirect_to_message(
...
def get_appointment_settings():
	settings = saashq.get_cached_value(
...
def get_appointment_slots(date, timezone):
	...
	# Database queries
	settings = saashq.get_doc("Appointment Booking Settings")
	holiday_list = saashq.get_doc("Holiday List", settings.holiday_list)
...
def create_appointment(date, time, tz, contact):
	...
	appointment = saashq.new_doc("Appointment")
...
def check_availabilty(timeslot, settings):
	return saashq.db.count("Appointment", {"scheduled_time": timeslot}) < settings.number_of_agents
