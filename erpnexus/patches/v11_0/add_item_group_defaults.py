...
def execute():
	...
	saashq.reload_doc("setup", "doctype", "item_group")

	companies = saashq.get_all("Company")
	item_groups = saashq.db.sql(
	...
	if len(companies) == 1:
		for item_group in item_groups:
			doc = saashq.get_doc("Item Group", item_group.get("name"))
			item_group_defaults = []
		...
		for item_group in item_groups:
			...
			for d in [
				...
				if item_group.get(d[0]):
					company = saashq.get_value(d[1], item_group.get(d[0]), "company", cache=True)
