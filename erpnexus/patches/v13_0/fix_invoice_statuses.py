...
def execute():
	...
	if not saashq.db.exists("DocType", "Party Specific Item"):
		return
	...
	for doctype in ("Purchase Invoice", "Sales Invoice"):
		...
		invoices_to_update = saashq.get_all(
			doctype,
			fields=fields,
			filters={
				"docstatus": 1,
		...
		payment_schedule_items = saashq.get_all(
			"Payment Schedule",
		...
		for status, docs in status_map.items():
			saashq.db.set_value(doctype, {"name": ("in", docs)}, "status", status, update_modified=False)
