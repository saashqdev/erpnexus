...
def execute():
	...
	"""
	work_order = saashq.qb.DocType("Work Order")
	query = (
		saashq.qb.from_(work_order)
		.select(
			work_order.name,
	...
	for row in results:
		so_item = saashq.get_value("Material Request Item", row.material_request_item, "sales_order_item")
		saashq.db.set_value("Work Order", row.name, "sales_order_item", so_item)

		if so_item:
			wo = saashq.get_doc("Work Order", row.name)
