...
def execute():
	...
	saashq.db.MAX_WRITES_PER_TRANSACTION += len(broken_sles)
	...
	for sle in broken_sles:
		...
		saashq.db.set_value("Stock Ledger Entry", sle.name, "serial_no", correct_sr_no, update_modified=False)
	...
	broken_sr_no_records = saashq.get_list(
		"Serial No",
		filters={
			"status": "Active",
	...
	saashq.db.MAX_WRITES_PER_TRANSACTION += len(broken_sr_no_records)

	patch_savepoint = "serial_no_patch"
	for serial_no in broken_sr_no_records:
		try:
			saashq.db.savepoint(patch_savepoint)
			sn = saashq.get_doc("Serial No", serial_no)
			sn.update_serial_no_reference()
			sn.db_update()
		except Exception:
			saashq.db.rollback(save_point=patch_savepoint)
