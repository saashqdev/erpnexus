...
def delete_broken_bins():
	# delete useless bins
	saashq.db.sql("delete from `tabBin` where item_code is null or warehouse is null")


def delete_and_patch_duplicate_bins():
	duplicate_bins = saashq.db.sql(
		"""
		SELECT
	...
	for duplicate_bin in duplicate_bins:
		...
		for broken_bin in existing_bins:
			saashq.delete_doc("Bin", broken_bin.name)
