...
def get_existing_cost_center_allocations():
	if not saashq.db.exists("DocType", "Distributed Cost Center"):
		return

	par = saashq.qb.DocType("Cost Center")
	child = saashq.qb.DocType("Distributed Cost Center")
	...
	cc_allocations = saashq._dict()
	for d in records:
		cc_allocations.setdefault(d.name, saashq._dict()).setdefault(d.cost_center, d.percentage_allocation)
