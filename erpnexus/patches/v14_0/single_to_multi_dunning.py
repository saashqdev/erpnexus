...
def execute():
	saashq.reload_doc("accounts", "doctype", "overdue_payment")
	saashq.reload_doc("accounts", "doctype", "dunning")
	...
	all_dunnings = saashq.get_all("Dunning", filters=filters, pluck="name")

	for dunning_name in all_dunnings:
		dunning = saashq.get_doc("Dunning", dunning_name)
		...
		payment_schedules = saashq.get_all(
			"Payment Schedule",
...
def get_accounts_closing_date():
	...
	accounts_frozen_till = saashq.db.get_single_value(
		"Accounts Settings", "acc_frozen_upto"
	)  # always returns datetime.date

	period_closing_date = saashq.db.get_value(
