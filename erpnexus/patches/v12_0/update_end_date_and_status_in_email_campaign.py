...
def execute():
	if saashq.db.exists("DocType", "Email Campaign"):
		email_campaign = saashq.get_all("Email Campaign")
		for campaign in email_campaign:
			doc = saashq.get_doc("Email Campaign", campaign["name"])
			send_after_days = []

			camp = saashq.get_doc("Campaign", doc.campaign_name)
