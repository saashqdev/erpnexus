...
def execute():
	...
	company = saashq.db.get_value("Company", {"country": "India"}, "name")
	if company:
...
def create_stock_entry_types():
	...
	for purpose in [
		...
		ste_type = saashq.get_doc({"doctype": "Stock Entry Type", "name": purpose, "purpose": purpose})
	...
	saashq.db.sql(" UPDATE `tabStock Entry` set stock_entry_type = purpose ")
...
def add_gst_hsn_code_field():
	...
	create_custom_fields(custom_fields, ignore_validate=saashq.flags.in_patch, update=True)
