...
def make_project_template(project_template_name, project_tasks=None):
	if project_tasks is None:
		project_tasks = []
	if not saashq.db.exists("Project Template", project_template_name):
		...
		doc = saashq.get_doc(dict(doctype="Project Template", name=project_template_name))
		for task in project_tasks:
			doc.append("tasks", {"task": task.name})
		doc.insert()

	return saashq.get_doc("Project Template", project_template_name)
