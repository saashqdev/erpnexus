...
from saashq.utils import flt, get_datetime, getdate
...
class OverWorkLoggedError(saashq.ValidationError):
	pass
...
class Timesheet(Document):
	...
	if TYPE_CHECKING:
		...
		from erpnexus.projects.doctype.timesheet_detail.timesheet_detail import TimesheetDetail
	...
	def validate_mandatory_fields(self):
		for data in self.time_logs:
			if not data.from_time and not data.to_time:
				saashq.throw(_("Row {0}: From Time and To Time is mandatory.").format(data.idx))

			if not data.activity_type and self.employee:
				saashq.throw(_("Row {0}: Activity Type is mandatory.").format(data.idx))

			if flt(data.hours) == 0.0:
				saashq.throw(_("Row {0}: Hours value must be greater than zero.").format(data.idx))
	...
	def update_task_and_project(self):
		...
		for data in self.time_logs:
			if data.task and data.task not in tasks:
				task = saashq.get_doc("Task", data.task)
				...
				saashq.get_doc("Project", data.project).update_project()
	...
	def validate_overlap(self, data):
		settings = saashq.get_single("Projects Settings")
	...
	def get_overlap_for(self, fieldname, args, value):
		timesheet = saashq.qb.DocType("Timesheet")
		timelog = saashq.qb.DocType("Timesheet Detail")

		from_time = get_datetime(args.from_time)
		to_time = get_datetime(args.to_time)

		existing = (
			saashq.qb.from_(timesheet)
			.join(timelog)
...
def get_projectwise_timesheet_data(project=None, parent=None, from_time=None, to_time=None):
	...
	return saashq.db.sql(query, filters, as_dict=1)


@saashq.whitelist()
def get_timesheet_detail_rate(timelog, currency):
	timelog_detail = saashq.db.sql(
...
def make_sales_invoice(source_name, item_code=None, customer=None, currency=None):
	target = saashq.new_doc("Sales Invoice")
	timesheet = saashq.get_doc("Timesheet", source_name)

	if not timesheet.total_billable_hours:
		saashq.throw(_("Invoice can't be made for zero billing hour"))

	if timesheet.total_billable_hours == timesheet.total_billed_hours:
		saashq.throw(_("Invoice already created for all billing hours"))
	...
	if customer:
		...
		default_price_list = saashq.get_value("Customer", customer, "default_price_list")
...
def get_timesheets_list(doctype, txt, filters, limit_start, limit_page_length=20, order_by="creation"):
	...
	contact = saashq.db.exists("Contact", {"user": user})
	if contact:
		# find customer
	...
	if customer:
		sales_invoices = [
			d.name for d in saashq.get_all("Sales Invoice", filters={"customer": customer})
		] or [None]
		projects = [d.name for d in saashq.get_all("Project", filters={"customer": customer})]
		# Return timesheet related data to web portal.
		timesheets = saashq.db.sql(
			f"""
			SELECT
