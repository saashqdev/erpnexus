...
from saashq.query_builder.functions import Count, CurDate, Date, Sum, UnixTimestamp
from saashq.utils import add_days, flt, get_datetime, get_time, get_url, nowtime, today
from saashq.utils.user import is_website_user

from erpnexus import get_default_company
from erpnexus.controllers.queries import get_filters_cond
from erpnexus.controllers.website_list_for_contact import get_customers_suppliers
from erpnexus.setup.doctype.holiday_list.holiday_list import is_holiday
...
class Project(Document):
	...
	if TYPE_CHECKING:
		...
		from erpnexus.projects.doctype.project_user.project_user import ProjectUser
	...
	def copy_from_template(self):  # nosemgrep
		...
		"""
		if self.project_template and not saashq.db.get_all("Task", dict(project=self.name), limit=1):
			# has a template, and no loaded tasks, so lets create
			if not self.expected_start_date:
				# project starts today
				self.expected_start_date = today()

			template = saashq.get_doc("Project Template", self.project_template)
			...
			for task in template.tasks:
				template_task_details = saashq.get_doc("Task", task.task)
	...
	def dependency_mapping(self, template_tasks, project_tasks):
		for project_task in project_tasks:
			template_task = saashq.get_doc("Task", project_task.template_task)
	...
	def after_insert(self):
		self.copy_from_template()  # nosemgrep
		if self.sales_order:
			saashq.db.set_value("Sales Order", self.sales_order, "project", self.name)

	def on_trash(self):
		saashq.db.set_value("Sales Order", {"project": self.name}, "project", "")

	def update_percent_complete(self):
		if self.status == "Completed":
			if (
				len(saashq.get_all("Task", dict(project=self.name))) == 0
		...
		total = saashq.db.count("Task", dict(project=self.name))
		...
		if not total:
			...
			if self.percent_complete_method == "Task Weight" and total > 0:
				weight_sum = saashq.db.sql(
					"""select sum(task_weight) from tabTask where
					project=%s""",
					self.name,
				)[0][0]
				weighted_progress = saashq.db.sql(
				...
				for row in weighted_progress:
					pct_complete += row["progress"] * saashq.utils.safe_div(row["task_weight"], weight_sum)
	...
	def update_costing(self):
		from saashq.query_builder.functions import Max, Min, Sum

		TimesheetDetail = saashq.qb.DocType("Timesheet Detail")
		from_time_sheet = (
			saashq.qb.from_(TimesheetDetail)
			.select(
	...
	def update_sales_amount(self):
		total_sales_amount = saashq.db.sql(
	...
	def update_billed_amount(self):
		# nosemgrep
		total_billed_amount = saashq.db.sql(
	...
	def after_rename(self, old_name, new_name, merge=False):
		if old_name == self.copied_from:
			saashq.db.set_value("Project", new_name, "copied_from", new_name)
...
def get_timeline_data(doctype: str, name: str) -> dict[int, int]:
	...
	timesheet_detail = saashq.qb.DocType("Timesheet Detail")

	return dict(
		saashq.qb.from_(timesheet_detail)
...
def get_project_list(doctype, txt, filters, limit_start, limit_page_length=20, order_by="creation"):
	customers, suppliers = get_customers_suppliers("Project", saashq.session.user)

	ignore_permissions = False
	if is_website_user() and saashq.session.user != "Guest":
		if not filters:
			filters = []
...
def get_cost_center_name(project):
	return saashq.db.get_value("Project", project, "cost_center")
...
def create_duplicate_project(prev_doc, project_name):
	...
	if project_name == prev_doc.get("name"):
		saashq.throw(_("Use a name that is different from previous project name"))

	# change the copied doc name to new project name
	project = saashq.copy_doc(prev_doc)
	...
	task_list = saashq.get_all("Task", filters={"project": prev_doc.get("name")}, fields=["name"])

	# Create duplicate task for all the task
	for task in task_list:
		task = saashq.get_doc("Task", task)
		new_task = saashq.copy_doc(task)
...
def send_project_update_email_to_users(project):
	doc = saashq.get_doc("Project", project)

	if is_holiday(doc.holiday_list) or not doc.users:
		return

	project_update = saashq.get_doc(
		{
	...
	incoming_email_account = saashq.db.get_value(
...
def collect_project_status():
	for data in saashq.get_all("Project Update", {"date": today(), "sent": 0}):
		replies = saashq.get_all(
			"Communication",
		...
		for d in replies:
			doc = saashq.get_doc("Project Update", data.name)
			user_data = saashq.db.get_values(
			...
			doc.append(
				"users",
				{
					...
					"project_status": saashq.utils.md_to_html(
...
def send_project_status_email_to_users():
	...
	for d in saashq.get_all("Project Update", {"date": yesterday, "sent": 0}):
		doc = saashq.get_doc("Project Update", d.name)

		project_doc = saashq.get_doc("Project", doc.project)
...
def update_project_sales_billing():
	sales_update_frequency = saashq.db.get_single_value("Selling Settings", "sales_update_frequency")
	if sales_update_frequency == "Each Transaction":
		return
	elif sales_update_frequency == "Monthly" and saashq.utils.now_datetime().day != 1:
		return

	# Else simply fallback to Daily
	for project in saashq.get_all("Project", filters={"status": ["!=", "Cancelled"]}):
		saashq.get_doc("Project", project.name).save()


@saashq.whitelist()
def create_kanban_board_if_not_exists(project):
	from saashq.desk.doctype.kanban_board.kanban_board import quick_kanban_board

	project = saashq.get_doc("Project", project)
	if not saashq.db.exists("Kanban Board", project.project_name):
...
def set_project_status(project, status):
	...
	if status not in ("Completed", "Cancelled"):
		saashq.throw(_("Status must be Cancelled or Completed"))

	project = saashq.get_doc("Project", project)
	saashq.has_permission(doc=project, throw=True)

	for task in saashq.get_all("Task", dict(project=project.name)):
		saashq.db.set_value("Task", task.name, "status", status)
...
def get_holiday_list(company=None):
	if not company:
		company = get_default_company() or saashq.get_all("Company")[0].name

	holiday_list = saashq.get_cached_value("Company", company, "default_holiday_list")
	if not holiday_list:
		saashq.throw(
			_("Please set a default Holiday List for Company {0}").format(saashq.bold(get_default_company()))
		)
	return holiday_list


def get_users_email(doc):
	return [d.email for d in doc.users if saashq.db.get_value("User", d.user, "enabled")]
