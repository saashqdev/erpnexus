...
from erpnexus.projects.doctype.project_template.test_project_template import make_project_template
from erpnexus.projects.doctype.task.test_task import create_task
from erpnexus.selling.doctype.sales_order.sales_order import make_project as make_project_from_so
...
class TestProject(IntegrationTestCase):
	def test_project_with_template_having_no_parent_and_depend_tasks(self):
		...
		saashq.db.sql(""" delete from tabTask where project = %s """, project_name)
		saashq.delete_doc("Project", project_name)
	...
	def test_project_template_having_parent_child_tasks(self):
		...
		if saashq.db.get_value("Project", {"project_name": project_name}, "name"):
			project_name = saashq.db.get_value("Project", {"project_name": project_name}, "name")
...
def make_project(args):
	...
	if args.project_name and saashq.db.exists("Project", {"project_name": args.project_name}):
		return saashq.get_doc("Project", {"project_name": args.project_name})
...
def task_exists(subject):
	result = saashq.db.get_list("Task", filters={"subject": subject}, fields=["name"])
	if not len(result):
		return False
	return saashq.get_doc("Task", result[0].name)
