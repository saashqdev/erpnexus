...
from erpnexus.projects.doctype.task.task import CircularReferenceError
...
class TestTask(IntegrationTestCase):
	...
	def test_reschedule_dependent_task(self):
		...
		self.assertEqual(
			getdate(saashq.db.get_value("Task", task2.name, "exp_start_date")),
			getdate(add_days(nowdate(), 21)),
		)

		self.assertEqual(
			getdate(saashq.db.get_value("Task", task2.name, "exp_end_date")), getdate(add_days(nowdate(), 25))
		)

		self.assertEqual(
			getdate(saashq.db.get_value("Task", task3.name, "exp_start_date")),
			getdate(add_days(nowdate(), 26)),
		)

		self.assertEqual(
			getdate(saashq.db.get_value("Task", task3.name, "exp_end_date")), getdate(add_days(nowdate(), 30))
		)

	def test_close_assignment(self):
		if not saashq.db.exists("Task", "Test Close Assignment"):
			task = saashq.new_doc("Task")
	...
	def test_overdue(self):
		...
		from erpnexus.projects.doctype.task.task import set_tasks_as_overdue

		set_tasks_as_overdue()

		self.assertEqual(saashq.db.get_value("Task", task.name, "status"), "Overdue")
...
def create_task(
	...
	if not saashq.db.exists("Task", subject):
		task = saashq.new_doc("Task")
		task.status = "Open"
		...
		task.project = (
			project or None if is_template else saashq.get_value("Project", {"project_name": "_Test Project"})
		)
		...
		task = saashq.get_doc("Task", subject)
