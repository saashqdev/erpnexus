saashq.listview_settings["Task"] = {
	...
	onload: function (listview) {
		var method = "erpnexus.projects.doctype.task.task.set_multiple_status";
		...
		if (task._assign) {
			...
			const assignment_wrapper = `
				...
				<span class="text-white">
					${assign_list.map((user) => saashq.user_info(user).fullname).join(", ")}
				</span>
			`;
