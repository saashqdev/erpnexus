...
from saashq.desk.form.assign_to import clear, close_all_assignments
from saashq.model.mapper import get_mapped_doc
from saashq.utils import add_days, add_to_date, cstr, date_diff, flt, get_link_to_form, getdate, today
from saashq.utils.data import format_date
from saashq.utils.nestedset import NestedSet


class CircularReferenceError(saashq.ValidationError):
	pass
...
class Task(NestedSet):
	...
	if TYPE_CHECKING:
		...
		from erpnexus.projects.doctype.task_depends_on.task_depends_on import TaskDependsOn
	...
	def get_customer_details(self):
		cust = saashq.db.sql("select customer_name from `tabCustomer` where name=%s", self.customer)
		if cust:
	...
	def validate_parent_expected_end_date(self):
		...
		parent_exp_end_date = saashq.db.get_value("Task", self.parent_task, "exp_end_date")
	...
	def validate_parent_project_dates(self):
		if not self.project or saashq.flags.in_test:
			return

		if project_end_date := saashq.db.get_value("Project", self.project, "expected_end_date"):
				...
				if task_date and date_diff(project_end_date, getdate(task_date)) < 0:
					saashq.throw(
						_("{0}'s {1} cannot be after {2}'s Expected End Date.").format(
							saashq.bold(saashq.get_desk_link("Task", self.name)),
							_(self.meta.get_label(fieldname)),
							saashq.bold(saashq.get_desk_link("Project", self.project)),
						),
						saashq.exceptions.InvalidDates,
					)
	...
	def validate_status(self):
		...
		if self.status != self.get_db_value("status") and self.status == "Completed":
			for d in self.depends_on:
				if saashq.db.get_value("Task", d.task, "status") not in ("Completed", "Cancelled"):
					saashq.throw(
						_(
							"Cannot complete task {0} as its dependant task {1} are not completed / cancelled."
						).format(saashq.bold(self.name), saashq.bold(d.task))
					)
	...
	def validate_progress(self):
		if flt(self.progress or 0) > 100:
			saashq.throw(_("Progress % for a task cannot be more than 100."))
	...
	def validate_parent_template_task(self):
		if self.parent_task:
			if not saashq.db.get_value("Task", self.parent_task, "is_template"):
				parent_task_format = f"""<a href="#Form/Task/{self.parent_task}">{self.parent_task}</a>"""
				saashq.throw(_("Parent Task {0} is not a Template Task").format(parent_task_format))

	def validate_depends_on_tasks(self):
		if self.depends_on:
			for task in self.depends_on:
				if not saashq.db.get_value("Task", task.task, "is_template"):
					dependent_task_format = f"""<a href="#Form/Task/{task.task}">{task.task}</a>"""
					saashq.throw(_("Dependent Task {0} is not a Template Task").format(dependent_task_format))

	def validate_completed_on(self):
		if self.completed_on and getdate(self.completed_on) > getdate():
			saashq.throw(_("Completed On cannot be greater than Today"))
	...
	def update_project(self):
		if self.project and not self.flags.from_project:
			saashq.get_cached_doc("Project", self.project).update_project()
	...
	def check_recursion(self):
		...
		for d in check_list:
			...
			while len(task_list) > count:
				...
				for b in tasks:
					if b[0] == self.name:
						saashq.throw(_("Circular Reference Error"), CircularReferenceError)
					if b[0]:
	...
	def reschedule_dependent_tasks(self):
		...
		if end_date:
			for task_name in saashq.db.sql(
				"""
			...
			""",
				...
				task = saashq.get_doc("Task", task_name.name)
				if (
					task.exp_start_date
	...
	def has_webform_permission(self):
		project_user = saashq.db.get_value(
			"Project User", {"parent": self.project, "user": saashq.session.user}, "user"
		)
		if project_user:
			return True
	...
	def populate_depends_on(self):
		if self.parent_task:
			parent = saashq.get_doc("Task", self.parent_task)
...
def check_if_child_exists(name):
	child_tasks = saashq.get_all("Task", filters={"parent_task": name})
	...
	for task in tasks:
		if task.status == "Pending Review":
			if getdate(task.review_date) > getdate(today()):
				continue
		saashq.get_doc("Task", task.name).update_status()
...
def add_multiple_tasks(data, parent):
	...
	new_doc["project"] = saashq.db.get_value("Task", {"name": parent}, "project") or ""

	for d in data:
		if not d.get("subject"):
			continue
		new_doc["subject"] = d.get("subject")
		new_task = saashq.get_doc(new_doc)
		new_task.insert()


def on_doctype_update():
	saashq.db.add_index("Task", ["lft", "rgt"])
