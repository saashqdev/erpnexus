...
@saashq.whitelist()
def create_custom_fields_for_saashq_crm():
...
def create_prospect_against_crm_deal():
	...
	try:
		prospect_name = saashq.db.get_value("Prospect", {"company_name": prospect.company_name})
		...
		saashq.log_error(
			...
			f"Error while creating prospect against CRM Deal: {saashq.form_dict.get('crm_deal_id')}",
		)
		pass
	...
	saashq.response["message"] = prospect_name
...
def create_contacts(contacts, organization=None, link_doctype=None, link_docname=None):
	for c in contacts:
		...
		if existing_contact:
			contact = saashq.get_doc("Contact", existing_contact)
		else:
...
def create_address(doctype, docname, address):
	...
	try:
		_address = saashq.db.exists("Address", address.get("name"))
		if not _address:
			...
			address = saashq.get_doc("Address", _address)
			link_doc(address, doctype, docname)
			address.save(ignore_permissions=True)
			return address.name
	except Exception:
		saashq.log_error(saashq.get_traceback(), f"Error while creating address for {docname}")
...
def contact_exists(email, mobile_no):
	email_exist = saashq.db.exists("Contact Email", {"email_id": email})
	mobile_exist = saashq.db.exists("Contact Phone", {"phone": mobile_no})

	doctype = "Contact Email" if email_exist else "Contact Phone"
	name = email_exist or mobile_exist

	if name:
		return saashq.db.get_value(doctype, name, "parent")
...
def create_customer(customer_data=None):
	...
	if not customer_data:
		customer_data = saashq.form_dict

	try:
		customer_name = saashq.db.exists("Customer", {"customer_name": customer_data.get("customer_name")})
		if not customer_name:
			customer = saashq.get_doc({"doctype": "Customer", **customer_data}).insert(
		...
		saashq.log_error(saashq.get_traceback(), "Error while creating customer against Saashq CRM Deal")
		pass
