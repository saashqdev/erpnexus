...
from erpnexus.crm.report.sales_pipeline_analytics.sales_pipeline_analytics import execute
...
def create_company():
	doc = saashq.db.exists("Company", "Best Test")
	if not doc:
		doc = saashq.new_doc("Company")
...
def create_customer():
	doc = saashq.db.exists("Customer", "_Test NC")
	if not doc:
		doc = saashq.new_doc("Customer")
		doc.customer_name = "_Test NC"
		doc.insert()


def create_opportunity():
	doc = saashq.db.exists({"doctype": "Opportunity", "party_name": "_Test NC"})
	if not doc:
		doc = saashq.new_doc("Opportunity")
		doc.opportunity_from = "Customer"
		customer_name = saashq.db.get_value("Customer", {"customer_name": "_Test NC"}, ["customer_name"])
