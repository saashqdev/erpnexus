...
class Prospect(CRMNote):
	...
	if TYPE_CHECKING:
		...
		from erpnexus.crm.doctype.prospect_lead.prospect_lead import ProspectLead
		from erpnexus.crm.doctype.prospect_opportunity.prospect_opportunity import ProspectOpportunity
	...
	def link_with_lead_contact_and_address(self):
		for row in self.leads:
			...
			for link in links:
				linked_doc = saashq.get_doc(link["parenttype"], link["parent"])
				exists = False
...
def make_customer(source_name, target_doc=None):
	def set_missing_values(source, target):
		...
		target.customer_group = source.customer_group or saashq.db.get_default("Customer Group")
