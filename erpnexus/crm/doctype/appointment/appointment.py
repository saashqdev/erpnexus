...
from saashq.share import add_docshare
from saashq.utils import get_url, getdate, now
...
class Appointment(Document):
	...
	def before_insert(self):
		number_of_appointments_in_same_slot = saashq.db.count(
			"Appointment", filters={"scheduled_time": self.scheduled_time}
		)
		number_of_agents = saashq.db.get_single_value("Appointment Booking Settings", "number_of_agents")
		if number_of_agents != 0:
			if number_of_appointments_in_same_slot >= number_of_agents:
				saashq.throw(_("Time slot is not available"))
		# Link lead
		if not self.party:
	...
	def send_confirmation_email(self):
		...
		if saashq.session.user == "Guest":
			saashq.msgprint(_("Please check your email to confirm the appointment"))
		else:
			saashq.msgprint(
	...
	def on_change(self):
		...
		if not self.calendar_event:
			return
		cal_event = saashq.get_doc("Event", self.calendar_event)
		cal_event.starts_on = self.scheduled_time
		cal_event.save(ignore_permissions=True)

	def set_verified(self, email):
		if email != self.customer_email:
			saashq.throw(_("Email verification failed."))
		# Create new lead
	...
	def get_assignee_from_latest_opportunity(self):
		if not self.party:
			return None
		if not saashq.db.exists("Lead", self.party):
			return None
		opporutnities = saashq.get_list(
			"Opportunity",
			filters={
		...
		if not opporutnities:
			return None
		latest_opportunity = saashq.get_doc("Opportunity", opporutnities[0].name)
		assignee = latest_opportunity._assign
		if not assignee:
			return None
		assignee = saashq.parse_json(assignee)[0]
		return assignee

	def create_calendar_event(self):
		if self.calendar_event:
			return
		appointment_event = saashq.get_doc(
			{
				"doctype": "Event",
				...
				"status": "Open",
				"type": "Public",
				"send_reminder": saashq.db.get_single_value(
	...
	def assign_agent(self, agent):
		if not saashq.has_permission(doc=self, user=agent):
...
def _get_agents_sorted_by_asc_workload(date):
	appointments = saashq.get_all("Appointment", fields="*")
	agent_list = _get_agent_list_as_strings()
	if not appointments:
		return agent_list
	appointment_counter = Counter(agent_list)
	for appointment in appointments:
		assigned_to = saashq.parse_json(appointment._assign)
		if not assigned_to:
			continue
...
def _get_agent_list_as_strings():
	...
	agent_list = saashq.get_doc("Appointment Booking Settings").agent_list
...
def _check_agent_availability(agent_email, scheduled_time):
	appointemnts_at_scheduled_time = saashq.get_all("Appointment", filters={"scheduled_time": scheduled_time})
...
def _get_employee_from_user(user):
	employee_docname = saashq.db.get_value("Employee", {"user_id": user})
	if employee_docname:
		return saashq.get_doc("Employee", employee_docname)
	return None
