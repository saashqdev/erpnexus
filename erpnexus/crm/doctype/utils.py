...
def get_last_interaction(contact=None, lead=None):
	...
	if contact:
		...
		if query_condition:
			...
			last_communication = saashq.db.sql(
				f"""
				SELECT `name`, `content`
	...
	if lead:
		last_communication = saashq.get_all(
			"Communication",
...
def get_scheduled_employees_for_popup(communication_medium):
	...
	now_time = saashq.utils.nowtime()
	weekday = saashq.utils.get_weekday()

	available_employee_groups = saashq.get_all(
