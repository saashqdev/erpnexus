...
from saashq.utils import add_days, nowdate


class TestContract(IntegrationTestCase):
	def setUp(self):
		saashq.db.sql("delete from `tabContract`")
	...
	def test_validate_start_date_before_end_date(self):
		...
		self.assertRaises(saashq.ValidationError, self.contract_doc.insert)
