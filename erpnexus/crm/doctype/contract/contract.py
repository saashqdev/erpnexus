...
class Contract(Document):
	...
	if TYPE_CHECKING:
		...
		from erpnexus.crm.doctype.contract_fulfilment_checklist.contract_fulfilment_checklist import (
	...
	def autoname(self):
		...
		if saashq.db.exists("Contract", name):
			count = len(saashq.get_all("Contract", filters={"name": ["like", f"%{name}%"]}))
			name = f"{name} - {count}"
	...
	def before_submit(self):
		self.signed_by_company = saashq.session.user
...
def update_status_for_contracts():
	...
	for contract in contracts:
		...
		saashq.db.set_value("Contract", contract.get("name"), "status", status)
