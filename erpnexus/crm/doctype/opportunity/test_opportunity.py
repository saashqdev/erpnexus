...
from saashq.utils import now_datetime, random_string, today

from erpnexus.crm.doctype.lead.lead import make_customer
from erpnexus.crm.doctype.lead.test_lead import make_lead
from erpnexus.crm.doctype.opportunity.opportunity import make_quotation
from erpnexus.crm.utils import get_linked_communication_list
...
class TestOpportunity(IntegrationTestCase):
	def test_opportunity_status(self):
		...
		doc = saashq.get_doc("Opportunity", doc.name)
	...
	def test_make_new_lead_if_required(self):
		...
		self.assertEqual(saashq.db.get_value("Lead", opp_doc.party_name, "email_id"), opp_doc.contact_email)
	...
	def test_carry_forward_of_email_and_comments(self):
		saashq.db.set_single_value("CRM Settings", "carry_forward_communication_and_comments", 1)
		lead_doc = make_lead()
		...
		opportunity_comment_count = saashq.db.count(
...
def make_opportunity_from_lead():
	...
	opp_doc = saashq.get_doc(args).insert(ignore_permissions=True)
