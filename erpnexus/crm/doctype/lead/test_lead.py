...
from saashq.utils import random_string, today

from erpnexus.crm.doctype.lead.lead import make_opportunity
from erpnexus.crm.utils import get_linked_prospect


class TestLead(IntegrationTestCase):
	def test_make_customer(self):
		from erpnexus.crm.doctype.lead.lead import make_customer

		saashq.delete_doc_if_exists("Customer", "_Test Lead")
	...
	def test_create_lead_and_unlinking_dynamic_links(self):
		...
		lead_doc.delete()
		address_1.reload()
		self.assertEqual(saashq.db.exists("Lead", lead_doc.name), None)
		self.assertEqual(len(address_1.get("links")), 1)

	def test_prospect_creation_from_lead(self):
		saashq.db.sql("delete from `tabLead` where lead_name='Rahul Tripathi'")
		saashq.db.sql("delete from `tabProspect` where name='Prospect Company'")
	...
	def test_opportunity_from_lead(self):
		saashq.db.sql("delete from `tabLead` where lead_name='Rahul Tripathi'")
		saashq.db.sql("delete from `tabOpportunity` where party_name='Rahul Tripathi'")
		...
		self.assertTrue(
			saashq.db.get_value("ToDo", {"reference_type": "Opportunity", "reference_name": opportunity.name})
		)
