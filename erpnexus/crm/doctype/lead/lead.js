...
erpnexus.LeadController = class LeadController extends saashq.ui.form.Controller {
	setup() {
	...
	refresh() {
		...
		if (!this.frm.is_new()) {
			saashq.contacts.render_address_and_contact(this.frm);
		} else {
			saashq.contacts.clear_address_and_contact(this.frm);
		}
	...
	add_lead_to_prospect(frm) {
		saashq.prompt(
			...
			function (data) {
				saashq.call({
					method: "erpnexus.crm.doctype.lead.lead.add_lead_to_prospect",
					args: {
						lead: frm.doc.name,
	...
	make_customer() {
		saashq.model.open_mapped_doc({
			method: "erpnexus.crm.doctype.lead.lead.make_customer",
			frm: this.frm,
		});
	}
	...
	make_quotation() {
		saashq.model.open_mapped_doc({
			method: "erpnexus.crm.doctype.lead.lead.make_quotation",
			frm: this.frm,
		});
	}
	...
	make_prospect() {
		...
		saashq.model.with_doctype("Prospect", function () {
			...
			let leads_row = saashq.model.add_child(prospect, "leads");
...
extend_cscript(cur_frm.cscript, new erpnexus.LeadController({ frm: cur_frm }));
