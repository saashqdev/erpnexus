saashq.ready(function () {
	...
	var reload_items = function (item_status, item, $btn) {
		$.ajax({
			...
			dataType: "json",
			data: {
				cmd: "erpnexus.templates.pages.projects.get_" + item + "_html",
				project: "{{ doc.name }}",
	...
	var close_item = function (item, item_name) {
		...
		saashq.call({
			btn: this,
			type: "POST",
			method: "erpnexus.templates.pages.projects.set_" + item + "_status",
			args: args,
			callback: function (r) {
				if (r.exc) {
					if (r._server_messages) saashq.msgprint(r._server_messages);
				} else {
					$(this).remove();
				}
			},
