...
class IncomingCallSettings(Document):
	...
	if TYPE_CHECKING:
		...
		from erpnexus.telephony.doctype.incoming_call_handling_schedule.incoming_call_handling_schedule import (
		...
		for day in week_days:
			...
			for i in range(1, len(timeslots)):
				if self.check_timeslots_overlap(timeslots[i - 1], timeslots[i]):
					saashq.throw(_("Please fix overlapping time slots for {0}.").format(day))
