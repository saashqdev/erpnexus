...
saashq.query_reports["Supplier Quotation Comparison"] = {
	filters: [
		{
			fieldtype: "Link",
			label: __("Company"),
		...
		},
		{
			...
			fieldtype: "Link",
			get_query: () => {
				let quote = saashq.query_report.get_filter_value("supplier_quotation");
				if (quote != "") {
					return {
						query: "erpnexus.stock.doctype.quality_inspection.quality_inspection.item_query",
						filters: {
		...
		},
		{
			...
			get_data: function (txt) {
				return saashq.db.get_link_options("Supplier", txt);
			},
		},
		{
			fieldtype: "MultiSelectList",
			...
			default: "",
			get_data: function (txt) {
				return saashq.db.get_link_options("Supplier Quotation", txt, { docstatus: ["<", 2] });
			},
		},
		{
			fieldtype: "Link",
	...
	formatter: (value, row, column, data, default_formatter) => {
		...
		if (column.fieldname === "valid_till" && data.valid_till) {
			if (saashq.datetime.get_diff(data.valid_till, saashq.datetime.nowdate()) <= 1) {
				value = `<div style="color:red">${value}</div>`;
			} else if (saashq.datetime.get_diff(data.valid_till, saashq.datetime.nowdate()) <= 7) {
	...
	onload: (report) => {
		...
		report.page.add_inner_button(
			...
			() => {
				let reporter = saashq.query_reports["Supplier Quotation Comparison"];
	...
	make_default_supplier_dialog: (report) => {
		...
		dialog.set_primary_action(__("Set Default Supplier"), () => {
			...
			if (values) {
				...
				saashq.call({
					method: "erpnexus.buying.report.supplier_quotation_comparison.supplier_quotation_comparison.set_default_supplier",
					args: {
					...
					freeze: true,
					callback: (r) => {
						saashq.msgprint(__("Successfully Set Supplier"));
						dialog.hide();
					},
				});
			}
