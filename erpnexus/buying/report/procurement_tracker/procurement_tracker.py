...
def get_mapped_mr_details(filters):
	mr_records = {}
	parent = saashq.qb.DocType("Material Request")
	child = saashq.qb.DocType("Material Request Item")
	...
	for record in mr_details:
		if record.per_ordered:
			mr_records.setdefault(record.name, []).append(saashq._dict(record))
		else:
...
def get_mapped_pi_records():
	...
	return saashq._dict(pi_records)


def get_mapped_pr_records():
	pr = saashq.qb.DocType("Purchase Receipt")
	pr_item = saashq.qb.DocType("Purchase Receipt Item")
	pr_records = (
		saashq.qb.from_(pr)
		.from_(pr_item)
	...
	return saashq._dict(pr_records)


def get_po_entries(filters):
	parent = saashq.qb.DocType("Purchase Order")
	child = saashq.qb.DocType("Purchase Order Item")
