...
saashq.ui.form.on("Request for Quotation", {
	setup: function (frm) {
	...
	refresh: function (frm, cdt, cdn) {
		if (frm.doc.docstatus === 1) {
			...
			frm.add_custom_button(
				...
				function () {
					saashq.call({
						method: "erpnexus.buying.doctype.request_for_quotation.request_for_quotation.send_supplier_emails",
						freeze: true,
						args: {
						...
						(data) => {
							var w = window.open(
								saashq.urllib.get_full_url(
									"/api/method/erpnexus.buying.doctype.request_for_quotation.request_for_quotation.get_pdf?" +
										new URLSearchParams({
											name: frm.doc.name,
											supplier: data.supplier,
											print_format: data.print_format || "Standard",
											language: data.language || saashq.boot.lang,
	...
	make_supplier_quotation: function (frm) {
		...
		var dialog = new saashq.ui.Dialog({
			...
			primary_action: (args) => {
				...
				return saashq.call({
					type: "GET",
					method: "erpnexus.buying.doctype.request_for_quotation.request_for_quotation.make_supplier_quotation_from_rfq",
					args: {
...
saashq.ui.form.on("Request for Quotation Item", {
	items_add(frm, cdt, cdn) {
		if (frm.doc.schedule_date) {
			saashq.model.set_value(cdt, cdn, "schedule_date", frm.doc.schedule_date);
		}
	},
});
saashq.ui.form.on("Request for Quotation Supplier", {
	supplier: function (frm, cdt, cdn) {
		var d = locals[cdt][cdn];
		saashq.call({
			method: "erpnexus.accounts.party.get_party_details",
			args: {
				party: d.supplier,
				party_type: "Supplier",
			},
			callback: function (r) {
				if (r.message) {
					saashq.model.set_value(cdt, cdn, "contact", r.message.contact_person);
					saashq.model.set_value(cdt, cdn, "email_id", r.message.contact_email);
				}
			},
		});
	},
...
erpnexus.buying.RequestforQuotationController = class RequestforQuotationController extends (
	...
	refresh() {
		...
		if (this.frm.doc.docstatus === 0) {
				...
				function () {
					...
					var dialog = new saashq.ui.Dialog({
						...
						primary_action: (args) => {
							...
							erpnexus.utils.map_current_doc({
								method: "erpnexus.buying.doctype.request_for_quotation.request_for_quotation.get_item_from_material_requests_based_on_supplier",
			...
			this.frm.add_custom_button(
				...
				function () {
					erpnexus.buying.link_to_mrs(me.frm);
				},
				__("Tools")
			);
		...
		var dialog = new saashq.ui.Dialog({
			...
			fields: [
				{
					...
					onchange() {
						if (dialog.get_value("search_type") == "Tag") {
							saashq
								.call({
									method: "erpnexus.buying.doctype.request_for_quotation.request_for_quotation.get_supplier_tag",
								})
								.then((r) => {
			...
			primary_action: (args) => {
				...
				if (args.search_type === "Tag" && args.tag) {
					return saashq.call({
						type: "GET",
						method: "saashq.desk.doctype.tag.tag.get_tagged_docs",
						args: {
							doctype: "Supplier",
							tag: "%" + args.tag + "%",
						},
...
extend_cscript(cur_frm.cscript, new erpnexus.buying.RequestforQuotationController({ frm: cur_frm }));
