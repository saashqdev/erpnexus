...
class TestSupplierScorecardCriteria(IntegrationTestCase):
	def test_variables_exist(self):
		...
		self.assertRaises(saashq.ValidationError, saashq.get_doc(test_bad_criteria[0]).insert)

	def test_formula_validate(self):
		delete_test_scorecards()
		self.assertRaises(saashq.ValidationError, saashq.get_doc(test_bad_criteria[1]).insert)
		self.assertRaises(saashq.ValidationError, saashq.get_doc(test_bad_criteria[2]).insert)


def delete_test_scorecards():
	# Delete all the periods so we can delete all the criteria
	saashq.db.sql("""delete from `tabSupplier Scorecard Period`""")
	saashq.db.sql(
	...
	for d in test_good_criteria:
		if saashq.db.exists("Supplier Scorecard Criteria", d.get("name")):
			# Delete all the periods, then delete the scorecard
			saashq.delete_doc(d.get("doctype"), d.get("name"))
