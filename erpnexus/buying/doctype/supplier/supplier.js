...
saashq.ui.form.on("Supplier", {
	setup: function (frm) {
		...
		frm.set_query("supplier_primary_contact", function (doc) {
			return {
				query: "erpnexus.buying.doctype.supplier.supplier.get_supplier_primary_contact",
				filters: {
					supplier: doc.name,
				},
			};
	...
	refresh: function (frm) {
		if (saashq.defaults.get_default("supp_master_name") != "Naming Series") {
		...
		if (frm.doc.__islocal) {
			...
			frm.add_custom_button(
				__("Accounts Payable"),
				function () {
					saashq.set_route("query-report", "Accounts Payable", {
						party_type: "Supplier",
						party: frm.doc.name,
					});
				},
			...
			frm.add_custom_button(
				__("Bank Account"),
				function () {
					erpnexus.utils.make_bank_account(frm.doc.doctype, frm.doc.name);
				},
				__("Create")
			);
			...
			frm.add_custom_button(
				__("Pricing Rule"),
				function () {
					erpnexus.utils.make_pricing_rule(frm.doc.doctype, frm.doc.name);
				},
				__("Create")
			);
