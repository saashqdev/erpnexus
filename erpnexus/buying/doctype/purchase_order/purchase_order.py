...
from saashq.utils import cint, cstr, flt, get_link_to_form
...
from erpnexus.accounts.party import get_party_account, get_party_account_currency
from erpnexus.buying.utils import check_on_hold_or_closed_status, validate_for_items
...
from erpnexus.stock.doctype.item.item import get_item_defaults, get_last_purchase_details
from erpnexus.stock.stock_balance import get_ordered_qty, update_bin_qty
...
class PurchaseOrder(BuyingController):
	...
	if TYPE_CHECKING:
		...
		from erpnexus.buying.doctype.purchase_order_item.purchase_order_item import PurchaseOrderItem
		from erpnexus.buying.doctype.purchase_order_item_supplied.purchase_order_item_supplied import (
	...
	def validate_with_previous_doc(self):
		...
		if cint(saashq.db.get_single_value("Buying Settings", "maintain_same_rate")):
	...
	def validate_supplier(self):
		prevent_po = saashq.db.get_value("Supplier", self.supplier, "prevent_pos")
		if prevent_po:
			standing = saashq.db.get_value("Supplier Scorecard", self.supplier, "status")
			if standing:
				saashq.throw(
		...
		warn_po = saashq.db.get_value("Supplier", self.supplier, "warn_pos")
		if warn_po:
			standing = saashq.db.get_value("Supplier Scorecard", self.supplier, "status")
			saashq.msgprint(
				_(
	...
	def validate_minimum_order_qty(self):
		...
		itemwise_min_order_qty = saashq._dict(
			saashq.db.sql(
			...
			if not self.is_old_subcontracting_flow:
				for item in self.items:
					if not item.fg_item:
						...
						if not saashq.get_value("Item", item.fg_item, "is_sub_contracted_item"):
							saashq.throw(
								_("Row #{0}: Finished Good Item {1} must be a sub-contracted item").format(
									item.idx, item.fg_item
								)
							)
						elif not item.bom and not saashq.get_value("Item", item.fg_item, "default_bom"):
							saashq.throw(
					...
					if not item.fg_item_qty:
						saashq.throw(_("Row #{0}: Finished Good Item Qty can not be zero").format(item.idx))
		else:
			for item in self.items:
	...
	def get_schedule_dates(self):
		for d in self.get("items"):
			if d.material_request_item and not d.schedule_date:
				d.schedule_date = saashq.db.get_value(
		...
		for d in self.get("items"):
			if d.item_code:
				...
				if last_purchase_details:
					...
					item_last_purchase_rate = saashq.get_cached_value(
	...
	def check_modified_date(self):
		mod_db = saashq.db.sql("select modified from `tabPurchase Order` where name = %s", self.name)
		date_diff = saashq.db.sql(f"select '{mod_db[0][0]}' - '{cstr(self.modified)}' ")
	...
	def update_delivered_qty_in_sales_order(self):
		...
		for so_name in sales_orders_to_update:
			so = saashq.get_doc("Sales Order", so_name)
	...
	def update_ordered_qty_in_so_for_removed_items(self, removed_items):
		...
		for item in removed_items:
			prev_ordered_qty = saashq.get_cached_value(
	...
	def auto_create_subcontracting_order(self):
		if self.is_subcontracted and not self.is_old_subcontracting_flow:
			if saashq.db.get_single_value("Buying Settings", "auto_create_subcontracting_order"):
	...
	def update_subcontracting_order_status(self):
		...
		if self.is_subcontracted and not self.is_old_subcontracting_flow:
			sco = saashq.db.get_value("Subcontracting Order", {"purchase_order": self.name, "docstatus": 1})
...
def item_last_purchase_rate(name, conversion_rate, item_code, conversion_factor=1.0):
	...
	if last_purchase_details:
		...
		item_last_purchase_rate = saashq.get_cached_value("Item", item_code, "last_purchase_rate")
		if item_last_purchase_rate:
...
def close_or_unclose_purchase_orders(names, status):
	if not saashq.has_permission("Purchase Order", "write"):
		saashq.throw(_("Not permitted"), saashq.PermissionError)

	names = json.loads(names)
	for name in names:
		po = saashq.get_doc("Purchase Order", name)
		if po.docstatus == 1:
			if status == "Closed":
...
def make_purchase_invoice_from_portal(purchase_order_name):
	...
	if doc.contact_email != saashq.session.user:
		saashq.throw(_("Not Permitted"), saashq.PermissionError)
	doc.save()
	saashq.db.commit()
	saashq.response["type"] = "redirect"
	saashq.response.location = "/purchase-invoices/" + doc.name
...
def get_mapped_purchase_invoice(source_name, target_doc=None, ignore_permissions=False):
	...
	def update_item(obj, target, source_parent):
		...
		target.cost_center = (
			obj.cost_center
			or saashq.db.get_value("Project", obj.project, "cost_center")
...
def make_subcontracting_order(source_name, target_doc=None, save=False, submit=False, notify=False):
	...
	if (save or submit) and saashq.has_permission(target_doc.doctype, "create"):
		target_doc.save()

		if submit and saashq.has_permission(target_doc.doctype, "submit", target_doc):
			try:
				target_doc.submit()
	...
	def post_process(source_doc, target_doc):
		...
		for idx, item in enumerate(target_doc.items):
			...
			if not target_doc.supplier_warehouse:
				...
				target_doc.supplier_warehouse = saashq.get_cached_value(
...
def is_subcontracting_order_created(po_name) -> bool:
	return (
		True
		if saashq.db.exists("Subcontracting Order", {"purchase_order": po_name, "docstatus": ["=", 1]})
		else False
	)
