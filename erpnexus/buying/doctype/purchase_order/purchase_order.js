...
saashq.provide("erpnexus.buying");
...
erpnexus.accounts.taxes.setup_tax_validations("Purchase Order");
erpnexus.buying.setup_buying_controller();

saashq.ui.form.on("Purchase Order", {
	setup: function (frm) {
	...
	get_materials_from_supplier: function (frm) {
		...
		if (po_details && po_details.length) {
			frm.add_custom_button(
				...
				() => {
					frm.call({
						method: "erpnexus.controllers.subcontracting_controller.get_materials_from_supplier",
						freeze: true,
						...
						callback: function (r) {
							if (r && r.message) {
								const doc = saashq.model.sync(r.message);
								saashq.set_route("Form", doc[0].doctype, doc[0].name);
							}
						},
					});
				},
	...
	onload: function (frm) {
		...
		erpnexus.queries.setup_queries(frm, "Warehouse", function () {
			return erpnexus.queries.warehouse(frm.doc);
		});
	...
	get_subcontracting_boms_for_finished_goods: function (fg_item) {
		return saashq.call({
			method: "erpnexus.subcontracting.doctype.subcontracting_bom.subcontracting_bom.get_subcontracting_boms_for_finished_goods",
			args: {
				fg_items: fg_item,
			},
		});
	...
	get_subcontracting_boms_for_service_item: function (service_item) {
		return saashq.call({
			method: "erpnexus.subcontracting.doctype.subcontracting_bom.subcontracting_bom.get_subcontracting_boms_for_service_item",
			args: {
...
saashq.ui.form.on("Purchase Order Item", {
	schedule_date: function (frm, cdt, cdn) {
		var row = locals[cdt][cdn];
		if (row.schedule_date) {
			if (!frm.doc.schedule_date) {
				erpnexus.utils.copy_value_in_all_rows(frm.doc, cdt, cdn, "items", "schedule_date");
			} else {
				set_schedule_date(frm);
			}
		}
	...
	fg_item: async function (frm, cdt, cdn) {
		if (frm.doc.is_subcontracted && !frm.doc.is_old_subcontracting_flow) {
			...
			if (row.fg_item) {
				...
				if (result.message && Object.keys(result.message).length) {
					saashq.model.set_value(cdt, cdn, "item_code", result.message.service_item);
					...
					saashq.model.set_value(cdt, cdn, "uom", result.message.service_item_uom);
				}
			}
		}
	},
...
erpnexus.buying.PurchaseOrderController = class PurchaseOrderController extends (
	erpnexus.buying.BuyingController
) {
	setup() {
	...
	get_items_from_open_material_requests() {
		erpnexus.utils.map_current_doc({
			method: "erpnexus.stock.doctype.material_request.material_request.make_purchase_order_based_on_supplier",
			args: {
			...
			get_query_method:
				"erpnexus.stock.doctype.material_request.material_request.get_material_requests_based_on_supplier",
		});
	}
	...
	make_stock_entry() {
		saashq.call({
			method: "erpnexus.controllers.subcontracting_controller.make_rm_stock_entry",
			args: {
	...
	make_purchase_receipt() {
		saashq.model.open_mapped_doc({
			method: "erpnexus.buying.doctype.purchase_order.purchase_order.make_purchase_receipt",
			frm: this.frm,
	...
	make_purchase_invoice() {
		saashq.model.open_mapped_doc({
			method: "erpnexus.buying.doctype.purchase_order.purchase_order.make_purchase_invoice",
			frm: this.frm,
		});
	}
	...
	make_subcontracting_order() {
		saashq.model.open_mapped_doc({
			method: "erpnexus.buying.doctype.purchase_order.purchase_order.make_subcontracting_order",
			frm: this.frm,
	...
	add_from_mappers() {
		...
		this.frm.add_custom_button(
			...
			function () {
				erpnexus.utils.map_current_doc({
					method: "erpnexus.stock.doctype.material_request.material_request.make_purchase_order",
					source_doctype: "Material Request",
					target: me.frm,
					setters: {
		...
		this.frm.add_custom_button(
			...
			function () {
				erpnexus.utils.map_current_doc({
					method: "erpnexus.buying.doctype.supplier_quotation.supplier_quotation.make_purchase_order",
		...
		this.frm.add_custom_button(
			...
			function () {
				...
				saashq.call({
					method: "erpnexus.buying.utils.get_linked_material_requests",
					args: {
						items: my_items,
					},
					callback: function (r) {
						...
						while (i < item_length) {
							...
							(r.message[0] || []).forEach(function (d) {
								if (
									...
									if (qty > 0) {
										saashq.msgprint("Splitting " + qty + " units of " + d.item_code);
										var new_row = saashq.model.add_child(
											me.frm.doc,
...
extend_cscript(cur_frm.cscript, new erpnexus.buying.PurchaseOrderController({ frm: cur_frm }));

cur_frm.cscript.update_status = function (label, status) {
	saashq.call({
		method: "erpnexus.buying.doctype.purchase_order.purchase_order.update_status",
...
function set_schedule_date(frm) {
	if (frm.doc.schedule_date) {
		erpnexus.utils.copy_value_in_all_rows(
			frm.doc,
			frm.doc.doctype,
			frm.doc.name,
			"items",
...
saashq.ui.form.on("Purchase Order", "is_subcontracted", function (frm) {
	if (frm.doc.is_old_subcontracting_flow) {
		erpnexus.buying.get_default_bom(frm);
	}
});
