...
from saashq.utils import add_days, add_years, get_last_day, getdate, nowdate

from erpnexus.buying.doctype.supplier_scorecard_period.supplier_scorecard_period import (
...
class SupplierScorecard(Document):
	...
	if TYPE_CHECKING:
		...
		from erpnexus.buying.doctype.supplier_scorecard_scoring_criteria.supplier_scorecard_scoring_criteria import (
			SupplierScorecardScoringCriteria,
		)
		from erpnexus.buying.doctype.supplier_scorecard_scoring_standing.supplier_scorecard_scoring_standing import (
	...
	def calculate_total_score(self):
		...
		for scp in scorecards:
			my_sc = saashq.get_doc("Supplier Scorecard Period", scp.name)
		...
		for standing in self.standings:
			if (not standing.min_grade or (standing.min_grade <= self.supplier_score)) and (
				...
				for fieldname in ("prevent_pos", "prevent_rfqs", "warn_rfqs", "warn_pos"):
					...
					saashq.db.set_value("Supplier", self.supplier, fieldname, self.get(fieldname))


@saashq.whitelist()
def get_timeline_data(doctype, name):
	# Get a list of all the associated scorecards
	scs = saashq.get_doc(doctype, name)
	out = {}
	timeline_data = {}
	scorecards = saashq.db.sql(
		"""
	...
	for sc in scorecards:
		start_date, end_date, total_score = saashq.db.get_value(
...
def refresh_scorecards():
	...
	for sc in scorecards:
		...
		if make_all_scorecards(sc.name) > 0:
			...
			saashq.get_doc("Supplier Scorecard", sc.name).save()


@saashq.whitelist()
def make_all_scorecards(docname):
	sc = saashq.get_doc("Supplier Scorecard", docname)
	supplier = saashq.get_doc("Supplier", sc.supplier)
