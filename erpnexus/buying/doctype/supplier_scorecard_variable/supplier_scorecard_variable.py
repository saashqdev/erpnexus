...
class VariablePathNotFound(saashq.ValidationError):
	pass
	...
	def validate_path_exists(self):
		if "." in self.path:
			try:
				...
				saashq.throw(_("Could not find path for " + self.path), VariablePathNotFound)
...
def get_item_workdays(scorecard):
	"""Gets the number of days in this period"""
	supplier = saashq.get_doc("Supplier", scorecard.supplier)
	total_item_days = saashq.db.sql(
		"""
			SELECT
...
def get_cost_of_on_time_shipments(scorecard):
	...
	total_delivered_on_time_costs = saashq.db.sql(
		"""
			SELECT
...
def get_total_days_late(scorecard):
	...
	supplier = saashq.get_doc("Supplier", scorecard.supplier)
	total_delivered_late_days = saashq.db.sql(
		"""
			SELECT
	...
	total_missed_late_days = saashq.db.sql(
		"""
			SELECT
...
def get_on_time_shipments(scorecard):
	...
	# Look up all PO Items with delivery dates between our dates
	total_items_delivered_on_time = saashq.db.sql(
		"""
			SELECT
				COUNT(pr_item.qty)
			FROM
