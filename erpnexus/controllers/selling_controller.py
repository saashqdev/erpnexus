...
import saashq
from saashq import _, bold, throw
from saashq.utils import cint, flt, get_link_to_form, nowtime

from erpnexus.accounts.party import render_address
from erpnexus.controllers.accounts_controller import get_taxes_and_charges
...
from erpnexus.stock.doctype.item.item import set_item_default
from erpnexus.stock.get_item_details import get_bin_details, get_conversion_factor
from erpnexus.stock.utils import get_incoming_rate, get_valuation_method
...
class SellingController(StockController):
	...
	def set_missing_lead_customer_details(self, for_validate=False):
		...
		if customer:
			from erpnexus.accounts.party import _get_party_details

			fetch_payment_terms_template = False
			if self.get("__islocal") or self.company != saashq.db.get_value(
			...
			from erpnexus.crm.doctype.lead.lead import get_lead_details
	...
	def validate_max_discount(self):
		for d in self.get("items"):
			if d.item_code:
				discount = flt(saashq.get_cached_value("Item", d.item_code, "max_discount"))

				if discount and flt(d.discount_percentage) > discount:
					saashq.throw(_("Maximum discount for Item {0} is {1}%").format(d.item_code, discount))
	...
	def validate_selling_price(self):
		...
		if self.get("is_return") or not saashq.db.get_single_value(
		...
		for item in self.items:
			...
			last_purchase_rate, is_stock_item = saashq.get_cached_value(
		...
		or_conditions = (
			f"""(item_code = {saashq.db.escape(valuation_rate[0])}
			and warehouse = {saashq.db.escape(valuation_rate[1])})"""
			for valuation_rate in valuation_rate_map
		)

		valuation_rates = saashq.db.sql(
			f"""
			select
	...
	def has_product_bundle(self, item_code):
		product_bundle = saashq.qb.DocType("Product Bundle")
		return (
			saashq.qb.from_(product_bundle)
			.select(product_bundle.name)
			.where((product_bundle.new_item_code == item_code) & (product_bundle.disabled == 0))
		).run()

	def get_already_delivered_qty(self, current_docname, so, so_detail):
		delivered_via_dn = saashq.db.sql(
		...
		delivered_via_si = saashq.db.sql(
	...
	def check_sales_order_on_hold_or_close(self, ref_fieldname):
		for d in self.get("items"):
			if d.get(ref_fieldname):
				status = saashq.db.get_value("Sales Order", d.get(ref_fieldname), "status")
				if status in ("Closed", "On Hold") and not self.is_return:
					saashq.throw(_("Sales Order {0} is {1}").format(d.get(ref_fieldname), status))
	...
	def update_reserved_qty(self):
		...
		for so, so_item_rows in so_map.items():
			if so and so_item_rows:
				sales_order = saashq.get_doc("Sales Order", so)
	...
	def set_incoming_rate(self):
		...
		for d in items:
			if not saashq.get_cached_value("Item", d.item_code, "is_stock_item"):
				continue
	...
	def update_stock_ledger(self):
		...
		for d in self.get_item_list():
			if saashq.get_cached_value("Item", d.item_code, "is_stock_item") == 1 and flt(d.qty):
	...
	def get_po_nos(self, ref_doctype, ref_fieldname, po_nos):
		...
		if doc_list:
			po_nos += [
				d.po_no
				for d in saashq.get_all(ref_doctype, "po_no", filters={"name": ("in", doc_list)})
				if d.get("po_no")
			]
	...
	def validate_for_duplicate_items(self):
		...
		if cint(saashq.db.get_single_value("Selling Settings", "allow_multiple_items")):
			return
		...
		for d in self.get("items"):
			...
			duplicate_items_msg = _("Item {0} entered multiple times.").format(saashq.bold(d.item_code))
			duplicate_items_msg += "<br><br>"
			duplicate_items_msg += _("Please enable {} in {} to allow same item in multiple rows").format(
				saashq.bold(_("Allow Item to Be Added Multiple Times in a Transaction")),
				get_link_to_form("Selling Settings", "Selling Settings"),
			)
			if saashq.db.get_value("Item", d.item_code, "is_stock_item") == 1:
				if stock_items in check_list:
					saashq.throw(duplicate_items_msg)
				else:
	...
	def validate_target_warehouse(self):
		...
		for d in items:
			if d.get("target_warehouse") and d.get("warehouse") == d.get("target_warehouse"):
				warehouse = saashq.bold(d.get("target_warehouse"))
				saashq.throw(
					_(
		...
		if not self.get("is_internal_customer") and any(d.get("target_warehouse") for d in items):
			...
			saashq.msgprint(msg, title="Internal Transfer", alert=True)

	def validate_items(self):
		# validate items to see if they have is_sales_item enabled
		from erpnexus.controllers.buying_controller import validate_item_type
...
def get_serial_and_batch_bundle(child, parent):
	...
	if not saashq.db.get_single_value("Stock Settings", "auto_create_serial_and_batch_bundle_for_outward"):
		return

	item_details = saashq.db.get_value("Item", child.item_code, ["has_serial_no", "has_batch_no"], as_dict=1)
