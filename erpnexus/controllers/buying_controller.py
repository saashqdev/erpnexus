...
import saashq
from saashq import ValidationError, _, msgprint
from saashq.contacts.doctype.address.address import render_address
from saashq.utils import cint, flt, getdate
from saashq.utils.data import nowtime

import erpnexus
from erpnexus.accounts.doctype.budget.budget import validate_expense_against_budget
from erpnexus.accounts.party import get_party_details
from erpnexus.buying.utils import update_last_purchase_rate, validate_for_items
from erpnexus.controllers.sales_and_purchase_return import get_rate_for_return
from erpnexus.controllers.subcontracting_controller import SubcontractingController
...
class BuyingController(SubcontractingController):
	...
	def validate(self):
		...
		if getattr(self, "supplier", None) and not self.supplier_name:
			self.supplier_name = saashq.db.get_value("Supplier", self.supplier, "supplier_name")
	...
	def onload(self):
		...
		self.set_onload(
			"backflush_based_on",
			saashq.db.get_single_value("Buying Settings", "backflush_raw_materials_of_subcontract_based_on"),
		)
	...
	def set_supplier_from_item_default(self):
		if self.meta.get_field("supplier") and not self.supplier:
			for d in self.get("items"):
				supplier = saashq.db.get_value(
					"Item Default", {"parent": d.item_code, "company": self.company}, "default_supplier"
				)
				if supplier:
					self.supplier = supplier
				else:
					item_group = saashq.db.get_value("Item", d.item_code, "item_group")
					supplier = saashq.db.get_value(
	...
	def validate_asset_return(self):
		...
		if self.return_against:
			not_cancelled_asset = [
				d.name
				for d in saashq.db.get_all("Asset", {purchase_doc_field: self.return_against, "docstatus": 1})
			]
	...
	def set_incoming_rate(self):
		...
		allow_at_arms_length_price = saashq.get_cached_value(
	...
	def set_sales_incoming_rate_for_internal_transfer(self):
		...
		for d in self.get("items"):
			if not d.get(saashq.scrub(ref_doctype)):
				posting_time = self.get("posting_time")
				if not posting_time:
				...
				d.sales_incoming_rate = flt(
					saashq.db.get_value(ref_doctype, d.get(saashq.scrub(ref_doctype)), field)
	...
	def validate_for_subcontracting(self):
		if self.is_subcontracted and self.get("is_old_subcontracting_flow"):
			if self.doctype in ["Purchase Receipt", "Purchase Invoice"] and not self.supplier_warehouse:
				saashq.throw(_("Supplier Warehouse mandatory for sub-contracted {0}").format(self.doctype))

			for item in self.get("items"):
				if item in self.sub_contracted_items and not item.bom:
					saashq.throw(_("Please select BOM in BOM field for Item {0}").format(item.item_code))
			if self.doctype != "Purchase Order":
				return
			for row in self.get("supplied_items"):
				if not row.reserve_warehouse:
					msg = f"Reserved Warehouse is mandatory for the Item {saashq.bold(row.rm_item_code)} in Raw Materials supplied"
					saashq.throw(_(msg))
		else:
			for item in self.get("items"):
				if item.get("bom"):
	...
	def set_qty_as_per_stock_uom(self):
		allow_to_edit_stock_qty = saashq.db.get_single_value(
		...
		for d in self.get("items"):
			if d.meta.get_field("stock_qty"):
				...
				if not d.conversion_factor and d.item_code:
					saashq.throw(_("Row {0}: Conversion Factor is mandatory").format(d.idx))
	...
	def validate_purchase_return(self):
		for d in self.get("items"):
			if self.is_return and flt(d.rejected_qty) != 0:
				saashq.throw(_("Row #{0}: Rejected Qty can not be entered in Purchase Return").format(d.idx))
	...
	def validate_accepted_rejected_qty(self):
		for d in self.get("items"):
			...
			if flt(val, d.precision("received_qty")) != flt(d.received_qty, d.precision("received_qty")):
				...
				saashq.throw(msg=message, title=_("Mismatch"), exc=QtyMismatchError)
			...
			if flt(item_row[fieldname]) < 0:
				saashq.throw(
					_("Row #{0}: {1} can not be negative for item {2}").format(
						item_row["idx"],
						saashq.get_meta(item_row.doctype).get_label(fieldname),
						item_row["item_code"],
					)
				)
	...
	def check_for_on_hold_or_closed_status(self, ref_doctype, ref_fieldname):
		for d in self.get("items"):
			if d.get(ref_fieldname):
				status = saashq.db.get_value(ref_doctype, d.get(ref_fieldname), "status")
				if status in ("Closed", "On Hold"):
					saashq.throw(_("{0} {1} is {2}").format(ref_doctype, d.get(ref_fieldname), status))
				...
				if pr_qty:
					if d.from_warehouse and (
						...
						if self.is_internal_transfer() and self.is_return and self.docstatus == 2:
							serial_and_batch_bundle = saashq.db.get_value(
								"Stock Ledger Entry",
	...
	def update_ordered_and_reserved_qty(self):
		...
		for po, po_item_rows in po_map.items():
			if po and po_item_rows:
				po_obj = saashq.get_doc("Purchase Order", po)

				if po_obj.status in ["Closed", "Cancelled"]:
					saashq.throw(
						_("{0} {1} is cancelled or closed").format(_("Purchase Order"), po),
						saashq.InvalidStatusError,
					)
	...
	def on_submit(self):
		...
		if self.doctype in [
			...
			"Purchase Receipt",
			"Purchase Invoice",
		] and not saashq.db.get_single_value("Buying Settings", "disable_last_purchase_rate"):
	...
	def auto_make_assets(self, asset_items):
						...
						if len(created_assets) > 5:
							...
							messages.append(
								_("{} Assets created for {}").format(
									len(created_assets), saashq.bold(d.item_code)
								)
							)
						else:
							assets_link = list(
								map(lambda d: saashq.utils.get_link_to_form("Asset", d), created_assets)
							)
							assets_link = saashq.bold(",".join(assets_link))

							is_plural = "s" if len(created_assets) != 1 else ""
							messages.append(
								_("Asset{} {assets_link} created for {}").format(
									is_plural, saashq.bold(d.item_code), assets_link=assets_link
								)
							)
					else:
						saashq.throw(
		...
		for message in messages:
			saashq.msgprint(message, title="Success", indicator="green", alert=alert)

	def make_asset(self, row, is_grouped_asset=False):
		if not row.asset_location:
			saashq.throw(_("Row {0}: Enter location for the asset item {1}").format(row.idx, row.item_code))

		item_data = saashq.get_cached_value(
	...
	def update_fixed_asset(self, field, delete_asset=False):
		for d in self.get("items"):
			if d.is_fixed_asset:
				is_auto_create_enabled = saashq.db.get_value("Item", d.item_code, "auto_create_assets")
				assets = saashq.db.get_all("Asset", filters={field: self.name, "item_code": d.item_code})
				...
				for asset in assets:
					...
					if delete_asset and is_auto_create_enabled:
						...
						for movement in movements:
							saashq.delete_doc("Asset Movement", movement.name, force=1)
						saashq.delete_doc("Asset", asset.name, force=1)
						continue
					...
					if self.docstatus == 2:
						...
						if asset.docstatus == 1 and delete_asset:
							saashq.throw(
								...
								).format(saashq.utils.get_link_to_form("Asset", asset.name))
							)
	...
	def delete_linked_asset(self):
		...
		asset_movement = saashq.db.get_value("Asset Movement", {"reference_name": self.name}, "name")
		saashq.delete_doc("Asset Movement", asset_movement, force=1)
		...
		if self.schedule_date:
			for d in self.get("items"):
				...
				if (
					...
					saashq.throw(_("Row #{0}: Reqd by Date cannot be before Transaction Date").format(d.idx))
		else:
			saashq.throw(_("Please enter Reqd by Date"))
...
def validate_item_type(doc, fieldname, message):
	...
	item_list = ", ".join(["%s" % saashq.db.escape(d) for d in items])
