...
from erpnexus.utilities.product import get_item_codes_by_attributes


class ItemVariantExistsError(saashq.ValidationError):
	pass


class InvalidItemAttributeValueError(saashq.ValidationError):
	pass


class ItemTemplateCannotHaveStock(saashq.ValidationError):
	pass
...
def get_variant(template, args=None, variant=None, manufacturer=None, manufacturer_part_no=None):
	...
	"""
	item_template = saashq.get_doc("Item", template)
	...
	if not attribute_args:
		saashq.throw(_("Please specify at least one attribute in the Attributes table"))
...
def make_variant_based_on_manufacturer(template, manufacturer, manufacturer_part_no):
	...
	if not saashq.db.exists("Item Manufacturer", {"item_code": variant.name, "manufacturer": manufacturer}):
		manufacturer_doc = saashq.new_doc("Item Manufacturer")
...
def validate_is_incremental(numeric_attribute, attribute, value, item):
	...
	if increment == 0:
		...
		saashq.throw(_("Increment for Attribute {0} cannot be 0").format(attribute))
...
def validate_item_attribute_value(attributes_list, attribute, attribute_value, item, from_variant=True):
	allow_rename_attribute_value = saashq.db.get_single_value(
		...
		if from_variant:
			saashq.throw(
				_("{0} is not a valid Value for Attribute {1} of Item {2}.").format(
					saashq.bold(attribute_value), saashq.bold(attribute), saashq.bold(item)
				),
				InvalidItemAttributeValueError,
			...
			msg = _("The value {0} is already assigned to an existing Item {1}.").format(
				saashq.bold(attribute_value), saashq.bold(item)
			)
			msg += "<br>" + _(
				"To still proceed with editing this Attribute Value, enable {0} in Item Variant Settings."
			).format(saashq.bold(_("Allow Rename Attribute Value")))

			saashq.throw(msg, InvalidItemAttributeValueError, title=_("Edit Not Allowed"))


def get_attribute_values(item):
	if not saashq.flags.attribute_values:
		attribute_values = {}
		numeric_values = {}
		for t in saashq.get_all("Item Attribute Value", fields=["parent", "attribute_value"]):
			attribute_values.setdefault(t.parent.lower(), []).append(t.attribute_value)

		for t in saashq.get_all(
		...
		saashq.flags.attribute_values = attribute_values
		saashq.flags.numeric_values = numeric_values

	return saashq.flags.attribute_values, saashq.flags.numeric_values
...
def find_variant(template, args, variant_item_code=None):
	...
	for variant in possible_variants:
		variant = saashq.get_doc("Item", variant)
...
def create_variant(item, args, use_template_image=False):
	use_template_image = saashq.parse_json(use_template_image)
	if isinstance(args, str):
		args = json.loads(args)

	template = saashq.get_doc("Item", item)
	variant = saashq.new_doc("Item")
...
def enqueue_multiple_variant_creation(item, args, use_template_image=False):
	...
	if total_variants >= 600:
		saashq.throw(_("Please do not create more than 500 items at a time"))
		return
	if total_variants < 10:
		return create_multiple_variants(item, args, use_template_image)
	else:
		saashq.enqueue(
			"erpnexus.controllers.item_variant.create_multiple_variants",
			item=item,
			args=args,
...
def create_multiple_variants(item, args, use_template_image=False):
	...
	template_item = saashq.get_doc("Item", item)
...
def copy_attributes_to_variant(item, variant):
	...
	allow_fields = [d.field_name for d in saashq.get_all("Variant Field", fields=["field_name"])]
...
def make_variant_item_code(template_item_code, template_item_name, variant):
	...
	for attr in variant.attributes:
		...
		if not item_attribute:
			continue
			# saashq.throw(_('Invalid attribute {0} {1}').format(saashq.bold(attr.attribute),
			# 	saashq.bold(attr.attribute_value)), title=_('Invalid Attribute'),
...
def create_variant_doc_for_quick_entry(template, args):
	variant_based_on = saashq.db.get_value("Item", template, "variant_based_on")
	args = json.loads(args)
