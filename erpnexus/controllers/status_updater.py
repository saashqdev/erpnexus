...
from saashq.utils import comma_or, flt, get_link_to_form, getdate, now, nowdate


class OverAllowanceError(saashq.ValidationError):
	pass


def validate_status(status, options):
	if status not in options:
		saashq.throw(_("Status must be one of {0}").format(comma_or(options)))
...
class StatusUpdater(Document):
		...
		for args in self.status_updater:
			...
			for d in self.get_all_children():
				if hasattr(d, "qty") and d.qty < 0 and not self.get("is_return"):
					saashq.throw(_("For an item {0}, quantity must be positive number").format(d.item_code))

				if hasattr(d, "qty") and d.qty > 0 and self.get("is_return"):
					saashq.throw(_("For an item {0}, quantity must be negative number").format(d.item_code))

				if not saashq.db.get_single_value("Selling Settings", "allow_negative_rates_for_items"):
					if hasattr(d, "item_code") and hasattr(d, "rate") and flt(d.rate) < 0:
						saashq.throw(
							_(
								...
								saashq.bold(_("`Allow Negative rates for Items`")),
	...
	def check_overflow_with_allowance(self, item, args):
		...
		role_allowed_to_over_deliver_receive = saashq.db.get_single_value(
			"Stock Settings", "role_allowed_to_over_deliver_receive"
		)
		role_allowed_to_over_bill = saashq.db.get_single_value(
		...
		if overflow_percent - allowance > 0.01:
			...
			if role not in saashq.get_roles():
	...
	def limits_crossed_error(self, args, item, qty_or_amount):
		...
		saashq.throw(
			_(
				...
				saashq.bold(_(item["target_ref_field"].title())),
				saashq.bold(item["reduce_by"]),
				saashq.bold(_(args.get("target_dt"))),
				saashq.bold(_(self.doctype)),
				saashq.bold(item.get("item_code")),
			)
			+ "<br><br>"
			+ action_msg,
			OverAllowanceError,
	...
	def _update_children(self, args, update_modified):
		...
		for d in self.get_all_children():
			...
			if (
				...
				args["second_source_condition"] = saashq.db.sql(
	...
	def _update_percent_field(self, args, update_modified=True):
		...
		if update_data:
			target = saashq.get_doc(args["target_parent_dt"], args["name"])
	...
	def _update_modified(self, args, update_modified):
		...
		args["update_modified"] = ", modified = {}, modified_by = {}".format(
			saashq.db.escape(now()), saashq.db.escape(saashq.session.user)
		)

	def update_billing_status_for_zero_amount_refdoc(self, ref_dt):
		ref_fieldname = saashq.scrub(ref_dt)

		ref_docs = [item.get(ref_fieldname) for item in (self.get("items") or []) if item.get(ref_fieldname)]
		if not ref_docs:
			return

		zero_amount_refdocs = saashq.db.sql_list(
			f"""
			SELECT
				name
			from
	...
	def update_billing_status(self, zero_amount_refdoc, ref_dt, ref_fieldname):
		for ref_dn in zero_amount_refdoc:
			...
			ref_doc = saashq.get_doc(ref_dt, ref_dn)
...
def get_allowance_for(
	...
	if item_allowance is None:
		item_allowance = {}
	if qty_or_amount == "qty":
		if item_allowance.get(item_code, saashq._dict()).get("qty"):
			return (
		...
		if item_allowance.get(item_code, saashq._dict()).get("amount"):
			return (
	...
	qty_allowance, over_billing_allowance = saashq.get_cached_value(
	...
	if qty_or_amount == "qty" and not qty_allowance:
		if global_qty_allowance is None:
			global_qty_allowance = flt(
				saashq.get_cached_value("Stock Settings", None, "over_delivery_receipt_allowance")
			)
		qty_allowance = global_qty_allowance
	elif qty_or_amount == "amount" and not over_billing_allowance:
		if global_amount_allowance is None:
			global_amount_allowance = flt(
				saashq.get_cached_value("Accounts Settings", None, "over_billing_allowance")
			)
		over_billing_allowance = global_amount_allowance

	if qty_or_amount == "qty":
		allowance = qty_allowance
		item_allowance.setdefault(item_code, saashq._dict()).setdefault("qty", qty_allowance)
	else:
		allowance = over_billing_allowance
		item_allowance.setdefault(item_code, saashq._dict()).setdefault("amount", over_billing_allowance)
