...
from saashq.utils import add_days, getdate, nowdate
...
from erpnexus.buying.doctype.purchase_order.test_purchase_order import prepare_data_for_internal_transfer
...
class TestAccountsController(IntegrationTestCase):
	...
	def create_company(self):
		...
		if saashq.db.exists("Company", company_name):
			company = saashq.get_doc("Company", company_name)
		else:
	...
	def create_account(self):
		...
		for x in accounts:
			if not saashq.db.get_value("Account", filters={"account_name": x.name, "company": self.company}):
	...
	def setup_advance_accounts_in_party_master(self):
		company = saashq.get_doc("Company", self.company)
		...
		supplier = saashq.get_doc("Supplier", self.supplier)
		supplier.append(
			"accounts",
			{
				"company": self.company,
	...
	def test_15_gain_loss_on_different_posting_date(self):
		...
		self.assertEqual(
			saashq.db.get_value("Journal Entry", exc_je_for_si[0].parent, "posting_date"),
	...
	def test_16_internal_transfer_at_arms_length_price(self):
		...
		saashq.db.set_single_value("Stock Settings", "allow_internal_transfer_at_arms_length_price", 0)
		si.items[0].rate = arms_length_price
		si.save()
	...
	def test_40_cost_center_from_payment_entry(self):
		...
		cc = saashq.db.get_value("Company", self.company, "cost_center")
		saashq.db.set_value("Company", self.company, "cost_center", None)
		...
		saashq.db.set_value("Company", self.company, "cost_center", cc)
	...
	def setup_dimensions(self):
		...
		loc = saashq.get_doc("Accounting Dimension", "Location")
