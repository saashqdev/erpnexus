...
from saashq.utils import cint, cstr, flt, get_link_to_form, getdate
...
from erpnexus.accounts.utils import cancel_exchange_gain_loss_journal, get_fiscal_year
...
from erpnexus.stock.doctype.inventory_dimension.inventory_dimension import (
	get_evaluated_inventory_dimension,
)
from erpnexus.stock.doctype.serial_and_batch_bundle.serial_and_batch_bundle import (
	get_type_of_transaction,
)
from erpnexus.stock.stock_ledger import get_items_to_be_repost


class QualityInspectionRequiredError(saashq.ValidationError):
	pass


class QualityInspectionRejectedError(saashq.ValidationError):
	pass


class QualityInspectionNotSubmittedError(saashq.ValidationError):
	pass


class BatchExpiredError(saashq.ValidationError):
	pass
...
class StockController(AccountsController):
	...
	def validate_items_exist(self):
		...
		exists_items = saashq.get_all("Item", filters={"name": ("in", items)}, pluck="name")
		non_exists_items = set(items) - set(exists_items)

		if non_exists_items:
			saashq.throw(_("Items {0} do not exist in the Item master.").format(", ".join(non_exists_items)))
			...
			if data:
				...
				saashq.throw(
					_("Serial and Batch Bundle {0} is already used in {1} {2}.").format(
						saashq.bold(data.serial_and_batch_bundle), data.voucher_type, data.voucher_no
					)
				)
	...
	def make_gl_entries(self, gl_entries=None, from_repost=False, via_landed_cost_voucher=False):
		...
		if (
			cint(erpnexus.is_perpetual_inventory_enabled(self.company))
	...
	def validate_serialized_batch(self):
		...
		for d in self.get("items"):
			...
			if flt(d.qty) > 0.0 and d.get("batch_no") and self.get("posting_date") and self.docstatus < 2:
				expiry_date = saashq.get_cached_value("Batch", d.get("batch_no"), "expiry_date")
	...
	def clean_serial_nos(self):
		from erpnexus.stock.doctype.serial_no.serial_no import clean_serial_no_string
	...
	def make_bundle_using_old_serial_batch_fields(self, table_name=None, via_landed_cost_voucher=False):
		...
		if saashq.flags.in_test and saashq.flags.use_serial_and_batch_fields:
			return
				...
				if not data.get("qty"):
					saashq.throw(
						_("For the {0}, no stock is available for the return in the warehouse {1}.").format(
							saashq.bold(row.item_code), row.get(warehouse_field)
						)
					)
		...
		for row in self.get(table_name):
			if data := available_dict.get(row.get(field)):
				...
				if self.doctype in ["Sales Invoice", "Delivery Note"]:
					row.db_set(
						"incoming_rate", saashq.db.get_value("Serial and Batch Bundle", bundle, "avg_rate")
					)
	...
	def is_serial_batch_item(self, item_code) -> bool:
		if not saashq.db.exists("Item", item_code):
			saashq.throw(_("Item {0} does not exist.").format(bold(item_code)))

		item_details = saashq.db.get_value("Item", item_code, ["has_serial_no", "has_batch_no"], as_dict=1)
		...
		bundle_details.update(
			{
				...
				"warehouse": warehouse,
				"batches": saashq._dict({row.batch_no: qty}) if row.batch_no else None,
	...
	def set_use_serial_batch_fields(self):
		if saashq.db.get_single_value("Stock Settings", "use_serial_batch_fields"):
	...
	def get_gl_entries(self, warehouse_account=None, default_expense_account=None, default_cost_center=None):
		...
		for item_row in voucher_details:
			...
			if abs(sle_rounding_diff) > (1.0 / (10**precision)) and self.is_internal_transfer():
				...
				expense_account = saashq.get_cached_value("Company", self.company, "default_expense_account")
		...
		if warehouse_with_no_account:
			for wh in warehouse_with_no_account:
				if saashq.get_cached_value("Warehouse", wh, "company"):
					saashq.throw(
						_(
	...
	def get_debit_field_precision(self):
		if not saashq.flags.debit_field_precision:
			saashq.flags.debit_field_precision = saashq.get_precision("GL Entry", "debit_in_account_currency")

		return saashq.flags.debit_field_precision

	def get_voucher_details(self, default_expense_account, default_cost_center, sle_map):
		if self.doctype == "Stock Reconciliation":
			reconciliation_purpose = saashq.db.get_value(self.doctype, self.name, "purpose")
	...
	def get_stock_ledger_details(self):
		stock_ledger = {}
		stock_ledger_entries = saashq.db.sql(
			"""
			select
		...
		if not item.get("expense_account"):
			...
			saashq.throw(
				_("Row #{0}: Expense Account not set for the Item {1}. {2}").format(
					item.idx, saashq.bold(item.item_code), msg
				),
			...
			is_expense_account = (
				saashq.get_cached_value("Account", item.get("expense_account"), "report_type")
				== "Profit and Loss"
			)
			if (
				self.doctype
	...
	def set_serial_and_batch_bundle(self, table_name=None, ignore_validate=False):
		...
		for row in self.get(table_name):
			for field in QTY_FIELD.keys():
				if row.get(field):
					saashq.get_doc("Serial and Batch Bundle", row.get(field)).set_serial_and_batch_values(
		...
		sl_dict = saashq._dict(
			{
				...
				"stock_uom": saashq.get_cached_value(
			...
			if not sl_dict.get(dimension.target_fieldname) and dimension.fetch_from_parent:
				...
				if not sl_dict.get(dimension.target_fieldname):
					fieldname = next(
						(
							field.fieldname
							for field in saashq.get_meta(self.doctype).fields
	...
	def make_sl_entries(self, sl_entries, allow_negative_stock=False, via_landed_cost_voucher=False):
		from erpnexus.stock.stock_ledger import make_sl_entries
	...
	def get_serialized_items(self):
		...
		if item_codes:
			serialized_items = saashq.db.sql_list(
	...
	def validate_inspection(self):
		...
		for row in self.get("items"):
			qi_required = False
			if inspection_required_fieldname and saashq.db.get_value(
	...
	def validate_qi_presence(self, row):
		...
		if not row.quality_inspection:
			msg = f"Row #{row.idx}: Quality Inspection is required for Item {saashq.bold(row.item_code)}"
			if self.docstatus == 1:
				saashq.throw(_(msg), title=_("Inspection Required"), exc=QualityInspectionRequiredError)
			else:
				saashq.msgprint(_(msg), title=_("Inspection Required"), indicator="blue")

	def validate_qi_submission(self, row):
		"""Check if QI is submitted on row level, during submission"""
		action = saashq.db.get_single_value("Stock Settings", "action_if_quality_inspection_is_not_submitted")
		qa_docstatus = saashq.db.get_value("Quality Inspection", row.quality_inspection, "docstatus")

		if qa_docstatus != 1:
			link = saashq.utils.get_link_to_form("Quality Inspection", row.quality_inspection)
			msg = f"Row #{row.idx}: Quality Inspection {link} is not submitted for the item: {row.item_code}"
			if action == "Stop":
				saashq.throw(_(msg), title=_("Inspection Submission"), exc=QualityInspectionNotSubmittedError)
			else:
				saashq.msgprint(_(msg), alert=True, indicator="orange")

	def validate_qi_rejection(self, row):
		"""Check if QI is rejected on row level, during submission"""
		action = saashq.db.get_single_value("Stock Settings", "action_if_quality_inspection_is_rejected")
		qa_status = saashq.db.get_value("Quality Inspection", row.quality_inspection, "status")

		if qa_status == "Rejected":
			link = saashq.utils.get_link_to_form("Quality Inspection", row.quality_inspection)
			msg = f"Row #{row.idx}: Quality Inspection {link} was rejected for item {row.item_code}"
			if action == "Stop":
				saashq.throw(_(msg), title=_("Inspection Rejected"), exc=QualityInspectionRejectedError)
			else:
				saashq.msgprint(_(msg), alert=True, indicator="orange")
	...
	def update_blanket_order(self):
		...
		for blanket_order in blanket_orders:
			saashq.get_doc("Blanket Order", blanket_order).update_ordered_qty()

	def validate_customer_provided_item(self):
		for d in self.get("items"):
			# Customer Provided parts will have zero valuation rate
			if saashq.get_cached_value("Item", d.item_code, "is_customer_provided_item"):
	...
	def validate_multi_currency(self):
		if self.currency != self.company_currency:
			saashq.throw(_("Internal transfers can only be done in company's default currency"))

	def validate_packed_items(self):
		if self.doctype in ("Sales Invoice", "Delivery Note Item") and self.get("packed_items"):
			saashq.throw(_("Packed Items cannot be transferred internally"))
	...
	def validate_internal_transfer_qty(self):
		...
		precision = saashq.get_precision(self.doctype + " Item", "qty")

		over_receipt_allowance = saashq.db.get_single_value(
	...
	def get_item_wise_inter_transfer_qty(self):
		...
		parent_tab = saashq.qb.DocType(parent_doctype)
		child_tab = saashq.qb.DocType(child_doctype)

		query = (
			saashq.qb.from_(parent_doctype)
	...
	def get_item_wise_inter_received_qty(self):
		...
		parent_tab = saashq.qb.DocType(self.doctype)
		child_tab = saashq.qb.DocType(child_doctype)

		query = (
			saashq.qb.from_(self.doctype)
			.inner_join(child_tab)
			.on(child_tab.parent == parent_tab.name)
			.select(
				child_tab.item_code,
	...
	def validate_putaway_capacity(self):
		...
		from erpnexus.stock.doctype.putaway_rule.putaway_rule import get_available_putaway_capacity
		...
		if not saashq.get_all("Putaway Rule", limit=1):
			return
		...
		if valid_doctype:
			...
			for rule, values in rule_map.items():
				if flt(values["qty_put"]) > flt(values["capacity"]):
					...
					saashq.throw(msg=message, title=_("Over Receipt"))

	def prepare_over_receipt_message(self, rule, values):
		message = _("{0} qty of Item {1} is being received into Warehouse {2} with capacity {3}.").format(
			saashq.bold(values["qty_put"]),
			saashq.bold(values["item"]),
			saashq.bold(values["warehouse"]),
			saashq.bold(values["capacity"]),
		)
		message += "<br><br>"
		rule_link = saashq.utils.get_link_to_form("Putaway Rule", rule)
	...
	def repost_future_sle_and_gle(self, force=False, via_landed_cost_voucher=False):
		...
		if force or future_sle_exists(args) or repost_required_for_queue(self):
			item_based_reposting = cint(
				saashq.db.get_single_value("Stock Reposting Settings", "item_based_reposting")
			)
...
def show_accounting_ledger_preview(company, doctype, docname):
	filters = saashq._dict(company=company, include_dimensions=1)
...
def show_stock_ledger_preview(company, doctype, docname):
	filters = saashq._dict(company=company)
	doc = saashq.get_doc(doctype, docname)
...
def get_accounting_ledger_preview(doc, filters):
	from erpnexus.accounts.report.general_ledger.general_ledger import get_columns as get_gl_columns
...
def get_stock_ledger_preview(doc, filters):
	from erpnexus.stock.report.stock_ledger.stock_ledger import get_columns as get_sl_columns
...
def get_gl_entries_for_preview(doctype, docname, fields):
	return saashq.get_all("GL Entry", filters={"voucher_type": doctype, "voucher_no": docname}, fields=fields)
...
def repost_required_for_queue(doc: StockController) -> bool:
	...
	consuming_sles = saashq.db.get_all(
		"Stock Ledger Entry",
		filters={
...
def make_quality_inspections(doctype, docname, items, inspection_type):
	...
	for item in items:
		...
		quality_inspection = saashq.get_doc(
			{
				"doctype": "Quality Inspection",
				"inspection_type": inspection_type,
				"inspected_by": saashq.session.user,
...
def future_sle_exists(args, sl_entries=None, allow_force_reposting=True):
	if allow_force_reposting and saashq.db.get_single_value(
		"Stock Reposting Settings", "do_reposting_for_each_stock_transaction"
	):
		return True

	key = (args.voucher_type, args.voucher_no)
	if not hasattr(saashq.local, "future_sle"):
		saashq.local.future_sle = {}
	...
	for d in data:
		saashq.local.future_sle[key][(d.item_code, d.warehouse)] = d.total_row
...
def validate_future_sle_not_exists(args, key, sl_entries=None):
	...
	if not sl_entries and hasattr(saashq.local, "future_sle"):
		if key not in saashq.local.future_sle:
			return False

		if not saashq.local.future_sle.get(key) or (
			item_key and item_key not in saashq.local.future_sle.get(key)
		):
			return True
...
def get_cached_data(args, key):
	if key not in saashq.local.future_sle:
		saashq.local.future_sle[key] = saashq._dict({})

	if args.get("item_code"):
		item_key = (args.get("item_code"), args.get("warehouse"))
		count = saashq.local.future_sle[key].get(item_key)

		return True if (count or count == 0) else False
	else:
		return saashq.local.future_sle[key]
...
def get_conditions_to_validate_future_sle(sl_entries):
	...
	for warehouse, items in warehouse_items_map.items():
		or_conditions.append(
			f"""warehouse = {saashq.db.escape(warehouse)}
				and item_code in ({', '.join(saashq.db.escape(item) for item in items)})"""
		)
...
def create_repost_item_valuation_entry(args):
	...
	repost_entry = saashq.new_doc("Repost Item Valuation")
...
def make_bundle_for_material_transfer(**kwargs):
	...
	bundle_doc = saashq.get_doc("Serial and Batch Bundle", kwargs.serial_and_batch_bundle)

	if not kwargs.type_of_transaction:
		kwargs.type_of_transaction = "Inward"

	bundle_doc = saashq.copy_doc(bundle_doc)
