...
from saashq.utils import flt, has_common
from saashq.utils.user import is_website_user


def get_list_context(context=None):
	return {
		"global_number_format": saashq.db.get_default("number_format") or "#,###.##",
...
def get_webform_list_context(module):
	if get_module_app(module) != "erpnexus":
		return
	...
	if txt:
		if meta.get_field("items"):
			if meta.get_field("items").options:
				...
				for item in saashq.get_all(child_doctype, {"item_name": ["like", "%" + txt + "%"]}):
					child = saashq.get_doc(child_doctype, item.name)
					or_filters.append([doctype, "name", "=", child.parent])

	if or_filters:
		for r in saashq.get_list(
			doctype,
			fields=fields,
			filters=filters,
...
def get_customers_suppliers(doctype, user):
	...
	if has_common(["Supplier", "Customer"], saashq.get_roles(user)):
		suppliers = get_parents_for_user("Supplier")
		customers = get_parents_for_user("Customer")
	elif saashq.has_permission(doctype, "read", user=user):
		customer_list = saashq.get_list("Customer")
...
def get_parents_for_user(parenttype: str) -> list[str]:
	portal_user = saashq.qb.DocType("Portal User")

	return (
		saashq.qb.from_(portal_user)
		.select(portal_user.parent)
		.where(portal_user.user == saashq.session.user)
...
def has_website_permission(doc, ptype, user, verbose=False):
	...
	if customers:
		return saashq.db.exists(doctype, get_customer_filter(doc, customers))
	elif suppliers:
		fieldname = "suppliers" if doctype == "Request for Quotation" else "supplier"
		return saashq.db.exists(doctype, {"name": doc.name, fieldname: ["in", suppliers]})
	else:
		return False
...
def add_role_for_portal_user(portal_user, role):
	...
	saashq.msgprint(_("Added {1} Role to User {0}.").format(saashq.bold(user_doc.name), role), alert=True)
