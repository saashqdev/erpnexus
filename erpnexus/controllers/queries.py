...
import saashq
from saashq import qb, scrub
from saashq.desk.reportview import get_filters_cond, get_match_cond
from saashq.query_builder import Criterion, CustomFunction
from saashq.query_builder.functions import Concat, Locate, Sum
from saashq.utils import nowdate, today, unique
from pypika import Order

import erpnexus
from erpnexus.stock.get_item_details import _get_item_tax_template
...
def lead_query(doctype, txt, searchfield, start, page_len, filters):
	...
	searchfields = saashq.get_meta(doctype).get_search_fields()
...
def tax_account_query(doctype, txt, searchfield, start, page_len, filters):
	doctype = "Account"
	company_currency = erpnexus.get_company_currency(filters.get("company"))
...
def item_query(doctype, txt, searchfield, start, page_len, filters, as_dict=False):
	...
	meta = saashq.get_meta(doctype, cached=True)
	...
	if filters and isinstance(filters, dict):
		if filters.get("customer") or filters.get("supplier"):
			...
			item_rules_list = saashq.get_all(
				"Party Specific Item",
				filters={"party": party},
	...
	description_cond = ""
	if saashq.db.count(doctype, cache=True) < 50000:
...
def get_project_name(doctype, txt, searchfield, start, page_len, filters):
	...
	searchfields = [
		x for x in saashq.get_meta(doctype).get_search_fields() if x not in ["customer", "status"]
	]
...
def get_batches_from_serial_and_batch_bundle(searchfields, txt, filters, start=0, page_len=100):
	bundle = saashq.qb.DocType("Serial and Batch Entry")
	stock_ledger_entry = saashq.qb.DocType("Stock Ledger Entry")
	batch_table = saashq.qb.DocType("Batch")
...
def get_account_list(doctype, txt, searchfield, start, page_len, filters):
	...
	return saashq.desk.reportview.execute(
		doctype,
		filters=filter_list,
...
def get_blanket_orders(doctype, txt, searchfield, start, page_len, filters):
	return saashq.db.sql(
		...
		where
			...
			item_code=saashq.db.escape(filters.get("item")),
			blanket_order_type=filters.get("blanket_order_type"),
			company=saashq.db.escape(filters.get("company")),
		)
	)
...
def warehouse_query(doctype, txt, searchfield, start, page_len, filters):
	...
	meta = saashq.get_meta("Warehouse")
	...
	query = """select `tabWarehouse`.`{warehouse_field}`,
		...
		start=start,
		page_len=page_len,
		txt=saashq.db.escape(f"%{txt}%"),
	)

	return saashq.db.sql(query)
...
def get_batch_numbers(doctype, txt, searchfield, start, page_len, filters):
	query = """select batch_id from `tabBatch`
			...
			and name like {txt}""".format(txt=saashq.db.escape(f"%{txt}%"))

	if filters and filters.get("item"):
		query += " and item = {item}".format(item=saashq.db.escape(filters.get("item")))

	return saashq.db.sql(query, filters)
...
def item_manufacturer_query(doctype, txt, searchfield, start, page_len, filters):
	...
	item_manufacturers = saashq.get_all(
		"Item Manufacturer",
...
def get_purchase_receipts(doctype, txt, searchfield, start, page_len, filters):
	query = """
		...
		and pr.name like {txt}""".format(txt=saashq.db.escape(f"%{txt}%"))

	if filters and filters.get("item_code"):
		query += " and pritem.item_code = {item_code}".format(
			item_code=saashq.db.escape(filters.get("item_code"))
		)
...
def get_purchase_invoices(doctype, txt, searchfield, start, page_len, filters):
	query = """
		...
		and pi.name like {txt}""".format(txt=saashq.db.escape(f"%{txt}%"))
...
def get_doctypes_for_closing(doctype, txt, searchfield, start, page_len, filters):
	doctypes = saashq.get_hooks("period_closing_doctypes")
	if txt:
...
def get_tax_template(doctype, txt, searchfield, start, page_len, filters):
	item_doc = saashq.get_cached_doc("Item", filters.get("item_code"))
	...
	if not taxes:
		return saashq.get_all("Item Tax Template", filters={"disabled": 0, "company": company}, as_list=True)
	else:
