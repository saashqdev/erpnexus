...
from saashq.utils import cint, flt, round_based_on_smallest_currency_fraction
from saashq.utils.deprecations import deprecated

import erpnexus
from erpnexus.accounts.doctype.journal_entry.journal_entry import get_exchange_rate
from erpnexus.accounts.doctype.pricing_rule.utils import get_applied_pricing_rules
...
class calculate_taxes_and_totals:
	def __init__(self, doc: Document):
		...
		saashq.flags.round_row_wise_tax = saashq.db.get_single_value(
			"Accounts Settings", "round_row_wise_tax"
		)

		self._items = self.filter_rows() if self.doc.doctype == "Quotation" else self.doc.get("items")

		get_round_off_applicable_accounts(self.doc.company, saashq.flags.round_off_applicable_accounts)
		self.calculate()
	...
	def validate_conversion_rate(self):
		...
		company_currency = erpnexus.get_company_currency(self.doc.company)
	...
	def calculate_shipping_charges(self):
		...
		if hasattr(self.doc, "shipping_rule") and self.doc.shipping_rule:
			shipping_rule = saashq.get_doc("Shipping Rule", self.doc.shipping_rule)
	...
	def round_off_totals(self, tax):
		if tax.account_head in saashq.flags.round_off_applicable_accounts:
	...
	def apply_discount_amount(self):
		if self.doc.discount_amount:
			if not self.doc.apply_discount_on:
				saashq.throw(_("Please select Apply Discount On"))
	...
	def calculate_outstanding_amount(self):
		...
		if self.doc.doctype in ["Sales Invoice", "Purchase Invoice"]:
			...
			if (
				...
				write_off_limit = flt(
					saashq.db.get_value("POS Profile", self.doc.pos_profile, "write_off_limit")
				)
	...
	def set_total_amount_to_default_mop(self, total_amount_to_pay):
		...
		if pending_amount > 0:
			default_mode_of_payment = saashq.db.get_value(
				"POS Payment Method",
	...
	for tax in taxes:
		...
		if item_tax_map:
			for item_code, tax_data in item_tax_map.items():
				itemised_tax.setdefault(item_code, saashq._dict())
				...
				itemised_tax[item_code][tax.description] = saashq._dict(
...
def get_itemised_taxable_amount(items):
	itemised_taxable_amount = saashq._dict()
	for item in items:
...
def get_rounding_tax_settings():
	return saashq.db.get_single_value("Accounts Settings", "round_row_wise_tax")
	...
	def set_account_currency(self):
		...
		for d in self.doc.get(self.tax_field):
			if not d.account_currency:
				account_currency = saashq.get_cached_value("Account", d.expense_account, "account_currency")
